...
#define FTM_SNPRINTF(buf,len,fmt,...)  do{\
                                       if(snprintf((char *)(buf), len, fmt,##__VA_ARGS__) < 0){\
                                           MND_ERR("snprintf error occurred");\
                                       }\
                                     }while(0)
static int gps_strtol_10(char* str) {
    char* endptr;
    long val;
    ...
    if ((errno == ERANGE && (val == LONG_MAX || val == LONG_MIN)) ||
        (errno != 0)) {
        LOGE("strtol fail, errno=%d, val=%ld, %s",errno, val, strerror(errno));
    }
...
static double gps_strtod(char* str) {
    char* endptr;
    double val;
    ...
    if ((errno == ERANGE && (val == HUGE_VAL || val == -HUGE_VAL || val == 0)) ||
        (errno != 0)) {
        LOGE("strtol fail, errno=%d, val=%f, %s",errno, val, strerror(errno));
    }
...
static int gps_ttff_check_res()
{
    ...
    if(!gps_l5_support()) { 
        ...
        // 打印日志：显示当前是 L1 -only 模式，并输出 L1 的检查结果
        LOGD(TAG"L1 only:%d", ttff_check_res);
        
        // 直接返回 L1 的测试结果
        // ttff_check_res: 通常为 1 (成功/已定位) 或 0 (失败/未定位)
        return ttff_check_res;
    } else {
        // 情况 B: 支持 L5 (双频 GPS)
        
        // 打印日志：分别显示 L1 和 L5 的检查结果
        // 例如：L1 SV check:1, L5 SV check:1 表示两个频段都搜到了卫星
        LOGD(TAG"L1 SV check:%d, L5 SV check:%d", ttff_check_res, ttff_l5_check_res);
        
        // 返回逻辑与运算结果
        // 只有当 L1 成功 (ttff_check_res == 1) 且 L5 也成功 (ttff_l5_check_res == 1) 时，才返回 1 (真)
        // 只要有一个频段失败，整体测试就判定为失败
...
void factory_request_gps(bool turnOn) {
    ...
    while (factory_safe_sendto(FACTORY_TO_MNLD_SOCKET, _buff, _offset) == -1) {
        retry --;
        MND_MSG("FACTORY_safe_sendto failed!, retry = %d", retry);
        if (retry <= 0) {
            break;
        }
        sleep(1);
   ...
   if (CheckSum(head, strlen(head))) {
      ...
      if (FetchField(start, result)) {
         ...
         if (point != NULL){
             ...
             tmp[len]='\0';
             g_gpsInfo.Lat = (float)(gps_strtod(tmp));
             strncpy(tmp, result+len, strlen(result)-len);
             tmp[strlen(result)-len]='\0';
             g_gpsInfo.Lat += (float)(gps_strtod(tmp)/60.0);
         }
      }
      ...
      if (FetchField(start, result)) {
         ...
         if (point != NULL){
             ...
             strncpy(tmp, result, len);
             tmp[len]='\0';
             g_gpsInfo.Lon = (float)(gps_strtod(tmp));
             strncpy(tmp, result+len, strlen(result)-len);
             tmp[strlen(result)-len]='\0';
             g_gpsInfo.Lon += (float)(gps_strtod(tmp)/60.0);
         }
      }
      ...
      if (FetchField(start, result)) {
         ...
         tmp[2] = '\0';
         g_gpsInfo.hour = gps_strtol_10(tmp);
         // Min
         strncpy(tmp, result+2, 2);
         tmp[2] = '\0';
         g_gpsInfo.min = gps_strtol_10(tmp);
         // Sec
         strncpy(tmp, result+4, strlen(result)-4);
         tmp[strlen(result)-4] = '\0';
         g_gpsInfo.sec = (float)(gps_strtod(tmp));
      }
...
void RMC_Parse( char *head)
{
   ...
   if (CheckSum(head, strlen(head))) {
      ...
      if (FetchField(start, tmp)) {
         ...
         result[2]='\0';
         g_gpsInfo.hour = gps_strtol_10(result);
         // Min
         strncpy( result, tmp+2, 2);
         result[2]='\0';
         g_gpsInfo.min = gps_strtol_10(result);
         // Sec
         strncpy(result, tmp+4, strlen(tmp)-4);
         result[strlen(tmp)-4]='\0';
         g_gpsInfo.sec = (float)(gps_strtod(result));
      }
      ...
      if (FetchField(start, result)) {
         ...
         if (point != NULL){
             ...
             strncpy(tmp, result, len);
             tmp[len]='\0';
             g_gpsInfo.Lat = (float)(gps_strtol_10(tmp));
             strncpy(tmp, result+len, strlen(result)-len);
             tmp[strlen(result)-len]='\0';
             g_gpsInfo.Lat += (float)(gps_strtod(tmp)/60.0);
         }
      ...
      if (FetchField(start, result)) {
         ...
         if (point != NULL){
             ...
             strncpy(tmp, result, len);
             tmp[len]='\0';
             g_gpsInfo.Lon = (float)(gps_strtol_10(tmp));
             strncpy(tmp, result+len, strlen(result)-len);
             tmp[strlen(result)-len]='\0';
             g_gpsInfo.Lon += (float)(gps_strtod(tmp)/60.0);
         }
      ...
      if (start != NULL)
         ...
         return;
      if (FetchField(start, result)) {
         g_gpsInfo.Speed = (float)(gps_strtod(result) * Knot2Kmhr);
      }
      ...
      if (FetchField(start, result))
      {
         g_gpsInfo.Track = (float)(gps_strtod(result));
      }
      ...
      if (FetchField(start, result)) {
         ...
         strncpy(tmp, result, 2);
         tmp[2]='\0';
         g_gpsInfo.day=gps_strtol_10(tmp);

         // Month
         strncpy(tmp, result+2, 2);
         tmp[2]='\0';
         g_gpsInfo.mon=gps_strtol_10(tmp);

         // Year
         strncpy(tmp, result+4, 2);
         tmp[2]='\0';
         g_gpsInfo.year=gps_strtol_10(tmp)+2000;
      }
...
void GSA_Parse( char *head)
{
   ...
   if (CheckSum(head, strlen(head)))
   {
      ...
      if ((*result == 'A') || (*result == 'M')) {
         ...
         if (!FetchField(start, result))
            return;
         int fix_type = gps_strtol_10(result)-1;
         if (fix_type > 0) {         // Fix
            g_gpsInfo.FixType = (unsigned char)fix_type;
            if (g_gpsInfo.FixService == 0) {
      ...
      for (i = 0; i < 12; i++) {
         ...
         if (strlen(result) > 0)
            g_gpsInfo.fixSV[sv_cnt++] = gps_strtol_10(result);
      }
      ...
      if (!FetchField(start, result))
         return;
      g_gpsInfo.PDOP = (float)(gps_strtod(result));
      ...
      if (!FetchField(start, result))
         return;
      g_gpsInfo.HDOP = (float)(gps_strtod(result));
      ...
      if (!FetchField(start, result))
         return;
      g_gpsInfo.VDOP = (float)(gps_strtod(result));
   }
}
#if 0
//---------------------------------------------------------------------------
...
void GGA_Parse( char *head)
{
   ...
   if (CheckSum(head, strlen(head))) {
      ...
      if (!FetchField( start, result))
         return;
      g_gpsInfo.FixService = gps_strtol_10(result);
      ...
      if (start != NULL)
         ...
         return;
      if (FetchField(start, result)) {
         g_gpsInfo.Alt = (float)(gps_strtod(result));
      }
...
void ZDA_Parse( char *head)
{
   ...
   if (CheckSum(head, strlen(head)))
   {
      ...
      if (start != NULL)
         ...
         return;
      if (FetchField( start, result))
         g_gpsInfo.day = gps_strtol_10(result);
      ...
      if (start != NULL)
         ...
         return;
      if (FetchField( start, result))
         g_gpsInfo.mon = gps_strtol_10(result);
      ...
      if (start != NULL)
         ...
         return;
      if (FetchField(start, result))
         g_gpsInfo.year = gps_strtol_10(result);
   }
}
...
static bool gps_update_info(struct gps_desc *gps, char *info) {
    ...
    if (fd_mnld2factory != C_INVALID_SOCKET) {
        ...
        if (read_leng < 0) {
            ...
            FTM_SNPRINTF(gps->info, INFO_SIZE, "%s\n", uistr_info_gps_error);
            gps->exit_thd = true;
            return false;
        }
            ...
            if (((g_gpsInfo.FixType != 0) && (ttff != 0)/*avoid prev second's NMEA*/)
             || (fixed == 1)) {  // 2D or 3D fixed
                offset_ptr = snprintf(ptr, INFO_SIZE, "%s: %d\n", uistr_info_gps_ttff, open_time);
                if(offset_ptr  < 0) {
                    LOGD(TAG"snprintf failed");
                } else {
                    ptr += offset_ptr;
                }
            ...
            if(gps_l5_support()) {
                ...
                for (i = 0; i < g_gpsInfo.SV_cnt; i++) {
                    if (g_svInfo[i].SNR > 0) {
                        offset_ptr =  snprintf(ptr, INFO_SIZE, "%s[%d] : %d\n", uistr_info_gps_svid, g_svInfo[i].SVid, g_svInfo[i].SNR);
                        if(offset_ptr < 0) {
                            LOGD(TAG"snprintf failed");
                        } else {
                            ptr += offset_ptr;
...
static void* gps_update_ui(void *priv) {
    ...
    if (GPS_param.test_type == 3) {
        FTM_SNPRINTF(GPS_result, sizeof(GPS_result), "%d:%s", gps->mod->id, result[gps->mod->test_result]);
        write_data_to_pc(GPS_result, strlen(GPS_result));
    }
    gps->exit_thd = true;
    LOGD(TAG "%s, gps->exit_thd = true\n", __FUNCTION__);
    if (pthread_join(gps->update_thd, NULL) != 0) {
        LOGE("pthread_join fail: %s\n", strerror(errno));
    }
...
static void *gps_update_iv_thread(void *priv)
{
    ...
    memset(gps->info, 0, sizeof(gps->info));
    FTM_SNPRINTF(gps->info, INFO_SIZE, "%s\n", uistr_info_gps_init);
    if (GPS_param.test_type != 3) {  /* 3 means test on background  // 非后台测试模式才刷新UI*/
        iv->redraw(iv);
    }
    sleep(1);
    // 打开GPS设备
    ...
    if (init_status != 0) {   // GPS init fail
        memset(gps->info, 0, sizeof(gps->info));
        FTM_SNPRINTF(gps->info, INFO_SIZE, "%s (%d)\n", uistr_info_gps_error, init_status);
        if (GPS_param.test_type != 3) {
            iv->redraw(iv);
        }
        timeout = -1;
...
int gps_entry(struct ftm_param *param, void *priv)
{
    ...
    if (pTimeout != NULL) {
        timeout = gps_strtol_10(pTimeout);
...
#ifdef FEATURE_FTM_TOUCH_MODE  // 手动测试模式：
    ...
    // 创建界面更新线程
    if (pthread_create(&gps->update_thd, NULL, gps_update_iv_thread, priv) != 0) {
        LOGE("gps_update_iv_thread create fail: %s\n", strerror(errno));
    }
    if (GPS_param.test_type == 3) {
        if (pthread_create(&gps->update_ui, NULL, gps_update_ui, priv) != 0) {
            LOGE("gps_update_ui create fail: %s\n", strerror(errno));
        }
        iv->redraw(iv);
    }
    if (GPS_param.test_type != 3) {
...
int gps_init(void)
{
    int ret = -1;                    // 初始化返回值为-1（失败状态）
    struct ftm_module *mod;           // FTM模块结构体指针
    struct gps_desc *gps;             // GPS描述符结构体指针

    LOGD(TAG "%s\n", __FUNCTION__);   // 打印调试日志，显示函数名

    // 1. 分配FTM模块内存
    mod = ftm_alloc(ITEM_GPS, sizeof(struct gps_desc));
    if (mod == NULL) {                 // 内存分配失败检查
        LOGD("ftm_alloc failed!!!\n");
        return ret;                    // 返回-1
    }
    
    // 2. 将mod指针转换为gps_desc指针
    // mod_to_gps可能是一个宏：#define mod_to_gps(p)  (struct gps_desc*)((char*)(p) + sizeof(struct ftm_module))
    gps  = mod_to_gps(mod);

    // 3. 保存mod指针到gps结构体中，便于后续访问
    gps->mod      = mod;

    // 4. 注册GPS模块
    // - mod: FTM模块指针
    // - gps_entry: GPS模块的入口函数（处理命令的回调）
    // - gps: 私有数据指针，会在回调中传回
    ret = ftm_register(mod, gps_entry, (void*)gps);

    return ret;    // 返回注册结果（0成功，负数失败）
}
