...
/*
该函数的主要功能是将事件通知传递给特定的充电设备驱动，使驱动能够根据事件类型执行相应的操作

参数说明：
	chg_dev：指向充电设备的指针
	event：事件类型，如EVENT_FULL、EVENT_RECHARGE等
	args：事件参数，提供事件的额外信息
操作函数指针：
	chg_dev->ops：指向设备操作函数集的指针
	chg_dev->ops->event：指向具体设备驱动的事件处理函数
*/
int charger_dev_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	// 检查充电设备结构体和其操作函数指针是否有效
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->event)
		return chg_dev->ops->event(chg_dev, event, args);	// 调用具体设备驱动的event函数
...
int charger_dev_enable_ship_mode(struct charger_device *chg_dev, bool enable)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->enable_ship_mode)
		return chg_dev->ops->enable_ship_mode(chg_dev, enable);
