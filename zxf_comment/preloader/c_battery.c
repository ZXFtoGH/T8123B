...
void fuel_gauge_init(void)
{
	...
	bfgadc_con1 = upmu_get_reg_value(MT6358_FGADC_CON1);
	if (do_init_fgadc_reset == 1) {
		/* reset CON1 */
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x1f30, 0x1f00, 0x0);
		fgadc_con1 = upmu_get_reg_value(MT6358_FGADC_CON1);

		pal_log_err("BATTERY PRELOADER:ret=%d, bef FGADC_CON1=0x%x ,af:0x%x\n", ret, bfgadc_con1, fgadc_con1);
		mdelay(1);
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x0030, 0x1f00, 0x0);
	...
	pal_log_err(
		...
		shutdown_pmic_time %d hw_id 0x%x sw_id 0x%x, %d %d 0x%x 0x%x %d %d efuse_cal %d\n",
