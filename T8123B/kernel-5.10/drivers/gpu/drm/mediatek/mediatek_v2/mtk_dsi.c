...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_FTS_FT8205P_SPI)
...
int mtk_dsi_esd_cmp(struct mtk_ddp_comp *comp, void *handle, void *ptr)
{
	...
	mtk_ddp_comp_io_cmd(comp, NULL, GET_PANEL_NAME,
					&panel_name);
#if IS_ENABLED(CONFIG_TOUCHSCREEN_FTS_FT8205P_SPI)
	if (strcmp(panel_name,"panel-ft8201p_HJR110178D_A0-90hz") == 0) {
		if (lcd_need_reset == 0)
			ret = 0;
		else if(lcd_need_reset == 1) {
			DDPPR_ERR("[DSI]esd lcd_need_reset=(%d)\n",lcd_need_reset);
			lcd_need_reset = 0;
			ret = -1;
		}
		return ret;
