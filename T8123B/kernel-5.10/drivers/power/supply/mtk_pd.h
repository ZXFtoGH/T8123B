...
do {								\
	if (1/*pd_get_debug_level() >= PD_ERROR_LEVEL*/) {	\
		pr_notice(fmt, ##args);				\
	}							\
} while (0)
...
do {								\
	if (1/*pd_get_debug_level() >= PD_INFO_LEVEL*/) { \
		pr_notice(fmt, ##args);				\
	}							\
} while (0)
...
do {								\
	if (1/*pd_get_debug_level() >= PD_DEBUG_LEVEL*/) {	\
		pr_notice(fmt, ##args);				\
	}							\
} while (0)
...
struct mtk_pd {
	...
	int dcs_chg1_input_current;	
	int dcs_chg2_input_current;
