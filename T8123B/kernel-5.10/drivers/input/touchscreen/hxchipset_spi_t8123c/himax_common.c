...
extern struct gpio_desc *lcm_rst_gpio;
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	if (himax_int_gpio_read(hx_s_ts->rst_gpio)) {
		...
		if (!lcm_rst_gpio || IS_ERR(lcm_rst_gpio)) {
			pr_err("Failed to get lcm_rst_gpio: %ld\n", PTR_ERR(lcm_rst_gpio));
		}else{
			gpiod_set_value(lcm_rst_gpio, 0);//休眠时不在lcm驱动中下lcm_rst，在这里下以控制时序与规格书一致
		}
