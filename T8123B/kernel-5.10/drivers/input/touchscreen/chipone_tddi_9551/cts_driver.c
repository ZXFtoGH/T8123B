...
int cts_disp_notifier_callback(struct notifier_block *self,
        unsigned long event, void *data)
{
	...
	if (evdata && evdata->data) {
		...
		cts_info("cts touch can not suspend or resume");
		return -1;
	}
...
#endif
{
	...
	printk("tom add CFG_CTS_DRM_NOTIFIER\n");
#endif
    ...
    cts_info("tom add Probe i2c client: name='%s' addr=0x%02x flags=0x%02x irq=%d",
...
#ifndef CFG_CTS_PLATFORM_MTK_TPD_SUPPORTED
#ifdef CONFIG_CTS_PM_FB_NOTIFIER
        cts_data->pdata->fb_notifier.notifier_call = cts_disp_notifier_callback;
        if (mtk_disp_notifier_register("chipone-tddi", &cts_data->pdata->fb_notifier))
                cts_err("Failed to register disp notifier client");
