...
/*USER DEF*/
extern bool gesture_wp_ft8201p_enable;
...
static void tpd_gpio_output(int level)
{
    ...
    FTS_INFO("%s level = %d", __func__, level);
...
int fts_get_pinctrl(struct spi_device *spi_dev)
{
    int ret;

    FTS_INFO("[tpd] mt_tpd_pinctrl+");
    pinctrl1 = devm_pinctrl_get(spi_dev->controller->dev.parent);
    if (IS_ERR(pinctrl1)) {
        ret = PTR_ERR(pinctrl1);
        FTS_ERROR("Cannot find pinctrl1!");
        return ret;
    }
    pins_default = pinctrl_lookup_state(pinctrl1, "default");
    if (IS_ERR(pins_default)) {
        ret = PTR_ERR(pins_default);
        FTS_INFO("Cannot find pinctrl default %d!", ret);
    }
    ...
    if (IS_ERR(pin_spi_mode)) {
        ...
        FTS_ERROR("Failed to get pinctrl state:%s, ret:%d",
                "spi_mode", ret);
    }
    ...
    if (IS_ERR(rst_output0)) {
        ...
        FTS_ERROR("Cannot find pinctrl ts_reset_suspend!");
        return ret;
    }
    rst_output1 =
    ...
    if (IS_ERR(rst_output1)) {
        ...
        FTS_ERROR("Cannot find pinctrl ts_reset_active!");
        return ret;
    }
    spi_output0 =
    ...
    if (IS_ERR(spi_output0)) {
        ...
        FTS_ERROR("Cannot find pinctrl spi1_to_gpio!");
        return ret;
    }
    FTS_INFO("[tpd] mt_tpd_pinctrl-");
    return 0;
...
#if IS_ENABLED(CONFIG_EMDOOR_DEVICEINFO)
    if(hw_dev) {
        strcpy(hw_dev->dev_ctp.model, "FT8201P");
        strcpy(hw_dev->dev_ctp.supplier, "FTS");
    }
#endif
    return ret;
}
