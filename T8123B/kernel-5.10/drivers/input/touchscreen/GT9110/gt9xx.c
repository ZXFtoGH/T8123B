...
extern int lcm_unprepare_power(void);
extern int lcm_prepare_power(void);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	pdata->vdd_en_gpio = of_get_named_gpio(np, "vdd_en-gpios", 0);
	if (!gpio_is_valid(pdata->vdd_en_gpio))
		dev_err(dev, "No valid vdd_en gpio");
...
static int gtp_power_on(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->vdd_en_gpio)) {
		gpio_direction_output(ts->pdata->vdd_en_gpio, 1);
...
static int gtp_power_off(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->vdd_en_gpio)) {
		gpio_direction_output(ts->pdata->vdd_en_gpio, 0);
	}
	return ret;
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("GTP Driver Version: %s\n", GTP_DRIVER_VERSION);
	pr_info("GTP I2C Address: 0x%02x\n", client->addr);
