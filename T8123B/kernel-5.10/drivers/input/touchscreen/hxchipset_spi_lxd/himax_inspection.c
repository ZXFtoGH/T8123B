...
void himax_switch_data_type(uint8_t checktype, char **str_mode)
{
...
uint32_t himax_check_mode(uint8_t checktype, char **str_mode)
{
	int ret = 0;
	uint8_t tmp_data[4] = {0};
...
uint32_t himax_wait_sorting_mode(uint8_t checktype, char **str_mode)
{
	uint8_t tmp_data[4] = {0};
	uint8_t wait_pwd[2] = {0};
	int count = 0;
...
static int himax_chip_self_test_user(void)
{
    int ret = HX_INSP_OK;
    uint32_t test_size = hx_s_ic_data->tx_num    * hx_s_ic_data->rx_num
            + hx_s_ic_data->tx_num + hx_s_ic_data->rx_num;
...
void himax_inspection_init(void)
{
	...
	hx_s_core_fp._chip_self_test_user = himax_chip_self_test_user;
}
