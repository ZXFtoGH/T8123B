...
{
    if(gesture_wp_hx83102j_enable == 0)
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (buf[0] == '1'){
        ...
        gesture_wp_hx83102j_enable = 1;
    }
    else if (buf[0] == '0'){
        ts->SMWP_enable = 0;
        ts->gesture_cust_en[0] = 0;
        gesture_wp_hx83102j_enable = 0;
    }
...
static DEVICE_ATTR_RW(gesture_enable);

static ssize_t himax_self_test_user(void)
{
    ...
    himax_int_enable(0);/* disable irq */
    if (hx_s_core_fp._chip_self_test_user != NULL)
        ret = hx_s_core_fp._chip_self_test_user();
    else {
        E("%s: Doesn't turn on inspection function!\n", __func__);
    }
/*
...
{
    int ret;
    ret = himax_self_test_user();
    if (ret == 1)
        return sprintf(buf, "=======Tp test succeeds\n");
    else
        return sprintf(buf, "=======Tp test fails\n");
}
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    D("%s ,buf :%s",__func__,buf);
    return count;
}
static DEVICE_ATTR_RW(self_test);
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID,
					i + 1);
#endif
#ifdef HX_REPORT_POINT_ROTATION_180
...
#if defined(HX_BOOT_UPGRADE) || defined(HX_ZERO_FLASH)
	if (1 == ts->panel_name)
        ts->boot_fw_name = BOOT_UPGRADE_FWNAME_LXD;
    else
