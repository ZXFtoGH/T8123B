...
static struct pinctrl_state *cs_mode = NULL;
static struct pinctrl_state *gpio_mode = NULL;
static struct pinctrl *pinctrl;

int ctp_get_pins(struct device *dev)
{
    int ret = 0;
    pinctrl = devm_pinctrl_get(dev);
    if (IS_ERR(pinctrl) || (NULL == pinctrl)) {
        dev_err(dev, "Cannot find spi cs pinctrl!");
        ret = PTR_ERR(pinctrl);
        return ret;
    }

    pins_default = pinctrl_lookup_state(pinctrl, "pins_default");
    if (IS_ERR(pins_default) || NULL == pins_default) {
        ret = PTR_ERR(pins_default);
        pr_err("%s : pinctrl err, pins_default\n", __func__);
    }
    cs_mode = pinctrl_lookup_state(pinctrl, "cs_mode");
    if (IS_ERR(cs_mode) || NULL == cs_mode) {
        ret = PTR_ERR(cs_mode);
        pr_err("%s : pinctrl err, cs_mode\n", __func__);
    }
    gpio_mode = pinctrl_lookup_state(pinctrl, "gpio_mode");
    if (IS_ERR(gpio_mode) || NULL == gpio_mode) {
        ret = PTR_ERR(gpio_mode);
        pr_err("%s : pinctrl err, gpio_mode\n", __func__);
    }
    return 0;
}

static int ctp_set_mode(int mode)
{
