...
static struct pinctrl_state *eint_as_int, *eint_output0,
        *pin_spi_mode, *rst_output0,
        *spi_output0, *rst_output1;

static DEFINE_MUTEX(tpd_set_gpio_mutex);
static void tpd_gpio_output(int level)
{
    ...
    if (level) {
		I(" spi mode\n");
        pinctrl_select_state(pinctrl1, pin_spi_mode);
		...
		I(" spi gpio\n");
        pinctrl_select_state(pinctrl1, spi_output0);
    }
    mutex_unlock(&tpd_set_gpio_mutex);
}

static int himax_get_pinctrl(struct spi_device *spi_dev)
{
    int ret;

    I("[tpd] mt_tpd_pinctrl+");
    pinctrl1 = devm_pinctrl_get(spi_dev->controller->dev.parent);
    if (IS_ERR(pinctrl1)) {
        ret = PTR_ERR(pinctrl1);
        E("Cannot find pinctrl1!\n");
        return ret;
    }
    pins_default = pinctrl_lookup_state(pinctrl1, "default");
    if (IS_ERR(pins_default)) {
        ret = PTR_ERR(pins_default);
        I("Cannot find pinctrl default %d!\n", ret);
    }

    /* default spi mode */
    pin_spi_mode = pinctrl_lookup_state(pinctrl1, "spi_mode");
    if (IS_ERR(pin_spi_mode)) {
        ret = PTR_ERR(pin_spi_mode);
        E("Failed to get pinctrl state:%s, ret:%d\n",
                "spi_mode", ret);
    }
    ...
    if (IS_ERR(rst_output0)) {
        ...
        E("Cannot find pinctrl ts_reset_suspend!\n");
        return ret;
    }
    rst_output1 =
    ...
    if (IS_ERR(rst_output1)) {
        ...
        E("Cannot find pinctrl ts_reset_active!\n");
        return ret;
    }
    spi_output0 =
        pinctrl_lookup_state(pinctrl1, "spi1_to_gpio");
    if (IS_ERR(spi_output0)) {
        ret = PTR_ERR(spi_output0);
        E("Cannot find pinctrl spi1_to_gpio!\n");
        return ret;
    }
    I("[tpd] mt_tpd_pinctrl-\n");
    return 0;
...
int himax_input_register_ol_device(struct input_dev *input_dev)
{
	return input_register_device(input_dev);
}

int himax_parse_dt_ol(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
...
EXPORT_SYMBOL(himax_parse_dt_ol);
...
void himax_parse_dt_ol_ic_info(struct himax_ts_data *ts,
		struct himax_platform_data *pdata)
{
	...
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->rx_num = data;
		I("%s,Now parse:%s=%d\n", __func__, buff, hx_s_ic_data_ol->rx_num);
	} else
		I("%s, No definition: %s!\n", __func__, buff);

	hx_generate_ic_info_from_dt(g_proj_id, buff, ts->chip_name, str_tx_num);
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->tx_num = data;
		I("%s,Now parse:%s=%d\n", __func__, buff, hx_s_ic_data_ol->tx_num);
	} else
		I("%s, No definition: %s!\n", __func__, buff);

	hx_generate_ic_info_from_dt(g_proj_id, buff, ts->chip_name, str_bt_num);
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->bt_num = data;
		I("%s,Now parse:%s=%d\n", __func__, buff, hx_s_ic_data_ol->bt_num);
	} else
		I("%s, No definition: %s!\n", __func__, buff);

	hx_generate_ic_info_from_dt(g_proj_id, buff, ts->chip_name, str_max_pt);
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->max_pt = data;
		I("%s,Now parse:%s=%d\n", __func__, buff, hx_s_ic_data_ol->max_pt);
	} else
		I("%s, No definition: %s!\n", __func__, buff);
	...
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->int_is_edge = data;
		I("%s,Now parse:%s=%d\n",
			__func__, buff, hx_s_ic_data_ol->int_is_edge);
	} else
		I("%s, No definition: %s!\n", __func__, buff);
	...
	if (of_property_read_u32(dt, buff, &data) == 0) {
		hx_s_ic_data_ol->stylus_func = data;
		I("%s,Now parse:%s=%d\n",
			__func__, buff, hx_s_ic_data_ol->stylus_func);
	} else
		I("%s, No definition: %s!\n", __func__, buff);
...
EXPORT_SYMBOL(himax_parse_dt_ol_ic_info);
#endif
...
//	t.tx_buf = cmd;
	...
	for (retry = 0; retry < HIMAX_BUS_RETRY_TIMES; retry++) {
		error = spi_sync(hx_s_ts_ol->spi, &m);
		if (unlikely(error))
			E("SPI read error: %d\n", error);
		else
			break;
...
static int himax_spi_write(uint8_t *buf, uint32_t length)
{
	...
	status = spi_sync(hx_s_ts_ol->spi, &m);
...
int himax_bus_read_ol(uint8_t cmd, uint8_t *buf, uint32_t len)
{
	int result = -1;
...
//	result = himax_spi_read(spi_format_buf, BUS_R_HLEN, buf, len);
	if (hx_s_ts_ol->select_slave_reg) {
		hw_addr = hx_s_ts_ol->slave_read_reg;
		I("%s: now addr=0x%02X!\n", __func__, hw_addr);
	} else
		hw_addr = 0xF3;
	...
	mutex_unlock(&(hx_s_ts_ol->rw_lock));

	return result;
}
EXPORT_SYMBOL(himax_bus_read_ol);

int himax_bus_write_ol(uint8_t cmd, uint32_t addr, uint8_t *data, uint32_t len)
{
	int result = -1;
	uint8_t offset = 0;
	...
	if (hx_s_ts_ol->select_slave_reg) {
		hw_addr = hx_s_ts_ol->slave_write_reg;
		I("%s: now addr=0x%02X!\n", __func__, hw_addr);
	} else
		hw_addr = 0xF2;
	...
	if (addr != NUM_NULL) {
		hx_parse_assign_cmd_ol(addr, tmp_addr, 4);
...
EXPORT_SYMBOL(himax_bus_write_ol);

void himax_int_enable_ol(int enable)
{
	...
	if (enable == 1 && atomic_read(&ts->irq_state) == 0) {
		...
		enable_irq(irqnum);
		hx_s_ts_ol->irq_enabled = 1;
	} else if (enable == 0 && atomic_read(&ts->irq_state) == 1) {
		atomic_set(&ts->irq_state, 0);
		disable_irq_nosync(irqnum);
		hx_s_ts_ol->irq_enabled = 0;
	}

	I("enable = %d\n", enable);
	spin_unlock_irqrestore(&ts->irq_lock, irqflags);
}
EXPORT_SYMBOL(himax_int_enable_ol);

#if defined(HX_RST_PIN_FUNC)
void himax_rst_gpio_set_ol(int pinnum, uint8_t value)
{
	gpio_direction_output(pinnum, value);
}
EXPORT_SYMBOL(himax_rst_gpio_set_ol);
#endif
...
{
	int retval;
	/* struct i2c_client *client = hx_s_ts_ol->client; */

	pdata->vcc_dig = regulator_get(hx_s_ts_ol->dev, "vdd");
	...
	pdata->vcc_ana = regulator_get(hx_s_ts_ol->dev, "avdd");
...
int himax_gpio_power_config(struct himax_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->gpio_irq)) {
		...
		hx_s_ts_ol->hx_irq = gpio_to_irq(pdata->gpio_irq);
	} else {
...
int himax_ts_register_interrupt(void)
{
	...
	if (hx_s_ts_ol->hx_irq) {/*INT mode*/
		...
		if (ret == 0) {
			...
			irq_set_irq_wake(hx_s_ts_ol->hx_irq, 1);
#endif
		} else {
			ts->use_irq = 0;
		...
		I("%s: hx_s_ts_ol->hx_irq is empty, use polling mode.\n",
			__func__);
	}
...
int himax_ts_unregister_interrupt(void)
{
	...
	/* Work functon */
	if (hx_s_ts_ol->hx_irq && ts->use_irq) {/*INT mode*/
#if defined(HX_SMART_WAKEUP)
