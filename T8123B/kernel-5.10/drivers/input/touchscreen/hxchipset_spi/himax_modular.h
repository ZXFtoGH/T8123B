...
extern uint32_t CFG_TABLE_FLASH_ADDR_T;
	...
	extern void (himax_rst_gpio_set_ol)(int pinnum, uint8_t value);
#endif
...
extern int himax_bus_read_ol(uint8_t cmd, uint8_t *buf, uint32_t len);
extern int himax_bus_write_ol(uint8_t cmd, uint32_t addr, uint8_t *data,
	uint32_t len);


extern void himax_int_enable_ol(int enable);
