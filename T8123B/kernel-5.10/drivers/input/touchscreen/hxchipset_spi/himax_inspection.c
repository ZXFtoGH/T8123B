...
static void himax_press_powerkey(void)
{
	...
	input_report_key(hx_s_ts_ol->input_dev, KEY_POWER, 1);
	input_sync(hx_s_ts_ol->input_dev);

	msleep(100);

	I(" %s POWER KEY event %x release\n", __func__, KEY_POWER);
	input_report_key(hx_s_ts_ol->input_dev, KEY_POWER, 0);
	input_sync(hx_s_ts_ol->input_dev);
}
...
{
	int i = 0, j;
	int all_mut_len = hx_s_ic_data_ol->tx_num*hx_s_ic_data_ol->rx_num;
	uint32_t rx_num = hx_s_ic_data_ol->rx_num;
...
{
	int temp, i, j;
	int len = hx_s_ic_data_ol->rx_num * hx_s_ic_data_ol->tx_num;
	int *ArrayData;
	...
	uint32_t ret_val)
{
	...
	if (ret_val == HX_INSP_ESCREEN) {
		...
		for (i = 0;
			i < hx_s_ic_data_ol->tx_num*hx_s_ic_data_ol->rx_num;
			i++) {
			if (i > 1 && ((i + 1) % hx_s_ic_data_ol->rx_num) == 0)
...
static void himax_raw_data_dbg(int RAW[])
{
	...
	for (j = 0; j < hx_s_ic_data_ol->rx_num; j++) {
		if (j == 0)
			PI("      RX%2d", j + 1);
		else
			PI("  RX%2d", j + 1);
	...
	for (i = 0; i < hx_s_ic_data_ol->tx_num; i++) {
		PI("TX%2d", i + 1);
		for (j = 0; j < hx_s_ic_data_ol->rx_num; j++) {
			PI("%5d ", RAW[index]);
			index++;
	...
	for (i = 0; i < hx_s_ic_data_ol->rx_num; i++) {
		if (i == 0)
			PI("RX: %2d", RAW[index]);
		else
			PI(", %2d", RAW[index]);
...
static uint32_t himax_get_rawdata(int RAW[], uint32_t len, uint8_t checktype)
{
	...
	get_raw_rlst = hx_s_core_fp_ol._get_DSRAM_data(tmp_rawdata, false);
	if (!get_raw_rlst)
		goto DIRECT_END;
	...
	for (i = 0; i < len; i++) {
		...
		if (i < (len - hx_s_ic_data_ol->rx_num - hx_s_ic_data_ol->tx_num)) {
			if (i == 0)
	...
	if (hx_s_ts_ol->debug_log_level & BIT(4))
...
static void himax_switch_data_type(uint8_t checktype)
{
	...
	hx_s_core_fp_ol._diag_register_set(datatype, 0x00, false);
}
...
static void himax_bank_search_set(uint16_t Nframe, uint8_t checktype)
{
	...
	hx_s_core_fp_ol._register_read(
		hx_s_ic_setup_ol._addr_skip_frame, tmp_data, 4);
...
void himax_neg_noise_sup(uint8_t *data)
{
	...
	hx_s_core_fp_ol._register_read(
		hx_s_ic_setup_ol._addr_neg_noise_sup, tmp_data, 4);

	if ((tmp_data[3] & 0x04) == 0x04) {
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_neg_noise, tmp_data,
			sizeof(tmp_data));
...
static void himax_set_N_frame(uint16_t Nframe, uint8_t checktype)
{
	...
	if (checktype == HX_WT_NOISE ||
		...
		checktype == HX_LP_WT_NOISE ||
		checktype == HX_LP_ABS_NOISE)
		hx_s_core_fp_ol._neg_noise_sup(tmp_data);
	if (hx_s_ts_ol->debug_log_level & BIT(4)) {
...
void himax_get_noise_base(uint8_t checktype)/*Normal Threshold*/
{
	uint8_t tmp_data[4];
	uint8_t tmp_data2[4];
	...
	case HX_WT_NOISE:
	case HX_SELF_WT_NOISE:
		addr32 = hx_s_ic_setup_ol._addr_normal_noise_thx;
		break;
	case HX_LP_WT_NOISE:
		addr32 = hx_s_ic_setup_ol._addr_lpwug_noise_thx;
		break;
	default:
		I("%s Not support type\n", __func__);
	}
	hx_s_core_fp_ol._register_read(
		hx_s_ic_setup_ol._addr_noise_scale, tmp_data2, 4);
	tmp_data2[1] = tmp_data2[1]>>4;
	if (tmp_data2[1] == 0)
		tmp_data2[1] = 1;

	/*normal : 0x1000708F, LPWUG:0x10007093*/
	hx_s_core_fp_ol._register_read(addr32, tmp_data, 4);
	NOISEMAX = tmp_data[3] * tmp_data2[1];

	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_recal_thx, tmp_data, 4);
...
static uint16_t himax_get_palm_num(void)/*Palm Number*/
{
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_palm_num, tmp_data, 4);
...
int himax_get_noise_weight_test(uint8_t checktype)
{
	...
	hx_s_core_fp_ol._register_read(
		hx_s_ic_setup_ol._addr_weight_sup, tmp_data, 4);
	...
	case HX_WT_NOISE:
	case HX_SELF_WT_NOISE:
		addr32 = hx_s_ic_setup_ol._addr_normal_weight_a;
		break;
	case HX_LP_WT_NOISE:
		addr32 = hx_s_ic_setup_ol._addr_lpwug_weight_a;
		break;
	default:
		I("%s Not support type\n", __func__);
	}
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_weight_b, tmp_data, 4);
...
static uint32_t himax_check_mode(uint8_t checktype)
{
	...
	if (hx_s_core_fp_ol._check_sorting_mode != NULL) {
		ret = hx_s_core_fp_ol._check_sorting_mode(tmp_data);
		if (ret != NO_ERR)
			return ret;
	}
...
static uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		if (hx_s_core_fp_ol._check_sorting_mode != NULL)
			hx_s_core_fp_ol._check_sorting_mode(tmp_data);
		if (hx_s_ts_ol->debug_log_level & BIT(4))
		...
		if (hx_s_ts_ol->debug_log_level & BIT(4)) {
			hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._addr_chk_fw_status, tmp_data, 4);
			I(TEMP_LOG, __func__, hx_s_ic_setup_ol._addr_chk_fw_status,
				tmp_data[0], tmp_data[1],
				tmp_data[2], tmp_data[3]);

			hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._addr_flag_reset_event,
				tmp_data, 4);
			I(TEMP_LOG, __func__,
				hx_s_ic_setup_ol._addr_flag_reset_event,
				tmp_data[0], tmp_data[1],
			...
			hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._addr_fw_dbg_msg, tmp_data, 4);
			I(TEMP_LOG, __func__, hx_s_ic_setup_ol._addr_fw_dbg_msg,
				tmp_data[0], tmp_data[1],
				tmp_data[2], tmp_data[3]);
...
{
	int i = 0;
	int rx_num = hx_s_ic_data_ol->rx_num;
...
static int himax_gap_test_vertical_raw(int test_type, int *org_raw)
{
	...
	int tx_num = hx_s_ic_data_ol->tx_num;
	int rx_num = hx_s_ic_data_ol->rx_num;
...

{
	...
	int idx_max = 0;
	int idx_min = 0;
	int block_num = hx_s_ic_data_ol->tx_num * hx_s_ic_data_ol->rx_num;
	int self_block_num = hx_s_ic_data_ol->tx_num + hx_s_ic_data_ol->rx_num;
	uint16_t palm_num = 0;
	...
	case HX_OPEN:
		...
		break;
	case HX_SELF_OPEN:
		idx_max = IDX_SELF_OPENMAX;
		idx_min = IDX_SELF_OPENMIN;
		break;
	case HX_MICRO_OPEN:
	...
	case HX_SHORT:
		...
		break;
	case HX_SELF_SHORT:
		idx_max = IDX_SELF_SHORTMAX;
		idx_min = IDX_SELF_SHORTMIN;
		break;
	case HX_RAWDATA:
		idx_max = IDX_RAWMAX;
		idx_min = IDX_RAWMIN;
	...
	case HX_BPN_RAWDATA:
		...
		break;
	case HX_SELF_BPN_RAWDATA:
		idx_max = IDX_SELF_BPN_RAWMAX;
		idx_min = IDX_SELF_BPN_RAWMIN;
		break;
	case HX_SC:
		idx_max = IDX_SCMAX;
		idx_min = IDX_SCMIN;
	...
	case HX_WT_NOISE:
		...
		break;
	case HX_SELF_WT_NOISE:
		idx_max = IDX_SELF_WT_NOISEMAX;
		idx_min = IDX_SELF_WT_NOISEMIN;
		break;
	case HX_ABS_NOISE:
	...
	case HX_LP_WT_NOISE:
		noise_count = 0;
		hx_s_core_fp_ol._get_noise_base(checktype);
		palm_num = himax_get_palm_num();
		for (i = 0; i < (hx_s_ic_data_ol->tx_num * hx_s_ic_data_ol->rx_num);
		i++) {
	 ...
	 */
		/*Check weightingt*/
		if (hx_s_core_fp_ol._get_noise_weight_test(checktype) < 0) {
			I("%s: %s FAIL %X\n", __func__,
	...
	case HX_ACT_IDLE_BPN_RAWDATA:
	case HX_ACT_IDLE_NOISE:
		block_num = hx_s_ic_data_ol->ic_adc_num;
		for (i = 0; i < block_num; i++) {
...
static int himax_get_max_dc(void)
{
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_max_dc,
		tmp_data, DATA_LEN_4);
...
static uint32_t mpTestFunc(uint8_t checktype, uint32_t datalen)
{
	...
	if (check_sort_sts) {
		...
		if (hx_s_core_fp_ol._reload_disable != NULL)
			hx_s_core_fp_ol._reload_disable(1);
#endif
	...
	while (g_hx_inspt_crtra_name[i] != NULL) {
		...
		if (start_ptr != NULL) {
			...
			if (chk_ptr != NULL) {
				I("Now run to self!\n");
				tx_num = 1;
				rx_num = hx_s_ic_data_ol->rx_num
					+ hx_s_ic_data_ol->tx_num;
			} else {
				I("Now run to mutual!\n");
				tx_num = hx_s_ic_data_ol->tx_num;
				rx_num = hx_s_ic_data_ol->rx_num;
			}
...
static void hx_print_fw_info(void)
{
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_fw_vendor,
		data, DATA_LEN_4);
	cid_maj_ver = data[2];
	cid_min_ver = data[3];
...
static int himax_self_test_data_init(void)
{
	 ...
	 * 2: get twice size
	 */
	g_1kind_raw_size = 5 * hx_s_ic_data_ol->rx_num * hx_s_ic_data_ol->tx_num * 2;
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		g_inspection_criteria[i] = kcalloc((hx_s_ic_data_ol->tx_num
...
{
	int ret = HX_INSP_OK;
	uint32_t test_size = hx_s_ic_data_ol->tx_num	* hx_s_ic_data_ol->rx_num
			+ hx_s_ic_data_ol->tx_num + hx_s_ic_data_ol->rx_num;
	int i = 0;
	int j = 0;
#if !defined(HX_ZERO_FLASH)
	...
	hx_s_ts_ol->suspend_resume_done = 0;
...
#if defined(HX_ZERO_FLASH)
	ret = hx_s_core_fp_ol._0f_op_file_dirly(g_fw_mp_upgrade_name);
	if (ret) {
	...
	if (hx_s_ts_ol->debug_log_level & BIT(4)) {
		...
		g_mp_fw_ver = (hx_s_ic_data_ol->vendor_cid_maj_ver << 8
	...
	if (do_lpwg_test) {
		...
		/* Wait suspend done */
		while (hx_s_ts_ol->suspend_resume_done != 1) {
...
#if defined(HX_ZERO_FLASH)
UPDATE_MPFW_FAIL:
	hx_s_core_fp_ol._0f_op_file_dirly(g_fw_boot_upgrade_name);
#else
	if (hx_s_ts_ol->debug_log_level & BIT(4))
		I("%s:start sense off!\n", __func__);
	hx_s_core_fp_ol._sense_off(true);
	...
	hx_s_core_fp_ol._register_write(0x10007294, tmp_data, 4);
	if (hx_s_core_fp_ol._reload_disable != NULL)
		hx_s_core_fp_ol._reload_disable(0);
...
{
	I("%s: enter, %d\n", __func__, __LINE__);
	hx_s_core_fp_ol._chip_self_test = himax_chip_self_test;
}
