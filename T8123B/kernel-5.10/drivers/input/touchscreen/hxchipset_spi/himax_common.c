...
struct himax_ts_data *hx_s_ts_ol;
EXPORT_SYMBOL(hx_s_ts_ol);

struct himax_ic_data *hx_s_ic_data_ol;
EXPORT_SYMBOL(hx_s_ic_data_ol);

struct himax_report_data *hx_s_touch_data_ol;
EXPORT_SYMBOL(hx_s_touch_data_ol);

struct himax_core_fp hx_s_core_fp_ol;
EXPORT_SYMBOL(hx_s_core_fp_ol);

struct himax_debug *hx_s_debug_data_ol;
EXPORT_SYMBOL(hx_s_debug_data_ol);

struct proc_dir_entry *hx_touch_proc_dir_ol;
EXPORT_SYMBOL(hx_touch_proc_dir_ol);

struct ic_setup_collect hx_s_ic_setup_ol;
EXPORT_SYMBOL(hx_s_ic_setup_ol);

int g_mmi_refcnt_ol;
EXPORT_SYMBOL(g_mmi_refcnt_ol);
...
/*ts_work about start*/
struct himax_target_report_data *g_target_report_data_ol;
EXPORT_SYMBOL(g_target_report_data_ol);
...
u8 HX_EXCP_RESET_ACTIVATE_OL;
EXPORT_SYMBOL(HX_EXCP_RESET_ACTIVATE_OL);
...
uint8_t HX_PROC_SEND_FLAG_OL;
EXPORT_SYMBOL(HX_PROC_SEND_FLAG_OL);
...
static ssize_t himax_self_test(struct seq_file *s, void *v)
{
	...
	hx_s_ts_ol->in_self_test = 1;

	himax_int_enable_ol(0);/* disable irq */
	if (hx_s_core_fp_ol._chip_self_test != NULL)
		hx_s_core_fp_ol._chip_self_test(s, v);
	else {
...
/*
 *#if defined(HX_EXCP_RECOVERY)
 *	HX_EXCP_RESET_ACTIVATE_OL = 1;
 *#endif
 *	himax_int_enable_ol(1); //enable irq
 */

#if defined(HX_EXCP_RECOVERY)
	HX_EXCP_RESET_ACTIVATE_OL = 1;
#endif
	himax_int_enable_ol(1);

	hx_s_ts_ol->in_self_test = 0;
		...
		size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG_OL) {
		...
		if (temp_buf != NULL) {
			...
			kfree(temp_buf);
			HX_PROC_SEND_FLAG_OL = 1;
		} else {
			E("%s, Failed to allocate memory\n", __func__);
		}
	} else {
		HX_PROC_SEND_FLAG_OL = 0;
	}
		...
		size_t len, loff_t *pos)
{
	...
	hx_s_core_fp_ol._set_HSEN_enable(ts->HSEN_enable, ts->suspended);
		...
		size_t len, loff_t *pos)
{
	...
	hx_s_core_fp_ol._set_SMWP_enable(ts->SMWP_enable, ts->suspended);
		...
		size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0' && ts->SMWP_enable == 1) {
		...
		hx_s_core_fp_ol._black_gest_ctrl(false);
	} else if (buf[0] == '1' && ts->SMWP_enable == 1) {
		ts->psensor_flag = true;
		hx_s_core_fp_ol._black_gest_ctrl(true);
				...
				size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG_OL) {
		temp_buf = kcalloc(len, sizeof(char), GFP_KERNEL);
		ret += snprintf(temp_buf + ret, len - ret,
			"IC = %s\n", hx_s_ts_ol->chip_name);

		ret += snprintf(temp_buf + ret, len - ret,
			"FW_VER = 0x%2.2X\n", hx_s_ic_data_ol->vendor_fw_ver);

		if (hx_s_ts_ol->chip_cell_type == CHIP_IS_ON_CELL) {
			ret += snprintf(temp_buf + ret, len - ret,
				"CONFIG_VER = 0x%2.2X\n",
		...
		if (hx_s_ic_data_ol->vendor_cid_maj_ver < 0
		&& hx_s_ic_data_ol->vendor_cid_min_ver < 0) {
			ret += snprintf(temp_buf + ret, len - ret,
					"CID_VER = NULL\n");
		} else {
			ret += snprintf(temp_buf + ret, len - ret,
		...
		if (hx_s_ic_data_ol->vendor_panel_ver < 0) {
			ret += snprintf(temp_buf + ret, len - ret,
					"PANEL_VER = NULL\n");
		} else {
			ret += snprintf(temp_buf + ret, len - ret,
		...
		if (hx_s_ts_ol->chip_cell_type == CHIP_IS_IN_CELL) {
			ret += snprintf(temp_buf + ret, len - ret,
					"Cusomer = %s\n",
...
{
	hx_touch_proc_dir_ol = proc_mkdir(HX_PROC_TOUCH_FOLDER, NULL);

	if (hx_touch_proc_dir_ol == NULL) {
		E(" %s: hx_touch_proc_dir_ol file create failed!\n", __func__);
		return -ENOMEM;
	}
#if defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT)
	...
	proc_self_test_file = proc_create(HX_PROC_SELF_TEST_FILE, 0444,
		hx_touch_proc_dir_ol, &hx_proc_selftest_ops);
	...
	proc_hsen_file = proc_create(HX_PROC_HSEN_FILE, 0666,
			hx_touch_proc_dir_ol, &hx_proc_hsen_ops);
	...
	proc_SMWP_file = proc_create(HX_PROC_SMWP_FILE, 0666,
			hx_touch_proc_dir_ol, &hx_proc_smwp_ops);
	...
	proc_gesture_file = proc_create(HX_PROC_GESTURE_FILE, 0666,
			hx_touch_proc_dir_ol, &himax_proc_Gesture_ops);
	...
	proc_psensor_file = proc_create(HX_PROC_PSENSOR_FILE, 0666,
			hx_touch_proc_dir_ol, &himax_proc_psensor_ops);
	...
	proc_vendor_file = proc_create(HX_PROC_VENDOR_FILE, 0444,
		hx_touch_proc_dir_ol, &hx_proc_vendor_ops);
...
#if defined(HX_SMART_WAKEUP)
#if defined(HX_ULTRA_LOW_POWER)
	remove_proc_entry(HX_PROC_PSENSOR_FILE, hx_touch_proc_dir_ol);
fail_5:
#endif
	remove_proc_entry(HX_PROC_GESTURE_FILE, hx_touch_proc_dir_ol);
fail_4:
	remove_proc_entry(HX_PROC_SMWP_FILE, hx_touch_proc_dir_ol);
fail_3:
#endif
#if defined(HX_HIGH_SENSE)
	remove_proc_entry(HX_PROC_HSEN_FILE, hx_touch_proc_dir_ol);
fail_2:
#endif
	remove_proc_entry(HX_PROC_SELF_TEST_FILE, hx_touch_proc_dir_ol);
fail_1:
	return -ENOMEM;
}
...
{
	remove_proc_entry(HX_PROC_VENDOR_FILE, hx_touch_proc_dir_ol);
#if defined(HX_SMART_WAKEUP)
#if defined(HX_ULTRA_LOW_POWER)
	remove_proc_entry(HX_PROC_PSENSOR_FILE, hx_touch_proc_dir_ol);
#endif
...
void hx_parse_assign_cmd_ol(uint32_t addr, uint8_t *cmd, int len)
{
	/*I("%s: Entering!\n", __func__);*/
...
EXPORT_SYMBOL(hx_parse_assign_cmd_ol);

int himax_input_register_ol(struct himax_ts_data *ts)
{
	int ret = 0;
#if defined(HX_SMART_WAKEUP)
	int i = 0;
...
/*			0, 0);*/
	...
	if (himax_input_register_ol_device(ts->input_dev) == 0) {
		ret = NO_ERR;
	} else {
	...
	input_set_abs_params(ts->stylus_dev, ABS_X, ts->pdata->abs_x_min,
			((ts->pdata->abs_x_max+1)*hx_s_ic_data_ol->stylus_ratio-1),
			ts->pdata->abs_x_fuzz, 0);
	input_set_abs_params(ts->stylus_dev, ABS_Y, ts->pdata->abs_y_min,
			((ts->pdata->abs_y_max+1)*hx_s_ic_data_ol->stylus_ratio-1),
			ts->pdata->abs_y_fuzz, 0);

	if (himax_input_register_ol_device(ts->stylus_dev) == 0) {
		ret = NO_ERR;
	} else {
...
EXPORT_SYMBOL(himax_input_register_ol);
...
static int himax_auto_update_check(void)
{
	...
	if (himax_get_fw_ver_bin() == 0) {
		if (((hx_s_ic_data_ol->vendor_fw_ver < g_i_FW_VER)
		|| (hx_s_ic_data_ol->vendor_config_ver < g_i_CFG_VER))) {
			I("%s: Need update\n", __func__);
			ret = 0;
		} else {
			I("%s: Need not update!\n", __func__);
...
static int i_get_FW(void)
{
	...
	ret = request_firmware(&hxfw, g_fw_boot_upgrade_name, hx_s_ts_ol->dev);
...
update_retry:
	...
	ret = hx_s_core_fp_ol._firmware_update_0f(hxfw, 0);
	if (ret != 0) {
		upgrade_times++;
...
#else
	I("%s:Now fw size = %d!\n", __func__, hxfw->size);
	ret = hx_s_core_fp_ol._fts_ctpm_fw_upgrade_with_sys_fs(
...
#else
	hx_s_core_fp_ol._excp_ic_reset();
#endif
	himax_report_all_leave_event(hx_s_ts_ol);
	if (hx_s_ts_ol->diag_cmd != 0)
		hx_s_core_fp_ol._diag_register_set(hx_s_ts_ol->diag_cmd,
			hx_s_ts_ol->diag_storage_type,
			hx_s_ts_ol->diag_dirly);
...
static void gest_pt_log_coordinate(int rx, int tx)
{
	...
	gest_pt_x[gest_pt_cnt] = rx * (hx_s_ic_data_ol->x_res) / 255;
	gest_pt_y[gest_pt_cnt] = tx * (hx_s_ic_data_ol->y_res) / 255;
}
#endif
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	memcpy(buf, hx_s_touch_data_ol->event_buf, hx_s_touch_data_ol->event_size);
	...
	if (!ts->gesture_cust_en[gesture_pos]) {
		...
		g_target_report_data_ol->SMWP_event_chk = 0;
		ret = 0;
	} else {
		g_target_report_data_ol->SMWP_event_chk =
...
{
	int KEY_EVENT = g_target_report_data_ol->SMWP_event_chk;
	...
	if (KEY_EVENT) {
		...
		input_report_key(hx_s_ts_ol->input_dev, KEY_EVENT, 1);
		input_sync(hx_s_ts_ol->input_dev);
		I("%s SMART WAKEUP KEY event %d release\n",
				__func__, KEY_EVENT);
		input_report_key(hx_s_ts_ol->input_dev, KEY_EVENT, 0);
		input_sync(hx_s_ts_ol->input_dev);
#if defined(HX_GESTURE_TRACK)
...
int himax_report_data_init_ol(void)
{
	if (hx_s_touch_data_ol->coord_buf != NULL) {
		kfree(hx_s_touch_data_ol->coord_buf);
		hx_s_touch_data_ol->coord_buf = NULL;
	}

	if (hx_s_touch_data_ol->rawdata_buf != NULL) {
		kfree(hx_s_touch_data_ol->rawdata_buf);
		hx_s_touch_data_ol->rawdata_buf = NULL;
	}

#if defined(HX_SMART_WAKEUP)
	hx_s_touch_data_ol->event_size = hx_s_core_fp_ol._get_touch_data_size();

	if (hx_s_touch_data_ol->event_buf != NULL) {
		kfree(hx_s_touch_data_ol->event_buf);
		hx_s_touch_data_ol->event_buf = NULL;
	}
	...
	if (g_target_report_data_ol != NULL) {
		if (hx_s_ic_data_ol->stylus_func) {
			kfree(g_target_report_data_ol->s);
			g_target_report_data_ol->s = NULL;
		}

		kfree(g_target_report_data_ol->p);
		g_target_report_data_ol->p = NULL;
		kfree(g_target_report_data_ol);
		g_target_report_data_ol = NULL;
	}

	hx_s_touch_data_ol->touch_all_size =
		HX_FULL_STACK_RAWDATA_SIZE;
	hx_s_touch_data_ol->touch_all_size_full_stack =
		HX_FULL_STACK_RAWDATA_SIZE;
	hx_s_touch_data_ol->touch_all_size_normal =
		hx_s_core_fp_ol._get_touch_data_size();

	hx_s_touch_data_ol->raw_cnt_max = hx_s_ic_data_ol->max_pt / 4;
	hx_s_touch_data_ol->raw_cnt_rmd = hx_s_ic_data_ol->max_pt % 4;
	/* more than 4 fingers */
	if (hx_s_touch_data_ol->raw_cnt_rmd != 0x00) {
		hx_s_touch_data_ol->rawdata_size =
			hx_s_core_fp_ol._cal_data_len(
				hx_s_touch_data_ol->raw_cnt_rmd,
				hx_s_ic_data_ol->max_pt,
				hx_s_touch_data_ol->raw_cnt_max);

		hx_s_touch_data_ol->touch_info_size = (hx_s_ic_data_ol->max_pt
				+ hx_s_touch_data_ol->raw_cnt_max + 2) * 4;
		...
		hx_s_touch_data_ol->touch_info_size = (hx_s_ic_data_ol->max_pt
				+ hx_s_touch_data_ol->raw_cnt_max + 1) * 4;
	}

	if (hx_s_ic_data_ol->stylus_func) {
		hx_s_touch_data_ol->touch_info_size += STYLUS_INFO_SZ;
		hx_s_touch_data_ol->rawdata_size -= STYLUS_INFO_SZ;
	}

	// hx_s_touch_data_ol->touch_info_size = HX_STACK_ORG_LEN;
	hx_s_touch_data_ol->rawdata_size_normal =
		hx_s_touch_data_ol->rawdata_size;
	hx_s_touch_data_ol->rawdata_size =
		hx_s_touch_data_ol->touch_all_size - HX_STACK_ORG_LEN;
	hx_s_touch_data_ol->rawdata_size_full_stack =
		hx_s_touch_data_ol->touch_all_size - HX_STACK_ORG_LEN;

	if ((hx_s_ic_data_ol->tx_num
	* hx_s_ic_data_ol->rx_num
	+ hx_s_ic_data_ol->tx_num
	+ hx_s_ic_data_ol->rx_num)
	% hx_s_touch_data_ol->rawdata_size == 0)
		hx_s_touch_data_ol->rawdata_frame_size =
			(hx_s_ic_data_ol->tx_num
			* hx_s_ic_data_ol->rx_num
			+ hx_s_ic_data_ol->tx_num
			+ hx_s_ic_data_ol->rx_num)
	...
	hx_s_touch_data_ol->coord_buf = kzalloc(sizeof(uint8_t)
			* (hx_s_touch_data_ol->touch_info_size),
			GFP_KERNEL);
...
#if defined(HX_SMART_WAKEUP)
	wake_event_buffer = kcalloc(hx_s_touch_data_ol->event_size,
			sizeof(uint8_t), GFP_KERNEL);
	if (wake_event_buffer == NULL)
		goto mem_alloc_fail_smwp;

	hx_s_touch_data_ol->event_buf = kzalloc(sizeof(uint8_t)
			* (hx_s_touch_data_ol->event_size), GFP_KERNEL);
	if (hx_s_touch_data_ol->event_buf == NULL)
		goto mem_alloc_fail_event_buf;
#endif

	hx_s_touch_data_ol->rawdata_buf = kzalloc(sizeof(uint8_t)
		* (hx_s_touch_data_ol->touch_all_size
		- HX_STACK_ORG_LEN),
		GFP_KERNEL);
 ...
 */
		...
		g_target_report_data_ol->p = kzalloc(
			sizeof(struct himax_target_point_data)
			*(hx_s_ic_data_ol->max_pt), GFP_KERNEL);
		if (g_target_report_data_ol->p == NULL)
			goto mem_alloc_fail_report_data_p;

		if (!hx_s_ic_data_ol->stylus_func)
			goto skip_stylus_operation;

		g_target_report_data_ol->s = kzalloc(
			sizeof(struct himax_target_stylus_data)*2, GFP_KERNEL);
		if (g_target_report_data_ol->s == NULL)
...
EXPORT_SYMBOL(himax_report_data_init_ol);
...
void himax_cable_detect_func(bool force_renew)
{
	...
	if (ts->cable_config) {
		if ((connect_status != ts->usb_connected) || force_renew) {
			...
			hx_s_core_fp_ol._usb_detect_set(ts->cable_config);
...
static int himax_ts_work_status(struct himax_ts_data *ts)
{
	...
	hx_s_touch_data_ol->diag_cmd = ts->diag_cmd;
	if (hx_s_touch_data_ol->diag_cmd)
		result = HX_REPORT_COORD_RAWDATA;

#if defined(HX_SMART_WAKEUP)
	if (atomic_read(&ts->suspend_mode)
	&& (ts->SMWP_enable)
	&& (!hx_s_touch_data_ol->diag_cmd))
		...
		int ts_path, int ts_status)
{
	...
	if (g_ts_dbg_ol != 0) {
		I("%s: Entering, ts_status=%d!\n",
			__func__, ts_status);
		I("%s: Max Size=%d\n",
			__func__, hx_s_touch_data_ol->touch_all_size);
	}
...
#if defined(HX_EXCP_RECOVERY)
			|| (HX_EXCP_RESET_ACTIVATE_OL)
#endif
		) {
			read_size = hx_s_touch_data_ol->touch_all_size;
		} else {
			read_size = hx_s_touch_data_ol->touch_info_size;
		}
		break;
#if defined(HX_SMART_WAKEUP)
	...
	case HX_REPORT_SMWP_EVENT:
		...
		msleep(20);
		read_size = hx_s_touch_data_ol->event_size;
	...
	if (read_size == 0) {
		...
		if (!hx_s_core_fp_ol._read_event_stack(buf, read_size)) {
		...
		int ts_path, int ts_status)
{
	...
	switch (ts_path) {
	case HX_REPORT_COORD:
		length = hx_s_touch_data_ol->touch_info_size;
		break;
#if defined(HX_SMART_WAKEUP)
/* SMWP */
	case HX_REPORT_SMWP_EVENT:
	...
	if (check_sum_cal % 0x100 != 0) {
		...
		/*I("%s:raw_out_sel=%x , hx_s_touch_data_ol->diag_cmd=%x.\n",*/
		...
		if (g_ts_dbg_ol != 0)
			I("%s: skip check rawout select, now val=%d!\n",
				__func__, raw_data_sel);
		if (!hx_s_debug_data_ol->
			is_stack_full_raw) {
			if ((raw_data_sel != 0x0F)
			&& (raw_data_sel != hx_s_touch_data_ol->diag_cmd)) {
				/*I("%s:raw data out not match.\n", __func__);*/
				if (!hx_s_touch_data_ol->diag_cmd) {
					/*Need to clear event stack here*/
					hx_s_core_fp_ol._read_event_stack(buf,
						(HX_STACK_ORG_LEN -
						hx_s_touch_data_ol->
							touch_info_size));
				}
		...
		const uint8_t *buf, int ts_path, int ts_status)
{
	...
	if (hx_EB_event == length) {
		...
		hx_s_core_fp_ol._0f_reload_to_active();
	}

	if ((hx_excp_event == length || hx_ED_event == length)
		&& (g_hw_rst_activate == 0)
		&& (HX_EXCP_RESET_ACTIVATE_OL == 0)
		&& (hx_s_touch_data_ol->diag_cmd == 0)) {
		shaking_ret = hx_s_core_fp_ol._ic_excp_recovery(
	...
	} else if (HX_EXCP_RESET_ACTIVATE_OL) {
		HX_EXCP_RESET_ACTIVATE_OL = 0;
		I("%s: Skip by HX_EXCP_RESET_ACTIVATE_OL.\n", __func__);
 ...
 *		if (hx_s_core_fp_ol._0f_esd_check() < 0) {
...
static int himax_distribute_touch_data(uint8_t *buf,
		int ts_path, int ts_status)
{
	uint8_t hx_state_info_pos = hx_s_touch_data_ol->touch_info_size - 3;
	...
	if (ts_path == HX_REPORT_COORD) {
		memcpy(hx_s_touch_data_ol->coord_buf, &buf[0],
				hx_s_touch_data_ol->touch_info_size);

		if (buf[hx_state_info_pos] != 0xFF
		&& buf[hx_state_info_pos + 1] != 0xFF)
			memcpy(hx_s_touch_data_ol->state_info,
					&buf[hx_state_info_pos], 2);
		else
			memset(hx_s_touch_data_ol->state_info, 0x00,
					sizeof(hx_s_touch_data_ol->state_info));
		...
		|| (HX_EXCP_RESET_ACTIVATE_OL)
			...
			if (!hx_s_debug_data_ol->
				is_stack_full_raw) {
				memcpy(hx_s_touch_data_ol->rawdata_buf,
						&buf[hx_s_touch_data_ol->
							touch_info_size],
						hx_s_touch_data_ol->touch_all_size
						- hx_s_touch_data_ol->
							touch_info_size);
		...
		if (!hx_s_debug_data_ol->
			is_stack_full_raw) {
			memcpy(hx_s_touch_data_ol->rawdata_buf,
					&buf[hx_s_touch_data_ol->touch_info_size],
					hx_s_touch_data_ol->touch_all_size
					- hx_s_touch_data_ol->touch_info_size);
		} else {
			memcpy(hx_s_touch_data_ol->rawdata_buf,
		...
		memcpy(hx_s_touch_data_ol->event_buf, buf,
				hx_s_touch_data_ol->event_size);
#endif
	} else {
		E("%s, Fail Path!\n", __func__);
...
static bool wgp_pen_id_crc(uint8_t *p_id)
{
	...
	if (devidend == hash_id) {
		g_target_report_data_ol->s[0].id = pen_id;
		return 1;
	}

	g_target_report_data_ol->s[0].id = 0;
	return 0;
}
		...
		int ts_path, int ts_status)
{
	...
	uint8_t ratio = hx_s_ic_data_ol->stylus_ratio;
	...
	base = hx_s_touch_data_ol->touch_info_size;
	...
	p_p_on = 0;
	base -= STYLUS_INFO_SZ;
	if (hx_s_ic_data_ol->stylus_id_v2) {/*Pen format ver2*/
		p_x = hx_s_touch_data_ol->coord_buf[base] << 8
			| hx_s_touch_data_ol->coord_buf[base + 1];
		p_y = (hx_s_touch_data_ol->coord_buf[base + 2] << 8
			| hx_s_touch_data_ol->coord_buf[base + 3]);
		p_w = (hx_s_touch_data_ol->coord_buf[base + 4] << 8
			| hx_s_touch_data_ol->coord_buf[base + 5]);
		p_tilt_x = (int8_t)hx_s_touch_data_ol->coord_buf[base + 6];
		p_tilt_y = (int8_t)hx_s_touch_data_ol->coord_buf[base + 7];
		p_hover = hx_s_touch_data_ol->coord_buf[base + 8] & 0x01;
		p_btn = hx_s_touch_data_ol->coord_buf[base + 8] & 0x02;
		p_btn2 = hx_s_touch_data_ol->coord_buf[base + 8] & 0x04;
		p_id_en = hx_s_touch_data_ol->coord_buf[base + 8] & 0x08;
		if (!p_id_en) {
			g_target_report_data_ol->s[0].battery_info =
				hx_s_touch_data_ol->coord_buf[base + 9];
			if (g_ts_dbg_ol != 0) {
			...
			p_id_sel =
			(hx_s_touch_data_ol->coord_buf[base + 8] & 0xF0) >> 4;
			p_id[p_id_sel*2] =
			hx_s_touch_data_ol->coord_buf[base + 9];
			p_id[p_id_sel*2 + 1] =
			hx_s_touch_data_ol->coord_buf[base + 10];
			if (g_ts_dbg_ol != 0) {
		...
		p_w = (hx_s_touch_data_ol->coord_buf[base + 4] << 8
			| hx_s_touch_data_ol->coord_buf[base + 5]);
		p_tilt_x = (int8_t)hx_s_touch_data_ol->coord_buf[base + 6];
		p_hover = hx_s_touch_data_ol->coord_buf[base + 7];
		p_btn = hx_s_touch_data_ol->coord_buf[base + 8];
		p_btn2 = hx_s_touch_data_ol->coord_buf[base + 9];
		p_tilt_y = (int8_t)hx_s_touch_data_ol->coord_buf[base + 10];
	}
	if (g_ts_dbg_ol != 0) {
	...
	&& p_y <= ((ts->pdata->abs_y_max+1)*ratio-1)) {
		g_target_report_data_ol->s[0].x = p_x;
		g_target_report_data_ol->s[0].y = p_y;
		g_target_report_data_ol->s[0].w = p_w;
		g_target_report_data_ol->s[0].hover = p_hover;
		g_target_report_data_ol->s[0].btn = p_btn;
		g_target_report_data_ol->s[0].btn2 = p_btn2;
		g_target_report_data_ol->s[0].tilt_x = p_tilt_x;
		g_target_report_data_ol->s[0].tilt_y = p_tilt_y;
		g_target_report_data_ol->s[0].on = 1;
		ts->hx_stylus_num++;
	} else {/* report coordinates */
		g_target_report_data_ol->s[0].x = 0;
		g_target_report_data_ol->s[0].y = 0;
		g_target_report_data_ol->s[0].w = 0;
		g_target_report_data_ol->s[0].hover = 0;
		g_target_report_data_ol->s[0].btn = 0;
		g_target_report_data_ol->s[0].btn2 = 0;
		g_target_report_data_ol->s[0].tilt_x = 0;
		g_target_report_data_ol->s[0].tilt_y = 0;
		g_target_report_data_ol->s[0].on = 0;
	}

	if (g_ts_dbg_ol != 0) {
		if (p_p_on != g_target_report_data_ol->s[0].on) {
			I("s[0].on = %d, hx_stylus_num=%d\n",
				g_target_report_data_ol->s[0].on,
				ts->hx_stylus_num);
			p_p_on = g_target_report_data_ol->s[0].on;
		}
	}
skip_stylus_operation:
	...
	hx_s_touch_data_ol->finger_num =
		hx_s_touch_data_ol->coord_buf[base - 4] & 0x0F;
	hx_s_touch_data_ol->finger_on = 1;
	AA_press = 1;

	g_target_report_data_ol->finger_num = hx_s_touch_data_ol->finger_num;
	g_target_report_data_ol->finger_on = hx_s_touch_data_ol->finger_on;
	g_target_report_data_ol->ig_count =
		hx_s_touch_data_ol->coord_buf[base - 5];

	if (g_ts_dbg_ol != 0)
		I("%s:finger_num = 0x%2X, finger_on = %d\n", __func__,
				g_target_report_data_ol->finger_num,
				g_target_report_data_ol->finger_on);

	for (i = 0; i < ts->nFinger_support; i++) {
		base = i * 4;
		x = hx_s_touch_data_ol->coord_buf[base] << 8
			| hx_s_touch_data_ol->coord_buf[base + 1];
		y = (hx_s_touch_data_ol->coord_buf[base + 2] << 8
			| hx_s_touch_data_ol->coord_buf[base + 3]);
		w = hx_s_touch_data_ol->coord_buf[(ts->nFinger_support * 4) + i];
		...
		&& y <= ts->pdata->abs_y_max) {
			...
			g_target_report_data_ol->p[i].x = x;
			g_target_report_data_ol->p[i].y = y;
			g_target_report_data_ol->p[i].w = w;
			g_target_report_data_ol->p[i].id = 1;

			/*I("%s: g_target_report_data_ol->x[loop_i]=%d,*/
			/*g_target_report_data_ol->y[loop_i]=%d,*/
			/*g_target_report_data_ol->w[loop_i]=%d",*/
			/*__func__, g_target_report_data_ol->x[loop_i],*/
			/*g_target_report_data_ol->y[loop_i],*/
			/*g_target_report_data_ol->w[loop_i]); */
			...
			g_target_report_data_ol->p[i].y = y;
			g_target_report_data_ol->p[i].w = w;
			g_target_report_data_ol->p[i].id = 0;
	...
	if (g_ts_dbg_ol != 0) {
		for (i = 0; i < ts->nFinger_support; i++)
			D("DBG X=%d  Y=%d ID=%d\n",
				g_target_report_data_ol->p[i].x,
				g_target_report_data_ol->p[i].y,
				g_target_report_data_ol->p[i].id);
		...
		int ts_path, int ts_status)
{
	...
	EN_NoiseFilter =
		(hx_s_touch_data_ol->coord_buf[HX_TOUCH_INFO_POINT_CNT + 2] >> 3);
	/* I("EN_NoiseFilter=%d\n", EN_NoiseFilter); */
	EN_NoiseFilter = EN_NoiseFilter & 0x01;
	/* I("EN_NoiseFilter2=%d\n", EN_NoiseFilter); */
	p_point_num = ts->hx_point_num;

	if (hx_s_touch_data_ol->coord_buf[HX_TOUCH_INFO_POINT_CNT] == 0xff)
		ts->hx_point_num = 0;
	else
		ts->hx_point_num =
			hx_s_touch_data_ol->coord_buf[HX_TOUCH_INFO_POINT_CNT]
			& 0x0f;
	...
	case HX_REPORT_COORD_RAWDATA:
		...
		if (hx_s_debug_data_ol != NULL) {
			if (hx_s_debug_data_ol->is_stack_full_raw) {
				if (g_ts_dbg_ol != 0)
					I("%s: full stack raw!\n", __func__);
				if (hx_s_debug_data_ol->
					_raw_full_stack(hx_s_ic_data_ol,
					hx_s_touch_data_ol))
				...
				if (g_ts_dbg_ol != 0)
					I("%s: event stack raw!\n", __func__);
				if (hx_s_debug_data_ol->_set_diag_cmd(hx_s_ic_data_ol,
					hx_s_touch_data_ol))
					I("%s:raw data_checksum not match\n",
						__func__);
			}
...
static void himax_point_report(struct himax_ts_data *ts)
{
	int i;
	bool valid;
	#if IS_ENABLED(CONFIG_HIMAX_SUPPORT_SWAP_XY)
	int temp = 0;
	#endif
	if (g_ts_dbg_ol != 0) {
		I("%s:start hx_s_touch_data_ol->finger_num=%d\n",
			__func__, hx_s_touch_data_ol->finger_num);
	}
	for (i = 0; i < ts->nFinger_support; i++) {
		if (g_target_report_data_ol->p[i].x >= 0
		&& g_target_report_data_ol->p[i].x <= ts->pdata->abs_x_max
		&& g_target_report_data_ol->p[i].y >= 0
		&& g_target_report_data_ol->p[i].y <= ts->pdata->abs_y_max)
			valid = true;
		else
			valid = false;
		if (g_ts_dbg_ol != 0)
		...
		if (valid) {
			if (g_ts_dbg_ol != 0) {
				I("report_data->x[i]=%d,y[i]=%d,w[i]=%d",
					g_target_report_data_ol->p[i].x,
					g_target_report_data_ol->p[i].y,
					g_target_report_data_ol->p[i].w);
			}
			#if IS_ENABLED(CONFIG_HIMAX_SUPPORT_SWAP_XY)
				temp = g_target_report_data_ol->p[i].x;
				g_target_report_data_ol->p[i].x = ts->pdata->abs_x_max - (g_target_report_data_ol->p[i].y * ts->pdata->abs_x_max / ts->pdata->abs_y_max);
				g_target_report_data_ol->p[i].y = ts->pdata->abs_y_max - (temp  * ts->pdata->abs_y_max / ts->pdata->abs_x_max);
			#endif
#if !defined(HX_PROTOCOL_A)
			input_mt_slot(ts->input_dev, i);
...
static void himax_point_leave(struct himax_ts_data *ts)
{
	...
	if (g_ts_dbg_ol != 0)
		I("%s: start!\n", __func__);
#if defined(HX_PALM_REPORT)
	if (himax_palm_detect(hx_s_touch_data_ol->coord_buf) == PALM_REPORT) {
	...
	hx_s_touch_data_ol->finger_on = 0;
	g_target_report_data_ol->finger_on  = 0;
	g_target_report_data_ol->finger_num = 0;
	AA_press = 0;
...
static void himax_stylus_report(struct himax_ts_data *ts)
{
	...
	if (g_ts_dbg_ol != 0) {
		I("%s:start hx_s_touch_data_ol->stylus_num=%d\n",
			__func__, ts->hx_stylus_num);
	}

	if (g_target_report_data_ol->s[0].x >= 0
	&& g_target_report_data_ol->s[0].x <= ((ts->pdata->abs_x_max+1)*ratio-1)
	&& g_target_report_data_ol->s[0].y >= 0
	&& g_target_report_data_ol->s[0].y <= ((ts->pdata->abs_y_max+1)*ratio-1)
	&& (g_target_report_data_ol->s[0].on == 1))
		valid = true;
	else
		valid = false;
	...
	if (valid) {/*stylus down*/
		if (g_ts_dbg_ol != 0)
			I("s[i].x=%d, s[i].y=%d, s[i].w=%d\n",
					g_target_report_data_ol->s[0].x,
					g_target_report_data_ol->s[0].y,
					g_target_report_data_ol->s[0].w);
		...
		if (g_target_report_data_ol->s[0].btn !=
		g_target_report_data_ol->s[0].pre_btn) {
			if (g_ts_dbg_ol != 0)
				I("BTN_STYLUS:%d\n",
					g_target_report_data_ol->s[0].btn);
		...
		if (g_target_report_data_ol->s[0].btn2
		!= g_target_report_data_ol->s[0].pre_btn2) {
			if (g_ts_dbg_ol != 0)
				I("BTN_STYLUS2:%d\n",
					g_target_report_data_ol->s[0].btn2);
		...
		input_report_abs(ts->stylus_dev, ABS_TILT_X,
				g_target_report_data_ol->s[0].tilt_x);

		input_report_abs(ts->stylus_dev, ABS_TILT_Y,
				g_target_report_data_ol->s[0].tilt_y);

		input_report_key(ts->stylus_dev, BTN_TOOL_PEN, 1);

		if (g_target_report_data_ol->s[0].hover == 0) {
		...
		g_target_report_data_ol->s[0].pre_btn = 0;
		g_target_report_data_ol->s[0].pre_btn2 = 0;
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	if (hx_s_debug_data_ol != NULL) {
		if (hx_s_debug_data_ol->is_checking_irq) {
			if (g_ts_dbg_ol != 0)
				I("Now checking IRQ, skip it!\n");
			return;
		}
		hx_s_debug_data_ol->_ts_dbg_func(ts, HX_FINGER_ON);
	}
	...
	memset(ts->xfer_buff,
		0x00,
		hx_s_touch_data_ol->touch_all_size * sizeof(uint8_t));
...
#if defined(HX_ZERO_FLASH)
	if (hx_s_core_fp_ol._0f_reload_to_active)
		hx_s_core_fp_ol._0f_reload_to_active();
#endif
END_FUNCTION:
	if (hx_s_debug_data_ol != NULL)
		hx_s_debug_data_ol->_ts_dbg_func(ts, HX_FINGER_LEAVE);
...
static int hx_chk_flash_sts(uint32_t size)
{
	...
	rslt = (!hx_s_core_fp_ol._calculateChecksum(false, size));
	/*avoid the FW is full of zero*/
	rslt |= hx_s_core_fp_ol._flash_lastdata_check(size);
...
static void himax_boot_upgrade(struct work_struct *work)
{
...
#else
	if (hx_chk_flash_sts(hx_s_ic_data_ol->flash_size) == 1) {
		...
		hx_s_core_fp_ol._read_FW_ver();
		hx_s_core_fp_ol._tp_info_check();
	}
#endif
	...
	hx_s_core_fp_ol._bin_desc_get((unsigned char *)hxfw->data, HX1K);
...
{
	himax_chip_common_resume(hx_s_ts_ol);
}
...
void himax_get_panel(struct himax_platform_data *pdata, struct himax_ts_data *ts)
{
    ...
    if ((panel_id0 == 0) && (panel_id1 == 1)) {
    I(" DT: panel is lxd\n");
        ts->panel_name = 1;
    } else if ((panel_id0 == 0) && (panel_id1 == 0)) {
        I(" DT: panel is hjr\n");
        ts->panel_name = 2;
    } else {
        I(" DT: panel is OL\n");
...
int himax_chip_common_init(void)
{
	...
	I("hx_s_ic_data_ol START\n");
	hx_s_ic_data_ol = kzalloc(sizeof(struct himax_ic_data), GFP_KERNEL);
	if (hx_s_ic_data_ol == NULL) { /*Allocate IC data space*/
		err = -ENOMEM;
		goto err_dt_ic_data_fail;
	}
	memset(hx_s_ic_data_ol, 0xFF, sizeof(struct himax_ic_data));
	/* default 128k, different size please follow HX83121A style */
	hx_s_ic_data_ol->flash_size = 131072;

	/* allocate report data */
	hx_s_touch_data_ol = kzalloc(sizeof(struct himax_report_data), GFP_KERNEL);
	if (hx_s_touch_data_ol == NULL) {
		err = -ENOMEM;
	...
	if (himax_parse_dt_ol(ts, pdata) < 0) {
	...
	if (hx_s_core_fp_ol._chip_init != NULL) {
		hx_s_core_fp_ol._chip_init();
	} else {
		E("%s: function point of chip_init is NULL!\n", __func__);
		goto error_ic_detect_failed;
	}
#ifdef HX_PARSE_FROM_DT
	himax_parse_dt_ol_ic_info(ts, pdata);
#endif
	hx_s_core_fp_ol._touch_information();
	...
	himax_int_enable_ol(0);

#if defined(HX_ZERO_FLASH)
	g_update_cfg_buf = kzalloc(hx_s_ic_data_ol->dsram_sz *
		sizeof(uint8_t),
		GFP_KERNEL);
	...
	hx_s_core_fp_ol._calc_touch_data_size();
 ...
 */
	...
	if (himax_debug_init_ol()) {
		E(" %s: debug initial failed!\n", __func__);
		goto err_debug_init_failed;
	}
#endif

	ret = himax_input_register_ol(ts);
	if (ret) {
...
err_alloc_dt_pdata_failed:
	kfree(hx_s_touch_data_ol);
	hx_s_touch_data_ol = NULL;
err_alloc_touch_data_failed:
	kfree(hx_s_ic_data_ol);
	hx_s_ic_data_ol = NULL;
err_dt_ic_data_fail:
	kfree(pdata);
	pdata = NULL;
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	if (hx_s_debug_data_ol != NULL
		&& hx_s_debug_data_ol->flash_dump_going == true) {
	...
	if (ts->in_self_test == 1) {
		...
		if (hx_s_core_fp_ol._ap_notify_fw_sus != NULL)
			hx_s_core_fp_ol._ap_notify_fw_sus(1);
		ts->suspend_resume_done = 1;
		goto END;
	}

	hx_s_core_fp_ol._suspend_proc(ts->suspended);

	if (ts->SMWP_enable) {
		if (hx_s_core_fp_ol._0f_overlay != NULL)
			hx_s_core_fp_ol._0f_overlay(2, 0);
	...
	himax_int_enable_ol(0);
	if (hx_s_core_fp_ol._suspend_ic_action != NULL)
		hx_s_core_fp_ol._suspend_ic_action();
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (hx_s_core_fp_ol._ic_reset != NULL)
		hx_s_core_fp_ol._ic_reset(0);
#endif

	hx_s_core_fp_ol._resume_proc(ts->suspended);
	himax_report_all_leave_event(ts);
	himax_int_enable_ol(1);
/*
