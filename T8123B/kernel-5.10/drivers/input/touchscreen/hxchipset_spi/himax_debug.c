...
static int chk_slave_reg(char *str, int str_len)
{
	...
	hx_s_ts_ol->slave_read_reg = cal % 0x100;

	I("%s: str=%s, str_len=%d\n", __func__, str, str_len);
	I("%s: buf_read_slave_cmd=%s, read_cmd_len=%d, slave_read_reg=%d\n",
		__func__, buf_read_slave_cmd,
		read_cmd_len, hx_s_ts_ol->slave_read_reg);

	hx_s_ts_ol->select_slave_reg = true;
...
static int himax_crc_test_read(struct seq_file *m)
{
	...
	I("%s: now flash = %d\n", __func__, hx_s_ic_data_ol->flash_size);

	hx_s_core_fp_ol._sense_off(true);
	msleep(20);

	size = hx_s_ic_data_ol->flash_size;
	result = hx_s_core_fp_ol._calculateChecksum(false, size);
...
static int himax_layout_read(struct seq_file *m)
{
	...
	if (hx_s_debug_data_ol->is_call_help) {
		seq_printf(m, HELP_LAYOUT);
		hx_s_debug_data_ol->is_call_help = false;
	} else {
		seq_printf(m, "%d ",
				ts->pdata->abs_x_min);
		seq_printf(m, "%d ",
...
static ssize_t himax_layout_write(char *buf, size_t len)
{
	...
	if (strnstr(buf, "help", len) != NULL) {
		hx_s_debug_data_ol->is_call_help = true;
		goto END;
	} else {
...
static int test_irq_pin(void)
{
	...
	hx_parse_assign_cmd_ol(0x00000002, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(0x90028060, tmp_data, DATA_LEN_4);
	usleep_range(20000, 20001);
	hx_s_core_fp_ol._register_read(0x90028060, tmp_read, DATA_LEN_4);
	...
	hx_s_core_fp_ol._register_write(0x90028064, tmp_data, DATA_LEN_4);
	usleep_range(20000, 20001);
	hx_s_core_fp_ol._register_read(0x90028064, tmp_read, DATA_LEN_4);
	I("R%08XH = 0x%02X%02X%02X%02X\n",
		0x90028064,
		tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);

	usleep_range(20000, 20001);
	hx_parse_assign_cmd_ol(0x00000000, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(0x90028068, tmp_data, DATA_LEN_4);
	usleep_range(20000, 20001);
	hx_s_core_fp_ol._register_read(0x90028068, tmp_read, DATA_LEN_4);
	I("R%08XH = 0x%02X%02X%02X%02X\n",
		0x90028068,
		tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);
	...
	hx_s_debug_data_ol->is_checking_irq = false;
...
static ssize_t himax_int_en_write(char *buf, size_t len)
{
	...
	if (buf[0] == '0') {
		...
		hx_s_debug_data_ol->is_checking_irq = true;
...
static int himax_irq_info_read(struct seq_file *m)
{
	...
	if (hx_s_core_fp_ol._read_ic_trigger_type() == 1)
		seq_printf(m,
			"IC Interrupt type is edge trigger.\n");
	else if (hx_s_core_fp_ol._read_ic_trigger_type() == 0)
		seq_printf(m,
	...
	if (hx_s_ic_data_ol->int_is_edge)
		seq_printf(m,
...
static int himax_proc_register_read(struct seq_file *m)
{
	...
	if (hx_s_debug_data_ol->is_call_help) {
		...
		if (!hx_s_ts_ol->acc_slave_reg
			&& hx_s_ts_ol->select_slave_reg) {
			I("First read Slave, test it !\n");
			ret = himax_bus_read_ol(0x13, test_data, 1);
			if (ret < 0) {
				E("%s: bus access fail!\n", __func__);
			...
			hx_s_ts_ol->acc_slave_reg = true;

		}
		if (hx_s_ts_ol->select_slave_reg)
			seq_printf(m, "[Slave W:0x%02X, R:0x%02X]\n",
				hx_s_ts_ol->slave_write_reg,
				hx_s_ts_ol->slave_read_reg);
		...
		if (proc_reg_addr_type == 1) {
			ret = himax_bus_read_ol(proc_reg_addr[0], proc_reg_buf,
				128);
			if (ret < 0) {
				E("%s: bus access fail!\n", __func__);
				return BUS_FAIL;
			...
			addr32 = proc_reg_addr[3] << 24 |
					...
					proc_reg_addr[1] << 8 |
					proc_reg_addr[0];
			hx_s_core_fp_ol._register_read(addr32, proc_reg_buf,
				128);
		}
...
bus_fail_end:
	seq_puts(m, "\n");
	if (hx_s_ts_ol->select_slave_reg)
		hx_s_ts_ol->select_slave_reg = false;
	return ret;
}
...
static ssize_t himax_proc_register_write(char *buf, size_t len)
{
	...
	&& buf[1] == ':' && buf[2] == 'x') {
		...
		if (proc_reg_addr_type == 1) {
			ret = himax_bus_write_ol(proc_reg_addr[0],
				NUM_NULL,
				w_data,
				byte_length);
			if (ret < 0) {
			...
			addr32 = proc_reg_addr[3] << 24 |
					...
					proc_reg_addr[1] << 8 |
					proc_reg_addr[0];
			hx_s_core_fp_ol._register_write(addr32, w_data,
				byte_length / 2);
		}
...
int himax_set_diag_cmd(struct himax_ic_data *hx_s_ic_data_ol,
		struct himax_report_data *hx_s_touch_data_ol)
{
	...
	if (hx_s_touch_data_ol->diag_cmd >= 1 && hx_s_touch_data_ol->diag_cmd <= 7) {
		/* Check event stack CRC */
		if (!hx_s_core_fp_ol._diag_check_sum(hx_s_touch_data_ol))
			goto bypass_checksum_failed_packet;

#if defined(HX_TP_PROC_2T2R)
		if (Is_2T2R_OL && (hx_s_touch_data_ol->diag_cmd >= 4 &&
			hx_s_touch_data_ol->diag_cmd <= 6)) {
			mutual_data = getMutualBuffer_2();
			self_data = getSelfBuffer();
			/* initiallize the block number of mutual and self */
			mul_num = hx_s_ic_data_ol->rx_num_2 *
				hx_s_ic_data_ol->tx_num_2;
			self_num = hx_s_ic_data_ol->rx_num_2 +
				hx_s_ic_data_ol->tx_num_2;
		} else
#endif
		{
			...
			self_data = getSelfBuffer();
			/* initiallize the block number of mutual and self */
			mul_num = hx_s_ic_data_ol->rx_num * hx_s_ic_data_ol->tx_num;
			self_num = hx_s_ic_data_ol->rx_num + hx_s_ic_data_ol->tx_num;
		}
		hx_s_core_fp_ol._diag_parse_raw_data(hx_s_touch_data_ol, mul_num,
			self_num, hx_s_touch_data_ol->diag_cmd, mutual_data,
			self_data);
	} else if (hx_s_touch_data_ol->diag_cmd == 8) {
		memset(diag_coor, 0x00, sizeof(diag_coor));
		memcpy(&(diag_coor[0]), &hx_s_touch_data_ol->coord_buf[0],
		  hx_s_touch_data_ol->touch_info_size);
	}

	/* assign state info data */
	memcpy(&(state_info[0]), &hx_s_touch_data_ol->state_info[0], 2);
	return NO_ERR;
bypass_checksum_failed_packet:
	return 1;
}

int hx_dbg_raw_full_stack(struct himax_ic_data *hx_s_ic_data_ol,
		struct himax_report_data *hx_s_touch_data_ol)
{
	struct himax_ts_data *ts = hx_s_ts_ol;
	int32_t *mutual_data;
	int32_t *self_data;
	int x_channel = hx_s_ic_data_ol->rx_num;
	int y_channel = hx_s_ic_data_ol->tx_num;
	int mul_num;
	int self_num;
	int update_idx;
	int data_status = 0;
	...
	data_status = hx_s_core_fp_ol._diag_parse_raw_data(hx_s_touch_data_ol,
		mul_num,
		self_num, hx_s_touch_data_ol->diag_cmd, mutual_data,
		self_data);
	...
	memcpy(&g_rb_frame.rawdata[update_idx].self[0],
		&hx_s_touch_data_ol->rawdata_buf[4 + 2 * (x_channel * y_channel)],
		2 * (x_channel + y_channel) * sizeof(uint8_t));
	memcpy(&g_rb_frame.rawdata[update_idx].mutual[0],
		&hx_s_touch_data_ol->rawdata_buf[4],
		2 * (x_channel * y_channel) * sizeof(uint8_t));
...
void himax_log_touch_data(struct himax_ts_data *ts, int start)
{
	...
	if (print_size > hx_s_touch_data_ol->touch_all_size) {
...
void himax_log_touch_event(struct himax_ts_data *ts, int start)
{
	...
	/* finger log info */
	if (g_target_report_data_ol->finger_on > 0 &&
	...
	} else if (g_target_report_data_ol->finger_on == 0
	&& g_target_report_data_ol->finger_num == 0) {
		I("All Finger leave\n");
	} else {
	...
	if (hx_s_ic_data_ol->stylus_func) {
		...
		I("Hover status is %d!\n",
			g_target_report_data_ol->s[0].hover);
	}
}
...
static int himax_change_mode(uint8_t str_pw, uint8_t end_pw)
{
	...
	if (hx_s_core_fp_ol._assign_sorting_mode != NULL)
		hx_s_core_fp_ol._assign_sorting_mode(data);

	/*sense on*/
	hx_s_core_fp_ol._sense_on(1);
	/*wait mode change*/
	do {
		if (hx_s_core_fp_ol._check_sorting_mode != NULL)
			hx_s_core_fp_ol._check_sorting_mode(data);
...
static ssize_t himax_diag_cmd_write(char *buf, size_t len)
{
	...
	if (buf[0] == 'r' && buf[1] == 'b'
		&& hx_s_debug_data_ol->is_stack_full_raw == true){
		if (len > 0) {
	...
	case 1:/*raw out select - diag,X*/
		if (!kstrtoint(buf, 16, &rst)) {
			...
			if (dsram_flag) {
				...
				himax_int_enable_ol(1);
				hx_s_core_fp_ol._return_event_stack();
			}

			hx_s_core_fp_ol._diag_register_set(ts->diag_cmd,
					process_type, false);
			...
			if (ts->diag_cmd == 0) {
				...
				hx_s_debug_data_ol->is_stack_full_raw = false;
			}
...
void himax_get_mutual_edge(void)
{
	...
	for (i = 0; i < (hx_s_ic_data_ol->rx_num * hx_s_ic_data_ol->tx_num); i++) {
...
{
	if (transpose)
		seq_printf(s, "%6d", diag_mutual[j + i * hx_s_ic_data_ol->rx_num]);
	else
		seq_printf(s, "%6d", diag_mutual[i + j * hx_s_ic_data_ol->rx_num]);
}
...
bool himax_ts_diag_func(int dsram_type)
{
	...
	if (!hx_s_core_fp_ol._get_DSRAM_data(info_data, dsram_flag)) {
...
{
	int x_num = hx_s_ic_data_ol->rx_num;
	int y_num = hx_s_ic_data_ol->tx_num;
	size_t ret = 0;
...
static int himax_sram_read(struct seq_file *s, void *v, uint8_t rs)
{
	...
	int d_type;
	int current_size =
		((hx_s_ic_data_ol->tx_num + 1) * (hx_s_ic_data_ol->rx_num + 1)
		+ hx_s_ic_data_ol->tx_num + hx_s_ic_data_ol->rx_num) * 10 + 256;
	if (hx_s_debug_data_ol->is_call_help) {
		seq_puts(s, HELP_DIAG);
		hx_s_debug_data_ol->is_call_help = false;
	} else {
		...
		if (!overflow) {
			if (!process_type) {
				himax_int_enable_ol(0);
				hx_s_core_fp_ol._diag_register_set(d_type,
					0, false);
				...
				ts->diag_cmd = 0;
				hx_s_core_fp_ol._diag_register_set(0, 0, false);
				himax_int_enable_ol(1);
			}
		}
...
static void test_rst_pin(void)
{
	...
	usleep_range(20000, 20001);
	hx_parse_assign_cmd_ol(0x00000001, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(0x900000F0, tmp_data, DATA_LEN_4);
	usleep_range(20000, 20001);
	hx_s_core_fp_ol._register_read(0x900000F0, tmp_read, DATA_LEN_4);
	...
	usleep_range(20000, 20001);
	do {
		hx_parse_assign_cmd_ol(0x900000A8, tmp_addr, DATA_LEN_4);
		hx_s_core_fp_ol._register_read(0x900000A8, tmp_read, DATA_LEN_4);
		I("R%02X%02X%02X%02XH = 0x%02X%02X%02X%02X\n",
			tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0],
			tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);
		rst_sts1 = tmp_read[0];
	...
	hx_parse_assign_cmd_ol(0x900000F0, tmp_addr, DATA_LEN_4);
	hx_s_core_fp_ol._register_read(0x900000F0, tmp_read, DATA_LEN_4);
	I("R%02X%02X%02X%02XH = 0x%02X%02X%02X%02X\n",
		tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0],
		tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);
...
static ssize_t himax_reset_write(char *buf, size_t len)
{
	...
	if (strnstr(buf, "help", len) != NULL) {
		...
		if (buf[0] == '1')
			...
			hx_s_core_fp_ol._ic_reset(1);
		else if (buf[0] == '3')
			hx_s_core_fp_ol._ic_reset(2);
		else if (buf[0] == '4')
			hx_s_core_fp_ol._ic_reset(3);
		/* else if (buf[0] == '5') */
		/*	ESD_HW_REST(); */
...
void hx_dump_prog_set(uint8_t prog)
{
	...
	if (prog == ONGOING)
		hx_s_debug_data_ol->flash_dump_going = ONGOING;
	else
		hx_s_debug_data_ol->flash_dump_going = START;
}
...
static int himax_flash_dump_show(struct seq_file *s, void *v)
{
	...
	if (g_dump_size != 0)
		...
		dump_size = hx_s_ic_data_ol->flash_size;
 ...
 *	queue_work(hx_s_ts_ol->dump_wq, &hx_s_ts_ol->dump_work);
 *} else if (buf[0] == '2') {
...
void himax_ts_dump_func(void)
{
	...
	if (g_dump_cmd == 1) {
		himax_int_enable_ol(0);
		hx_s_core_fp_ol._flash_dump_func(g_dump_cmd, g_dump_size,
		  g_dump_buffer);
		g_flash_dump_rst = true;
		himax_int_enable_ol(1);
		...
		if (g_dump_addr != -948794) {
			...
			ret = hx_s_core_fp_ol._register_read(g_dump_addr,
...
static int himax_proc_guest_info_read(struct seq_file *m)
{
	...
	if (hx_s_debug_data_ol->is_call_help) {
		...
		if (hx_s_core_fp_ol.guest_info_get_status()) {
			seq_printf(m,
				"Not Ready\n");
			goto END_FUNCTION;
		} else {
...
static ssize_t himax_proc_guest_info_write(char *buf, size_t len)
{
	...
	if (strnstr(buf, "help", len) != NULL) {
		...
		if (buf[0] == 'r') {
			...
			queue_work(hx_s_ts_ol->guest_info_wq,
					&hx_s_ts_ol->guest_info_work);
		}
	}
	return len;
}
...
static int himax_test_bus_read(struct seq_file *m)
{
	...
	if (hx_s_core_fp_ol._read_i2c_status())
		seq_printf(m,
...
static int himax_info_read(struct seq_file *m)
{
	...
	if (hx_s_ts_ol->protocol_type == PROTOCOL_TYPE_A)
		seq_printf(m,
			"Protocol : TYPE_A\n");
	else
		seq_printf(m,
	...
	seq_printf(m,
		"BT Num : %d\n", hx_s_ic_data_ol->bt_num);
	seq_printf(m,
		"X Resolution : %d\n", hx_s_ic_data_ol->x_res);
	seq_printf(m,
		"Y Resolution : %d\n", hx_s_ic_data_ol->y_res);
	seq_printf(m,
		"Max Point : %d\n", hx_s_ic_data_ol->max_pt);
	seq_printf(m,
		"Stylus Support : %d\n", hx_s_ic_data_ol->stylus_func);
#if defined(HX_TP_PROC_2T2R)
	if (Is_2T2R_OL) {
		seq_printf(m,
			"2T2R panel\n");
...
static int himax_pen_info_read(struct seq_file *m)
{
	...
	seq_printf(m,
		"Pen ID:\n");
	seq_printf(m,
		"%llx\n", g_target_report_data_ol->s[0].id);
...
static int hx_node_update(char *buf, size_t is_len)
{
	...
	result = request_firmware(&fw, fileName, hx_s_ts_ol->dev);
	if (result < 0) {
	...
	hx_s_core_fp_ol._bin_desc_get((unsigned char *)fw->data, HX1K);

	result = hx_s_core_fp_ol._firmware_update_0f(fw, fw_type);
	if (result) {
		fw_update_complete = false;
	...
	if (hx_s_core_fp_ol._fts_ctpm_fw_upgrade_with_sys_fs(
...
{
	hx_s_debug_data_ol->_ts_dbg_func = himax_ts_dbg_func;
	hx_s_debug_data_ol->_set_diag_cmd = himax_set_diag_cmd;
	hx_s_debug_data_ol->_raw_full_stack = hx_dbg_raw_full_stack;
	hx_s_debug_data_ol->flash_dump_going = false;
	hx_s_debug_data_ol->is_checking_irq = false;
	hx_s_debug_data_ol->is_call_help = false;
	hx_s_debug_data_ol->is_stack_full_raw = false;
	hx_s_ts_ol->select_slave_reg = false;
	hx_s_ts_ol->acc_slave_reg = false;
}
...
{
	hx_s_core_fp_ol.read_guest_info();
}
#endif
...
static void himax_ts_diag_work_func(struct work_struct *work)
{
	...
	if (process_type != 0) {
		queue_delayed_work(hx_s_ts_ol->himax_diag_wq,
			&hx_s_ts_ol->himax_diag_delay_work, 1 / 10 * HZ);
	} else {
		dsram_flag = false;
	}
}
...
fail_4: remove_proc_entry(HIMAX_PROC_DEBUG_FILE, hx_touch_proc_dir_ol);
...
fail_2_1: remove_proc_entry(HIMAX_PROC_DIAG_FOLDER, hx_touch_proc_dir_ol);
	return -ENOMEM;
}
...
void himax_touch_proc_deinit(void)
{
	...
	remove_proc_entry(HIMAX_PROC_FLASH_DUMP_FILE, hx_touch_proc_dir_ol);
	remove_proc_entry(HIMAX_PROC_DEBUG_FILE, hx_touch_proc_dir_ol);
	...
	remove_proc_entry(HIMAX_PROC_DIAG_FOLDER, hx_touch_proc_dir_ol);
}
...
int himax_debug_init_ol(void)
{
	...
	hx_s_debug_data_ol = kzalloc(sizeof(struct himax_debug), GFP_KERNEL);
	if (hx_s_debug_data_ol == NULL) { /*Allocate debug data space*/
		E("%s: hx_s_debug_data_ol allocate failed\n", __func__);
		goto err_alloc_debug_data_fail;
	}

	himax_himax_data_init();

	g_dump_buffer = kcalloc(hx_s_ic_data_ol->flash_size,
		sizeof(uint8_t),
		GFP_KERNEL);
...
#if defined(HX_TP_PROC_2T2R)
	if (Is_2T2R_OL) {
		setMutualBuffer_2(hx_s_ic_data_ol->rx_num_2,
			hx_s_ic_data_ol->tx_num_2);
...
err_dump_buf_alloc_failed:
	kfree(hx_s_debug_data_ol);
	hx_s_debug_data_ol = NULL;
...
EXPORT_SYMBOL(himax_debug_init_ol);

int himax_debug_remove_ol(void)
{
	struct himax_ts_data *ts = hx_s_ts_ol;
...
EXPORT_SYMBOL(himax_debug_remove_ol);
