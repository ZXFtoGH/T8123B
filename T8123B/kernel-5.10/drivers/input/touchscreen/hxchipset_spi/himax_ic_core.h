...
extern struct himax_debug *hx_s_debug_data_ol;
...
#if defined(HX_EXCP_RECOVERY)
extern u8 HX_EXCP_RESET_ACTIVATE_OL;
#endif
...
extern struct ic_setup_collect hx_s_ic_setup_ol;
...
/* CORE_FW */
	void (*_parse_raw_data)(struct himax_report_data *hx_s_touch_data_ol,
			int mul_num,
			int self_num,
			uint8_t diag_cmd,
			int16_t *mutual_data,
...
/* CORE_DRIVER */
	...
	bool (*_diag_check_sum)(struct himax_report_data *hx_s_touch_data_ol);
	int (*_diag_parse_raw_data)(struct himax_report_data *hx_s_touch_data_ol,
			int mul_num,
			int self_num,
			uint8_t diag_cmd,
			int32_t *mutual_data,
...
/*Inspection*/
	void (*_get_noise_base)(uint8_t checktype);
...
extern bool himax_mcu_diag_check_sum(struct himax_report_data *hx_s_touch_data_ol);
