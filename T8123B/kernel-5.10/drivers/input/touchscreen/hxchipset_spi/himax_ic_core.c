...
void himax_mcu_leave_safe_mode(void)
{
	...
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_leave_safe_mode,
		tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_leave_safe_mode,
		tmp_data, DATA_LEN_4);
}
		...
		uint8_t hb, uint8_t lb)
{
	...
	while (retry++ < 40) { /* ceil[16.6*2] */
		hx_s_core_fp_ol._register_read(tmp_addr, r_data, DATA_LEN_4);
		if (r_data[1] == lb && r_data[0] == hb)
			break;
		else if (r_data[1] == hb && r_data[0] == lb)
			return NO_ERR;

		hx_s_core_fp_ol._register_write(tmp_addr, tmp_data, DATA_LEN_4);
...
void himax_mcu_interface_on(void)
{
	...
	ret = himax_bus_read_ol(ADDR_AHB_RDATA_BYTE_0,
		tmp_data, DATA_LEN_4);
	...
	do {
		hx_parse_assign_cmd_ol(ADDR_AHB_CONTINOUS, tmp_addr, DATA_LEN_1);
		hx_parse_assign_cmd_ol(PARA_AHB_CONTINOUS, tmp_data, DATA_LEN_1);
		ret = himax_bus_write_ol(tmp_addr[0], NUM_NULL, tmp_data, 1);
		if (ret < 0) {
			E("%s: bus access fail!\n", __func__);
			return;
		}

		hx_parse_assign_cmd_ol(ADDR_AHB_INC4, tmp_addr, DATA_LEN_1);
		hx_parse_assign_cmd_ol(PARA_AHB_INC4, tmp_data, DATA_LEN_1);
		ret = himax_bus_write_ol(tmp_addr[0], NUM_NULL, tmp_data, 1);
		if (ret < 0) {
			E("%s: bus access fail!\n", __func__);
			return;
		...
		/*Check cmd*/
		himax_bus_read_ol(ADDR_AHB_CONTINOUS, tmp_data, 1);

		himax_bus_read_ol(ADDR_AHB_INC4, tmp_data2, 1);
...
bool himax_mcu_wait_wip(int Timing)
{
	...
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_trans_fmt,
		tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_flash_spi200_trans_fmt,
		tmp_data, DATA_LEN_4);
	tmp_data[0] = 0x01;

	do {
		hx_parse_assign_cmd_ol(
			hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_1,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
			tmp_data, DATA_LEN_4);

		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_cmd_1,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_cmd,
			tmp_data, DATA_LEN_4);

		tmp_data[0] = tmp_data[1] = tmp_data[2] = tmp_data[3] = 0xFF;
		hx_s_core_fp_ol._register_read(
			hx_s_ic_setup_ol._addr_flash_spi200_data,
			tmp_data, 4);
...
void himax_mcu_init_psl(void)
{
	...
	hx_parse_assign_cmd_ol(DATA_CLEAR, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_psl, tmp_data,
		sizeof(tmp_data));
...
void himax_mcu_power_on_init(void)
{
	...
	hx_parse_assign_cmd_ol(DATA_CLEAR, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_rawout_sel,
		tmp_data, DATA_LEN_4);
	/*DSRAM func initial*/
	hx_s_core_fp_ol._assign_sorting_mode(tmp_data);
	/*N frame initial*/
	/* reset N frame back to default value 1 for normal mode */
	hx_parse_assign_cmd_ol(0x00000001, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_set_frame,
		tmp_data, 4);
	/*FW reload done initial*/
	hx_parse_assign_cmd_ol(DATA_CLEAR, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_chk_fw_reload2,
		tmp_data, DATA_LEN_4);

	hx_s_core_fp_ol._sense_on(0x00);

	I("%s: waiting for FW reload data\n", __func__);

	while (retry++ < 30) {
		// hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_chk_fw_reload2,
		// tmp_addr, DATA_LEN_4);
		// hx_s_core_fp_ol._register_read(tmp_addr, tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_chk_fw_reload2,
			tmp_data, DATA_LEN_4);
		...
		hx_s_core_fp_ol._read_FW_status();
...
/* CORE_FW */
/* FW side start*/
static int diag_mcu_parse_raw_data(struct himax_report_data *hx_s_touch_data_ol,
		...
		int32_t *mutual_data, int32_t *self_data)
{
	...
	if (!hx_s_debug_data_ol->is_stack_full_raw) {
		if (hx_s_touch_data_ol->rawdata_buf[0]
		== hx_s_ic_setup_ol._data_rawdata_ready_lb
		&& hx_s_touch_data_ol->rawdata_buf[1]
		== hx_s_ic_setup_ol._data_rawdata_ready_hb
		&& hx_s_touch_data_ol->rawdata_buf[2] > 0
		&& hx_s_touch_data_ol->rawdata_buf[3] == diag_cmd) {
			RawDataLen_word = hx_s_touch_data_ol->rawdata_size / 2;
			index = (hx_s_touch_data_ol->rawdata_buf[2] - 1)
				* RawDataLen_word;
		...
		checksum += (((int8_t)hx_s_touch_data_ol->
					rawdata_buf[1]) * 256
					+ hx_s_touch_data_ol->
					rawdata_buf[0]);
		checksum += (((int8_t)hx_s_touch_data_ol->
...
uint32_t himax_mcu_calc_crc_by_hw(uint8_t *start_addr, int reload_length)
{
	...
	ret = hx_s_core_fp_ol._register_write(
		hx_s_ic_setup_ol._addr_reload_addr_from,
	...
	tmp_data[0] = length;
	ret = hx_s_core_fp_ol._register_write(
		hx_s_ic_setup_ol._addr_reload_addr_cmd_beat,
		tmp_data, DATA_LEN_4);
	if (ret < NO_ERR) {
		E("%s: bus access fail!\n", __func__);
		return HW_CRC_FAIL;
	...
	do {
		ret = hx_s_core_fp_ol._register_read(
			hx_s_ic_setup_ol._addr_reload_status,
			tmp_data, DATA_LEN_4);
		if (ret < NO_ERR) {
			E("%s: bus access fail!\n", __func__);
			return HW_CRC_FAIL;
		...
		if ((tmp_data[0] & 0x01) != 0x01) {
			ret = hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._addr_reload_crc32_result,
				tmp_data, DATA_LEN_4);
			if (ret < NO_ERR) {
				E("%s: bus access fail!\n", __func__);
				return HW_CRC_FAIL;
...
int himax_mcu_ulpm_in(void)
{
	...
	do {
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_ulpm_11,
			tmp_write, DATA_LEN_1);
		if (himax_bus_write_ol(hx_s_ic_setup_ol._addr_ulpm_34,
			NUM_NULL,
			tmp_write, 1) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read_ol(hx_s_ic_setup_ol._addr_ulpm_34,
			tmp_data, 1) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
		...
		rtimes++;
	} while (tmp_data[0] != hx_s_ic_setup_ol._data_ulpm_11);
	...
	do {
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_ulpm_33,
			tmp_write, DATA_LEN_1);
		if (himax_bus_write_ol(hx_s_ic_setup_ol._addr_ulpm_33,
			NUM_NULL,
			tmp_write, 1) < 0) {
			I("%s: spi write fail!\n", __func__);
			continue;
		}
		if (himax_bus_read_ol(hx_s_ic_setup_ol._addr_ulpm_33,
			tmp_data, 1) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
		...
		I("%s:retry times %d,addr=0x33,correct 0x33=current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
	} while (tmp_data[0] != hx_s_ic_setup_ol._data_ulpm_33);
	...
	do {
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_ulpm_22,
			tmp_write, DATA_LEN_1);
		if (himax_bus_write_ol(hx_s_ic_setup_ol._addr_ulpm_34,
			NUM_NULL,
			tmp_write, 1) < 0) {
		...
		I("%s:retry times %d,addr=0x34,correct 0x22=current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
	} while (tmp_data[0] != hx_s_ic_setup_ol._data_ulpm_22);
	...
	do {
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_ulpm_aa,
			tmp_write, DATA_LEN_1);
		if (himax_bus_write_ol(hx_s_ic_setup_ol._addr_ulpm_33,
			NUM_NULL,
			tmp_write, 1) < 0) {
		...
		I("%s:retry times %d,addr=0x33, correct 0xAA=current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
	} while (tmp_data[0] != hx_s_ic_setup_ol._data_ulpm_aa);
...
int himax_mcu_black_gest_ctrl(bool enable)
{
	...
	I("%s:enable=%d, ts->is_suspended=%d\n", __func__,
			enable, hx_s_ts_ol->suspended);

	if (hx_s_ts_ol->suspended) {
		if (enable) {
#if defined(HX_RST_PIN_FUNC)
			hx_s_core_fp_ol._pin_reset();
#else
			I("%s: Please enable TP reset define\n", __func__);
#endif
		} else {
			hx_s_core_fp_ol._ulpm_in();
		}
	} else {
		hx_s_core_fp_ol._sense_on(0);
	}
	return ret;
}
#endif
...
void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		if (SMWP_enable) {
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				tmp_data, 4);
			hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._func_smwp,
				tmp_data, DATA_LEN_4);
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				back_data, 4);
		} else {
			hx_parse_assign_cmd_ol(
				hx_s_ic_setup_ol._func_dis,
				tmp_data,
				4);
			hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._func_smwp,
				tmp_data, DATA_LEN_4);
		...
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._func_smwp, tmp_data,
			DATA_LEN_4);
...
void himax_mcu_set_HSEN_enable(uint8_t HSEN_enable, bool suspended)
{
	...
	do {
		if (HSEN_enable) {
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				tmp_data, 4);
			hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._func_hsen,
				tmp_data, DATA_LEN_4);
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				back_data, 4);
		} else {
		...
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._func_hsen, tmp_data,
			DATA_LEN_4);
	...
	do {
		if (cable_config[1] == 0x01) {
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				tmp_data, 4);
			hx_s_core_fp_ol._register_write(
				hx_s_ic_setup_ol._func_usb_detect,
				tmp_data, DATA_LEN_4);
			hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_en,
				back_data, 4);
		...
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._func_usb_detect,
			tmp_data,
			DATA_LEN_4);
		...
		uint8_t storage_type, bool is_dirly)
{
	...
	hx_s_core_fp_ol._interface_on();
	tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0x00;
	do {
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_rawout_sel,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_rawout_sel,
			back_data,
			DATA_LEN_4);
		I(PRT_DATA,	__func__,
			back_data[3],
	...
	hx_s_ts_ol->diag_storage_type = storage_type;
	hx_s_ts_ol->diag_dirly = is_dirly;
}
...
void himax_mcu_idle_mode(int disable)
{
	...
	do {
		I("%s,now %d times!\n", __func__, retry);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_fw_mode_status,
			tmp_data, DATA_LEN_4);

		if (disable)
			switch_cmd = hx_s_ic_setup_ol._para_idle_dis;
		else
			switch_cmd = hx_s_ic_setup_ol._para_idle_en;

		tmp_data[0] = switch_cmd;
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_fw_mode_status,
			tmp_data, DATA_LEN_4);
...
void himax_mcu_reload_disable(int disable)
{
	...
	if (disable) { /*reload disable*/
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_fw_reload_dis,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_chk_fw_reload,
			tmp_data, DATA_LEN_4);
	} else { /*reload enable*/
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_fw_reload_en,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_chk_fw_reload,
			tmp_data, DATA_LEN_4);
	}
...
int himax_mcu_read_ic_trigger_type(void)
{
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_chk_irq_edge,
		tmp_data, DATA_LEN_4);
...
void himax_mcu_read_FW_ver(void)
{
	...
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_fw_ver,
		data, DATA_LEN_4);
	hx_s_ic_data_ol->vendor_panel_ver =  data[0];
	hx_s_ic_data_ol->vendor_fw_ver = data[1] << 8 | data[2];
	I("PANEL_VER : %X\n", hx_s_ic_data_ol->vendor_panel_ver);
	I("FW_VER : %X\n", hx_s_ic_data_ol->vendor_fw_ver);

	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_fw_cfg,
		data, DATA_LEN_4);
	hx_s_ic_data_ol->vendor_config_ver = data[2] << 8 | data[3];
	/*I("CFG_VER : %X\n",hx_s_ic_data_ol->vendor_config_ver);*/
	hx_s_ic_data_ol->vendor_touch_cfg_ver = data[2];
	I("TOUCH_VER : %X\n", hx_s_ic_data_ol->vendor_touch_cfg_ver);
	hx_s_ic_data_ol->vendor_display_cfg_ver = data[3];
	I("DISPLAY_VER : %X\n", hx_s_ic_data_ol->vendor_display_cfg_ver);
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_fw_vendor, data,
		DATA_LEN_4);
	hx_s_ic_data_ol->vendor_cid_maj_ver = data[2];
	hx_s_ic_data_ol->vendor_cid_min_ver = data[3];
	I("CID_VER : %X\n", (hx_s_ic_data_ol->vendor_cid_maj_ver << 8
			| hx_s_ic_data_ol->vendor_cid_min_ver));

	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_cus_info, data, 12);
	memcpy(hx_s_ic_data_ol->vendor_cus_info, data, 12);
	I("Cusomer ID = %s\n", hx_s_ic_data_ol->vendor_cus_info);

	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_proj_info, data, 12);
	memcpy(hx_s_ic_data_ol->vendor_proj_info, data, 12);
	I("Project ID = %s\n", hx_s_ic_data_ol->vendor_proj_info);
#if IS_ENABLED(CONFIG_EMDOOR_DEVICEINFO)
    if(hw_dev) {
        strcpy(hw_dev->dev_ctp.model, "HX83102J");
        strcpy(hw_dev->dev_ctp.supplier, "LXD");
        strcpy(hw_dev->dev_ctp.tclid, "NA");
        sprintf(hw_dev->dev_ctp.param, "0x%X", hx_s_ic_data_ol->vendor_touch_cfg_ver);
    }
#endif
}
...
bool himax_mcu_read_event_stack(uint8_t *buf, uint32_t length)
{
	...
	if (hx_s_ts_ol->debug_log_level & BIT(2))
		time_func(&t_start);

	himax_bus_read_ol(hx_s_ic_setup_ol._addr_event_stack, buf, length);
	...
	if (hx_s_ts_ol->debug_log_level & BIT(2)) {
		...
		hx_s_ts_ol->bus_speed = (len * 9 * 1000000
...
void himax_mcu_return_event_stack(void)
{
	...
	do {
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_rawdata_end,
			tmp_data, DATA_LEN_4);
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_rawdata_end,
			target_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_rawdata_buf,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_rawdata_buf,
			tmp_data, DATA_LEN_4);
		retry--;
		usleep_range(10000, 11000);
...
bool himax_mcu_calculateChecksum(bool change_iref, uint32_t size)
{
	...
	CRC_result = hx_s_core_fp_ol._calc_crc_by_hw(tmp_data, size);
	msleep(50);
...
void himax_mcu_read_FW_status(void)
{
	...
	for (i = 0; i < len; i++) {
		hx_parse_assign_cmd_ol(dbg_reg_ary[i], addr, 4);
		hx_s_core_fp_ol._register_read(dbg_reg_ary[i], data, DATA_LEN_4);
...
{
	if (switch_on) {
		if (hx_s_ts_ol->use_irq)
			himax_int_enable_ol(switch_on);
		else
			hrtimer_start(&hx_s_ts_ol->timer, ktime_set(1, 0),
				HRTIMER_MODE_REL);
		...
		if (hx_s_ts_ol->use_irq)
			...
			hrtimer_cancel(&hx_s_ts_ol->timer);
			cancel_work_sync(&hx_s_ts_ol->work);
		}
	}
}
...
int himax_mcu_assign_sorting_mode(uint8_t *tmp_data)
{
	...
	I("%s:addr: 0x%08X, write to:0x%02X%02X%02X%02X\n",
		__func__,
		hx_s_ic_setup_ol._addr_sorting_mode_en,
		tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
	...
	while (retry++ < 3) {
		hx_s_core_fp_ol._register_write(
			...
			tmp_data, DATA_LEN_4);
		usleep_range(1000, 1100);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_sorting_mode_en,
			rdata, DATA_LEN_4);
...
int himax_mcu_check_sorting_mode(uint8_t *tmp_data)
{
	...
	ret = hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_sorting_mode_en,
		tmp_data,
		DATA_LEN_4);
...
void hx_ap_notify_fw_sus(int suspend)
{
	...
	if (suspend) {
		...
		I("%s,NonSuspend mode!\n", __func__);
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._func_dis,
			data_tmp, DATA_LEN_4);
	}

	I("%s: R%08XH<-0x%02X%02X%02X%02X\n",
		__func__,
		hx_s_ic_setup_ol._func_ap_notify_fw_sus,
	...
	do {
		...
		read_sts = hx_s_core_fp_ol._register_read(
			hx_s_ic_setup_ol._func_ap_notify_fw_sus,
			read_tmp,
			sizeof(read_tmp));
...
void himax_mcu_chip_erase(void)
{
	...
	if (hx_s_core_fp_ol._init_psl != NULL)
		hx_s_core_fp_ol._init_psl();

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_trans_fmt,
		tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(
		hx_s_ic_setup_ol._addr_flash_spi200_trans_fmt,
		tmp_data, DATA_LEN_4);

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_2,
			tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(
		hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
		tmp_data, DATA_LEN_4);

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_cmd_2,
			tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_flash_spi200_cmd,
		tmp_data, DATA_LEN_4);

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_cmd_3,
		tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_flash_spi200_cmd,
		tmp_data, DATA_LEN_4);
	...
	if (!hx_s_core_fp_ol._wait_wip(100))
		E("%s: Chip_Erase Fail\n", __func__);
...
bool himax_mcu_block_erase(int start_addr, int length)
{
	...
	page_prog_start = page_prog_start + block_size) {
		...
		hx_parse_assign_cmd_ol(
			hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_2,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
			tmp_data, DATA_LEN_4);
		...
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_addr,
			tmp_data, DATA_LEN_4);

		hx_parse_assign_cmd_ol(
			hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_3,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
			tmp_data, DATA_LEN_4);

		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_cmd_4,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_cmd,
			tmp_data, DATA_LEN_4);
...
bool himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	page_prog_start += FLASH_RW_MAX_LEN) {
	    ...
	    /* ===WEL Write Control ===*/
		hx_parse_assign_cmd_ol(
			hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_6,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
			tmp_data,
		...
		hx_parse_assign_cmd_ol(
			hx_s_ic_setup_ol._data_flash_spi200_trans_ctrl_4,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_trans_ctrl,
			tmp_data, DATA_LEN_4);
		...
		if (hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_data,
		...
		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_flash_spi200_cmd_6,
			tmp_data, DATA_LEN_4);
		hx_s_core_fp_ol._register_write(
			hx_s_ic_setup_ol._addr_flash_spi200_cmd,
			tmp_data, DATA_LEN_4);
		...
		if (!hx_s_core_fp_ol._wait_wip(1)) {
...
static void himax_flash_speed_set(uint8_t speed)
{
	...
	hx_s_core_fp_ol._register_write(
		hx_s_ic_setup_ol._addr_flash_spi200_flash_speed,
		tmp_data, 4);
}
		...
		int len, bool change_iref)
{
	...
	hx_s_core_fp_ol._ic_reset(0);

	for (counter = 0; counter < 3; counter++) {
		hx_s_core_fp_ol._sense_off(true);
		himax_flash_speed_set(HX_FLASH_SPEED_12p5M);
		hx_s_core_fp_ol._block_erase(0x00, len);
		if (!hx_s_core_fp_ol._flash_programming(fw, len)) {
			hx_s_core_fp_ol._ic_reset(0);

			continue;
		}

		hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_program_reload_from,
			tmp_addr, DATA_LEN_4);
		if (hx_s_core_fp_ol._calc_crc_by_hw(tmp_addr,
			len) == 0) {
			burnFW_success = 1;
			break;
		}
		...
		int Flash_Size, uint8_t *flash_buffer)
{
	...
	page_prog_start += 128) {
		hx_s_core_fp_ol._register_read(page_prog_start,
		flash_buffer+page_prog_start, 128);
	}

	hx_s_core_fp_ol._sense_on(0x01);
}
...
bool himax_mcu_flash_lastdata_check(uint32_t size)
{
	...
	temp_addr = temp_addr + flash_page_len) {
		...
		hx_s_core_fp_ol._register_read(temp_addr, &flash_tmp_buffer[0],
			flash_page_len);
	}
...
bool hx_mcu_bin_desc_get(unsigned char *fw, uint32_t max_sz)
{
	...
	do {
		...
		/*Get related data*/
		keep_on_flag = hx_s_core_fp_ol._bin_desc_data_get(addr_t, fw_buf);
...
int hx_mcu_diff_overlay_flash(void)
{
	...
	diff_val = (hx_s_ic_data_ol->vendor_fw_ver);
...
bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	uint8_t x_num = hx_s_ic_data_ol->rx_num;
	uint8_t y_num = hx_s_ic_data_ol->tx_num;
	/*int m_key_num = 0;*/
	...
	if (strcmp(hx_s_ts_ol->chip_name, HX_83121A_SERIES_PWON) == 0) {
	...
	/* m_key_num = hx_s_ic_data_ol->bt_num; */
	/* I("%s,m_key_num=%d\n",__func__ ,m_key_num); */
	/* total_size += m_key_num * 2; */

	/* 2. Start DSRAM Rawdata and Wait Data Ready */
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._pwd_get_rawdata_start,
			tmp_data, DATA_LEN_4);
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._pwd_get_rawdata_end,
			end_data, DATA_LEN_4);
	fw_run_flag = himax_write_read_reg(hx_s_ic_setup_ol._addr_rawdata_buf,
			tmp_data,
			end_data[1],
			end_data[0]);
	...
	while (retry++ < 5) {
		...
		while (remain_size > 0) {
			...
			addr = hx_s_ic_setup_ol._addr_rawdata_buf + i;


			if (remain_size >= max_bus_size) {
				hx_s_core_fp_ol._register_read(addr,
...
void himax_resume_proc(bool suspended)
{
	...
	I("%s: Entering!\n", __func__);
#if defined(HX_ZERO_FLASH)
	if (hx_s_core_fp_ol._0f_op_file_dirly != NULL) {
		result = hx_s_core_fp_ol._0f_op_file_dirly(g_fw_boot_upgrade_name);
		if (result)
			E("%s: update FW fail, code[%d]!!\n", __func__, result);
	}
#else
	if (hx_s_core_fp_ol._resend_cmd_func != NULL)
		hx_s_core_fp_ol._resend_cmd_func(suspended);
#endif

	if (hx_s_core_fp_ol._ap_notify_fw_sus != NULL)
		hx_s_core_fp_ol._ap_notify_fw_sus(0);
}
...
void himax_mcu_ic_reset(int level)
{
	...
	case 0:
		...
		himax_mcu_pin_reset();
#else
		if (hx_s_core_fp_ol._system_reset != NULL)
			hx_s_core_fp_ol._system_reset();
		else
	...
	case 1:
		...
		break;
	case 2:
		hx_s_core_fp_ol._leave_safe_mode();
		break;
	case 3:
		hx_s_core_fp_ol._sense_on(0x01);
		break;
...
uint8_t himax_mcu_tp_info_check(void)
{
	...
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_info_channel_num,
		addr, DATA_LEN_4);
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_info_channel_num,
		data, DATA_LEN_4);
	rx_num = data[2];
	tx_num = data[3];

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_info_max_pt, addr, DATA_LEN_4);
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_info_max_pt,
		data, DATA_LEN_4);
	max_pt = data[0];

	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_chk_irq_edge, addr, DATA_LEN_4);
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_chk_irq_edge,
		data, DATA_LEN_4);
	if ((data[1] & 0x01) == 1)
		int_is_edge = true;
	...
	/*1. Read number of MKey R100070E8H to determin data size*/
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._addr_mkey, addr, DATA_LEN_4);
	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_mkey, data, DATA_LEN_4);

	bt_num = data[0] & 0x03;


	hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._addr_info_def_stylus,
		data, DATA_LEN_4);
	stylus_func = data[3];

	if (hx_s_ic_data_ol->rx_num != rx_num) {
		err_cnt++;
		W("%s: RX_NUM, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->rx_num, rx_num);
	}

	if (hx_s_ic_data_ol->tx_num != tx_num) {
		err_cnt++;
		W("%s: TX_NUM, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->tx_num, tx_num);
	}

	if (hx_s_ic_data_ol->bt_num != bt_num) {
		err_cnt++;
		W("%s: BT_NUM, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->bt_num, bt_num);
	}

	if (hx_s_ic_data_ol->max_pt != max_pt) {
		err_cnt++;
		W("%s: MAX_PT, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->max_pt, max_pt);
	}

	if (hx_s_ic_data_ol->int_is_edge != int_is_edge) {
		err_cnt++;
		W("%s: INT_IS_EDGE, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->int_is_edge, int_is_edge);
	}

	if (hx_s_ic_data_ol->stylus_func != stylus_func) {
		err_cnt++;
		W("%s: STYLUS_FUNC, Set = %d ; FW = %d", __func__,
			hx_s_ic_data_ol->stylus_func, stylus_func);
	}

	if (hx_s_ic_data_ol->stylus_func) {
		hx_s_core_fp_ol._register_read(
			hx_s_ic_setup_ol._addr_info_stylus_ratio,
			data, DATA_LEN_4);
		stylus_id_v2 = data[2];/*0x100071FE 0=off 1=on*/
		stylus_ratio = data[3];
		/*0x100071FF 0=ratio_1 10=ratio_10*/
		if (hx_s_ic_data_ol->stylus_id_v2 != stylus_id_v2) {
			err_cnt++;
			W("%s: STYLUS_ID_V2, Set = %d ; FW = %d", __func__,
				hx_s_ic_data_ol->stylus_id_v2, stylus_id_v2);
		}
		if (hx_s_ic_data_ol->stylus_ratio != stylus_ratio) {
			err_cnt++;
			W("%s: STYLUS_RATIO, Set = %d ; FW = %d", __func__,
				hx_s_ic_data_ol->stylus_ratio, stylus_ratio);
		}
	}
...
{
	if (hx_s_ic_data_ol->rx_num == 0xFFFFFFFF)
		hx_s_ic_data_ol->rx_num = FIX_HX_RX_NUM;

	if (hx_s_ic_data_ol->tx_num == 0xFFFFFFFF)
		hx_s_ic_data_ol->tx_num = FIX_HX_TX_NUM;

	if (hx_s_ic_data_ol->bt_num == 0xFFFFFFFF)
		hx_s_ic_data_ol->bt_num = FIX_HX_BT_NUM;

	if (hx_s_ic_data_ol->max_pt == 0xFFFFFFFF)
		hx_s_ic_data_ol->max_pt = FIX_HX_MAX_PT;

	if (hx_s_ic_data_ol->int_is_edge == 0xFF)
		hx_s_ic_data_ol->int_is_edge = FIX_HX_INT_IS_EDGE;

	if (hx_s_ic_data_ol->stylus_func == 0xFF)
		hx_s_ic_data_ol->stylus_func = FIX_HX_STYLUS_FUNC;

	if (hx_s_ic_data_ol->stylus_id_v2 == 0xFF)
		hx_s_ic_data_ol->stylus_id_v2 = FIX_HX_STYLUS_ID_V2;

	if (hx_s_ic_data_ol->stylus_ratio == 0xFF)
		hx_s_ic_data_ol->stylus_ratio = FIX_HX_STYLUS_RATIO;

	hx_s_ic_data_ol->y_res = hx_s_ts_ol->pdata->screenHeight;
	hx_s_ic_data_ol->x_res = hx_s_ts_ol->pdata->screenWidth;

	I("%s:rx_num =%d,tx_num =%d\n", __func__,
		hx_s_ic_data_ol->rx_num, hx_s_ic_data_ol->tx_num);
	I("%s:max_pt=%d\n", __func__, hx_s_ic_data_ol->max_pt);
	I("%s:HX_Y_RES=%d,HX_X_RES =%d\n", __func__,
		hx_s_ic_data_ol->y_res, hx_s_ic_data_ol->x_res);
	I("%s:int_is_edge =%d,stylus_func = %d\n", __func__,
	hx_s_ic_data_ol->int_is_edge, hx_s_ic_data_ol->stylus_func);
		I("%s:HX_STYLUS_ID_V2 =%d,HX_STYLUS_RATIO = %d\n", __func__,
	hx_s_ic_data_ol->stylus_id_v2, hx_s_ic_data_ol->stylus_ratio);
}

void himax_mcu_calcTouchDataSize(void)
{
	struct himax_ts_data *ts_data = hx_s_ts_ol;

	ts_data->x_channel = hx_s_ic_data_ol->rx_num;
	ts_data->y_channel = hx_s_ic_data_ol->tx_num;
	ts_data->nFinger_support = hx_s_ic_data_ol->max_pt;

	HX_TOUCH_INFO_POINT_CNT = hx_s_ic_data_ol->max_pt * 4;
	if ((hx_s_ic_data_ol->max_pt % 4) == 0)
		HX_TOUCH_INFO_POINT_CNT +=
			(hx_s_ic_data_ol->max_pt / 4) * 4;
	else
		HX_TOUCH_INFO_POINT_CNT +=
			((hx_s_ic_data_ol->max_pt / 4) + 1) * 4;

	if (himax_report_data_init_ol())
...
bool himax_mcu_diag_check_sum(struct himax_report_data *hx_s_touch_data_ol)
{
	...
	i < (hx_s_touch_data_ol->touch_all_size
	- hx_s_touch_data_ol->touch_info_size);
	i += 2) {
		check_sum_cal += (hx_s_touch_data_ol->rawdata_buf[i + 1]
			* FLASH_RW_MAX_LEN
			+ hx_s_touch_data_ol->rawdata_buf[i]);
	}
...
int himax_mcu_diag_parse_raw_data(
		struct himax_report_data *hx_s_touch_data_ol,
		int mul_num, int self_num, uint8_t diag_cmd,
		int32_t *mutual_data, int32_t *self_data)
{
	return	diag_mcu_parse_raw_data(hx_s_touch_data_ol, mul_num, self_num,
		...
		uint8_t *flash_tmp_buffer)
{
	...
	result = hx_s_core_fp_ol._calc_crc_by_hw(tmp_addr, flash_page_len);
	I("Checksum = 0x%8X\n", result);
	if (result != 0)
		goto END_FUNC;
	...
	temp_addr = temp_addr + 128) {
		...
		hx_s_core_fp_ol._register_read(temp_addr,
...
int hx_read_guest_info(void)
{
	...
	hx_s_core_fp_ol._sense_off(true);
	/* hx_s_core_fp_ol._burst_enable(1); */
	...
	/* himax_burst_enable(hx_s_ts_ol->client, 0); */
	hx_s_core_fp_ol._sense_on(0x01);
...
void himax_mcu_resend_cmd_func(bool suspended)
{
	...
	hx_s_core_fp_ol._set_SMWP_enable(ts->SMWP_enable, suspended);
#endif
#if defined(HX_HIGH_SENSE)
	hx_s_core_fp_ol._set_HSEN_enable(ts->HSEN_enable, suspended);
#endif
...
int hx_turn_on_mp_func(int on)
{
	...
	/* char *tmp_chipname = hx_s_ts_ol->chip_name; */

	if (strcmp(HX_83102D_SERIES_PWON, hx_s_ts_ol->chip_name) == 0) {
		if (on) {
			I("%s : Turn on MPAP mode!\n", __func__);
			hx_parse_assign_cmd_ol(
				hx_s_ic_setup_ol._data_ctrl_mpap_ovl_on,
				tmp_data, sizeof(tmp_data));
			do {
				hx_s_core_fp_ol._register_write(
					hx_s_ic_setup_ol._addr_ctrl_mpap_ovl,
					tmp_data, DATA_LEN_4);
			...
			hx_parse_assign_cmd_ol(DATA_CLEAR, tmp_data,
				sizeof(tmp_data));
			do {
				hx_s_core_fp_ol._register_write(
					hx_s_ic_setup_ol._data_ctrl_mpap_ovl_on,
...
void himax_mcu_clean_sram_0f(uint32_t addr, int write_len, int type)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			hx_s_core_fp_ol._register_write(address,
	...
	const uint8_t *data, uint32_t len)
{
	...
	while (remain_len > 0) {
		...
		if (remain_len > max_bus_size) {
			hx_s_core_fp_ol._register_write(address, (uint8_t *)data+i,
				max_bus_size);
...
int himax_sram_write_crc_check(uint8_t *addr, const uint8_t *data, uint32_t len)
{
	...
	do {
		hx_s_core_fp_ol._write_sram_0f(addr, data, len);
		crc = hx_s_core_fp_ol._calc_crc_by_hw(addr, len);
		retry++;
	...
	const struct firmware *fw, int type)
{
	...
	code_sdata[0] = hx_s_ic_setup_ol._ovl_sorting_reply;

	if (type == 0) {
#if defined(HX_SMART_WAKEUP)
		if (hx_s_ts_ol->suspended && hx_s_ts_ol->SMWP_enable) {
			code_idx_t = ovl_idx[1];
			code_sdata[0] = hx_s_ic_setup_ol._ovl_gesture_reply;
		} else {
			code_idx_t = ovl_idx[2];
			code_sdata[0] = hx_s_ic_setup_ol._ovl_border_reply;
		}
#else
		code_idx_t = ovl_idx[2];
		code_sdata[0] = hx_s_ic_setup_ol._ovl_border_reply;
	...
	I("%s: upgrade code overlay section[%d]\n",
		__func__,
		code_idx_t);
	if (hx_s_core_fp_ol._write_sram_0f_crc(info[code_idx_t].sram_addr,
		&fw->data[info[code_idx_t].fw_addr],
		info[code_idx_t].write_size) != 0) {
		E("%s: code overlay HW CRC FAIL\n", __func__);
		code_sdata[0] = hx_s_ic_setup_ol._ovl_fault;
		ret = 2;
	}

	retry = 0;
	do {
		hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._ovl_addr_handshake,
			code_sdata,
			DATA_LEN_4);
		usleep_range(1000, 1100);
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._ovl_addr_handshake,
			rdata,
			DATA_LEN_4);
		retry++;
	...
	const struct firmware *fw)
{
	...
	alg_sdata[3] = hx_s_ic_setup_ol._ovl_alg_reply;
	alg_sdata[2] = hx_s_ic_setup_ol._ovl_alg_reply;
	alg_sdata[1] = hx_s_ic_setup_ol._ovl_alg_reply;
	alg_sdata[0] = hx_s_ic_setup_ol._ovl_alg_reply;

	hx_s_core_fp_ol._reload_disable(0);

	/*hx_s_core_fp_ol._power_on_init();*/
	/*Rawout Sel initial*/
	hx_parse_assign_cmd_ol(DATA_CLEAR, tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_rawout_sel,
		tmp_data, DATA_LEN_4);
	...
	do {
		...
		hx_s_core_fp_ol._register_read(hx_s_ic_setup_ol._ovl_addr_handshake,
			rdata, DATA_LEN_4);
	} while ((rdata[0] != hx_s_ic_setup_ol._ovl_alg_request
	|| rdata[1] != hx_s_ic_setup_ol._ovl_alg_request
	|| rdata[2] != hx_s_ic_setup_ol._ovl_alg_request
	|| rdata[3] != hx_s_ic_setup_ol._ovl_alg_request)
	&& retry++ < 30);
	...
	if (retry > 30) {
		...
		for (i = 0; i < 10; i++) {
			usleep_range(10000, 10100);
			hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._ovl_addr_handshake,
				rdata, DATA_LEN_4);
	...
	if (hx_s_core_fp_ol._write_sram_0f_crc(info[alg_idx_t].sram_addr,
	...
	if (retry > HIMAX_REG_RETRY_TIMES) {
		...
		while (retry++ < 30) {
			hx_s_core_fp_ol._register_read(
				hx_s_ic_setup_ol._addr_chk_fw_reload2,
				tmp_data, DATA_LEN_4);
...
static int himax_zf_part_info(const struct firmware *fw, int type)
{
	...
	if (g_zf_opt_crc.en_crc_clear)
		hx_s_core_fp_ol._opt_crc_clear();
	if (g_zf_opt_crc.en_opt_hw_crc >= OPT_CRC_RANGE)
		hx_s_core_fp_ol._setup_opt_hw_crc(fw);
	if (hx_s_core_fp_ol._en_hw_crc != NULL)
		hx_s_core_fp_ol._en_hw_crc(1);
	...
	if (ovl_idx == NULL) {
		ovl_idx = kzalloc(hx_s_ic_setup_ol._ovl_section_num, GFP_KERNEL);
		if (ovl_idx == NULL) {
		...
		memset(ovl_idx, 0, hx_s_ic_setup_ol._ovl_section_num);
	}
	...
	for (i = 1; i < pnum; i++) {
		...
		&& buf[1] == 0x8C && buf[0] == 0xE0)) {
			...
			if (buf[15] == 0x55 && buf[14] == 0x66) {
				/* current mechanism */
				j = buf[13];
				if (j < hx_s_ic_setup_ol._ovl_section_num)
					ovl_idx[j] = i;
			} else {
				/* previous mechanism */
				if (j < hx_s_ic_setup_ol._ovl_section_num)
	...
	if (cfg_sz > (hx_s_ic_data_ol->dsram_sz)) {
		E("%s: config size error[%d, %d]!!\n", __func__,
			cfg_sz, (hx_s_ic_data_ol->dsram_sz));
		ret = LENGTH_FAIL;
		goto ALOC_CFG_BUF_FAIL;
	}

	memset(g_update_cfg_buf, 0x00,
		hx_s_ic_data_ol->dsram_sz * sizeof(uint8_t));
	...
	/* 4. write to sram */
	/* FW entity */
	if (hx_s_core_fp_ol._write_sram_0f_crc(info[0].sram_addr,
	...
	I("%s: i_min=%d, fw_addr=0x%08x, sram_addr=0x%08X, cfg_sz=%d\n",
		...
		info[i_min].cfg_addr,
		cfg_sz);
	cfg_crc_sw = hx_s_core_fp_ol._calc_crc_by_ap(g_update_cfg_buf, 0, cfg_sz);
	I("%s: now cfg_crc_sw=0x%08X\n", __func__, cfg_crc_sw);
	do {
		hx_s_core_fp_ol._write_sram_0f(info[i_min].sram_addr,
			g_update_cfg_buf,
			cfg_sz);
		cfg_crc_hw = hx_s_core_fp_ol._calc_crc_by_hw(
	...
	|| defined(HX_USB_DETECT_GLOBAL)
		hx_s_core_fp_ol._resend_cmd_func(hx_s_ts_ol->suspended);
#endif
	}
...
int himax_mcu_firmware_update_0f(const struct firmware *fw, int type)
{
	...
	hx_parse_assign_cmd_ol(hx_s_ic_setup_ol._data_system_reset,
		tmp_data, DATA_LEN_4);
	hx_s_core_fp_ol._register_write(hx_s_ic_setup_ol._addr_system_reset,
		tmp_data, DATA_LEN_4);

	hx_s_core_fp_ol._sense_off(false);
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	reqret = request_firmware(&fw, file_name, hx_s_ts_ol->dev);
	if (reqret < 0) {
	...
	ret = hx_s_core_fp_ol._firmware_update_0f(fw, type);
...
if (!g_has_alg_overlay) {
	if (type == 1)
		hx_s_core_fp_ol._turn_on_mp_func(1);
	else
		hx_s_core_fp_ol._turn_on_mp_func(0);
	hx_s_core_fp_ol._reload_disable(0);
	hx_s_core_fp_ol._power_on_init();
}
		...
		const uint8_t *addr, int start_index, int read_len)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (i == 0)
			address = addr32;
		if (total_size_temp >= max_bus_size) {
			hx_s_core_fp_ol._register_read(address,
...
void hx_mcu_opt_hw_crc(void)
{
	...
	if (g_zf_opt_crc.en_start_addr) {
		...
		do {
			...
			hx_s_core_fp_ol._register_write(g_zf_opt_crc.start_addr,
				g_zf_opt_crc.start_data, DATA_LEN_4);
			usleep_range(1000, 1100);
			hx_s_core_fp_ol._register_read(g_zf_opt_crc.start_addr,
				data, DATA_LEN_4);
	...
	if (g_zf_opt_crc.en_end_addr) {
		...
		do {
			...
			hx_s_core_fp_ol._register_write(g_zf_opt_crc.end_addr,
				data, DATA_LEN_4);
			usleep_range(1000, 1100);
			hx_s_core_fp_ol._register_read(g_zf_opt_crc.end_addr,
				data, DATA_LEN_4);
	...
	if (g_zf_opt_crc.en_switch_addr) {
		...
		do {
			...
			hx_s_core_fp_ol._register_write(g_zf_opt_crc.switch_addr,
				data, DATA_LEN_4);
			usleep_range(1000, 1100);
			hx_s_core_fp_ol._register_read(g_zf_opt_crc.switch_addr,
				data, DATA_LEN_4);
