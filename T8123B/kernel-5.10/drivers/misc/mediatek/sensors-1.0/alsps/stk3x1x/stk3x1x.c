...
#define PRINT_ALSPS(fmt, args...)   printk(stk3x1x_DEV_NAME fmt,##args)
#else
#define PRINT_ALSPS(fmt, args...)   printk(stk3x1x_DEV_NAME fmt,##args)
#endif
...
/*----------------------------------------------------------------------------*/
static int stk3x1x_i2c_suspend(struct device *device, pm_message_t msg);
static int stk3x1x_i2c_resume(struct device *device);
...
#endif
#ifdef MTK_CALI
#define STK_ALS_FACTORY_TARGET_LUX 400
#define ALS_CALI_DATA_DEFAULT 2280
static int 	als_factory_cali_data =  ALS_CALI_DATA_DEFAULT;// factory cali raw data
#endif
...
#endif
static int als_debug_log_flag = 0;
struct als_debug_data {
	u16 als;
	u32 als_data;
	u16 cali_data;
};
static struct als_debug_data als_debug_data_cust;
...
{
    {.compatible = "mediatek,alsps-stk3x1x"},
    {},
};
#endif
...
#endif
#ifdef MTK_CALI
	als_debug_data_cust.als = obj->als;
	als_debug_data_cust.als_data = als_data;
	als_debug_data_cust.cali_data = als_factory_cali_data;
	

	if(als_debug_log_flag > 0) {
		pr_err("stk-als-cali-collect-data:[lux=%d]-[als_data=%d]-[cali_data=%d]\n",
			obj->als, als_data, als_factory_cali_data);
	}

	if(als_factory_cali_data > 0) { //aready cali
		*data = (STK_ALS_FACTORY_TARGET_LUX * als_data)/(als_factory_cali_data);//CWF
	}
	else
		*data = (als_data*191)/1000;
#else
    *data = (als_data*191)/1000;
#endif
#ifdef STK_IRS
...
int stk3x1x_read_id(struct i2c_client *client)
{
    ...
    obj->pid = buf[0];
    PRINT_ALSPS("%s: PID=0x%x, VID=0x%x\n", __func__, buf[0], buf[1]);
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
    ...
    if ((obj->psi_set != 0))
    {
        if (obj->ps_distance_last == 1)
        {
            ...
            if (ret == 0)
            {
                ...
                word_data = (buf[0] << 8) | buf[1];
                APS_LOG("%s: CTTRACKING data=%d, HT=%d, LT=%d\n", __func__,word_data,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
    ...
    if (ret == 0)
    {
        ...
        APS_LOG("%s: enable auto K data=%d, HT=%d, LT=%d\n", __func__,word_data,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
        if (word_data == 0)
        {
...
{
#if defined(STK_TUNE0) || defined(STK_GES)
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
	...
	#else
	  gpio_irq = of_get_named_gpio(stk3x1x_obj->irq_node, "int-gpio", 0);
                APS_ERR("gpio_irq = %d\n", gpio_irq);
                if(gpio_irq < 0){
                        APS_ERR("of_get_named_gpio fail!\n");
                        return -EIO;
                }
                ret = gpio_request(gpio_irq, "stk3x1x-int-gpio");
                if(ret){
                        APS_ERR("gpio request fail!\n");
                        return -EIO;
                }
                gpio_direction_input(gpio_irq);
                stk3x1x_obj->irq = irq_of_parse_and_map(stk3x1x_obj->irq_node, 0);

                //stk3x1x_obj->irq = gpio_to_irq(gpio_irq);
                if(!stk3x1x_obj->irq){
                        APS_ERR("irq_of_parse_and_map fail!!\n");
                        return -EINVAL;
                }
                APS_ERR("stk3x1x_obj->irq = %d\n", stk3x1x_obj->irq);
                if (!stk3x1x_obj->irq) {
                        APS_ERR("irq_of_parse_and_map fail!!\n");
                        return -EINVAL;
                }
                if (request_irq
                    (stk3x1x_obj->irq, stk3x1x_eint_handler, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
                        APS_ERR("IRQ LINE NOT AVAILABLE!!\n");
                        return -EINVAL;
                }
...
static int stk3x1x_init_client(struct i2c_client *client)
{
    ...
    {
        PRINT_ALSPS("software reset error, err=%d", err);
        return err;
    }

    if ((err = stk3x1x_read_id(client)))
    {
        PRINT_ALSPS("stk3x1x_read_id error, err=%d", err);
        return err;
    }
...
static ssize_t ps_fty_test_show(struct device_driver *ddri, char *buf)
{
    ...
    ret = stk3x1x_enable_ps(stk3x1x_obj->client, 1, 1);
    if (ret)
    {
    ...
    ret = stk3x1x_enable_ps(stk3x1x_obj->client, 0, 1);
    if (ret)
    {
...
#endif
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_debug_show(struct device_driver *ddri, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "[lux=%d]-[als_data=%d]-[cali_data=%d]\n",
		als_debug_data_cust.als, 
		als_debug_data_cust.als_data, 
		als_debug_data_cust.cali_data);
}

static ssize_t stk3x1x_debug_store(struct device_driver *ddri, const char *buf, size_t count)
{
       int rv;
       rv = kstrtoint(buf, 0, &als_debug_log_flag);
       if (rv != 0)
               return -EINVAL;
...
static DRIVER_ATTR_RW(stk3x1x_debug);
#endif
/*----------------------------------------------------------------------------*/
...
static int als_get_data(int* value, int* status)
{
    ...
    else
    {
        ...
        //PRINT_ALSPS("[ryan] lux = %d, rawdata = %d", *value, obj->als);
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
    }
...
static int stk3x1x_als_factory_get_raw_data(int32_t *data)
{
	...
	err =  stk3x1x_master_recv(obj->client, obj->addr.data1_als, buf, 0x2);
	if (err < 0) {
		APS_ERR("%s failed\n", __func__);
		return -1;
	}
...
{
	APS_FUN();
	als_factory_cali_data = 0;
	return 0;
}
static int stk3x1x_als_factory_set_cali(int32_t offset)
{
	APS_FUN();
	APS_ERR("WARNING: %s: stk als factory cali = %d\n", __func__, offset);

	if(offset > 0 )
		als_factory_cali_data = offset;
	else {
		als_factory_cali_data = ALS_CALI_DATA_DEFAULT;
		APS_ERR("WARNING: %s: stk als no factory cali, set to default cali value = %d\n", __func__, als_factory_cali_data);
		return -1;
	}
	return 0;
}
...
static int stk3x1x_ps_factory_get_threashold(int32_t threshold[2])
{
	...
	APS_ERR("%s get threshold high: 0x%x, low: 0x%x, ps_cali=%d\n", __func__, threshold[0], threshold[1], obj->ps_cali);
	return 0;
}
#endif
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (err)
    {
        PRINT_ALSPS("get customization info from dts failed\n");
        return -EFAULT;
    }
    ...
    //obj->irq_node = of_find_matching_node(client->dev.of_node, alsps_of_match);
    ...
    if (obj->hw->polling_mode_ps == 0)
    {
        PRINT_ALSPS("%s: enable PS interrupt\n", __FUNCTION__);
    }
    ...
    if (obj->hw->polling_mode_als == 0)
    {
        ...
        PRINT_ALSPS("%s: enable ALS interrupt\n", __FUNCTION__);
    }
...
#ifdef MTK_CALI
	if((err = alsps_factory_device_register(&stk3x1x_factory_device)))
	{
		printk("stk3x1x factory register failed\n");
    ...
    if (err)
    {
        PRINT_ALSPS("als_control register fail = %d\n", err);
    ...
    if (err)
    {
        PRINT_ALSPS("als_data register fail = %d\n", err);
        goto exit_sensor_obj_attach_fail;
    }
    ...
    if (err)
    {
        PRINT_ALSPS("ps_control register fail = %d\n", err);
        goto exit_sensor_obj_attach_fail;
    }
    ...
    if (err)
    {
        PRINT_ALSPS("ps_data register fail = %d\n", err);
        goto exit_sensor_obj_attach_fail;
    }
    ...
    PRINT_ALSPS("[ryan]stk3x1x set device_init_wakeup\n");
    ...
    PRINT_ALSPS("stk3x1x_i2c_probe() OK!\n");
    return 0;
#ifdef STK_GES
