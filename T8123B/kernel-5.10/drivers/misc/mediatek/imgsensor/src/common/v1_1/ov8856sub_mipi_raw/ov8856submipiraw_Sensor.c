...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856SUB_SENSOR_ID,		/*record sensor id defined in Kd_imgsensor.h*/
	...
	.i2c_addr_table = {0x6C, 0x42, 0xff},/*record sensor support all write id addr, only supprt 4must end with 0xff*/
};
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			//ov8856 main id = 0x885A, ov8856 sub id = 0x885B
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("ov8856sub i2c write id: 0x%x, sensor id: 0x%x, ver = 0x%x<0xB1=r2a,0xB0=r1a>\n",
					imgsensor.i2c_write_id, *sensor_id, read_cmos_sensor(0x302A));
				return ERROR_NONE;
			}
			printk("ov8856sub Read sensor id fail,write_id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
UINT32 OV8856SUB_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	OV8856SUB_MIPI_RAW_SensorInit	*/
