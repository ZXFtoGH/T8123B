...
#if VERSION_CHECK
struct proc_dir_entry *gc05a2_proc_vendor_file;
static uint8_t GC05A2_PROC_FLAG = 0;
...
//#endif
	...
	.i2c_addr_table = {0x6e, 0x7e, 0xff},  //tom add
	.i2c_speed = 400,
};
...
#if VERSION_CHECK
static void gc05a2_otp_read_group(kal_uint16 addr, kal_uint8 *data, kal_uint16 length)
{
	kal_uint16 i = 0;
...
static kal_uint16 gc05a2_otp_init_addr_data[] = {
	0x0af6, 0x00,
	0x0b90, 0x10,
	0x0b91, 0x00,
	0x0b92, 0x00,
...
static kal_uint16 gc05a2_otp_close_addr_data[] = {
	0x0a70, 0x00,
	0x0a67, 0x00,
};

static ssize_t gc05a2_vendor_read(struct file *file, char *buf,
...
static const struct proc_ops gc05a2_proc_vendor_ops = {
	.proc_read = gc05a2_vendor_read,
};
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				table_write_cmos_sensor(gc05a2_otp_init_addr_data,
					sizeof(gc05a2_otp_init_addr_data) / sizeof(kal_uint16));

				mdelay(10);

				gc05a2_otp_read_group(0x2000, &group_flag, 1);

				if(group_flag==0x01){
					gc05a2_otp_read_group(0x2008, &module_info[0], 10);
				}
				else if(group_flag==0x13){
					gc05a2_otp_read_group(0x5b30, &module_info[0], 10);
				}

				table_write_cmos_sensor(gc05a2_otp_close_addr_data,
					sizeof(gc05a2_otp_close_addr_data) / sizeof(kal_uint16));

				gc05a2_proc_vendor_file = proc_create("otp_camera_sub", 0444,
					NULL, &gc05a2_proc_vendor_ops);
				if (gc05a2_proc_vendor_file == NULL) {
	...
	printk("[tom add gc05a2_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc05a2_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("[gc05a2_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
