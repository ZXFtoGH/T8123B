...
#define GC08A8MAIN_OTP_DEBUG  	0
#define GC08A8MAIN_I2C_ID     	0x24 /*0x20*/
...
static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, GC08A8MAIN_I2C_ID);
...
static void write_cmos_sensor(u16 addr, u8 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 3, GC08A8MAIN_I2C_ID);
}

static void gc08a8main_otp_init(void)
{
...
static void gc08a8main_otp_close(void)
{
	write_cmos_sensor(0x0316, 0x01);
	write_cmos_sensor(0x0a67, 0x00);
}

static u16 gc08a8main_otp_read_group(u16 addr, u8 *data, u16 length)
{
	u16 i = 0;
...
#if GC08A8MAIN_OTP_DEBUG
	printk("[gc08a8main]addr = 0x%x, data = 0x%x\n", addr + i * 8, data[i]);
#endif
	}
	return 0;
}
...
int gc08a8main_iReadData(unsigned int ui4_offset,
	unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;

	printk("[ryan]gc08a8main otp ui4_offset = 0x%x, ui4_length = %d \n", ui4_offset, ui4_length);

	gc08a8main_otp_init();
	mdelay(10);

	i4RetValue = gc08a8main_otp_read_group(ui4_offset, pinputdata, ui4_length);
	if (i4RetValue != 0) {
		printk("[ryan]I2C iReadData failed!!\n");
		return -1;
	}

	gc08a8main_otp_close();

	return 0;
}

unsigned int gc08a8main_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	printk("[ryan] gc08a8main_read_region enter\n");
	if (gc08a8main_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
