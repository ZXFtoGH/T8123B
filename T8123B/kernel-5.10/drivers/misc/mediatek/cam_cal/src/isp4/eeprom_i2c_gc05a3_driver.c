...
#include "eeprom_i2c_gc05a3_driver.h"

static DEFINE_SPINLOCK(g_spinLock);
static struct i2c_client *g_pstI2CclientG;

#define EEPROM_I2C_MSG_SIZE_READ 2
#define GC05A3_OTP_DEBUG  0
#define GC05A3_I2C_ID     0x6e /*0x20*/
...
static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, GC05A3_I2C_ID);
...
static void write_cmos_sensor(u16 addr, u8 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 3, GC05A3_I2C_ID);
}

static void gc05a3_otp_init(void)
{
...
static void gc05a3_otp_close(void)
{
	write_cmos_sensor(0x0a70, 0x00);
	write_cmos_sensor(0x0a67, 0x00);
}

static u16 gc05a3_otp_read_group(u16 addr, u8 *data, u16 length)
{
	u16 i = 0;
...
int gc05a3_iReadData(unsigned int ui4_offset,
	unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;

	pr_debug("gc05a3 otp ui4_offset = 0x%x, ui4_length = %d \n", ui4_offset, ui4_length);

	gc05a3_otp_init();
	mdelay(10);

	i4RetValue = gc05a3_otp_read_group(ui4_offset, pinputdata, ui4_length);
	if (i4RetValue != 0) {
...
unsigned int gc05a3_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	if (gc05a3_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
