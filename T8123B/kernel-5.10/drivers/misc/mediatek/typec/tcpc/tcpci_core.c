...
#if IS_ENABLED(CONFIG_TCPC_HUSB320)
uint8_t     typec_cc;
uint8_t     typec_cc_orientation;
uint8_t get_typec_cc_orientation(uint8_t x1) {
       typec_cc_orientation = x1;
    return typec_cc_orientation;
}
EXPORT_SYMBOL(get_typec_cc_orientation);
EXPORT_SYMBOL(typec_cc);
#endif
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	switch (offset) {
	/*Add a interface to recognise positive and negative of type-C for factory charge test*/
	case TCPC_DESC_REMOTE:
		if (tcpc->typec_remote_cc[0] && !tcpc->typec_remote_cc[1]) {
			typec_orientation = 1;
		} else if (!tcpc->typec_remote_cc[0] && tcpc->typec_remote_cc[1]) {
			typec_orientation = 2;
		} else {
			//snprintf(buf, 256, "%x\n", get_typec_cc_orientation(typec_cc));
			#if IS_ENABLED(CONFIG_TCPC_HUSB320)
			typec_orientation = get_typec_cc_orientation(typec_cc);
			#endif
		}
		snprintf(buf, 256, "%x\n", typec_orientation);
		break;
