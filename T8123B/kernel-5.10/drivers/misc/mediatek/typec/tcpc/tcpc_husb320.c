...
void husb320_status_check(void)
{
	struct husb320_chip *chip = i2c_get_clientdata(g_client);
	int rc;
	u8 state;

	rc = i2c_smbus_read_word_data(g_client,
			HUSB320_REG_CONTROL1);
	if (rc < 0) {
	...
	if(state != 1) {
		husb320_reset_device(chip);
		printk("%s do husb320_reset_device\n", __func__);
	}
}
...
static void husb320_orient_changed(struct husb320_chip *chip)
{
	...
	typec_cc = get_typec_cc_orientation(chip->orientation);
...
static void husb320_detach(struct husb320_chip *chip)
{
	...
	typec_cc = get_typec_cc_orientation(0x0);
...
late_initcall(husb320_i2c_init);
