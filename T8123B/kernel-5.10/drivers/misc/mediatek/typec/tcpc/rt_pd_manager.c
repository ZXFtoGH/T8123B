...
#include <charger_class.h>
	...
	#include "../../../../sound/soc/codecs/mt6358-analog.h"
#endif
...
#define RT_CALL_TIME	7000
#define RT_PLUGIN_TIME	13000
...
static bool typec_device_inserted;
static struct delayed_work pga_work;
static struct delayed_work plugin_work;
extern bool mt6358_accdet_probe_enter_finish(void);
extern bool mt6358_accdet_plugin_finish(bool enable);
#endif
//end
...
/*
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
extern void typec_accdet_irq_handle(void);
extern void audio_en_hp(bool en);
#endif
*/
//tom add 2024-02-26
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
static void typec_hotplug_callback(struct work_struct *work) {
	int ret = 0;
    if (typec_device_inserted) {
		//msleep(RT_WAIT_TIME);
		mt6358_accdet_plugin_finish(false);
		#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
		ret = fsa4480_usbc_analog_setup_switches_hp(1);
		#else
		audio_en_hp(1);
		#endif				
		//audio_en_hp(1);
		typec_accdet_irq_handle(ret);
    }else{
		#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
		ret = fsa4480_usbc_analog_setup_switches_hp(1);
		#else
		...
		#endif			
		//audio_en_hp(0);
		typec_accdet_irq_handle(0);
	}
}

static void typec_plugin_work_callback(struct work_struct *work) {
    plugin_flag = true;
	mt6358_accdet_plugin_finish(false);
	printk("plugin_flag = true %s \n", __func__);
}
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
			//tom add 2024-02-26
			typec_device_inserted = true;
			init_flag = mt6358_accdet_probe_enter_finish();
			if(init_flag){
				printk("audio plug in %s : init_flag=1 !\n", __func__);
				if(!plugin_flag){
					plugin_flag = true;
					mt6358_accdet_plugin_finish(true);
				}
				#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
				ret = fsa4480_usbc_analog_setup_switches_hp(1);
				#else
				...
				printk("audio plug in %s : schedule_delayed_work !\n", __func__);
				schedule_delayed_work(&pga_work, msecs_to_jiffies(RT_CALL_TIME));
			}			
			#endif
			...
			typec_device_inserted = false;
			init_flag = mt6358_accdet_probe_enter_finish();
			printk("tom add, flag=%d, %d, %s !\n", init_flag, __LINE__, __func__);
			if(init_flag){
				#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
				fsa4480_usbc_analog_setup_switches_hp(0);
				#else
				audio_en_hp(0);	
				#endif					
				typec_accdet_irq_handle(0);				
			...
			printk("tom add %s : disable connection !\n", __func__);
			#endif
		}
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
	//tom add 2024-02-26
	INIT_DELAYED_WORK(&pga_work, typec_hotplug_callback);
	INIT_DELAYED_WORK(&plugin_work, typec_plugin_work_callback);
	schedule_delayed_work(&plugin_work, msecs_to_jiffies(RT_PLUGIN_TIME));
	#endif
	return 0;
