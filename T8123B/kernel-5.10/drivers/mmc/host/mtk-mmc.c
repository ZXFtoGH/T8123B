...
static int msdc_get_cd(struct mmc_host *mmc)
{
	...
	card_present = mmc_gpio_get_cd(mmc); //host->card_inserted;

	return host->card_inserted;
}

u8 card_slot_present(void){
	return card_present;
}
EXPORT_SYMBOL(card_slot_present);
...
static DEVICE_ATTR(slot_state, S_IRUGO, slot_state_show, NULL);
#endif
...
#endif
#if SUPPORT_SLOT_STATE
	ret = device_create_file(&mmc->class_dev, &dev_attr_slot_state);
	if (ret) {
