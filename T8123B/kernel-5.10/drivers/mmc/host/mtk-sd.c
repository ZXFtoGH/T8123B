...
static void dump_register(struct msdc_host *host)
{
	...
	if (host->top_base) {
		dev_err(host->dev, "top reg[0]: %08x.\n",
				readl(host->top_base + 0x0));
		dev_err(host->dev, "top reg[4]: %08x.\n",
				readl(host->top_base + 0x4));
		dev_err(host->dev, "top reg[c]: %08x.\n",
				readl(host->top_base + 0xc));
	}
...
static struct msdc_delay_phase get_best_delay(struct msdc_host *host, u64 delay)
{
	...
	dev_info(host->dev, "phase: [map:%llx] [maxlen:%d] [final:%u]\n",
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	dev_info(host->dev, "dump MSDC regs before tune\n");
	msdc_dump_all_register(host);
	if (host->dev_comp->data_tune && host->dev_comp->async_fifo) {
		...
		if (host->vcore_reg) {
			/* restore voltage after tuning */
			ret = regulator_set_voltage(host->vcore_reg, 0, INT_MAX);
			if (ret < 0)
				dev_info(host->dev, "failed to restore vcore: %d\n", ret);
		}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->vcore_reg = devm_regulator_get_optional(&pdev->dev, "dvfsrc-vcore");
