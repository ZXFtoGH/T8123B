...
static void mt6768_mt6358_ext_hp_amp_turn_on_or_off(int on)
{
	u32 i;
	/* enable spk amp */
	if (ext_amp_gpio >= 0){
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
		mt6768_mt6358_ext_hp_amp_turn_on_or_off(1);
		//tom add
		#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
	...
	case SND_SOC_DAPM_PRE_PMD:
		/* spk amp off control */
		mt6768_mt6358_ext_hp_amp_turn_on_or_off(0);
		//tom add
		#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
