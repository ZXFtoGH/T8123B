		...
		int trim_times, int discard_num, int useful_num)
{
	...
	for (i = discard_num; i < trim_times - discard_num; i++) {
		...
		//tom del
		//dev_info(priv->dev, "%s(), offset diff = %d, on = %d, off = %d\n",__func__,on_value[i] - off_value[i], on_value[i], off_value[i]);
	}
...
static void mt6358_codec_init_reg(struct mt6358_priv *priv)
{
	...
	/* accdet s/w enable */
	//tom add: pop sound when insert earphone
	//regmap_update_bits(priv->regmap, MT6358_ACCDET_CON13, 0xFFFF, 0x700E);	
	regmap_update_bits(priv->regmap, MT6358_ACCDET_CON13, 0xFFFF, 0x3006);
	//end
...
static int mt6358_parse_dt(struct mt6358_priv *priv)
{
	...
	if (ret) {
		...
		//tom del
		//dev_info(dev,"%s(), get pull_down_stay_enable fail, default 0\n", __func__);
	}
