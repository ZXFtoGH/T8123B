...
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
static int  GPIO_TYPEC_EARPHONE;
#endif

#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
#include "mt6358-analog.h"
...
//tom add init_flag 2024-02-26
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
static bool init_flag = false;
static bool plugin_flag = false;
#endif

#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
int typec_inserted;
static bool mic_time_flag = false;
#endif
//end
...
/*
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
void audio_en_hp(bool en);
void typec_accdet_irq_handle(int insert);
#endif
*/
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
		if(cur_AB == ACCDET_STATE_AB_00 && typec_inserted > 1){
			pr_notice("tom add fsa4480-1:cur_AB=%d, %s, %d\n",cur_AB,__FUNCTION__,__LINE__);
			fsa4480_switch_value(0);
			if(mic_time_flag){
				pr_notice("tom add mic_time_flag %s:\n",__FUNCTION__);
				mic_time_flag = false;
...
void accdet_irq_handle(void)
{
	...
	if ((irq_status & ACCDET_IRQ_MASK_SFT) && (eintID == 0)) {
		...
		eintID = PMIC_EINT0;
		//pmic_eint_queue_work(eintID);
		pr_notice("%s do pmic_eint_queue_work!\n", __func__);
#else
...
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
	GPIO_TYPEC_EARPHONE = of_get_named_gpio(node, "gpio_typec_earphone", 0);
	gpio_request(GPIO_TYPEC_EARPHONE, "GPIO_TYPEC_EARPHONE");
	if(GPIO_TYPEC_EARPHONE < 0) {
		pr_err("[  err] %s: GPIO_TYPEC_EARPHONE < 0 !!! \r\n",  __func__);
		ret = -ENODEV;
	}
	gpio_direction_output(GPIO_TYPEC_EARPHONE, 0);
	gpio_set_value(GPIO_TYPEC_EARPHONE, 0);
#endif
...
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
	pr_info("%s analog hp, init_flag=%d \n", __func__, init_flag);
#else
...
#if IS_ENABLED(CONFIG_TYPEC_ANALOG_HP)
#define DELAY_TIME	2600
void typec_headphone_irq_handler(void);
void accdet_irq_handle_for_typec(void);
...
void audio_en_hp(bool en)
{
	if(en) {
		...
		if(plugin_flag){
			plugin_flag = false;
			pr_info("%s tom add on, %d \n", __func__, __LINE__);
			msleep(DELAY_TIME);
			gpio_set_value(GPIO_TYPEC_EARPHONE, 1);
		}else{
			pr_info("%s tom add on, %d \n", __func__, __LINE__);
			gpio_set_value(GPIO_TYPEC_EARPHONE, 1);
		}
		...
		//disable typec hp
		pr_info("%s tom add audio_en_hp off\n", __func__);
		gpio_set_value(GPIO_TYPEC_EARPHONE, 0);
	}
}
EXPORT_SYMBOL_GPL(audio_en_hp);

void typec_accdet_irq_handle(int insert)
{
	pr_info("%s tom add insert = %d\n", __func__, insert);
	#if IS_ENABLED(CONFIG_FSA4480_USB_AUDIO_SWITCH)
	typec_inserted = insert;
	#endif
	//tom add init_flag 2024-02-26
	typec_headphone_irq_handler();
	//end
}
EXPORT_SYMBOL_GPL(typec_accdet_irq_handle);

bool mt6358_accdet_plugin_finish(bool enable)
{
	plugin_flag = enable;
	return plugin_flag;
}
EXPORT_SYMBOL_GPL(mt6358_accdet_plugin_finish);

//tom add init_flag 2024-02-26
bool mt6358_accdet_probe_enter_finish(void)
{
	return init_flag;
}
EXPORT_SYMBOL_GPL(mt6358_accdet_probe_enter_finish);

//采用api 调用的方式实现typec 和 accdet模块的通信交互
void typec_headphone_irq_handler(void)
{
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		//printk("tom add %d, %s !\n", __LINE__, __func__);
	...
	//pr_info("tom add queue_work %d, %s !\n", __LINE__, __func__);
