...
extern int thermal_over39;
//extern int chgtmp_value;
int pd_battery_temp = 0;
int ntc_35_flag = BAT_TEMP_NORMAL;
int ntc_40_flag = BAT_TEMP_NORMAL;
int chgtmp_5v_flag = 0;
			...
			int *newidx)
{
	...
	/*temp policy*/
	if (pd->bat_psy == NULL || IS_ERR(pd->bat_psy)) {
		pd_err("%s Couldn't get bat_psy\n", __func__);
	} else {
		power_supply_get_property(pd->bat_psy,POWER_SUPPLY_PROP_TEMP, &prop);
		pd_battery_temp = prop.intval / 10;
		pd_err("%s pd_battery_temp = %d\n", __func__, pd_battery_temp);
	}
	...
	if(uisoc >= pd->pd_stop_battery_soc){
		...
		*newidx = 0;
		buck = true;	
	//}else if(chgtmp_value > 40 && chgtmp_5v_flag){
	}else if(0){
		*newidx = 0;
		buck = true;
	}else{
...
static int pd_dcs_set_charger(struct chg_alg_device *alg)
{
	...
	if (pd->input_current_limit1 != -1 &&
		...
		pd->input_current1 = pd->dcs_chg1_input_current;
	...
	pd->input_current1 = pd->dcs_chg1_input_current;
	pd->charging_current1 = pd->dcs_chg1_charger_current;
	pd->input_current2 = pd->dcs_chg2_input_current;
	pd->charging_current2 = pd->dcs_chg2_charger_current;
	//if(chgtmp_value >=49 && pd_battery_temp >=35){
	if(0) {
		// 14W
		pd->input_current1 = 850000;
		pd->charging_current1 = 1500000;
		pd->input_current2 = 850000;
		pd->charging_current2 = 1500000;
		ntc_35_flag = BAT_TEMP_HIGH;
	} else if(ntc_35_flag == BAT_TEMP_HIGH) {
		//if(chgtmp_value <= 45) {
		if(0) {
			ntc_35_flag = BAT_TEMP_NORMAL;
		} else {
			//14W
			pd->input_current1 = 850000;
			pd->charging_current1 = 1500000;
	...
	//if(chgtmp_value >=49 && pd_battery_temp >=40){
	if(0) {
		// 10W
		pd->input_current1 = 1000000;
		pd->charging_current1 = 1500000;
		pd->input_current2 = 1000000;
		pd->charging_current2 = 1500000;
		ntc_40_flag = BAT_TEMP_HIGH;
		chgtmp_5v_flag = 1;
	} else if(ntc_40_flag == BAT_TEMP_HIGH) {
		//if(chgtmp_value <= 40) {
		if(0) {
			ntc_40_flag = BAT_TEMP_NORMAL;
			chgtmp_5v_flag = 0;
		} else {
			//10W
			pd->input_current1 = 1000000;
	...
	if(pd_hal_get_uisoc(alg) >= 95){
		pd->input_current1 = pd->input_current1 + pd->input_current2;
	}
	pd_hal_set_charging_current(alg,
		CHG1, pd->charging_current1);
	pd_hal_set_input_current(alg,
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "dcs_chg1_input_current", &val) >= 0) {
		pd->dcs_chg1_input_current = val;
	} else {
		pd_err("use default dcs_chg1_input_current:%d\n",
			PD_DCS_INPUT_CURRENT);
		pd->dcs_chg1_input_current = PD_DCS_INPUT_CURRENT;
	}

	if (of_property_read_u32(np, "dcs_chg2_input_current", &val) >= 0) {
		pd->dcs_chg2_input_current = val;
	} else {
		pd_err("use default dcs_chg2_input_current:%d\n",
			PD_DCS_INPUT_CURRENT);
		pd->dcs_chg2_input_current = PD_DCS_INPUT_CURRENT;
	}
