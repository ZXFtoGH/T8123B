...
static int sgm41513_get_usb_type(struct sgm41513 *sgm, bool en)
{
	...
	if (sgm->psy_desc.type != POWER_SUPPLY_USB_TYPE_DCP) {
		sgm_info("zxf sgm41513 retry get type\n");
...
{
	unsigned char val = 0;
	printk("zxf forcedpdm start\n");
	return sgm41513_update_bits(sgm, (unsigned char) (SGM41513_CON7),
				       (unsigned char) (val),
...
static void sgm41513_retry_chgdet_work(struct work_struct *work)
{
	   ...
	   printk("zxf forcedpdm end  ret = %d\n", ret);
