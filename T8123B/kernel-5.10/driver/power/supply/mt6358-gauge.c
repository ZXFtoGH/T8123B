	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (!IS_ERR(gauge->chan_vcdt_to_vbus)) {
			if (iio_read_channel_processed(gauge->chan_vcdt_to_vbus, &_val) < 0){
				bm_err("[chan_vcdt_to_vbus]read fail\n", __func__);
				val->intval = 0;
			}else{
				_val = (((330 + 39) * 100 * _val) / 39) / 100;	//calculate vbus voltage
				val->intval = _val;
			}
		} else {
			val->intval = 0;
...
static int mt6358_gauge_probe(struct platform_device *pdev)
{
	...
	gauge->chan_vcdt_to_vbus = devm_iio_channel_get(
		&pdev->dev, "pmic_vcdt_vbus");
	if (IS_ERR(gauge->chan_vcdt_to_vbus)) {
		ret = PTR_ERR(gauge->chan_vcdt_to_vbus);
		bm_err("chan_vcdt_to_vbus auxadc get fail, ret=%d\n",
			ret);
	}
