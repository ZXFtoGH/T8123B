...
static std::vector<HwLight> supportedLights;
static sp<ILight> gLightHalHidlV2_0_;
static std::shared_ptr<ILights> gLightHalAidl_;
...
write_int(Type light_type, int value)
{
    ...
    Return<Status> ret = gLightHalHidlV2_0_->setLight(light_type, brightness);
...
static void setLight(LightType type, int color) {
    if (gLightHalAidl_ != nullptr) {
        int bcolor = color & 0x000000ff;
        bcolor = 0xff000000 | (bcolor << 16) | (bcolor << 8) | bcolor;
        HwLightState lightState = {
            .color = bcolor,
        ...
        int id = 0;
        for (auto i = supportedLights.begin(); i != supportedLights.end(); i++) {
            if ((*i).type == type)
                id = (*i).id;
            }
        ndk::ScopedAStatus status = gLightHalAidl_->setLightState(id, lightState);
        if (!status.isOk())
            LOGE("Failed to set light to 0x%x for type %d, ret=%d\n",
                lightState.color, id, status.getExceptionCode());
    } else {
        write_int(static_cast<Type>(type), color);
    }
}
...
{
    setLight(LightType::BACKLIGHT, brightness_seq[seq_index]);
...
static int backlight_entry(struct ftm_param *param, void *priv)
{
    ...
    setLight(LightType::BACKLIGHT, 255);

    return 0;
}

static HalVersion connectLightHal() {
    if (gLightHalAidl_ == nullptr && ILights::descriptor != nullptr) {
        const std::string instance = std::string() + ILights::descriptor + "/default";
        ndk::SpAIBinder lightBinder = ndk::SpAIBinder(
            AServiceManager_getService(instance.c_str()));
        gLightHalAidl_ = ILights::fromBinder(lightBinder);
    }
    if (gLightHalAidl_ != nullptr) {
        LOGD("Successfully connected to Light HAL AIDL service.");
        ndk::ScopedAStatus status = gLightHalAidl_->getLights(&supportedLights);
        if (!status.isOk()) {
            LOGE("Failed to get available lights, ret=%d\n",
                status.getExceptionCode());
            gLightHalAidl_ = nullptr;
        } else {
               return HalVersion::AIDL;
        }
    } else {
        LOGD("Couldn't load Light HAL AIDL service");

        if (gLightHalHidlV2_0_ == nullptr) {
            gLightHalHidlV2_0_ = ILight::getService();
        }
        if (gLightHalHidlV2_0_ != nullptr) {
            LOGD("Successfully connected to Light HAL HIDL 2.0 service.");
            return HalVersion::HIDL_2_0;
       }
    }
    LOGD("Failed connected to Light service.");
...
int backlight_init(void)
{
   ...
   if (connectLightHal() == HalVersion::NONE) {
        LOGE("%s: unable to getService by aidl.\n", __func__);
        return -ENODEV;
    }
   if (read_int(BOOTMODE_PATH) == META_BOOT) {
        LOGD(TAG "Meta mode, off backlight");
        setLight(LightType::BACKLIGHT, 0);
    } else {
        LOGD(TAG "!Meta mode, on backlight");
        setLight(LightType::BACKLIGHT, 255);
    }
