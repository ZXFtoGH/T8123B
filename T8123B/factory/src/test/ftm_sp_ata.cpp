...
#ifdef FEATURE_FTM_NTC
        {14, ITEM_NTC, "AT+NTC", (hdlr)display_ntc_ata},
#endif
#ifdef FEATURE_FTM_KEYBOX
        {15, ITEM_KEYBOX, "AT+KEYBOX", (hdlr)display_keybox_state_ata},
#endif

#ifdef FEATURE_FTM_DCDC
        {16, ITEM_DCDC, "AT+DCDC", (hdlr)display_dcdc_state_ata},
#endif
        {17, ITEM_GMS_INFO, "AT+GMS", (hdlr)display_gms_ata},
        {ITEM_MAX_IDS, ITEM_MAX_IDS,  "", (hdlr)NULL},
};

static ftm_cmd_hdlr cmd_hdlr[ITEM_MAX_IDS]={
        ...
        {17, ITEM_CLRESTORAGE, "AT+FLASH", (hdlr)ftm_item_entry_cb},
    #endif
    #else  /* NAND */
...
/* goodix end */
	...
	{45, ITEM_HDMI, "AT+HDMI", (hdlr)ftm_item_entry_cb},
        ...
        {46, ITEM_HALL, "AT+HALL", (hdlr)ftm_item_entry_cb},
    #endif
...
//  #endif
	...
	//call init_cmd_hdlr to insert item by dynamic
};

static bool insert_cmd_hdlr(ftm_cmd_hdlr &item)
{
	int idx = 0;
	ftm_cmd_hdlr end_item = {ITEM_MAX_IDS, ITEM_MAX_IDS,  "", (hdlr)NULL};
	for(idx; idx<ITEM_MAX_IDS-1; idx++)
	{
		if(cmd_hdlr[idx].cmd_index == ITEM_MAX_IDS)
			break;
	}

	if(idx == ITEM_MAX_IDS-1)
	{
		LOGD(TAG "no memory to insert cmd hdlr\n");
		return false;
	}

	memcpy(&cmd_hdlr[idx], &item, sizeof(ftm_cmd_hdlr));
	memcpy(&cmd_hdlr[idx+1], &end_item, sizeof(ftm_cmd_hdlr));
...
{
#ifdef FEATURE_FTM_SLAVE_CHARGER
	if(is_support_dual_charger())
	{
		ftm_cmd_hdlr item = {46, ITEM_SLAVE_CHARGER, "AT+SLAVECHARGER", (hdlr)ftm_item_entry_cb};
		insert_cmd_hdlr(item);
	}
#endif
...
/*
	if(is_support_xxxx())
	{
		ftm_cmd_hdlr item = {48, ITEM_xxxx, "AT+xxxx", (hdlr)ftm_item_entry_cb};
		insert_cmd_hdlr(item);
	}
*/

	for(int i=0; i<ITEM_MAX_IDS; i++)
	{
		if(cmd_hdlr[i].cmd_index == ITEM_MAX_IDS)
			break;

		LOGD(TAG "ftm_cmd_hdlr string - %s\n", cmd_hdlr[i].cmd_string);
	}
}
...
static void ftm_set_property(at_cmd *test_item_struct, char *test_result)
{
	...
	if(strlen(test_item_struct->param) > sizeof(param)){
    	strncpy(param, test_item_struct->param, sizeof(param)-1);
	}else{
    ...
    if(value == NULL)
    {
		...
		test_result[strlen("parameter is wrong!\r\n")] = '\0';
        return;
    }
    else
    {
...
char* dispatch_data_to_pc_cb(at_cmd *test_item_struct, char* result)
{
    ...
    if((test_item_struct == NULL) || (result == NULL))
    {
        ...
        LOGD(TAG "Information invalid and quit\n");
        return NULL ;
    }
    LOGD(TAG "test_item_struct->index %d compared by other_cmd_hdlr %lu\n", test_item_struct->index,sizeof(other_cmd_hdlr)/sizeof(other_cmd_hdlr[0]));
    LOGD(TAG "test_item_struct->cmd_type %d\n", test_item_struct->cmd_type);
...
static char* ftm_item_add_cb(at_cmd *test_item_struct, char* test_result)
{
	...
	LOGD(TAG "test_item_struct->index %d compared by cmd_hdlr %lu\n", test_item_struct->index,sizeof(cmd_hdlr)/sizeof(cmd_hdlr[0]));
...
MSocket* createSocket(SOCKET_TYPE type)
{
	for(int i=0; i<SOCKET_END; i++)
	{
		if(m_socket[i] == NULL)
		{
	        ...
	        {
		        LOGD(TAG "[Socket] Create socket fail.");
				return NULL;
			}
		}
	}
	
	LOGD(TAG "[Socket] no empty socket object!");
	return NULL;
...
MSocket* getSocket(SOCKET_TYPE type)
{
	...
	LOGD(TAG "[Socket] To get socket object!");
	for(int i=0; i<SOCKET_END; i++)
	{
		if(m_socket[i] != NULL)
		{
			LOGD(TAG "[Meta][Socket] Get socket,  idx=%d, pSocket=0x%08x, type=%d", i, m_socket[i], m_socket[i]->m_type);		
			if(m_socket[i]->m_type == type)
				return m_socket[i];
		}
	}

	LOGD(TAG "[Socket] Can not find socket object!");
	return NULL;	
}

void delSocket(SOCKET_TYPE type)
{
	LOGD(TAG "[Socket] To delete socket object!");
	for(int i=0; i<SOCKET_END; i++)
	{
		if(m_socket[i] != NULL)
		{
...
static bool is_pc_control()
{
    ...
    if(pSocket == NULL){
        LOGD(TAG "Cread socket failed");
        return 	pc_control;
    }

    while(1)
    {
    	if(!pSocket->getClientConnectStatus())
		{
			if(-1 == pSocket->connect())
			{
				pSocket->close_server();
				return pc_control;
			}
		}
		
		usb_com_port = pSocket->getClientSocketID();
    ...
    else if(read_from_usb > 3)
    {
        ...
        LOGD(TAG "read_from_usb > 0, len=%zd\n", strlen(USB_read_buffer));
          ...
          if(at_command_type == 0)
          {
              ...
              if(item_testing == 1)
              {
                  if(at_cmd_struct.index < 0|| at_cmd_struct.index >= sizeof(cmd_hdlr)/sizeof(cmd_hdlr[0])){
				      LOGD(TAG "over cmd_hdlr size\n");
				      break;
			      }
              ...
              if(at_cmd_struct.cmd_type == 0)
              {
				  if(at_cmd_struct.index < 0|| at_cmd_struct.index >= sizeof(other_cmd_hdlr)/sizeof(other_cmd_hdlr[0])){
					  LOGD(TAG "over other_cmd_hdlr size\n");
					  break;
				  }
...
int init_atci_socket()
{
	...
	if(pSocket == NULL)
	{
		...
		if(pSocket != NULL)
		{
			int bInit = pSocket->initServer("factory-atci", ANDROID_SOCKET_NAMESPACE_RESERVED,false);
			if(bInit == 0)
			{
