...
int find_auxadc_iio_path(char * temp_path)
{
	...
	char *iio_path1 = NULL, *iio_path2 = NULL, *iio_path_tmp = NULL;;
	...
	if (dir) {
		while ((entry = readdir(dir)) != NULL) {
			if (strstr(entry->d_name, "auxadc")) {
				str1_exist = true;
				iio_path_tmp = NULL;
				ret = asprintf(&iio_path_tmp, "%s/%s", AP_AUXADC_IIO_PATH, entry->d_name);
				break;
			}
		}
		closedir(dir);
		...
		if ((iio_path_tmp == NULL) || (ret <= 0))
			goto err;

		iio_path1 = iio_path_tmp;
		...
		if (dir) {
			while ((entry = readdir(dir)) != NULL) {
				if(strstr(entry->d_name, "iio")) {
					str2_exist = true;
					iio_path_tmp = NULL;
					ret = asprintf(&iio_path_tmp, "%s/%s", iio_path1, entry->d_name);
					break;
				}
			}
			closedir(dir);
			if (!str2_exist) {
				free(iio_path1);
				goto err;
			} else if ((iio_path_tmp != NULL) && (ret > 0)) {
				iio_path2 = iio_path_tmp;
...
iio:
	for (i = 0; i < MAX_CHN_NUM; i++) {
		iio_full_path2 = NULL;
		ret = asprintf(&iio_full_path2, "%s/%s", tmp_buf, auxadc_sample_data[i]);

		if ((iio_full_path2 == NULL) || (ret <= 0))
			continue;

		iio_full_path = iio_full_path2;
...
int ap_auxadc_entry(struct ftm_param *param, void *priv)
{
	...
	if (len < 0) {
		LOGE(AUXADC_LOG_TAG"fail snprintf, err = %d\n", len);
		return -1;
	}
