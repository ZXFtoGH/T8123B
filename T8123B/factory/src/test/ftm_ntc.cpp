...
void print_ntc(char *info, int *len, char *tag, char *msg)                                               
{                                                                                                            
	char buf[256] = {0};                                                                                       
	int _len = 0;                                                                                              
	int tag_len = 0;                                                                                           
	...
	*len = _len;                                                                                               
	//LOGD(TAG "In factory mode: g_ntc_nr_lines = %d\n", g_ntc_nr_lines);                                              
}                                                                                                                                                                                                                   
 
                                                                                                           
static int get_ntc_value(char *info, int *len)                                                           
{                                                                                                            
    char val[128] = {0};                                                                                     
    char ntc_var[32] = "-1";                                                                        
    ...
    ntc_fd = open("/sys/class/power_supply/battery/temperature",O_RDONLY);
    if(ntc_fd!=-1)                                                                                    
    {                                                                                                        
        read(ntc_fd, ntc_var, 32);                                                                
        close(ntc_fd);                                                                                
    }                                                                                                        
                                                                                    
    if(ntc_var[strlen(ntc_var)-1]=='\n') ntc_var[strlen(ntc_var)-1]=0;                           
                                                                                                             
                                                   
    print_ntc(info, len,  "NTC     ", ntc_var);                                                          
                                                                                                                                              
                                              
    return_data.ntc.ntc = atoi(ntc_var);
                                                                                                             
    return *len;                                                                                              
}                                                                                                            

                                                                                                             
int get_ntc(char *info, int size)                                                              
{                                                                                                            
                                                                                                             
    int len = 0;                                                                                             
    g_ntc_nr_lines = 0;                                                                                            
                                                                                                             
    get_ntc_value(info, &len);                                                                                                                                               
                                                                                                             
    return 0;                                                                                                
}                                                                                                            
 
                                                                                                            
char ** trans_ntc(const char *str, int *line)                                                            
{                                                                                                            
	char **pstrs = NULL;                                                                                       
	int  len     = 0;                                                                                          
	int  row     = 0;                                                                                          
    ...
    pstrs = (char**)malloc(g_ntc_nr_lines * sizeof(char*));                                                      
...
void ntc_tear_down(char **pstr, int row)                                                                         
{                                                                                                            
    int i;                                                                                                   
    if(pstr == NULL)                                                                                         
    {                                                                                                        
...
*/                                                                                                           
char* display_ntc_ata(int index, char* result)                                                           
{                                                                                                            
    if(result == NULL)                                                                                       
    {                                                                                                        
        return NULL;                                                                                         
    }                                                                                                        
    return display_ntc(index, result, true);	                                                           
}                                                                                                            


                                                                                                          
char* display_ntc(int index, char* result, bool autoreturn)                                              
{                                                                                                            
	char *buf = NULL;                                                                                          
	...
	vi.set_text(&vi, &info);                                                                                   
	vi.m_pstr = trans_ntc(info.string, &nr_line);                                                          
	vi.m_nr_lines = g_ntc_nr_lines;                                                                                
	LOGD(TAG "g_ntc_nr_lines is %d, avail_lines is %d\n", g_ntc_nr_lines, avail_lines);                                
	vi.m_start = 0;                                                                                            
	...
	LOGD(TAG "Before ntc_tear_down\n");                                                                            
	ntc_tear_down(vi.m_pstr, nr_line);                                                                             
	if (buf)                                                                                                   
    {                                                                                                        
		free(buf);                                                                                               
        buf = NULL;                                                                                          
