...
#define DATA_USER_PARTITION "/data/user/0"
...
static int get_boot_type(void) {
	...
	close(fd);
	if (s <= 0) {
		LOGD(TAG "could not read kernel version\n");
		return -1;
	}

	ver[s-1] = 0x0;
	if (sscanf(ver, "Linux version %d.%d", &maj, &min) == EOF) {
		LOGD(TAG "could not read kernel version\n");
		return -1;
	}

	/* Old kernel support combo, 4.4/4.9/4.14 use boot_type node */
	if (maj == 4 && min < 19)
		goto old_boot_type;

	/* New kernel support combo, after 4.19 use atag,boot */
	fd = open("/proc/device-tree/chosen/atag,boot", O_RDONLY);
	if (fd < 0) {
		LOGD(TAG "fail to open: %s\n", "/proc/device-tree/chosen/atag,boot");
		return -1;
	}

	s = read(fd, &buffer, sizeof(int)*8);
	close(fd);
	if (s <= 0) {
		LOGD(TAG "could not read boot type\n");
		return -1;
	}

	LOGD(TAG "read boot type = %d\n", buffer[3]);
	return buffer[3];
...
int ensure_root_path_unmounted(const char *root_path)
{
	...
	if(ret != 0) {
		LOGD(TAG "unmount_mounted_volume fail: %s, do retry\n",
			strerror(errno));
		ret = unmount_mounted_volume_detach(volume);
	}

	if (ret < 0)
		LOGD(TAG "unmount_mounted_volume_detach fail: %s\n", strerror(errno));
...
int clear_bootdev_entry(struct ftm_param *param, void *priv)
{
	...
	while (cnt++ <= 3) {
		...
		result = ensure_root_path_unmounted(DATA_USER_PARTITION);
		if (result) {
			sleep(1);
			continue;
		}
...
int clear_bootdev_init(void)
{
	...
	mod = ftm_alloc(ITEM_CLRESTORAGE, sizeof(struct bootdev));
	if (!mod)
		return -ENOMEM;
