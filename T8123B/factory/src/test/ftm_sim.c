...
static int CheckSimExist(const char* rsp_buf) {
    ...
    //+ESIMEXIST: 1
    retValue = sprintf(tok_esim_exist, "+%s: ", AT_ESIMEXIST);
    if (retValue < 0) {
        return -1;
    }
...
int sim_entry(struct ftm_param *param, void *priv)
{
...
#ifdef FTM_SIM_SWITCH
    retValue = snprintf(dev_node_1, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM_IOCTL, MD_SYS1));
    if (retValue < 0) {
        return -1;
    }
    retValue = snprintf(dev_node_2, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM_IOCTL, MD_SYS5));
    if (retValue < 0) {
        return -1;
    }
    retValue = snprintf(dev_node_data_1, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS1));
    if (retValue < 0) {
        return -1;
    }
    retValue = snprintf(dev_node_data_2, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS5));
    if (retValue < 0) {
        return -1;
    }
    ...
    pthread_create(&sim->update_thread, NULL, sim_update_thread_for_sim_swtich, priv);
        ...
        retValue = snprintf(dev_node_1, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS1));
        if (retValue < 0) {
            return -1;
        }
        retValue = snprintf(dev_node_2, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS2));
        if (retValue < 0) {
            return -1;
        }
        ...
        /* Disable MD3 SIM detection, since external SIM switch is supported, 2015/01/19
           ...
           Disable MD3 SIM detection, since external SIM switch is supported, 2015/01/19 }*/
        if (isMDENSupport(FTM_SIM_ENABLE_MD1)) {
            retValue = snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS1));
        } else if (isMDENSupport(FTM_SIM_ENABLE_MD2)) {
            retValue = snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS2));
        } else if (isMDENSupport(FTM_SIM_ENABLE_MD5)) {
            retValue = snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_FACTORY_SIM, MD_SYS5));
        } else {
            LOGD("not open md1,md2,md5");
        }
        if (retValue < 0) {
...
static int check3GSwitchStatus(const int fd) {
    ...
    while (p_es3g == NULL) {
        ...
        LOGD(TAG "3G Capability: rsp_len=%zu, rsp_buf=%s\n", strlen(rsp_buf), rsp_buf);
...
static void sendEsuo(const int fd, int value, int wait_echo) {
    ...
    char cmd_buf[BUF_SIZE] = {0};
    char rsp_buf[BUF_SIZE] = {0};
    retValue = sprintf(cmd_buf, "AT+ESUO=%d\r\n", value);
    if (retValue < 0) {
        return;
    }
    write(fd, cmd_buf, strlen(cmd_buf));
...
static int sim_detect(const int fd, int id) {
    ...
    for (j = 0; j < 5; j++) {
        ...
        tobreak = 0;
        while (tobreak == 0) {
            nread = read(fd, rsp_buf, BUF_SIZE - 1);
            rsp_len = strlen(rsp_buf);
            LOGD(TAG "------AT+ESIMS(SIM%d) start------\n", id);
            LOGD(TAG "nread= %d len=%d buf=%s \n", nread, rsp_len,rsp_buf);
            LOGD(TAG "------AT+ESIMS(SIM%d) end------\n", id);
