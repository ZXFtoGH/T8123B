...
// To ignore the warning of chinese character (GB 2312)
...
bool bSaveJPEGImg(char *a_pBuf,  unsigned int a_u4Size)
{
    int strCnt = 0;
    MINT32 i4WriteCnt = 0;
    strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s//%04d.jpg" , MEDIA_PATH, g_u4ImgCnt);

    if ((strCnt <= 0) || (strCnt > FILE_NAME_SIZE)) {
        FTM_CAMERA_ERR("snprintf fail");
        return MFALSE;
    }
    ...
    i4WriteCnt = fwrite(a_pBuf, 1, a_u4Size , pFp);
    if (i4WriteCnt != a_u4Size)
        FTM_CAMERA_ERR("fwrite failed\n");

    FTM_CAMERA_DBG("Save image file name:%s\n", szFileName);

    if (fclose(pFp))
        FTM_CAMERA_ERR("fclose failed\n");
                         ...
                         AnaImage* I)
{
    ...
    MUINT16 *R = NULL, *GR = NULL, *B = NULL, *GB = NULL, *rawTmp = NULL;
    MUINT32 rows = height >> 1;
    MUINT32 cols = width >> 1;
    MUINT32 size = rows * cols;
    int nread = 0, strCnt = 0;

    char szFileName[FILE_NAME_SIZE];
    strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/ftm_pureRAW_10.raw" , MEDIA_PATH);
    ...
    if (rawTmp == NULL) {
        FTM_CAMERA_ERR("[Error] %s: rawTmp malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    R  = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (R == NULL) {
        FTM_CAMERA_ERR("[Error] %s: R malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    GR = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (GR == NULL) {
        FTM_CAMERA_ERR("[Error] %s: GR malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    GB = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (GB == NULL) {
        FTM_CAMERA_ERR("[Error] %s: GB malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    B  = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (B == NULL) {
        FTM_CAMERA_ERR("[Error] %s: B malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    ...
    if (nread != (size << 2)) {
        FTM_CAMERA_DBG("[Warning] %s: EOF or fread error.\n", __FUNCTION__);
    }
                         ...
                         AnaImage* I)
{
    ...
    MUINT8  *yuvTmp = NULL;
    MUINT16 *YU = NULL, *YV = NULL, *U = NULL, *V = NULL;
    MUINT32 rows = height;
    MUINT32 cols = width >> 1;
    ...
    switch(format)
    {
        case YUVFmt_UYVY : strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/YUVImg_UYVY.yuv" , MEDIA_PATH);
            break;
        case YUVFmt_VYUY : strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/YUVImg_VYUY.yuv" , MEDIA_PATH);
            break;
        case YUVFmt_YUY2 : strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/YUVImg_YUY2.yuv" , MEDIA_PATH);
            break;
        case YUVFmt_YVYU : strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/YUVImg_YVYU.yuv" , MEDIA_PATH);
            break;
        default: strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/YUVImg_Unkonw.yuv" , MEDIA_PATH);
            break;
    }
    ...
    if (yuvTmp == NULL) {
        FTM_CAMERA_ERR("[Error] %s: yuvTmp malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    YU = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (YU == NULL) {
        FTM_CAMERA_ERR("[Error] %s: YU malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    YV = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (YV == NULL) {
        FTM_CAMERA_ERR("[Error] %s: YV malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    U  = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (U == NULL) {
        FTM_CAMERA_ERR("[Error] %s: U malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
    V  = (MUINT16 *)malloc(sizeof(MUINT16) * size);
    if (V == NULL) {
        FTM_CAMERA_ERR("[Error] %s: V malloc failed\n",__FUNCTION__);
        goto FAIL_EXIT;
    }
                           ...
                           MUINT32 inShutter)
{
    ...
    MINT32 *histC1 = NULL, *histC2 = NULL, *histC3 = NULL, *histC4 = NULL;
    ...
    if (histC1 == NULL) {
        FTM_CAMERA_ERR("histCl is null\n");
        return 0;
    }
    histC2 = (MINT32 *)malloc(sizeof(MINT32) << bit);
    if (histC2 == NULL) {
        free(histC1);
        FTM_CAMERA_ERR("histC2 is null\n");
        return 0;
    }
    ...
    if (breakPoint == NULL) {
        ...
        free(histC3);
        free(histC4);
        FTM_CAMERA_ERR("breakPoint is null\n");
        return 0;
    }
...
//
/////////////////////////////////////////////////////////////////////////
static void vCapCb(void *a_pParam __attribute__((__unused__)))
{
...
static void vAutoCapCb_patg(void *a_pParam)
{
    ...
    if(pImgBufInfo->eImgType == PURE_RAW10_TYPE)
    {
        ...
        if((gPinError_pass == MFALSE) || u32ATA_Mode)
        {
            ...
            int strCnt = 0;
            if(u32ATA_Mode)
                strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/ftm_CamRawData.raw" , MEDIA_PATH);
            else
                strCnt = snprintf(szFileName, FILE_NAME_SIZE, "%s/ftm_pureRAW_10.raw" , MEDIA_PATH);
            ...
            if (i4WriteCnt != pImgBufInfo->RAWImgBufInfo.imgSize)
                FTM_CAMERA_ERR("[vAutoCapCb_patg] fwrite failed\n");

            if (fflush(pFp))
                FTM_CAMERA_ERR("[vAutoCapCb_patg] fflush failed\n");
            ...
            if(0 != fsync(fileno(pFp)))
            {
                ...
                gCapCBDone = MTRUE;
                if (fclose(pFp))
                    FTM_CAMERA_ERR("[vAutoCapCb_patg] fclose failed\n");
    ...
    else if(pImgBufInfo->eImgType == YUV_TYPE)
    {
        ...
        if((gPinError_pass == MFALSE) || u32ATA_Mode)
        {
            ...
            if (i4WriteCnt != pImgBufInfo->imgBufInfo.imgSize)
                FTM_CAMERA_ERR("[vAutoCapCb_patg] fwrite fail\n");
...
static int camera_capture_autotest_patg()
{
    ...
    MUINT32 u4RetLen = 0, u32checksum = 0;
    MBOOL bRet;
    char *rawVal = NULL;
...
MVOID vQuickViewCallback(MVOID *a_pArg __attribute__((__unused__)))
{
        ...
        while(1)
        {
            ...
            if(camera_state == CAMERA_STATE_IDLE)
            {
                ...
                if(0 != acdkIFInit())
                {
                    ...
                    strCnt = sprintf(cam->info, "acdkIFInit Fail\n");
                    if (strCnt <= 0) {
                        FTM_CAMERA_ERR("sprintf fail");
                    }
                    iv->set_text(iv, &cam->text);
                    iv->redraw(iv);
    ...
    else
    {
        ...
        do
        {
            ...
            case ITEM_PASS:
                ...
                {
                    memset(cam->info, 0, 1024);
                    strCnt = sprintf(cam->info, "Not test done !! \n");
                    if (strCnt <= 0)
                        FTM_CAMERA_ERR("sprint fail\n");
                    iv->set_text(iv, &cam->text);
                    iv->redraw(iv);
                }
                break;
