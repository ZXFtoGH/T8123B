...
#define OTG_STATE_FILE "/sys/class/switch/otg_state/state"
...
/*
 * Return 1 if otg_state is 0 or if the online status of USB/AC is 1.
 * Otherwise, return 0.
 * If file doesn't exist, return -1.
 */
static int get_online_status(void)
{
	int fd = -1;
	int ret = 0;
	char otg_buf[5] = {0};
	char ac_buf[5] = {0};
	char usb_buf[5] = {0};
	int otg_sta, ac_online, usb_online;

	/*
	 * Find otg_state first, if it exists then check its state;
	 * Else, check the online status of usb and ac power supply.
	 */
	fd = open(OTG_STATE_FILE, O_RDONLY, 0);
	if (fd < 0) {
		LOGD(TAG "Can't open %s, judge online status instead\n",
			OTG_STATE_FILE);
		goto online_sta;
	}
	ret = read(fd, otg_buf, sizeof(otg_buf));
	if (ret < 0) {
		LOGD(TAG "%s read fail, ret=%d\n", OTG_STATE_FILE, errno);
		close(fd);
		return -1;
	};
	otg_sta = atoi(otg_buf);
	close(fd);

	return !otg_sta;

online_sta:
	fd = open(AC_ONLINE_FILE, O_RDONLY, 0);
	if (fd < 0) {
		LOGD(TAG "Can't open %s\n", AC_ONLINE_FILE);
		return -1;
	}
	ret = read(fd, ac_buf, sizeof(ac_buf));
	if (ret < 0) {
		LOGD(TAG "%s read fail, ret=%d\n", AC_ONLINE_FILE, errno);
		close(fd);
		return -1;
	};
	ac_online = atoi(ac_buf);
	close(fd);

	fd = open(USB_ONLINE_FILE, O_RDONLY, 0);
	if (fd < 0) {
		LOGD(TAG "Can't open %s\n", USB_ONLINE_FILE);
		return -1;
	}
	ret = read(fd, usb_buf, sizeof(usb_buf));
	if (ret < 0) {
		LOGD(TAG "%s read fail, ret=%d\n", USB_ONLINE_FILE, errno);
		close(fd);
		return -1;
	};
	usb_online = atoi(usb_buf);
	close(fd);
...
int battery_entry(struct ftm_param *param, void *priv)
{
	...
	if (gBatteryAutoTest) {
	iv->redraw(iv);
	sleep(1); //waiting for screen display
	...
	if(temp_v_bat > AUTO_TEST_THD_VALUE)
	{
		...
		//ignore I_charging test because the battery is full, no charging current
               ...
               LOGD(TAG "[FTM_BAT 2] ignore I_charging test because the battery is full, so set return current to 488mA\n");
