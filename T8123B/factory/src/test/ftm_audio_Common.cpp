...
int Common_Audio_init(void)
{
    ...
    if (gAudioHardware == NULL||AudioHwhndl_factory==NULL)
    {
        ...
        do
        {
            ...
            {
                ret_str = snprintf(path, sizeof(path), "%s/%s.default.so",
                                   HAL_LIBRARY_PATH1, prop);
                if (ret_str >= 0 && ret_str < sizeof(path)) {
            ...
            {
                ret_str = snprintf(path, sizeof(path), "%s/%s.%s.so",
...
bool ftmWavePlaybackCalib(void)
{
    return gAudioFtmBase->ftmWavePlaybackCalib();
}
//-------------------------------------------------------------------
//FUNCTION:
//      Audio_Wave_playback
...
static void *Audio_Wave_Playabck_routine(void *arg)
{
    ...
    if (read_size != 1) {
        ALOGW("%s(), fread files %s header fail, read_size: %d",
              __FUNCTION__, pWavePlaydata->FileName, read_size);
    }
  ...
  //create output stream
    ...
    if (!streamOutput)
    {
        ALOGE("%s() openOutputStream fail", __FUNCTION__);
    ...
    if (pWavePlaydata->i4Output == Output_HS)
    {
        ...
        rcvVolumeStr = ftm_get_prop("Audio.Rcv.Volume");
        if (rcvVolumeStr != NULL){
            rcvVolume = atof(rcvVolumeStr);
        } else {
            rcvVolume = 0.5;
        }
    }
    ...
    gAudioHardware->setMasterVolume(rcvVolume);
...
extern "C" {
    ...
    bool ftmWavePlaybackCalib() { return false; }
