...
int irtx_led_test_pre_set(void *priv)
{
    ...
    while (try_times-- > 0) {
        ...
        if (fd >= 0) {
            test_steps = 1;
            if (sprintf(irtxledstu->info + strlen(irtxledstu->info), "%s %s: %s \n"
                    , uistr_info_irtx_led_test, uistr_info_irtx_open_device, uistr_info_pass) < 0)
                LOGE(TAG "%s:%d, snprintf fail\n", __func__, __LINE__);
            ret = ioctl(fd, IRTX_IOC_SET_IRTX_LED_OUT, &enable);
            close(fd);
            if (ret) {
                if (sprintf(irtxledstu->info + strlen(irtxledstu->info), "%s %s: %s \n"
                        , uistr_info_irtx_led_test, uistr_info_irtx_call_ioctl, uistr_info_fail) < 0)
                    LOGE(TAG "%s:%d, snprintf fail\n", __func__, __LINE__);
                ...
                if (sprintf(irtxledstu->info + strlen(irtxledstu->info), "%s %s: %s \n"
                        , uistr_info_irtx_led_test, uistr_info_irtx_call_ioctl, uistr_info_pass) < 0)
                    LOGE(TAG "%s:%d, snprintf fail\n", __func__, __LINE__);
                break;
            }
        } else {
            ret = -1;
            if (sprintf(irtxledstu->info + strlen(irtxledstu->info), "%s %s: %s \n"
                    , uistr_info_irtx_led_test, uistr_info_irtx_open_device, uistr_info_fail) < 0)
                LOGE(TAG "%s:%d, snprintf fail\n", __func__, __LINE__);
            ...
            switch (chosen) {
                ...
                case C_BTN_DOWN:
                    ...
                    else
                    {
                        ...
                        if (sprintf(irtxledstu->info, "Not test done !! \n") < 0)
                            LOGE(TAG "%s:%d, snprintf fail\n", __func__, __LINE__);
                        iv->set_text(iv, &irtxledstu->text);
                        iv->redraw(iv);
                    }
