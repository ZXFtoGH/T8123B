...
extern ftm_init_fn ftm_init_funcs[ITEM_MAX_IDS];
...
static int ftm_default_entry(struct ftm_param *param,void *priv)
{
    ...
    //param->name;//Just for resovle build warning
...
void ftm_destroy(void)
{
	...
	for (i = 0; i < ITEM_MAX_IDS; i++) {
		if (!ftm_mods[i])
			continue;
		LOGD(TAG "ftm_destroy ftm_mods[i]->id=%d",ftm_mods[i]->id);
		ftm_free(ftm_mods[i]);
	}
...
struct ftm_module *ftm_alloc(int id, int extra)
{
    ...
    mod = (ftm_module *)malloc(sizeof(struct ftm_module) + extra);
...
{
    struct ftm_prop *p=NULL, *h=NULL, *t=NULL;
    ...
    LOGD(TAG "%s name(%s) value(%s)", __FUNCTION__, name, val);
    namelen  = strlen(name);
    valuelen = strlen(val);
    ...
    p = (struct ftm_prop *) malloc(sizeof(struct ftm_prop) + namelen + 1);
    if (p == NULL)
        return -ENOMEM;

    memset(p, 0, sizeof(struct ftm_prop) + namelen + 1);
