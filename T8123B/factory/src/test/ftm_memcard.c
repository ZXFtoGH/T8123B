...
static bool mcard_avail(struct mcard *mc)
{
    ...
    if(sprintf(name, "mmc%d", mc->id - MIN_SDCARD_IDX) < 0){
        LOGD(TAG "sprintf name failed\n");
    }
...
#define UFS_BLOCK_DEVICE "/dev/block/sdc"
static bool check_ufs(void) {
    if(access(UFS_BLOCK_DEVICE, 0) == 0){//test for existence
        return true;
     }
