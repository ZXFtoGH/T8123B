...
#define FM_FACTORY_AUDIO_SUPPORT
#include <linux/fm.h>
#ifdef FM_FACTORY_AUDIO_SUPPORT
#include "ftm_audio_Common.h"
#endif /* FM_FACTORY_AUDIO_SUPPORT */
...
static int fm_get_headset_info(void)
{
    ...
    #define HEADSET_NEW_STATE_PATH "/sys/bus/platform/drivers/pmic-codec-accdet/state"
    ...
    if (fd < 0){
        LOGD(TAG "Can't open %s, try new path\n", HEADSET_STATE_PATH);
        fd = open(HEADSET_NEW_STATE_PATH, O_RDONLY, 0);
        if (fd < 0) {
            LOGD(TAG "Can't open %s\n", HEADSET_NEW_STATE_PATH);
            ret = -1;
            goto out;
        }
    }
...
int fm_entry(struct ftm_param *param, void *priv)
{
    ...
    fmr->fgAudioDisable = 0;
    rawVal = ftm_get_prop("FMRadio.AudioDisable");
    if (rawVal) {
        fmr->fgAudioDisable = atoi(rawVal);
        LOGD(TAG "fgAudioDisable:%d\n", fmr->fgAudioDisable);
    }
    ...
    if (fmr->fgAudioDisable == 0) {
        ...
        for (i=0; i<8; i++) {
            ...
            if (headset_state > 0) {
                ...
                ret = sprintf(fmr->info, "%s", uistr_info_fmr_no_headset_warning);
                if (ret < 0) {
...
if(FTM_AUTO_ITEM == param->test_type)
{
    ...
    if (g_rssi >= rssi_th) {
        ...
        ret = snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info)-strlen(fmr->info)-1, uistr_info_fmr_pass, rssi_th);
        if (ret < 0) {
            LOGD(TAG "sprintf failed\n");
        }
        iv->redraw(iv);
        ...
        ret = snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info)-strlen(fmr->info)-1, uistr_info_fmr_failed, rssi_th);
        if (ret < 0) {
            LOGD(TAG "sprintf failed\n");
        }
        iv->redraw(iv);
...
static void read_preferred_freq()
{
    ...
    for (i = 0; i < sizeof(fm_freq_list)/sizeof(fm_freq_list[0]); i++) {
        ...
        ret = sprintf(channel_no, "FMRadio.CH%d", i+1);
        if (ret < 0) {
            LOGD(TAG "sprintf failed\n");
        }
...
static int read_rssi_th(void)
{
    unsigned int rssi = FM_RSSI_PASS;
    char *pRSSI_TH = NULL;
    ...
    if (pRSSI_TH) {
        rssi = atoi(pRSSI_TH);
        LOGD(TAG "rssi th(from ini): %d\n", rssi);
    } else {
        LOGD(TAG "rssi th(default): %d\n", rssi);
    }
    return rssi;
}
...
static void *fm_update_thread(void *priv)
{
    ...
    if (g_fm_fd < 0) {
        ...
        ret = sprintf(fmr->info, "%s", uistr_info_fmr_open_fail);
        if (ret < 0) {
            LOGD(TAG "sprintf failed\n");
        }
        iv->redraw(iv);
 ...
 #endif
        g_fm_fd = -1;
        ret = sprintf(fmr->info, "%s", uistr_info_fmr_poweron_fail);
        if (ret < 0) {
            LOGD(TAG "sprintf failed\n");
        }
        iv->redraw(iv);
...
#endif
            g_fm_fd = -1;
            ret = sprintf(fmr->info, "%s", uistr_info_fmr_mute_fail);
            if (ret < 0) {
                LOGD(TAG "sprintf failed\n");
            }
            iv->redraw(iv);
    ...
    ret = sprintf(fmr->info, "%s", uistr_info_fmr_poweron_ok);
    if (ret < 0) {
        LOGD(TAG "sprintf failed\n");
    }
    iv->redraw(iv);
        ...
        if (g_setfreq) {
            ...
            if (fmr->fgAudioDisable == 0) {
                ...
                if (!FMAudioEnable) {
                    ...
                    if (ret) {
                        g_fm_fd = -1;
                        ret = sprintf(fmr->info, "%s", "matv_set_chipdep failed\n");
                        if (ret < 0) {
                            LOGD(TAG "sprintf failed\n");
                        }
                        iv->redraw(iv);
...
#endif
#else /* FM_FACTORY_AUDIO_SUPPORT */
            ret = 0;
#endif /* FM_FACTORY_AUDIO_SUPPORT */
            if (ret) {
                //sprintf(fmr->info+strlen(fmr->info), uistr_info_fmr_fail);
                snprintf(fmr->info + strlen(fmr->info), sizeof(fmr->info) - strlen(fmr->info) - 1, "%s", uistr_info_fmr_fail);
                iv->redraw(iv);
