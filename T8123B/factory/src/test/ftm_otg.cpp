...
#define DEV_BUS_PATH_NUM 3
static char* dev_bus_path[DEV_BUS_PATH_NUM] = {
	"/sys/bus/usb/devices/1-1/speed",
	"/sys/bus/usb/devices/2-1/speed",
	"/sys/bus/usb/devices/3-1/speed",
};
...
static void otg_update_info(struct otg *hds, char *info)
{
	...
	for (int i = 0; i  < DEV_BUS_PATH_NUM; i++) {
		fd_state = open(dev_bus_path[i], O_RDONLY, 0);
		if (fd_state < 0) {
			LOGD(TAG "Can't open %s\n", dev_bus_path[i]);
		} else {
			LOGD(TAG "Can open %s\n", dev_bus_path[i]);
...
#else
    if(device_state == 1)
        ptr += sprintf(ptr, "OTG devices plugged in, 5V applied!!\n");
    else
        ptr += sprintf(ptr, "OTG test fail !!\n");
#endif
    #endif
    return;
}
...
int otg_entry(struct ftm_param *param, void *priv)
{
    ...
    do {
        if(!get_is_ata()) {
			...
			else if(2 == com_type) //USB com port
			{
