...
extern int lens_main3_init(void);
...
ftm_init_fn ftm_init_funcs[ITEM_MAX_IDS] = {
...
bool insert_ftm_init_funcs(ftm_init_fn func)
{
	int idx = 0;
	for(idx; idx<ITEM_MAX_IDS-1; idx++)
	{
		if(ftm_init_funcs[idx] == NULL)
			break;
	}

	if(idx == ITEM_MAX_IDS-1)
	{
		LOGD(TAG "no memory to insert ftm_init_funcs");
		return false;
	}

	ftm_init_funcs[idx] = func;
	ftm_init_funcs[idx+1] = NULL;
...
void init_ftm_funcs()
{
	...
	{
		insert_ftm_init_funcs((ftm_init_fn)slave_chg_init);
	}
#endif

	if (is_support_efuse_writer())
	{
		insert_ftm_init_funcs((ftm_init_fn)efuse_init);
	}

/*
	if(is_support_xxx)
	{
		insert_ftm_init_funcs(xxx)
	}
*/
}
