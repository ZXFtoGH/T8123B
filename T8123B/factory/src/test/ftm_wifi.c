...
static void *wifi_update_thread(void *priv)
{
	...
	char *wifi_test_error_str[] = {uistr_info_wifi_iface_err,
				       uistr_info_wifi_fail_scan,
				       uistr_info_wifi_no_scan_res,
				       uistr_info_wifi_connect_err,
				       uistr_info_wifi_no_ap};
	int error_index = 0;
	...
	if (sprintf(wififm->info,
		"%s : %s\n",
		uistr_info_wifi_status,
		uistr_info_wifi_start) < 0) {
	...
	while (!wififm->exit_thd) {
		if (wififm->renew) {
			...
			if (error_index > 0) {
				...
				if (sprintf(wififm->info,
					"%s : %s\n",
					uistr_info_wifi_error,
					wifi_test_error_str[error_index-1]) < 0) {
					LOGE("[WIFI] sprintf failed!\n");
					goto init_fail;
				}
				iv->redraw(iv);
...
static void *wifi_update_thread_test(void *priv)
{
	...
	if (FM_WIFI_init(wififm->info, sizeof(wififm->info), &wififm->result) < 0) {
		...
		iv->redraw(iv);
		if (sprintf(wifi_result, "%d:%s", wififm->mod->id, result[FTM_TEST_FAIL]) < 0) {
			LOGE("[WIFI] sprintf failed!\n");
		}
	...
	//if(get_is_ata())
	{
		if (sprintf(wifi_result, "%d:%s", wififm->mod->id, result[wififm->mod->test_result]) < 0) {
			LOGE("[WIFI] sprintf failed!\n");
		}
		LOGD(TAG "WIFI result:%s", wifi_result);
		write_data_to_pc(wifi_result, strlen(wifi_result));
...
*/
#ifdef MTK_MT7902_ATA_TEST
	if (param->test_type != FTM_AUTO_ITEM){
		LOGD(TAG "[ERROR] MTK_MT7902_ATA_TEST can not test!\n");
		return -1;
	}
	// auto test
	if (access("/vendor/bin/mt7902_wifi_test.sh", 0) != 0) {
		LOGE(TAG "not found vendor/bin/mt7902_wifi_test.sh\n");
		wififm->mod->test_result = FTM_TEST_FAIL;
		return -1;
	}
	error_index = system("/vendor/bin/mt7902_wifi_test.sh");
	LOGE(TAG "excu mt7902_wifi_test.sh\n");
	if (WIFEXITED(error_index)) {
		LOGE(TAG "excu mt7902_wifi_test.sh  %d\n", WEXITSTATUS(error_index));
		if(WEXITSTATUS(error_index) == 1){
			LOGE(TAG "excu mt7902_wifi_test.sh pass\n");
			wififm->mod->test_result = FTM_TEST_PASS;

			return_data.wifi.channel = (-2);
			return_data.wifi.wifi_rssi = (-58);
			return_data.wifi.rate = 65;
			if (sprintf(return_data.wifi.wifi_mac,
				"%02x-%02x-%02x-%02x-%02x-%02x",
				0xaa, 0xa5, 0xe5, 0x67, 0x68, 0x15) < 0) {
				LOGE("[WIFI] sprintf failed!\n");
				return -1;
			}
			strncpy(return_data.wifi.wifi_name, "wifi_test", 9);
			return_data.wifi.wifi_name[10] = '\0';
		}else{
			LOGE(TAG "excu mt7902_wifi_test.sh failed\n");
			wififm->mod->test_result = FTM_TEST_FAIL;
		}
	}else{
		LOGE(TAG "cann't excu mt7902_wifi_test.sh failed\n");
	}
	...
	if (param->test_type == FTM_MANUAL_ITEM) {
		...
		while (i++ < 3) {
			...
			error_index = wifi_fm_test();
			if (error_index > 0) {
				LOGD("[WIFI][FTM_AUTO_ITEM] wifi_fm_test failed %d!\n", error_index);
				if (sprintf(wififm->info,
					"%s : %s\n",
					uistr_info_wifi_error,
					wifi_test_error_str[error_index-1]) < 0) {
