...
void md_init(int md_num) {
    ...
    for(i = 0; i < MAX_MODEM_INDEX; i++)
    {
        ...
        {
        	if(j < 0 || j >= MAX_MODEM_INDEX)
        	{
        	    break;
        	}
            if(strncpy(ccci_path[j],temp_ccci_path[i],
				sizeof(ccci_path[j])>strlen(temp_ccci_path[i])?strlen(temp_ccci_path[i]):sizeof(ccci_path[j]))){
				LOGD(TAG "strncpy failed\n");
			}
			ccci_path[j][31] = '\0';
            j++ ;
        }
...
int sigtest_entry(struct ftm_param *param, void *priv)
{
    ...
    if (FTM_MANUAL_ITEM == param->test_type) {
        ...
        if(sprintf(st->info, "%s\n", uistr_info_emergency_call_not_start) <0 ){
        ...
        do {
            ...
            switch (chosen) {
                ...
                case ITEM_CALL_FOR_MODEM_2:
                    pret = 0 ;
                    if(sprintf(st->info, "%s\n", uistr_info_emergency_call_testing) < 0){
                        LOGD(TAG "sprintf failed\n");
                    }
                    iv->redraw(iv);
                    ...
                    if(1 == pret) {
                        ...
                        if(sprintf(st->info, "%s%d\n", uistr_info_emergency_call_success_in_modem, chosen+1) <0 ){
								LOGD(TAG "sprintf failed\n");
							}
                    } else {
                        LOGD(TAG "Dial 112 Fail in modem%d \n", chosen+1);
                        if(sprintf(st->info, "%s%d\n", uistr_info_emergency_call_fail_in_modem, chosen+1) <0 ){
								LOGD(TAG "sprintf failed\n");
							}
                    }
                break;
        ...
        for (int i = 0; i < modem_number; i++) {
            if(sprintf(st->info, "%s in modem%d\n", uistr_info_emergency_call_testing, i+1) <0 ){
               	LOGD(TAG "sprintf failed\n");
            }
            iv->redraw(iv);
            ...
            { //C2K modem and it's the last member
                ...
                if(1 == pret) {
                    ...
                    if(sprintf(st->info, "%s%d\n", uistr_info_emergency_call_success_in_modem, i+1) <0 ){
                    	LOGD(TAG "sprintf failed\n");
					}
                    temp_result[i] = TRUE ;
                } else {
                    LOGD(TAG "Dial 112 Fail in c2k modem \n");
                    if(sprintf(st->info, "%s%d\n", uistr_info_emergency_call_fail_in_modem, i+1) <0 ){
                    	LOGD(TAG "sprintf failed\n");
					}
                    temp_result[i] = FALSE ;
                }
