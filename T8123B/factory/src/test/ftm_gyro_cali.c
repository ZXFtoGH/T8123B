...
static void *gyro_cali_update_iv_thread(void *priv)
{
...
#if 1
    flags = true;
    while ((err = ioctl(gyroc->fd, GYROSCOPE_IOCTL_INIT, &flags)) && (retry ++ < max_retry))
    usleep(retry_period*1000);
    if (err) {
        LOGD("enable gyro fail: %s", strerror(errno));
        return NULL;
    }
#endif
	...
	while(1)
	{
		...
		if(op == GYRO_OP_CLEAR)
		{
			...
			if(err)
			{
				len = snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "%s\n", uistr_info_sensor_cali_fail);
			}
			else
			{
		...
		else if(op == GYRO_OP_CALI)
		{
			if(!dat->gyroc.cali_delay || !dat->gyroc.cali_num || !dat->gyroc.cali_tolerance)
			{
				GSCLOGE("ignore calibration: %d %d %d\n", dat->gyroc.cali_delay, dat->gyroc.cali_num, dat->gyroc.cali_tolerance);                
			                          ...
			                          dat->gyroc.cali_tolerance, &cali)) != 0)
			{
	...
	flags = false;
    ioctl(gyroc->fd, GYROSCOPE_IOCTL_INIT, &flags);
