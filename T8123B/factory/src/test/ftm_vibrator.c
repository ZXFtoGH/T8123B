...
#define VIBRATOR_STATE_NEW "/sys/class/leds/vibrator/state"
...
int judge_vib_path(void)
{
    ...
    ret = access(VIBRATOR_STATE_NEW, F_OK);
    if (ret < 0) {
        LOGD("vibrator not has path:%s\n", VIBRATOR_STATE_NEW);
        return ret;
    }
...
static void *update_vibrator_thread_default(__attribute__((unused)) void *priv)
{
    ...
    if (vib_new_p == true && write_int(VIBRATOR_STATE_NEW, 1) < 0)
        LOGE("no vibrator state path\n");
...
int vibrator_autotest_entry(struct ftm_param *param, void *priv)
{
    ...
    if (write_int(VIBRATOR_ENABLE, 2000) == 0)
        ...
        write_int(VIBRATOR_STATE_NEW, 1) == 0 &&
