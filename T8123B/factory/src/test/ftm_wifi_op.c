...
int find_wifi_device()
{
	...
	if (fh != NULL) {
		...
		if (fgets(szbuff, sizeof(szbuff), fh) == NULL)
			goto out;
		if (fgets(szbuff, sizeof(szbuff), fh) == NULL)
			goto out;
   ...
   out:
		if (fclose(fh) != 0) {
			LOGE(TAG "[find_wifi_device] close file failed\n");
		}
	} else
		LOGE(TAG "[find_wifi_device]%s failed!!!\n", name);
...
int write_data_to_driver(char *data, size_t length)
{
	...
	for (retry_cont = 0; retry_cont < 5; retry_cont++) {
		LOGD("[write_data_to_driver] open retry_cont = (%d)",
								retry_cont);
	...
	if (fd < 0) {
		LOGE("[write_data_to_driver] open(%s) for write failed: %s (%d)",
				WIFI_POWER_PATH, strerror(errno), errno);
		goto out;
	}

	for (retry_cont = 0; retry_cont < 5; retry_cont++) {
		LOGD("[write_data_to_driver] write retry_cont = (%d)",
							retry_cont);
		sz = write(fd, data, length);

		if (sz < 0) {
#if 0
			LOGE("[write_data_to_driver] write(%s) failed: %s (%d)",
				WIFI_POWER_PATH, strerror(errno), errno);
			goto out;
#endif
...
int wifiNVRAMCtrl(bool fgSet, PNVRAM_ACCESS_STRUCT pnvram)
{
	...
	LOGD(TAG "NVRAMCtrl fg(%d) len(%d)\n",
				fgSet, pnvram->dataLen);


	if (fgSet) {
		LOGD(TAG "[wifiNVRAMCtrl] write offset:%d (0x%04X)\n",
				pnvram->dataOffset, pnvram->dataOffset);
		iFD = open(gNvInfo.cFileName,
				O_WRONLY | O_CREAT, S_IRUSR | S_IRGRP);

		if (iFD >= 0) {
			if (lseek(iFD, pnvram->dataOffset, SEEK_SET) < 0) {
				LOGE(TAG "[wifiNVRAMCtrl] read fail!");
				close(iFD);
				return -1;
			}
			...
			if (NVM_ProtectDataFile(gFile_wifi, 1) != 1) {
				LOGE(TAG "NVM_ProtectDataFile(): set failed\n");
				ret = -1;
			} else {
				// invoke auto backup mechanism
				NVM_AddBackupFileNum(gFile_wifi);
				ret = 0;
			}
		} else {
			LOGE(TAG "[wifiNVRAMCtrl] open file :%s fail!, iFD=%d",
							gNvInfo.cFileName, iFD);
			return -1;
		}
	} else {
		/* post-check for read access */
		LOGD(TAG "[wifiNVRAMCtrl] read offset:%d (0x%04X)\n",
					pnvram->dataOffset, pnvram->dataOffset);

		if (NVM_ProtectDataFile(gFile_wifi, 0) != 1) {
			LOGE(TAG "NVM_ProtectDataFile(): get failed\n");
			return -1;
		}
		...
		if (iFD >= 0) {
			if (lseek(iFD, pnvram->dataOffset, SEEK_SET) < 0) {
				LOGE(TAG "[wifiNVRAMCtrl] lseek fail!");
				close(iFD);
				return -1;
			}
			...
			if (readByteLen <= 0) {
				LOGE(TAG "[wifiNVRAMCtrl] read fail! ,readByteLen :%d",
									readByteLen);
				close(iFD);
				return -1;
			}
			...
			LOGE(TAG "[wifiNVRAMCtrl]  open file :%s fail!",
								gNvInfo.cFileName);
			return -1;
		}
	}
...
void wifwNVRAMWriteDataToDriver(void)
{
	...
	if (!pQueryNv) {
		LOGE(TAG "[wifwNVRAMWriteDataToDriver] out of memory in allocating pQueryNv\n");
		return ;
	}
	...
	if (!pCmdNvram) {
		LOGE(TAG "[wifwNVRAMWriteDataToDriver] out of memory in allocating acnvram\n");
		goto exit;
	}
	...
	LOGD(TAG " write NVRAM[%d] to driver done! ret =%d \n", readLen, ret);
...
static int wifi_set_power(int on)
{
	...
	LOGD("wifi_set_power, %s state %d", WIFI_POWER_PATH, on);

	fd = open(WIFI_POWER_PATH, O_WRONLY);
	if (fd < 0) {
		LOGE("open(%s) for write failed: %s (%d)", WIFI_POWER_PATH, strerror(errno), errno);
		goto out;
	}
	sz = write(fd, &buffer, 1);
	if (sz < 0) {
		LOGE("write(%s) failed: %s (%d)", WIFI_POWER_PATH, strerror(errno), errno);
		goto out;
	}
	ret = 0;
...
static int check_driver_loaded() {
    ...
    while ((fgets(line, sizeof(line), proc)) != NULL) {
        if (strncmp(line, DRIVER_MODULE_TAG, strlen(DRIVER_MODULE_TAG)) == 0) {
            if (fclose(proc) != 0) {
                LOGE(TAG "[check_driver_loaded] close file failed\n");
            }
            return 1;
        }
    }
...
int wifi_init_iface(char *ifname)
{
	...
	if (ioctl(s, SIOCGIFFLAGS, &ifr) != 0) {
		ret = errno ? -errno : -999;
		LOGE("Could not read interface %s flags: %s",
			   ifname, strerror(errno));
		ret = -1;
                goto exit;
	}

        //Make sure interface is up prior to set mode
        if (ifr.ifr_flags & IFF_UP) {
                LOGD("[WIFI] wifi interface is already up");
        } else {
                ifr.ifr_flags |= IFF_UP;
                if (ioctl(s, SIOCSIFFLAGS, &ifr) != 0) {
                        ret = errno ? -errno : -999;
                        LOGE("Could not set interface %s flags up: "
				"%s",
				ifname, strerror(errno));
                }
                LOGD("[WIFI] wifi interface is set to up");
        }

        LOGD("[WIFI] wifi_init_iface %s: set mode\n", ifname);
        struct iwreq wrq;
        char buf[33];
	...
	if (ioctl(s, SIOCSIWMODE, &wrq) < 0) {
		LOGE("ioctl(SIOCSIWMODE) fail, err is %s",
			strerror(errno));
		ret = -1;
		goto exit;
	}
	...
	if (ioctl(s, SIOCSIWESSID, &wrq) < 0) {
		LOGE("ioctl(SIOCSIWESSID) fail, err is %s",
			strerror(errno));
		ret = -1;
		goto exit;
	}
...
int wifi_load_driver(void)
{
	...
	if (!hcustlib) {
		LOGE("[wifi_load_driver] dlopen() fail! errno=%s\n", dlerror());
		return -1;
	}

	/* get AP_CFG_RDEB_FILE_WIFI_LID */
	pLID = (int *) dlsym(hcustlib, "iFileWIFILID");
	if (!pLID) {
		LOGE("[wifi_load_driver] dlsy() fail! errno=%s\n", dlerror());
		dlclose(hcustlib);
		return -1;
	}
	gFile_wifi = *pLID;
	...
	wifi_nvram_fd = NVM_GetFileDesc(gFile_wifi, &rec_size, &rec_num, ISWRITE);
	gNvInfo = NVM_ReadFileVerInfo(gFile_wifi);
	NVM_CloseFileDesc(wifi_nvram_fd);
	LOGD(TAG "NVRAM FileVer:%s\n", gNvInfo.cFileVer);
	LOGD(TAG "NVRAM FileName:%s\n", gNvInfo.cFileName);
	LOGD(TAG "NVRAM RecSize:%d\n", gNvInfo.i4RecSize);
	LOGD(TAG "NVRAM RecNum:%d\n", gNvInfo.i4RecNum);
	LOGD(TAG "NVRAM MaxFileLid:%d\n", gNvInfo.i4MaxFileLid);
	dlclose(hcustlib);

	wifwNVRAMWriteDataToDriver();

	if (wifi_set_power(1) != 0)
		LOGE("[WIFI] wifi_set_power on failed\n");
...
error:
	...
	if (wifi_set_power(0) != 0)
		LOGE("[WIFI] wifi_set_power off failed\n");
	return -1;
}
	...
	if ((has_range) && (range->we_version_compiled > 11))
	{
		...
		while (fgets(buf, 255, fp)) {
			...
			if(if_match && contain_comm) {
				...
				bp = strtok(bp, " ");
				if (bp == NULL) {
					LOGE("[iw_get_stats] strtok return NULL\n");
					break;
				}
				if (1 == sscanf(bp, "%X", &t)) {
					LOGD("status:%d\n", t);
					...
					LOGE("[iw_get_stats] sscanf failed!\n");
					break;
				}
				/* link */
				bp = strtok(NULL, " ");
				...
				if (strchr(bp, '.') != NULL) {
					...
					LOGE("[iw_get_stats] strchr return NULL\n");
					break;
				}
				if (1 == sscanf(bp, "%d", &t)) {
					LOGD("link: %d\n", t);
				...
				if (1 == sscanf(bp, "%d", &stats->discard.nwid)) {
					LOGD("nwid=%d\n", stats->discard.nwid);
				} else {
					LOGE("[iw_get_stats] sscanf failed!\n");
					break;
				...
				if (1 == sscanf(bp, "%d", &stats->discard.code)) {
					LOGD("code=%d\n", stats->discard.code);
				} else {
					LOGE("[iw_get_stats] sscanf failed!\n");
					break;
				...
				if (1 == sscanf(bp, "%d", &stats->discard.misc)) {
					LOGD("misc=%d\n", stats->discard.misc);
				} else {
					LOGE("[iw_get_stats] sscanf failed!\n");
					break;
				...
				if (fclose(fp) != 0) {
					LOGE(TAG "[iw_get_stats] close file failed\n");
				}
	...
	case SIOCGIWESSID:
		if (wscan) {
			...
			if ((event->u.essid.pointer) && (event->u.essid.length))
				memcpy(wscan->b.essid, event->u.essid.pointer,
	...
	case SIOCGIWENCODE:
		if (wscan) {
			...
			if (event->u.data.pointer)
				memcpy(wscan->b.key, event->u.essid.pointer,
						event->u.data.length);
			else
	 ...
	 struct wireless_info *	info)
{
	...
	if (iw_get_basic_config(skfd, ifname, &(info->b)) < 0) {
		...
		if (ioctl(skfd, SIOCGIFFLAGS, &ifr) < 0) {
			LOGE("SIOCGIFFLAGS fail, err is %s",
				strerror(errno));
			return(-ENODEV);
		} else {
...
int wifi_update_status(void)
{
	...
	if (sprintf(return_data.wifi.wifi_mac,
		"%02x-%02x-%02x-%02x-%02x-%02x",
		apinfo.mac[0], apinfo.mac[1], apinfo.mac[2],
		apinfo.mac[3], apinfo.mac[4], apinfo.mac[5]) < 0) {
		LOGE("[WIFI] sprintf failed!\n");
		return -1;
	}
	strncpy(return_data.wifi.wifi_name, apinfo.ssid, 32);
...
int wifi_connect(char * ssid)
{
	...
	if (ioctl(skfd, SIOCSIWMODE, &wrq) < 0) {
		LOGE("ioctl(SIOCSIWMODE), err is %s",
				strerror(errno));
		ret = -1;
		goto exit;
	}
	...
	if (ioctl(skfd, SIOCSIWESSID, &wrq) < 0) {
		LOGE("ioctl(SIOCSIWESSID) failed, err is %s",
				strerror(errno));
		ret = -1;
		goto exit;
	}
...
int wifi_fm_test(void)
{
	...
	if (mtk_get_iwinfo( skfd, "wlan0", &wlan_info) < 0) {
		...
		if (g_output_buf) {
			...
			if (sprintf(g_output_buf, "[ERROR] can't get wlan info\n") < 0) {
				LOGE("[WIFI] sprintf failed!\n");
			}
		}
		return 1;
	...
	if (iw_scan(skfd, "wlan0", 21, &scanlist) < 0) {
		...
		if (g_output_buf) {
			memset(g_output_buf, 0, g_output_buf_len);
			if (sprintf(g_output_buf, "[ERROR] scan failed\n") < 0) {
				LOGE("[WIFI] sprintf failed!\n");
			}
		}
		return 2;
	...
	if (scanlist.result == NULL) {
		...
		if (g_output_buf) {
			memset(g_output_buf, 0, g_output_buf_len);
			if (sprintf(g_output_buf, "[WARN]no network avail.\n") < 0) {
				LOGE("[WIFI] sprintf failed!\n");
			}
		}
		return 3;
	...
	for(item = scanlist.result; item!= NULL ;item=item->next) {
		...
		if (item->has_ap_addr) {
			...
			if (sprintf(ctemp, "mac : %02x-%02x-%02x-%02x-%02x-%02x",
				mac[0], mac[1], mac[2],
				mac[3], mac[4], mac[5]) < 0) {
				LOGE("[WIFI] sprintf failed!\n");
			}
		...
		if (item->b.has_mode) {
			LOGD("[wifi_select_ap] mode :  %s!\n",
				item->b.mode==2?"Infrastructure" : (item->b.mode==1?"Ad-hoc":"unknown"));
		}
	...
	if (ap) {
		...
		usleep(3000000); /* avoid scan again before scan done */
		if (wifi_connect(ap->b.essid) < 0) {
			LOGE("[wifi_select_ap] wifi_connect failed\n");
			if (g_output_buf) {
				memset(g_output_buf,0,g_output_buf_len);
				if (sprintf(g_output_buf, "[ERROR] connect failed\n") < 0) {
					LOGE("[WIFI] sprintf failed!\n");
				}
			}
				ret = 4;
		...
		if (g_output_buf) {
			memset(g_output_buf,0,g_output_buf_len);
			if (sprintf(g_output_buf, "[WARN] no siutable AP\n") < 0) {
				LOGE("[WIFI] sprintf failed!\n");
			}
		}
		ret = 5;
...
int read_preferred_ssid(char *ssid, int len)
{
	...
	if (temp != NULL && strlen(temp) < 33) {
		LOGD(TAG "[read_preferred_ssid] Find perferred ssid %s(%d)\n", temp, len);
...
void update_Text_Info(ap_info *pApInfo, char *output_buf, int buf_len)
{
	...
	if (!pApInfo){
		LOGE("[update_Text_Info]invalid param, %d\n", buf_len);
		return;
	}
...
int FM_WIFI_init(char *output_buf, int buf_len, int *p_result)
{
	...
	*p_result = 0;
	...
	{
		LOGE("[FM_WIFI_init] failed to open socket, \
            skfd = %d, err is %s\n",
            skfd, strerror(errno));
		return -1;
	}
	...
	if (sPflink < 0) {
		LOGE("[FM_WIFI_init] failed socket(PF_NETLINK), \
            sPflink = %d, err is %s\n",
            sPflink, strerror(errno));
		close(skfd);
		skfd = -1;
		return -1;
	}
	...
	bindResult = bind(sPflink, (struct sockaddr *) &local, sizeof(local));
	if (bindResult < 0) {
		LOGE("[FM_WIFI_init] failed bind(netlink) \
            bindResult = %d, err is %s\n",
            bindResult, strerror(errno));
		close(skfd);
		skfd = -1;
		close(sPflink);
		sPflink = -1;
