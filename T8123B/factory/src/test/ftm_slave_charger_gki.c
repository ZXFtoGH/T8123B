...
#define SLAVE_CHARGER_FILE "/sys/class/power_supply/mtk-slave-charger/present"
...
static bool is_slave_chg_exist(void)
{
    ...
    fd = open(SLAVE_CHARGER_FILE, O_RDONLY, 0);
    if (fd == -1) {
        LOGD(TAG "Can't open %s, ret=%d\n", SLAVE_CHARGER_FILE, errno);
        return false;
    }
...
bool is_support_dual_charger()
{
	bool ret = false;
	int chg_cfg = 0;

	chg_cfg = get_int_value("/sys/devices/platform/charger/Charger_Config");
	if (chg_cfg > 0)
		ret = true;
