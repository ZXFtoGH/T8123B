...
#define ERR(f, ...)     ALOGE(TAG "%s: " f, __FUNCTION__, ##__VA_ARGS__)
#define WAN(f, ...)     ALOGW(TAG "%s: " f, __FUNCTION__, ##__VA_ARGS__)
#if BT_FM_DEBUG
#define DBG(f, ...)     ALOGD(TAG "%s: " f, __FUNCTION__, ##__VA_ARGS__)
#define TRC(f)          ALOGW(TAG "%s #%d", __FUNCTION__, __LINE__)
#else
#define DBG(...)        ((void)0)
#define TRC(f)          ((void)0)
#endif
...
{
    UINT8 HCI_INQUIRY[] =
        {0x01, 0x01, 0x04, 0x05, 0x33, 0x8B, 0x9E, 0x05, 0x02};
...
void updateTextInfo(TEST_INFO_T *pTest, char *output_buf, int buf_len)
{
    ...
    if (g_pBtListHear == NULL) {
        if (g_inquiry_complete == FALSE) {
            ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_inquiry_start);
            if (ret < 0) ERR("cBuf sprintf error!\n");
        }
        else {
            /* Can not find any device */
            ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_no_dev);
            if (ret < 0) ERR("cBuf sprintf error!\n");
        ...
        if (g_inquiry_complete == FALSE) {
            if (loop == 0) {
                ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_inquiry_1);
                if (ret < 0) ERR("cBuf sprintf error!\n");
                loop = 1;
            } else {
                ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_inquiry_2);
                if (ret < 0) ERR("cBuf sprintf error!\n");
                loop = 0;
            }
        } else {
            if (loop == 0) {
                ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_scan_1);
                if (ret < 0) ERR("cBuf sprintf error!\n");
                loop = 1;
            } else {
                ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_scan_2);
                if (ret < 0) ERR("cBuf sprintf error!\n");
                loop = 0;
            }
        }
    } else {
        ret = snprintf(cBuf, sizeof(cBuf), "%s", uistr_info_bt_scan_complete);
        if (ret < 0) ERR("cBuf sprintf error!\n");
    }
    ...
    while (pBtInfo) {
        if (strlen((const char*)pBtInfo->name)) {
            ...
            ret = snprintf(cBuf  + strlen(cBuf), sizeof(cBuf) - strlen(cBuf), " %d\n", pBtInfo->rssi);
            if (ret < 0) ERR("cBuf sprintf error!\n");
            if  ((return_data.bt.num < 10) && (return_data.bt.num >= 0))
                strncpy(return_data.bt.bt[return_data.bt.num].bt_name, pBtInfo->name, 31);
        }
        else {
            /* Inquiry result */
            ret = snprintf(cBuf  + strlen(cBuf), sizeof(cBuf) - strlen(cBuf), "%02x%02x%02x%02x%02x%02x %d\n",
            ...
            if  ((return_data.bt.num < 10) && (return_data.bt.num >= 0)) {
                ...
                ret = sprintf(return_data.bt.bt[return_data.bt.num].bt_mac, "%02x%02x%02x%02x%02x%02x\n",
                        pBtInfo->btaddr[0], pBtInfo->btaddr[1], pBtInfo->btaddr[2],
                        pBtInfo->btaddr[3], pBtInfo->btaddr[4], pBtInfo->btaddr[5]);
                if (ret < 0) ERR("cBuf sprintf error!\n");
            }
    ...
    if (g_pBtListHear) {
        if (g_scan_complete == FALSE) {
            if (g_inquiry_complete == TRUE) {
                ret = snprintf(cBuf + strlen(cBuf), sizeof(cBuf) - strlen(cBuf), "%s", uistr_info_bt_dev_list_end);
                if (ret < 0) ERR("cBuf sprintf error!\n");
            }
        } else {
            ret = snprintf(cBuf + strlen(cBuf), sizeof(cBuf) - strlen(cBuf), "%s", uistr_info_bt_scan_list_end);
            if (ret < 0) ERR("cBuf sprintf error!\n");
            if (FTM_MANUAL_ITEM != pTest->test_type)
