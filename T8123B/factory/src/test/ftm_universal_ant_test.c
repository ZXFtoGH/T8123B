...
static int read_preferred_para_index(const char *pstr1, const char *pstr2, int index, int default_value) {
    ...
    if (index > 0) {
        err = sprintf(path,"%s%d.%s", pstr1, index, pstr2);
    } else {
        err = sprintf(path,"%s.%s", pstr1, pstr2);
    }
    if (err < 0) {
        LOGD(TAG "sprintf preferred para index fail");
        return value;
    }
...
static int gsm_ant_test(char *info, int *infoLen, ftm_universal_ant_rat rat, int index) {
    ...
    if (rat == ANT_RAT_LTE || rat == ANT_RAT_GSM) {
        ...
        while(tryCount++ < tryCountMax) {
            ...
            if (read(fd, rsp_buf, BUF_SIZE-1) == -1) {
                LOGD(TAG "Can't read response from MD!\n");
            }
            ...
            if(p_epbse!= NULL) {
                ...
                int umts_band = 0;
                int lte_band_1_32 = 0;
                int lte_band_33_64 = 0;
                int lte_band_65_96 = 0;
                int lte_band_97_128 = 0;
                int lte_band_129_160 = 0;
                int lte_band_161_192 = 0;
                int lte_band_193_224 = 0;
                int lte_band_225_256 = 0;
    ...
    switch (rat) {
        case ANT_RAT_LTE:
        {
            ...
            pVal = NULL;
            if (index > 0) {
               err = sprintf(path, "LTE%d.RF.rssi1.dbm", index);
            } else {
                err = sprintf(path, "LTE.RF.rssi1.dbm");
            }
            if (err < 0) {
                LOGD(TAG "sprintf LTE RF rssi1 dbm fail");
            }
            pVal = ftm_get_prop(path);
            if (pVal != NULL) {
            ...
            memset(cmd_buf, 0, sizeof(cmd_buf));
            err = sprintf(cmd_buf,"AT+ERFTX=7,%d,%d\r\n", mLteRfBand, mLteRfDlEarfcn);
            if (err < 0) {
                LOGD(TAG "sprintf LTE RF fail");
                goto err;
            }
            send_at(fd, cmd_buf);
            if (wait4_ack(fd, NULL, 5000)) goto err;
        ...
        case ANT_RAT_WCDMA:
        {
            ...
            pVal = NULL;
            if (index > 0) {
                err = sprintf(path, "WCDMA%d.RF.rssi1.dbm", index);
            } else {
                err = sprintf(path, "WCDMA.RF.rssi1.dbm");
            }
            if (err < 0) {
                LOGD(TAG "sprintf WCDMA RF rssi1 dbm fail");
            }
            pVal = ftm_get_prop(path);
            if (pVal != NULL) {
                mRfRssi1dbm = (int)atoi(pVal);
            ...
            memset(cmd_buf, 0, sizeof(cmd_buf));
            err = sprintf(cmd_buf,"AT+ERFTX=0,2,%d\r\n",mWcdmaRfUarfcn);
            if (err < 0) {
                LOGD(TAG "sprintf WCDMA RF fail");
            }
            send_at(fd, cmd_buf);
        ...
        case ANT_RAT_TDSCDMA:
        {
            ...
            pVal = NULL;
            if (index > 0) {
                err = sprintf(path, "TDSCDMA%d.RF.rssi1.dbm", index);
            } else {
                err = sprintf(path, "TDSCDMA.RF.rssi1.dbm");
            }
            if (err < 0) {
                LOGD(TAG "sprintf TDSCDMA RF rss1 dbm fail");
            }
            pVal = ftm_get_prop(path);
            if (pVal != NULL) {
                mRfRssi1dbm = (int)atoi(pVal);
            ...
            err = sprintf(cmd_buf,"AT+ERFTX=0,2,%d\r\n",mTdscdmaRfUarfcn);
            if (err < 0) {
                LOGD(TAG "sprintf TDSCDMA RF fail");
            }
            send_at(fd, cmd_buf);
            if (wait4_ack(fd, NULL, 5000)) goto err;
            break;
        ...
        case ANT_RAT_GSM:
        {
            ...
            pVal = NULL;
            if (index > 0) {
                err = sprintf(path, "GSM%d.RF.rssi1.dbm", index);
            } else {
                err = sprintf(path, "GSM.RF.rssi1.dbm");
            }
            if (err < 0) {
                LOGD(TAG "sprintf GSM RF rss1 dbm fail");
            }
            pVal = ftm_get_prop(path);
            if (pVal != NULL) {
                mRfRssi1dbm = (int)atoi(pVal);
            ...
            memset(cmd_buf, 0, sizeof(cmd_buf));
            err = sprintf(cmd_buf,"AT+ERFTX=2,2,%d,%d,%d,%d\r\n", mGsmRfArfcn, mGsmRfGain, mGsmRfBand, mGsmRfPattern);
            if (err < 0) {
                LOGD(TAG "sprintf GSM RF fail");
            }
            send_at(fd, cmd_buf);
            if (wait4_ack(fd, NULL, 5000)) goto err;
            break;
...
static int nr_ant_test(char *info, int *infoLen, ftm_universal_ant_rat rat, int index) {
    ...
    switch (rat) {
        case ANT_RAT_NR:
        {
            ...
            memset(cmd_buf, 0, sizeof(cmd_buf));
            err = sprintf(cmd_buf,"AT+EGMC=1,\"NrRssi\",%d,%d\r\n", mNrRfBand, mNrRfDlEarfcn);
            if (err < 0) {
                LOGD(TAG "sprintf NR RSSI fail");
                goto err;
            }
            send_at(fd, cmd_buf);
            break;
    ...
    while (tryCount++ < tryCountMax) {
        ...
        if (p != NULL) {
            ...
            tok_start(&p);
            //act
            if (0 != at_tok_nextstr(&p, &skipstr)) LOGD(TAG "+EGMC nextstr error!\n");
            //parse out "-80" and assigned to rssi0_dbm
            at_tok_nextint(&p, &rssi0_dbm);
            //eamplex: p now is pointed to "-96"
            at_tok_nextint(&p, &rssi1_dbm);
...
if (isC2kSupport() == 1) {
    ...
    memset(cmd_buf, 0, sizeof(cmd_buf));
    err = sprintf(cmd_buf,"AT+ECBAND=1,%d,%d,%d\r\n", mC2kRfSubsystem, mC2kRfBand, mC2kRfChannel);
    if (err < 0) {
        LOGD(TAG "sprintf NR RSSI fail");
        goto err;
    }
    ...
    while (tryCount++ < tryCountMax) {
        ...
        memset(rsp_buf, 0, sizeof(rsp_buf));
        if (read(fd3, rsp_buf, BUF_SIZE-1) == -1) {
            LOGD(TAG "Can't read response from MD!\n", );
        }
...
static int get_ccci_path(int modem_index,char * path)
{
    ...
    if(is_support_modem(idx[modem_index]))
    {
       ...
       if(modem_index == 3){
            ...
            err = snprintf(path, 32, "%s", ccci_get_node_name(USR_C2K_AT, (CCCI_MD)md_sys[modem_index]));
            LOGD(TAG "got md3 node %s", path);
        } else {
            err = snprintf(path, 32, "%s", ccci_get_node_name(USR_FACTORY_DATA, (CCCI_MD)md_sys[modem_index]));
        }
#else
       err = snprintf(path, 32, "%s", ccci_get_node_name(USR_FACTORY_DATA, (CCCI_MD)md_sys[modem_index]));
#endif
       if (err < 0) {
           LOGD(TAG "snprintf get_ccci_path fail");
           return 0 ;
       }
...
int wait4_ack_with_retry_error (const int fd, char *pACK, char *pRetryError, int timeout)
{
    ...
    for(rdCount = 0; rdCount < LOOP_MAX; ++rdCount)
    {
        ...
        if (read(fd, buf, BUF_SIZE_EXT-1) == -1) {
            LOGD(TAG "Can't read response from MD!\n", );
        }
