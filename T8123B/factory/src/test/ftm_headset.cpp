...
static item_t headset_items_ex_dual_mic[] =
{
    {ITEM_RINGTONE, uistr_info_audio_ringtone, COLOR_BLACK, 0},
...
static bool read_preferred_recorddump(void)
{
    ...
    if (snprintf(uName, sizeof(uName), "%s", "Audio.Record.Dump") < 0)
    {
        return false;
    }
...
static void read_preferred_magnitude(int *pUpper, int *pLower)
{
    ...
    *pLower = 0;
    ...
    ret_str = snprintf(uMagLower, sizeof(uMagLower), "%s", "Lower.Magnitude.Headset");
    if (ret_str >= 0 && ret_str < sizeof(uMagLower))
    {
        pMagLower = ftm_get_prop(uMagLower);
    }

    ret_str = snprintf(uMagUpper, sizeof(uMagUpper), "%s", "Upper.Magnitude.Headset");
    if (ret_str >= 0 && ret_str < sizeof(uMagUpper))
    {
...
static void *Audio_Record_thread(void *mPtr)
{
    ...
    else
    {
        if (headset_phonetest_state == 0)   // L ch
        {
			...
			/*** add by zkc start ***/
			if(return_data.headset.amplL > 1000 && return_data.headset.amplL < 10000)
				return_data.headset.amplL += 10000;
			/*** add by zkc end ***/
        }
        else if (headset_phonetest_state == 1)  // R ch
        {
			...
			/*** add by zkc start ***/
			if(return_data.headset.amplR > 1000 && return_data.headset.amplR < 10000)
				return_data.headset.amplR += 10000;
			/*** add by zkc end ***/
        }
    }
...
static void *Headset_THD_Record_thread(void *mPtr)
{
    ...
    int lenL = 0, lenR = 0;
    float thdData[2][FFT_DAT_MAXNUM] = {0};
    ...
    else
    {
        ...
        if (get_is_ata() == 0) {
            if (numMicString && atoi(numMicString) >= 2) {
                iv->set_items(iv, headset_items_ex_dual_mic, 0);
            } else {
