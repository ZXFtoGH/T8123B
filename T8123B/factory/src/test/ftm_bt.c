...
static void *bt_update_thread(void *priv)
{
    ...
    if(snprintf(bf->info, sizeof(bf->info), "%s", uistr_info_bt_init) < 0){
		return NULL;
    }
    iv->redraw(iv);

#ifdef MTK_MT7902_ATA_TEST
	if( (access("/sys/class/bluetooth", 0 )) != -1 ) {
		LOGE(TAG "/sys/class/BT_chrdev is exit\n");
		bf_test.test_result == TEST_RESULT_PASS;
	}else{
		LOGE(TAG "fail: /sys/class/BT_chrdev is not exit!!\n");
    ...
    if (FM_BT_init() == false) {
        LOGD(TAG "%s: Exit\n", __FUNCTION__);
        if(snprintf(bf->info, sizeof(bf->info), "%s", uistr_info_bt_init_fail) < 0)
            LOGD("snprintf error!\n");
        iv->redraw(iv);
        pthread_exit(NULL);
        goto exit;
    ...
    if(snprintf(bf->info, sizeof(bf->info), "%s", uistr_info_bt_init_ok) < 0)
        LOGD("snprintf error!\n");
    iv->redraw(iv);
...
#else
    if( (access("/sys/class/BT_chrdev", 0 )) != -1 )
#endif
