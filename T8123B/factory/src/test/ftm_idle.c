...
{
    char value[100] = {0};
    property_get("ro.vendor.mtk_protocol1_rat_config", value, "");
...
void sendEsuo(const int fd, int value, int wait_echo) {
    ...
    int retVal = sprintf(cmd_buf, "AT+ESUO=%d\r\n", value);
    if (retVal < 0) {
        LOGD(TAG "[sendEsuo] Failed to allocate buffer by sprintf!\n");
        return;
    }
...
void handleLPlusGMd1() {
    ...
    if (strcmp(value, "G") == 0) {
        ...
        int retVal;
        retVal = snprintf(ccci_md1_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS1));
        if (retVal < 0) {
            LOGE(TAG "[handleLPlusGMd1]Failed to get ccci_md1_dev!\n");
            return;
        }
        if (-1 == fd_atcmd) {
...
bool ifSupportEpofReviseNewVersion ()
{
    ...
    retVal = snprintf(ccci_md1_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[ifSupportEpofReviseNewVersion]Failed to get ccci_md1_dev!\n");
        return false;
    }
...
int set_md_power_and_flight_mode (bool md_power)
{
    ...
    retVal = snprintf(ccci_md1_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[set_md_power_and_flight_mode]Failed to get ccci_md1_dev!\n");
        return -1;
    }
    LOGD(TAG "Open CCCI MD1 dev node: %s\n", ccci_md1_dev);
    fd_atcmd = openDeviceWithDeviceName(ccci_md1_dev);
    ...
    retVal = snprintf(ccci_md1_power_ioctl_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE_IOCTL, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[set_md_power_and_flight_mode]Failed to get ccci_md1_power_ioctl_dev!\n");
        return -1;
    }
...
int get_ccci_path_md3(int modem_index,char * path)
{
    ...
    if(is_support_modem(idx[modem_index]))
    {
       if (modem_index == 3){
           ...
           int retVal;
           retVal = snprintf(path, 32, "%s", ccci_get_node_name(USR_C2K_AT, (CCCI_MD)md_sys[modem_index]));
           if (retVal < 0) {
               LOGE(TAG "[get_ccci_path_md3]Failed to get path!\n");
               return 1;
           }
           ...
           int retVal;
           retVal = snprintf(path, 32, "%s", ccci_get_node_name(USR_FACTORY_DATA, (CCCI_MD)md_sys[modem_index]));
           if (retVal < 0) {
               LOGE(TAG "[get_ccci_path_md3]Failed to get path!\n");
               return 1;
           }
...
void EnterFlightModeForSrlte()
{
    ...
    ssize_t ioctl_ret;
    int retVal;
    retVal = snprintf(ret, 32, "%s", ccci_get_node_name(USR_RILD_IOCTL, MD_SYS3));
    if (retVal < 0) {
        LOGE(TAG "[EnterFlightModeForSrlte]Failed to get ret!\n");
        return;
    }
...
void LeaveFlightModeForSrlte()
{
    ...
    retVal = snprintf(ret, 32, "%s", ccci_get_node_name(USR_RILD_IOCTL, MD_SYS3));
    if (retVal < 0) {
        LOGE(TAG "[LeaveFlightModeForSrlte]Failed to get ret!\n");
        return;
    }
...
static int power_off_md()
{
    ...
    retVal = snprintf(ccci_md1_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[power_off_md]Failed to get ccci_md1_dev!\n");
        return -1;
    }
    ...
    if (is_md1_enable() > 0) {
        ...
        retVal = snprintf(ccci_md1_power_ioctl_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE_IOCTL, MD_SYS1));
        if (retVal < 0) {
            LOGE(TAG "[power_off_md]Failed to get ccci_md1_power_ioctl_dev!\n");
            return -1;
        }
        LOGD(TAG "Open CCCI MD1 Power IOCTL dev node: %s\n", ccci_md1_power_ioctl_dev);
        fd_ioctl = openDeviceWithDeviceName(ccci_md1_power_ioctl_dev);
    ...
    */
    ...
    if (is_md2_enable() > 0) {
        retVal = snprintf(ccci_md2_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS2));
        if (retVal < 0) {
            LOGE(TAG "[power_off_md]Failed to get ccci_md2_dev!\n");
            return -1;
        }
        ...
        retVal = snprintf(ccci_md2_power_ioctl_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE_IOCTL, MD_SYS2));
        if (retVal < 0) {
            LOGE(TAG "[power_off_md]Failed to get ccci_md2_power_ioctl_dev!\n");
...
static int power_on_md()
{
    ...
    retVal = snprintf(ccci_md1_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[power_on_md]Failed to get ccci_md1_dev!\n");
        return false;
    }
    LOGD(TAG "Open CCCI MD1 dev node: %s\n", ccci_md1_dev);
    fd_atcmd = openDeviceWithDeviceName(ccci_md1_dev);
    ...
    retVal = snprintf(ccci_md1_power_ioctl_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE_IOCTL, MD_SYS1));
    if (retVal < 0) {
        LOGE(TAG "[power_on_md]Failed to get ccci_md1_power_ioctl_dev!\n");
        return false;
    }
    LOGD(TAG "Open CCCI MD1 Power IOCTL dev node: %s\n", ccci_md1_power_ioctl_dev);
    fd_ioctl = openDeviceWithDeviceName(ccci_md1_power_ioctl_dev);
...
static int Enter_Exit_Idle(IDLE_STATE idle_state)
{
    ...
    if(ENTER_IDLE == idle_state)
    {
		...
		int retVal;
		retVal = snprintf(filename, sizeof(filename), "/dev/graphics/fb0");
		if (retVal < 0) {
		    LOGE(TAG "[Enter_Exit_Idle]Failed to get filename!\n");
		    return -1;
		}
		fd_fb = open(filename, O_RDWR);
		if (fd_fb < 0) {
			LOGD("Failed to open fb0 device: %s", strerror(errno));
			LOGD("display blank by drm flow");
			ui_blank(true);
		} else {
    ...
    else
    {
		...
		int retVal = snprintf(filename, sizeof(filename), "/dev/graphics/fb0");
		if (retVal < 0) {
		    LOGE(TAG "[Enter_Exit_Idle]Failed to get filename!\n");
		    return -1;
		}
		...
		if (fd_fb < 0) {
			LOGD("Failed to open fb0 device: %s", strerror(errno));
			LOGD("display unblank by drm flow");
			ui_blank(false);
		} else {
...
{
	FTM_COM_TYPE com_type = getComType();
