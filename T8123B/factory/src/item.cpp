...
/* goodix end */
	...
	item(ITEM_CLRESTORAGE,"AT+FLASH"),
	#endif
    #else  /* NAND */
...
item_t ftm_test_items[ITEM_MAX_IDS] =
{
...
*/
...
#ifdef FEATURE_FTM_MAIN3_LENS
    item(ITEM_MAIN3_LENS,  uistr_main3_lens),
#endif
    ...
    item(ITEM_HALL, uistr_hall),
#endif    
    item(ITEM_MAX_IDS, NULL),
};

static bool insert_ftm_test_item(item_t &it)
{
	int idx;
	for(idx=0; idx<ITEM_MAX_IDS-1; idx++)
	{
		if(ftm_test_items[idx].id == ITEM_MAX_IDS)
			break;
	}

	if(idx == ITEM_MAX_IDS-1)
	{
		LOGD(TAG "no memory to insert cmd hdlr - %s\n", it.name);
		return false;
	}

	ftm_test_items[idx].id = it.id;
	ftm_test_items[idx].name = it.name;
	ftm_test_items[idx].mode = 0;
	ftm_test_items[idx].background= 0;

	ftm_test_items[idx+1].id = ITEM_MAX_IDS;
	ftm_test_items[idx+1].name = NULL;
	ftm_test_items[idx+1].mode = 0;
	ftm_test_items[idx+1].background= 0;

	return true;

}

void init_ftm_test_item()
{
#ifdef FEATURE_FTM_SLAVE_CHARGER
	if(is_support_dual_charger())
	{
		item_t it = item(ITEM_SLAVE_CHARGER, uistr_info_title_slave_charger);
		insert_ftm_test_item(it);
	}
#endif

	if (is_support_efuse_writer())
	{
		item_t it = item(ITEM_EFUSE, uistr_info_efuse_test);
		insert_ftm_test_item(it);
	}
...
/*
	if(is_support_xxxx())
	{
		item_t it = item(ITEM_xxxx, xxxx);
		insert_ftm_test_item(it);
	}
*/
	for(int i=0; i<ITEM_MAX_IDS; i++)
	{
		if(ftm_test_items[i].id == ITEM_MAX_IDS)
			break;

		LOGD(TAG "ftm_test_item - %s\n", ftm_test_items[i].name);
	}
}
