...
//define in CFG_Wifi_File.h, only use aucMacAddress
typedef struct _LD20_MT6620_CFG_PARAM_STRUCT {
    unsigned short      u2Part1OwnVersion;
    unsigned short      u2Part1PeerVersion;
    unsigned char       aucMacAddress[6];
}WIFI_LD20_CFG_PARAM_STRUCT;
#define WIFI_DATA_LEN 10

WIFI_LD20_CFG_PARAM_STRUCT g_wifi_nvram;
...
static int count = 0;
	  ...
	  if(!strncmp(buf_cmd, "AT+SWITCH",9))
	  {
		  ...
		  #ifnde fEVDO_FTM_DT_VIA_SUPPORT
		  if(isC2kSupport() == 1){
			  g_mdFlag=3;
			}  
		  #else
...
//	    LOGD(TAG "len = %d\n",len);    
	    if (len>0)
	    {
			...
			if (2 == at_pre_ret)
            {
            	if (0 == sync_start)
            	{
	                wr_len = write_chars (g_hUsbComPort, pass, strlen(pass));
				    if (wr_len != (int)strlen(pass))
				    {
            ...
            {
             	wr_len = write_chars (g_hUsbComPort, ok_str, strlen(ok_str));
                continue;
            }
            ...
            {
            	LOGD(TAG "The data write to AP from PC is %s\n", result);
                wr_len = write_chars (g_hUsbComPort, result, strlen(result));
			    if (wr_len != (int)strlen(result))
		            LOGE(TAG "AP_CCCI_RX: wr_len != rd_len\n");
            }
            else
            {
            	
             	LOGD(TAG "The data write to MD from PC is %s\n", buf_cmd);
				if(g_mdFlag-1 >= 0){
...
void ate_signal(void)
{
    ...
    if(sprintf(buf, "%s", "ATE Signaling Test\nEmergency call is not started\n") < 0){
		LOGD(TAG "Sprintf result information failed\n");
    }		 
    ate.redraw(&ate);
...
void open_ate_modem()
{
    ...
    for (i = 0; i<modem_number; i++)
    {
        ...
        if(1 == modem[i].Open_ATEChannel(ccci_path[i],i+1,g_ATE_Callback))
        {
    ...
    if(1 == SDIO_Ready_flag)
    {
          ...
          strncpy(asciDevice,ccci_path[modem_number],strlen(ccci_path[modem_number]));
        ...
        #endif
        if(1 == modem[modem_number+1].Open_ATEChannel(asciDevice,modem_number+1,g_ATE_Callback))
        {
...
void rmmi_eamdimg_hdlr(rmmi_string_struct *cmd, char *md_idx)
{
    ...
    if (cmd == NULL) 
    { 
        LOGD(TAG "The parameter cmd is NULL\n");
        return;
    }

	if(g_mdFlag-1 < 0){
        LOGD(TAG "g_mdFlag is invalid\n");
		return;
	}

    if(md_idx == NULL)// reserved parameter, for resolving warning
    { 
        LOGD(TAG "The parameter md_idx is NULL\n");
    }

    if((cmd->cmd_owner != CMD_OWENR_AP) || (cmd->cmd_class != RMMI_EXTENDED_CMD))
    {
        if(sprintf(cmd->result, "%s", return_err) < 0){
			LOGD(TAG "Printf result failed\n");
        }
        return;
    }
    for(i = 0; i < MAX_MODEM_INDEX; i++)
                ...
                if (0 == ioctl(modem[g_mdFlag-1].GetHandle(), CCCI_IOC_GET_MD_IMG_EXIST, &mdimg_type))
                {
                    for(i = 0; i < 16; i++)
                    {
                        if(mdimg_type[i] != 0)
                        {
                            ...
                            if(sprintf(temp_result, "\r\n%s%d,", cmd->result, mdimg_type[i]) < 0){
								LOGD(TAG "Printf result failed\n");
							}
                            if(sprintf(cmd->result, "%s", temp_result) < 0){
								LOGD(TAG "Printf result failed\n");
							}
                        }
...
void rmmi_eabt_hdlr (rmmi_string_struct* cmd, char *addr)
{
    ...
    if(addr == NULL)// reserved parameter, for resolving warning
    { 
        LOGD(TAG "The parameter addr is NULL\n");
    }
	    ...
	    case   RMMI_SET_OR_EXECUTE_MODE: //AT+EABT=1\r
            ...
            else
            {
   			...
   			if(FileOp_BackupToBinRegion_All())
   			{
                ...
                if(sprintf(cmd->result, "%s", return_ok) < 0){
					LOGD(TAG "Printf result failed\n");
				}
   			}
   			else
...
//            memcpy(addr, g_bt_nvram.addr, sizeof(g_bt_nvram.addr));
    	...
    	case RMMI_TEST_MODE:                     //AT+EABT=?\r
            ...
            if(sprintf(cmd->result, "%s%s", "\r\n+EABT:(0,1)(1)", return_ok) < 0){
...
*/    			memset(&g_wifi_nvram,0,rec_num*rec_size);
    			...
    			LOGD("read wifi addr:%02x%02x%02x%02x%02x%02x\n", 
                    g_wifi_nvram.aucMacAddress[0], g_wifi_nvram.aucMacAddress[1], g_wifi_nvram.aucMacAddress[2], g_wifi_nvram.aucMacAddress[3], g_wifi_nvram.aucMacAddress[4], 
    			g_wifi_nvram.aucMacAddress[5]);
//                memcpy(addr, g_wifi_nvram.aucMacAddress, sizeof(g_wifi_nvram.aucMacAddress)) < 0){
...
{
	int i;
	for (i=0; i<(int)(sizeof(g_cmd_handler)/sizeof(CMD_HDLR)); i++)
	{
...
CMD_OWENR_ENUM rmmi_cmd_processor(unsigned char *cmd_str, char *result)
{
	...
	if (source_string_ptr.cmd_owner == CMD_OWENR_AP && source_string_ptr.cmd_index >= 0)
...
int write_wifi(char *value, char *result)
{
	...
	void *hcustlib;
	unsigned int *iFileWIFILID;

	hcustlib = dlopen("libcustom_nvram.so", RTLD_LAZY);
	if (!hcustlib) {
		LOGD(TAG "dlopen libcustom_nvram.so fail!\n");
		return -1;
	}

	iFileWIFILID = (unsigned int*)(dlsym(hcustlib, "iFileWIFILID"));
	if (!iFileWIFILID) {
		LOGD(TAG "get AP_CFG_RDEB_FILE_WIFI_LID value fail!\n");
		ret = -1;
		goto write_wifi_done;
	}

	LOGD(TAG "AP_CFG_RDEB_FILE_WIFI_LID = %d", *iFileWIFILID);
	
    nvram_fd = NVM_GetFileDesc(*iFileWIFILID, &rec_size, &rec_num, ISREAD);
    LOGD(TAG "rec_size=%d, rec_num=%d\n", rec_size, rec_num);
    if(1 != rec_num)
    {
         LOGD(TAG "error:unexpected record num %d\n",rec_num);
		 if(sprintf(result, "%s", return_err) < 0){
			 LOGD(TAG "error:printf result infroamtion failed!\n");
		 }
         ret = -1;
		 goto write_wifi_done;
    }
/*  if(sizeof(WIFI_CFG_PARAM_STRUCT) != rec_size)
    ...
    printf("read wifi addr:%02x%02x%02x%02x%02x%02x\n", 
              ...
              g_wifi_nvram.aucMacAddress[5]);
*/
	memset(&g_wifi_nvram,0, sizeof(WIFI_LD20_CFG_PARAM_STRUCT));
    len = read(nvram_fd.iFileDesc, &g_wifi_nvram, WIFI_DATA_LEN);
    if(-1 == len||WIFI_DATA_LEN != len)
    {
        LOGD(TAG "error:read wifi mac addr fail!/n");
        if(sprintf(result, "%s", return_err) < 0){
			LOGD(TAG "error:printf result infroamtion failed!\n");
		}
        ret = -1;
	...
	LOGD(TAG "read wifi addr:%02x%02x%02x%02x%02x%02x\n", 
              g_wifi_nvram.aucMacAddress[0], g_wifi_nvram.aucMacAddress[1], g_wifi_nvram.aucMacAddress[2], g_wifi_nvram.aucMacAddress[3], g_wifi_nvram.aucMacAddress[4], 
              g_wifi_nvram.aucMacAddress[5]);

	NVM_CloseFileDesc(nvram_fd);

    nvram_fd = NVM_GetFileDesc(*iFileWIFILID, &rec_size, &rec_num, ISWRITE);
    LOGD(TAG "rec_size=%d,rec_num=%d\n",rec_size,rec_num);
    if(1 != rec_num)
    {
        LOGD(TAG "error:unexpected record num %d\n",rec_num);
...
*/  memset(g_wifi_nvram.aucMacAddress,0, wifi_length);
    length = strlen(value);
    if(length != 12)
    {
        LOGD(TAG "error:bt address length is not right!\n");
        if(sprintf(result, "%s", return_err) < 0){
			LOGD(TAG "error:printf result infroamtion failed!\n");
		}
        ret = -1;
    ...
    len = convStrtoHex(value,output,wifi_length,&length);
    if(-1 == len)
    {
        LOGD(TAG "error:convert wifi address to hex fail\n");
        if(sprintf(result, "%s", return_err) < 0){
			LOGD(TAG "error:printf result infroamtion failed!\n");
		}
    ...
    {
        LOGD(TAG "WIFI Address:%s\n", output);
    }
    for(i=0;i<wifi_length;i++)
    {   
        g_wifi_nvram.aucMacAddress[i] = output[i];
    }
    LOGD(TAG "write wifi addr:%02x%02x%02x%02x%02x%02x, value:%02x%02x%02x%02x%02x%02x\n", 
            g_wifi_nvram.aucMacAddress[0], g_wifi_nvram.aucMacAddress[1], 
            g_wifi_nvram.aucMacAddress[2], g_wifi_nvram.aucMacAddress[3], 
            g_wifi_nvram.aucMacAddress[4], g_wifi_nvram.aucMacAddress[5],
            output[0], output[1], output[2], output[3], output[4], output[5]
    );
    len = write(nvram_fd.iFileDesc, &g_wifi_nvram ,WIFI_DATA_LEN);
    if(-1 == len||WIFI_DATA_LEN != len)
    {
        LOGD(TAG "error:write wifi addr fail!\n");
        if(sprintf(result, "%s", return_err) < 0){
			LOGD(TAG "error:printf result infroamtion failed!\n");
        }
        ret = -1;
    ...
    NVM_CloseFileDesc(nvram_fd);
    LOGD(TAG "write wifi addr success!\n");
    if(FileOp_BackupToBinRegion_All())
    {
        LOGD(TAG "backup nvram data to nvram binregion success!\n");
        if(sprintf(result, "%s", return_ok) <0){
            LOGD(TAG "error:printf result infroamtion failed!\n");
        }
    } 
    else
    {
        LOGD(TAG "error:backup nvram data to nvram binregion fail!\n");
        if(sprintf(result, "%s", return_err) <0 ){
            LOGD(TAG "error:printf result infroamtion failed!\n");
        }
        ret = -1;
...
*/  memset(&g_wifi_nvram,0, sizeof(WIFI_LD20_CFG_PARAM_STRUCT));
    ...
    LOGD(TAG "read wifi addr:%02x%02x%02x%02x%02x%02x\n", 
            g_wifi_nvram.aucMacAddress[0], g_wifi_nvram.aucMacAddress[1], g_wifi_nvram.aucMacAddress[2], g_wifi_nvram.aucMacAddress[3], g_wifi_nvram.aucMacAddress[4], 
            g_wifi_nvram.aucMacAddress[5]);
    if(sprintf(result, "%02x%02x%02x%02x%02x%02x", g_wifi_nvram.aucMacAddress[0], 
              g_wifi_nvram.aucMacAddress[1], g_wifi_nvram.aucMacAddress[2], g_wifi_nvram.aucMacAddress[3], 
              g_wifi_nvram.aucMacAddress[4], g_wifi_nvram.aucMacAddress[5]) < 0){
        LOGD(TAG "error:printf result infroamtion failed!\n");
    }
    NVM_CloseFileDesc(nvram_fd);
...
void g_ATE_Callback(const char *pdata, int len)
{
	...
	strncpy(temp, pdata,(len>(sizeof(temp)-1))?(sizeof(temp)-1):len);
	temp[(len>(sizeof(temp)-1))?(sizeof(temp)-1):len] = '\0';

    if(strlen(temp)!=0)
	{
		
		wr_len = write_chars (g_hUsbComPort, temp, strlen(temp));
		if (wr_len != (int)strlen(temp))
		{
			LOGE(TAG "AP_CCCI_RX: wr_len != rd_len\n");
		}
	}
