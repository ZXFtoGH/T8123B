...
#define TEST_REPORT_PATH_LEN  64
#define TEST_REPORT_PATH      "/data/vendor_de/factory/"
#define TEST_REPORT_PATH_EXT  "/data/vendor_de/meta/mddb/"
#define TEST_REPORT_FILE_NAME "testreport.log"
char test_report[TEST_REPORT_PATH_LEN] = {0};
...
static item_t ftm_menu_items[] = {
    ...
    #endif
    #ifdef FEATURE_FTM_CLEARSTORAGE
        item(ITEM_CLRESTORAGE,  uistr_clear_storage),
    #endif
    item(ITEM_VERSION,   uistr_version),
    #ifdef FEATURE_FTM_NTC
    item(ITEM_NTC,   uistr_ntc),
    #endif
    #ifdef FEATURE_FTM_KEYBOX
    item(ITEM_KEYBOX,   uistr_keybox),
    #endif
    #ifdef FEATURE_FTM_DCDC
    item(ITEM_DCDC,   uistr_dcdc),
    #endif
	item(ITEM_GMS_INFO,   uistr_gms),
...
void get_test_report_path()
{
    if(file_exist(TEST_REPORT_PATH))
    {
        snprintf(test_report, TEST_REPORT_PATH_LEN, "%s%s", TEST_REPORT_PATH, TEST_REPORT_FILE_NAME);
    }
    else if(file_exist(TEST_REPORT_PATH_EXT))
    {
        snprintf(test_report, TEST_REPORT_PATH_LEN, "%s%s", TEST_REPORT_PATH_EXT, TEST_REPORT_FILE_NAME);
    }

    LOGD(TAG "Test report path(%s)\n", test_report);
}
...
int restore_test_report()
{
    ...
    if(file_exist(test_report))
    {
        ...
        while(!feof(fp))
        {
            if(fgets(item_result, 32, fp) == NULL){
				LOGD(TAG "Read file failed");
 				break;
            }
...
int create_report(item_t *item, item_t *rpt_items, int maxitems, char *buf, int size)
{
    ...
    while (i < maxitems && item->name)
    {
        ...
        if (mod && mod->visible && len < size && mod->test_result <3)
        {
            ptr = buf + len;
			if(mod->test_result >= FTM_TEST_MAX || mod->test_result < 0){
				len += sprintf(ptr, "[%c] %s ",	result[FTM_TEST_UNKNOWN], item->name);
			}else{

            	len += sprintf(ptr, "[%c] %s ", result[mod->test_result], item->name);
			}
...
static void init_factory_base() noexcept{
    init_factory_string();
    init_cmd_hdlr();
    init_ftm_funcs();
    init_ftm_test_item();
            ...
            int chosen_item = fiv.run(&fiv,NULL);
		        switch (chosen_item)
		        {
                  ...
                  case ITEM_VERSION:
                      ...
                      break;
	              case ITEM_NTC:
			          display_ntc(0, NULL, false);
			          break;
		          case ITEM_KEYBOX:
			          display_keybox_state(0, NULL, false);
			          break;
#ifdef FEATURE_FTM_DCDC
		          case ITEM_DCDC:
			          display_dcdc_state(0, NULL, false);
			          break;
#endif
			      case ITEM_GMS_INFO:
			          display_gms(0, NULL, false);
			          break;
                  case ITEM_REBOOT:
                      exit = 1;
                      fiv.exit(&fiv);
...
// define in CFG_PRODUCT_INFO_File.h
...
int get_barcode_from_nvram(char *barcode_result)
{
    ...
    void *hcustlib;
    unsigned int *iFilePRODUCT_INFOLID;
    ...
    iFilePRODUCT_INFOLID = (unsigned int*)(dlsym(hcustlib, "iFilePRODUCT_INFOLID"));
    if (!iFilePRODUCT_INFOLID) {
        LOGD(TAG "get AP_CFG_REEB_PRODUCT_INFO_LID value fail!\n");
        dlclose(hcustlib);
        return 0;
    }

    LOGD(TAG "iFilePRODUCT_INFOLID = %d", *iFilePRODUCT_INFOLID);
    ...
    fid = NVM_GetFileDesc(*iFilePRODUCT_INFOLID, &rec_size, &rec_num, isread);
