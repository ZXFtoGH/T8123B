...
static void gr_rotate(GRRotation rot) {
  rotation = rot;
}
...
/*
	...
	Font* Font_Big = nullptr;
    	...
    	#include "font_36x64.h"
    	Font* Font_Small  = &font_36_64;
    	Font* Font_Big = &font_36_64;
    	#elif defined(FEATURE_FTM_FONT_32x60)
    	#include "font_32x60.h"
    	Font* Font_Small  = &font_32_60;
    	Font* Font_Big = &font_32_60;
    	#elif defined(FEATURE_FTM_FONT_24x44)
    	#include "font_24x44.h"
    	Font* Font_Small  = &font_24_44;
    	Font* Font_Big = &font_24_44;
    	#elif defined(FEATURE_FTM_FONT_16x30)
    	#include "font_16x30.h"
    	Font* Font_Small  = &font_16_30;
    	Font* Font_Big = &font_16_30;
    	#elif defined(FEATURE_FTM_FONT_16x28)
    	#include "font_16x28.h"
    	Font* Font_Small  = &font_16_28;
    	Font* Font_Big = &font_16_28;
    	#elif defined(FEATURE_FTM_FONT_12x22)
    	#include "font_12x22.h"
    	Font* Font_Small  = &font_12_22;
    	Font* Font_Big = &font_12_22;
    	#elif defined(FEATURE_FTM_FONT_10x18)
    	#include "font_10x18.h"
    	Font* Font_Small  = &font_10_18;
    	Font* Font_Big = &font_10_18;
    	#elif defined(FEATURE_FTM_FONT_8x14)
    	#include "font_8x14.h"
    	Font* Font_Small  = &font_8_14;
    	Font* Font_Big = &font_8_14;
    	#elif defined(FEATURE_FTM_FONT_6x10)
    	#include "font_6x10.h"
    	Font* Font_Small = &font_6_10;
    	Font* Font_Big = = &font_6_10;
    	#else
    	#include "font_12x22.h"
    	Font* Font_Small  = &font_12_22;
    	Font* Font_Big = &font_12_22;
...
static GGLPixelFormat getPIXEL_FORMAT(){
	LOGE("getPIXEL_FORMAT");


#if defined(FACTORY_MODE_SUPPORT_RGBA_8888)
		if(redoffset_32bit){
		    if(gr_draw == NULL){ //fbdev
...
#else
		return GGL_PIXEL_FORMAT_RGB_565;
#endif
...
static int gr_font_texture_split(Font *font, int* block, int* residue)
{
    int pivotal = 0;     
    if((block == NULL) || (residue == NULL))
    {
        return -1;
    }
...
static int gr_font_which_context(int cwidth, const char* s, unsigned int* str_offset, unsigned int* off)
{
      int pivotal;
      int which = 0;
      int coord;      
      pivotal = (8192+cwidth-1)/cwidth * cwidth;
      
      if((*s & 0x80) != 0)// GB2312
      {
            *str_offset = 2;
            coord = gr_font_get_coord(*s, *(s+1));
            which = coord / (pivotal/cwidth);  //coord is 0 based.
            *off = coord % (pivotal/cwidth);   //coord is 0 based.
      }
      else
      {
            *str_offset = 1;
...
//================end================
static void gr_init_font_chn(Font *font, GRFont **gr_font)
{    
    unsigned char *bits = NULL, *org_bits = NULL;
    unsigned char *in = NULL, data=0; 
    int block = 0, residue = 0, pivotal = 0, surface_num = 0;
    int i = 0;
    unsigned int j = 0;
    void* bits_split = NULL;
    void* bits_split_residue = NULL;
    ...
    *gr_font = (GRFont *)calloc(sizeof(GRFont), 1);
    ...
    org_bits = bits =(unsigned char *)malloc(font->width * font->height);    
    if((org_bits == NULL) || (bits == NULL))
    {
        return;
    }
    ...
    pivotal = gr_font_texture_split(font, &block, &residue);
    ...
    (*gr_font)->texture.chn = (GGLSurface *)malloc(surface_num*sizeof(GGLSurface));
      
	if((*gr_font)->texture.chn == NULL){
		free(org_bits);   
		return;
	}
    ...
    for(i=0; i<block; i++)
    {
        bits_split = (void*)malloc(pivotal * font->height);
		if(bits_split == NULL){
			   free(org_bits);   
			return;
			}
        (*gr_font)->texture.chn[i].version = sizeof(GGLSurface);
        (*gr_font)->texture.chn[i].width = pivotal;
        (*gr_font)->texture.chn[i].height = font->height;
        (*gr_font)->texture.chn[i].stride = pivotal;
        (*gr_font)->texture.chn[i].data = (unsigned char *) bits_split;
        (*gr_font)->texture.chn[i].format = GGL_PIXEL_FORMAT_A_8;
        
        //fprintf(stderr, "bits_split%d\n", bits_split);
        for(j=0; j<font->height; j++)
        {
          memcpy((unsigned char*)bits_split+ j*pivotal, (unsigned char*)(org_bits+i*pivotal+j*font->width), pivotal);
        }     
    }
   // fprintf(stderr, "block over\n");
    if(residue != 0)
    {
        bits_split_residue = (void*)malloc(residue * font->height);
		if(bits_split_residue == NULL){
			free(org_bits);   
			return;
		}
        (*gr_font)->texture.chn[i].version = sizeof(GGLSurface);
        (*gr_font)->texture.chn[i].width = residue;
        (*gr_font)->texture.chn[i].height = font->height;
        (*gr_font)->texture.chn[i].stride = residue;
        (*gr_font)->texture.chn[i].data = (unsigned char *) bits_split_residue;
        (*gr_font)->texture.chn[i].format = GGL_PIXEL_FORMAT_A_8;
        
        for(j=0; j<font->height; j++)
        {
          memcpy((unsigned char*)bits_split_residue+ j*residue, (unsigned char*)(org_bits+block*pivotal+j*font->width), residue);
        }
    }
...
int gr_text_chn(GRFont *gr_font, int x, int y, const char *s)
{
    GGLContext *gl = gr_context;
    GRFont *font = gr_font;
    unsigned int off, str_offset;
   ...
   while(*s != 0)
   {
          ...
          str_offset=0;
          off =0;
          which = gr_font_which_context(gr_font->cwidth, s, &str_offset, &off);
          gl->bindTexture(gl, &font->texture.chn[which]);
...
int gr_measure_chn(GRFont *gr_font,const char *s)
{
      int count = 0;
      while(*s != 0)
      {
...
static void gr_init_font_eng(Font *font, GRFont **gr_font)
{
    GGLSurface *ftex;
...
int gr_text_eng(GRFont *gr_font, int x, int y, const char *s)
{
    GGLContext *gl = gr_context;
    GRFont *font = gr_font;
    unsigned off;
    ...
    gl->bindTexture(gl, &font->texture.eng);
    gl->texEnvi(gl, GGL_TEXTURE_ENV, GGL_TEXTURE_ENV_MODE, GGL_REPLACE);
    gl->texGeni(gl, GGL_S, GGL_TEXTURE_GEN_MODE, GGL_ONE_TO_ONE);
    gl->texGeni(gl, GGL_T, GGL_TEXTURE_GEN_MODE, GGL_ONE_TO_ONE);
    gl->enable(gl, GGL_TEXTURE_2D);
...
int gr_measure_eng(GRFont *gr_font,const char *s)
{
...
{
	if(is_gb2312())
		gr_init_font_chn(font, gr_font);
	else
		gr_init_font_eng(font, gr_font);
}
...
int gr_text(GRFont *gr_font, int x, int y, const char *s)
{
	int ret = 0;
	if(is_gb2312())
		ret = gr_text_chn(gr_font, x, y, s);
	else
		ret = gr_text_eng(gr_font, x, y, s);
...
int gr_measure(GRFont *gr_font,const char *s)
{
	int ret = 0;
	if(is_gb2312())
		ret = gr_measure_chn(gr_font, s);
	else
		ret = gr_measure_eng(gr_font, s);
...
static int get_framebuffer(GGLSurface *fb)
{
    ...
    memset(&fi, 0, sizeof(struct fb_fix_screeninfo));
    void *bits = NULL;
    if(fb == NULL)
    {
        return -1;
    ...
    //LOGD(TAG "FACTORY_MODE_SUPPORT_RGBA_8888\n");
    if (ioctl(fd, FBIOGET_VSCREENINFO, &vi) < 0) {
...
/*	if(0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3)
	||0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 2)){
	  ...
	  //LOGD(TAG "fb->width=%d, fb->height=%d\n", fb->width, fb->height);
    fb->data =(GGLubyte*) bits;
    fb->format = getPIXEL_FORMAT();
...
/*	if(0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3)
    ...
    fb->data = (GGLubyte*) (((char*)bits) + vi.yres * vi.xres_virtual * 4);
#else
    fb->data = (GGLubyte*) (((char*)bits) + vi.yres * vi.xres_virtual * 2);
#endif
...
static int get_framebuffer_drm(GGLSurface *fb)
	{
		...
		if(gr_draw == nullptr){
			perror("failed to get gr_draw");
			
			return -1;
        }

		vi.xres = gr_draw->width;
		vi.yres = gr_draw->height;
		vi.xres_virtual = gr_draw->width;
...
#if defined(FACTORY_MODE_SUPPORT_RGBA_8888)
		//LOGD("vi.yres=%d, vi.xres_virtual=%d\n", vi.yres, vi.xres_virtual);
		fb->data = (GGLubyte*) (((char*)gr_draw->data()) + vi.yres * vi.xres_virtual * 4);
#else
		fb->data = (GGLubyte*) (((char*)gr_draw->data()) + vi.yres * vi.xres_virtual * 2);
#endif
	    fb->format = getPIXEL_FORMAT();
		return 0;
	}
...
#if defined(FACTORY_MODE_SUPPORT_RGBA_8888)
  ms->data = (GGLubyte*)malloc(vi.xres * vi.yres * 4);
#else
  ms->data = (GGLubyte*)malloc(vi.xres * vi.yres * 2);
#endif

	if(ms->data == NULL){
		return;
	}
	ms->format = getPIXEL_FORMAT();
...
int readSys_int(char const * path)
{
    ...
    if (fd >= 0) 
	{
		...
		LOGD(TAG " read boot mode struct len = %d\n", len);
		if(len > 0)
		{
			LOGD(TAG "boot mode size = %d, tag = %d, mode = %d\n", buffer[0], buffer[1], buffer[2]);
			close(fd);
			return buffer[2];
        }
		close(fd);
    }
    LOGD(TAG "read boot mode failed to open %s\n", path);
    return -1;    
}
...
int getBootMode()
{
	...
	if(NORMAL_BOOT== bootMode)
		LOGD(TAG "Normal mode boot!");
	else if(META_BOOT== bootMode)
		LOGD(TAG "Meta mode boot!");
	else if(FACTORY_BOOT == bootMode)
		LOGD(TAG "Meta mode factory boot ");
	else {
		LOGD(TAG "Not Support boot mode! BootMode=%d",bootMode);
		bootMode = -1;
	}
...
void gr_flip(void)
{
    ...
    char property_factory[256] = { 0 };
	property_get("vendor.mtk.factory.start",property_factory,"0");
	int factoryStart = atoi(property_factory);

    if(bootMode == META_BOOT && factoryStart == 0)
    {
        LOGD(TAG "Disable factory service UI");
	    return;
    }
...
//    unsigned int *d_temp;
    ...
    d_new = (unsigned int*)d;
#else
    unsigned short *s_temp;
//    unsigned short *d_temp;
    unsigned short *d_new;

    s_temp = (unsigned short*)s;
    d_new = (unsigned short*)d;
#endif

	if(d == NULL || s == NULL)
    {
        LOGE("gr_flip no resource memory!");
		return;
	}

/*	LOGE("MTK_LCM_PHYSICAL_ROTATION + %s", MTK_LCM_PHYSICAL_ROTATION);
    ...
    {
        for (j=vi.xres - 1, m = 0; m < vi.xres; m++, j--){
    ...
    {
        for (j=height - 1, m = 0; m < height; m++, j--)
        {
            for (k=0, l= vi.xres-1; k< vi.xres; k++, l--)
            {
                 d_temp = d_new + (width * j + l);
                *d_temp = *s_temp++;
            }
        }
    }
   ...
   // }
	...
	if(gr_draw!= nullptr){
		LOGE("Show ui by calling libdrm flip + ");
		gr_draw = gr_backend->Flip_setCrtc();
	}else
	{
		LOGE("set_active_framebuffer + ");
...
void gr_blank(bool bank)
{

	if(gr_draw!= nullptr){
		LOGE("Drm blank crtc ");
		gr_backend->Blank(bank);
	}else
	{
		LOGE("Invalid DRM srouce in using blank crtc. ");
...
void gr_blit_fbdev(gr_surface source, int sx, int sy, int w, int h, int dx, int dy) {
    if (gr_context == NULL) {
        return;
    }
    GGLContext *gl = gr_context;
...
void gr_blit_drm(gr_surface source, int sx, int sy, int w, int h, int dx, int dy) {


	uint8_t* src_p = ((GGLSurface*) source)->data+ sy * ((GGLSurface*) source)->width + sx * 4;
    uint8_t* dst_p = gr_mem_surface.data + dy * gr_draw->row_bytes + dx * 4;

    for (int i = 0; i < h; ++i) {
      memcpy(dst_p, src_p, w * 4);
      src_p += ((GGLSurface*) source)->width*4;
      dst_p += gr_mem_surface.width*4;
    }
...
void gr_blit(gr_surface source, int sx, int sy, int w, int h, int dx, int dy) {
    ...
    if(gr_draw!= nullptr){
	    ...
	    gr_blit_drm(source,sx,sy,w,h,dx,dy);
    }else{
	    gr_blit_fbdev(source,sx,sy,w,h,dx,dy);
...
{ 
    ...
    GGLContext *gl = gr_context;
  	...
  	if(is_gb2312())
  	{
//  		LOGD(TAG "init Chinese font library: 32*32");
        font_32_32.rundata = &rundata_3232[0];	
  		Font_Small =  &font_32_32;
		Font_Big   = &font_32_32;
  	}
	else
	{
//		LOGD(TAG "init English font library: 24*44");
        font_24_44.rundata = &rundata_2444[0];
		Font_Small =  &font_24_44;
		Font_Big   = &font_24_44;
	}
	
    gr_init_font(Font_Small,  &gr_font_small);
    gr_init_font(Font_Big, &gr_font_big);



    std::string format = "ABGR_8888";
    if (format == "ABGR_8888") {
    ...
    auto backend = std::make_unique<MinuiBackendDrm>();
      gr_draw = backend->Init();

	if(!gr_draw){
  		LOGD(TAG "init display as fbdev");
	    ...
	    if (gr_fb_fd < 0) {
  		...
  		LOGD(TAG "init display as drm");
		if(get_framebuffer_drm(gr_framebuffer) == -1){
			return -1;
			}
	}
	...
	if(fprintf(stderr, "framebuffer: fd %d (%d x %d)\n",
			gr_fb_fd, gr_framebuffer[0].width, gr_framebuffer[0].height) < 0){
    ...
    if (gr_draw == nullptr)
	{
        LOGD(TAG "refresh background test in fbdev");
        gr_flip();
    }else{
        LOGD(TAG "refresh background in drm");
