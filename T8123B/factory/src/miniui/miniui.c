...
static void *input_thread(void *cookie)
{
    ...
    {
        LOGD(TAG "The cookie parameter is  NULL\n");
	}
    for (;;) {
...
void ui_print(const char *fmt, ...)
{
    ...
    pthread_mutex_lock(&gUpdateMutex);
    if (text_row > 0 && text_col > 0 && text_row < MAX_ROWS && text_col < MAX_COLS) {
        char *ptr;
        for (ptr = buf; *ptr != '\0'; ++ptr) {
			if (text_row < 0 || text_row >= MAX_ROWS || text_col < 0 || text_col >= MAX_COLS ) {
				break;
			}
            if (*ptr == '\n' || text_col >= text_cols) {
...
*/
    ...
    if (ptext) {
        ...
        {
            if(text_row < 0  || text_row >= MAX_ROWS || text_col < 0 ){
                LOGD(TAG "invlide row and col%s\n", __FUNCTION__);
                break;
            }
            if (*ptr == '\n' || word_cnt  >= temp_text_cols || text_col >= MAX_COLS*2 || ((((*ptr)&0x80) != 0) && text_col >= MAX_COLS*2-1))
            {
                text_col = 0;
                word_cnt = 0;
                text_row = (text_row + 1) % text_rows;
...
int ui_font_width()
{
	int nWidth = FONT_WIDTH_CHN;
	
	if(!is_gb2312())
		nWidth = FONT_WIDTH_ENG;
...
int ui_font_height()
{
	int nHeight = FONT_HEIGHT_CHN;
	
	if(!is_gb2312())
		nHeight = FONT_HEIGHT_ENG;
