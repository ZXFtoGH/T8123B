...
// For camera Drm
static int g_hdisplay;
static int g_vdisplay;
static int g_drm_fd;
static unsigned int g_connector_id;
static unsigned int g_crtc_id;
static drmModeModeInfo *g_crtc_mode;

#define TAG "[Graphics_drm] "

static PixelFormat gr_pixel_format() {
  return pixel_format;
}
...
// -----------------------------------------------------------------------------
// for camera drm
int MinuiBackendDrm::getDisplayWidth() {
    return g_hdisplay;
}

int MinuiBackendDrm::getDisplayHeight() {
    return g_vdisplay;

}
int MinuiBackendDrm::getDrmFd() {
    return g_drm_fd;
}

unsigned int MinuiBackendDrm::getDrmConnectId() {
    return g_connector_id;
}

unsigned int MinuiBackendDrm::getDrmCrtcId() {
    return g_crtc_id;
}

drmModeModeInfo* MinuiBackendDrm::getDrmCrtcMode() {
    return g_crtc_mode;
}
// -----------------------------------------------------------------------------
GRSurface* MinuiBackendDrm::Init() {
  ...
  // for camera
  g_connector_id = main_monitor_connector->connector_id;
  ...
  // for camera
  g_crtc_id = main_monitor_crtc->crtc_id;

  main_monitor_crtc->mode = main_monitor_connector->modes[selected_mode];

  g_crtc_mode = &(main_monitor_crtc->mode);
...
GRSurface* MinuiBackendDrm::Flip_setCrtc() {
	   
    if(current_buffer < 0 || current_buffer > 1){
		LOGD(TAG "current_buffer is a invalid index");
        current_buffer = 0;
    }			
    if(drmModeSetCrtc(drm_fd, main_monitor_crtc->crtc_id, GRSurfaceDrms[current_buffer]->fb_id,
		0, 0, &main_monitor_connector->connector_id, 1,  &main_monitor_crtc->mode) != 0 ){
		perror("Failed to drmModeSetCrtc");
		LOGD(TAG "Failed to drmModeSetCrtc%d\n",GRSurfaceDrms[current_buffer]->fb_id);
    }
