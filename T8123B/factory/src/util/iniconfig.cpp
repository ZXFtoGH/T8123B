...
extern item_t ftm_test_items[ITEM_MAX_IDS];
...
int read_config(char *filename)
{
    ...
    if(filename == NULL)
    {
		LOGD(TAG "config file is invalid\n");
        return -1;
    }

    if (NULL == (fp = fopen(filename, "r"))){
		
		LOGD(TAG "Open config file [%s] failed:\n",filename);
        return num;
    }
	...
	if (fp!=NULL)
	{
		if(fclose(fp)!= 0)
		{
		    LOGE(TAG "[read_config]close file failed\n");
		}
	}
    return num;
