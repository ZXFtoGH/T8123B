...
int getIMEI(int sim, Connection& modem ,char *result)
{
    ...
    //for security check LOGD(TAG "get IMEI is %s",imei);

	if(strncpy(result, imei, strlen(imei)+1)  ==NULL ){
		 LOGD(TAG "failed strncpy\n");
	}	
	
	if(result[0] == '\0'){
   	    LOGD(TAG "IMEI Can't read back\n");
		
		if(strncpy(result, "unknown", strlen("unknown")+1)  ==NULL ){
			 LOGD(TAG "failed strncpy\n");
		}   
    }
   
   //for security check  LOGE("getIMEI %s",result);
    return 0;
}
...
int getModemVersion(Connection& modem, char *result)
{
    ...
    //for security check  LOGD("ver %s",ver);


	if(result == NULL){
		LOGD(TAG "resilt is emptyand spienyfailed strncpy\n");
		return 0;
	}

    if(strncpy(result,ver,strlen(ver)+1)  ==NULL){
        LOGD(TAG "failed strncpy\n");
    }
    if(result[0] == '\0'){
    ...
    //for security check  LOGD(TAG "getModemVersion result = %s\n", result);
    return 0;
}
...
int getBarcode(Connection& modem, char *result)
{
    ...
    //for security check  LOGD("Barcode %s",barcode);

     if(strncpy(result,barcode,strlen(barcode)+1)  ==NULL){
		LOGD(TAG "failed strncpy\n");

    }
	result[strlen(barcode)] = '\0';
    ...
    //for security check  LOGE("getBarcode result = %s\n", result);
    return 0;    
}
...
{   
    ...
    //for security check LOGD("MEID: %s",meid);
    p = strchr(meid, 'x');
    if(NULL == p)
    {
	...
	len = 128>strlen(p)?strlen(p):128;

	
	LOGE("get MEID result len: %d ", len);
	if(strncpy(result, p, len)  ==NULL ){
    ...
    //for security check LOGD(TAG "getMEID result = %s\n", result);
    return 0;
}
...
*/
    ...
    while(msg_len>0)
    {
        ...
        if(strncpy(buf, msg, 256>strlen(msg)?strlen(msg):256) == NULL){
			LOGD(TAG "failed strncpy\n");
        }
        buf[256>strlen(msg)?strlen(msg):256] = 0;
		printf("%s:\n", buf);
...
static int create_md_verinfo(char *info, int *len) {
	...
	if(md_num > MAX_MODEM_INDEX)
		md_num = MAX_MODEM_INDEX; // Max count is MAX_MODEM_INDEX + 1

	
	if(md_num <=0){
       LOGD("modem count is invalid");
	   return 0;
	}
    ...
    for(i = 0; i < MAX_MODEM_INDEX; i++)
    {
        ...
        {
  			if(j >= MAX_MODEM_INDEX)
				break;

			strncpy(ccci_path[j],temp_ccci_path[i], sizeof(ccci_path[j])>(strlen(temp_ccci_path[i])+1)?(strlen(temp_ccci_path[i])+1):(sizeof(ccci_path[j])-1));
            j++ ;
        }
    }
    ...
    if(isC2kSupport() == 1){
        ...
        if(lte_md_num == 1)
        {
            ...
            int ret_size = sprintf(return_data.version.modem_ver,"%s", md_ver[0]);
			if(ret_size < 0){
        ...
        {
            print_verinfo(info, len,  "Modem Ver.  ", md_ver[0]);
            if(sprintf(return_data.version.modem_ver,"%s", md_ver[0]) < 0){
                LOGD("error:printf result infroamtion failed!\n");
            }
...
char val[128] = {0};
    ...
    if(kernel_ver_fd!=-1)
    {
        ret = read(kernel_ver_fd, kernel_ver, 255);
		//for security check LOGD("kernel_ver = %s",kernel_ver);
        close(kernel_ver_fd);
    }
	else
	{
	...
	//for security check LOGD("kernel_ver = %s",ptr);
    ...
    if(snprintf(return_data.version.sw_ver,sizeof(return_data.version.sw_ver),"%s", val) <0 ){
		LOGD("error:printf result infroamtion failed!\n");
    }
    return *len;
}
...
char* display_version(int index, char* result, bool autoreturn)
{
    ...
    //for security check LOGD(TAG "g_nr_lines is %d, avail_lines is %d\n", g_nr_lines, avail_lines);
    vi.m_start = 0;
    vi.m_end = (nr_line < avail_lines ? nr_line : avail_lines);
    //for security check LOGD(TAG "vi.m_end is %d\n", vi.m_end);
