...
{
    //refernce to /vendor/mediatek/proprietary/bootable/bootloader/lk/platform/mt6735/atags.c bootmode struct.
    int fd;
    if (path == NULL)
        return -1;
    ...
    if (fd >= 0) {
		...
		LOGE(TAG "[Meta] read boot mode struct len = %d\n", len);
		if(len > 0)
		{
			LOGE(TAG "[Meta] boot mode size = %d, tag = %d, mode = %d\n", buffer[0], buffer[1], buffer[2]);
        	close(fd);
			return buffer[2];
	    }
		close(fd);
    ...
    LOGE(TAG " read boot mode failed to open %s\n", path);
    return -errno;    
}
...
void ftm_save_key_map(int key_code)
{
	...
	if(keys_num >KEY_NUM_MAX || keys_num < 0){
		LOGD(TAG "Invalid key num");
		return;
	}
...
void ftm_get_key_map(void)
{
	...
	char buf[256] = {0,};
	for(m = 0; m <32; m++)	{
		int bytes = sprintf(name, "/dev/input/event%d", m);
		if(bytes < 0){
			LOGD(TAG "Failed to sprintf");
		}
		if((fd = open(name, O_RDONLY, 0))>=0)		{
			...
			LOGD(TAG "open device failed + %s", name);
		}
	}
}
...
int getLoadType()  //eng : 1 or user : 2
{
	char szVal[128] = {0};
	int  ret  = 0;
	...
	LOGD(TAG "[Meta] ro.build.type = %s ret = %d.",szVal,ret);
	return ret;
