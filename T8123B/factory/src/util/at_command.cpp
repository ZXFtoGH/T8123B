...
void initTermIO(int portFd, int cmux_port_speed)
{
    ...
    if (ioctl(portFd, TIOCMBIS, &status) < 0)
    {
        LOGE(TAG "ioctl fail");
    }
    ...
    if(is_support_modem(idx[modem_index]))
    {
        if(modem_index == 3)
        {
            if (isC2kSupport()) {
                ...
                #else
                ret_size = snprintf(path, 32, "%s", ccci_get_node_name(USR_C2K_AT, (CCCI_MD)md_sys[modem_index]));
                #endif
            }
        }
        else
        {
            ret_size = snprintf(path, 32, "%s", ccci_get_node_name(USR_FACTORY_DATA, (CCCI_MD)md_sys[modem_index]));          
        }
		if(ret_size < 0){
			LOGD(TAG "Get CCCI Path failed"); 
			return 0;
		}
...
/* // There is not any function call this api. // jing W2016
...
loop:
        ...
        if(length > sizeof(rbuff[has_read])){
			read_length = sizeof(rbuff[has_read]);
		}
...
int wait4_readAck(const int fd, int timeout, char *buf) {
    ...
    while (has_read < BUF_SIZE) {
        ...
        if (FD_ISSET(fd, &rfds)) {
            ...
            ret = read(fd, &buf[has_read], BUF_SIZE -1);
...
int C2Kmodemsignaltest(Connection& modem)
{
    ...
    if(signal(SIGALRM,deal_URC_VPUP) == SIG_ERR){
	    LOGD(TAG "Failed to signal()!");
	}
...
{
    LOGD(TAG "%s start\n", __FUNCTION__);
    LOGD(TAG "singaltest phaseout\n");
    int flag_ATH = 0 ;
    flag_ATH = modem.TerminateCall() ;
    LOGD(TAG "flag_ATH = %d",flag_ATH);

	//singaltest phaseout. Keep code to check .
	return -1;
...
int wait_URC(int i)
{
    ...
    if(ID_EIND == i)
    {
        ...
        if(signal(SIGALRM,deal_URC_EIND) == SIG_ERR){
			LOGD(TAG "Failed to signal()!");
		}
    ...
    {
        v.it_value.tv_sec=10;
        if(signal(SIGALRM,deal_URC_VPON) == SIG_ERR){
			LOGD(TAG "Failed to signal()!");
		}
    ...
    {
        v.it_value.tv_sec=10;
        if(signal(SIGALRM,deal_URC_EUSIM) == SIG_ERR){
			LOGD(TAG "Failed to signal()!");
		}
    ...
    {
        v.it_value.tv_sec=10;
        if(signal(SIGALRM,deal_URC_UIMST) == SIG_ERR){
			LOGD(TAG "Failed to signal()!");
		}
