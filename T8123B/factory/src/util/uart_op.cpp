...
#define COM_PORT_TYPE_FILE "/proc/device-tree/chosen/atag,meta" //for common kernel
...
#define UART_PORT_INFO_FILE "/proc/device-tree/chosen/atag,meta" //for common kernel
...
int set_baudrate_length_parity_stopbits(int fd, unsigned int new_baudrate, int length, char parity_c, int stopbits)
{
	...
	if(CBAUDEX != speed){
		...
		LOGD(TAG "Standard baud\r\n");
	}else{
		LOGD(TAG "Custom baud\r\n");
...
int open_uart_port(int uart_id, int baudrate, int length, char parity_c, int stopbits)
{
	...
	ret_size = snprintf(dev,sizeof(dev),"%s%d","/dev/ttyMT", (int)(uart_id-1));
	if(ret_size < 0){
		LOGD(TAG "Ouput dev fail!\n");
		return -1;
	}
	ret = access(dev, F_OK);
	if (ret <= 0) {
        memset(dev, 0 , strlen(dev));
		ret_size = snprintf(dev, sizeof(dev), "%s%d", "/dev/ttyS", (int)(uart_id-1));
		if(ret_size < 0){
		    LOGD(TAG "Ouput dev fail!\n");
		return -1;
	}
	...
	if(-1 == set_baudrate_length_parity_stopbits(
		fd, localBaudrate, localLen, localParity_c, localStopbits))
	{
		LOGD(TAG "Set baudrate fail!\n");
		return -1;
	}

	/* Open success */
	LOGD(TAG "Open %s success!\n", dev); */
#endif
	return fd;
}
...
int open_usb_port(int uart_id, int baudrate, int length, char parity_c, int stopbits)
{
    ...
    if (baudrate == 0 || length == 0 || parity_c == 0 || stopbits == 0)
    {
        LOGD(TAG "Warning: The parameter is 0\n");
	}

	//sprintf(dev, "/dev/ttyMT%d", (int)(uart_id-1) );
	
	ret_size = snprintf(dev,sizeof(dev),"%s%d","/dev/ttyGS", (int)(uart_id-1));
	if(ret_size < 0){
		LOGD(TAG "Ouput dev fail!\n");
		return -1;
	}
...
{
    int usb_fd = -1;
  /*  usb_fd = open("dev/ttyGS0", O_RDWR|O_NOCTTY|O_NONBLOCK);
	if(usb_fd == -1)
	{
...
/*	char buf[12];
	...
	if (fd != -1)
	{
	    while(i < 3)
        {
    		...
    		while (bytes_read < 10)
    		{
    			res = read(fd, buf + bytes_read, 10-bytes_read);
    			if (res > 0)
    				bytes_read += res;
    			else
    				break;
            ...
            if(type != 0)
            {
                LOGD(TAG "usb state is not CONFIGURED\n");
                i++;
                usleep(10000);
            }
            else
		...
		LOGD(TAG "Query USB State OK.");
	}
	else
	{
		LOGD(TAG "Failed to open:/sys/class/android_usb/android0/state");
	}
*/
...
int  read_a_line(int fd, char rbuff[], unsigned int length)
{
	...
	LOGD(TAG "Enter read_a_line(): uart = %d\n", fd);
...
int  read_a_line_for_ata(int fd, char rbuff[], unsigned int length)
{
	unsigned int has_read=0;
	ssize_t      ret_val;
...
//        LOGE("usb_plus_in = %d\n", usb_plus_in);
		...
		if (pthread_mutex_lock (&uart_mutex))
		{
			LOGE(TAG "read_a_line pthread_mutex_lock ERROR!\n"); 
		}
		//LOGD("read_a_line uart_mutex lock done\n");
		...
		if (pthread_mutex_unlock (&uart_mutex))
		{
			LOGE(TAG "read_a_line pthread_mutex_unlock ERROR!\n"); 
		}
		//LOGD("read: %c, %d, %d", rbuff[has_read], ret_val, has_read);
		//LOGD("read_a_line uart_mutex unlock done\n");
		if(-1 == ret_val){
...
int write_chars(int fd, char wbuff[], unsigned int length)
{
    ...
    int  refer_time = 0, curr_time = 0;
    ssize_t      ret_val;
    
    if(-1 == fd)
    	return -1;
    LOGD(TAG "Enter write_chars()\n");
	
    /* Get refer time */
    refer_time = time(NULL);
    if(refer_time < 0){
		LOGD(TAG "Get time failed\n");
		return -1;	
    }
    while(has_write < length){
		...
		if (pthread_mutex_lock (&uart_mutex))
		{
			LOGE(TAG "write_chars pthread_mutex_lock ERROR!\n"); 
		}
		...
		if (pthread_mutex_unlock (&uart_mutex))
		{
			LOGE(TAG "write_chars pthread_mutex_unlock ERROR!\n"); 
		}
		//LOGD("write_chars uart_mutex unlock done\n");
#endif 

		if (-1==ret_val){
			LOGE(TAG "write_chars write ERROR! Error code = 0x%x\n", errno); 
    		return -1;
    	}
...
FTM_COM_TYPE getComType(void)
{
	FTM_COM_TYPE Com_type = FTM_UNKNOWN_COM;
...
/*		memset(buf, 0, COM_PORT_TYPE_STR_LEN + 1);
		...
		close(fd);
		Com_type = (FTM_COM_TYPE)atoi(buf);
*/
		int buffer[8] = {0};
		int len = read(fd, &buffer, sizeof(int)*8);
		LOGD(TAG "[Meta] read com type struct len = %d\n", len);
		if(len > 0)
		{
			LOGD(TAG "[Meta] com type size = %d, tag = %d, meta_com_type = %d uart = %d", buffer[0], buffer[1], buffer[2], buffer[4]);
		}
		close(fd);
		Com_type  = (FTM_COM_TYPE)buffer[2];
	}
	else //if (fd != -1)
	{
		LOGD(TAG "Failed to open com port type file %s", COM_PORT_TYPE_FILE);
	}
...
/*		if (read(fd, buf, sizeof(char)*COM_PORT_TYPE_STR_LEN) <= 0)
		{
			LOGE(TAG "ERROR can not read uart port id");
		}
		else
		{
			nPort = atoi(buf);
...
*/
	...
	{
		LOGE(TAG "Failed to open uart port file %s", UART_PORT_INFO_FILE);
	}
	LOGE(TAG "uart com port: %d", nPort);
...
int open_uart(void)
{
	int UART_Port_ID = getUartPort();
        if(UART_Port_ID>=1 && UART_Port_ID<=255)
	    return open_uart_port(UART_Port_ID, 115200, 8, 'N', 1);
	else
	    return -1;
}
