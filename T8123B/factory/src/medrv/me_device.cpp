...
unsigned int CDevice::dumpDataInHexString(const char* con, int length, unsigned int bytesPerRow)
{
	...
	for(i = 0; i < length; i++)
	{
		...
		if (j == rowLength - 1 || i == length - 1)
		{
			...
			if(getLoadType() != 2 && getPropValue("persist.vendor.meta.dumpdata") == 1){
                ME_Print(ME_TAG "[Medrv] type:%d", getLoadType());
                ME_Print(ME_TAG "[Medrv] dumpdata:%d", getPropValue("persist.vendor.meta.dumpdata"));
                ME_Print(ME_TAG "[Medrv] data:%s", hex);
            }
		}
	}
