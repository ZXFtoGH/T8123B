...
#define TEEI_RESERVED_REE_SHAREMEM_SIZE (2*1024*1024) /* 2MB */
#endif
#if (CFG_RPMB_SET_KEY && CFG_RPMB_KEY_PROGRAMED_IN_KERNEL)
...
static void teei_boot_param_dump(tee_arg_t_ptr teearg) {
    ...
    IMSG_DEBUG("%s teearg->tee_dev[4].dev_type : 0x%x\n", __func__, teearg->tee_dev[4].dev_type);
    IMSG_DEBUG("%s teearg->tee_dev[4].base_addr : 0x%x\n", __func__, teearg->tee_dev[4].base_addr);
    IMSG_DEBUG("%s teearg->tee_dev[4].intr_num : 0x%x\n", __func__, teearg->tee_dev[4].intr_num);
    IMSG_DEBUG("%s teearg->tee_dev[4].apc_num : 0x%x\n", __func__, teearg->tee_dev[4].apc_num);
    IMSG_DEBUG("%s teearg->tee_dev[4].param[0]: 0x%x\n", __func__, teearg->tee_dev[4].param[0]);
}
#endif
...
void teei_boot_param_prepare(u32 param_addr, u32 tee_entry, u64 teei_sec_dram_size, u64 dram_base,
    ...
    teearg->tee_log_base = get_tee_log_info();
    ...
    teearg->share_mem_paddr = get_tee_ree_share_mem_info();
...
#if (CFG_RPMB_SET_KEY && CFG_RPMB_KEY_PROGRAMED_IN_KERNEL)
	if (rpmb_set_flag == 1)
		CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_RPMB_KEY_NOT_PROGRAMMED, ISEE_FLAGS_RPMB_KEY_PROGRAM_MASK, ISEE_FLAGS_RPMB_KEY_PROGRAM_OFFSET);
	else
		CONFIG_ISEE_FLAGS(&teearg->flags, ISEE_FLAGS_RPMB_KEY_IS_PROGRAMMED, ISEE_FLAGS_RPMB_KEY_PROGRAM_MASK, ISEE_FLAGS_RPMB_KEY_PROGRAM_OFFSET);
#endif

    /* Add this for enc ta log start */
#if CFG_MICROTRUST_TEE_LOG_CRYPTO_SUPPORT
    ...
    CONFIG_ISEE_DEVS(&(teearg->tee_dev[1]), MT_SEC_GPT,0, MT_SEC_GPT_IRQ,
                         MT_SEC_GPT_DAPC, 0);
#else
    CONFIG_ISEE_DEVS(&(teearg->tee_dev[1]), MT_SEC_GPT, 0, 0, 0, 0);
#endif
 ...
 * TEE FUNCTIONS
 **************************************************************************/
static u32 tee_log_buf_start;
static u32 tee_ree_share_buf_start;
int get_tee_log_buf(void)
{
        u64 limit_addr = 0xC0000000;

	       u64 addr	= mblock_reserve_ext(&bootarg.mblock_info,
			(u64)TEE_LOG_BUF_SIZE, 0x200000,
			limit_addr, 0, "tee_log_reserved");

	if(!addr){
		IMSG_ERROR("Fail to allocate tee log buffer: 0x%x, 0x%x\n",
				TEE_LOG_BUF_SIZE, 0x200000);
		return -1;

	}
	tee_log_buf_start = (unsigned int)addr;
	return 0;

}

u32 get_tee_log_info(void)
{
	return  tee_log_buf_start;
}

static int get_tee_ree_share_mem_buf(void)
{
    u64 limit_addr = 0x80000000;

   u64 addr = mblock_reserve_ext(&bootarg.mblock_info,
    (u64)TEEI_RESERVED_REE_SHAREMEM_SIZE, 0x200000,
    limit_addr, 0, "tee_ree_reserved");

    if(!addr){
        IMSG_ERROR("Fail to allocate tee log buffer: 0x%x, 0x%x\n",
                TEEI_RESERVED_REE_SHAREMEM_SIZE, TEEI_RESERVED_REE_SHAREMEM_SIZE);
        return -1;

    }
    tee_ree_share_buf_start = (unsigned int)addr;
    return 0;

}

u32 get_tee_ree_share_mem_info(void)
{
    return  tee_ree_share_buf_start;
}
