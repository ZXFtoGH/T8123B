...
U16 u2g_num_dlycell_perT_all[DRAM_DFS_SRAM_MAX][CHANNEL_NUM];///TODO: to be removed by Francis
U16 u2gdelay_cell_ps_all[DRAM_DFS_SRAM_MAX][CHANNEL_NUM];///TODO: to be removed by Francis
U32 u4gVcore[DRAM_DFS_SRAM_MAX];
                ...
                u1VrefLevel += u1VrefStep)
            {
                ...
                for (iDelay = pi_start; iDelay <= pi_end; iDelay += pi_step)
                {
                    ...
                    else
                    {
                         ...
                         * iDelay >=0 and autok
                         */
                        u32 bitmap0[CATRAINING_NUM_LP5] = {0};
                        u32 bitmap1[CATRAINING_NUM_LP5] = {0};
                        u32 bitmap[2] = {0};
                        u16 steps;
...
static DRAM_STATUS_T rxdqs_gating_auto_cal_status(DRAMC_CTX_T *p,
    ...
    struct rxdqs_gating_best_win *best_win)
{
    U8 mck_center[DQS_NUMBER] = {0}, ui_center[DQS_NUMBER] = {0}, pi_center[DQS_NUMBER] = {0};
    U8 mck_left[DQS_NUMBER] = {0},ui_left[DQS_NUMBER]= {0}, pi_left[DQS_NUMBER] = {0};
    U8 mck_right[DQS_NUMBER] = {0}, ui_right[DQS_NUMBER] = {0}, pi_right[DQS_NUMBER] = {0};
                                              ...
                                              U8 K_Type)
{
    ...
    U16 backup_RX_FinalVref_Value[DQS_NUMBER]={0};
    U32 backup_DQS_Dly_Value[DQS_NUMBER]={0};
    U32 backup_DQM_Dly_Value[DQS_NUMBER]={0};
...
//#endif
        ...
        if (u1UseTestEngine == PATTERN_TEST_ENGINE)
        {
            for (u1ByteIdx=0; u1ByteIdx<(p->data_width/DQS_BIT_NUMBER); u1ByteIdx++)
            {
                if(((u1min_winsize[u1ByteIdx] * gHQALOG_RX_delay_cell_ps_075V * DDRPhyFMeter(p) * 2) )/1000000 < 50)
                {
...
void DramcJmeterCalib(DRAMC_CTX_T *p, JMETER_T *pJmtrInfo, U16 u2JmDlyStep, U8 u1IsJmtrK)
{
 ...
 */
static void GetVcoreDelayCellTime(DRAMC_CTX_T *p, U8 SRAMIndex)
{
...
#if __ETT__
#if (FOR_DV_SIMULATION_USED==0 && SW_CHANGE_FOR_SIMULATION==0)
    u4gVcore[SRAMIndex] = pmic_vcore_voltage_read();
#endif

    /* delay cell calculation is skipped if vcore is same as previous shuffle's */
    if (u4gVcore[SRAMIndex] != u4previousVcore)
    {
        u4previousVcore = u4gVcore[SRAMIndex];
        DramcMiockJmeter(p);
    }
#else
        DramcMiockJmeter(p);
    ...
    {
        u2g_num_dlycell_perT_all[SRAMIndex][channel_i] = u2g_num_dlycell_perT;
        u2gdelay_cell_ps_all[SRAMIndex][channel_i] = u2gdelay_cell_ps;
    }
#if __ETT__
    mcSHOW_DBG_MSG(("Freq=%d, CH_%d, VCORE=%d, cell=%d\n", p->frequency, p->channel, u4gVcore[SRAMIndex], u2gdelay_cell_ps_all[SRAMIndex][p->channel]));
#endif
...
void DramcMiockJmeterHQA(DRAMC_CTX_T *p)
{
    ...
    SRAMIndex = vGet_Current_SRAMIdx(p);

    if(p->channel == CHANNEL_A)
    {
        if (p->frequency <= 600)
        {
            u2g_num_dlycell_perT_all[SRAMIndex][p->channel] = 0; // always lookup table
            u2gdelay_cell_ps_all[SRAMIndex][p->channel] = 270; // @Darren, Wait arnold for lookup table
        }
        else
            GetVcoreDelayCellTime(p, SRAMIndex);
    }

    u2gdelay_cell_ps_all[SRAMIndex][CHANNEL_B] = u2gdelay_cell_ps_all[SRAMIndex][CHANNEL_A];

#ifdef FOR_HQA_TEST_USED
    if (u2g_num_dlycell_perT_all[SRAMIndex][p->channel] == 0) GetVcoreDelayCellTimeFromTable(p); //lookup table
#endif

    /* Use highest freq's delay cell time measurement results as reference */
    p->u2num_dlycell_perT = u2g_num_dlycell_perT_all[SRAMIndex][p->channel];
    p->u2DelayCellTimex100 = u2gdelay_cell_ps_all[SRAMIndex][p->channel];
    mcSHOW_DBG_MSG3(("DelayCellTimex100 CH_%d, (VCORE=%d, cell=%d)\n",p->channel, u4gVcore[SRAMIndex], p->u2DelayCellTimex100));
}
#endif
                ...
                if (JmtrInfo.JmtrInfo[u1JmtrPrintCnt].u1TransLevel == 1) // find the High Level
                {
                    ...
                    else if (u18Phase_SM == DQS_8PH_DEGREE_45)
                    {
                        ...
                        if (u2P > u2R0) // u2P ~= DQS_8PH_DEGREE_180
                        {
                            ...
                            mcSHOW_DBG_MSG(("diff (P-R) = %d, min = %d, early break count = %d, R45 (H) = %d\n", s2Err_code, s2Err_code_min, u1loop_cnt, u2P));
...
***/
    ...
    {   // freq_region == 1
        ...
        //DRVN 10~30
        //ODTN 3~12
        if (!(gDramcSwImpedanceResult[freq_region][DRVP] >= 1 && gDramcSwImpedanceResult[freq_region][DRVP] <= 24) ||
...
S8 gcFinal_K_CLK_delay_cell[DQS_NUMBER];
S8 gcFinal_K_DQS_delay_cell[DQS_NUMBER];
S8 gcFinal_K_WCK_delay_cell[DQS_NUMBER];
#if APPLY_DQDQM_DUTY_CALIBRATION
S8 gcFinal_K_DQ_delay_cell[DQS_NUMBER];
S8 gcFinal_K_DQM_delay_cell[DQS_NUMBER];
#endif
        ...
        #endif
        {
            ...
            if (Get_Duty_Calibration_Mode(p) == DUTY_DEFAULT)
            {
                //clear global variables to 0 delay
                    gcFinal_K_CLK_delay_cell[0] = 0;
                    gcFinal_K_DQS_delay_cell[0] = 0;
                    gcFinal_K_DQS_delay_cell[1] = 0;
                    gcFinal_K_WCK_delay_cell[0] = 0;
                    gcFinal_K_WCK_delay_cell[1] = 0;
                #if APPLY_DQDQM_DUTY_CALIBRATION
                    gcFinal_K_DQM_delay_cell[0] = 0;
                    gcFinal_K_DQM_delay_cell[1] = 0;
                    gcFinal_K_DQ_delay_cell[0] = 0;
                    gcFinal_K_DQ_delay_cell[1] = 0;
                #endif
            }
#endif
            ...
            {
                DramcClockDutySetClkDelayCell(p, gcFinal_K_CLK_delay_cell);
                DQSDutyScan_SetDqsDelayCell(p, gcFinal_K_DQS_delay_cell);
                WCKDutyScan_SetWCKDelayCell(p, gcFinal_K_WCK_delay_cell);

#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
                if(p->femmc_Ready==0)
                {
                    p->pSavetimeData->s1ClockDuty_clk_delay_cell[p->channel][RANK_0] = gcFinal_K_CLK_delay_cell[RANK_0];
                    p->pSavetimeData->s1ClockDuty_clk_delay_cell[p->channel][RANK_1] = gcFinal_K_CLK_delay_cell[RANK_1];
                    p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel][0] = gcFinal_K_DQS_delay_cell[0];
                    p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel][1] = gcFinal_K_DQS_delay_cell[1];
                    p->pSavetimeData->s1WCKDuty_clk_delay_cell[p->channel][0] = gcFinal_K_WCK_delay_cell[0];
                    p->pSavetimeData->s1WCKDuty_clk_delay_cell[p->channel][1] = gcFinal_K_WCK_delay_cell[1];
                }
#endif

                    #if APPLY_DQDQM_DUTY_CALIBRATION
                DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, gcFinal_K_DQM_delay_cell, DutyScan_Calibration_K_DQM);
                DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, gcFinal_K_DQ_delay_cell, DutyScan_Calibration_K_DQ);

#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
                    if(p->femmc_Ready==0)
                    {
                    p->pSavetimeData->s1DQMDuty_clk_delay_cell[p->channel][0] = gcFinal_K_DQM_delay_cell[0];
                    p->pSavetimeData->s1DQMDuty_clk_delay_cell[p->channel][1] = gcFinal_K_DQM_delay_cell[1];
                    p->pSavetimeData->s1DQDuty_clk_delay_cell[p->channel][0] = gcFinal_K_DQ_delay_cell[0];
                    p->pSavetimeData->s1DQDuty_clk_delay_cell[p->channel][1] = gcFinal_K_DQ_delay_cell[1];
                    }
#endif
                    #endif
