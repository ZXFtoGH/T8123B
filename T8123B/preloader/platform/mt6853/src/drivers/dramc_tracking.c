...
void Enable_TX_Tracking(DRAMC_CTX_T *p, U32 u4DramcShuOffset)
{
    ...
    if ((vGet_Current_SRAMIdx(p) == SRAM_SHU6)
#if ENABLE_DDR400_OPEN_LOOP_MODE_OPTION
        || (vGet_Current_SRAMIdx(p) == SRAM_SHU7)) //add if(u1ShuffleIdx==DRAM_DFS_SRAM_MAX) to avoid enable tx-tracking when running DDR800 as RG-SHU0
#else
        )
#endif
    {
...
void Enable_RDSEL_Tracking(DRAMC_CTX_T *p, U32 u4DramcShuOffset)
{
    ...
    if (vGet_Current_SRAMIdx(p) == SRAM_SHU0) //add if(u1ShuffleIdx==DRAM_DFS_SRAM_MAX) to avoid enable tx-tracking when running DDR800 as RG-SHU0
    {
...
void Enable_Gating_Tracking(DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset)
{
    if ((vGet_Current_SRAMIdx(p) == SRAM_SHU6)
#if ENABLE_DDR400_OPEN_LOOP_MODE_OPTION
        || (vGet_Current_SRAMIdx(p) == SRAM_SHU7))
#else
       )
#endif
    {
...
void DramcSWTxTracking(DRAMC_CTX_T *p)
{
    ...
    for (shuIdx = 0; shuIdx < DRAM_DFS_SRAM_MAX; shuIdx++)
    {
        if (shuIdx == vGet_Current_SRAMIdx(p)) {
...
void FreqJumpRatioCalculation(DRAMC_CTX_T *p)
{
    ...
    if (vGet_DDR_Loop_Mode(p) == CLOSE_LOOP_MODE)
    {
        ...
        for (shuffle_dst_index = SRAM_SHU0; shuffle_dst_index < DRAM_DFS_SRAM_MAX; shuffle_dst_index++)
        {
            ...
            mcSHOW_DBG_MSG3(("Jump_RATIO [%d]: %x\t SRAM %d -> SRAM %d\tDDR%d -> DDR%d\n", jump_ratio_index, u2JumpRatio[jump_ratio_index], vGet_Current_SRAMIdx(p), shuffle_dst_index, shuffle_src_freq << 1, u2Freq << 1));
            jump_ratio_index++;
        }
    }
