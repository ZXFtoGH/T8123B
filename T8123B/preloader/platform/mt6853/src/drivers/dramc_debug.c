...
void print_HQA_measure_message(DRAMC_CTX_T *p)
{
    ...
    if (p->support_rank_num == RANK_DUAL)
    {
        ...
        if (u1IsLP4Family(p->dram_type)) {
            ...
            HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("Vdd1_HQA = %d\n", read_voltage_value)); /* confirm with pmic owner,this vio18 is pmic 6359 's vm18, is different with vio18 of system */
    ...
    */
    ...
    //if(p->frequency == u2DFSGetHighestFreq(p))
    {
        if (u1IsLP4Family(p->dram_type))    //LP4 Series
        {
            ...
            for (shuffle_index = SRAM_SHU0; shuffle_index < DRAM_DFS_SRAM_MAX; shuffle_index++)
            {
                mcSHOW_DBG_MSG(("\nSHUFFLE %d\n", shuffle_index + 1));
    ...
    else
    {
        ...
        mcSHOW_JV_LOG_MSG(("%d%%(bit %d)         %d%%(bit %d)\n\n",
                                        ((min_rx_value[0][0] * u2gdelay_cell_ps_all[SRAMIdx][0] * p->frequency * 2) + (1000000 - 1)) / 1000000, min_RX_DQ_bit[0][0],
                                        ((min_rx_value[0][1] * u2gdelay_cell_ps_all[SRAMIdx][0] * p->frequency * 2) + (1000000 - 1)) / 1000000, min_RX_DQ_bit[0][1]));
    }
#endif
    ...
    if (print_type == EYESCAN_TYPE_RX || print_type == EYESCAN_TYPE_TX)
    {
        ...
        for (u1BitIdx = u1BitIndex_Begin; u1BitIdx < u1BitIndex_End; u1BitIdx++)
        {
            ...
            else //if (print_type==2)
            {
                ...
                Perbit_Eye_Area = (gEyeScan_TotalPassCount[u1BitIdx] * 10 * 3 * vddq / (32 * DDRPhyFMeter(p)))*100; //total count*1/32UI*(1/freq*10^6 ps)*(0.6%vddq)
            }
...
void DumpAoShuRkReg(DRAMC_CTX_T *p)
{
    ...
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_B1_TXDLY0, DDRPHY_REG_SHU_RK_B1_BIST_CTRL);
...
void vApplyProgramSequence(DRAMC_CTX_T *p)
{
    ...
    #elif SAMSUNG_TEST_MODE_MRS_FOR_PRELOADER == 6
