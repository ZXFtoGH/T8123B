...
static bool rtc_init_done_check(void);
...
static bool rtc_android_init(void)
{
	...
	RTC_Write(RTC_AL_YEA, (RTC_Read(RTC_AL_YEA) & ~(RTC_AL_YEA_MASK) | ((1970 - RTC_MIN_YEAR) & RTC_AL_YEA_MASK) | RTC_K_EOSC_RSV_1 | RTC_K_EOSC_RSV_0));
...
static bool rtc_lpd_init(void)
{
	...
	// Enable both LPD, however XOSC LPD is only used to check whether initialization is completed or not.
...
void rtc_enable_k_eosc(void)
{
	...
	Write_trigger();
	/* We use solution 2 of eosc cali to fix mt6359p 32k */
	RTC_Write(RTC_AL_YEA, RTC_Read(RTC_AL_YEA) | RTC_K_EOSC_RSV_2);
	Write_trigger();
...
void rtc_init(void)
{
	...
	if ((RTC_Read(RTC_CON) & RTC_CON_LPSTA_RAW) || check_mode_flag || !(RTC_Read(RTC_DIFF) & RTC_POWER_DETECTED) || !rtc_init_done_check())
	{
		if (!rtc_first_boot_init(result)) {
			rtc_recovery_flow();
		}
		// XOSC LPD is no longer useful, we use it for initialization done check.
		// Disable XOSC LPD after initialization completed.
	...
	RTC_Write(RTC_AL_YEA,(RTC_Read(RTC_AL_YEA) & (~RTC_K_EOSC_RSV_2)) | (RTC_K_EOSC_RSV_1) | (RTC_K_EOSC_RSV_0));
...
static bool rtc_init_done_check(void)
{
	...
	// If only EOSC LPD is enabled means initialization done.
	return ((RTC_Read(RTC_AL_SEC) & RTC_LPD_OPT_MASK) == RTC_LPD_OPT_EOSC_LPD) ? true : false;
}
