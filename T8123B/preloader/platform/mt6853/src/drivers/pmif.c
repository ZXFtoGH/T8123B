...
static int mt6xxx_regs[] = {
	...
	/* hw mpu */
	[PMIF_PMIC_ALL_RGN_EN] =		0x0F94,
	[PMIF_PMIC_ALL_RGN_0_START] =		0x0F98,
	[PMIF_PMIC_ALL_RGN_0_END] =		0x0F9C,
	[PMIF_PMIC_ALL_RGN_1_START] =		0x0FA0,
	[PMIF_PMIC_ALL_RGN_1_END] =		0x0FA4,
	[PMIF_PMIC_ALL_RGN_2_START] =		0x0FA8,
	[PMIF_PMIC_ALL_RGN_2_END] =		0x0FAC,
	[PMIF_PMIC_ALL_RGN_3_START] =		0x0FB0,
	[PMIF_PMIC_ALL_RGN_3_END] =		0x0FB4,
	[PMIF_PMIC_ALL_INVLD_SLVID] =		0x0FD8,
	[PMIF_PMIC_ALL_RGN_0_PER0] =		0x0E00,
	[PMIF_PMIC_ALL_RGN_1_PER0] =		0x0E08,
	[PMIF_PMIC_ALL_RGN_2_PER0] =		0x0E10,
	[PMIF_PMIC_ALL_RGN_3_PER0] =		0x0E18,
	[PMIF_PMIC_ALL_RGN_OTHERS_PER0] =	0x0E40,
};
...
static struct pmif pmif_spmi_arb[] = {
	...
	},
	{
		...
		.pmif_enable_swinf = pmif_spmi_enable_swinf,
		.pmif_enable_cmdIssue = pmif_spmi_enable_cmdIssue,
		.pmif_enable = pmif_spmi_enable_p,
		.is_pmif_init_done = is_pmif_spmi_init_done,
	},
};
...
static int pmif_config_mpu(int mstid)
{
	...
	struct pmif_mpu pmif_mpu_table[2] = {
		0x3, 0x0000, 0x13ff, 0xfff3ccc3,
		0x3, 0x1400, 0x16ff, 0xfff3ccc0,
	};
#else
	struct pmif_mpu pmif_mpu_table[4] = {
		0x9, 0x0000, 0x0226, 0xfff3cfc3,
		0x9, 0x022a, 0x02fc, 0xfff3ccc3,
		0x9, 0x0300, 0x05ff, 0xfff3cfc3,
		0x9, 0x0700, 0x0714, 0xfff7ccd7,
	};
#endif
...
static int pmif_config_mpu_p(int mstid)
{
	...
	/* slaveid, start addr, end addr, permission */
	struct pmif_mpu pmif_mpu_table[2] = {
		0x8, 0x0000, 0x05ff, 0xffffffff,
		0x8, 0x0700, 0x0714, 0xfffffcfc,
	};
	...
	if (en == TRUE) {
		...
		if ((mstid == SPMI_MASTER_0) || (mstid == SPMI_MASTER_1)) {
			...
			DRV_WriteReg32(arb->base + arb->regs[PMIF_MPU_CTRL],
					0x71);/* scp domain is 12, no used */
			DRV_WriteReg32(arb->base + arb->regs[PMIF_PMIC_ALL_INVLD_SLVID],
					0xfdf6);/*add id=0x3, 0x9(for dts)*/
		...
		if ((mstid == SPMI_MASTER_0) || (mstid == SPMI_MASTER_1)) {
			...
			DRV_WriteReg32(arb->base + arb->regs[PMIF_MPU_CTRL],
					0x71);/* scp domain is 12, no used */
			DRV_WriteReg32(arb->base + arb->regs[PMIF_PMIC_ALL_INVLD_SLVID],
					0xfdf6);/*add id=0x9, 0x3(for dts)*/
			/* enable rgn0~3, others protection */
			DRV_WriteReg32(arb->base + arb->regs[PMIF_PMIC_ALL_RGN_EN],
					0x10f);
		} else if (mstid == SPMI_MASTER_P_1) {
			/* wk, invalid slive can't set CTRL[16](slvid=0) to 1,
			 * it will cause acc_vio always be triggered
			 */
			DRV_WriteReg32(arb->base + arb->regs[PMIF_MPU_CTRL],
...
#endif /* end of #if PMIF_NORMAL_BOOT */
	/* Add latency limitation */
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_CNTER_EN], 0x2F6);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_1], 0x4);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_2], 0x8);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_4], 0x8);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_6], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_9], 0x4);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_LOADING], 0x2F6);

	/* config mpu */
	pmif_config_mpu(mstid);

	DRV_WriteReg32(arb->base + arb->regs[PMIF_INF_EN], 0x2F6);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_ARB_EN], 0x2F6);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_TIMER_CTRL], 0x3);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_INIT_DONE], 0x1);
#endif
	PMIF_INFO("%s done\n", __func__);
...
static void pmif_spmi_enable_p(int mstid)
{
	...
	hw_bytecnt = PMIF_BYTECNT_MAX -1;
	if (hw_bytecnt > 0) {
		bytecnt_per = hw_bytecnt << 16 | hw_bytecnt << 12 |
			hw_bytecnt << 4 | hw_bytecnt << 0;
	}
	DRV_WriteReg32(arb->base + arb->regs[PMIF_INF_MAX_BYTECNT_PER_0],
			bytecnt_per);
...
#endif /* end of #if PMIF_NORMAL_BOOT */
	/* Add latency limitation */
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_CNTER_EN], 0x19);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_0], 0x0);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_3], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_4], 0x8);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_LOADING], 0x19);

	/* config mpu */
	pmif_config_mpu_p(mstid);

	DRV_WriteReg32(arb->base + arb->regs[PMIF_INF_EN], 0x19);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_ARB_EN], 0x19);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_TIMER_CTRL], 0x3);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_INIT_DONE], 0x1);
