...
void UpdateHighestFreqInDFSTbl(DRAMC_CTX_T *p, DRAM_PLL_FREQ_SEL_T new_freq_sel)
{
    ...
    #endif
    mcSHOW_DBG_MSG(("[UpdateHighestFreqInDFSTbl] Get Highest Freq is %d\n", u2HighestFreq));
#endif
}
...
void UpdateHighestFreqToDDR3733(DRAMC_CTX_T *p)
{
    ...
    if (emi_set->highest_freq == 3733) {
        ...
        mcSHOW_DBG_MSG(("[3733]UpdateHighestFreqToDDR3733 overwrite\n"));
    }
}
...
void SV_BroadcastOn_DramcInit(DRAMC_CTX_T *p)
{
    DramcBroadcastOnOff(DRAMC_BROADCAST_ON);
...
DRAM_STATUS_T DramcInit(DRAMC_CTX_T *p)
{
    ...
    gddrphyfmeter_value = DDRPhyFreqMeter() & 0xFFFF;
