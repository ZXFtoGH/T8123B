...
void GetVcoreDelayCellTimeFromTable(DRAMC_CTX_T *p)
{
    ...
    {
        u2gdelay_cell_ps_all[vGet_Current_SRAMIdx(p)][channel_i] = u2gdelay_cell_ps;
        u4gVcore[vGet_Current_SRAMIdx(p)] = get_vcore/1000;
    }
}
#endif
...
void PRE_MIOCK_JMETER_HQA_USED(DRAMC_CTX_T *p)
{
    ...
    if(p->femmc_Ready==1)
    {
        ...
        {
            //for (shuffleIdx = SRAM_SHU0; shuffleIdx < DRAM_DFS_SRAM_MAX; shuffleIdx++)
            {
                u2g_num_dlycell_perT_all[vGet_Current_SRAMIdx(p)][channel_idx] = p->pSavetimeData->u2num_dlycell_perT;
                u2gdelay_cell_ps_all[vGet_Current_SRAMIdx(p)][channel_idx] = p->pSavetimeData->u2DelayCellTimex100;
            }
        }
    ...
    if(p->femmc_Ready==0)
    {
        ...
        for(channel_idx=CHANNEL_A; channel_idx<p->support_channel_num; channel_idx++)
        {
            for (shuffleIdx = SRAM_SHU0; shuffleIdx < DRAM_DFS_SRAM_MAX; shuffleIdx++)
            {
                p->pSavetimeData->u2g_num_dlycell_perT_all[channel_idx] = u2g_num_dlycell_perT_all[vGet_Current_SRAMIdx(p)][channel_idx];
                p->pSavetimeData->u2gdelay_cell_ps_all[channel_idx] = u2gdelay_cell_ps_all[vGet_Current_SRAMIdx(p)][channel_idx];
            }
        }
        #endif
...
#endif
        {
            if (!(u1IsPhaseMode(p) && (p->rank == RANK_1))) // skip for DDR400/800 rank1
            {
                mcSHOW_DBG_MSG(("\n----->DramcWriteLeveling(PI) begin...\n"));
...
void vDramCalibrationAllChannel(DRAMC_CTX_T *p)
{
    ...
    DFSRuntimeMRW_preset(p, vGet_Current_SRAMIdx(p));
#endif
...
int Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_EXTERN_T dram_cbt_mode_extern, DRAM_INFO_BY_MRR_T *DramInfo, U8 get_mdl_used)
{
    ...
    {
        #if (fcFOR_CHIP_ID == fcMouton) // @Darren, new chip need double confirm
        get_dram_frequency_from_hw_trap();
        UpdateHighestFreqToDDR3733(p); // @cc, coload, change freq if necessary
        if (p->DRAMPinmux == PINMUX_DSC)
            UpdateHighestFreqInDFSTbl(p, LP4_DDR3200);
        #endif
    ...
    {
        #if (fcFOR_CHIP_ID == fcMouton) // @Darren, new chip need double confirm
        if ((ett_fix_freq == 0xff) && (p->DRAMPinmux == PINMUX_DSC) && (gFreqTbl[u1ShuIdx].SRAMIdx== SRAM_SHU1))
            continue;
        #endif
        ...
        #if (fcFOR_CHIP_ID == fcMouton) // @Darren, new chip need double confirm
        if ((ett_fix_freq == 0xff) && (p->DRAMPinmux == PINMUX_DSC) && (gFreqTbl[u1ShuIdx].SRAMIdx== SRAM_SHU0))
            DramcSaveToShuffleSRAM(p, DRAM_DFS_REG_SHU0, gFreqTbl[u1ShuIdx - 1].SRAMIdx); // Copy SRAM_SHU0 to SRAM_SHU1
        #endif
...
//    #define __FW_VER__ "WCK leveling with DLY +16! and MRinit for FSP1 -- 777"
    ...
    #endif
    {
        ...
        #if ENABLE_SRAM_DMA_WA
        DPHYSaveToSRAMShuWA(p, gFreqTbl[s1ShuIdx].SRAMIdx);
        #endif
    }
#endif
    ///TODO: wait for porting ---
