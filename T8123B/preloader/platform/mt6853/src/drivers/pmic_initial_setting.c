...
{
#ifdef MTK_5G_B_MT6360_MT6315
	/* VRF09 use MT6359 VMODEM in MT6853 */
	...
	/* VRF13 use MT6360 VBUCK2 in MT6853 */
#else
	/* VRF09 use MT6362 VBUCK5 in MT6853 */
	/* VRF13 use MT6362 VBUCK6 in MT6853 */
#endif
	/* VRF18 use VRF18 in MT6853 */
...
void pmic_default_voltage(void)
{
	...
	int vgpu_vol = 750000;
	/* TODO:for APU bring up, remove it after bring up */
	int vpu_vol = 775000;
	...
	if (mtk_regulator_set_voltage(&reg_vgpu, vgpu_vol, vgpu_vol))
		ret |= (1 << 5);
	/* TODO:for APU bring up, remove it after bring up */
	if (mtk_regulator_set_voltage(&reg_vpu, vpu_vol, vpu_vol))
		ret |= (1 << 6);
	if (ret) {
...
static void mt6359_code_check(void)
{
	...
	if (code != 0x3) {
#endif
		pal_log_info("MT6359 PF_CODE=%d is wrong for MT6853\n", code);
		ASSERT(0);
	}
}
