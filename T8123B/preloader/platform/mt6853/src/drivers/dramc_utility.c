...
U16 DDRPhyFMeter(DRAMC_CTX_T *p)
{
    static U16 u2Prefreq = 0, u2Freqmeter = 0;

    if(p->frequency != u2Prefreq)
    {
        if(vGet_Div_Mode(p) == DIV8_MODE)
        {
            u2Freqmeter = gddrphyfmeter_value << 2;
        }
        else if(vGet_Div_Mode(p) == DIV4_MODE)
        {
            u2Freqmeter = gddrphyfmeter_value << 1;
        }
        else if(vGet_Div_Mode(p) == DIV16_MODE)
        {
            u2Freqmeter = gddrphyfmeter_value << 4;
        }
        else
        {
            mcSHOW_ERR_MSG(("[DDRPhyFMeter] Get DIV mode error!\n"));
        #if __ETT__
            while (1);
        #endif
        }

        if(u2Freqmeter != 0)  //@tg avoid updating Prefreq before do DramcInit(When SWImpcal).
        {
            u2Prefreq = p->frequency;
        }
    }

    /* The result may be used as divisor, so need to avoid 0 returned */
    return (u2Freqmeter ? u2Freqmeter : p->frequency);
}
...
void vPrintCalibrationBasicInfo(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG(("===============================================================================\n"
                    ...
                    "===============================================================================\n",
                        p->dram_type, DDRPhyFMeter(p)?DDRPhyFMeter(p):p->frequency, p->freqGroup, p->channel, p->rank,
