...
static const u8 buck2_ctrl_init[] = {
	0xa9, 0x80, 0x81, 0x6a, 0x20, 0x80, 0x02, 0x40,
};

static const u8 ldo6_ctrl_init[] = { 0x06, 0x80, 0x02, 0xec, 0x44 };
static const u8 ldo7_ctrl_init[] = { 0x06, 0x80, 0x02, 0xec, 0x44 };
#if 0
...
static int mt6360_pmic_is_hw_exist(void)
{
	...
	if (ret < 0 || (tmp & 0x60) != 0x60) {
		dbg_print("%s: ID:%x is wrong for MT6853\n", __func__, tmp);
		ASSERT(0);
	}
