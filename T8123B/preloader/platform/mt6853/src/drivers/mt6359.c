...
#else
	print("latch VPU(VPROC2) %d uV(0x%x)\n", vproc2_min_uV + vproc2_step_uV * gray_to_binary(val), val);
#endif
...
#ifdef MTK_5G_B_MT6360_MT6315
	print("latch VRF09(VMODEM) %d uV(0x%x)\n", vproc2_min_uV + vproc2_step_uV * gray_to_binary(val), val);
#else
	print("latch VPROC2(VMODEM) %d uV(0x%x)\n", vmodem_min_uV + vmodem_step_uV * gray_to_binary(val), val);
#endif
