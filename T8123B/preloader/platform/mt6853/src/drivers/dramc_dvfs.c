...
void DFSRuntimeMRW_preset(DRAMC_CTX_T *p, U8 sram_shu_level)
{
    ...
    U8 u1MR03_Value = 0, u1MD32Offset = 0;
    ...
    for (u1ChIdx = CHANNEL_A; u1ChIdx < p->support_channel_num; u1ChIdx += DPM_CH_NUM)
    {
        ...
        if (u1ChIdx >= CHANNEL_C)
            u1MD32Offset = ((u1ChIdx >> 1) << POS_BANK_NUM);
#endif
        //! save shux mr12/mr14
        vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_0 + (DRAM_DFS_SRAM_MAX << 4) + u1MD32Offset + (sram_shu_level << 4),
            P_Fld(u1MR12Value[u1ChIdx][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_0_MR_OP_SET_SHU_0_0) |
            P_Fld(u1MR12Value[u1ChIdx][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_0_MR_OP_SET_SHU_0_1) |
            P_Fld(u1MR12Value[u1ChIdx+1][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_0_MR_OP_SET_SHU_0_2) |
            P_Fld(u1MR12Value[u1ChIdx+1][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_0_MR_OP_SET_SHU_0_3));

        vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_1 + (DRAM_DFS_SRAM_MAX << 4) + u1MD32Offset + (sram_shu_level << 4),
            P_Fld(u1MR14Value[u1ChIdx][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_1_MR_OP_SET_SHU_0_4) |
            P_Fld(u1MR14Value[u1ChIdx][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_1_MR_OP_SET_SHU_0_5) |
            P_Fld(u1MR14Value[u1ChIdx+1][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_1_MR_OP_SET_SHU_0_6) |
            P_Fld(u1MR14Value[u1ChIdx+1][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_1_MR_OP_SET_SHU_0_7));
    }
...
static void DFSRuntimeMRWEn(DRAMC_CTX_T *p, U8 cur_shu_mux_index, U8 nxt_shu_level, U8 pingpong_fsp)
{
    ...
    for (u1ChIdx = ch_start; u1ChIdx < ch_end; u1ChIdx++)
    {
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            //! get shux mr12/mr14/
            rtmr12 = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_0 + (DRAM_DFS_SRAM_MAX << 4) + MD32Offset + (nxt_shu_level << 4),  Fld(8, u1FldIdx*8));
            rtmr14 = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_1 + (DRAM_DFS_SRAM_MAX << 4) + MD32Offset + (nxt_shu_level << 4),  Fld(8, u1FldIdx*8));
...
U32 gSRAMBackupIdx[DRAM_DFS_SRAM_MAX][4] = { // @Darren, LP5 don't use DDRPHY_REG_SHU_R0_CA_RXDLY6 !!
...
#if (fcFOR_CHIP_ID == fcMouton) // @Darren, for Mar_gaux New setting for ddrphy shuffle (sync mode)
...
void EnableDramcTrackingByShuffle(DRAMC_CTX_T *p, U8 shu_level, U8 u1OnOff)
{
    if (u1OnOff == ENABLE)
    {
        ...
        if ((shu_level != SRAM_SHU6)     //0.625V @DDR800 tx tacking always disable
#if ENABLE_DDR400_OPEN_LOOP_MODE_OPTION
        || (shu_level != SRAM_SHU7))
#else
        )
#endif
            vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, 0, LPIF_LOW_POWER_CFG_1_TX_TRACKING_DIS);
    ...
    else if (u1OnOff == DISABLE)
    {
        ...
        vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, 0, LPIF_LOW_POWER_CFG_1_DPHY_RXDLY_TRACK_EN);

        mcSHOW_DBG_MSG3(("Disable TX-Tracking\n"));
        vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, 1, LPIF_LOW_POWER_CFG_1_TX_TRACKING_DIS);
    }
}
...
void DramcSaveToShuffleSRAM(DRAMC_CTX_T *p, DRAM_DFS_REG_SHU_T srcRG, DRAM_DFS_SRAM_SHU_T dstRG)
{
    ...
    #if ENABLE_SRAM_DMA_WA
    DPHYSaveToSRAMShuWA(p, vGet_Current_SRAMIdx(p));
    #endif
