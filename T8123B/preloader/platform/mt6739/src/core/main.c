...
//    disable_FBB_SW();
    ...
    if (ret) {
       pal_log_err("load loader_ext_etc fail (%x)\n", ret);
       ASSERT(0);
    }
...
#define ROUNDUP(a,b)    (((a) + ((b)-1)) & ~((b)-1))
    ...
    pal_log_info("%s SSPM Start! (with g_rgu_status 0x%x & %s ddr reserved mode)\n", MOD, g_rgu_status,
...
int bldr_load_loader_ext_dram(void)
{
    ...
    memset(&loader_ext_info, 0, sizeof(loader_ext_info));
    ...
    if (0 != ret) {
        pal_log_err("get boot part fail: %d\n", ret);
    } else {
        pal_log_info("boot_part: %d\n", boot_part);
    }
    ...
    ms = get_timer(ms);
    pal_log_warn("img: %s vfy(%d ms)\n", "loader_ext_dram", ms);
#endif

    if (ret) {
        pal_log_err("load loader_ext_dram fail\n");
    }
...
void bldr_jump64(u32 addr, u32 arg1, u32 arg2)
{
    ...
    #if CFG_LOAD_SLT_AARCH64_KERNEL
	pal_log_err("%s jump to 64 bit SLT kernel!\n", MOD);
	jumparch64_slt();
    #endif
...
if (0 == aarch64_slt_done())
{
    ...
    *(unsigned int*) 0x10200250 = CFG_BOOTA64_MEMADDR; //cpu7
    pal_log_err("%s Aarch64 Kernel SLT , jump to 64 bit kernel, address: 0x%x\n", MOD,jump_addr);
