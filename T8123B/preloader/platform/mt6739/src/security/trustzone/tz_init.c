...
#define DBG_MSG(str, ...) pal_log_debug(str, ##__VA_ARGS__)
#define DBG_INFO(str, ...) pal_log_info(str, ##__VA_ARGS__)
#define DBG_ERR(str, ...) pal_log_err(str, ##__VA_ARGS__)
...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    if (!tee_log_buf_start)
        print("Fail to allocate tee log buffer: 0x%x, 0x%x\n",
