    ...
    u32 tbase_sec_dram_size, u32 tee_smem_size)
{
    ...
    sec_mem_arg.tplay_mem_size = 0;
    sec_mem_arg.tplay_mem_start = 0;
    /* 4KB for t-play table (backward) */
    sec_mem_arg.tplay_table_size = 0;
    sec_mem_arg.tplay_table_start = 0;
    /* 16KB for M4U (backward) */
    sec_mem_arg.m4u_mem_size = SEC_MEM_RESERVED_M4U_SIZE;
    sec_mem_arg.m4u_mem_start = tee_entry + (tee_smem_size - SEC_MEM_RESERVED_M4U_SIZE);
    ...
    sec_mem_arg.tplay_table_size = 0;
    sec_mem_arg.tplay_table_start = 0;
    sec_mem_arg.svp_mem_end = tee_entry + tee_smem_size;
    if(sec_mem_arg.svp_mem_end < sec_mem_arg.svp_mem_start){
    ...
    u64 tbase_sec_dram_size, u64 dram_base, u64 dram_size)
{
    ...
    teearg->ssiq_number           = (32 + 110);                      /* reserve SPI ID 292 for <t-base, which is ID 324 */
