...
#if defined(PMIC_CHIP_MT6357)
	int ret;
	unsigned int val = 0, uret, vdram_vocal, vdram_vosel;
    unsigned int dram_type;
...
void clr_emi_mpu_prot(void)
{
	...
	for (region = 0; region < EMI_MPU_REGION_NUMBER; region++) {
		mt_reg_sync_writel(EMI_MPU_SA(region), 0x0);
		mt_reg_sync_writel(EMI_MPU_EA(region), 0x0);
		mt_reg_sync_writel(EMI_MPU_APC(region, 0), 0x0);
	}
#endif
}
