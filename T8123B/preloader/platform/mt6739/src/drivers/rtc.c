...
static bool rtc_busy_wait(void)
{
	...
	do {												
		while (RTC_Read(RTC_BBPU) & RTC_BBPU_CBUSY)
		{
			...
			{
				pal_log_err("rtc cbusy time out\n");
				return false;
			}
		}
	} while (0);
...
static bool rtc_spar_alarm_clear_wait(void)
{
	...
	while (RTC_Read(RTC_BBPU) & RTC_BBPU_CLR)
	{
		...
		{
			pal_log_err("rtc spar/alarm clear time out\n");
			return false;
		}
	}
...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	while( (FQMTR_BUSY&RTC_Read(RG_FQMTR_BUSY))==FQMTR_BUSY )
	{
		if (begin > 1000)
		{
			pal_log_err("get frequency time out\n");
			break;
		}
		begin++;
		mdelay(1);
	...
	pal_log_debug("[RTC] get_frequency_meter: input=0x%x, ouput=%d\n",val, ret);
...
static bool rtc_frequency_meter_check(void)
{
	...
	{
		pal_log_debug("[RTC] OSC32CON_ANALOG_SETTING = 0x%x\n", OSC32CON_ANALOG_SETTING);
		osc32con = OSC32CON_ANALOG_SETTING;
	...
	if (rtc_eosc_check_clock(result))
	{
		...
		val = eosc_cali();
		pal_log_debug("[RTC] EOSC cali val = 0x%x\n", val);
		//EMB_HW_Mode
		osc32con = OSC32CON_ANALOG_SETTING | RTC_REG_XOSC32_ENB; 
...
{
	U16 val;
	pal_log_debug("[RTC] rtc_init_after_recovery write powerkey\n");
	...
	pal_log_debug("[RTC] rtc_init_after_recovery done\n");
	return true;
}
static bool rtc_recovery_mode_check(void)
{
...
{
	U8 count = 0;
	pal_log_debug("[RTC] rtc_recovery_flow\n");
...
static bool rtc_hw_init(void)
{
	...
	if (RTC_Read(RTC_BBPU) & RTC_BBPU_INIT) {
		pal_log_err("[RTC] hw_init timeout\n");
		return false;
	}
...
static U16 eosc_cali(void)
{
	...
	regval = RTC_Read(RG_FQMTR_CKSEL) & (~(RG_FQMTR_CKSEL_MASK << RG_FQMTR_CKSEL_SHIFT));
	RTC_Write(RG_FQMTR_CKSEL, regval | FQMTR_FIX_CLK_EOSC_32K);//select EOSC_32 as fixed clock
	pal_log_debug("[RTC] EOSC_Cali: RG_FQMTR_CKSEL=0x%x\n", RTC_Read(RG_FQMTR_CKSEL));
	while( left<=(right) )
	{
		middle = (right + left) / 2;
		if(middle == left)
		...
		pal_log_debug("[RTC] EOSC_Cali: val=0x%x\n", val);
...
static void rtc_lpd_state_clr(void)
{
	...
	Write_trigger();
	spar0 = RTC_Read(RTC_SPAR0);
	pal_log_debug("[RTC] RTC_SPAR0=0x%x \n", spar0);
}
...
static bool rtc_lpd_init(void)
{
	...
	pal_log_debug("[RTC] rtc_lpd_init RTC_CON=0x%x\n",RTC_Read(RTC_CON));
...
static bool rtc_first_boot_init(U16 *result)
{
	pal_log_err("rtc_first_boot_init\n");
	...
	if (!Writeif_unlock())
	{
		pal_log_err("Writeif_unlock fail1\n");
		return false;
	}
	...
	if (!Write_trigger())
	{
		pal_log_err("Write_trigger fail1\n");
		return false;
	}
	if (!Writeif_unlock())
	{
		pal_log_err("Writeif_unlock fail2\n");
		return false;
	}
	...
	if (!Write_trigger())
	{
		pal_log_err("Write_trigger fail2\n");
		return false;
	}
	if (!Writeif_unlock())
	{
		pal_log_err("Writeif_unlock fail3\n");
		return false;
	}
...
void rtc_init(void)
{
	...
	{
		pal_log_err("Enter first boot init.\n");
		check_mode_flag = true;
	}
	pal_log_debug("[RTC] rtc_boot_check1 powerkey1 = 0x%x, powerkey2 = 0x%x, %s LPD\n",
		RTC_Read(RTC_POWERKEY1), RTC_Read(RTC_POWERKEY2),(RTC_Read(RTC_CON) & RTC_CON_LPSTA_RAW) ? "with" : "without" );
	pal_log_debug("[RTC] bbpu = 0x%x, con = 0x%x, osc32con = 0x%x, sec = 0x%x, yea = 0x%x\n", RTC_Read(RTC_BBPU), RTC_Read(RTC_CON), RTC_Read(RTC_OSC32CON), RTC_Read(RTC_AL_SEC), RTC_Read(RTC_AL_YEA));
	...
	/* normally HW reload is done in BROM but check again here */
		pal_log_debug("[RTC] rtc_boot_check2 powerkey1 = 0x%x, powerkey2 = 0x%x\n",
		...
		}else
		{
			...
			{
				pal_log_debug("[RTC] rtc_boot_check Writeif_unlock\n");
				if (RTC_Read(RTC_POWERKEY1) != RTC_POWERKEY1_KEY ||
					RTC_Read(RTC_POWERKEY2) != RTC_POWERKEY2_KEY)
				{
					pal_log_debug("[RTC] rtc_boot_check3 powerkey1 = 0x%x, powerkey2 = 0x%x\n",
						RTC_Read(RTC_POWERKEY1), RTC_Read(RTC_POWERKEY2));
					if (!rtc_first_boot_init(result)) {
						rtc_recovery_flow();
					}
	...
	if(rtc_get_xosc_mode())
	{
		...
		pal_log_debug("[RTC] RTC/32k mode\n");
	}
	else
	{
		RTC_Write(RTC_SPAR0, (spar0 & ~RTC_SPAR0_32K_LESS) );
		rtc_set_xomode(true);
		pal_log_debug("[RTC] 32k-less mode\n");
	}
	Write_trigger();
	rtc_save_2sec_stat();
	RTC_Write(RTC_BBPU, RTC_Read(RTC_BBPU) | RTC_BBPU_KEY | RTC_BBPU_RELOAD);
...
bool rtc_boot_check(void)
{
	...
	pal_log_debug("[RTC] irqsta = 0x%x, pdn1 = 0x%x, pdn2 = 0x%x, spar0 = 0x%x, spar1 = 0x%x\n",
		  irqsta, pdn1, pdn2, spar0, spar1);
	pal_log_debug("[RTC] new_spare0 = 0x%x, new_spare1 = 0x%x, new_spare2 = 0x%x, new_spare3 = 0x%x\n",
		  RTC_Read(RTC_AL_HOU), RTC_Read(RTC_AL_DOM), RTC_Read(RTC_AL_DOW), RTC_Read(RTC_AL_MTH));
	pal_log_debug("[RTC] bbpu = 0x%x, con = 0x%x, cali = 0x%x, osc32con = 0x%x\n", RTC_Read(RTC_BBPU), RTC_Read(RTC_CON), RTC_Read(RTC_AL_SEC), RTC_Read(RTC_OSC32CON));
	...
	if (irqsta & RTC_IRQ_STA_AL) {
		...
		if (pdn1 & RTC_PDN1_PWRON_TIME) {	/* power-on time is available */
			...
			pal_log_debug("[RTC] now = %d/%d/%d %d:%d:%d (%u)\n",
				  now_yea, now_mth, now_dom, now_hou, now_min, now_sec, now_time);
			pal_log_debug("[RTC] power-on = %d/%d/%d %d:%d:%d (%u)\n",
...
{
	pal_log_info("do not support fg_set_2sec_reboot()");
}

static bool rtc_2sec_stat_clear(void)
{
	pal_log_debug("[RTC] rtc_2sec_stat_clear\n");
...
void rtc_save_2sec_stat(void)
{
	...
	pal_log_debug("[RTC] rtc_2sec_reboot_check 0x%x, %s 2sec reboot, type 0x%x\n", reboot,
	...
	if (reboot & RTC_BBPU_2SEC_EN) {
		switch((reboot & RTC_BBPU_2SEC_MODE_MSK) >> RTC_BBPU_2SEC_MODE_SHIFT) {
			...
			case 2:
				if(reboot & RTC_BBPU_2SEC_STAT_STA) {
					...
					{
						pal_log_debug("[RTC] rtc IPO shutdown disable auto reboot\n");
		...
		pal_log_err("rtc 2sec reboot not enabled\n");
	}

#if (CFG_PMIC_FULL_RESET)
	if (RTC_Read(RTC_AL_MTH) & 0x100) {
		pal_log_debug("Force SPAR power on\n");
