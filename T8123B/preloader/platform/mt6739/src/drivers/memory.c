...
// platform hard-code define
// 5G10, EMI h/w supports emi mpu latch reiong info
int mem_clear_region[] = {7, 8, 9, 10, 11, 12, 13};
#define MEM_CLEAR_REGION_COUNT (sizeof(mem_clear_region) / sizeof(mem_clear_region[0]))
...
static int do_clear_region(unsigned int start, unsigned int len);
#endif
...
mt_mem_init (void)
{
  ...
  int cmp_err;
#ifdef DDR_RESERVE_MODE
	struct region_to_clear md[MEM_CLEAR_REGION_COUNT];
	int rnum;
#endif
  ...
  if((g_ddr_reserve_enable==1) && (g_ddr_reserve_success==1))
  {
    ...
    pal_log_info("[DDR Reserve mode] EMI dummy read CONA = 0x%x  EMI_CONC_CH0=0x%x\n", emi_cona ,emi_conc_ch0);
    ...
    //DRV_WriteReg32(EMI_MPU_CTRL, DRV_Reg32(EMI_MPU_CTRL)&0xFFFFFFFE);
		...
		if (g_ddr_reserve_ta_err != 0) {
			pal_log_info("[DDR Reserve mode] TA2 ERR = %d -> overwrite tag.ddr_reserve_success to 0\n", g_ddr_reserve_ta_err);
		}

		for (rnum = 0; rnum < MEM_CLEAR_REGION_COUNT; rnum++) {
			md[rnum].region_id = mem_clear_region[rnum];
			// query SA & EA of md.region_id
			md[rnum].check = check_emi_mpu_addr(&md[rnum]);
		}

		/* clear EMI MPU protection */
		clr_emi_mpu_prot();

		/* clear the  of regions */
		for (rnum = 0; rnum < MEM_CLEAR_REGION_COUNT; rnum++) {
			mtk_wdt_restart();// to avoide WDT when clear dram content
			if (md[rnum].check == 0)
				do_clear_region(md[rnum].region_sa, md[rnum].region_ea + 1 - md[rnum].region_sa);
		}
  }
  else /* normal boot */
#endif
...
static int do_clear_region(unsigned int start, unsigned int len)
{
	...
	pal_log_info("[do_clear_region] start: 0x%x len:%d\n", start, len);
	for (i = 0; i < (len >> 2); ++i) {
#if TEST_OVERWRITE
		...
		if (new_val != PATTERN1) {
			pal_log_debug("[DEBUG] error 0x%x ", new_val);
			return -1;
		}
		dsb();
#else
	...
	dsb();
	pal_log_info("[do_clear_region] CLEARED\n");
	return 0;
}
