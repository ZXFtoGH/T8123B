...
void vApplyProgramSequence(DRAMC_CTX_T *p)
{
    ...
    //vendor_id:1, revision_id=0x8
    if (p->vendor_id == 0x1 && (p->revision_id & 0xff)==0x8)
    {
        ProgramModeEnter(p);
        ProgramCodeInput(p, 0, 0, 0);
        ProgramCodeInput(p, 3, 9, 0);
        ProgramCodeInput(p, 8, 0, 0);
        ProgramCodeInput(p, 0, 0xA, 6);
        ProgramCodeInput(p, 8, 6, 0xA);
        ProgramCodeInput(p, 8, 2, 1);
        mcSHOW_DBG_MSG(("TMRS enter -> 000 -> 390 -> 800 -> 0A6 -> 86A -> 821\n"));   
    }
