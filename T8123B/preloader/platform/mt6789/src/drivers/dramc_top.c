...
#if (FOR_DV_SIMULATION_USED==0)
#include <pmic.h>
/* now we can use definition MTK_PMIC_MT6358
 * ==============================================================
 * PMIC   |Power |Dflt. Volt. |Step   |Support FPWM |Cmt.
 * --------------------------------------------------------------
 * MT6366 |Vcore  |0.75v      |6.25mV |Yes          |
 *        |vio18  |1.8v       |0.1V   |No           |
 *        |Vdram  |1.125v     |12.5mV |Yes          |(DRAM Vdsram)
 *        |vmddr  |0.75v      |10mV   |No           |(AP Vdram)
 *        |Vddq   |0.6v       |10mV   |No           |
 * ==============================================================
 */
#define MTK_PMIC_MT6358
#endif
...
void vdram2_set_lv_vol(struct mtk_regulator *mreg, int slt_volt)
{
        int ret = 0, cal_volt = 0, set_cal = 0;
        unsigned int fixed_volt = 0;

        fixed_volt = mtk_regulator_get_voltage(mreg);
        cal_volt = mtk_regulator_get_votrim(mreg);
        set_cal = (slt_volt - fixed_volt);
        dramc_debug("vdram2_set_lv_vol: cal_volt= %d\n", cal_volt);
        dramc_debug("vdram2_set_lv_vol: slt_volt= %d\n", slt_volt);
        dramc_debug("vdram2_set_lv_vol: fixed_volt= %d\n", fixed_volt);

		ret  = mtk_regulator_set_votrim(mreg, set_cal);
        if (ret)
                dramc_debug("vdram2_set_lv_vol: mtk_regulator_set_votrim %s failed\n", mreg->name);
}

unsigned int dramc_set_vmddq_voltage(unsigned int ddr_type, unsigned int vddq)
{
	// unsigned int twist = 0;
	unsigned int efuse_volt,efuse;
	...
	if(vddq<600000){
		vdram2_set_lv_vol(&reg_vddq, vddq);
	} else{
		mtk_regulator_set_voltage(&reg_vddq, vddq,MAX_VDDQ);
	}
#endif
...
unsigned int dramc_get_vmddq_voltage(unsigned int ddr_type)
{
	...
	vol = mtk_regulator_get_voltage(&reg_vddq);
	vot = mtk_regulator_get_votrim(&reg_vddq);
...
unsigned int mt_get_ddr_gpio_trap_value(void)
{
	...
	if (!trap_detected_done) {
		/* bit 15 latch GPIO172  */
		trap_value = (*((volatile unsigned int *)GPIO_TRAPPING_REG) >> 15) & 0x1;
		dramc_debug("%s : %d \n", __func__, trap_value);
...
unsigned int mt_get_dram_type_from_hw_trap(void)
{
	...
	if (!ddr_type_detected_done) {
		unsigned int trap;
		//trap = get_dram_type() & 0x7; /*pmic may not ready when SB,suggest use gpio trapping*/
                trap= mt_get_ddr_gpio_trap_value()& 0x7;
		switch (trap) {
			case 0:
			case 1:
			...
			case 3:
			default:
				dramc_crit("[dramc] Wrong pmic Trapping value is %d.\n",trap);
				DRAMC_ASSERT(0);
				break;
		}
...
#if !__ETT__
	mcSHOW_ERR_MSG(("[DDR Reserve] MTK_DRM_BASE(0x%x) : 0x%x\n", MTK_DRM_BASE, READ_REG(MTK_DRM_BASE)));
	mcSHOW_ERR_MSG(("[DDR Reserve] MTK_DRM_DEBUG_CTL(0x%x) : 0x%x\n", MTK_DRM_DEBUG_CTL, READ_REG(MTK_DRM_DEBUG_CTL)));
#endif
#ifdef DDR_RESERVE_MODE
	int counter = TIMEOUT;
...
void reserve_dramc_dummy_read(void)
{
	...
	for (i = 0; i < (unsigned int)rank_nr; i++) {
		...
		if (dram_addr.full_sys_addr > get_config_max_dram_size())
			break;
#endif
		reserve_start = mblock_alloc_range(0x1000, 0x1000, 0, (u64)(dram_addr.full_sys_addr), 0, 0, reserve_name[i]);
		if (reserve_start != ((u64)(dram_addr.full_sys_addr) - 0x1000)) {
