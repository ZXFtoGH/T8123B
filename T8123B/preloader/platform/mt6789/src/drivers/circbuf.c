...
{
    ASSERT (buf != NULL);
    ASSERT (size <= USBTTY_BUFFER_SIZE);
...
unsigned int buf_pop (circbuf_t * buf, char *dest, unsigned int len)
{
    unsigned int i;
    char *p = buf->top;
    char *end = buf->end;
...
unsigned int buf_push (circbuf_t * buf, const char *src, unsigned int len)
{
    ...
    if (((unsigned long) buf->size + (unsigned long) len) > UINT_MAX)
    {
        buf->size = UINT_MAX;
    }
    else
