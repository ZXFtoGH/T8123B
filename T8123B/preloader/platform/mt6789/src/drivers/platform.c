...
void platform_vusb_on(void)
{
    ...
    if (ret == 0)
        pal_log_info("[%s] VUSB33 is on\n", __func__);
    else
        pal_log_info("[%s] Failed to turn on VUSB33!\n", __func__);
#endif
    return;
}
...
void platform_set_boot_args_by_atag(void *ptr)
{
  ...
  //----------------
...
#if !CFG_FPGA_PLATFORM
    tags->u.emi_info.dram_type = 0x06;//(u8)mt_get_dram_qvl_type();
    ...
    /* DDR INFO */
    tags->hdr.size = boot_tag_size(boot_tag_ddr_info);
    tags->hdr.tag  = BOOT_TAG_DDR_INFO;
    tags->u.ddr_info.mdl_number = get_dram_mdl_number();
    tags = boot_tag_next(tags);
...
int platform_get_mcp_id(u8 *id, u32 len, u32 *fw_id_len)
{
    ...
    if (ret)
        ret = mmc_get_device_id(id, len,fw_id_len);
#else
    ret = ufs_get_device_id(id, len, fw_id_len);  /* only boot ufs */
#endif
#else
    return (u32)nand_init_device();
#endif
...
static boot_reason_t platform_boot_status(void)
{
	...
	if (pmic_reboot == 1) {
		...
		if (val < 3400) {
			...
			//return BR_POWER_LOSS; //modify by zkc
		}
	}
    ...
    pal_log_err("zkc will in usb_accessory_in\n");
...
{
    /* Disable BJT rotation and enable Low pass filter */
    DRV_WriteReg32(0x1000C404, 0x000F0000);
}
...
void platform_pre_init(void)
{
    ...
    /* init dcm */
    //TBD mtk_dcm_init();
    BOOTING_TIME_PROFILING_LOG("DCM");
    /* init pll */
    mt_pll_init();
    ...
    BOOTING_TIME_PROFILING_LOG("SPMI_NO_SRCLKEN_RC");
...
/******************************************************************************
* NOTE: This function is for the internal projects of MT6789.
*        -------------------------------------------
*       | GPIO_111    GPIO_101    GPIO100   PCB_ID  |
*       |   Low         TBD         TBD       EVB   |
*       |   High        TBD         TBD       Phone |
*        -------------------------------------------
******************************************************************************/
hw_ver_t drdi_get_hw_ver(void)
{
    ...
    mt_set_gpio_pull_enable(GPIO111, GPIO_PULL_DISABLE);

    if (mt_get_gpio_in(GPIO111)) {
        pal_log_info("Detect GPIO111 High: EVB\n");
        return HW_VER_V0;
    } else {
        pal_log_info("Detect GPIO111 Low: Phone\n");
        return HW_VER_V1;
    }
