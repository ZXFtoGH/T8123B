...
static int spmi_config_io(int mstid, int driving)
{
	...
	idx = driving/2 - 1;
	DRV_WriteReg32(SPMI_DRV_CFG_SET, io[idx].sd << SPMI_DRV_SPMI_OFFSET);
	DRV_WriteReg32(SPMI_DRV_CFG_CLR, io[idx].cd << SPMI_DRV_SPMI_OFFSET);

	DRV_WriteReg32(SPMI_IES_CFG0_CLR, 0x1 << SPMI_M_SCL_IES_CFG_OFFSET);	//PAD_SPMI_M_SCL_IES=0
	DRV_WriteReg32(SPMI_IES_CFG1_SET, 0x1 << SPMI_M_SDA_IES_CFG_OFFSET);	//PAD_SPMI_M_SDA_IES=1
	DRV_WriteReg32(SPMI_IES_CFG1_CLR, 0x1 << SPMI_P_SCL_IES_CFG_OFFSET);	//PAD_SPMI_P_SCL_IES=0
	DRV_WriteReg32(SPMI_IES_CFG1_SET, 0x1 << SPMI_P_SDA_IES_CFG_OFFSET);	//PAD_SPMI_P_SDA_IES=1

	DRV_WriteReg32(SPMI_PD_CFG_CLR, 0xf << SPMI_PD_CFG_OFFSET);	//PAD_SPMI_M_SCL/SDA PAD_SPMI_P_SCL/SDA PD=0
	DRV_WriteReg32(SPMI_PU_CFG_CLR, 0xf << SPMI_PU_CFG_OFFSET);	//PAD_SPMI_M_SCL/SDA PAD_SPMI_P_SCL/SDA PU=0
	DRV_WriteReg32(SPMI_RDSEL_CFG_CLR, 0x3 << SPMI_RDSEL_CFG_OFFSET);  //PAD_SPMI_M_SCL/SDA PAD_SPMI_P_SCL/SDA RDSEL=0
	DRV_WriteReg32(SPMI_SMT_CFG_SET, 0x1 << SPMI_SMT_CFG_OFFSET);  //PAD_SPMI_M_SCL/SDA PAD_SPMI_P_SCL/SDA SMT=1
	DRV_WriteReg32(SPMI_TDSEL_CFG_CLR, 0xf << SPMI_TDSEL_CFG_OFFSET);  //PAD_SPMI_M_SCL/SDA PAD_SPMI_P_SCL/SDA TDSEL=0

	SPMI_CRI("%s input driving:%d set bit:%d clr bit%d IOCFG_DRV_CFG 0x%x\n", __func__,
		 driving, io[idx].sd, io[idx].cd, DRV_Reg32(SPMI_DRV_CFG));
	return 0;
}
...
static int spmi_config_slave(int mstid, struct spmi_device *dev)
{
	...
	/* add glue module initial setting for MDRF, Gen97 MD only
	 * [8]:write
	 * [7:4]:slvid
	 * [3]:bytecnt
	 * [2]:pmifid
	 * [1:0]:cmd
	 * */
	DRV_WriteReg32(VLPCFG_BASE + 0x24, 0x157);
...
static int spmi_read_check(struct spmi_device *dev)
{
	...
	if ((rdata & dev->hwcid_mask) != (dev->hwcid_val & dev->hwcid_mask)) {
		...
		SPMI_CRI("spmi_read_check done, slvid:%d rdata = 0x%x\n",
			dev->slvid, rdata);
	...
	for (i = 0; i < ARRAY_SIZE(wdata); i++) {
		...
		if (dev->mstid == SPMI_MASTER_P_1) {
			if (rdata != wdata_1[i]) {
				...
				SPMI_ERR("%s pass slvid:%d rdata = 0x%x count = %d\n",
				__func__, dev->slvid, rdata, j);
			}
		} else {
			if (rdata != wdata[i]) {
...
static int spmi_cali_config_io(int dri)
{
	int idx = 0;
	struct io_driving io[] = {
					{0x0, 0x7},/* 2ma */
...
{
	unsigned int i = 0, j = 0, cali = 0, dri = 0, start = 0, end = spmi_device_cnt;
	unsigned int spmi_p_start = get_spmi_p_device_cnt();
		...
		for (i = 0;i < 6; i++) {
			for ( j = start; j < end; j++) {
				...
				if (spmi_read_check(&spmi_dev[j]) == 0) {
					SPMI_CRI("[Pass]non-rcs dly:%d, pol:%d, sampl:0x%x\n",
						cali_data[i].dly, cali_data[i].pol,
						spmi_readl(spmi_dev[j].mstid, SPMI_MST_SAMPL));
				} else {
					SPMI_CRI("[Fail]non-rcs dly:%d, pol:%d, sampl:0x%x SPMI_DEC_DBG:0x%x, SPMI_MST_DBG:0x%x\n",
						cali_data[i].dly, cali_data[i].pol,
						spmi_readl(spmi_dev[j].mstid, SPMI_MST_SAMPL),
				...
				spmi_writel(spmi_dev[j].mstid, SPMI_MST_SAMPL,
					cali | ((cali_data[i].dly + 1) << 4));
				if (spmi_read_check(&spmi_dev[j]) == 0) {
					SPMI_CRI("[Pass]rcs_en dly:%d, pol:%d, sampl:0x%x\n",
						cali_data[i].dly, cali_data[i].pol,
						spmi_readl(spmi_dev[j].mstid, SPMI_MST_SAMPL));
				} else {
					SPMI_CRI("[Fail]rcs_en dly:%d, pol:%d, sampl:0x%x SPMI_DEC_DBG:0x%x SPMI_MST_DBG:0x%x\n",
						cali_data[i].dly, cali_data[i].pol,
						spmi_readl(spmi_dev[j].mstid, SPMI_MST_SAMPL),
						spmi_readl(spmi_dev[j].mstid, SPMI_DEC_DBG),
						spmi_readl(spmi_dev[j].mstid, SPMI_MST_DBG));
	...
	SPMI_CRI("[CALI FAIL] IO driving:%d\n", dri);
	return -ENODEV;
}
