...
static struct mt6358_ldo_info ldo_ext_info[] = {
	...
	mt6358_ldo_decl(vefuse_volts, vefuse_idxs),
#if USE_PMIC_MT6366
	mt6358_ldo_decl(vm18_volts, vm18_idxs),
	mt6358_ldo_decl(vmddr_volts, vmddr_idxs),
#endif
};
#endif /*--LDO_SUPPORT--*/
...
static struct mt6358_ldo_trim_info ldo_trim_info[] = {
	...
	mt6358_ldo_trim_decl(vefuse, votrim_2_type),
#if USE_PMIC_MT6366
	mt6358_ldo_trim_decl(vm18, votrim_2_type),
	mt6358_ldo_trim_decl(vmddr, votrim_2_type),
#endif
};
...
static int mt6358_get_voltage(unsigned char id)
{
	...
	if (mt6358_regu_info[id].rtype != REGULAR_VOLTAGE) {
		ret = pmic_read_interface(mt6358_regu_info[id].vocal_reg,
					  &offset,
					  mt6358_regu_info[id].vocal_mask,
					  mt6358_regu_info[id].vocal_shift);
		volt += offset * 10000;
	}
#else
	else
...
static int mt6358_get_votrim(unsigned char id)
{
	...
	ret = pmic_read_interface(
		mt6358_regu_info[id].triminfo->trim_reg,
		&selector,
		mt6358_regu_info[id].triminfo->trim_mask,
		mt6358_regu_info[id].triminfo->trim_shift);
	if (ret)
		return -1;
	...
	for (i = 0; i <= n_size; i++) {
		if (transtype == VOLTOSEL) {
			if (cnvdata >= pVoltage[i] && cnvdata < pVoltage[i] + 100000) {
				...
				/* Unlock for writting protect register */
				pmic_config_interface(PMIC_TMA_KEY_ADDR, 0x9CA7, PMIC_TMA_KEY_MASK, PMIC_TMA_KEY_SHIFT);
				pmic_config_interface(mt6358_regu_info[id].vocal_reg,
						      offset,
						      mt6358_regu_info[id].vocal_mask,
						      mt6358_regu_info[id].vocal_shift);
...
static struct mtk_regulator mt6358_regulator[] = {
	...
	{
		/* vsram_proc11/vsram_proc12 swap in MT6789 */
		.name = "vsram_proc12",
	...
	{
		/* vproc11/vproc12 swap in MT6789 */
		.name = "vproc12",
