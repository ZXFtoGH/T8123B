...
#if ((fcFOR_CHIP_ID == fcPalmer) && (FOR_DV_SIMULATION_USED == 0 && SW_CHANGE_FOR_SIMULATION == 0))
...
{
    p->support_channel_num = GetChannelInfoToConf(p);
}
...
void TA2_Show_Cnt(DRAMC_CTX_T * p, U32 u4ErrorValue)
{
    ...
    for (u1RankIdx = 0; u1RankIdx < p->support_rank_num; u1RankIdx++)
    {
        ...
        {
            err_count++;
            mcSHOW_ERR_MSG(("HW channel(%d) Rank(%d), TA2 failed, pass_cnt:%d, err_cnt:%d, err_bit:0x%x\n", p->channel, u1RankIdx, pass_count, err_count, u4IO32Read4B(DRAMC_REG_ADDR(DRAMC_REG_CMP_ERR))));
        }
        else
        {
            pass_count++;
            mcSHOW_ERR_MSG(("HW channel(%d) Rank(%d), TA2 pass, pass_cnt:%d, err_cnt:%d\n", p->channel, u1RankIdx, pass_count, err_count));
        }
    }
}
