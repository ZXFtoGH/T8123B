...
inline void spmi_dump_pmif_busy_reg(int mstid)
{
...
inline void spmi_dump_pmif_swinf_reg(int mstid)
{
	struct pmif *arb = get_pmif_controller(PMIF_SPMI, mstid);
...
inline void spmi_dump_pmif_reg(int mstid)
{
	struct pmif *arb = get_pmif_controller(PMIF_SPMI, mstid);
	unsigned int i = 0, offset = 0, tmp_dat = 0;
	unsigned int start = 0, end = 0;
...
inline void spmi_dump_pmif_record_reg(int mstid)
{
	struct pmif *arb = get_pmif_controller(PMIF_SPMI, mstid);
	unsigned int i = 0, offset = 0, j = 0, tmp_dat = 0;
...
#endif
inline void spmi_dump_spmimst_reg(int mstid)
{
	struct pmif *arb = get_pmif_controller(PMIF_SPMI, mstid);
	unsigned int i = 0, offset = 0, tmp_dat = 0;
	unsigned int start = 0, end = 0;
...
inline void spmi_dump_mst_record_reg(int mstid)
{
	struct pmif *arb = get_pmif_controller(PMIF_SPMI, mstid);
...
inline void spmi_dump_slv_record_reg(struct spmi_device *dev)
{
	...
	for (offset = 0x34; offset < 0x50; offset += 4)
	{
		spmi_ext_register_readl(dev,
			(MT6319_PLT0_ID_ANA_ID + offset), &rdata1, 1);
		spmi_ext_register_readl(dev,
			(MT6319_PLT0_ID_ANA_ID + offset + 1), &rdata2, 1);
		spmi_ext_register_readl(dev,
			(MT6319_PLT0_ID_ANA_ID + offset + 2), &rdata3, 1);
		spmi_ext_register_readl(dev,
			(MT6319_PLT0_ID_ANA_ID + offset + 3), &rdata4, 1);
		if ((offset + 3) == 0x37) {
			i = (rdata4 & 0xc) >> 2;
			if (i == 0)
...
/* CH5(TIA HW2), CH6(TIA HW1), and CH16(STAUPD HW) are not monitored */
static unsigned int is_hw_monitor_mode = 0, chan = 0x7eff9f;
static unsigned int target_write = 0xffff, target_en = 0x3fc;
static struct pmif_hw_mon_match_table pmif_match_table[8] = {
	0xffff, 0x03a3, 0xffff, 0x11,
	0xffff, 0x03a4, 0xffff, 0x22,
	0xffff, 0x03a6, 0x0001, 0x1,
	0xffff, 0x03a6, 0x000f, 0xa,
	0x00ff, 0x001a, 0xffff, 0x33,
	0xff00, 0x1b00, 0xffff, 0x44,
	0xffff, 0x001c, 0x0003, 0x1,
	0xffff, 0x001d, 0x00f0, 0x50,
};
#if MTK_DOE_CONFIG_ENV_SUPPORT
#define DOE_UPDATE_ADDR_AND_MASK(_mask, _addr, idx)	\
{	\
	doe_addr_mask = dconfig_getenv(#_mask);	\
	pmif_match_table[idx].addr_mask = atoi(doe_addr_mask);\
	doe_addr = dconfig_getenv(#_addr);	\
	pmif_match_table[idx].addr = atoi(doe_addr);\
	pal_log_info("mask: %x addr:%x\n", pmif_match_table[idx].addr_mask, \
			pmif_match_table[idx].addr);\
}
#define DOE_UPDATE_DATA_AND_MASK(_mask, _data, idx)	\
{	\
	doe_data_mask = dconfig_getenv(#_mask);	\
	pmif_match_table[idx].data_mask = atoi(doe_data_mask);\
	doe_data = dconfig_getenv(#_data);	\
	pmif_match_table[idx].data = atoi(doe_data);\
	pal_log_info("mask: %x data:%x\n", pmif_match_table[idx].data_mask, \
			pmif_match_table[idx].data);\
}
#ifndef DOE_UPDATE
...
{	\
	...
	pal_log_info(#_item " %x\n", _item);\
}
#endif

void spmi_pmif_doe_load(void)
{
	char *doe_is_hw_monitor_mode;
	char *doe_chan, *doe_target_write;
	char *doe_addr_mask, *doe_addr;
	char *doe_data_mask, *doe_data;
	char *doe_target_en;

	DOE_UPDATE(is_hw_monitor_mode);
	DOE_UPDATE(chan);
	DOE_UPDATE(target_write);
	DOE_UPDATE(target_en);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask0, addr0, 0);
	DOE_UPDATE_DATA_AND_MASK(data_mask0, data0, 0);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask1, addr1, 1);
	DOE_UPDATE_DATA_AND_MASK(data_mask1, data1, 1);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask2, addr2, 2);
	DOE_UPDATE_DATA_AND_MASK(data_mask2, data2, 2);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask3, addr3, 3);
	DOE_UPDATE_DATA_AND_MASK(data_mask3, data3, 3);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask4, addr4, 4);
	DOE_UPDATE_DATA_AND_MASK(data_mask4, data4, 4);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask5, addr5, 5);
	DOE_UPDATE_DATA_AND_MASK(data_mask5, data5, 5);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask6, addr6, 6);
	DOE_UPDATE_DATA_AND_MASK(data_mask6, data6, 6);

	DOE_UPDATE_ADDR_AND_MASK(addr_mask7, addr7, 7);
	DOE_UPDATE_DATA_AND_MASK(data_mask7, data7, 7);
}
#endif
int spmi_pmif_dbg_init(struct pmif *arb)
{
	unsigned int i = 0, offset = 0, int_en = 0;

	arb->dbgregs = mt6xxx_pmif_dbg_regs;

	if (is_hw_monitor_mode == 1) {
		/* enable matching mode */
		PMIF_CRI("PMIF Matching Mode\n");

		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_CLR_0],
				0xffffffff);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_CLR_1],
				0xffffffff);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_CLR_2],
				0xffffffff);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_CLR_3],
				0xffffffff);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_CLR_4],
				0xffffffff);

		int_en = DRV_Reg32(arb->base + arb->dbgregs[PMIF_IRQ_EVENT_EN_3]);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_EVENT_EN_3],
				int_en | (0x1 << 7));
		for (i = 0 ; i < ARRAY_SIZE(pmif_match_table); i++) {
			/* set monitor channel, should same as ARB_EN */
			offset = arb->dbgregs[PMIF_MONITOR_TARGET_CHAN_0] + (i * 4);
			DRV_WriteReg32(arb->base + offset, chan);

			/* [31:16] addr mask, ffff mean all bit check
			 * [15:0] addr, which addr been checked
			 */
			offset = arb->dbgregs[PMIF_MONITOR_TARGET_ADDR_0] + (i * 4);
			DRV_WriteReg32(arb->base + offset,
				((pmif_match_table[i].addr_mask << 16) & 0xffff0000) |
				(pmif_match_table[i].addr & 0xffff));
			/* [31:16] wdata mask, ffff mean all bit check
			 * [15:0] data, which data been checked
			 */
			offset = arb->dbgregs[PMIF_MONITOR_TARGET_WDATA_0] + (i * 4);
			DRV_WriteReg32(arb->base + offset,
				((pmif_match_table[i].data_mask << 16) & 0xffff0000) |
				(pmif_match_table[i].data & 0xffff));
		}
		...
		*/
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_MONITOR_TARGET_WRITE],
				target_write);
		PMIF_CRI("PMIF_MONITOR_TARGET_WRITE:%x\n",
			DRV_Reg32(arb->base + arb->dbgregs[PMIF_MONITOR_TARGET_WRITE]));
		/* reset then enable */
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_MONITOR_CTRL],
				0x800);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_MONITOR_CTRL],
				(target_en << 2) | 0x1);
		PMIF_CRI("PMIF_MONITOR_CTRL:%x\n",
			DRV_Reg32(arb->base + arb->dbgregs[PMIF_MONITOR_CTRL]));
	} else {
		/* init pmif debug mechanism, sw logging mode */
		offset = arb->dbgregs[PMIF_MONITOR_TARGET_CHAN_0];
		DRV_WriteReg32(arb->base + offset, chan);
		offset = arb->dbgregs[PMIF_MONITOR_TARGET_ADDR_0];
		DRV_WriteReg32(arb->base + offset, 0x0);
		offset = arb->dbgregs[PMIF_MONITOR_TARGET_WDATA_0];
		DRV_WriteReg32(arb->base + offset, 0x0);
		offset = arb->dbgregs[PMIF_MONITOR_TARGET_WRITE];
		DRV_WriteReg32(arb->base + offset, 0x1);
		offset = arb->dbgregs[PMIF_MONITOR_CTRL];
		DRV_WriteReg32(arb->base + offset, 0x800);
		DRV_WriteReg32(arb->base + offset, 0x5);
	}
