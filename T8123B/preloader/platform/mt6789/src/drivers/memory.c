...
#if MEM_TEST
#define BIT32_MAX_RANGE_4GB 				0xFFFFFFFF
#define RESERVE_SIZE_FOR_COMPLEX_MEM_TEST 	28	//complex_mem_test will access 28bit after memery (start+len)
...
void mt_mem_init(void)
{
	...
	if (check_last_dram_fatal_exception() != 0 || check_gating_err_in_dramc_latch() != 0) {
		...
		ret = set_err_flag_to_storage();
		print("[save cal log to storage] set_err_flag_to_storage done (ret=%d)\n", ret);
#endif
#endif
	}
#endif

#if 0// Disable for BRINGUP
	...
	for (region = 0; region < EMI_MPU_REGION_NUM; region++) {
		...
		DRV_WriteReg32(SUB_EMI_MPU_SA(region), 0x0);
		DRV_WriteReg32(SUB_EMI_MPU_EA(region), 0x0);
		for (dgroup = EMI_MPU_DGROUP_NUM - 1; dgroup >= 0; dgroup--)
			DRV_WriteReg32(SUB_EMI_MPU_APC(region, dgroup), 0x0);
#endif
	}
#endif
...
#ifdef DDR_RESERVE_MODE
	if(g_ddr_reserve_enable==1)
		ddr_reserve_emi_init();
	...
	doe_feature = dconfig_getenv("full_range_dramtest");
	if (doe_feature)
		is_fullrange = atoi(doe_feature);

	if (is_fullrange) {
		print("full_range_dramtest=%d\n", is_fullrange);
		// disable watchdog avoid timeout when doing mem test
		mtk_wdt_disable();
		print("==================== full_range_dramtest start ============================\n");
		full_range_dramtest(&rk0_err, &rk1_err);
		print("==================== full_range_dramtest end ============================\n");
		mtk_wdt_init();
	}
#endif
...
#endif
	/* EMI settings which should be after DRAM calibration is complete */
	emi_init3();
}
...
#if MTK_DOE_CONFIG_ENV_SUPPORT
#define MAXIMUM_MEM_SIZE_3GB 0xC0000000
typedef struct {
	int result;// 0:success, others:fail
	unsigned int addr;//address of test failure
} MEM_TEST_RESULT;

void GetPhyPllFrequency_mem(DRAMC_CTX_T *p)
{
	...
	print("[F] DataRate=%d at SHU%d\n", u4DataRate, u1ShuLevel);
}

void do_dvfs(unsigned int shuff_idx) {
	...
	vSetDFSFreqSelByTable(p, get_FreqTbl_by_SRAMIndex(p, shuff_idx));
	...
	GetPhyPllFrequency_mem(p);
}

MEM_TEST_RESULT full_range_complex_mem_test(unsigned int test_base, unsigned int remain_length) {
	extern DRAMC_CTX_T DramCtx_LPDDR4;
	DRAMC_CTX_T *p = &DramCtx_LPDDR4;
	MEM_TEST_RESULT test_info = {0};
	unsigned int loop_count = 0;
	unsigned int test_length = 0x20000;//128K
	unsigned int test_count = 0;
	unsigned int dfs_max_cnt = sizeof(gFreqTbl)/sizeof(DRAM_DFS_FREQUENCY_TABLE_T);
	unsigned int shuff_idx = 0;
	
	if (remain_length >= MAXIMUM_MEM_SIZE_3GB || remain_length < test_length) {
		print("[%s] error:The value of remain_length exceeds the allowed range!\n", __func__);
		ASSERT(0);
	}
	test_count = remain_length / test_length;
	if (remain_length % test_length) {
		test_count++;
	}
	
	for (loop_count = 0; loop_count < test_count; loop_count++) {
		print("[%s] loop_count:%d, test_count:%d, test_base:0x%x, remain_length:0x%x\n", __func__, loop_count, test_count, test_base, remain_length);
		
		//set dvfs
		do_dvfs(shuff_idx);
		shuff_idx++;
		if (shuff_idx == dfs_max_cnt)
			shuff_idx = 0;
	
		//mem test
		test_info.result = complex_mem_test(test_base, test_length);
		if (test_info.result == 0) {
			print("[%s] pass (start addr:0x%x)\n", __func__, test_base);
		} else {
			print("[%s] fail :%d (start addr:0x%x)\n\n", __func__, test_info.result, test_base);
			test_info.addr = test_base;
			return test_info;
		}
		
		//next mem test address
		test_base += test_length;
		remain_length -= test_length;
		print("+++++++++++++++++++++++ [%s]The next test_base:0x%x\n\n", __func__, test_base);
		
		if (remain_length <= RESERVE_SIZE_FOR_COMPLEX_MEM_TEST || test_base >= BIT32_MAX_RANGE_4GB - RESERVE_SIZE_FOR_COMPLEX_MEM_TEST) {
			loop_count++;
			print("Mem test size exceeds maximum limit.Stop now!\n\n");
			break;
		}
		if (remain_length < test_length + RESERVE_SIZE_FOR_COMPLEX_MEM_TEST || test_base > BIT32_MAX_RANGE_4GB - RESERVE_SIZE_FOR_COMPLEX_MEM_TEST - test_length) {
			test_length = BIT32_MAX_RANGE_4GB - test_base - RESERVE_SIZE_FOR_COMPLEX_MEM_TEST;
			print("Last test!Remain length is less than test_length.Fix to:0x%x\n", test_length);
		}
	}

	if (loop_count % 2) {
		// keep dvfs count as multiple of two
		// PHYPLL - CLRPLL
		do_dvfs(1);
		vSetVcoreByFreq(p);
	}
...
int full_range_dramtest(unsigned int *rk0_err, unsigned int *rk1_err) {
	MEM_TEST_RESULT test_info = {0};
	u64 rank_size[DRAMC_MAX_RK] = {0};
	u64 total_remain_length = 0;
	
	get_dram_rank_size(rank_size);
	total_remain_length = rank_size[0] + rank_size[1];
	if (total_remain_length >= MAXIMUM_MEM_SIZE_3GB){
		total_remain_length = MAXIMUM_MEM_SIZE_3GB - 1;
	}
	
	test_info = full_range_complex_mem_test(DDR_BASE, total_remain_length);
	if (test_info.result) {
#ifdef LAST_DRAMC
			if ((test_info.addr - DDR_BASE) < rank_size[0])
				*rk0_err = 1;
			else
				*rk1_err = 1;
#else
...
#endif // end of MTK_DOE_CONFIG_ENV_SUPPORT
#endif