...
void LP_Mode_DRM_Setting(void)
{
    ...
    if(fgOpenLoop == 0x1)
    {
        ...
        mcDELAY_US(1);
        vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CG_CTRL0, 0, MISC_CG_CTRL0_RESERVED_MISC_CG_CTRL0_BIT7);//DDRPHY_AO_BASE_ADDRESS + 0x04EC[7]
    ...
    if(fgOpenLoop == 0x1)
    {
        ...
        vIO32WriteFldMulti_All(DDRPHY_REG_MISC_CG_CTRL0, 
                    P_Fld(1, MISC_CG_CTRL0_RG_FREERUN_MCK_CG) |
                    P_Fld(1, MISC_CG_CTRL0_RESERVED_MISC_CG_CTRL0_BIT7));
    }
}
...
void Dramc_DDR_Reserved_Mode_setting(void)
{
    ...
    mcSHOW_ERR_MSG(("@@@ conf_shuffle_level[%d] sram shuffle level[%d]\n", u4ConfShuLevel, u4SramShuLevel));
#if EXIT_DRM_WITH_SAME_FREQ 
    mcSHOW_ERR_MSG(("@@@ Out DRM with the same freq.\n"));
...
#endif
    mcSHOW_ERR_MSG(("@@@ Out DRM with DDR[%d]\n", p->frequency << 1));
    vSetChannelNumber(p);
    vSetRankNumber(p);
...
//    mcSHOW_DRM_MSG(("### dram type =%x \n", u4DramType));
    ...
    {
        vIO32WriteFldAlign_All(DDRPHY_REG_CA_RXDVS0, 0x0, CA_RXDVS0_R_RX_DLY_TRACK_ENA_CA);
    }
    else
    {
    ...
    if(p->DRAMPinmux == PINMUX_DSC)
    {
        vIO32WriteFldAlign_All(DDRPHY_REG_CA_RXDVS0, 1, CA_RXDVS0_R_RX_DLY_TRACK_CLR_CA);
    }
    else
    {
    ...
    if(p->DRAMPinmux == PINMUX_DSC)
    {
        vIO32WriteFldAlign_All(DDRPHY_REG_RK_CA_RXDVS2, 0x0, RK_CA_RXDVS2_R_RK0_DVS_MODE_CA);
        vIO32WriteFldAlign_All(DDRPHY_AO_RANK_OFFSET + DDRPHY_REG_RK_CA_RXDVS2, 0x0, RK_CA_RXDVS2_R_RK0_DVS_MODE_CA);
    }
    else
    {
...
//          (u4IO32ReadFldAlign(DDRPHY_REG_MISC_DMA_DEBUG0 + SHIFT_TO_CHD_ADDR, MISC_DMA_DEBUG0_SRAM_DONE) != 1));
    ...
    if(u4ConfShuLevel)//from CLRPLL
    {
        ...
        mcSHOW_DRM_MSG(("DRM_SHU_SRAM_WA \n"));
#endif        
    ...
    if(p->DRAMPinmux == PINMUX_DSC)
    {
        vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD11, 0x0, SHU_CA_CMD11_RG_RX_ARCA_DVS_EN_CA);
    }
    else
    {
...
/*  mcSHOW_DRM_MSG(("Pipe line dcm off\n"));
    ...
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B0_DQ1, 0x0, SHU_B0_DQ1_RG_ARPI_MIDPI_EN_B0);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B1_DQ1, 0x0, SHU_B1_DQ1_RG_ARPI_MIDPI_EN_B1);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD1, 0x0, SHU_CA_CMD1_RG_ARPI_MIDPI_EN_CA);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B0_DQ1, 0x0, SHU_B0_DQ1_RG_ARPI_MIDPI_CKDIV4_EN_B0);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B1_DQ1, 0x0, SHU_B1_DQ1_RG_ARPI_MIDPI_CKDIV4_EN_B1);
    ...
    if (p->frequency <= 933) //DDR1866; open loop mode MIDPI_EN_*=0, MIDPI_CKDIV4_EN_*=0; set 1 only cost power since all DLL enabled
    {
        vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B0_DQ1, 0x1, SHU_B0_DQ1_RG_ARPI_MIDPI_CKDIV4_EN_B0);
        vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B1_DQ1, 0x1, SHU_B1_DQ1_RG_ARPI_MIDPI_CKDIV4_EN_B1);
        vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD1, 0x1, SHU_CA_CMD1_RG_ARPI_MIDPI_CKDIV4_EN_CA);
    }
    else
    {
...
//    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CG_CTRL0, 0, MISC_CG_CTRL0_W_CHG_MEM);//disable memory clock change
    ...
    if(p->DRAMPinmux == PINMUX_DSC)
    {
        vIO32WriteFldAlign(DDRPHY_REG_SHU_B1_DLL1, 0x1, SHU_B1_DLL1_RG_ARDLL_PHDET_EN_B1);
#if (CHANNEL_NUM > 2)
        vIO32WriteFldAlign(DDRPHY_REG_SHU_B1_DLL1 + SHIFT_TO_CHC_ADDR, 0x1, SHU_B1_DLL1_RG_ARDLL_PHDET_EN_B1);
#endif

        mcSHOW_DRM_MSG(("DELAY 1 us\n"));
        mcDELAY_US(1);

        vIO32WriteFldAlign(DDRPHY_REG_SHU_B1_DLL1 + SHIFT_TO_CHB_ADDR, 0x1, SHU_B1_DLL1_RG_ARDLL_PHDET_EN_B1);
#if (CHANNEL_NUM > 2)
        vIO32WriteFldAlign(DDRPHY_REG_SHU_B1_DLL1 + SHIFT_TO_CHD_ADDR, 0x1, SHU_B1_DLL1_RG_ARDLL_PHDET_EN_B1);
#endif
...
BOOL IsDRAMCSREFState(DRAMC_CTX_T *p)
{
    ...
    {
        vSetPHY2ChannelMapping(p, u1channel);
        isSREF |= u4IO32ReadFldAlign(DRAMC_REG_MISC_STATUSA,MISC_STATUSA_SREF_STATE);
    }
    vSetPHY2ChannelMapping(p, bkChannel);
    return isSREF;
}
...
void ETT_DRM_Main(DRAMC_CTX_T *p)
{
   ...
   // DDRPhyFreqMeter(p);
    ...
    if(IsDRAMCSREFState(p))
    {
        mcSHOW_DBG_MSG(("SREF State \n"));
    }
    else
    {
        for(i = 0; i < 10; i++)
