...
#ifdef DRAM_SLT
#define SOCKET_MARGIN_DISGRADE 5 //POP (have PCB inside, golden socket), use 9, side by side (black socket), use 5
#else
#define SOCKET_MARGIN_DISGRADE 0
#endif
...
#endif
{
        ...
        for (u1ChannelIdx = CHANNEL_A; u1ChannelIdx < p->support_channel_num; u1ChannelIdx++)
        {
            ...
            {
                if (((min_ca_value[u1ChannelIdx][u1RankIdx] * 100 + ((u1IsLP4Family(p->dram_type))? 63: 31)) / ((u1IsLP4Family(p->dram_type))? 64: 32)) < 30)
                {
...
else
{
        ...
        for (u1ChannelIdx = CHANNEL_A; u1ChannelIdx < p->support_channel_num; u1ChannelIdx++)
        {
            for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
            {
                if (((min_rx_value[u1ChannelIdx][u1RankIdx] * gHQALOG_RX_delay_cell_ps_075V * DDRPhyGetRealFreq(p) * 2) + (1000000 - 1)) / 1000000 < (40-SOCKET_MARGIN_DISGRADE) )
                {
                    mcSHOW_ERR_MSG(("[dramc] DRAM_FATAL_ERR_FLAG = 0x00000001, line: %d\n",__LINE__));
                    ASSERT(0);
                }
...
#endif
{
        ...
        for (u1ChannelIdx = CHANNEL_A; u1ChannelIdx < p->support_channel_num; u1ChannelIdx++)
        {
            for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
            {
                if ((min_tx_value[u1ChannelIdx][u1RankIdx] * 100 + (vGet_DDR_Loop_Mode(p) == DDR800_CLOSE_LOOP? 63: 31)) / (vGet_DDR_Loop_Mode(p) == DDR800_CLOSE_LOOP? 64: 32) < (45-SOCKET_MARGIN_DISGRADE) )
                {
                    mcSHOW_ERR_MSG(("[dramc] DRAM_FATAL_ERR_FLAG = 0x00000001, line: %d\n",__LINE__));
                    ASSERT(0);
                }
...
for (u1ByteIdx = u1ByteIdx_Begin; u1ByteIdx <= u1ByteIdx_End; u1ByteIdx++)
{
    ...
    {   U8 space_num;
        ...
        else// if (print_type==2)
        {
            ...
            u2DQDelayEnd = u2DQDelayBegin + 96;
...
void vApplyProgramSequence(DRAMC_CTX_T *p)
{
    ...
    while(1)
    {
        ...
        {
            if((is_need_update_tmrs_for_samsung() == 0) || (((gu2MR6_Value[0] & 0xF) == 0x8) && is_discrete_lpddr4()))
            apply_cond = TRUE;
        }
        if( apply_cond == TRUE)
        {
            ...
            mcSHOW_DBG_MSG3(("TMRS 2386 2387: TMRS enter -> 000 -> 390 -> 120 -> 386 -> 387\n"));
            apply_cond = FALSE;
        }
#endif
...
{
    U8 final_shu;
    S8 u1ShuIdx=0, Test_Start_Idx=SRAM_SHU0, Test_End_Idx=(DRAM_DFS_SRAM_MAX - 1);
    ...
    int err=0;
    U32 test_voltage_type_i=0, test_voltage_type_i_start;
    int vcore_step=6250;
    ...
    {
        Test_Start_Idx = ett_fix_freq;
        Test_End_Idx = ett_fix_freq;
    }
#if DUAL_FREQ_K
    for (u1ShuIdx = Test_Start_Idx; u1ShuIdx <= Test_End_Idx; u1ShuIdx++)
#endif
    {
        if (ett_fix_freq == 0xff)
        {
            ...
            DramcDFSDirectJump_SPMMode(p, u1ShuIdx); //p->pDFSTable->SRAMIdx);
        ...
        test_voltage_type_i_start = 0;
        test_voltage_type_i_max = 6;
        ...
        //VMDDR 0.75V ~ 0.64V
        //VDD2 1.1V~0.99V
        for(test_voltage_type_i = test_voltage_type_i_start; test_voltage_type_i < test_voltage_type_i_max; test_voltage_type_i++)
        {
            switch(test_voltage_type_i)
            {
                case 0:
                    //VCORE
                    VCORE_SHMOO_START_VCORE = dramc_get_vcore_voltage();
                    VCORE_SHMOO_END_VCORE=(VCORE_SHMOO_START_VCORE*8)/10;
                    break;
                case 1:
                    //VDDQ
                    VCORE_SHMOO_START_VCORE = dramc_get_vmddq_voltage();
                    VCORE_SHMOO_END_VCORE = 200000;
                    vcore_step = 10000;
                    break;
                case 2:
                    break;
                case 3:
                    //VDD2
                    VCORE_SHMOO_START_VCORE = 1200000; //dramc_get_vdd2_voltage() return -1, so use 12000000
            ...
            if ((gHQAVcoreShmooTest_flag & (1<<test_voltage_type_i)) != 0)
            for(vcore_i = VCORE_SHMOO_START_VCORE; vcore_i>=VCORE_SHMOO_END_VCORE; vcore_i -= vcore_step)
            {
                ...
                switch(test_voltage_type_i)
                {
                    ...
                    case 1:
                        //VDDQ
                        dramc_set_vmddq_voltage(vcore_i);
                        mcDELAY_US(100);
                        read_vcore_value = dramc_get_vmddq_voltage();
                        break;
                    case 2:
                        break;
                    case 3:
