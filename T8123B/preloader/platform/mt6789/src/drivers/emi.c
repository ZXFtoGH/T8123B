...
static void emi_cen_config(unsigned cfg_base_addr) {
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000000,0x03530154); // For bring-up, will be overwritten
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000028,0x00421000); // For bring-up, will be overwritten
   mt_emi_sync_write(EMI_APB_BASE+0x00000038,0x88880033); // For bring-up, will be overwritten
   mt_emi_sync_write(EMI_APB_BASE+0x0000003c,0x00073200);

   mt_emi_sync_write(EMI_APB_BASE+0x00000040,0x00008802);
   mt_emi_sync_write(EMI_APB_BASE+0x00000048,0x00000000);
   mt_emi_sync_write(EMI_APB_BASE+0x00000060,0x007812ff); // reserved buffer for normal read/write: 8/7
   mt_emi_sync_write(EMI_APB_BASE+0x00000068,0x00200000); // [21]: cen_bsec_act = 0x1
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x0000083c,0x465a78a0);
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000d04,0x00000009); //MDR shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d0c,0x00000000); //MDR shf1 event select
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000d1c,0x00000028); //MDW shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d24,0x00000000); //MDW shf1 event select
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000d34,0x00000080); //APR shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d3c,0x00000000); //APR shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d44,0x30201008); //APR shf0/shf1
   mt_emi_sync_write(EMI_APB_BASE+0x00000d48,0x00000800); //APW shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d50,0x00000000); //APW shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d58,0x00008000); //MMR shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d60,0x00020000); //MMR shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d64,0x00001000); //MMR shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d68,0x00010000); //MMR shf2 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d6c,0x00000800); //MMR shf2 event select
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000d78,0x00040000); //MMW shf0 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d80,0x00100000); //MMW shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d84,0x00004000); //MMW shf1 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d88,0x00080000); //MMW shf2 event select
   mt_emi_sync_write(EMI_APB_BASE+0x00000d8c,0x00002000); //MMW shf2 event select
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000714,0x0000fc00); // dvfs level setting for chn_emi rw switching shf
   ...
   // hash rule 
   mt_emi_sync_write(EMI_APB_BASE+0x000007a4,0x80000007);
...
static void emi_chn_config(unsigned cfg_base_addr) {
   ...
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000000,0x0488005d); // For bring-up, will be overwritten
   ...
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000020,0x00000040); // [6]: ebg_dis = 0x1, will be overwritten
   ...
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000050,0x38460032); // [1]: MD_RD_AFT_WR_EN, [5:4]: chn_bsec_act = 0x3
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000058,0x00000000);
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000090,0x00000000); // [9]: ap_early_cke_en = 0x0, will be overwritten
   ...
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000154,0x64f3fc79); // update timeout setting: bit 12~15 
   ...
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x000001b0,0x000c802e); // Rank arbitration; [0]: rank_arb_en = 0x0, will be overwritten
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x000001b4,0xbd3f3f7e); // Rank arbitration
   mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x000001b8,0x7e003d7e); // Rank arbitration
...
static void emi_sw_setting(void)
{
	...
	*((volatile unsigned int *) EMI_BWCT0_6TH) = 0x07FF0605;
	*((volatile unsigned int *) EMI_BWCT0_3RD) = 0x0DFF0905;
...
unsigned int emi_rank_size_calc(void)
{
    unsigned int EMI_CHANNEL_APB_BASE = CHN0_EMI_BASE;
    unsigned int emi_config_tmp;
    unsigned int emi_config = 0x0u;

    unsigned int bit_num = 0x0u;

    emi_config_tmp = mt_emi_sync_read(EMI_CHANNEL_APB_BASE+0x00000000);
    emi_config |= ((emi_config_tmp & (0xFu << 16)) >> 16);
    emi_config |= ((emi_config_tmp & (0x1u << 8)) >> 4);

    if ((emi_config_tmp & 0x1u) == 0x1u && emi_config == 0x0u) {
        bit_num += (((emi_config_tmp & (0x3u << 12)) >> 12) | (emi_config_tmp & (0x1u << 2)));
        bit_num += ((emi_config_tmp & (0x3u << 4)) >> 4);

        emi_config = ((0x1u << bit_num) >> 2);
    }

    return emi_config;
}

static void update_earlycke_setting(void)
{
    unsigned int EMI_APB_BASE = EMI_BASE;
    unsigned int EMI_CHANNEL_APB_BASE = CHN0_EMI_BASE;
    unsigned int emi_config_tmp;
    unsigned int reg_early_cke_addr_decode_0 = 0x0u;

    emi_config_tmp = mt_emi_sync_read(EMI_APB_BASE+0x00000000);
    // 4chn_en
    reg_early_cke_addr_decode_0 |= ((emi_config_tmp & (0x1u << 9)) >> 8);
    // chn_en
    reg_early_cke_addr_decode_0 |= ((emi_config_tmp & (0x1u << 8)) >> 6);
    // chn_pos
    reg_early_cke_addr_decode_0 |= ((emi_config_tmp & (0x3u << 2)) << 2);
    // chn_loc
    reg_early_cke_addr_decode_0 |= ((emi_config_tmp & (0x1u << 10)) >> 4);
    // dual_rank_en
    reg_early_cke_addr_decode_0 |= ((emi_config_tmp & (0x1u << 17)) >> 10);
    // remap_shift
    reg_early_cke_addr_decode_0 |= (0x3u << 13);
    // rank_cke_ext0
    reg_early_cke_addr_decode_0 |= (0xFu << 24);
    // rank_cke_ext1
    reg_early_cke_addr_decode_0 |= (0xFu << 28);

    emi_config_tmp = emi_rank_size_calc();
    // rank_dec0
    reg_early_cke_addr_decode_0 |= (((emi_config_tmp & 0x1Fu) << 1) << 16);

    mt_emi_sync_write(EMI_APB_BASE+0x000007f4, reg_early_cke_addr_decode_0);
    mt_emi_sync_write_or(EMI_APB_BASE+0x000007f4, 0x00000001);
    mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a490, reg_early_cke_addr_decode_0);

    mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000090, 0x000002ff);
}

static void update_ebg_setting(void)
{
  unsigned int EMI_APB_BASE = EMI_BASE;
  unsigned int EMI_CHANNEL_APB_BASE = CHN0_EMI_BASE;
  unsigned int emi_config_tmp;
  unsigned int reg_ebg_addr_decode_0;

  reg_ebg_addr_decode_0 = mt_emi_sync_read(EMI_APB_BASE+0x000007c0);
  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a498, reg_ebg_addr_decode_0);
  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a4a4, reg_ebg_addr_decode_0);

  reg_ebg_addr_decode_0 = mt_emi_sync_read(EMI_APB_BASE+0x000007c4);
  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a49c, reg_ebg_addr_decode_0);
  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a4a8, reg_ebg_addr_decode_0);

  emi_config_tmp = emi_rank_size_calc();
  reg_ebg_addr_decode_0 = mt_emi_sync_read(EMI_APB_BASE+0x000007ec);
  // rank_decode
  reg_ebg_addr_decode_0 |= (((emi_config_tmp & 0x1Fu) << 1) << 10);

  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a4a0, reg_ebg_addr_decode_0);
  mt_emi_sync_write(MCUSYS_PAR_WRAP_BASE+0x0000a4ac, reg_ebg_addr_decode_0);

  mt_emi_sync_write(EMI_CHANNEL_APB_BASE+0x00000020,0x3fff0184);
}

void emi_dcm_setting(void)
{
   // ----- from dcm_setting.c ----- 
   ...
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x128, 0x01FF8000);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x12C, 0x00000000);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x130, 0x2000B040);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x134, 0x00000000);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x138, 0x00000000);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x13C, 0x00000000);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x140, 0x10024F20);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x144, 0x00019040);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x148, 0x000A0018);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x14C, 0x00000378);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x150, 0xC001C080);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x154, 0x33000E01);
   mt_emi_sync_write(INFRACFG_AO_MEM_BASE+0x158, 0x18007FF1);
...
void emi_init2(void)
{
  ...
  unsigned int emi_temp_data;
   ...
   mt_emi_sync_write(EMI_APB_BASE+0x00000028,0x00421000); // EMI_CONF
   ...
   // Enable Chn EMI
   mt_emi_sync_write_or(EMI_CHANNEL_APB_BASE+0x00000010,0x00000001); // [0]: emi_enable = 0x1
   // Enable Cen EMI
   mt_emi_sync_write_or(EMI_APB_BASE+0x00000060,0x00000400); // [10]: emi_conf_done = 0x1
...
{
  mt_emi_sync_write_or(MCUSYS_PAR_WRAP_BASE+0x0000A490,0x00000001); // [0] early_cke enable  0:disable  1:enable
}

void enable_mcusys_ebg(void)
{
  mt_emi_sync_write_or(MCUSYS_PAR_WRAP_BASE+0x0000a4a0, 0x00040000);
  mt_emi_sync_write_or(MCUSYS_PAR_WRAP_BASE+0x0000a4ac, 0x00040000);
  emi_log("[EMI] EMI_EBG on\n");
}
...
int update_emi_setting(EMI_SETTINGS *default_emi_setting, EMI_INFO_T *emi_info)
{
	...
	if (((cen_conf_shf - 9) * 4) < 21) {
      ...
      emi_log("[EMI] error: CEN_CONF = 0x%x", cen_conf_shf);
		return -1;
	}
...
void ddr_reserve_emi_init(void)
{
...
#if (CFG_HASH_RULE_MP)
	emi_addr2dram_disph(0);
	emi_addr2dram_hash(emi_temp_data);
#else
	emi_addr2dram_disph(0);
	emi_addr2dram_hash(emi_temp_data);
#endif
	...
	if (emi_mpu_drm == 1) {
		...
		/* clear EMI MPU protection */
		clr_emi_mpu_prot();
		/* disable EMI APB protection */
		dis_emi_apb_prot();
