...
void UpdateHighestFreqInDFSTbl(DRAMC_CTX_T *p, DRAM_PLL_FREQ_SEL_T new_freq_sel)
{
#if(FOR_DV_SIMULATION_USED==0 && SW_CHANGE_FOR_SIMULATION==0)
...
void UpdateHighestFreqToDDR3733(DRAMC_CTX_T *p)
{
    EMI_SETTINGS *emi_set;
...
void XRTWTW_SHU_Setting(DRAMC_CTX_T * p)
{
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_BYTE_NUMBER; u1ByteIdx++)
    {
        if (p->support_rank_num == RANK_SINGLE)
            u2TxDly_OEN_RK_max = u2TxDly_OEN_RK[0][u1ByteIdx];
        else
...
unsigned int DDRPhyFreqMeter(DRAMC_CTX_T *p)
{
    ...
    reg0 = DRV_Reg32(Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x70c) ;
    backup_phypll = reg0;
    DRV_WriteReg32  (Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x70c  , reg0 | (1 << 16));
    reg0 = DRV_Reg32(Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x72c) ;
    backup_clrpll = reg0;
    DRV_WriteReg32  (Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x72c  , reg0 | (1 << 16));

    // abist_clk29: AD_MPLL_CK
    frq_result = FMeter(ABIST_FMETER, 29) ;
    mcSHOW_DBG_MSG4(("AD_MPLL_CK FREQ=%d\n", frq_result));
    ...
    #if 1
    if((DRV_Reg32(Channel_B_DDRPHY_NAO_BASE_ADDRESS + 0x50c) & (1<<8))==0)
    {
    ...
    else
    {
        ...
        mcSHOW_DBG_MSG4(("AD_RPHYPLL_DIV4_CK_ch02 FREQ=%d\n", frq_result));
    ...
    mcSHOW_DBG_MSG4(("fmem_ck_bfe_dcm_ch0 FREQ=%d\n", before_value));
    ...
    mcSHOW_DBG_MSG4(("fmem_ck_aft_dcm_ch0 FREQ=%d\n", after_value));
    ...
    #else
    DRV_WriteReg32  (Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x70c  , backup_phypll);
    DRV_WriteReg32  (Channel_B_DDRPHY_AO_BASE_ADDRESS + 0x72c  , backup_clrpll);
    #endif
    ...
    if((DRV_Reg32(Channel_C_DDRPHY_NAO_BASE_ADDRESS + 0x50c) & (1<<8))==0)
    {
        ...
        mcSHOW_DBG_MSG4(("AD_RCLRPLL_DIV4_CK_ch13 FREQ=%d\n", frq_result));
    ...
    else
    {
        ...
        mcSHOW_DBG_MSG4(("AD_RPHYPLL_DIV4_CK_ch13 FREQ=%d\n", frq_result));
    ...
    mcSHOW_DBG_MSG4(("fmem_ck_aft_dcm_ch2 FREQ=%d\n", before_value));
    ...
    mcSHOW_DBG_MSG4(("fmem_ck_aft_dcm_ch3 FREQ=%d\n", after_value));
