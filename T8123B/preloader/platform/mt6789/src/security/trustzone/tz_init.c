...
void trustzone_jump(u64 addr, u32 arg1, u32 arg2)
{
    ...
    if (abnormal_boot)
        ...
        /* Will warm reset to 64 bit BL2 extension */
        platform_set_addr_to_reset_vector(PL_BL2_EXT_BASE);
    }
    DBG_ERR("%s Jump to %s: 0x%x\n\n", MOD, abnormal_boot ? "AEE-LK" : "BL2_EXT",
            abnormal_boot ? CFG_AEE_LK_MEMADDR : PL_BL2_EXT_BASE);
