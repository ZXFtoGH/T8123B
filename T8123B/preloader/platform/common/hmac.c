		    ...
		    uint8_t *msg, uint32_t msg_len, uint8_t *mac)
{
	...
	(void)memset(hmac_metadata.i_key_pad, 0x36, I_KEY_PAD_SIZE);
	(void)memset(hmac_metadata.o_key_pad, 0x5C, O_KEY_PAD_SIZE);
	...
	uint8_t *i_key_pad_position = hmac_metadata.i_key_pad;
	uint8_t *o_key_pad_position = hmac_metadata.o_key_pad;
	/* hash: first pass */
	ret = sha256(i_key_pad_position,
