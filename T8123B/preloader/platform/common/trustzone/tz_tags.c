...
static u32 tee_static_reserved_secmem_addr = 0;
static u32 tee_static_reserved_secmem_size = 0;
void tz_secmem_static_reserve(unsigned int req_size)
{
    u32 addr;

    if ((req_size % SECMEM_ALIGNMENT) != 0) {
        pal_log_err("[%s] Un-aligned to 0x%x!\n", MOD, SECMEM_ALIGNMENT);
        ASSERT(0);
    }

    addr = (u32)mblock_reserve_ext(&bootarg.mblock_info,
        (u64)req_size, (u64)SECMEM_ALIGNMENT,
        (u64)SECMEM_LIMITATION, 0, SECMEM_RESERVED_NAME);
    if (!addr) {
        pal_log_warn("[%s] mblock reserve failed!\n", MOD);
        ASSERT(0);
    }

    tee_static_reserved_secmem_addr = addr;
    tee_static_reserved_secmem_size = req_size;
}
    ...
    unsigned int tz_sec_mem_start, unsigned int tz_sec_mem_end)
{
    ...
    if (IS_ZERO(tz_sec_mem_start) && IS_ZERO(tz_sec_mem_end)) {
        tz_sec_mem_start = tee_static_reserved_secmem_addr;
        tz_sec_mem_end = tee_static_reserved_secmem_addr + tee_static_reserved_secmem_size;
        pal_log_info("[%s] 0x%x-0x%x (res)\n", MOD, tz_sec_mem_start, tz_sec_mem_end);
    }
