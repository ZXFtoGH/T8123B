...
int emi_mpu_set_protection(struct emi_region_info_t *region_info)
{
	...
	for (dgroup = EMI_MPU_DGROUP_NUM - 1; dgroup >= 0; dgroup--) {
		...
		*((volatile unsigned int *)SUB_EMI_MPU_SA(region_info->region)) = start;
		*((volatile unsigned int *)SUB_EMI_MPU_EA(region_info->region)) = end;
		*((volatile unsigned int *)SUB_EMI_MPU_APC(region_info->region, dgroup)) = region_info->apc[dgroup];
#endif
	}
#endif
