...
#define DA_MAGIC_HPB_SIZE_GB_MAX     0x003FFFFF
#define DA_MAGIC_TW_SIZE_GB_MAX      0x003FFFFE
#define DA_MAGIC_TW_SIZE_GB_1_OF_16  0x003FFFFF
 ...
 *  descriptor mask
 */
#define MASK_DEV_DSCR_HPB_VER_MAJ 		0xFF00
#define MASK_DEV_DSCR_HPB_VER_MIN 		0x00F0
#define MASK_DEV_DSCR_HPB_VER_SUF 		0x000F
...
enum {
    CUSTOM_FORCE_PROVISION      = BIT(0),
    CUSTOM_TW_GB                = BIT(1),
    CUSTOM_TW_NO_RED            = BIT(2),
    CUSTOM_HPB_REGION_COUNT     = BIT(3),
    CUSTOM_HPB_PINNED           = BIT(4),
    CUSTOM_LU3                  = BIT(5),
    CUSTOM_HPB_CTRL_MODE        = BIT(6),
...
struct ufs_custom_info {
	u32  custom_flag;
	u32  force_provision; /* default:0, force:1, skip:2 */
	u32  tw_size_gb;
	u32  tw_no_red;
	u32  hpb_size_gb;
	u64  lu3_size_mb;
	u32  lu3_type;
	u32  hpb_ctrl_mode; /* host control:0, device control:1 */
	u16  hpb_pinned_start_idx;
	u16  hpb_pinned_regions;
};
...
#define STR_PRFX_EQUAL(s1, s2) !strnncmp(s1, s2, strlen(s1))
#else
...
extern int          ufs_aio_poweroff_notify(struct ufs_hba *hba);
