...
/* use size 64bit to read/write */
ssize_t storage_wrap_read(uint32_t phys_part, uint64_t offset, uint8_t *buf,
		     uint64_t size)
{
	uint32_t phys_part_id = 0;
	blkdev_t *blkdev = NULL;
	...
	if (NULL == (blkdev = blkdev_get(CFG_BOOT_DEV))) {
		pal_log_err("[%s] can't find boot device(%d)\n", __func__, CFG_BOOT_DEV);
		return -1;
	}
	phys_part_id = storage_get_part_id(phys_part);

	if (0 == blkdev_read64(blkdev, offset, size, buf, phys_part_id))
		return 0;
	else {
		pal_log_err("[%s] raw_read fail\n", __func__);
		return -1;
...
ssize_t storage_wrap_write(uint32_t phys_part, uint64_t offset, uint8_t *buf,
		      uint64_t size)
{
	blkdev_t *blkdev = NULL;
	uint32_t phys_part_id = 0;
	...
	if (0 == blkdev_write64(blkdev, offset, size, buf, phys_part_id))
		return 0;
	else {
		pal_log_err("[%s] raw_read fail\n", __func__);
		return -1;
...
ssize_t storage_erase(uint32_t phys_part, uint64_t offset, uint64_t size)
{
	blkdev_t *blkdev = NULL;
	uint32_t phys_part_id = 0;
	...
	phys_part_id = storage_get_part_id(phys_part);
	if (0 == blkdev_erase(blkdev, offset, size, phys_part_id))
		return 0;
	else {
		pal_log_err("[%s] raw_erase fail\n", __func__);
		return -1;
	}
