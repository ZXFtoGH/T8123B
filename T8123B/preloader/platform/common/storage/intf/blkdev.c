...
{
    return bdev->erase(bdev, src, size, part_id);
}
...
int blkdev_read64(blkdev_t *bdev, u64 src, u64 size, u8 *dst, u32 part_id)
{
    u8 *buf;
    u32 blksz;
    u64 end, part_start, part_end, part_len, aligned_start, aligned_end;
...
int blkdev_write64(blkdev_t *bdev, u64 dst, u64 size, u8 *src, u32 part_id)
{
    u8 *buf;
    u32 blksz;
    u64 end, part_start, part_end, part_len, aligned_start, aligned_end;
...
{
    return blkdev_read64(bdev, src, (u64)size, dst, part_id);
}

int blkdev_write(blkdev_t *bdev, u64 dst, u32 size, u8 *src, u32 part_id)
{
    return blkdev_write64(bdev, dst, (u64)size, src, part_id);
}
