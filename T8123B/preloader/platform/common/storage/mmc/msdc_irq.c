...
#if defined(MMC_MSDC_DRV_LK) || defined(_FLASH_MODE_DA_)

#ifdef MMC_MSDC_DRV_LK
...
extern void event_init(event_t *, bool initial, uint flags);
extern status_t event_wait(event_t *);
extern status_t event_wait_timeout(event_t *, time_t); /* wait on the event with a timeout */
extern status_t event_signal(event_t *, bool reschedule);

#endif
static event_t msdc_int_event;
static u32 g_int_status = 0;

void lk_msdc_irq_handler(unsigned int id)
{
	u32 intrs,sts;
	...
	MSG(INT, "[%s]host[%d] enable:0x%x 0x%x\n", __func__, id, intrs, MSDC_READ32(MSDC_INT));
	if ((sts = MSDC_READ32(MSDC_INT)) & intrs) {
		g_int_status = sts;
		MSG(INT, "[%s]send event, 0x%x\n", __func__, g_int_status);
...
static const u32 msdc_irq_line[MSDC_MAX_NUM] = {MSDC0_IRQ_BIT_ID, MSDC1_IRQ_BIT_ID};
...
static bool msdc_use_gic = true;
//static bool msdc_use_gic = false;
...
#if defined(MMC_MSDC_DRV_LK) || defined(_FLASH_MODE_DA_)
#ifndef MSDC1_EMMC
	u32 irq = MT_MSDC0_IRQ_ID;
	char *description = "MSDC0";
#else
	u32 irq = MT_MSDC1_IRQ_ID;
	char *description = "MSDC1";
#endif

	host->use_gic = msdc_use_gic;
	if (host->use_gic) {
		msdc_intr_mask(host, 0xffffffff);
		request_irq(irq, LEVEL_SENSITIVE, (irq_handler_t)lk_msdc_irq_handler,
			(void*)(host->id), FALSE, description);
