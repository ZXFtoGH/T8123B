...
static u64 mmc_get_emmc_size(struct mmc_card *card)
{
	...
	size_tmp += 1024 *
			(u64)(card->ext_csd.gp_sz_kb[0] + card->ext_csd.gp_sz_kb[1] + card->ext_csd.gp_sz_kb[2] + card->ext_csd.gp_sz_kb[3]);
...
int mmc_set_boot_part(u32 bootpart)
{
	...
	cur_acken = (part_config >> 6) & 0x1;

	if (((u32)cur_bootpart) == bootpart && cur_acken)
		goto out;

	part_config = (part_config & (~(0x7 << 3))) | (bootpart << 3) | EXT_CSD_PART_CFG_EN_ACK;

	msdc_pr_info("[SD0] Set boot part %d->%d, boot acken %d->1\n", cur_bootpart, bootpart, cur_acken);
...
static int mmc_wrap_erase(blkdev_t *bdev, u64 src, u64 size, u32 part_id)
{
   struct mmc_host *host = (struct mmc_host*)bdev->priv;

   return mmc_do_erase(host->id, src, size, part_id);
}
...
u32 mmc_init_device(void)
{
	...
	if (!blkdev_get(BOOTDEV_SDMMC)) {
		...
		g_mmc_bdev.erase  = mmc_wrap_erase;
...
static int mmc_do_erase(int dev_num, u64 start_addr, u64 len, u32 part_id)
{
	/* NOT support erase operation if not define FEATURE_ERASE */
	return MMC_ERR_INVALID;
}
