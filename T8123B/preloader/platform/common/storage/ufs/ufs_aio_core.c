...
#if defined(MTK_UFS_DRV_LK) || defined(MTK_UFS_DRV_PRELOADER_DA)
#if defined(MTK_UFS_DRV_LK)
#define UFS_ERASE_BUF_SIZE	(512 * 1024)
#else
#define UFS_ERASE_BUF_SIZE	(4096)
#endif
static unsigned char ufs_erase_buf[UFS_ERASE_BUF_SIZE] __attribute__((aligned(UFS_PLATFORM_CACHE_LINE_SIZE)));
#endif


#if defined(MTK_UFS_DRV_DA) || defined(MTK_UFS_DRV_LK) || defined(MTK_UFS_DRV_CTP) || defined(MTK_UFS_DRV_PRELOADER_DA)
...
void ufs_aio_scsi_cmd_inquiry(struct ufs_aio_scsi_cmd * cmd, u32 tag, u32 lun, unsigned long * buf, u32 evpd, u32 page)
{
	memset(cmd->cmd_data, 0, MAX_CDB_SIZE);
...
void ufs_aio_scsi_cmd_start_stop_unit_pd(struct ufs_aio_scsi_cmd *cmd, u32 tag)
{
	memset(cmd->cmd_data, 0, MAX_CDB_SIZE);
...
#if defined(MTK_UFS_DRV_LK) || defined(MTK_UFS_DRV_PRELOADER_DA)
	/* Write 0 to make sure the data is cleared */

	if (blk_cnt > UFS_CLR_BLK_CNT)
		blk_cnt = UFS_CLR_BLK_CNT;

	wr_blk_cnt = UFS_ERASE_BUF_SIZE / UFS_BLOCK_SIZE;

	while (blk_cnt > 0) {
		if (blk_cnt >= wr_blk_cnt) {
			ufs_aio_block_write(0, lun, blk_start, wr_blk_cnt, (unsigned long *)ufs_erase_buf);
			blk_start += wr_blk_cnt;
			blk_cnt -= wr_blk_cnt;
		} else {
			ufs_aio_block_write(0, lun, blk_start, blk_cnt, (unsigned long *)ufs_erase_buf);
			break;
		}
	}
#endif
...
int ufs_aio_cfg_callback(struct ufs_hba * hba, u8 mode)
{
	...
	|| defined(MTK_UFS_DRV_PRELOADER_DA)
...
int ufs_aio_otp_update_lock_mark(struct ufs_hba * hba, u64 otp_start_addr, u32 lock)
{
	...
	int ret = 0;
#if defined(MTK_UFS_DRV_PRELOADER_DA)
	blkdev_t *bdev = blkdev_get(BOOTDEV_UFS);
#endif
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
	ret = blkdev_write64(bdev, otp_lock_mark_addr, UFS_BLOCK_SIZE, lock_mark, hba->active_lun);
#else
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
	ret = blkdev_read64(bdev, otp_lock_mark_addr, UFS_BLOCK_SIZE, lock_mark, hba->active_lun);
#else
