...
u32 ufs_init_device(void)
{
	...
	if (!blkdev_get(BOOTDEV_UFS) || hba->custom_info.force_provision) {
...
#if !defined(MTK_UFS_DRV_PRELOADER_INGNORE_WP)
		ufs_clr_write_protect();
#endif
	}
	return 0;
}
...
int ufs_poweroff_notify(void)
{
	struct ufs_hba *hba;

	hba = ufs_aio_get_host(UFS_DEFAULT_HOST_ID);

	return ufs_aio_poweroff_notify(hba);
}
#endif
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
int ufs_erase(u64 address, u64 length, const struct progress_cb* cb)
#else
...
#endif
{
	...
	if (-1 == hba->active_lun) {
		UFS_DBG_LOGE("[UFS] err: interface_ufs_write: active_lun is not initialized");
		return STATUS_UFS_ERR;
	}

	UFS_DBG_LOGV("[UFS] info: erase %llx,%llx,%d,%d,%d\n", address, length,
		hba->active_lun, blk_start, blk_cnt);
	...
	while (blk_cnt) {
		if ((blk_cnt >= blk_cnt_1_100) && (blk_cnt_1_100 > 0)) {
			...
			if (ret != UFS_ERR_NONE) {
				UFS_DBG_LOGE("[UFS] err: erase failed, ret: %d\n", ret);
				break;
			}
			...
			if ((cb) && (progress < 100)) {
				ret = cb->cb(cb->user_arg, progress, (char*) &stop_flag);
				if(FAIL(ret))
					break;
				progress++;
			}
			...
			if (cb)
				cb->cb(cb->user_arg, 100, (char*) &stop_flag);
			break;
		}
	}
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
int ufs_get_info(struct ufs_info_struct* info)
#else
...
#endif
{
	...
	if (hba->dev_info.product_id[0] != 0) {
		strlcpy((char *)info->cid, hba->dev_info.product_id,
			MAX_PRODUCT_ID_LEN + 1);
		UFS_DBG_LOGE("[UFS] ufs id : %s\n", info->cid);
	}

	if (hba->dev_info.product_revision_level[0] != 0) {
		strlcpy((char *)info->fwver, hba->dev_info.product_revision_level,
			MAX_PRODUCT_REVISION_LEVEL_LEN + 1);
		UFS_DBG_LOGE("[UFS] fwver: %s\n", info->fwver);
	}

	// get UFS serial number
	if (hba->dev_info.serial_number_len != 0) {
		strlcpy((char *)info->sn, hba->dev_info.serial_number,
			hba->dev_info.serial_number_len * 2 + 1);
		UFS_DBG_LOGE("[UFS] serial number: %s\n", info->sn);
	}
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
int ufs_device_ctrl(u32 ctrl_code, void* in, u32 in_len, void* out, u32 out_len, u32* ret_len)
#else
...
#define ROUND_TO_BLOCK(x,y) (((x) + (y-1)) & (~(y-1)))
		...
		case STORAGE_CRCODE_SET_UFS_FORCE_PROVISION:
			if (*(u32*) in <= 2)
				hba->custom_info.force_provision = *(u32*) in;
			else
		...
		case STORAGE_CRCODE_SET_UFS_LU3:
			hba->custom_info.lu3_size_mb = *(u32*) in;
			hba->custom_info.lu3_type = *((u32*) in + 1);
			hba->custom_info.custom_flag |= CUSTOM_LU3;
			break;

		case STORAGE_CRCODE_SET_UFS_HPB_PINNED_REGION:
			hba->custom_info.hpb_pinned_start_idx = *(u16*) in;
			hba->custom_info.hpb_pinned_regions = *((u16*) in + 1);
			hba->custom_info.custom_flag |= CUSTOM_HPB_PINNED;
			break;

		case STORAGE_CRCODE_SET_UFS_HPB_CTRL_MODE:
			hba->custom_info.hpb_ctrl_mode = *(u32*) in;
			hba->custom_info.custom_flag |= CUSTOM_HPB_CTRL_MODE;
			break;
