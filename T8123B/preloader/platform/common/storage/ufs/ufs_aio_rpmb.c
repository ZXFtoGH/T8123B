...
static int ufs_rpmb_get_rnd(u32 *rnd)
{
	...
	*rnd = 0;
	...
	ret = get_rnd(rnd);
#elif defined(MTK_UFS_DRV_PRELOADER)
	ret = platform_get_rnd(rnd);
#else
	#error "MUST PORTING ufs_rpmb_get_rnd()\n"
#endif
...
static int ufs_rpmb_pre_nonce(u8* buf, u32 size)
{
	...
	if (size == 0) {
		UFS_DBG_LOGE("[WARNING]%s: nonce size is not match %d(%d)\n",
...
out:
	if (ret) {
		UFS_DBG_LOGE("%s:get_rnd is not ready, ret:%d\n",
			__func__, ret);
		BUG_ON(1);
	}
        ...
        u16 blk_num, u32 wr_cnt, STORAGE_WP_TYPE type, unsigned long blknr, u32 blkcnt, ufs_logical_unit partition,u8 wp_entry,const u8 *data_buf)
{
	...
	u8 rpmb_hmac_derived[RPMB_FRM_KEY_MAC_LEN] = {0};
#endif
#if defined(MTK_UFS_DRV_PRELOADER) && !defined(MTK_UFS_DRV_PRELOADER_DA)
...
int ufs_rpmb_read_data(u32 addr, u32 *buf, u32 buf_len, int *result)
{
	...
	for (i = 0; i < blk_len; i++) {
		...
		/* Step3. Check Response and parser data*/
		rpmb_frame = cmd.data_buf;
		*result = cpu_to_be16(rpmb_frame->result);
		if (*result) {
			UFS_DBG_LOGE("[%s]: result = %d \n", __func__, *result);
			return -UFS_RPMB_RESULT_ERROR;
		}
