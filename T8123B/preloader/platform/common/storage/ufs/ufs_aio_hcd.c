...
#include "platform.h"
#endif
#if defined(MTK_UFS_DRV_PRELOADER_DA)
#include <event.h>
#include <mt_irq.h>
#endif
#endif
...
static void ufs_aio_test_unit_ready_all_device(struct ufs_hba *hba);
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
	request_irq(hba->irq, LEVEL_SENSITIVE, (irq_handler_t) ufshcd_irq_handler,
			(void*) hba->irq, FALSE, "UFS");
#else
...
static int ufshcd_hba_enable(struct ufs_hba *hba)
{
	...
	ufshcd_writel(hba, ufshcd_readl(hba, REG_XOUFS_CTRL) | 0x80,
		REG_XOUFS_CTRL);
...
static int ufshcd_complete_dev_init(struct ufs_hba *hba)
{
	...
	for (retries = QUERY_REQ_RETRIES; retries > 0; retries--) {
		...
		UFS_DBG_LOGE("error %d retrying\n", err);
	}
	if (err) {
		UFS_DBG_LOGE("setting fDeviceInit flag failed with error %d\n", err);
		goto out;
	}
#if defined(MTK_UFS_DRV_CTP)
	...
	if (err)
		UFS_DBG_LOGE("reading fDeviceInit flag failed with error %d\n", err);
	else if (flag_res) {
		UFS_DBG_LOGE("fDeviceInit was not cleared by the device\n");
	...
	if (UTP_CMD_TYPE_DEV_MANAGE == lrbp->command_type)  /* for device management commands */
		...
		if (resp & MASK_TASK_RESPONSE) {
			...
			if (UPIU_TRANSACTION_RESPONSE == value) {
				...
				UFS_DBG_LOGE("[UFS] Sense Data(all): ");
				ufs_aio_util_dump_config_table_hex(lrbp->ucd_rsp_ptr->sr.sense_data, 18, 18, 0);
			}

			 /* sense key is UNIT_ATTENTION, will retry */
			if ((lrbp->ucd_rsp_ptr->sr.sense_data[2] &0xF) == UNIT_ATTENTION)
				ret = UFS_UNIT_ATTENTION_RETRY;
			else {
...
#if defined(MTK_UFS_DRV_DA) || defined(MTK_UFS_DRV_PRELOADER_DA)
...
__WEAK unsigned int config_hpb_ctrl_mode(struct ufs_device_info* dev_info)
{
	if (dev_info->wmanufacturerid == UFS_VENDOR_WDC)
		return 0x00; // host control mode
	else
		return 0x01; // device control mode
}

__WEAK unsigned int config_TW_size_GB(void)
{
	return DA_MAGIC_TW_SIZE_GB_1_OF_16;
}
...
{
	return DA_MAGIC_HPB_SIZE_GB_MAX;
}
...
static void ufshcd_init_custom_info(struct ufs_hba *hba)
{
	...
	if ((hba->custom_info.custom_flag & CUSTOM_FORCE_PROVISION) == 0)
		hba->custom_info.force_provision = 0;

	if ((hba->custom_info.custom_flag & CUSTOM_HPB_CTRL_MODE) == 0) {
		UFS_DBG_LOGE("[UFS] warn: HPB control mode not set explicitly. "
			"Use default value.\n");
		hba->custom_info.hpb_ctrl_mode = config_hpb_ctrl_mode(&hba->dev_info);
	}
	...
	if ((hba->custom_info.custom_flag & CUSTOM_LU3) == 0) {
		hba->custom_info.lu3_size_mb = 0;
		hba->custom_info.lu3_type = 0;
	}

	if ((hba->custom_info.custom_flag & CUSTOM_HPB_PINNED) == 0) {
		hba->custom_info.hpb_pinned_start_idx = 0;
		hba->custom_info.hpb_pinned_regions = 0;
	}

	/* Set custom info done flag */
	hba->custom_info.custom_flag |= CUSTOM_SET_DONE;

	UFS_DBG_LOGI("[UFS] Custom force provison=%d, TW size=%dGB, TW no red=%d, "
		"HPB size=%dGB, Control mode=%d\n",
		hba->custom_info.force_provision,
		hba->custom_info.tw_size_gb,
		hba->custom_info.tw_no_red,
		hba->custom_info.hpb_size_gb,
		hba->custom_info.hpb_ctrl_mode);
}
#endif
...
static int ufshcd_probe_hba(struct ufs_hba *hba)
{
	...
	if (hba->custom_info.force_provision == 2)
		goto skip_provision;
	...
	if ((hba->dev_info.bootable == 0) ||
	    ...
	    (hba->custom_info.force_provision == 1)) {
		hba->custom_info.force_provision = 0;
...
#if defined(MTK_UFS_DRV_PRELOADER_DA)
static char ufs_polling_mode = 0;
...
ufshcd_init_reinit_host:
	...
	if (!ufshcd_irq_init(hba))
		hba->drv_status |= UFS_DRV_STS_DMA_WAIT_INTERRUPT;
#elif defined(MTK_UFS_DRV_PRELOADER_DA)
	 if ((!ufs_polling_mode) && (!ufshcd_irq_init(hba)))
		 hba->drv_status |= UFS_DRV_STS_DMA_WAIT_INTERRUPT;
#endif
...
static int ufs_aio_pre_pwr_change(struct ufs_hba *hba, struct ufs_pa_layer_attr *desired, struct ufs_pa_layer_attr *final)
{
	...
	/* ADAPT */
	if (final->pwr_rx == FAST_MODE || final->pwr_rx == FASTAUTO_MODE) {
		if ((final->gear_tx == 4) && (final->gear_rx == 4))
	...
	if (final->pwr_rx == SLOW_MODE || final->pwr_rx == SLOWAUTO_MODE) {
		...
		UFS_DBG_LOGE("[UFS] info: RX HS-G%d, setting is %d\n", final->gear_rx, UFS_DEV_MAX_GEAR_RX);
		UFS_DBG_LOGE("[UFS] info: TX HS-G%d, setting is %d\n", final->gear_tx, UFS_DEV_MAX_GEAR_TX);
	}
	...
	if (final->pwr_rx == SLOW_MODE || final->pwr_rx == SLOWAUTO_MODE)
		UFS_DBG_LOGI("[UFS] info: PWM-G%d, %d lane\n", final->gear_rx, final->lane_rx);
	else
		UFS_DBG_LOGI("[UFS] info: HS-G%d-%d, %d lane\n", final->gear_rx, final->hs_rate, final->lane_rx);
...
u32 ufs_aio_get_lu_cfg_size(struct ufs_hba *hba, struct ufs_geometry_info *info, u32 lun_id)
{
	...
	if((info->d_segment_size <= 0) || (info->b_allocation_units_size <= 0))
	{
		UFS_DBG_LOGE("[UFS] d_segment_size or b_allocation_units_size error\n");
		return -1;
	}
	 ...
	 * Others: Reserved
	*/
	if (hba->custom_info.lu3_type == 1)
		lu3_adj_factor = info->w_adj_factor_system_code;
	else if (hba->custom_info.lu3_type == 2)
		lu3_adj_factor = info->w_adj_factor_non_persist;
	else if (hba->custom_info.lu3_type == 3)
		lu3_adj_factor = info->w_adj_factor_enahnced_1;
	else if (hba->custom_info.lu3_type == 4)
		lu3_adj_factor = info->w_adj_factor_enahnced_2;
	else if (hba->custom_info.lu3_type == 5)
		lu3_adj_factor = info->w_adj_factor_enahnced_3;
	else if (hba->custom_info.lu3_type == 6)
		lu3_adj_factor = info->w_adj_factor_enahnced_4;
	else
		lu3_adj_factor = 256;
	...
	if (lun_id < 2) { /* boot lu */
		...
		/*
		 * LU3 size MB which may have enhance feature, real size
		 * shoule be 4MB * (lu3_adj_factor / 256)
		 */
		UFS_DBG_LOGE("[UFS] lu3_size_mb=%lld, lu3_type=%d\n", hba->custom_info.lu3_size_mb, hba->custom_info.lu3_type);
		UFS_DBG_LOGE("[UFS] lu3_adj_factor=%d\n", lu3_adj_factor);

		d_num_alloc_units = ((hba->custom_info.lu3_size_mb << 11) * (lu3_adj_factor >> 8)) /
			info->d_segment_size / info->b_allocation_units_size;
	} else { /* user lu */

		d_num_alloc_units = (u32)(((info->q_total_raw_device_capacity) -
					   (u64)(2 * (UFS_BOOT_LU_SIZE_BYTE >> 9) * (info->w_adj_factor_enahnced_1 >> 8)) -
					   (u64)((hba->custom_info.lu3_size_mb << 11) * (lu3_adj_factor >> 8))) /
						info->d_segment_size / info->b_allocation_units_size);
		...
		if (hba->dev_info.tw_support) {
			...
			if (tw_size_gb == DA_MAGIC_TW_SIZE_GB_1_OF_16) {
				UFS_DBG_LOGI("[UFS] *1/16 of LU size as TW size\n");
				tw_size_gb = (((d_num_alloc_units * info->b_allocation_units_size * info->d_segment_size) >> 21) + 15) >> 4;
			}

			/* for customer set to max au */
			if (tw_size_gb == DA_MAGIC_TW_SIZE_GB_MAX) {
				UFS_DBG_LOGI("[UFS] *maximize TW size\n");
				info->d_tw_buf_au = info->d_tw_buf_max_au;
				tw_size_gb = (info->d_tw_buf_max_au * info->b_allocation_units_size * info->d_segment_size) >> 21;
				/* Samsung 3.0 not follow spec */
				if ((hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) && (hba->dev_info.tw_ver == 0x0101) && (info->b_tw_buf_adj_fac))
					tw_size_gb = tw_size_gb / info->b_tw_buf_adj_fac;
			} else {
				/* Samsung 3.0 not follow spec */
				if ((hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) && (hba->dev_info.tw_ver == 0x0101))
			...
			if (info->d_tw_buf_au > info->d_tw_buf_max_au) {
				if ((hba->custom_info.tw_size_gb == DA_MAGIC_TW_SIZE_GB_MAX) || (hba->custom_info.tw_size_gb == DA_MAGIC_TW_SIZE_GB_1_OF_16))
					info->d_tw_buf_au = info->d_tw_buf_max_au;
				else
		...
		if ((hba->dev_info.hpb_support) && (info->q_hpb_region_size)) {
			...
			UFS_DBG_LOGI("[UFS] LU2 size=0x%llx\n", lu2_size);
			...
			if (info->w_hpb_lu_max_active_regions > info->w_hpb_device_max_active_regions) {
				UFS_DBG_LOGI("[UFS] HPB max LU regions=0x%x > device max active regions=0x%x\n",
			...
			if (hba->custom_info.hpb_size_gb != DA_MAGIC_HPB_SIZE_GB_MAX) {
...
int ufs_aio_check_lu_cfg(struct ufs_hba *hba)
{
	...
	/* HPB support, read configuration descriptor with HPB, length = 0xD0(HPB1.0)/0xE2(HPB2.0) */
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, hba->quirk_sel, 
			(u8 *)&g_ufs_temp_buf[0], ufs_query_desc_max_size[QUERY_DESC_IDN_CONFIGURATION]);
	...
	for (i = 0; i < 8; i++) { /* traverse 8 Unit Descriptor configurable parameters */
		...
		if (i == 2){
			/* check whether HPB is enabled for LU2 if avaliable */
			if ((hba->dev_info.hpb_support) && (p[CONF_DESC_UNIT_B_LU_ENABLE] != 0x02)){
				UFS_DBG_LOGE("[UFS] error: HPB is supported but not enabled on LU%d \n", i);
			}
		}
	}
	...
	info.w_adj_factor_system_code = (p[0x24] << 8) | p[0x25];
	info.w_adj_factor_non_persist = (p[0x2A] << 8) | p[0x2B];
	info.w_adj_factor_enahnced_1 = (p[0x30] << 8) | p[0x31];
	info.w_adj_factor_enahnced_2 = (p[0x36] << 8) | p[0x37];
	info.w_adj_factor_enahnced_3 = (p[0x3C] << 8) | p[0x3D];
	info.w_adj_factor_enahnced_4 = (p[0x42] << 8) | p[0x43];
...
int ufs_aio_poweroff_notify(struct ufs_hba *hba)
{
	int ret = 0;
	int tag;
	int retry = 3;
	...
	ufs_aio_scsi_cmd_start_stop_unit_pd(&cmd, tag);

	do {
		ret = ufshcd_queuecommand(hba, &cmd);
		if (ret == 0){
			UFS_DBG_LOGI("[UFS] notify power-off success\n");
			break;
		}
		retry--;
	} while (ret && retry > 0);
...
static u64 bit_shift_left(u32 shift)
{
  integer_64 itg;
  itg.quad_part = 0;
  if(shift >= 32) {
      itg.u.high_part = 1 << (shift-32);
  } else {
      itg.u.low_part = 1 << shift;
  }
  return itg.quad_part;
}
...
int ufs_aio_configure_new_ufs(struct ufs_hba *hba)
{
	...
	u32 d_num_alloc_units_boot_lu;
	u32 d_num_alloc_units_user_lu;
	u32 d_num_alloc_units_lu3;
	u8  b_tw_buf_type = 0;
	bool f_hpb_enabled = true;
	struct ufs_geometry_info info = {0};

	p = (u8 *)&g_ufs_temp_buf[0];

	memset(p, 0, UFS_TEMP_BUF_SIZE);
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_GEOMETRY, 0, hba->quirk_sel, 
			(u8 *)&g_ufs_temp_buf[0], ufs_query_desc_max_size[QUERY_DESC_IDN_GEOMETRY]);
	...
	if (hba->dev_info.hpb_support) {
		...
		info.q_hpb_region_size = bit_shift_left((u32)p[0x48]);
	...
	/*
	 * Use length from device descriptor, not from configuartion descriptor.
	 * Because some deivce may have 2 kinds configuartion descriptor.
	 */
	p[CONF_DESC_B_LENGTH]       = hba->dev_info.ud0_base_offset + (8 * hba->dev_info.ud_config_len);

	/* Force change configuartion descriptor to new size, old size may wrong */
	ufs_query_desc_max_size[QUERY_DESC_IDN_CONFIGURATION] = p[CONF_DESC_B_LENGTH];
	...
	/* decide LU size */
	size = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_0);
	if(size < 0)
	{
		UFS_DBG_LOGI("[UFS] info: d_num_alloc_units_boot_lu size error\n");
		return size;
	}
	else
	{
		d_num_alloc_units_boot_lu = size;
	}
	size = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_2);
	if(size < 0)
	{
		UFS_DBG_LOGI("[UFS] info: d_num_alloc_units_user_lu size error\n");
		return size;
	}
	else
	{
		d_num_alloc_units_user_lu = size;
	}
	size = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_3);
	if(size < 0)
	{
		UFS_DBG_LOGI("[UFS] info: d_num_alloc_units_lu3 size error\n");
		return size;
	}
	else
	{
		d_num_alloc_units_lu3 = size;
	}
	if (hba->dev_info.hpb_support)
		p[CONF_DESC_B_HPB_CONTROL] = hba->custom_info.hpb_ctrl_mode;
	...
	if (hba->dev_info.tw_support) {
		...
		b_tw_buf_type = p[CONF_DESC_B_TW_BUF_TYPE];
		/* 0: LU Base, 1: Single share */
		if (b_tw_buf_type == 1) {
	...
	for (i = 0; i < 8; i++) { /* traverse 8 Unit Descriptor configurable parameters */
		...
		if ((i == 2) && (hba->dev_info.hpb_support) && (info.w_hpb_lu_max_active_regions != 0)) {
			 /* Enable HPB */
			...
			p[CONF_DESC_UNIT_W_HPB_PINNED_REGION_START_IDX] = (hba->custom_info.hpb_pinned_start_idx >> 8) & 0xFF;
			p[CONF_DESC_UNIT_W_HPB_PINNED_REGION_START_IDX + 1] = hba->custom_info.hpb_pinned_start_idx & 0xFF;
			p[CONF_DESC_UNIT_W_NUM_HPB_PINNED_REGIONS] = (hba->custom_info.hpb_pinned_regions >> 8) & 0xFF;
			p[CONF_DESC_UNIT_W_NUM_HPB_PINNED_REGIONS + 1] = hba->custom_info.hpb_pinned_regions & 0xFF;
		} else {
			p[CONF_DESC_UNIT_B_LU_ENABLE] = 1;
		}

		/* 0: LU Base, 1: Single share */
		if ((i == 2) && (hba->dev_info.tw_support) && (b_tw_buf_type == 0)) {
		...
		} else if (i == 3) { /* new LU3 support */
			if (d_num_alloc_units_lu3) {
				p[CONF_DESC_UNIT_B_MEMORY_TYPE] = hba->custom_info.lu3_type;
				p[CONF_DESC_UNIT_D_NUM_ALLOC_UNITS]     = (d_num_alloc_units_lu3 >> 24) & 0xFF;
				p[CONF_DESC_UNIT_D_NUM_ALLOC_UNITS + 1] = (d_num_alloc_units_lu3 >> 16) & 0xFF;
				p[CONF_DESC_UNIT_D_NUM_ALLOC_UNITS + 2] = (d_num_alloc_units_lu3 >> 8) & 0xFF;
				p[CONF_DESC_UNIT_D_NUM_ALLOC_UNITS + 3] = d_num_alloc_units_lu3 & 0xFF;
			} else {
				p[CONF_DESC_UNIT_B_LU_ENABLE] = 0;
				p[CONF_DESC_UNIT_B_LOGICAL_BLOCK_SIZE]  = 0;
				p[CONF_DESC_UNIT_B_PROVISIONING_TYPE]   = 0;
				if (hba->dev_quirks & UFS_DEVICE_QUIRK_NEED_CONFIGURATION_FOR_UNUSED_LU)
					p[CONF_DESC_UNIT_B_LOGICAL_BLOCK_SIZE]  = 0xC;
			}
	...
	ufs_aio_util_dump_config_table_hex(&g_ufs_temp_buf[0], ufs_query_desc_max_size[QUERY_DESC_IDN_CONFIGURATION],
		hba->dev_info.ud0_base_offset, hba->dev_info.ud_config_len);

	ret = hba->write_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, hba->quirk_sel,
			&g_ufs_temp_buf[0], ufs_query_desc_max_size[QUERY_DESC_IDN_CONFIGURATION]);
	...
	// post setup
	/* fHPBEn is introduced in HPB 2.0.0. MDV = 0 */
	/* note: fHPBEn will be reset after write configuration descriptor */
	if (hba->dev_info.hpb_support) {
		if (!hba->quirk_sel && hba->dev_info.hpb_ver >= 0x0200) {
			UFS_DBG_LOGI("[UFS] fHPBEn set to %d \n", f_hpb_enabled);
			ret = hba->query_flag(hba, UPIU_QUERY_OPCODE_SET_FLAG, 0x12, &f_hpb_enabled); // 0x12 (fHPBEn)
			if (ret)
				UFS_DBG_LOGE("[UFS] error(%d): failed to set fHPBEn to %d\n",ret, f_hpb_enabled);
		} else {
			/* legacy Samsung HPB does not support fHPBEn either */
			UFS_DBG_LOGD("[UFS] fHPBEn is not supported \n");
		}
	}
...
read_feature:
	UFS_DBG_LOGI("Try selector: 0x%x\n", sel);
	/* Check supported features */
	UFS_DBG_LOGI("Dev. Descr. F. Support: 0x%x\n", p[DEVICE_DESC_PARAM_FEAT_SUP]);
	UFS_DBG_LOGI("Dev. Descr. Ext. F. Support: 0x%x 0x%x\n",
			p[DEVICE_DESC_PARAM_EXT_FEAT_SUP + 3], p[DEVICE_DESC_PARAM_EXT_FEAT_SUP + 2]);
	...
	if (card_data->hpb_support) {
		...
		UFS_DBG_LOGI("HPB Version = %x.%x.%x, Control mode = %d\n",
	...
	UFS_DBG_LOGI("ud0_base_offset = %d, ud_config_len = %d\n", card_data->ud0_base_offset, card_data->ud_config_len);

	if ((hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) && (sel == 0x00)) {
		// TODO: remove after EOL of Selen and Santos

		err = ufshcd_init_vendor_desc_sizes(hba);

		if (err) {
			// should failed on Santos
			UFS_DBG_LOGE("[UFS] err: %s: failed reading vendor Device Desc. err = %d\n", __func__ ,err);
			goto save_sel;
		}

		hba->read_descriptor(hba, QUERY_DESC_IDN_DEVICE, 0, 1, p, ufs_query_desc_max_size[QUERY_DESC_IDN_DEVICE]);

		UFS_DBG_LOGI("[UFS] %s: vendor Device Desc:\n", __func__);
		ufs_aio_util_dump_table_hex(p, ufs_query_desc_max_size[QUERY_DESC_IDN_DEVICE]);
		 ...
		 */
		if (p[DEVICE_DESC_PARAM_FEAT_SUP] & 0x80){
			// probably this is legacy device (Selen)
			sel++;
			goto read_feature;
		}
	}
...
save_sel:
	...
	memset(&g_ufs_temp_buf[0], 0, QUERY_DESC_HEALTH_MAX_SIZE);
	ufs_aio_scsi_cmd_inquiry(&cmd, tag, 0, (unsigned long *)&g_ufs_temp_buf[0], 0, 0);
	err = ufshcd_queuecommand(hba, &cmd);
	if (err) {
		UFS_DBG_LOGE("[UFS] err: ufshcd_queuecommand err\n");
		return -1;
	}
	...
	memset(&g_ufs_temp_buf[0], 0, QUERY_DESC_HEALTH_MAX_SIZE);
	ufs_aio_scsi_cmd_inquiry(&cmd, tag, 0, (unsigned long *)&g_ufs_temp_buf[0], 1, 0);
	err = ufshcd_queuecommand(hba, &cmd);
	if (err) {
		UFS_DBG_LOGE("[UFS] err: ufshcd_queuecommand err\n");
		return -1;
	...
	for (i = 4; i < g_ufs_temp_buf[3]; i++) {
		if (g_ufs_temp_buf[i] == 0x80) {
			vpd80 = true;
			break;
		}
	}
	...
	if (vpd80) {
		memset(&g_ufs_temp_buf[0], 0, QUERY_DESC_HEALTH_MAX_SIZE);
		ufs_aio_scsi_cmd_inquiry(&cmd, tag, 0, (unsigned long *)&g_ufs_temp_buf[0], 1, 0x80);
		err = ufshcd_queuecommand(hba, &cmd);
		if (err) {
			UFS_DBG_LOGE("[UFS] err: ufshcd_queuecommand err\n");
			return -1;
		...
		str_len = ufs_util_sanitize_inquiry_string(&p[4], g_ufs_temp_buf[3]);
		card_data->serial_number_len = str_len;
		if (str_len) {
			UFS_DBG_LOGD("[UFS] get unit serial number string len 0x%x\n", str_len);
	...
	UFS_DBG_LOGI("[UFS] quirk_sel: 0x%x\n", hba->quirk_sel);
	UFS_DBG_LOGI("[UFS] vendor id: 0x%x\n", hba->dev_info.wmanufacturerid);
	UFS_DBG_LOGI("[UFS] ufs id: %s\n", hba->dev_info.product_id);
	UFS_DBG_LOGI("[UFS] ufs fwver: %s\n", hba->dev_info.product_revision_level);
	UFS_DBG_LOGI("[UFS] ufs serial number: %s\n", hba->dev_info.serial_number);
	UFS_DBG_LOGI("[UFS] dev_quirk = 0x%x\n", hba->dev_quirks);
...
int ufs_aio_read_unit_desc_cfg_param(struct ufs_hba *hba)
{
	...
	/* HPB support, read configuration descriptor, length = 0xD0(HPB1.0)/0xE2(HPB2.0) */
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, hba->quirk_sel, 
			(u8 *)&g_ufs_temp_buf[0], ufs_query_desc_max_size[QUERY_DESC_IDN_CONFIGURATION]);

	if (ret) {
		UFS_DBG_LOGD("[UFS] err: ufs_aio_read_unit_desc_cfg_param read desc error\n");
		return ret;
	}
...
int ufs_aio_crypto_init(struct ufs_hba *hba)
{
	...
	u32 cap_id = 2; /* AES-CBC-ESSIV-128bit */
