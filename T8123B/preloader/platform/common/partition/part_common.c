...
uint32_t partition_exists(const char *part_name)
{
	uint32_t exist = 0;
	part_t *part = NULL;
			  ...
			  uint32_t op)
{
	...
	char part_name_a[PART_META_INFO_NAMELEN] = {0};
	...
	/* check the target partition with suffix a is exist */
	if (strlen(part_name) < PART_META_INFO_NAMELEN) {
		memcpy(part_name_a, part_name, strlen(part_name));
		memcpy(part_name_a + strlen(part_name), BOOTCTRL_SUFFIX_A, sizeof(BOOTCTRL_SUFFIX_A));
	} else {
		pal_log_err("[%s] wrong name %s is longer than %d\n",
			__func__, part_name, PART_META_INFO_NAMELEN);
		return ERR_PART_GENERAL_INVALID_NAME;
	}
	part_name_a[PART_META_INFO_NAMELEN - 1] = '\0';
	part_exist = partition_exists(part_name_a);
...
part_t *get_part(const char *name)
{
    part_t *part;
    part_t *ret = NULL;
    if (g_part_ready == 1) {
