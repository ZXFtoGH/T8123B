...
u32 crc32(u32 crc, const u8 *p, u32 len, u32* crc32_table)
{
    while (len--) {
        crc ^= *p++;
...
static u64 get_part_ptr_by_gpt(u32 part_id, u64 header_lba, u8 *entries_buf, part_t *part_ptr, const char *part_name, u32 blksz) {
    ...
    if (get_gpt_header(part_id, header_lba, entries_buf, blksz, gpt_sram_crc32_table)) {
        pal_log_err("%s get_gpt_header fail\n", TAG);
        return 1;
    }

    memcpy(header, entries_buf, sizeof(gpt_header));
    num_block_entry = (u64)(blksz / header->sizeof_partition_entry);
    entries_read_size = (u64)header->num_partition_entries * header->sizeof_partition_entry;

    calc_crc = ~0L;
    read_lba = header->partition_entry_lba;
    found = 0;
    while (entries_read_size > 0) {
        int entries_batch_size = 0;
        if (entries_read_size > GPT_SRAM_BUF_SIZE)
            entries_batch_size = GPT_SRAM_BUF_SIZE;
        else // the last partition entries do not aligned buffer suze
            entries_batch_size = entries_read_size;

        err = read_data((u8*)entries_buf, part_id, read_lba, entries_batch_size);
        read_lba = read_lba + entries_batch_size/blksz;
        entries_cnt = GPT_SRAM_BUF_SIZE/header->sizeof_partition_entry;
        calc_crc = efi_crc32(calc_crc, (u8 *)entries_buf, (u32)entries_batch_size, gpt_sram_crc32_table);
        entries_read_size -= entries_batch_size;
        for (i = 0; i < entries_cnt && found == 0; i++) {
            if(!entries[i].starting_lba)
                break;
            get_part_info(part_id, part_ptr, &entries[i]);
            if (!strcmp((char *)part_ptr->info->name, part_name)) {
                found = 1;
                break;
            }
        }
...
static int parse_gpt_header(u32 part_id, u64 header_lba, u8 *header_buf, u8 *entries_buf)
{
    ...
    if ((u64)header->num_partition_entries > PART_MAX_ENTRY_SUPPORT) {
        pal_log_err("%sparse_gpt_header fail, num_partition_entries(%d) overflow\n",
            TAG, (u64)header->num_partition_entries);
        return 1;
    }

    if ((u64)header->sizeof_partition_entry != sizeof(gpt_entry)) {
        pal_log_err("%sparse_gpt_header fail, sizeof_partition_entry(%d) overflow\n",
            TAG, (u64)header->sizeof_partition_entry);
        return 1;
    }
    ...
    if ((u64)entries_read_size > GPT_BUFFER_SIZE) {
        pal_log_err("%sparse_gpt_header fail, entries_read_size(%d) overflow\n",
            TAG, (u64)header->num_partition_entries);
        return 1;
    }
