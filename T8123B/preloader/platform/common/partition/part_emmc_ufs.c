...
static ssize_t partition_op(uint32_t op, const char *part_name, uint64_t offset,
			    uint8_t *buf, uint32_t size)
{
	...
	// Check update range
	if (offset + size > part->nr_sects * blk_sz) {
		pal_log_err("%s: Incorrect range: part: %s total: 0x%llx size: %zu\n",
			__FUNCTION__, part_name, part->nr_sects * blk_sz, size);
		len = -1;
		goto end;
	}
...
ssize_t partition_read(const char *part_name, uint64_t offset, uint8_t *buf,
		       uint32_t size)
{
	ssize_t len = 0;
	uint32_t ret = STATUS_OK;
	...
	active_part_name[input_part_name_sz] = '\0';
...
ssize_t partition_write(const char *part_name, uint64_t offset, uint8_t *buf,
			uint32_t size)
{
	ssize_t len = 0;
	uint32_t ret = STATUS_OK;
...
ssize_t partition_read_inactive(const char *part_name, uint64_t offset, uint8_t *buf,
				uint32_t size)
{
	ssize_t len = 0;
	uint32_t ret = STATUS_OK;
	...
	inactive_part_name[input_part_name_sz] = '\0';
...
ssize_t partition_write_inactive(const char *part_name, uint64_t offset, uint8_t *buf,
				 uint32_t size)
{
	ssize_t len = 0;
	uint32_t ret = STATUS_OK;
