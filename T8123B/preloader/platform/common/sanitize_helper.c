...
#define SANI_SHOW(fmt, ...)	print(fmt, ##__VA_ARGS__)
#define SANI_ABORT()		ASSERT(0)
...
static void ub_val2str(char *str, unsigned size, type_desc *type, void *val)
{
	...
	if (UB_SIGNED(type)) {
		...
		x = ll_x = (extra_bits >= 0 ? ((long)((unsigned long)val << extra_bits)) >> extra_bits : *(long long *)val);
		if (ll_x < 0) {
			x = -ll_x;
			*str++ = '-';
			size--;
...
static void ub_handle_type_mismatch(src_loc *loc, type_desc *type, unsigned long align, unsigned kind, void *ptr)
{
	...
	if (!ptr)
		...
		SANI_SHOW("%s misaligned address 0x%x(type: %s, align: %uB)\n"
			, kinds[kind], (unsigned)ptr, type->name, (unsigned)align);
	else
		SANI_SHOW("%s address 0x%x(type: %s) with insufficient space\n", kinds[kind], (unsigned)ptr, type->name);
	SANI_ABORT();
}
