...
void trustzone_jump_v7(u32 addr, u32 arg1, u32 arg2)
{
    ...
    tee_v7_arg_t_ptr teearg = (tee_v7_arg_t_ptr)trustzone_get_tee_boot_param_addr();
    ...
    teearg->log_port     = CFG_UART_LOG;;            /* UART logging : UART base address. Can be same as preloader's one or not */
    if (g_boot_mode == META_BOOT && g_meta_com_type == META_UART_COM)
        teearg->log_port     = UART2;            /* UART logging : UART base address. Can be same as preloader's one or not */
    teearg->log_baudrate = CFG_LOG_BAUDRATE;     /* UART logging : UART baud rate */
#if CFG_GOOGLE_TRUSTY_SUPPORT
    teearg->gicd_base    = GICD_BASE;                /* ARM GIC Distributor Interface Base Address */
    teearg->gicc_base    = GICC_BASE;                /* ARM GIC CPU Interface Base Address */
...
#if 0
    {
        int i;
        for(i=0; i<32; i+=4)
            DBG_MSG("%s rpmb_key[%d-%d] : 0x%x 0x%x 0x%x 0x%x\n", MOD,
            i, i+3, rpmb_key[i], rpmb_key[i+1],
            rpmb_key[i+2], rpmb_key[i+3]);
    }
#endif
#endif
#endif
