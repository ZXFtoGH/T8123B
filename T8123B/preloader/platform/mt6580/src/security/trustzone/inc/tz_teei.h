...
#ifdef CFG_MICROTRUST_TEE_LITE_FP_SUPPORT
//#define DEBUG_ENABLE
#ifdef DEBUG_ENABLE
	#define DEBUG (1 << 3)
#else
...
typedef struct SPI_BASE_INFO {
    unsigned long int   spi_base;
    unsigned int        pad;
    unsigned int        irq_no;
    unsigned int        devapc_no;
    unsigned int        dma_no;
} __attribute__((packed)) spi_dev_info;

#define SPI_DEV_ATTR(PLATFORM, SPI_NUM, GPIO_NUM, GPIO_MODE, SPI_BASE,  PAD, \
		IRQ, DEVAPC, DMA) \
	{ (unsigned long int)SPI_BASE, \
	(unsigned int)PAD, (unsigned int)IRQ, \
	(unsigned int)DEVAPC, (unsigned int)DMA }

#endif //CFG_MICROTRUST_TEE_LITE_FP_SUPPORT
