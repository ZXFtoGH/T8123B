...
unsigned int is_in_region(struct region *region1, struct region *region2)
{
	...
	if ((region1->start >= region2->start) &&
		((region1->start + region1->size) > (region1->start)) &&
...
U32 sec_region_check(U32 addr, U32 len, U32 is_read)
{
	...
	/* check if end_addr is overflow */
	if (addr + len < addr) {
		SMSG("[%s] %s: addr (0x%x) + len (0x%x) overflow!!\n", MOD, __func__, addr, len);
		return ERR_REGION_INVALID_OVERFLOW;
	}

	if (whitelist_check(addr, len, is_read)) {
		SMSG("[%s] Not Allowed\n", MOD);
		ret = ERR_REGION_INVALID_REGION;
	}
