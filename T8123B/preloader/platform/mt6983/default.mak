...
CFG_APWDT_DISABLE :=0
CFG_APWDT_LVL_IRQ :=1
CFG_APWDT_REQ_MODE :=0x3D00E3
CFG_APWDT_REQ_IRQ :=0x3900E3
CFG_FORCE_RECOVERY :=0
#
...
#
# CFG_DA_SYSTMR7 :=
#   0      : Disable SYSTIMER_7
#   1      : Enable SYSTIMER_7 oneshot timer for DA
#
CFG_SYSTIMER :=1
CFG_DA_SYSTMR7 := 1
CFG_RAM_CONSOLE :=1
...
CFG_BOOT_TAG_VERSION := 1

# 6K: 0x1800
CFG_SYS_STACK_SZ :=0x1800
...
###################################################################
#For Normal Boot
CFG_UBOOT_MEMADDR       :=0x50F00000
#For AEE-LK
CFG_AEE_LK_MEMADDR      :=0x50700000
CFG_AEE_LK_MEMSIZE      :=0x800000
#For Dummy AP
...
CFG_EMI_ADDR2DRAM_VER := 2
CFG_DRM_ENABLE := 1
CFG_LPDDR_ENABLE := 1
CFG_PCDDR_ENABLE := 0

# AOD-SCP support
MTK_AOD_SCP_SUPPORT := no

LOCK_STATE_SECCFG_CUSTOM := yes

ifeq ($(LOCK_STATE_SECCFG_CUSTOM), yes)
C_OPTION += -DLOCK_STATE_SECCFG_CUSTOM=1
else
ifeq ($(LOCK_STATE_CUSTOM), yes)
C_OPTION += -DLOCK_STATE_CUSTOM=1
else
ifeq ($(LOCK_STATE_RPMB), yes)
C_OPTION += -DLOCK_STATE_RPMB=1
endif
endif
endif

MTK_ENABLE_EMI_EBG := yes
