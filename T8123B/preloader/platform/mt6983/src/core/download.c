...
static u16 usbdl_verify_da(u8* da_addr, u32 da_len, u32 sig_len)
{
	...
	if (FALSE == seclib_sec_usbdl_enabled(TRUE)) {
		...
		if (da_len <= sig_len) {
			...
			status = E_DA_LEN_LESS_THAN_SIG_LEN;
			goto end;
		}
...
static void usbdl_send_da(void)
{
	...
	if (da_len > MAX_DA_LEN) {
		pal_log_err("%s Error: DA length overflow.\n", MOD);
		ASSERT(0);
	}
...
static void usbdl_get_cust_info(void)
{
	u16 status = 0;
	u8  da_key[DA_PUBK_SZ] = {DA_PUBK};
	u8  cust_info[CUST_INFO_LEN] = {0};

	/* return hash of da key in default */
	if (CUST_INFO_LEN >= SHA256_HASH_SZ)
		sha256(da_key, DA_PUBK_SZ, cust_info);

	usbdl_put_dword(CUST_INFO_LEN);
	usbdl_put_data(cust_info, CUST_INFO_LEN);
	usbdl_put_word(status);
	...
	while (1) {
		...
		switch (cmd) {
			...
			case CMD_ZEROIZATION:
				...
				usbdl_zeroization();
				break;
			case CMD_CUST_GET_INFO:
				DBGMSG("%s CMD_GET_CUST_INFO\n", MOD);
				/* customer self-defined function */
				usbdl_get_cust_info();
				break;
			default:
