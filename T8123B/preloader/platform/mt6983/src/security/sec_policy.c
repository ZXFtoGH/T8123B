...
unsigned int get_policy_entry_idx(const char *part_name)
{
    ...
    if (!part_name) {
        pal_log_info("[SEC_POLICY] part_name invalid, use default policy\n");
        return 0;
    }

    while (i < num_of_entries) {
        if (g_policy_map[i].part_name1 &&
            (0 == strcmp(part_name, g_policy_map[i].part_name1)))
            break;
        else if (g_policy_map[i].part_name2 &&
            (0 == strcmp(part_name, g_policy_map[i].part_name2)))
            break;
        else if (g_policy_map[i].part_name3 &&
            (0 == strcmp(part_name, g_policy_map[i].part_name3)))
            break;
        else if (g_policy_map[i].part_name4 &&
            (0 == strcmp(part_name, g_policy_map[i].part_name4)))
            break;
        i++;
    }
...
unsigned int get_lock_state_wrapper(unsigned int *lock_state)
{
#if (MTK_SECURITY_SW_SUPPORT)
#if (LOCK_STATE_SECCFG_CUSTOM)
    unsigned int ret = STATUS_OK;
    unsigned int lock_state1 = LKS_LOCK;
    unsigned int lock_state2 = LKS_LOCK;

    ret = get_lock_state(&lock_state1);
    if (ret != STATUS_OK) {
        pal_log_err("SEC Get lock state from seccfg failed!\n");
        *lock_state = LKS_LOCK;
        return ERR_VB_LKS_SECCFG_FAIL;
    }

    ret = custom_get_lock_state(&lock_state2);
    if (ret != STATUS_OK) {
        pal_log_err("[SEC] get lock state from custom failed!\n");
        *lock_state = LKS_LOCK;
        ret = ERR_VB_LKS_CUSTOM_FAIL;
    } else {
        if (lock_state1 == lock_state2) {
            /* Get right state */
            *lock_state = lock_state1;
        } else {
            pal_log_err("[SEC] Get %d from custom, get %d from seccfg, \
                    not equal, default set device lock!\n", lock_state1, lock_state2);
            *lock_state = LKS_LOCK;
            ret = ERR_VB_LKS_NOT_EQUAL;
        }
    }
...
#else
    /* Default */
    return get_lock_state(lock_state);
#endif
#else
    return 0;
#endif
...
static unsigned int get_sec_state(unsigned int *sboot_state, unsigned int *lock_state)
{
    ...
    ret = get_lock_state_wrapper(lock_state);
    if (ret)
