...
static uint8_t sec_img_buf[SEC_IMG_BUFFER_LENGTH] __attribute__ ((aligned(8)));
static uint8_t sec_working_buf[SEC_WORKING_BUFFER_LENGTH] __attribute__ ((aligned(8)));
static uint8_t sec_lib_heap_buf[SEC_LIB_HEAP_LENGTH] __attribute__ ((aligned(8)));
...
uint32_t sec_util_raw_read_from_boot1(uint64_t offset,
			              uint8_t *buf,
			              uint32_t size)
{
	ssize_t len = storage_read(STORAGE_PHYS_PART_BOOT1, offset, buf, size);
