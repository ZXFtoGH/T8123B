...
void trustzone_jump(u64 addr, u32 arg1, u32 arg2)
{
    ...
    DBG_ERR("%s Jump to %s: 0x%x\n\n", MOD, abnormal_boot ? "AEE_LK" : "BL2_EXT",
            abnormal_boot ? BL31_ENTRY : PL_BL2_EXT_BASE);
    if (abnormal_boot)
        exit_pl_to_next(CFG_AEE_LK_MEMADDR, &(g_dram_buf->bl31_info), reserved, reserved, reserved);
    else
        exit_pl_to_next(PL_BL2_EXT_BASE, &(g_dram_buf->bl31_info), reserved, reserved, reserved);
#else
