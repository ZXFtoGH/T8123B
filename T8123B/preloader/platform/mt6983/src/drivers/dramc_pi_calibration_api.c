...
U8 u1MR69Value[CHANNEL_NUM][RANK_MAX][FSP_MAX];
...
S8 gFinalMCK16XDuty[CHANNEL_NUM][DQS_NUMBER+1];
U16 gFinalMCK16XDuty_16[CHANNEL_NUM][DQS_NUMBER+1];
#endif

U8 gFinalCBTVrefDQ[CHANNEL_NUM][RANK_MAX];
U8 gFinalRXVrefDQ[CHANNEL_NUM][RANK_MAX][2];
U8 gFinalRXVrefDQ_perbit[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH];
U8 gFinalTXVrefDQ[CHANNEL_NUM][RANK_MAX];

U8 gFinalRXVrefDQForSpeedUp[CHANNEL_NUM][RANK_MAX][2/*ODT_onoff*/][2/*2bytes*/] = {0};
U8 gFinalRXVrefDQForSpeedUp_perbit[CHANNEL_NUM][RANK_MAX][2/*ODT_onoff*/][DQ_DATA_WIDTH] = {0};
...
#if defined(RELEASE) ||(CFG_DRAM_LOG_TO_STORAGE)
...
U16 gHQALOG_Vmddr_delay_cell_ps = 0;
...
#endif
U16 u2Delay_open_mode_bk[TX_DQ_DQS_CALTYPE_MAX][DQS_NUMBER] = {0};
U16 u2DQM_Delay_open_mode_bk[TX_DQ_DQS_CALTYPE_MAX][DQS_NUMBER] = {0};

#if ENABLE_RX_DVS_CAL
static U8 DVS_final_delay[RANK_MAX][DQS_NUMBER];
#endif
...
#if ENABLE_RDCC_CAL
static S8 RDCC_final_delay[RANK_MAX][DQS_NUMBER];
#endif

JMETER_DELAYCELL_T JMeter_DelayCell_Table[DRAM_DFS_SRAM_MAX];
U32 gDramcSwImpedanceResult[IMP_MAX_OHM][IMP_DRV_MAX] = {0};//ODT_ON/OFF x DRVP/DRVN/ODTP/ODTN
...
void Fast_K_CheckResult(DRAMC_CTX_T *p, U8 ucCalType)
{
    ...
    {
        DramcEngine2Init(p, &cfg, TA2_SEL_DEFAULT_CFG);
        CheckResult = DramcEngine2Run(p, &cfg);
        DramcEngine2End(p);
        FastK_Check_flag=1;
    }
    ...
    {
        DramcEngine2Init(p, &cfg, TA2_SEL_GATING_CFG);
...
// yr: calibration summary must print, use mcSHOW_MSG()
    ...
    //for(ucFreqIdx=0; ucFreqIdx<DRAM_DFS_SRAM_MAX; ucFreqIdx++)
    {
        ...
        for(ucCHIdx=0; ucCHIdx<p->support_channel_num; ucCHIdx++)
        {
            for(ucRankIdx=0; ucRankIdx<p->support_rank_num; ucRankIdx++)
            {
                ...
                mcSHOW_MSG("CH %d, Rank %d\n", ucCHIdx, ucRankIdx);
                //mcSHOW_MSG("[vPrintCalibrationResult] Channel = %d, Rank= %d, Freq.= %d, (ucCalExecute_All 0x%x, ucCalResult_All 0x%x)\n", ucCHIdx, ucRankIdx, ucFreqIdx, ucCalExecute_All, ucCalResult_All);
                ...
                for(ucCalIdx =0; ucCalIdx<DRAM_CALIBRATION_MAX; ucCalIdx++)
                {
                    ...
                    mcSHOW_MSG("%s: ", szCalibStatusName[ucCalIdx]);
...
#if 0 /* Global MR initialized in LPx_dram_init.c */
...
#if MRW_CHECK_ONLY
    memset((void *)u2MRRecord, 0xff, sizeof(u2MRRecord));
#endif
...
void vBeforeCalibration(DRAMC_CTX_T *p)
{
    ...
    /* Update B1 imp update condition and disable B2 update since not used */
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_MISC_SW_IMPCAL,
        ...
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQS2_DRVN_UPD_DIS) |
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQS2_DRVP_UPD_DIS) |
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQS2_ODTN_UPD_DIS) |
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQ2_DRVN_UPD_DIS) |
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQ2_DRVP_UPD_DIS) |
        P_Fld(0x1, SHU_MISC_SW_IMPCAL_DQ2_ODTN_UPD_DIS));
    ...
    else if (p->support_channel_num == CHANNEL_DUAL)
    {
        ...
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_ZQ_SET0), 0, ZQ_SET0_ZQCSMASK);
        channel_restore(p);
    ...
    else if (p->support_channel_num == CHANNEL_FOURTH)
    {
        ...
        // if CHA.ZQCSMASK=1, and then set CHA.ZQCALDISB=1 first, else set CHB.ZQCALDISB=1 first
	#if (fcFOR_CHIP_ID == fcMT8139 || fcFOR_CHIP_ID == fcFigeac)
        channel_backup_and_set(p, CHANNEL_A);

        do {
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_ZQ_SET0), u1ZQCsMask[p->channel], ZQ_SET0_ZQCSMASK);
            channel_advance(p);
...
#if SUPPORT_TYPE_LPDDR5 && defined(ZQCS_ENABLE_LP5)
	if (is_lp5_family(p)) {
...
void vAfterCalibration(DRAMC_CTX_T *p)
{
...
#if ENABLE_READ_LECC
    vSwitchRDBI_RECC_Setting(p, p->ECC_R_onoff, READ_OPT_CHG_ECC, TO_MR_ONLY, FALSE);
#endif

#if ENABLE_READ_DBI
    vSwitchRDBI_RECC_Setting(p, p->DBI_R_onoff[p->dram_fsp], READ_OPT_CHG_DBI, TO_MR_ONLY, FALSE);
#endif
    ...
    ENABLE_SingleEndEnable(p, ENABLE, TO_BOTH_RG_MR, TO_ALL_CHANNEL); //for last Freq is SE or single Freq
#endif
    ...
    channel_backup_and_set(p, CHANNEL_A);
    do {
        DIG_HW_NONSHUF_DQSOSC_CFG(p, &HW_DQSOSC_CTRL);
        channel_advance(p);
    } while (!channel_process_complete(p));
    channel_restore(p);
#endif
...
void RxPathOnOff(DRAMC_CTX_T *p, U8 u1OnOff)
{
...
#if APPLY_NEW_IO_API
    // DQ_IN_BUFF_EN
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ3),
                        PB_Fld(u1OnOff, B0_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B0),
                        PB_Fld(u1OnOff, B0_DQ3_RG_RX_ARDQS0_IN_BUFF_EN_B0));
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ3),
                        PB_Fld(u1OnOff, B1_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B1),
                        PB_Fld(u1OnOff, B1_DQ3_RG_RX_ARDQS0_IN_BUFF_EN_B1));
#else
    // DQ_IN_BUFF_EN
    ...
    //RX_ARDQS_DLY_LAT_EN
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ10), !u1OnOff, SHU_B0_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ10), !u1OnOff, SHU_B1_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B1);

    //RX_ARDQS_DQSIEN_FORCE_ON_EN
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ10), u1OnOff, SHU_B0_DQ10_RG_RX_ARDQS_DQSIEN_FORCE_ON_EN_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ10), u1OnOff, SHU_B1_DQ10_RG_RX_ARDQS_DQSIEN_FORCE_ON_EN_B1);

    //ARDQ_RANK_SEL
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ11), !u1OnOff, SHU_B0_DQ11_RG_RX_ARDQ_RANK_SEL_SER_EN_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ11), !u1OnOff, SHU_B1_DQ11_RG_RX_ARDQ_RANK_SEL_SER_EN_B1);
...
void O1PathOnOff(DRAMC_CTX_T *p, U8 u1OnOff)
{
    ...
    if (u1OnOff == ON)
    {
        ...
        if (p->dram_type == TYPE_LPDDR4) {
            ...
            mcSHOW_ERR_MSG("%s: Invalid mem_type %d\n", __func__, p->dram_type);
        #if __ETT__
            ASSERT(0);
        #endif
        }
...
static u8 get_cbt_autok_sweep_max_cnt(DRAMC_CTX_T *p, u8 pi_dly, u8 lenpi, u8 p2u)
{
	u8 max;
...
#if CBT_CHECK_DEBUG_INFO /* V24 new */
static void cbt_check_debug_info(DRAMC_CTX_T *p, U8 pin_num, U8 respi)
{
	U8 first_pass, last_pass;
	U8 u1PassWinCnt;
	U8 u1EBFlag;
	U8 u1WinIdx, u1Pin;

	u1EBFlag = u4IO32ReadFldAlign(DRAMC_REG_CBT_WLEV_STATUS, CBT_WLEV_STATUS_CBT_WLEV_ATK_EB_FLAG);

	mcSHOW_DBG_MSG("%s: %s: Early break flag: %d\n", __func__, __CHRK__, u1EBFlag);
	for (u1Pin = 0; u1Pin < pin_num; u1Pin++) {
		u1PassWinCnt = (u4IO32Read4B(DRAMC_REG_CBT_WLEV_ATK_6PW_RESULT0 + (u1Pin / 8) * 4) >> ((u1Pin % 8) * 4)) & 0xf;

		mcSHOW_DBG_MSG("CA%d PassWinCnt: %d\n", u1Pin, u1PassWinCnt);
		for (u1WinIdx = 0; u1WinIdx < 5; u1WinIdx++) {
			U32 len = u4IO32Read4B(DRAMC_REG_CBT_WLEV_ATK_6PW_PW1_RESULT0 + u1WinIdx * 0x20 + (u1Pin / 4) * 4);
			U32 ini = u4IO32Read4B(DRAMC_REG_CBT_WLEV_ATK_6PW_PW1_RESULT4 + u1WinIdx * 0x20 + (u1Pin / 4) * 4);
			first_pass = ((ini >> ((u1Pin % 4) * 8)) & 0xff) << respi;
			last_pass = (first_pass + ((len >> ((u1Pin % 4) * 8)) & 0xff)) << respi;

			mcSHOW_DBG_MSG("\tWin[%d]: FirstPass: %d, LastPass: %d\n", u1WinIdx, first_pass, last_pass);
		}
	}
}
#endif
	...
	*/
	...
	case AUTOK_CA:
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL1),
				...
				P_Fld(CBT_AUTOK_EB_THRESHOLD >> respi, CBT_WLEV_ATK_CTRL1_CBT_WLEV_ATK_EBLEN));
		break;
	case AUTOK_DQS:
	...
	case AUTOK_CS:
		...
		break;
	case AUTOK_CA:
#if CBT_CHECK_DEBUG_INFO /* Read before EN = 0 */
		cbt_check_debug_info(p, pin_num, respi);
#endif
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL1),
			...
			P_Fld(0x0, CBT_WLEV_ATK_CTRL1_CBT_WLEV_ATK_EBEN));
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL0), 0, CBT_WLEV_ATK_CTRL0_CATRAIN_ATKEN);
		break;
	case AUTOK_DQS:
...
#if SUPPORT_TYPE_LPDDR5
	if (is_lp5_family(p))
		return set_cbt_wlev_intv_lp5(p);
#endif
#if SUPPORT_TYPE_LPDDR4
    if (is_lp4_family(p))
		return set_cbt_wlev_intv_lp4(p);
#endif
}
...
void get_cmd_delay(DRAMC_CTX_T *p, struct CmdDelay_t *delay)
{
	...
	delay->u4CaPI = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), SHU_RK_CA_CMD_RG_ARPI_CMD);
	...
	delay->u4CsPI = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), SHU_RK_CA_CMD_RG_ARPI_CS);

	delay->u4ClkPI = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), SHU_RK_CA_CMD_RG_ARPI_CLK);
}
...
static void CBTDelayCA(DRAMC_CTX_T *p, struct CmdDelay_t *delay, S32 iDelay)
{
	if (iDelay < 0)
	{   /* Set CLK delay */
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD),
			P_Fld(0, SHU_RK_CA_CMD_RG_ARPI_CMD) |
			P_Fld(delay->u4ClkPI - iDelay, SHU_RK_CA_CMD_RG_ARPI_CLK) |
			P_Fld(delay->u4CsPI - iDelay, SHU_RK_CA_CMD_RG_ARPI_CS));
	}
	else
	{   /* Set CA output delay */
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD),
			P_Fld(iDelay, SHU_RK_CA_CMD_RG_ARPI_CMD) |
			P_Fld(delay->u4ClkPI, SHU_RK_CA_CMD_RG_ARPI_CLK) |
			P_Fld(delay->u4CsPI, SHU_RK_CA_CMD_RG_ARPI_CS));
	}
}
...
//#if (FOR_DV_SIMULATION_USED == 0)
	...
	{
		vIO32WriteFldMulti_All(DRAMC_REG_CMD_DEC_CTRL0, P_Fld(u1Force0, CMD_DEC_CTRL0_CS0FORCE0_SRK)
				| P_Fld(u1Force0, CMD_DEC_CTRL0_CS1FORCE0_SRK));
	}
	else
	{
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CMD_DEC_CTRL0), P_Fld(u1Force0, CMD_DEC_CTRL0_CS0FORCE0_SRK)
				| P_Fld(u1Force0, CMD_DEC_CTRL0_CS1FORCE0_SRK));
	}
//#endif
}
...
DRAM_RANK_TERM_ATTR_T get_rank_term_attr(DRAMC_CTX_T *p)
{
    DRAM_RANK_TERM_ATTR_T ret = RANK_TERM;

#if (FSP1_CBT_FOLLOW_SPEC == 0)
    return ret;
#endif

#if SUPPORT_TYPE_LPDDR5
    if (is_lp5_family(p) || (p->support_rank_num == RANK_SINGLE))
        return ret;
#endif

    /* Note, MR0[7] CATR is vendor specific.
     * For LP4X, it's may be RFU. For LP4, it relies on other conditions
     * and my not reflect real TERM status.
     * Seems that vendor all tie ODT_CA pad of RK1 to VSS,
     * Take R0 as termed rank is safe
     */
    ret = (rank_get(p) == RANK_0) ? RANK_TERM : RANK_UNTERM;
...
#if SUPPORT_TYPE_LPDDR4
    if (is_lp4_family(p))
        return (u8 *)uiLPDDR_O1_Mapping_POP[p->channel];
#endif
...
/* APHY RG to TA2 */
static u8 *GetPadTa2MappingTable(DRAMC_CTX_T *p)
{
    return (u8 *)uiLPDDR_PAD_TA2_Mapping_POP[p->channel];
}
...
static void vSetDramMRCBTOnOff(DRAMC_CTX_T *p, U8 u1OnOff, U8 operating_fsp)
{
    if (u1OnOff)
    {
        ...
        MRWriteFldMulti(p, 13, P_Fld(!operating_fsp, MR13_FSP_OP) |
                               P_Fld(operating_fsp, MR13_FSP_WR) |
                               P_Fld(1, MR13_CBT),
                               TO_MR);
...
static void CBTEntryLP4(DRAMC_CTX_T *p, U8 operating_fsp)
{
	...
	/* Switch TERM rank to FSP1 to share term if is calibrating UNterm rank */
	if ((get_rank_term_attr(p) == RANK_UNTERM) && (operating_fsp == FSP_1)) {
		u1MR13Value[!p->rank] |= (1 << Fld_shft(MR13_FSP_OP));
		DramcModeRegWriteByRank(p, !p->rank, 13, u1MR13Value[!p->rank]);

		/* HW limitation: After entering CBT, if MRW to MR13 with CBT set 0,
		 * HW will leave CBT status, cause misbehaviour.
		 * Here cheat DRAMC of entering CBT again but do not really send command out
		 */
...
static void CBTExitLP4(DRAMC_CTX_T *p, U8 operating_fsp)
{
    ...
    /* Switch TERM rank to FSP0 to disable term */
    if ((get_rank_term_attr(p) == RANK_UNTERM) && (operating_fsp == FSP_1)) {
        u1MR13Value[!p->rank] &= ~(1 << Fld_shft(MR13_FSP_OP));
        DramcModeRegWriteByRank(p, !p->rank, 13, u1MR13Value[!p->rank]);
    }
...
static void put_ca_delay(DRAMC_CTX_T *p, struct CmdDelay_t *delay)
{
	...
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), delay->u4CaPI, SHU_RK_CA_CMD_RG_ARPI_CMD);
}

static void put_cs_delay(DRAMC_CTX_T *p, struct CmdDelay_t *delay)
{
	put_cs_mck_ui(p, delay->u1CsMCK, delay->u1CsUI);
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), delay->u4CsPI, SHU_RK_CA_CMD_RG_ARPI_CS);
}
...
void put_cmd_delay(DRAMC_CTX_T *p, struct CmdDelay_t *delay)
{
	...
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), delay->u4ClkPI, SHU_RK_CA_CMD_RG_ARPI_CLK);
}
...
{
	U8 u1CA;
	S8 iCA_PerBit_DelayLine[11] = {0};

	for (u1CA = 0;u1CA < ca_pin_num;u1CA++)
	{
		iCA_PerBit_DelayLine[u1CA] = iCAFinalCenter[u1CA];
		#ifdef FOR_HQA_REPORT_USED
		if(gHQALog_flag)
		{
			HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0_2, "CA", "_CA_Mapping", u1CA, u1CA, NULL);
		}
		#endif
	}

	// Set CA perbit delay line calibration results
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_TXDLY0),
			P_Fld(iCA_PerBit_DelayLine[0], SHU_RK_CA_TXDLY0_TX_ARCA0_DLY) |
			P_Fld(iCA_PerBit_DelayLine[1], SHU_RK_CA_TXDLY0_TX_ARCA1_DLY) |
			P_Fld(iCA_PerBit_DelayLine[2], SHU_RK_CA_TXDLY0_TX_ARCA2_DLY) |
			P_Fld(iCA_PerBit_DelayLine[3], SHU_RK_CA_TXDLY0_TX_ARCA3_DLY));

	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_TXDLY1),
			P_Fld(iCA_PerBit_DelayLine[4], SHU_RK_CA_TXDLY1_TX_ARCA4_DLY) |
			P_Fld(iCA_PerBit_DelayLine[5], SHU_RK_CA_TXDLY1_TX_ARCA5_DLY) |
			P_Fld(iCA_PerBit_DelayLine[6], SHU_RK_CA_TXDLY1_TX_ARCA6_DLY) |
			P_Fld(iCA_PerBit_DelayLine[7], SHU_RK_CA_TXDLY1_TX_ARCA7_DLY));

	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_TXDLY5),
			P_Fld(iCA_PerBit_DelayLine[8], SHU_RK_CA_TXDLY5_TX_ARCA8_DLY) |
			P_Fld(iCA_PerBit_DelayLine[9], SHU_RK_CA_TXDLY5_TX_ARCA9_DLY) |
			P_Fld(iCA_PerBit_DelayLine[10], SHU_RK_CA_TXDLY5_TX_ARCA10_DLY));
}
...
static void switch_oe_tie_byte(DRAMC_CTX_T *p, u8 tie_val, u8 dq7, u8 dqm, u8 byte)
{
	u16 pad_dq_mask, pad_dqm_tie;
	u8 dq_oe;

	pad_dq_mask = (((1 << dq7) | (1 << dqm)) >> (byte * 8)) & 0xFF;
	pad_dqm_tie = (dq7 >= DQ_DATA_WIDTH || dqm >= DQ_DATA_WIDTH) ? 1 : 0;
	...
	if (byte) {
        ...
        //DQ0-8 same group, DQ8 will use DQ_OE_TIE_SEL
		if (pad_dqm_tie) {
			vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ16), tie_val, SHU_B1_DQ16_RG_TX_ARDQ8_OE_TIE_EN_B1);
		}
	} else {
        ...
        //DQ0-8 same group, DQ8 will use DQ_OE_TIE_SEL
		if (pad_dqm_tie) {
			vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ16), tie_val, SHU_B0_DQ16_RG_TX_ARDQ8_OE_TIE_EN_B0);
		}
	}
}
...
static void switch_oe_tie(DRAMC_CTX_T *p, u8 sw)
{
    ...
    if(uiLPDDR_TX_Mapping==NULL)
        ASSERT(0);

	dq7_b0 = uiLPDDR_TX_Mapping[7];
	dq7_b1 = uiLPDDR_TX_Mapping[15];
	dqm_b0 = uiLPDDR_TX_Mapping[DQ_DATA_WIDTH];
	dqm_b1 = uiLPDDR_TX_Mapping[DQ_DATA_WIDTH + 1];
	...
	dq_b1 = 1;
#else
	dq_b0 = (dq7_b0 < 8 || dq7_b0 == DQ_DATA_WIDTH /* DQM0 */) ? 0 : 1;
	dq_b1 = (dq7_b1 < 8 || dq7_b1 == DQ_DATA_WIDTH /* DQM0 */) ? 0 : 1;
#endif

	if (sw) {
		/* Set DRAM Byte 0 */
		switch_oe_tie_byte(p, 1, dq7_b0, dqm_b0, dq_b0);

		/* Set DRAM Byte 1 */
		if (is_byte_mode(p)) {
			/* Set DRAM Byte 0 */
			switch_oe_tie_byte(p, 1, dq7_b1, dqm_b1, dq_b1);
		}
	} else {
		/* Set DRAM Byte 0 */
		switch_oe_tie_byte(p, 0, dq7_b0, dqm_b0, dq_b0);

		/* Set DRAM Byte 1 */
		if (is_byte_mode(p)) {
			/* Set DRAM Byte 0 */
			switch_oe_tie_byte(p, 0, dq7_b1, dqm_b1, dq_b1);
		}
	}
}
...
#if SUPPORT_TYPE_LPDDR4 && !CBT_MOVE_CA_INSTEAD_OF_CLK
	if (is_lp4_family(p))
	{
		/* LPDDR4 */
		if ((vGet_DDR_Loop_Mode(p) == SEMI_OPEN_LOOP_MODE)||(vGet_DDR_Loop_Mode(p) == SEMI_OPEN_LOOP_MODE_CAONLY))
		{
...
#endif
	{
		...
		for (s2Delay = ScanDelay.s2DelayStart; s2Delay <= ScanDelay.s2DelayEnd; s2Delay += ScanDelay.u1DelayStep)
		{
			...
			if (autok) {
				...
				ScanDelay.u1DelayStep = get_cbt_autok_sweep_max_cnt(p, pi_dly,
...
#endif
			{
				 ...
				 */
				//Set CS output delay
				vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_CA_CMD), pi_dly, SHU_RK_CA_CMD_RG_ARPI_CS);
...
DRAM_STATUS_T CBTSingleRankLP45(DRAMC_CTX_T *p, int autok)
{
	...
	U32 u4RegBackupAddress[] =
	{
		...
		(MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_CTRL2)),
		(MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_CTRL3)),
		(MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_CTRL4)),
		(MIX_RG_CHECK(DRAMC_REG_SWCMD_CTRL0)),
		(MIX_RG_CHECK(DRAMC_REG_REFCTRL0)),

        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_VREF)),           //in O1PathOnOff()
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_VREF)),           //in O1PathOnOff()
#if CHIP_PROCESS <= 5
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL0)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL0)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL1)),
#endif
	};

#if CHIP_PROCESS > 5
    /* in O1 */
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_SHU_RK_B0_PHY_VREF_SEL, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B0},
        {DDRPHY_REG_SHU_RK_B0_PHY_VREF_SEL, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B0},
        {DDRPHY_REG_SHU_RK_B1_PHY_VREF_SEL, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B1},
        {DDRPHY_REG_SHU_RK_B1_PHY_VREF_SEL, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B1},
    };
#endif
	...
	mcSHOW_DBG_MSG("[CmdBusTrainingLP45] autok=%d\n", autok);
...
#if CHIP_PROCESS > 5
    DramcBackupMixedRG(p, u4MixRgBackupList, ARRAY_SIZE(u4MixRgBackupList), TO_ONE_CHANNEL);
#endif
...
#endif
	{
		...
		for (u1VrefRange = ScanVref.u1VrefRangeStart; u1VrefRange <= ScanVref.u1VrefRangeEnd; u1VrefRange++)
		{
			...
			if (u1CBTEyeScanEnable)
			{
				if (is_lp4_family(p) && u1VrefRange == 1)
				{
...
#if SUPPORT_TYPE_LPDDR4 && ENABLE_LP4Y_WA && SE_BACKUP_SOLUTION //@Darren, debugging for DFS stress
    if (is_lp4_family(p))
	{
...
#if SUPPORT_TYPE_LPDDR4 && FSP1_CBT_FOLLOW_SPEC
	if (is_lp4_family(p))
	{
		if (p->dram_fsp == FSP_1)
		{
...
#if CHIP_PROCESS > 5
    DramcRestoreMixedRG(p, u4MixRgBackupList, ARRAY_SIZE(u4MixRgBackupList), TO_ONE_CHANNEL);
#endif
...
{
    DRAM_STATUS_T ret = 0;
    U8 rank, rank_bak, ranks_processed = 0;
    DRAM_RANK_TERM_ATTR_T processing_rank = RANK_TERM;
    ...
    do {
        for (rank = RANK_0; rank < p->support_rank_num; rank++) {
            ...
            /* TERMed rank first */
            if (processing_rank == get_rank_term_attr(p)) {
            #if PINMUX_AUTO_TEST_PER_BIT_CA
                CheckCAPinMux(p);
            #endif

                ret |= CBTSingleRankLP45(p, autok);

            #if ENABLE_EYESCAN_GRAPH
                print_EYESCAN_LOG_message(p, 0);
            #endif
    ...
    } while (ranks_processed < p->support_rank_num);
...
void ShiftDQSWCK_UI_AllRK(DRAMC_CTX_T *p, S8 iShiftUI, BYTES_T eByteIdx, DRAM_SET_RANK_TYPE_T u1SetRkType)
{
    U8 u1BackupRank, u1RankIdx;
    U8 u1RankMax = (u1SetRkType == BY_SUPPORT_RANK_NUM)? p->support_rank_num: RANK_MAX;
    u1BackupRank = u1GetRank(p);

    for (u1RankIdx = RANK_0; u1RankIdx < u1RankMax; u1RankIdx++)
    {
        vSetRank(p, u1RankIdx);
...
U8 u1IsPhaseMode_DQ(DRAMC_CTX_T *p)
{
    DDR_LOOP_MODE_T loop_mode = vGet_DDR_Loop_Mode(p);

    if ((loop_mode == OPEN_LOOP_MODE) || (loop_mode == SEMI_OPEN_LOOP_MODE))
        return TRUE;
    else // DDR800_CLOSE_LOOP and NORMAL_CLOSE_LOOP
        return FALSE;
}

U8 u1IsPhaseMode_CA(DRAMC_CTX_T *p)
{
    DDR_LOOP_MODE_T loop_mode = vGet_DDR_Loop_Mode(p);

    if ((loop_mode == OPEN_LOOP_MODE) || (loop_mode == SEMI_OPEN_LOOP_MODE_CAONLY) || (loop_mode == SEMI_OPEN_LOOP_MODE))
        return TRUE;
    else // DDR800_CLOSE_LOOP and NORMAL_CLOSE_LOOP
        return FALSE;
}
...
static U8 FetchRGSettingVal(S8 step_val, U8 u1SignBit)
{
    if (step_val <= 0)
        return (U8)(-step_val);
    else
        return ((U8)step_val | (0x1<<u1SignBit));
}

static U8 InvertRGSettingVal(U8 u1RG_val, U8 u1SignBit)
{
    S8 s1DelayValue;

    if (((u1RG_val>>u1SignBit) & 0x1) == 1)
        s1DelayValue = (-1)*(u1RG_val & ~(0x1<<u1SignBit));
    else
        s1DelayValue = u1RG_val;
...
{
    U8 u1DlyNum;
    U8 u1ReservePiNum = 12;

    if (p->u2DelayCellTimex100 != 0)
    {
        u1DlyNum = (u1ReservePiNum * 100000000 / (DDRPhyGetRealFreq(p) << 6)) / p->u2DelayCellTimex100;
        mcSHOW_DBG_MSG3("GetReserveDlyNum by PI (%d PI = %d DLY)\n", u1ReservePiNum, u1DlyNum);
    }
    else
    {
        if (p->frequency >= 3750)
        ...
        {
            u1DlyNum = 10;
            mcSHOW_ERR_MSG(("GetReserveDlyNum unexpect freq, Please check\n"));
            #if (__ETT__)
            ASSERT(0);
            #endif
        }
        ...
        #if CHIP_PROCESS == 6
        //Only for reserve(use default value)
        #elif CHIP_PROCESS == 5
        u1DlyNum = (u1DlyNum*3)/2;
        #else // NEW CHIP_PROCESS
            mcSHOW_ERR_MSG(("GetReserveDlyNum NEW CHIP_PROCESS, Please check\n"));
            #if (__ETT__)
            ASSERT(0);
            #endif
        #endif

        mcSHOW_DBG_MSG3("GetReserveDlyNum by assigned Value (%d DLY)\n", u1DlyNum);
    }
    return u1DlyNum;
}
...
static void vSetWlevDly(DRAMC_CTX_T *p, U16 u2Delay, BYTES_T eByteIdx, WLEV_DELAY_BASED_T stDelayBase)
{
    ...
    else // stDelayBase == DLY_BASED // Adjust DQS(WCK) DLY output delay.
    {
        if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
        {
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY3), u2Delay, SHU_RK_B0_TXDLY3_TX_ARWCK_DLY_B0);
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY3), u2Delay, SHU_RK_B0_TXDLY3_TX_ARWCKB_DLY_B0);
        }
        if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
        {
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY3), u2Delay, SHU_RK_B1_TXDLY3_TX_ARWCK_DLY_B1);
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY3), u2Delay, SHU_RK_B1_TXDLY3_TX_ARWCKB_DLY_B1);
        }
    }
}
...
static U8 GetWlevDly(DRAMC_CTX_T *p, BYTES_T eByteIdx, WLEV_DELAY_BASED_T stDelayBase)
{
    ...
    if (stDelayBase == PI_BASED)
    {
        if ((eByteIdx== BYTE_0))
        {
            u1WlevDly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_ARPI_PBYTE_B0);
        }
        if ((eByteIdx== BYTE_1))
        {
            u1WlevDly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_ARPI_PBYTE_B1);
        }
    }
    else // stDelayBase == DLY_BASED
    {
        if ((eByteIdx== BYTE_0))
        {
            u1WlevDly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY3), SHU_RK_B0_TXDLY3_TX_ARWCK_DLY_B0);
        }
        if ((eByteIdx== BYTE_1))
        {
            u1WlevDly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY3), SHU_RK_B1_TXDLY3_TX_ARWCK_DLY_B1);
        }
    }
...
static DRAM_STATUS_T WLEV_AUTOK(DRAMC_CTX_T *p, U16 u2DlyBegin, U16 u2DlyEnd, U8 u1StepSize, U16 PI_bound, U16* wrlevel_dqs_delay, WLEV_DELAY_BASED_T stDelayBase)
{
    ...
    U8 u1DelayStep = get_wlev_autok_sweep_max_cnt(p, u1Initpi, u2DlyEnd - u2DlyBegin + 1, 32);
    U8 u1Lenpi = (u1DelayStep- 1) >> u1Respi;
    U16 wrlevel_dqs_delay_temp;
...
static DRAM_STATUS_T WLEV_SWK(DRAMC_CTX_T *p, U16 u2DlyBegin, U16 u2DlyEnd, U8 u1StepSize, U16 PI_bound, U16* wrlevel_dqs_delay, WLEV_DELAY_BASED_T stDelayBase)
{
    ...
    for (u2Delay = u2DlyBegin; u2Delay <= u2DlyEnd; u2Delay += u1StepSize)
    {
        if (stDelayBase == PI_BASED)
        {
            ...
            if ((u2Delay / PI_bound) == (u1OverBoundCnt + 1))
            {
                ...
                ShiftDQSWCK_UI_AllRK(p, PI_bound / DQPI_PER_UI, ALL_BYTES, BY_RANK_MAX);
                #else
        ...
        // Read result
        #if 0 //print O1 result for debug
        u4dq_o1 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQO1), MISC_DQO1_DQO1_RO);
        mcSHOW_DBG_MSG2("  O1(%X)  ", u4dq_o1);
        #endif
        ...
        if(NeedByteSwap(p))
        {
            u1CmpResult[BYTE_0] = (U8)((u1HW_cmp_raw_data >> BYTE_1) & 0x1);
            u1CmpResult[BYTE_1] = (U8)((u1HW_cmp_raw_data >> BYTE_0) & 0x1);
        }
        else
        {
            u1CmpResult[BYTE_0] = (U8)((u1HW_cmp_raw_data >> BYTE_0) & 0x1);
            u1CmpResult[BYTE_1] = (U8)((u1HW_cmp_raw_data >> BYTE_1) & 0x1);
        }

        // Deal with u1CmpResult no matter which is from O1 result or CMP_ERR.
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            ...
            if ((u1DoneFlg & (0x01 << u1ByteIdx)) == 0)// result not found of byte yet
            {
                ...
                #endif
                {
                    ...
                    {
                        wrlevel_dqs_delay[u1ByteIdx] = u2Delay - (u1Status[u1ByteIdx] - 2) * u1StepSize;
                        u1DoneFlg |= (0x01 << u1ByteIdx);
                    }
                }
            }
    ...
    {
        #if 1//(fcFOR_CHIP_ID == fcA60892)
        ShiftDQSWCK_UI_AllRK(p, -u1OverBoundCnt * (PI_bound / DQPI_PER_UI), ALL_BYTES, BY_RANK_MAX);
        #else
...
DRAM_STATUS_T DramcWriteLeveling(DRAMC_CTX_T *p, u8 isAutoK, WLEV_DELAY_BASED_T stDelayBase)
{
    ...
    DRAM_STATUS_T KResult = DRAM_FAIL;
#if SUPPORT_TYPE_LPDDR4 && WLEV_AUTOK_LP4_TA2TOGGLE_WA
    TA2_CFG_T ta2_cfg;
#endif
    U8 u1value = 0;
    U8 u1UIShift_num = 1;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_VREF)),           //in O1PathOnOff()
        (MIX_RG_CHECK(DRAMC_REG_DRAMC_PD_CTRL)),
        (MIX_RG_CHECK(DRAMC_REG_DCM_SUB_CTRL)),
        (MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_ATK_CTRL0)),
        (MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_ATK_CTRL1)),
        (MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_CTRL4)),
        (MIX_RG_CHECK(DRAMC_REG_CBT_WLEV_CTRL6)),
    #if CHIP_PROCESS <= 5
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL0)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL0)),
    ...
    if (p->isWLevInitShift[p->channel] == FALSE)// This flow would be excuted just one time, so all ranks(maybe rank0/1) should be adjusted at once.
    {
        ...
        #if 0// SUPPORT_TYPE_LPDDR5
        if (is_lp5_family(p) && (p->CKR==4))
            u1UIShift_num = 2;
        #endif

        #if SUPPORT_TYPE_LPDDR5 && WLEV_DONT_SHIFT_UI_ENABLE
        ...
        ShiftDQSWCK_UI_AllRK(p, -u1UIShift_num, ALL_BYTES, BY_RANK_MAX);
    }
    ...
    if ((stDelayBase == PI_BASED) && (isAutoK ==TRUE) && (u1IsPhaseMode_DQ(p)))
    {
    ...
    {
        #if SUPPORT_TYPE_LPDDR4 && WLEV_AUTOK_LP4_TA2TOGGLE_WA
        if (is_lp4_family(p) && (isAutoK == TRUE))
        {
            vSetRank(p, RANK_0);
            DramcEngine2Init(p, &ta2_cfg, TA2_SEL_DEFAULT_CFG);
            DramcEngine2Run(p, &ta2_cfg);
            DramcEngine2End(p);
            vSetRank(p, backup_rank);
        }
        #endif
        ...
        #if IPM_VERSION >= 24 && SUPPORT_TYPE_LPDDR5
        if (is_lp5_family(p))
        {
            if (u1GetRank(p) == RANK_0)
            {
    ...
    if (KResult == DRAM_FAIL)
    {
        ...
        #if __ETT__
        if (Get_MDL_Used_Flag() == NORMAL_USED) /* It's OK if is doing RANK detection for Single rank */
            ASSERT(0);
        #endif
    }
    ...
    // set WL result to RG
    // LIN-YI: Set both RK delay in singleRK or dualRK
    if (u1GetRank(p) == (p->support_rank_num-1))
    {
        if (p->support_rank_num == RANK_SINGLE)
        {
            if (stDelayBase == PI_BASED)
            {
                wrlevel_dqs_final_delay_PI[RANK_1][BYTE_0] = wrlevel_dqs_final_delay_PI[RANK_0][BYTE_0];
                wrlevel_dqs_final_delay_PI[RANK_1][BYTE_1] = wrlevel_dqs_final_delay_PI[RANK_0][BYTE_1];
            }
            else // stDelayBase == DLY_BASED
            {
                wrlevel_dqs_final_delay_DLY[RANK_1][BYTE_0] = wrlevel_dqs_final_delay_DLY[RANK_0][BYTE_0];
                wrlevel_dqs_final_delay_DLY[RANK_1][BYTE_1] = wrlevel_dqs_final_delay_DLY[RANK_0][BYTE_1];
            }
            mcSHOW_DBG_MSG("RANK1 use RANK0 value in single RK\n");
        }
        else //(p->support_rank_num == RANK_DUAL)
        {
            #if WLEV_PI_RANK_DIFF
            if ((is_lp5_family(p) && ((p->frequency < WLEV_PI_RANK_DIFF_THRESHOLD)||(is_heff_mode(p))))||(u1IsPhaseMode_DQ(p)))
            #else
...
static void RunTime_SW_Cmd(DRAMC_CTX_T *p, RUNTIME_SWCMD_SEL_T runtime_SW_cmd_sel)
{
	...
	U32 u4Response = 0;
	U32 u4WckStatutsFld = (p->rank == RANK_0)? WCK_STATUS_WCKEN_RK0_SM : WCK_STATUS_WCKEN_RK1_SM;
	...
	else if(runtime_SW_cmd_sel == RUNTIME_SWCMD_CAS_FS && (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_WCK_STATUS), u4WckStatutsFld)) == WCK_STATUS_FR_MODE)
	{
		//Disable CAS(WCK_FR)
		...
		#if __ETT__
		mcSHOW_ERR_MSG("[ERROR] %s(%d) status error prvious: WCK_FR_MODE", __func__, runtime_SW_cmd_sel);
		ASSERT(0);
		#endif
	}
...
#if APPLY_NEW_IO_API
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ13)   , PB_Fld(1, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_EN_B1  ),
																	PB_Fld(1, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_SEL_B1 ),
...
#if APPLY_NEW_IO_API
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ13)   , PB_Fld(u4BackupWckOETieEn, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_EN_B1  ),
																	PB_Fld(u4BackupWckOETieSel, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_SEL_B1 ),
																	PB_Fld(u4BackupWckBOETieEn, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_EN_B1 ),
																	PB_Fld(u4BackupWckBOETieSel, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_SEL_B1  ));
#else
...
DRAM_STATUS_T DramcDutyCycleMonitor(DRAMC_CTX_T *p)
{
    ...
    U16 u2pValue;
    U8 u1PassFlag = 0, u1PassCnt = 0, u1BitValue = 0xFF;
    S8 s1DCMBegin=-7, s1DCMEnd=7;
    S8 bestU, bestL, s1DCM_step, s1EarlyBreak_step = WHOLE_STEPS_NUM, s1BitIdx;
    S8 U1 = 0xFF, U0 = 0xFF, L1 = 0xFF, L0 = 0xFF;
    S8 s1DcaFinalDelay[DCM_BIT_NUM];
    ...
    {
        //SW variable initial
        for (s1BitIdx = DCM_BIT_NUM-1; s1BitIdx >= 0; s1BitIdx--)
        {
             s1DcaFinalDelay[s1BitIdx] = 0x7F;
        }
        ...
        for (s1DCM_step = s1DCMBegin; s1DCM_step <= s1DCMEnd; s1DCM_step++)
        {
            ...
            U8 u8RGSettingVal = FetchRGSettingVal(s1DCM_step, 3);
            mcSHOW_DBG_MSG("Set s1DCM_step = %d, u8RGSettingVal = 0x%x into MR30\n", s1DCM_step, u8RGSettingVal);
            ...
            {
                //Get each bit value
                u1BitValue = (u2pValue>>s1BitIdx) & 0x1;

                //case1: 0->1
                //case2: 1 in the beginging
                //case3: 0 in the end
                if ((s1DcaFinalDelay[s1BitIdx] == 0x7F) && ((u1BitValue == 1) || ((u1BitValue == 0) && (s1DCM_step == s1DCMEnd))))
                {
                    s1DcaFinalDelay[s1BitIdx] = s1DCM_step;
                    u1PassFlag |= 0x1<<s1BitIdx;
                }
            }
        ...
        for (s1DCM_step = s1DCMBegin; s1DCM_step <= s1DCMEnd; s1DCM_step++)
        {
            ...
            for (s1BitIdx = DCM_BIT_NUM-1; s1BitIdx >= 0; s1BitIdx--)
                mcSHOW_DBG_MSG("     %d ", (u2ResultDCM[s1DCM_step+7]>>(s1BitIdx))&0x1);
        ...
        //Get Final Value
        L0 = s1DcaFinalDelay[0];
        L1 = s1DcaFinalDelay[1];
        U0 = s1DcaFinalDelay[2];
        U1 = s1DcaFinalDelay[3];
        bestU = (U1 + U0) / 2;
        bestL = (L1 + L0) / 2;

        mcSHOW_DBG_MSG("U1: %d, U0: %d, L1: %d, L0: %d\n", U1, U0, L1, L0);
        mcSHOW_DBG_MSG("Finally value U: %d, L: %d\n", bestU, bestL);
        bestU = FetchRGSettingVal(bestU, 3);
        bestL = FetchRGSettingVal(bestL, 3);
...
void vResetDelayChainBeforeCalibration(DRAMC_CTX_T *p)
{
    ...
    #if TX_WPULL_K
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ16), 0, SHU_B0_DQ16_RG_TX_ARDQ_VREFCAL_SEL_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ16), 0, SHU_B1_DQ16_RG_TX_ARDQ_VREFCAL_SEL_B1);
    #endif
    ...
    for(u1RankIdx=RANK_0; u1RankIdx<RANK_MAX; u1RankIdx++)
    {
        ...
        #endif
        {
            ...
            //perbit wpull level
            #if TX_WPULL_K
            vIO32Write4B(DDRPHY_REG_SHU_RK_B0_DRV_WPULL, 0);
            vIO32Write4B(DDRPHY_REG_SHU_RK_B1_DRV_WPULL, 0);
            #endif
        }
    #endif
    }
...
#if APPLY_NEW_IO_API
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ9),
            PB_Fld(0, B0_DQ9_RG_RX_ARDQS0_STBEN_RESETB_B0),
            PB_Fld(0, B0_DQ9_RG_RX_ARDQ_STBEN_RESETB_B0));
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ9),
            PB_Fld(0, B1_DQ9_RG_RX_ARDQS0_STBEN_RESETB_B1),
            PB_Fld(0, B1_DQ9_RG_RX_ARDQ_STBEN_RESETB_B1));
    mcDELAY_US(1);//delay 10ns
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ9),
            PB_Fld(1, B1_DQ9_RG_RX_ARDQS0_STBEN_RESETB_B1),
            PB_Fld(1, B1_DQ9_RG_RX_ARDQ_STBEN_RESETB_B1));
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ9),
            PB_Fld(1, B0_DQ9_RG_RX_ARDQS0_STBEN_RESETB_B0),
            PB_Fld(1, B0_DQ9_RG_RX_ARDQ_STBEN_RESETB_B0));
#else
...
#if ((ZQ_BACKGROUND_MODE) && SUPPORT_TYPE_LPDDR4
    if (is_lp4_family(p))
    {
        //ZQCAL Start
...
typedef struct _RXOFFC_DELAY_T {
    S8 s1RxOffcDelayDQ[DQ_DATA_WIDTH];
    S8 s1RxOffcDelayDQM[DQS_NUMBER];
} RXOFFC_DELAY_T;

static U8 RxOffc_DelayExchange(S8 iOfst)
{
    U8 u1Value;
...
static void SetDelay_RxOffc_Single(DRAMC_CTX_T *p, S8 s1RxOffcDelay)
{
    U8 u1RxOffcRgValue;

    u1RxOffcRgValue = RxOffc_DelayExchange(s1RxOffcDelay);

    //mcSHOW_DBG_MSG2("s1DelayIdx= %2d, set %2d,", s1RxOffcDelay, u1RxOffcRgValue);

    //Delay of B0/B1 DQ0~DQ7.
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ0),
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ0_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ1_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ2_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ3_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ4_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ5_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ6_OFFC_B0) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B0_DQ0_RG_RX_ARDQ7_OFFC_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ0),
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ0_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ1_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ2_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ3_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ4_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ5_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ6_OFFC_B1) |
                                P_Fld(u1RxOffcRgValue, SHU_RK_B1_DQ0_RG_RX_ARDQ7_OFFC_B1));

    //Delay of B0/B1 DQM0
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ1), u1RxOffcRgValue, SHU_RK_B0_DQ1_RG_RX_ARDQ8_OFFC_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ1), u1RxOffcRgValue, SHU_RK_B1_DQ1_RG_RX_ARDQ8_OFFC_B1);
}


static void SetDelay_RxOffc(DRAMC_CTX_T *p, RXOFFC_DELAY_T *RxOffcDelay)
{
    U8 u1RxOffcRgValueDQ[DQ_DATA_WIDTH], u1RxOffcRgValueDQM[DQS_NUMBER];
    U8 u1BitIdx, u1ByteIdx;

    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
    {
        u1RxOffcRgValueDQ[u1BitIdx] = RxOffc_DelayExchange(RxOffcDelay->s1RxOffcDelayDQ[u1BitIdx]);
    }
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        u1RxOffcRgValueDQM[u1ByteIdx] = RxOffc_DelayExchange(RxOffcDelay->s1RxOffcDelayDQM[u1ByteIdx]);
    }

    //Delay of B0/B1 DQ0~DQ7.
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ0),
                                P_Fld(u1RxOffcRgValueDQ[0], SHU_RK_B0_DQ0_RG_RX_ARDQ0_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[1], SHU_RK_B0_DQ0_RG_RX_ARDQ1_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[2], SHU_RK_B0_DQ0_RG_RX_ARDQ2_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[3], SHU_RK_B0_DQ0_RG_RX_ARDQ3_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[4], SHU_RK_B0_DQ0_RG_RX_ARDQ4_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[5], SHU_RK_B0_DQ0_RG_RX_ARDQ5_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[6], SHU_RK_B0_DQ0_RG_RX_ARDQ6_OFFC_B0) |
                                P_Fld(u1RxOffcRgValueDQ[7], SHU_RK_B0_DQ0_RG_RX_ARDQ7_OFFC_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ0),
                                P_Fld(u1RxOffcRgValueDQ[8], SHU_RK_B1_DQ0_RG_RX_ARDQ0_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[9], SHU_RK_B1_DQ0_RG_RX_ARDQ1_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[10], SHU_RK_B1_DQ0_RG_RX_ARDQ2_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[11], SHU_RK_B1_DQ0_RG_RX_ARDQ3_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[12], SHU_RK_B1_DQ0_RG_RX_ARDQ4_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[13], SHU_RK_B1_DQ0_RG_RX_ARDQ5_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[14], SHU_RK_B1_DQ0_RG_RX_ARDQ6_OFFC_B1) |
                                P_Fld(u1RxOffcRgValueDQ[15], SHU_RK_B1_DQ0_RG_RX_ARDQ7_OFFC_B1));

    //Delay of B0/B1 DQM0
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ1), u1RxOffcRgValueDQM[BYTE_0], SHU_RK_B0_DQ1_RG_RX_ARDQ8_OFFC_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ1), u1RxOffcRgValueDQM[BYTE_1], SHU_RK_B1_DQ1_RG_RX_ARDQ8_OFFC_B1);
}

static void PrintDelay_RxOffc(DRAMC_CTX_T *p, RXOFFC_DELAY_T *RxOffcDelay)
{
    U8 u1BitIdx, u1ByteIdx;

    mcSHOW_DBG_MSG("[PrintDelay_RxOffc]\n");

    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
    {
       	mcSHOW_DBG_MSG("RX OFFC DQ%d : %d\n", u1BitIdx, RxOffcDelay->s1RxOffcDelayDQ[u1BitIdx]);
    }
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
       	mcSHOW_DBG_MSG("RX OFFC DQM%d : %d\n", u1ByteIdx, RxOffcDelay->s1RxOffcDelayDQM[u1ByteIdx]);
    }
    ...
    if (gHQALog_flag == 1)
    {
        for(u1BitIdx= 0; u1BitIdx <DQ_DATA_WIDTH; u1BitIdx++)
        {
            HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0_1, "RX_Input_Offset", "", u1BitIdx, RxOffcDelay->s1RxOffcDelayDQ[u1BitIdx], NULL);
        }
        for(u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
        {
            HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG("RX_Input_Offset  Channel%d Rank%d DQM%d %d\n", p->channel, p->rank, u1ByteIdx, RxOffcDelay->s1RxOffcDelayDQM[u1ByteIdx]);
        }
    }
    #endif
}

static void ScanRangeDelay_RxOffc(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
    ...
    ScanDelay->s2DelayStart = s2DelayBegin;
    ScanDelay->s2DelayEnd = s2DelayEnd;
    ScanDelay->u1DelayStep = u1DelayStep;
}

static void GetFinalDelay_RxOffc(DRAMC_CTX_T *p, S8 *s1BestDelay, RXOFFC_DELAY_T *RxOffcDelay)
{
    U8 u1BitIdx, u1ByteIdx;

    for(u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
    {
        RxOffcDelay->s1RxOffcDelayDQ[u1BitIdx] = s1BestDelay[u1BitIdx];
    }
    for(u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        RxOffcDelay->s1RxOffcDelayDQM[u1ByteIdx] = s1BestDelay[DQ_DATA_WIDTH+u1ByteIdx];
    }
}

static U32 GetRxOffcO1Result(DRAMC_CTX_T *p)
{
    U8 u1RestltDQ[DQS_NUMBER];
    U8 u1RestltDQM[DQS_NUMBER];
    U32 u4Restlt;

    u4Restlt = u4IO32Read4B(DRAMC_REG_ADDR(DDRPHY_REG_MISC_AD_RX_DQ_O1));

    u1RestltDQ[BYTE_0] = u4Restlt & 0xff;
    u1RestltDQ[BYTE_1] = (u4Restlt>>16) & 0xff;
    u1RestltDQM[BYTE_0] = (u4Restlt>>8) & 0x1;
    u1RestltDQM[BYTE_1] = (u4Restlt>>24) & 0x1;

    if (NeedPhyNaoByteSwap(p) == TRUE)
        u4Restlt = ((u1RestltDQM[BYTE_0]<<17) | (u1RestltDQM[BYTE_1]<<16)| (u1RestltDQ[BYTE_0]<<8) |u1RestltDQ[BYTE_1]);
    else
        u4Restlt = ((u1RestltDQM[BYTE_1]<<17) | (u1RestltDQM[BYTE_0]<<16)| (u1RestltDQ[BYTE_1]<<8) |u1RestltDQ[BYTE_0]);

    return u4Restlt;
}

static void RxOffc_INIT(DRAMC_CTX_T *p)
{
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RX_IN_GATE_EN_CTRL), 0xf, MISC_RX_IN_GATE_EN_CTRL_FIX_IN_GATE_EN);

    #if APPLY_NEW_IO_API
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6),
                                                        PB_Fld(1, B0_DQ6_RG_RX_ARDQ_BIAS_PS_B0),
                                                        PB_Fld(1, B0_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B0),
                                                        PB_Fld(0, B0_DQ6_RG_RX_ARDQ_DDR3_SEL_B0),
                                                        PB_Fld(1, B0_DQ6_RG_RX_ARDQ_DDR4_SEL_B0));
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6),
                                                        PB_Fld(1, B1_DQ6_RG_RX_ARDQ_BIAS_PS_B1),
                                                        PB_Fld(1, B1_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B1),
                                                        PB_Fld(0, B1_DQ6_RG_TX_ARDQ_DDR3_SEL_B1),
                                                        PB_Fld(1, B1_DQ6_RG_TX_ARDQ_DDR4_SEL_B1));
    #else
    ...
    #if APPLY_NEW_IO_API
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ3),
                                                        PB_Fld(1, B0_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B0),
                                                        PB_Fld(1, B0_DQ3_RG_RX_ARDQ_OFFC_EN_B0));
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ3),
                                                        PB_Fld(1, B1_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B1),
                                                        PB_Fld(1, B1_DQ3_RG_RX_ARDQ_OFFC_EN_B1));
    #else
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ3),
...
static void EnableRXOffc(DRAMC_CTX_T *p)
{
...
DRAM_STATUS_T DramcRxOffcCal(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    RXOFFC_DELAY_T FinalDelay_RxOffc;
    SCAN_DELAY_T ScanDelay_RxInputBuf;
    S8 s1BestDelay[DQ_DATA_WIDTH+DQS_NUMBER];
    U32 u4Result_Offc;
    S8 s1DelayIdx;
    U8 u1BitIdx, u1ByteIdx, u1RankIdx, u1Rank_bk;
    U8 u1FinishCount;
    U8 u1O1_value;
    TA2_CFG_T ta2_cfg;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DDRPHY_REG_B0_PHY3)),
        (MIX_RG_CHECK(DDRPHY_REG_B1_PHY3)),
#endif
        (MIX_RG_CHECK(DDRPHY_REG_MISC_RX_IN_GATE_EN_CTRL)),
    };

#if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_BIAS_PS_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_DDR3_SEL_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_DDR4_SEL_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_O1_SEL_B0},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_BIAS_PS_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_TX_ARDQ_DDR3_SEL_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_TX_ARDQ_DDR4_SEL_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_O1_SEL_B1},
        {DDRPHY_REG_B0_DQ3, B0_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B0},
        {DDRPHY_REG_B0_DQ3, B0_DQ3_RG_RX_ARDQ_OFFC_EN_B0},
        {DDRPHY_REG_B1_DQ3, B1_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B1},
        {DDRPHY_REG_B1_DQ3, B1_DQ3_RG_RX_ARDQ_OFFC_EN_B1},
        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_BUFF_EN_SEL_B0},
        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_BUFF_EN_SEL_B1},
    };
#endif

    mcSHOW_REG_ACCESS_MSG(__func__, GetFreqBySel(p, vGet_PLL_FreqSel(p)), ENABLE);

    u1Rank_bk = u1GetRank(p);
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG2("[DramcRxOffcCal] \n");

    // Switch RX from Rank1 to Rank0
    DramcEngine2Init(p, &ta2_cfg, TA2_SEL_GATING_CFG);
    DramcEngine2Run(p, &ta2_cfg);
    DramcEngine2End(p);
    ...
    // Choose Scan range
    ScanRangeDelay_RxOffc(p, &ScanDelay_RxInputBuf);

    // SW parameter initialization
    u1FinishCount = 0;
    memset(s1BestDelay, 0x7f, sizeof(s1BestDelay));

    // Sweep RX offset calibration code (RG_RX_*DQ*_OFFC<3:0>), the MSB is sign bit, sweep the code from -7(1111) to +7(0111)
    for(s1DelayIdx = ScanDelay_RxInputBuf.s2DelayStart; s1DelayIdx < ScanDelay_RxInputBuf.s2DelayEnd; s1DelayIdx+=ScanDelay_RxInputBuf.u1DelayStep)
    {
        SetDelay_RxOffc_Single(p, s1DelayIdx);
        ...
        u4Result_Offc = GetRxOffcO1Result(p);		
        mcSHOW_DBG_MSG2("[%d] u4Result_Offc = 0x%x\n", s1DelayIdx, u4Result_Offc);

        for(u1BitIdx= 0; u1BitIdx < DQ_DATA_WIDTH +DQS_NUMBER; u1BitIdx++)
        {
            u1O1_value = (u4Result_Offc >> u1BitIdx) & 0x1;

            if (s1BestDelay[u1BitIdx] == 0x7f)
            {
                if ((u1O1_value == 0) && (s1DelayIdx == ScanDelay_RxInputBuf.s2DelayStart))
                {
                    s1BestDelay[u1BitIdx] = s1DelayIdx;
                    //u1FinishCount++;
                    mcSHOW_DBG_MSG("[WARNING] didn't find trainsition point\n");
                }
                else if ((u1O1_value == 1) && (s1DelayIdx >= (ScanDelay_RxInputBuf.s2DelayEnd - ScanDelay_RxInputBuf.u1DelayStep)))
                {
                    s1BestDelay[u1BitIdx] = s1DelayIdx;
                    //u1FinishCount++;
                    mcSHOW_DBG_MSG("[WARNING] didn't find trainsition point\n");
                }
                else if (u1O1_value ==0) // 1 -> 0
                {
                    s1BestDelay[u1BitIdx] = s1DelayIdx;
                    u1FinishCount++;
                }
        ...
        if(u1FinishCount == (DQS_NUMBER+DQ_DATA_WIDTH)) // (DQ8 bits, DQM 1bit, total 9 bits.) x2 bytes
        {
            K_result = DRAM_OK;
    ...
    // Get Final Delay
    GetFinalDelay_RxOffc(p, s1BestDelay, &FinalDelay_RxOffc);

    // Print calibration result
    PrintDelay_RxOffc(p, &FinalDelay_RxOffc);

    // Set Final result
    for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
    {
        vSetRank(p, u1RankIdx);
        SetDelay_RxOffc(p, &FinalDelay_RxOffc);
    }
    vSetRank(p, u1Rank_bk);

    // Save final result for fastK
    #if SUPPORT_SAVE_TIME_FOR_CALIBRATION
    for(u1BitIdx= 0; u1BitIdx <DQ_DATA_WIDTH; u1BitIdx++)
    {
        p->pSavetimeData->s1DQOffc[p->channel][u1BitIdx] = FinalDelay_RxOffc.s1RxOffcDelayDQ[u1BitIdx];
    }
    for(u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
    {
        p->pSavetimeData->s1DQMOffc[p->channel][u1ByteIdx] = FinalDelay_RxOffc.s1RxOffcDelayDQM[u1ByteIdx];
    }
    #endif

    vSetCalibrationResult(p, DRAM_CALIBRATION_RX_OFFC, K_result);
    ...
    mcSHOW_DBG_MSG4("[DramcRxOffcCal] ====Done====\n");
    mcSHOW_REG_ACCESS_MSG(__func__, GetFreqBySel(p, vGet_PLL_FreqSel(p)), DISABLE);
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcRxOffcCal(DRAMC_CTX_T *p)
{
    RXOFFC_DELAY_T FinalDelay_RxOffc;
    U8 u1BitIdx, u1ByteIdx;
    ...
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[FAST_K] Bypass DramcRxOffcCal\n");

    // Load FinalDelay_ReadDcc from eMMC
    for(u1BitIdx= 0; u1BitIdx <DQ_DATA_WIDTH; u1BitIdx++)
   {
        FinalDelay_RxOffc.s1RxOffcDelayDQ[u1BitIdx] = p->pSavetimeData->s1DQOffc[p->channel][u1BitIdx];
   }

   for(u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
   {
        FinalDelay_RxOffc.s1RxOffcDelayDQM[u1ByteIdx] = p->pSavetimeData->s1DQMOffc[p->channel][u1ByteIdx];
   }

    rank_backup_and_set(p, RANK_0);

    do{
        // Set FinalDelay_RxOffc
        SetDelay_RxOffc(p, &FinalDelay_RxOffc);
        rank_advance(p);
    ...
    vSetCalibrationResult(p, DRAM_CALIBRATION_RX_OFFC, DRAM_FAST_K);
...
#if ENABLE_RX_RDDQC_CAL || ENABLE_RX_RDDQC_DQM_ONLY || ENABLE_GATING_CAL
...
void DramcRxWinRDDQCInit(DRAMC_CTX_T *p, U8 u1RXMode)
{
    ...
    if (is_lp5_family(p))
    {
        ...
        if (u1RXMode == PATTERN_RDDQC_ONLY_DQM)
        {
            ...
            DramcModeRegWriteByRank(p, p->rank, 13, u1MR13Value[p->rank]);
            #else
            vSwitchRDBI_RECC_Setting(p, ON, READ_OPT_CHG_DBI, TO_MR_ONLY, TRUE);
            #endif
        }
        #endif
...
#else
    uiLPDDR_O1_Mapping = (U8 *)uiLPDDR_MRR_Mapping_POP[p->channel];
...
void DramcRxWinRDDQCEnd(DRAMC_CTX_T *p, U8 u1RXMode)
{
    ...
    if (is_lp5_family(p))
    {
        ...
        if (u1RXMode == PATTERN_RDDQC_ONLY_DQM)
        {
            ...
            DramcModeRegWriteByRank(p, p->rank, 13, u1MR13Value[p->rank]);
            #else
            vSwitchRDBI_RECC_Setting(p, OFF, READ_OPT_CHG_DBI, TO_MR_ONLY, TRUE);
            #endif
        }
        #endif
...
/** Gating config **/
#define AUTO_K_SUPPORT_GATING 1
#define GATING_ATK_DBG 1
#define GATING_WITH_DQS_TRAINING_MODE 0
#define LP5_GATING_ENHANCED_RDQS_MODE 0
#define GATING_EARLY_BREAK 1
#define GATING_LEADLAG_LOW_LEVEL_CHECK 1
#define RD_DET_ERR 1
//#define DVT_GatingErrorFlag

#define GATING_PATTERN_NUM_LP5 0x23
#define GATING_GOLDEND_DQSCNT_LP5 0x4646
#define RXDQS_GATING_AUTO_DBG_REG_NUM 6

#define GATING_L_FILTER_PI 32
#define GATING_R_FILTER_PI 32

#define DQS_GW_COARSE_STEP 1
#define DQS_GW_FINE_STEP   2  //2^2 = 4
...
typedef struct _GATING_DELAY_T {
    U8 dqsien_ui[DQS_NUMBER];
    u8 dqsien_pi[DQS_NUMBER];
    //Delay you want to K in this calibration.
} GATING_DELAY_T;
...
struct rxdqs_gating_trans {
    ...
    #ifdef DVT_GatingErrorFlag
    U8 GatingErrorFlag[RANK_MAX][DQS_NUMBER];
    U8 GatingEmpErrorFlag[RANK_MAX][DQS_NUMBER];
    #else
    U8 GatingErrorFlag[DQS_NUMBER];
    #endif
    U32 DebugCnt[DQS_NUMBER];
};

static void Gating_ATK_INIT(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    U8 L_FilterPi = GATING_L_FILTER_PI;
    U8 R_FilterPi = GATING_R_FILTER_PI;
    U8 EarlyBreak = GATING_EARLY_BREAK;
    U8 DbgMode = GATING_ATK_DBG;
    U8 BurstLenth = RXDQS_BURST_LEN_8;

#if !GATING_WITH_DQS_TRAINING_MODE
    /* dummy read */
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD), 0x1, DUMMY_RD_DUMMY_RD_PA_OPT);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CG_CTRL0), 0x1, MISC_CG_CTRL0_RG_CG_PHY_OFF_DIABLE);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_TEST2_A4), 0x4, TEST2_A4_TESTAGENTRKSEL);

    //vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2), 1, MISC_STBCAL2_STBENCMPEN);
#endif

#if RD_DET_ERR
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RD_DET_CTRL), 1, MISC_RD_DET_CTRL_RD_DET_DEBUG_MODE);
#else
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RD_DET_CTRL), 0, MISC_RD_DET_CTRL_RD_DET_DEBUG_MODE);
#endif

    /*---------
     * DV settings
     *-------------------*/
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL), 0x0, MISC_STBCAL_PICGEN);
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG0), 
        P_Fld(ScanDelay->s2DelayStart, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_INI_UI) |
        P_Fld(ScanDelay->s2DelayEnd, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_END_UI) |
        P_Fld(ScanDelay->u1DelayStep, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_PI_OFFSET) |
        P_Fld(p->rank, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_CUR_RANK) |
        P_Fld(BurstLenth, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_BURST_LENGTH) |
        P_Fld(EarlyBreak, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_EARLY_BREAK_EN) |
        P_Fld(DbgMode, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_DEBUG_MODE_EN) |
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG1),
        P_Fld(L_FilterPi, MISC_DQSIEN_AUTOK_CFG1_DQSIEN_AUTOK_L_FILTER) |
        P_Fld(R_FilterPi, MISC_DQSIEN_AUTOK_CFG1_DQSIEN_AUTOK_R_FILTER) |
        P_Fld(0x1 , MISC_DQSIEN_AUTOK_CFG1_DQSIEN_AUTOK_FILTER_EN));

    #if GATING_WITH_DQS_TRAINING_MODE
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG1), 0x1, MISC_DQSIEN_AUTOK_CFG1_DQSIEN_AUTOK_RDDQC_MODE);
    #endif
}
...
static void rxdqs_gating_auto_cal_trigger(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG0), 0x1, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_GO);
}

static void rxdqs_gating_auto_cal_stop(DRAMC_CTX_T *p)
{
#if (GATING_WITH_DQS_TRAINING_MODE == 1)
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG1), 0x0, MISC_DQSIEN_AUTOK_CFG1_DQSIEN_AUTOK_RDDQC_MODE);
#endif

    mcSHOW_DBG_MSG("[Gating] AUTO K stop...\n");
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG0), 0x0, MISC_DQSIEN_AUTOK_CFG0_DQSIEN_AUTOK_GO);

    rxdqs_gating_auto_cal_reset(p);
}

static DRAM_STATUS_T Gating_AUTOK(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay, GATING_DELAY_T *FinalDelay)
{
    ...
    DRAM_STATUS_T K_Result = DRAM_OK;
    U8 done_bytes;
    U8 dqs_i, dqs_pinmux, byte_swap;

    Gating_ATK_INIT(p, ScanDelay);
    ...
    while (done_bytes < DQS_NUMBER)
    {
        for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++) {
            ...
            if (done[dqs_i] || error[dqs_i]) {
                ...
                for (dbg_reg_idx = 0; dbg_reg_idx < RXDQS_GATING_AUTO_DBG_REG_NUM; dbg_reg_idx++, dbg_reg_addr += 4)
                {
                     dbg_reg_val = u4IO32Read4B(dbg_reg_addr);

                      mcSHOW_DBG_MSG2("B%d Gating AUTOK DBG Status-%d: [0x%x]\n",
    ...
    for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
    {
        ...
        if (done[dqs_i])
        {
            if (error[dqs_i] == 0)
            {
                ...
                FinalDelay->dqsien_ui[dqs_i] = ui_center[dqs_i];
                FinalDelay->dqsien_pi[dqs_i] = pi_center[dqs_i];

                mcSHOW_DBG_MSG("\tcenter(%d, %d)\n", ui_center[dqs_i], pi_center[dqs_i]);
                mcSHOW_DBG_MSG("\tleft(%d, %d)\n", ui_left[dqs_i], pi_left[dqs_i]);
                #if (GATING_EARLY_BREAK == 0)
                mcSHOW_DBG_MSG("\tright(%d, %d)\n", ui_right[dqs_i], pi_right[dqs_i]);
                #endif
            }else{
                K_Result = DRAM_FAIL;
            }
...
static void Gating_SW_INIT(DRAMC_CTX_T *p)
{
#if !GATING_WITH_DQS_TRAINING_MODE
    TA2_CFG_T ta2_cfg;
#endif
    ...
    #if __IPMv2_TO_BE_PORTING__ //by cc. Could not FIND 
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RX_SET0), 0, RX_SET0_DM4TO1MODE);
    #endif

    /* enable &reset DQS counter */
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2), 
        P_Fld(1,MISC_STBCAL2_DQSGCNT_BYP_REF)| 
        P_Fld(1,MISC_STBCAL2_DQSG_CNT_EN));
...
#if RD_DET_ERR
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RD_DET_CTRL), P_Fld( 0, MISC_RD_DET_CTRL_RD_DET_HW_RST_EN) |
                                                                    P_Fld( 1, MISC_RD_DET_CTRL_RD_DET_DEBUG_MODE));
#else
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RD_DET_CTRL), P_Fld( 0, MISC_RD_DET_CTRL_RD_DET_HW_RST_EN) |
                                                                    P_Fld( 0, MISC_RD_DET_CTRL_RD_DET_DEBUG_MODE));
#endif
...
static void rxdqs_gating_set_dqsien_uidly(DRAMC_CTX_T *p, U8 dly_ui, BYTES_T eByteIdx)
{
    ...
    if(eByteIdx == BYTE_0 || eByteIdx == ALL_BYTES)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQSIEN_DLY), dly_ui, SHU_RK_B0_DQSIEN_DLY_DQSIEN_UI_B0);
        #if GATING_RODT_LATANCY_EN
    ...
    if(eByteIdx == BYTE_1 || eByteIdx == ALL_BYTES)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY), dly_ui, SHU_RK_B1_DQSIEN_DLY_DQSIEN_UI_B1);
        #if GATING_RODT_LATANCY_EN
...
static void rxdqs_gating_set_dqsien_pidly(DRAMC_CTX_T *p, U8 dly_pi, BYTES_T eByteIdx)
{
    if(eByteIdx == BYTE_0 || eByteIdx == ALL_BYTES)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQSIEN_DLY), dly_pi, SHU_RK_B0_DQSIEN_DLY_DQSIEN_PI_B0);
    }

    if(eByteIdx == BYTE_1 || eByteIdx == ALL_BYTES)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY), dly_pi, SHU_RK_B1_DQSIEN_DLY_DQSIEN_PI_B1);
    }
}
...
static void rxdqs_gating_sw_cal_trigger(DRAMC_CTX_T *p)
{
    ...
    DramcEngine2SelectConfig(p, &ta2_cfg, TA2_SEL_GATING_CFG);
    DramcEngine2Run(p, &ta2_cfg);
#endif
}

#ifdef DVT_GatingErrorFlag
static void DVT_GetGatingStatus(DRAMC_CTX_T *p, struct rxdqs_gating_trans *rxdqs_trans)
{
    U8 RankIdx;
    ...
    for(RankIdx = RANK_0; RankIdx < p->support_rank_num; RankIdx++)
    {
        if (RankIdx == RANK_0)
        {
            ...
            {
                rxdqs_trans->GatingErrorFlag[0][1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                    MISC_RD_DET_ERR_FLAG_B0_RD_BURST_CMP_ERR_RK0_B0);
                rxdqs_trans->GatingErrorFlag[0][0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B1),
                    MISC_RD_DET_ERR_FLAG_B1_RD_BURST_CMP_ERR_RK0_B1);

                rxdqs_trans->GatingEmpErrorFlag[0][1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                    MISC_RD_DET_ERR_FLAG_B0_RD_BURST_EMP_ERR_RK0_B0);
                rxdqs_trans->GatingEmpErrorFlag[0][0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B1),
                    MISC_RD_DET_ERR_FLAG_B1_RD_BURST_EMP_ERR_RK0_B1);
            }else{
                rxdqs_trans->GatingErrorFlag[0][0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                    MISC_RD_DET_ERR_FLAG_B0_RD_BURST_CMP_ERR_RK0_B0);
        ...
        else
        {
            if(NeedPhyNaoByteSwap(p))
            {
                rxdqs_trans->GatingErrorFlag[1][1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                    MISC_RD_DET_ERR_FLAG_B0_RD_BURST_CMP_ERR_RK1_B0);
                rxdqs_trans->GatingErrorFlag[1][0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B1),
                    MISC_RD_DET_ERR_FLAG_B1_RD_BURST_CMP_ERR_RK1_B1);

                rxdqs_trans->GatingEmpErrorFlag[1][1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                    MISC_RD_DET_ERR_FLAG_B0_RD_BURST_EMP_ERR_RK1_B0);
                rxdqs_trans->GatingEmpErrorFlag[1][0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                    DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B1),
                    MISC_RD_DET_ERR_FLAG_B1_RD_BURST_EMP_ERR_RK1_B1);
    ...
    if(NeedPhyNaoByteSwap(p))
    {
        rxdqs_trans->DebugCnt[1] = u4IO32Read4B(DRAMC_REG_ADDR(DDRPHY_REG_CAL_DQSG_CNT_B0));
        rxdqs_trans->DebugCnt[0] = u4IO32Read4B(DRAMC_REG_ADDR(DDRPHY_REG_CAL_DQSG_CNT_B1));
    }else{
        rxdqs_trans->DebugCnt[0] = u4IO32Read4B(DRAMC_REG_ADDR(DDRPHY_REG_CAL_DQSG_CNT_B0));
        rxdqs_trans->DebugCnt[1] = u4IO32Read4B(DRAMC_REG_ADDR(DDRPHY_REG_CAL_DQSG_CNT_B1));
    }

}
#else
static void GetGatingStatus(DRAMC_CTX_T *p, struct rxdqs_gating_trans *rxdqs_trans)
{
    /*Read Gating error*/
#if RD_DET_ERR
    if (p->rank == RANK_0)
    {
        if(NeedPhyNaoByteSwap(p))
        {
            rxdqs_trans->GatingErrorFlag[1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B0),
                MISC_RD_DET_ERR_FLAG_B0_RD_BURST_CMP_ERR_RK0_B0);
            rxdqs_trans->GatingErrorFlag[0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
                DDRPHY_REG_MISC_RD_DET_ERR_FLAG_B1),
                MISC_RD_DET_ERR_FLAG_B1_RD_BURST_CMP_ERR_RK0_B1);
        }else{
            rxdqs_trans->GatingErrorFlag[0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(
...
#endif
static void GetGatingLeadLag(DRAMC_CTX_T *p, 	struct rxdqs_gating_trans *rxdqs_trans, U8 dly_ui, U8 dly_pi, U8 u1PiStep)
{
    U8 dqs_i;
    U8 debounce_thrd_PI_l = GATING_L_FILTER_PI;
    #if GATING_LEADLAG_LOW_LEVEL_CHECK
    U8 debounce_thrd_PI_r = GATING_R_FILTER_PI;
    #endif
    ...
    for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
    {
        ...
        if ((rxdqs_trans->dqs_lead[dqs_i] == 1) && (rxdqs_trans->dqs_lag[dqs_i] == 1))
        {
            ...
            rxdqs_trans->dqsien_dly_ui_leadlag[dqs_i] = dly_ui;
            rxdqs_trans->dqsien_dly_pi_leadlag[dqs_i] = dly_pi;
        }
        else if ((rxdqs_trans->dqs_high[dqs_i] * u1PiStep) >= debounce_thrd_PI_l)
        {
            ...
            else if ((rxdqs_trans->dqs_lead[dqs_i] == 0) && (rxdqs_trans->dqs_lag[dqs_i] == 0))
            {
                ...
                if ((rxdqs_trans->dqs_low[dqs_i] * u1PiStep) >= debounce_thrd_PI_r)
                {
...
static DRAM_STATUS_T Gating_SWK(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay, GATING_DELAY_T *FinalDelay)
{
    U8 dly_ui, dly_pi, dqs_i, passed_bytes = 0;
    U8 u1PiStep = 1 << ScanDelay->u1DelayStep;
    U8 ui2pi = DQS_GW_PI_PER_UI;
    U8 GatingMode = u4IO32ReadFldAlign(DDRPHY_REG_SHU_B0_DQ10, SHU_B0_DQ10_RG_RX_ARDQS_DQSIEN_MODE_B0);
    U32 debug_pass_cnt, debug_cnt[DQS_NUMBER];
    DRAM_STATUS_T K_result = DRAM_FAIL;
...
#if SUPPORT_TYPE_LPDDR5
    if((is_lp5_family(p)) && (GatingMode != 1)) //For DQSIEN mode > 1, GOLDEN_DQSGCNT = cmd count
        debug_pass_cnt = (GATING_GOLDEND_DQSCNT_LP5 >> 1);
    else
#endif
        debug_pass_cnt = GATING_GOLDEND_DQSCNT_LP5;

    for(dly_ui = ScanDelay->s2DelayStart; dly_ui < ScanDelay->s2DelayEnd; dly_ui += DQS_GW_COARSE_STEP)
    {
        rxdqs_gating_set_dqsien_uidly(p, dly_ui, ALL_BYTES);

        for(dly_pi = 0; dly_pi < ui2pi; dly_pi += u1PiStep)
        {
            rxdqs_gating_set_dqsien_pidly(p, dly_pi, ALL_BYTES);
            DramPhyReset(p);
            rxdqs_gating_sw_cal_trigger(p);

            /*read gating error and dqsg counter*/
        #ifdef DVT_GatingErrorFlag
            DVT_GetGatingStatus(p, &rxdqs_trans);
        #else
            GetGatingStatus(p, &rxdqs_trans);
        #endif

            /* read (lead, lag) */
            GetGatingLeadLag(p, &rxdqs_trans, dly_ui, dly_pi, u1PiStep);

            mcSHOW_DBG_MSG2("%d %d | ", dly_ui, dly_pi);

        #ifdef DVT_GatingErrorFlag
            U8 u1RankIdx;
            mcSHOW_DBG_MSG2("B1->B0 | %x %x |", rxdqs_trans.DebugCnt[1], rxdqs_trans.DebugCnt[0]);

            for(u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
            {
                mcSHOW_DBG_MSG2(" %x %x |", rxdqs_trans.GatingErrorFlag[u1RankIdx][1], rxdqs_trans.GatingErrorFlag[u1RankIdx][0]);
                #if RD_DET_ERR
                mcSHOW_DBG_MSG2(" %x %x |", rxdqs_trans.GatingEmpErrorFlag[u1RankIdx][1], rxdqs_trans.GatingEmpErrorFlag[u1RankIdx][0]);
                #endif
            }
            ...
            mcSHOW_DBG_MSG2("B1->B0 | %x %x | %x %x | (%d %d) (%d %d)\n",
                rxdqs_trans.DebugCnt[1], rxdqs_trans.DebugCnt[0],
                rxdqs_trans.GatingErrorFlag[1], rxdqs_trans.GatingErrorFlag[0],
                rxdqs_trans.dqs_lead[1], rxdqs_trans.dqs_lag[1],
                rxdqs_trans.dqs_lead[0], rxdqs_trans.dqs_lag[0]);
        #endif
            ...
            for(dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
            {
                if (passed_bytes & (1 << dqs_i))
                    continue;
                #ifdef DVT_GatingErrorFlag
                if ((rxdqs_trans.GatingErrorFlag[p->rank][dqs_i] == 0) && (rxdqs_trans.DebugCnt[dqs_i] == debug_pass_cnt))
                #else
                if ((rxdqs_trans.GatingErrorFlag[dqs_i] == 0) && (rxdqs_trans.DebugCnt[dqs_i] == debug_pass_cnt))
                #endif
                {
                    ...
                    if (rxdqs_trans.dqs_transitioned[dqs_i] != 0)
                    {
                        ...
                        U8 offset = (pass_count * u1PiStep) / 2;
                        U8 tmp;

                        /* PI */
                        tmp = rxdqs_trans.dqsien_dly_pi_leadlag[dqs_i] + offset;
                        FinalDelay->dqsien_pi[dqs_i] = tmp % ui2pi;

                        /* UI - P0 */
                        tmp /= ui2pi;
                        tmp = rxdqs_trans.dqsien_dly_ui_leadlag[dqs_i] + tmp;
                        FinalDelay->dqsien_ui[dqs_i] = tmp;

                        mcSHOW_DBG_MSG("best dqsien dly found for B%d: (%d, %d)\n",
                            dqs_i,
                            FinalDelay->dqsien_ui[dqs_i],
                            FinalDelay->dqsien_pi[dqs_i]);
            ...
            {
                K_result = DRAM_OK;
                #if GATING_EARLY_BREAK
                dly_ui = ScanDelay->s2DelayEnd;
                break;
                #endif
            }
        }
...
static void ScanRangeDelay_Gating(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    U8 u1DelayStep = 1;
    ...
    if(is_lp5_family(p) && (MR10_PRE!=0))
    {
        switch(MR10_PRE)
        {
            ...
            case 0x3:
                ...
                break;
            default:
                mcSHOW_ERR_MSG("ERROR! Unexpected MR10 value\n");
                ASSERT(0);
        }
    }
    else
...
#if ENABLE_DDR800_OPEN_LOOP_MODE_OPTION
    if (u1IsPhaseMode_DQ(p) == TRUE)
        u1DelayStep = 4; // Divide by 16 (90 degree)
    else
#endif
        u1DelayStep = DQS_GW_FINE_STEP;

    ScanDelay->s2DelayStart = u1StartUI;
    ScanDelay->s2DelayEnd = u1StartUI + 32;
    ScanDelay->u1DelayStep = u1DelayStep;

    mcSHOW_DBG_MSG("Start: %d, End: %d, Step: %d\n", ScanDelay->s2DelayStart, ScanDelay->s2DelayEnd, ScanDelay->u1DelayStep);

}

static void Gating_COMMON_INIT(DRAMC_CTX_T *p)
{
    ...
    if (p->frequency == 800)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL1), 0x1, MISC_STBCAL1_STBCNT_SW_RST);
    }

    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL1), 0x1, MISC_STBCAL1_STBCNT_SHU_RST_EN);

    /* SELPH_MODE = BY RANK */
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2), 0x1, MISC_STBCAL2_DQSIEN_SELPH_BY_RANK_EN);

    #if SUPPORT_TYPE_LPDDR5
    if (is_lp5_family(p))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2), 0x1, MISC_STBCAL2_STB_PICG_EARLY_1T_EN);
	}
    #endif

    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL1), 0x1, MISC_STBCAL1_DIS_PI_TRACK_AS_NOT_RD);

    /* PICG_EARLY_EN */
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2), 0x1, MISC_STBCAL2_STB_PICG_EARLY_1T_EN);

    /* BURST_MODE */
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_STBCAL), 0x1, MISC_SHU_STBCAL_DQSIEN_BURST_MODE);

    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ9), 0x1, B0_DQ9_RG_RX_ARDQS0_DQSIENMODE_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ9), 0x1, B1_DQ9_RG_RX_ARDQS0_DQSIENMODE_B1);

    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6), 0x2, B0_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6), 0x2, B1_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B1);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL), 0x1, MISC_STBCAL_DQSIENMODE);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY2), 0x1, B0_PHY2_RG_RX_ARDQS_DQSIEN_UI_LEAD_LAG_EN_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY2), 0x1, B1_PHY2_RG_RX_ARDQS_DQSIEN_UI_LEAD_LAG_EN_B1);
...
static void Gating_ENTRY(DRAMC_CTX_T *p)
{
#if (GATING_WITH_DQS_TRAINING_MODE == 1)
#if SUPPORT_TYPE_LPDDR5
    ...
    DramcRxWinRDDQCInit(p, PATTERN_RDDQC);
#else
#if SUPPORT_TYPE_LPDDR5
    if(is_lp5_family(p))
        vSetLP5_WCKON_OnOff(p, ENABLE, WCK_ALWAYS_ON_SW);
...
#if (SUPPORT_TYPE_LPDDR5 == TRUE && LP5_GATING_ENHANCED_RDQS_MODE) 
    if (is_lp5_family(p))
    {
        DramcModeRegWriteByRank(p, p->rank, 46, 0x1);
    }
#endif
    //Calibration Entry Flow
}

static void Gating_EXIT(DRAMC_CTX_T *p)
{
#if SUPPORT_TYPE_LPDDR5 && LP5_GATING_ENHANCED_RDQS_MODE
    if (is_lp5_family(p)) 
    {
    ...
    DramcRxWinRDDQCEnd(p, PATTERN_RDDQC);
#else
    #if SUPPORT_TYPE_LPDDR5
    if (is_lp5_family(p))
    {
...
static void PrintFinalResult_Gating(DRAMC_CTX_T *p, GATING_DELAY_T *FinalDelay)
{
    U8 dqs_i;

    for (dqs_i = BYTE_0; dqs_i < DQS_NUMBER; dqs_i++)
    {
        /*TINFO="best DQS%d delay(0.5T, PI) = (%d, %d)\n", rxdqs_best_win.best_dqsien_dly_ui[dqs_i], rxdqs_best_win.best_dqsien_dly_pi[dqs_i])); */
        mcSHOW_DBG_MSG("RK%d best DQS%d dly(UI, PI) = (%d, %d)\n", p->rank, dqs_i, FinalDelay->dqsien_ui[dqs_i], FinalDelay->dqsien_pi[dqs_i]);
        ...
        HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0,
                "Gating_Center_", "05T", dqs_i, FinalDelay->dqsien_ui[dqs_i], NULL);
        HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0,
                "Gating_Center_", "PI", dqs_i, FinalDelay->dqsien_pi[dqs_i], NULL);
#endif
    }
}

static void rx_gating_misc_process(DRAMC_CTX_T *p, GATING_DELAY_T *FinalDelay)
{
#if GATING_ADJUST_TXDLY_FOR_TRACKING
    ...
    for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
    {
        u1TX_dly_DQSgated = FinalDelay->dqsien_ui[dqs_i];
        u1TX_dly_DQSgated >>= mck2ui;

        if (u1TX_dly_DQSgated < u1TXDLY_Cal_min)
            u1TXDLY_Cal_min = u1TX_dly_DQSgated;

        ucbest_coarse_ui_backup[p->rank][dqs_i] = FinalDelay->dqsien_ui[dqs_i];
    }

    for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
    {
        // find max gating TXDLY (should be in P1)
        u1TX_dly_DQSgated = FinalDelay->dqsien_ui[dqs_i] + freq_div;
        u1TX_dly_DQSgated >>= mck2ui;
    ...
    {
        //Byte 0
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_INI_UIPI), FinalDelay->dqsien_ui[0], SHU_RK_B0_INI_UIPI_CURR_INI_UI_B0);//UI
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_INI_UIPI), FinalDelay->dqsien_pi[0], SHU_RK_B0_INI_UIPI_CURR_INI_PI_B0); //PI
        //Byte 1
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_INI_UIPI), FinalDelay->dqsien_ui[1], SHU_RK_B1_INI_UIPI_CURR_INI_UI_B1);//UI
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_INI_UIPI), FinalDelay->dqsien_pi[1], SHU_RK_B1_INI_UIPI_CURR_INI_PI_B1); //PI
    }
#endif
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
static void rxdqs_gating_fastk_save_restore(DRAMC_CTX_T *p, GATING_DELAY_T *FinalDelay)
{
    u8 dqs_i;
    ...
    if (p->femmc_Ready == 1)
    {
        for (dqs_i = BYTE_0; dqs_i < DQS_NUMBER; dqs_i++)
        {
            FinalDelay->dqsien_ui[dqs_i] = p->pSavetimeData->u1Gating_UI_Save[p->channel][p->rank][dqs_i];
            FinalDelay->dqsien_pi[dqs_i] = p->pSavetimeData->u1Gating_PI_Save[p->channel][p->rank][dqs_i];
        }
    }
    else
    {
        for (dqs_i = BYTE_0; dqs_i < DQS_NUMBER; dqs_i++)
        {
            p->pSavetimeData->u1Gating_UI_Save[p->channel][p->rank][dqs_i] = FinalDelay->dqsien_ui[dqs_i];
            p->pSavetimeData->u1Gating_PI_Save[p->channel][p->rank][dqs_i] = FinalDelay->dqsien_pi[dqs_i];
        }
    }
}

DRAM_STATUS_T FastK_DramcRxGatingCal(DRAMC_CTX_T *p)
{
    U8 u1ByteIdx;
    GATING_DELAY_T FinalDelay;
    ...
    memset(&FinalDelay, 0, sizeof(GATING_DELAY_T));

    // Print current basic information
    vPrintCalibrationBasicInfo(p);

    rxdqs_gating_fastk_save_restore(p, &FinalDelay);

    for(u1ByteIdx = BYTE_0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        rxdqs_gating_set_dqsien_uidly(p, FinalDelay.dqsien_ui[u1ByteIdx], u1ByteIdx);
        rxdqs_gating_set_dqsien_pidly(p, FinalDelay.dqsien_pi[u1ByteIdx], u1ByteIdx);
    }

    PrintFinalResult_Gating(p, &FinalDelay);
    rx_gating_misc_process(p, &FinalDelay);
...
DRAM_STATUS_T DramcRxGatingCal(DRAMC_CTX_T *p, U8 u1IsAutoK)
{

    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    GATING_DELAY_T FinalDelay;
    SCAN_DELAY_T ScanDelay;
    U8 u1ByteIdx, u1RankIdx, startRank;
    U8 pi_per_ui = DQS_GW_PI_PER_UI;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DDRPHY_REG_B1_DQ6)),
        #endif
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL1)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL2)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_CTRL1)),
        //Need backup register
        (MIX_RG_CHECK(DRAMC_REG_DUMMY_RD)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_CG_CTRL0)),
        (MIX_RG_CHECK(DRAMC_REG_TEST2_A4)),
        (MIX_RG_CHECK(DRAMC_REG_RK_DUMMY_RD_ADR)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL1)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL2)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG0)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_DQSIEN_AUTOK_CFG1)),
    };

    #if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B0},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B1},
    };
    #endif
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRxGatingCal] Start\n");

    // Choose Delay scan range
    ScanRangeDelay_Gating(p, &ScanDelay);

    // Calibration common setting
    Gating_COMMON_INIT(p);

    // Calibration Entry Flow
    Gating_ENTRY(p);

    // Calibration scan delay
    #if AUTO_K_SUPPORT_GATING
    if (u1IsAutoK)
    {
        K_result = Gating_AUTOK(p, &ScanDelay, &FinalDelay);
    }
    else
    #endif
    {
        K_result = Gating_SWK(p, &ScanDelay, &FinalDelay);
    }

    // Calibration Exit Flow
    Gating_EXIT(p);

    if(p->support_rank_num == RANK_DUAL && ((vGet_DDR_Loop_Mode(p) == SEMI_OPEN_LOOP_MODE)||(vGet_DDR_Loop_Mode(p) == OPEN_LOOP_MODE)))
    {
        DqsienDly_backup[p->rank][0] = FinalDelay.dqsien_ui[0] * pi_per_ui + FinalDelay.dqsien_pi[0];
        DqsienDly_backup[p->rank][1] = FinalDelay.dqsien_ui[1] * pi_per_ui + FinalDelay.dqsien_pi[1];

        if(p->rank == RANK_1)
        {
            FinalDelay.dqsien_ui[0] = ((DqsienDly_backup[RANK_0][0] + DqsienDly_backup[RANK_1][0]) >> 1) / pi_per_ui;
            FinalDelay.dqsien_ui[1] = ((DqsienDly_backup[RANK_0][1] + DqsienDly_backup[RANK_1][1]) >> 1) / pi_per_ui;

            FinalDelay.dqsien_pi[0] = ((DqsienDly_backup[RANK_0][0] + DqsienDly_backup[RANK_1][0]) >> 1) % pi_per_ui;
            FinalDelay.dqsien_pi[1] = ((DqsienDly_backup[RANK_0][1] + DqsienDly_backup[RANK_1][1]) >> 1) % pi_per_ui;
        }
    ...
    for(u1RankIdx = startRank; u1RankIdx <= p->rank; u1RankIdx++)
    {
        // Set Final delay (DQ, DQM, DQS)
        for(u1ByteIdx = BYTE_0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            rxdqs_gating_set_dqsien_uidly(p, FinalDelay.dqsien_ui[u1ByteIdx], u1ByteIdx);
            rxdqs_gating_set_dqsien_pidly(p, FinalDelay.dqsien_pi[u1ByteIdx], u1ByteIdx);
    ...
    vSetCalibrationResult(p, DRAM_CALIBRATION_GATING, K_result);

    mcSHOW_DBG_MSG("[DramcRxGatingCal] Done\n");
    mcSHOW_REG_ACCESS_MSG(__func__, GetFreqBySel(p, vGet_PLL_FreqSel(p)), DISABLE);
...
void DramcRxdqsGatingPostProcess(DRAMC_CTX_T *p)
{
    ...
    else // for 1:4 mode and 1:16 mode
    {
    ...
    if (s1ChangeDQSINCTL != 0 || s1ChangeDQSUI != 0)  // need to change DQSINCTL and TXDLY of each byte
    {
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
            {
                ...
                mcSHOW_DBG_MSG("Rank: %d PostProcess best DQS%d UI = %d\n", u1RankIdx, dqs_i, ucbest_coarse_ui_backup[u1RankIdx][dqs_i]);
            }
        }
    ...
    if (gHQALog_flag == 1)
    {
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            for (dqs_i = 0; dqs_i < DQS_NUMBER; dqs_i++)
            {
                HQA_Log_Message_for_Report(p, p->channel, u1RankIdx, HQA_REPORT_FORMAT0, "PostProcess_Gating_", "UI", dqs_i, ucbest_coarse_ui_backup[u1RankIdx][dqs_i], NULL);
            }
        }
    }
#endif
    ...
    u4ReadDQSINCTL = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_RK_DQSCTL), MISC_SHU_RK_DQSCTL_DQSINCTL);
    ...
    {
        vSetRank(p, u1RankIdx);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_RK_DQSCTL), u4ReadDQSINCTL, MISC_SHU_RK_DQSCTL_DQSINCTL);  // DQSINCTL

        for (dqs_i = 0; dqs_i < (p->data_width / DQS_BIT_NUMBER); dqs_i++) {
            if (u1GatingMaxUI < ucbest_coarse_ui_backup[u1RankIdx][dqs_i])
                u1GatingMaxUI = ucbest_coarse_ui_backup[u1RankIdx][dqs_i];
        }
    }
    vSetRank(p, backup_rank);
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_RANKCTL),
            P_Fld(u4ReadDQSINCTL + (u1GatingMaxUI >> mck2ui_shift), MISC_SHU_RANKCTL_RANKINCTL_PHY) |  //RANKINCTL_PHY = DQSINCTL
            P_Fld(u4RankINCTL_ROOT, MISC_SHU_RANKCTL_RANKINCTL) | //RANKINCTL= DQSINCTL -3
            P_Fld(u4RankINCTL_ROOT, MISC_SHU_RANKCTL_RANKINCTL_ROOT1)); //RANKINCTL_ROOT1= DQSINCTL -3

#ifdef XRTR2R_PERFORM_ENHANCE_DQSG_RX_DLY
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_RANKCTL), u4RankINCTL_ROOT, MISC_SHU_RANKCTL_RANKINCTL_RXDLY);
...
#ifdef FOR_HQA_REPORT_USED
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT6, "PostProcess_", "DQSINCTL", 0, u4ReadDQSINCTL, NULL);
#endif
...
static void RXScanRange_Vref(DRAMC_CTX_T *p, RX_PATTERN_OPTION_T u1RXMode, U8 u1VrefScanEnable, U8 u1RXEyeScanEnable, U8 *pu1Begin, U8 *pu1End, U8 *pu1Step)
{
    ...
    if (u1VrefScanEnable)
    {
        ...
        else
        {
            if (p->dram_type == TYPE_LPDDR4)
            {
                u1VrefBegin = 0; /* cc: LP4 vref is smaller, from 0 is OK */
            ...
            else
            {
                    ...
                    if (is_lp5_family(p))
                    {
                        u1VrefBegin = RX_VREF_RANGE_BEGIN_ODT_OFF; //176.8mV (~250-75)
                        u1VrefEnd = RX_VREF_MAX_LIMIT; //322.2mV (~250+75)
                    }
                    #endif
            }
...
static void SetRxVref_underN5(DRAMC_CTX_T *p, U8 u1RxVref, BYTES_T eByteIdx)
{
    U8 u1RxVref_Offset, u1RxVref_CoarseValue, u1RxVref_FineValue;
    U8 u1VREF_SEL;

    u1RxVref_Offset = u1RxVref >> 7;
    u1RxVref_CoarseValue = (u1RxVref >> 4) & 0x7;
    u1RxVref_FineValue = u1RxVref & 0xf;
    u1VREF_SEL = (u1RxVref_Offset<<7)|(u1RxVref_CoarseValue<<4)|u1RxVref_FineValue;

    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL0),
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ0_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ1_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ2_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ3_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ4_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ5_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ6_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ7_VREF_SEL_B0));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL1),
                P_Fld(u1RxVref_FineValue, SHU_RK_B0_PHY_VREF_CTRL1_RG_RX_ARDQ8_VREF_SEL_B0)|
                P_Fld(u1VREF_SEL, SHU_RK_B0_PHY_VREF_CTRL1_RG_RX_ARDQ_VREF_SEL_RK_B0));	
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL0),
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ0_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ1_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ2_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ3_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ4_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ5_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ6_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ7_VREF_SEL_B1));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL1),
                P_Fld(u1RxVref_FineValue, SHU_RK_B1_PHY_VREF_CTRL1_RG_RX_ARDQ8_VREF_SEL_B1)|
                P_Fld(u1VREF_SEL, SHU_RK_B1_PHY_VREF_CTRL1_RG_RX_ARDQ_VREF_SEL_RK_B1));
    }
}

static void SetRxVref_aboveN5(DRAMC_CTX_T *p, U8 u1RxVref, BYTES_T eByteIdx)
{
    #if APPLY_NEW_IO_API
    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_SEL),
                PB_Fld(u1RxVref, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B0),
                PB_Fld(u1RxVref, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B0));
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_SEL),
                PB_Fld(u1RxVref, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B1),
                PB_Fld(u1RxVref, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B1));
    }
    #else
    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_SEL),
                P_Fld(u1RxVref, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B0) |
                P_Fld(u1RxVref, SHU_RK_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B0));
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_SEL),
                P_Fld(u1RxVref, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B1) |
                P_Fld(u1RxVref, SHU_RK_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B1));
    }
    #endif
}
...
{
    #if (CHIP_PROCESS<=5)
    SetRxVref_underN5(p, u1RxVref, eByteIdx);
    #else
    SetRxVref_aboveN5(p, u1RxVref, eByteIdx);
    #endif
}

#if ENABLE_RX_VREF_PERBIT
static void SetRxVref_underN5_perbit(DRAMC_CTX_T *p, U8 *u1BestRxPerbitVref, BYTES_T eByteIdx)
{
    U8 u1RxVref_Offset[DQS_NUMBER], u1RxVref_CoarseValue[DQS_NUMBER], u1RxVref_FineValue[DQ_DATA_WIDTH+DQS_NUMBER], u1RxVref_FineValue_avg[DQS_NUMBER];
    U16 u2RxVref_FineValue_sum[DQS_NUMBER] = {0};
    U8 u1VREF_SEL[DQS_NUMBER];
    U8 u1BitIdx, u1ByteIdx;
    U8 *uiLPDDR_PAD_TA2_Mapping;

    uiLPDDR_PAD_TA2_Mapping = GetPadTa2MappingTable(p);

    for (u1ByteIdx = 0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
    {
        // RX vref finevalue is perbit
        for (u1BitIdx = 0; u1BitIdx<DQS_BIT_NUMBER; u1BitIdx++)
        {
            u1RxVref_FineValue[u1ByteIdx*8+u1BitIdx] = u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] & 0xf; //DQ Vref
            u2RxVref_FineValue_sum[u1ByteIdx]+=u1RxVref_FineValue[u1ByteIdx*8+u1BitIdx];
        }
        u1RxVref_FineValue_avg[u1ByteIdx] = u2RxVref_FineValue_sum[u1ByteIdx]>>3;
        u1RxVref_FineValue[DQ_DATA_WIDTH + u1ByteIdx] = u1RxVref_FineValue_avg[u1ByteIdx]; //DQM Vref

        // RX vref offset and coarsvalue is per byte, use bit0 and bit8 instead(will be same)
        u1RxVref_Offset[u1ByteIdx] = u1BestRxPerbitVref[u1ByteIdx*8] >> 7;
        u1RxVref_CoarseValue[u1ByteIdx] = (u1BestRxPerbitVref[u1ByteIdx*8] >> 4) & 0x7;

        u1VREF_SEL[u1ByteIdx] = (u1RxVref_Offset[u1ByteIdx]<<7)|(u1RxVref_CoarseValue[u1ByteIdx]<<4)|u1RxVref_FineValue_avg[u1ByteIdx];
    }

    if ((eByteIdx == BYTE_0)||(eByteIdx == ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL0),
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[0]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ0_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[1]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ1_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[2]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ2_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[3]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ3_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[4]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ4_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[5]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ5_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[6]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ6_VREF_SEL_B0)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[7]], SHU_RK_B0_PHY_VREF_CTRL0_RG_RX_ARDQ7_VREF_SEL_B0));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_PHY_VREF_CTRL1),
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[16]], SHU_RK_B0_PHY_VREF_CTRL1_RG_RX_ARDQ8_VREF_SEL_B0)|
                P_Fld(u1VREF_SEL[BYTE_0], SHU_RK_B0_PHY_VREF_CTRL1_RG_RX_ARDQ_VREF_SEL_RK_B0));	
    }
    if ((eByteIdx == BYTE_1)||(eByteIdx == ALL_BYTES))
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL0),
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[8]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ0_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[9]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ1_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[10]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ2_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[11]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ3_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[12]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ4_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[13]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ5_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[14]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ6_VREF_SEL_B1)|
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[15]], SHU_RK_B1_PHY_VREF_CTRL0_RG_RX_ARDQ7_VREF_SEL_B1));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_PHY_VREF_CTRL1),
                P_Fld(u1RxVref_FineValue[uiLPDDR_PAD_TA2_Mapping[17]], SHU_RK_B1_PHY_VREF_CTRL1_RG_RX_ARDQ8_VREF_SEL_B1)|
                P_Fld(u1VREF_SEL[BYTE_1], SHU_RK_B1_PHY_VREF_CTRL1_RG_RX_ARDQ_VREF_SEL_RK_B1));
    }
}

void SetRxVref_perbit(DRAMC_CTX_T *p, U8 *u1BestRxPerbitVref, BYTES_T eByteIdx)
{
    #if (CHIP_PROCESS<=5)
    SetRxVref_underN5_perbit(p, u1BestRxPerbitVref, eByteIdx);
    #else
    mcSHOW_ERR_MSG("[WARNING] above N5 don't support RX perbit vref\n");
    #if __ETT__
    ASSERT(0);
    #endif
    #endif
...
static void SetRxDqDelay(DRAMC_CTX_T *p, S16 iDelay, BYTES_T eByteIdx)
{
    ...
    for (u1BitIdx = 0; u1BitIdx < 4; u1BitIdx++)
    {
        if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
        {
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY0 + (u1BitIdx * 4)),
                    P_Fld(iDelay, SHU_RK_B0_RXDLY0_RX_ARDQ0_R_DLY_B0) |
                    P_Fld(iDelay, SHU_RK_B0_RXDLY0_RX_ARDQ1_R_DLY_B0));
        }
        if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
        {
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY0 + (u1BitIdx * 4)),
...
{
    U16 u1DqsDelay = 0, u1DqDqmDelay = 0;
    ...
    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY5), u1DqsDelay, SHU_RK_B0_RXDLY5_RX_ARDQS0_R_DLY_B0);
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY5), u1DqsDelay, SHU_RK_B1_RXDLY5_RX_ARDQS0_R_DLY_B1);
    }

    // Adjust DQM output delay.
    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY4), u1DqDqmDelay, SHU_RK_B0_RXDLY4_RX_ARDQM0_R_DLY_B0);
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY4), u1DqDqmDelay, SHU_RK_B1_RXDLY4_RX_ARDQM0_R_DLY_B1);
    }
    //DramPhyReset(p);

#if PINMUX_AUTO_TEST_PER_BIT_RX
    if((gRX_check_per_bit_flag == 1) && (iDelay <= 0))
    {
...
#endif
    {
        ...
        SetRxDqDelay(p, u1DqDqmDelay, eByteIdx);
    }
}
 ...
 * 5. RDDQCEN = 0
 */
static U16 GetPassCriteria_RX(DRAMC_CTX_T *p)
{
    U16 u2PassCriteria;
    U8 u1PassPercent = 40;

    if (gHQALOG_RX_delay_cell_ps_075V)
        u2PassCriteria =  (u1PassPercent *1000000)/(gHQALOG_RX_delay_cell_ps_075V * DDRPhyGetRealFreq(p) * 2);
    else
        u2PassCriteria = RX_PASS_WIN_CRITERIA;

    return u2PassCriteria;
}
...
//RX Delay
...
#endif
    {
        for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH+u1CalDQMNum; u1BitIdx++)
        {
            if (u1BitIdx >= DQ_DATA_WIDTH) 
            {
                #ifdef ETT_PRINT_FORMAT
                mcSHOW_DBG_MSG("DQM%d, Center %d (%d ~ %d) %d\n", u1BitIdx-DQ_DATA_WIDTH, FinalWinPerBit[u1BitIdx].win_center, FinalWinPerBit[u1BitIdx].first_pass, FinalWinPerBit[u1BitIdx].last_pass, FinalWinPerBit[u1BitIdx].win_size);
                #else
    ...
    mcSHOW_DBG_MSG("\nDQS Delay:\nDQS0 = %d, DQS1 = %d\n"
                    "DQM Delay:\nDQM0 = %d, DQM1 = %d\n",
                        ...
                        FinalWinPerBit[DQ_DATA_WIDTH+0].best_dqdly, FinalWinPerBit[DQ_DATA_WIDTH+ 1].best_dqdly);
    #else
    mcSHOW_DBG_MSG("\nDQS Delay:\nDQS0 = %2d, DQS1 = %2d\n"
                    "DQM Delay:\nDQM0 = %2d, DQM1 = %2d\n",
                        s2DQSDlyPerbyte[0], s2DQSDlyPerbyte[1],
    ...
    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx = u1BitIdx + 4)
    {
    #ifdef ETT_PRINT_FORMAT
    ...
    for (u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
    {
            ...
            ((u2min_winsize_by_vref[u1ByteIdx] == u2min_winsize[u1ByteIdx]) && (u2TempWinSum[u1ByteIdx] > u2TempWinSum_by_vref[u1ByteIdx])))
        {
            ...
            for (u1BitIdx = (u1ByteIdx * DQS_BIT_NUMBER); u1BitIdx < (u1ByteIdx * DQS_BIT_NUMBER) + DQS_BIT_NUMBER; u1BitIdx++)
            {
                ...
                if (u1EnbleDqmK)
                {
                    FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].win_size = WinPerBit[DQ_DATA_WIDTH+u1ByteIdx].win_size;
                    FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].first_pass = WinPerBit[DQ_DATA_WIDTH+u1ByteIdx].first_pass;
                    FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].last_pass = WinPerBit[DQ_DATA_WIDTH+u1ByteIdx].last_pass;
                    FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].win_center = (FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].last_pass + FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].first_pass) >> 1;     // window center of each DQM bit
                }
...
static void RxChooseWinCenter(DRAMC_CTX_T *p, U8 u1RXMode, U8 u1EnbleDqmK, PASS_WIN_DATA_T *FinalWinPerBit, S16 *s2DQSDlyPerbyte)
{
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        ...
        if (u1EnbleDqmK)
            FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].best_dqdly = s2DQSDlyPerbyte[u1ByteIdx] + FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].win_center;
        else
            // calculate DQM as average of 8 DQ delay
            FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].best_dqdly = u2TmpDQMSum / DQS_BIT_NUMBER;
        ...
        HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0, "RX_Window_Center_", "DQM", u1ByteIdx, FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].best_dqdly, NULL);
        for (u1BitIdx = u1bit_first; u1BitIdx <= u1bit_last; u1BitIdx++)
        {
...
#if ENABLE_RX_VREF_PERBIT
static void PerbitVrefPostProcess_RX(DRAMC_CTX_T *p, U8 *u1BestRxPerbitVref)
{
    U8 u1ByteIdx, u1BitIdx;
    U8 u1RXVrefMax[DQS_NUMBER], u1RXVrefMin[DQS_NUMBER];
    U8 u1RXVrefAvg[DQS_NUMBER];
    U8 u1RXVrefRangeMax[DQS_NUMBER], u1RXVrefRangeMin[DQS_NUMBER];

    mcSHOW_DBG_MSG("\n[PerbitVrefPostProcess_RX]\n");

    #if RX_VREF_PERBIT_DEBUG_LOG
    //Print RX vref perbit before post process
    mcSHOW_DBG_MSG("\n[Before PostProcess]\n");
    for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
    {
        mcSHOW_DBG_MSG3("[RxVrefPerbit] CH%d RK%d DQ%d: u1BestRxPerbitVref = %d\n",
                p->channel, p->rank, u1BitIdx, u1BestRxPerbitVref[u1BitIdx]);
    }
    #endif
    ...
    {
        //initial
        u1RXVrefMax[u1ByteIdx] = 0x0;
        u1RXVrefMin[u1ByteIdx] = 0xFF;
        for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
        {
            //Get Max per Byte
            if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] > u1RXVrefMax[u1ByteIdx])
                u1RXVrefMax[u1ByteIdx] = u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx];
            //Get Min per Bit
            if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] < u1RXVrefMin[u1ByteIdx])
                u1RXVrefMin[u1ByteIdx] = u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx];
        }

        #if RX_VREF_PERBIT_DEBUG_LOG
        mcSHOW_DBG_MSG("Byte%d u1RXVrefMax = %d, u1RXVrefMin = %d\n", u1ByteIdx, u1RXVrefMax[u1ByteIdx], u1RXVrefMin[u1ByteIdx]);
        #endif

        if ((u1RXVrefMax[u1ByteIdx]>>4)==(u1RXVrefMin[u1ByteIdx]>>4)) // All bits in the same range
        {
            #if RX_VREF_PERBIT_DEBUG_LOG
            mcSHOW_DBG_MSG("CASE1: do nothing\n");
            #endif
        }
        else // Some bits in diff ranges
        {
            u1RXVrefAvg[u1ByteIdx] = (u1RXVrefMax[u1ByteIdx]+u1RXVrefMin[u1ByteIdx])>>1;
            if (((u1RXVrefAvg[u1ByteIdx]&0xF)>=4)&&((u1RXVrefAvg[u1ByteIdx]&0xF)<12)) // Don't shift -5%
            {
                u1RXVrefRangeMin[u1ByteIdx] = u1RXVrefAvg[u1ByteIdx] & 0xF0;
                u1RXVrefRangeMax[u1ByteIdx] = u1RXVrefRangeMin[u1ByteIdx] + 0xF;
                for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
                {
                    //Shift too large Rx vref to avalable range Max
                    if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] > u1RXVrefRangeMax[u1ByteIdx])
                    {
                        mcSHOW_DBG_MSG("CASE2: Don't shift -5 percent, Out of range Max, u1BestRxPerbitVref[%d]: %d -> %d\n"
                                                , u1ByteIdx*8+u1BitIdx, u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx], u1RXVrefRangeMax[u1ByteIdx]);
                        u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] = u1RXVrefRangeMax[u1ByteIdx];
                    }
                    //Shift too small Rx vref to avalable range Min
                    if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] < u1RXVrefRangeMin[u1ByteIdx])
                    {
                        mcSHOW_DBG_MSG("CASE3: Don't shift -5 percent, Out of range Min, u1BestRxPerbitVre[%d]: %d -> %d\n"
                                                , u1ByteIdx*8+u1BitIdx, u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx], u1RXVrefRangeMin[u1ByteIdx]);
                        u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] = u1RXVrefRangeMin[u1ByteIdx];
                    }
                }
            }
            else // Shift -5%
            {
                mcSHOW_DBG_MSG("Byte%d shift 5%\n");
                u1RXVrefAvg[u1ByteIdx] = (u1RXVrefAvg[u1ByteIdx] |(1<<7))+8;
                for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
                {
                    mcSHOW_DBG_MSG("u1BestRxPerbitVre[%d]: %d -> %d\n"	, u1ByteIdx*8+u1BitIdx, u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx], (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx]|(1<<7))+8);
                    u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] = (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx]|(1<<7))+8;
                }

                if (((u1RXVrefAvg[u1ByteIdx]&0xF)>=4)&&((u1RXVrefAvg[u1ByteIdx]&0xF)<12))
                {
                    u1RXVrefRangeMin[u1ByteIdx] = u1RXVrefAvg[u1ByteIdx] & 0xF0;
                    u1RXVrefRangeMax[u1ByteIdx] = u1RXVrefRangeMin[u1ByteIdx] + 0xF;
                    for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
                    {
                        ...
                        if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] > u1RXVrefRangeMax[u1ByteIdx])
                        {
                            mcSHOW_DBG_MSG("CASE4: Shift -5 percent, Out of range Max, u1BestRxPerbitVref[%d]: %d -> %d\n"
                                                    , u1ByteIdx*8+u1BitIdx, u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx], u1RXVrefRangeMax[u1ByteIdx]);
                            u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] = u1RXVrefRangeMax[u1ByteIdx];
                        }
                        //Shift too small Rx vref to avalable range Min
                        if (u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] < u1RXVrefRangeMin[u1ByteIdx])
                        {
                            mcSHOW_DBG_MSG("CASE5: Shift -5 percent, Out of range Min, u1BestRxPerbitVref[%d]: %d -> %d\n"
                                                    , u1ByteIdx*8+u1BitIdx, u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx], u1RXVrefRangeMin[u1ByteIdx]);
                            u1BestRxPerbitVref[u1ByteIdx*8+u1BitIdx] = u1RXVrefRangeMin[u1ByteIdx];
                        }
                    }
    ...
    #if RX_VREF_PERBIT_DEBUG_LOG
    //Print RX vref perbit after post process
    mcSHOW_DBG_MSG("\n[After PostProcess]\n");
    for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
    {
        mcSHOW_DBG_MSG3("[RxVrefPerbit] CH%d RK%d DQ%d: u1BestRxPerbitVref = %d\n",
...
static void RX_ENTRY(DRAMC_CTX_T *p, U8 u1RXMode, TA2_CFG_T *cfg)
{
    if (u1RXMode == PATTERN_TEST_ENGINE)
    {
        vAutoRefreshSwitch(p, ENABLE, u1GetRank(p), TO_ONE_CHANNEL);
        DramcEngine2Init(p, cfg, TA2_SEL_WORST_SI_CFG);//UI_SHIFT + LEN1
    }
    else
    {
...
DRAM_STATUS_T RX_AUTOK(DRAMC_CTX_T *p, RX_PATTERN_OPTION_T u1RXMode, U8 u1EnbleDqmK, S16 s2Begin, S16 s2End, U8 u1Step, U16 u1VrefIdx, PASS_WIN_DATA_T *WinPerBit, U8 u1VrefScanEnable)
{
    ...
    #if ENABLE_EYESCAN_GRAPH
    U8 EyeScan_index[DQ_DATA_WIDTH] = {0};
    #endif
    ...
    u1bk_autok_cg = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RX_AUTOK_CFG0), MISC_RX_AUTOK_CFG0_RX_AUTOK_CG_CTRL);//Set DCM off only once Init_DRAM()
    ...
    //check Bound missing
    if ((_bound_miss != 0) && (!u1IsPhaseMode_DQ(p)))
    {
    ...
    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH + u1CalDQMNum; u1BitIdx++)
    {
        ...
        if (u1PrintCalibrationProc)
        {
            if (u1BitIdx >= DQ_DATA_WIDTH) 
            {
                mcSHOW_DBG_MSG2("DQM %d: %d(%d)\n", u1BitIdx -DQ_DATA_WIDTH, _dq_win_begin, _dq_win_len);
            }
            else
            {
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RX_AUTOK_CFG0),
                        ...
                        P_Fld(u1bk_autok_cg, MISC_RX_AUTOK_CFG0_RX_AUTOK_CG_CTRL)); // RX autok clock disable

    return DRAM_OK;
}

DRAM_STATUS_T RX_SWK(DRAMC_CTX_T *p, RX_PATTERN_OPTION_T u1RXMode, U8 u1EnbleDqmK, S16 s2Begin, S16 s2End, U8 u1Step, U16 u1VrefIdx, TA2_CFG_T *ta2_cfg, PASS_WIN_DATA_T *WinPerBit, U8 u1VrefScanEnable)
{
    U8 u1RXEyeScanEnable = GetEyeScanEnable(p, 1);
    U8 u1PrintCalibrationProc = ((u1VrefScanEnable == FALSE) && (u1RXEyeScanEnable == FALSE));
    U8 u1CalDQMNum = u1EnbleDqmK?2:0; // 2: K DQM, 0: DQM get avg from DQ
    ...
    U8  u1BitIdx;
    S16 s2Firstpass_temp[DQ_DATA_WIDTH+DQM_NUMBER], s2Lastpass_temp[DQ_DATA_WIDTH+DQM_NUMBER];
    #if ENABLE_EYESCAN_GRAPH
    U8 EyeScan_index[DQ_DATA_WIDTH] = {0};
    #endif
    ...
    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH + u1CalDQMNum; u1BitIdx++)
    {
        ...
        #if ENABLE_EYESCAN_GRAPH
        if ((u1RXEyeScanEnable) && (u1BitIdx < DQ_DATA_WIDTH))
    ...
    for (s2Delay = s2Begin; s2Delay <= s2End; s2Delay += u1Step)
    {
        ...
        if (u1RXMode == PATTERN_TEST_ENGINE)
        {
            u4err_value = DramcEngine2Run(p, ta2_cfg);
        }
        else
        {
        ...
        for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH + u1CalDQMNum; u1BitIdx++)
        {
            ...
            if(u1PrintCalibrationProc)
            {
                if(u4err_value !=0)
                {
                    ...
                    if(u1BitIdx == (DQ_DATA_WIDTH + u1CalDQMNum-1))
                    {
                        mcSHOW_DBG_MSG3(" [MSB]\n");
                    }
                }
...
static void RXBackupDelayReg_DelayLine(DRAMC_CTX_T *p, PASS_WIN_DATA_T *FinalWinPerBit)
{
    ...
    {
        FinalWinPerBit[u1BitIdx].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY0+ u1BitIdx * 2), SHU_RK_B0_RXDLY0_RX_ARDQ0_R_DLY_B0);
        FinalWinPerBit[u1BitIdx + 1].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY0+ u1BitIdx * 2), SHU_RK_B0_RXDLY0_RX_ARDQ1_R_DLY_B0);
        FinalWinPerBit[u1BitIdx + 8].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY0+ u1BitIdx * 2), SHU_RK_B1_RXDLY0_RX_ARDQ0_R_DLY_B1);
        FinalWinPerBit[u1BitIdx + 9].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY0+ u1BitIdx * 2), SHU_RK_B1_RXDLY0_RX_ARDQ1_R_DLY_B1);
    }
}
...
static void RXSetDelayReg_DelayLine(DRAMC_CTX_T *p, PASS_WIN_DATA_T *FinalWinPerBit)
{
    ...
    for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx += 2)
    {
         vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY0 + u1BitIdx * 2),
                                        P_Fld((FinalWinPerBit[u1BitIdx].best_dqdly), SHU_RK_B0_RXDLY0_RX_ARDQ0_R_DLY_B0) |
                                        P_Fld((FinalWinPerBit[u1BitIdx + 1].best_dqdly), SHU_RK_B0_RXDLY0_RX_ARDQ1_R_DLY_B0));

         vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY0 + u1BitIdx * 2),
                                        P_Fld(FinalWinPerBit[u1BitIdx + 8].best_dqdly, SHU_RK_B1_RXDLY0_RX_ARDQ0_R_DLY_B1) |
                                        P_Fld(FinalWinPerBit[u1BitIdx + 9].best_dqdly, SHU_RK_B1_RXDLY0_RX_ARDQ1_R_DLY_B1));
    }
                                              ...
                                              u8 isAutoK)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    PASS_WIN_DATA_T WinPerBit[DQ_DATA_WIDTH+DQM_NUMBER], FinalWinPerBit[DQ_DATA_WIDTH+DQM_NUMBER];
    ...
    TA2_CFG_T ta2_cfg;
    U8 u1PerbitVrefEnble = DISABLE;
    U16 PassCriteria_RX = GetPassCriteria_RX(p);
#if ENABLE_RX_VREF_PERBIT
    U16 u2MaxWinSizePerbit[DQ_DATA_WIDTH];
    U8 u1BestRxPerbitVref[DQ_DATA_WIDTH];
    U8 u1BestRxPerbitVref_ORG[DQ_DATA_WIDTH];
#endif
    ...
    U32 u4RegBackupAddress[] =
    {
        (MIX_RG_CHECK(DRAMC_REG_SHU_TX_SET0)),
        (MIX_RG_CHECK(DRAMC_REG_SHU_COMMON0)),
        (MIX_RG_CHECK(DRAMC_REG_TX_SET0)),
        (MIX_RG_CHECK(DRAMC_REG_CKECTRL)),
        (MIX_RG_CHECK(DRAMC_REG_DRAMC_PD_CTRL)),
        (MIX_RG_CHECK(DDRPHY_REG_B0_DQ2)),
        (MIX_RG_CHECK(DDRPHY_REG_B1_DQ2)),
#if !APPLY_NEW_IO_API
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ10)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ10)),
#endif
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ13)),
        //(MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ13)), @cc: will report compile error. Marked since NOT used in RxCal.
        (MIX_RG_CHECK(DDRPHY_REG_MISC_RX_AUTOK_CFG0)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_RX_AUTOK_CFG1)),
    #if RDDQC_PINMUX_WORKAROUND
        (MIX_RG_CHECK(DDRPHY_REG_MISC_MRR_PINMUX_CTRL0)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_MRR_PINMUX_CTRL1)),
    #endif
    };

#if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_SHU_B0_DQ10, SHU_B0_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B0},
        {DDRPHY_REG_SHU_B1_DQ10, SHU_B1_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B1},
    };
    ...
    if (u1RXMode == PATTERN_TEST_ENGINE)
    {
        ...
        if ((((p->rank==RANK_0)||(p->frequency >= RX_VREF_DUAL_RANK_K_FREQ))&&Get_Vref_Calibration_OnOff(p)) ||u1RXEyeScanEnable)
            u1VrefScanEnable = 1;
        #if ENABLE_RX_VREF_PERBIT
        if ((p->odt_onoff == ODT_ON) && u1VrefScanEnable)
            u1PerbitVrefEnble= 1;
        #endif
    }
#endif
        ...
        for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH+DQM_NUMBER; u1BitIdx++)
        {
        ...
        #if ENABLE_RX_VREF_PERBIT
        if (u1PerbitVrefEnble)
        {
            for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
            {
                u1BestRxPerbitVref[u1BitIdx] = 0xFF;
        ...
        if (u1RXMode == PATTERN_RDDQC_ONLY_DQM)
        {
            ...
            FinalWinPerBit[DQ_DATA_WIDTH+BYTE_0].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY4), SHU_RK_B0_RXDLY4_RX_ARDQM0_R_DLY_B0);
            FinalWinPerBit[DQ_DATA_WIDTH+BYTE_1].best_dqdly = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY4), SHU_RK_B1_RXDLY4_RX_ARDQM0_R_DLY_B1);
            s2DQSDlyPerbyte[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY5), SHU_RK_B0_RXDLY5_RX_ARDQS0_R_DLY_B0);
            s2DQSDlyPerbyte[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY5), SHU_RK_B1_RXDLY5_RX_ARDQS0_R_DLY_B1);
        }
        #endif

        //Calibration start
        mcSHOW_DBG_MSG("[RxWindowPerbitCal] u1RXMode: %s, RX Vref Scan: %d, Autok: %d, DqmK: %d, u1PerbitVrefEnble = %d\n", ((u1RXMode==PATTERN_TEST_ENGINE)?"TA2":(u1RXMode==PATTERN_RDDQC)?"RDDQC":"RDDQC_DQM"), u1VrefScanEnable, isAutoK, u1EnbleDqmK, u1PerbitVrefEnble);
        ...
        for (u1VrefIdx = u1VrefBegin; u1VrefIdx <= u1VrefEnd; u1VrefIdx += u1VrefStep)
        {
            ...
            {
                RX_SWK(p, u1RXMode, u1EnbleDqmK, s2DelayBegin, s2DelayEnd, u1DelayStep, u1VrefIdx, &ta2_cfg, WinPerBit, u1VrefScanEnable);
            }

            #if SUPPORT_TYPE_LPDDR5 && (!ENABLE_DVFSQ_LP5)
            if (is_lp5_family(p) && (p->odt_onoff==FALSE) && (u1VrefIdx > RX_VREF_MAX_LIMIT))
            {
                mcSHOW_DBG_MSG("RX Vref Limit to %d\n", RX_VREF_MAX_LIMIT);
            }
            else
            #endif
            {
            ...
            if (u1PerbitVrefEnble)
            {
                ...
                for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
                {
                    if (u1VrefScanEnable && (WinPerBit[u1BitIdx].win_size > u2MaxWinSizePerbit[u1BitIdx])) {
                        ...
                        u1BestRxPerbitVref[u1BitIdx] = u1VrefIdx;
    
                        mcSHOW_DBG_MSG("[RxVrefPerbit] DQ[%d] max winsize %d, best vref %d\n",
                            u1BitIdx, u2MaxWinSizePerbit[u1BitIdx], u1BestRxPerbitVref[u1BitIdx]);
                    }
                }
            }
            #endif
            ...
            if (u1RXMode == PATTERN_TEST_ENGINE)
            {
                for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
                {
                    ...
                    if (u1PerbitVrefEnble)
                    {
                        gFinalRXPerbitWin[p->channel][p->rank][u1BitIdx] = u2MaxWinSizePerbit[u1BitIdx];
                    }
                    else
                    #endif
                    {
            ...
            if((u2MinWinsize[BYTE_0]>PassCriteria_RX && u2MinWinsize[BYTE_1]>PassCriteria_RX) &&
        ...
        {
            mcSHOW_ERR_MSG("[WARNING] RX window too small (PassCriteria_RX(p)=%d)\n", PassCriteria_RX);
            #if __ETT__
            ASSERT(0);
            #endif
        }
        ...
        if (p->femmc_Ready == 0)
        {
            ...
            for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
            {
                p->pSavetimeData->u1RxWinPerbit_DQM[p->channel][p->rank][u1ByteIdx] = FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].best_dqdly;
                if ((u1RXMode != PATTERN_RDDQC_ONLY_DQM))
    ...
    if (u1VrefScanEnable == TRUE)
    {
        ...
        if (u1PerbitVrefEnble)
        {
            PerbitVrefPostProcess_RX(p, u1BestRxPerbitVref);
        }
        #endif
        ...
        for (u1RankIdx = p->rank; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            if (u1PerbitVrefEnble)
            {
                SetRxVref_perbit(p, u1BestRxPerbitVref, ALL_BYTES);

                for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
                {
                    gFinalRXVrefDQ_perbit[p->channel][u1RankIdx][u1BitIdx] = u1BestRxPerbitVref[u1BitIdx];
                    gFinalRXVrefDQForSpeedUp_perbit[p->channel][p->rank][p->odt_onoff][u1BitIdx] = u1BestRxPerbitVref[u1BitIdx];
                    if (u1BestRxPerbitVref[u1BitIdx]>0x7F)
                    {
                        u1BestRxPerbitVref_ORG[u1BitIdx] = u1BestRxPerbitVref[u1BitIdx] - 136; //5% shift (+(0x1<<7) +8)
                    }
                    else
                    {
                        u1BestRxPerbitVref_ORG[u1BitIdx] = u1BestRxPerbitVref[u1BitIdx];//no 5% shift
                    }
                    mcSHOW_DBG_MSG3("[RxVrefPerbit] CH%d RK%d DQ%d: u1BestRxPerbitVref = %d (RG = 0x%x), MaxWinSize = %d\n",
                        p->channel, p->rank, u1BitIdx, u1BestRxPerbitVref_ORG[u1BitIdx], u1BestRxPerbitVref[u1BitIdx], u2MaxWinSizePerbit[u1BitIdx]);
                }
                ...
                if(p->femmc_Ready == 0)
                {
                    for (u1BitIdx = 0; u1BitIdx<DQ_DATA_WIDTH; u1BitIdx++)
                    {
                        p->pSavetimeData->u1RxWinVrefperBit[p->channel][p->rank][u1BitIdx] = u1BestRxPerbitVref[u1BitIdx];
                    }
                }
                #endif
            }
            ...
            #endif
            {
                for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
                {
                    ...
                    gFinalRXVrefDQForSpeedUp[p->channel][p->rank][p->odt_onoff][u1ByteIdx] = u1FinalVref[u1ByteIdx];
                    mcSHOW_DBG_MSG("\nFinal RX Vref Byte %d = %d to rank%d", u1ByteIdx, u1FinalVref[u1ByteIdx], u1RankIdx);

                    #if SUPPORT_SAVE_TIME_FOR_CALIBRATION && BYPASS_VREF_CAL
                    if(p->femmc_Ready == 0)
                    {
                        p->pSavetimeData->u1RxWinVrefperByte[p->channel][p->rank][u1ByteIdx] = u1FinalVref[u1ByteIdx];
                    }
                    #endif
                }
            }
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY5),
            P_Fld(s2DQSDlyPerbyte[BYTE_0], SHU_RK_B0_RXDLY5_RX_ARDQS0_R_DLY_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY5),
            P_Fld(s2DQSDlyPerbyte[BYTE_1], SHU_RK_B1_RXDLY5_RX_ARDQS0_R_DLY_B1));

    // set dqm delay
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RXDLY4),
            P_Fld(FinalWinPerBit[DQ_DATA_WIDTH+0].best_dqdly, SHU_RK_B0_RXDLY4_RX_ARDQM0_R_DLY_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RXDLY4),
            P_Fld(FinalWinPerBit[DQ_DATA_WIDTH+1].best_dqdly, SHU_RK_B1_RXDLY4_RX_ARDQM0_R_DLY_B1));
...
#if 0//(fcFOR_CHIP_ID == fcA60868) && SUPPORT_TYPE_LPDDR5
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcRxWindowPerbitCal(DRAMC_CTX_T *p, RX_PATTERN_OPTION_T u1RXMode)
{
    DRAM_RANK_T backup_rank;
    PASS_WIN_DATA_T FinalWinPerBit[DQ_DATA_WIDTH+DQM_NUMBER];
    S16 s2DQSDlyPerbyte[DQS_NUMBER];
    U8 u1ByteIdx, u1RankIdx, u1BitIdx;
    U8 u1FinalVref [DQS_NUMBER]= {0xe, 0xe};
    U16 u2MinWinsize[DQS_NUMBER]={0};
    ...
    if (u1RXMode == PATTERN_TEST_ENGINE)
    {
        if (((p->rank==RANK_0) || (p->frequency >= RX_VREF_DUAL_RANK_K_FREQ)) && Get_Vref_Calibration_OnOff(p))
            u1VrefScanEnable = 1;
        #if ENABLE_RX_VREF_PERBIT
        if ((p->odt_onoff == ODT_ON) && u1VrefScanEnable)
            u1PerbitVrefEnble= 1;
    ...
    if (u1VrefScanEnable)
    {
        ...
        if (u1PerbitVrefEnble)
        {
            for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
            {
                u1BestRxPerbitVref[u1BitIdx] = p->pSavetimeData->u1RxWinVrefperBit[p->channel][p->rank][u1BitIdx];
            }
        }
        else
        #endif
        {
            for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
            {
                u1FinalVref[u1ByteIdx] = p->pSavetimeData->u1RxWinVrefperByte[p->channel][p->rank][u1ByteIdx];
            }
        }
    }
    #endif
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        ...
        FinalWinPerBit[DQ_DATA_WIDTH+u1ByteIdx].best_dqdly = p->pSavetimeData->u1RxWinPerbit_DQM[p->channel][p->rank][u1ByteIdx];
    } 
    for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
    {
    ...
    if(u1RXMode==PATTERN_TEST_ENGINE)
        ...
        vSetCalibrationResult(p, DRAM_CALIBRATION_RX_RDDQC_DQM_ONLY, DRAM_FAST_K);
        ...
        for (u1RankIdx = p->rank; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            if (u1PerbitVrefEnble)
            {
                ...
                for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
                {
                    ...
                    mcSHOW_DBG_MSG3("[RxVrefPerbit] CH%d RK%d DQ%d: u1BestRxPerbitVref = %d (RG = 0x%x)\n",
                        p->channel, p->rank, u1BitIdx, u1BestRxPerbitVref_ORG[u1BitIdx], u1BestRxPerbitVref[u1BitIdx]);
                }
            }
            else
            #endif
...
#if RDDQC_DCA_WA
typedef struct _DCA_DELAY_T {
    S8 s1MR30_Dca[DQS_NUMBER];
} DCA_DELAY_T ;

void SetDelay_DCA(DRAMC_CTX_T *p, DCA_DELAY_T *DcaDeley)
{
    U8 u1DCAU = 0, u1DCAL = 0;

    u1DCAL = FetchRGSettingVal(DcaDeley->s1MR30_Dca[BYTE_0], 3);
    u1DCAU = FetchRGSettingVal(DcaDeley->s1MR30_Dca[BYTE_1], 3);

    MRWriteFldMulti(p, 30, P_Fld(u1DCAU, MR30_DCAU) |
                               P_Fld(u1DCAL, MR30_DCAL),
                               TO_MR);
}

void SetDelay_DCA_Single(DRAMC_CTX_T *p, S16 s2Delay, BYTES_T eByteIdx)
{
    U8 u1DCAU = 0, u1DCAL = 0;

    if (eByteIdx == ALL_BYTES)
    {
        u1DCAU = FetchRGSettingVal(s2Delay, 3);
        u1DCAL = FetchRGSettingVal(s2Delay, 3);
        MRWriteFldMulti(p, 30, P_Fld(u1DCAU, MR30_DCAU) |
                                   P_Fld(u1DCAL, MR30_DCAL),
                                   TO_MR);
    }
    else if (eByteIdx == BYTE_0)
    {
        u1DCAL = FetchRGSettingVal(s2Delay, 3);
        DramcMRWriteFldAlign(p, 30, u1DCAL, MR30_DCAL, TO_MR);
    }
    else if (eByteIdx == BYTE_1)
    {
        u1DCAU = FetchRGSettingVal(s2Delay, 3);
        DramcMRWriteFldAlign(p, 30, u1DCAU, MR30_DCAU, TO_MR);
    }
}

static void ScanRangeDelay_Dca(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
static void ScanRangeDelay_DcaRddqc(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
DRAM_STATUS_T DramDCA_RddqcMode(DRAMC_CTX_T *p)
{
    //Dca
    SCAN_DELAY_T ScanDelay_Dca;
    DCA_DELAY_T DcaDeley;
    S8 s1DcaDeleyIdx;
    S8 s1DcaFinalDelay[DQS_NUMBER] = {0x7F, 0x7F};
    U8 u1EarlyBreakCnt = 0;
    //RX perbit
    SCAN_DELAY_T ScanDelay_RxTa2;
    PASS_WIN_DATA_T WinPerBit[DQ_DATA_WIDTH];
    U8 isAutoK = 1, u1EnbleDqmK = 0, u1VrefScanEnable = 0;
    U8 u1RXMode = PATTERN_RDDQC;
    TA2_CFG_T ta2_cfg;
    U16 u2MinRxWindow[DQS_NUMBER] = {0xFFFF, 0xFFFF}, u2FinalMinRxWindow[DQS_NUMBER] = {0x0, 0x0};
    U16 u2RxWindowSum[DQS_NUMBER] = {0x0, 0x0}, u2FinalRxWindowSum[DQS_NUMBER] = {0x0, 0x0};
    //Others
    U8 u1ByteIdx, u1BitIdx;
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramDCA_RddqcMode] Start\n");

    // Print Calibration Mode Option
    mcSHOW_DBG_MSG("isAutoK: %d, u1RXMode: %s \n", isAutoK, ((u1RXMode==PATTERN_TEST_ENGINE)?"TA2":(u1RXMode==PATTERN_RDDQC)?"RDDQC":"RDDQC_DQM"));

    // Choose Delay scan range
    ScanRangeDelay_Dca(p, &ScanDelay_Dca);
    ScanRangeDelay_DcaRddqc(p, &ScanDelay_RxTa2);
    ...
    // Calibration scan delay
    for (s1DcaDeleyIdx = ScanDelay_Dca.s2DelayStart; s1DcaDeleyIdx < ScanDelay_Dca.s2DelayEnd; s1DcaDeleyIdx+=ScanDelay_Dca.u1DelayStep)
    {
        // MRW MR30 OP[7:4] = i(Set DCAU) and OP[3:0] = i(Set DCAL)
        SetDelay_DCA_Single(p, s1DcaDeleyIdx, ALL_BYTES);

        // Initialize RX min window
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            u2RxWindowSum[u1ByteIdx] = 0x0;
            u2MinRxWindow[u1ByteIdx] = 0xffff;
        }
        for (u1BitIdx = 0; u1BitIdx < DQ_DATA_WIDTH; u1BitIdx++)
        {
            pass_win_data_init(&WinPerBit[u1BitIdx]);
        ...
        // Calibration scan delay (get WinPerBit)
        if (isAutoK)
        {
            RX_AUTOK(p, u1RXMode, u1EnbleDqmK, ScanDelay_RxTa2.s2DelayStart, ScanDelay_RxTa2.s2DelayEnd, ScanDelay_RxTa2.u1DelayStep, 0, WinPerBit, u1VrefScanEnable);
        }
        else
        {
            RX_SWK(p, u1RXMode, u1EnbleDqmK, ScanDelay_RxTa2.s2DelayStart, ScanDelay_RxTa2.s2DelayEnd, ScanDelay_RxTa2.u1DelayStep, 0, &ta2_cfg, WinPerBit, u1VrefScanEnable);
        }

        //Get min rx window and window sum
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            for (u1BitIdx = (u1ByteIdx*DQS_BIT_NUMBER); u1BitIdx < ((u1ByteIdx+1)*DQS_BIT_NUMBER); u1BitIdx++)
            {
                if (WinPerBit[u1BitIdx].win_size < u2MinRxWindow[u1ByteIdx])
                {
                    u2MinRxWindow[u1ByteIdx] = WinPerBit[u1BitIdx].win_size;
                }
                u2RxWindowSum[u1ByteIdx] += WinPerBit[u1BitIdx].win_size;
            }
        }

        //Update best delay
        // 1. min rx window > final min rx window
        // 2. min rx window == final min rx window and rx window sum > final rx window sum
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            if ((u2MinRxWindow[u1ByteIdx] > u2FinalMinRxWindow[u1ByteIdx])
               ||((u2MinRxWindow[u1ByteIdx] == u2FinalMinRxWindow[u1ByteIdx])&&(u2RxWindowSum[u1ByteIdx] > u2FinalRxWindowSum[u1ByteIdx])))
            {
                u2FinalMinRxWindow[u1ByteIdx] = u2MinRxWindow[u1ByteIdx];
                u2FinalRxWindowSum[u1ByteIdx] = u2RxWindowSum[u1ByteIdx];
                s1DcaFinalDelay[u1ByteIdx] = s1DcaDeleyIdx;
                u1EarlyBreakCnt = 1;
            }
        }

        //Check early break count
        if ((u2MinRxWindow[BYTE_0] < u2FinalMinRxWindow[BYTE_0]) && (u2MinRxWindow[BYTE_1] < u2FinalMinRxWindow[BYTE_1]))
        {
            u1EarlyBreakCnt++;
        }

        mcSHOW_DBG_MSG2("CH%d RK%d [%d] u2MinRxWindow: B0 %d, B1 %d, u2FinalMinRxWindow: B0 %d, B1 %d ", p->channel, p->rank, s1DcaDeleyIdx, u2MinRxWindow[BYTE_0], u2MinRxWindow[BYTE_1], u2FinalMinRxWindow[BYTE_0], u2FinalMinRxWindow[BYTE_1]);
        mcSHOW_DBG_MSG2("(s1BestDelay: B0 %d, B1 %d, u1EarlyBreakCnt %d)\n", s1DcaFinalDelay[BYTE_0], s1DcaFinalDelay[BYTE_1], u1EarlyBreakCnt);
        DramPhyReset(p);

        if (u1EarlyBreakCnt>=5)
        {
            mcSHOW_DBG_MSG("EarlyBreak !!\n");
            break;
        }
    }
    ...
    //Get Final Value
    mcSHOW_DBG_MSG("Finally value U: %d, L: %d\n", s1DcaFinalDelay[BYTE_1], s1DcaFinalDelay[BYTE_0]);
    if(NeedByteSwap(p))
    {
        DcaDeley.s1MR30_Dca[BYTE_0] = s1DcaFinalDelay[BYTE_1];
        DcaDeley.s1MR30_Dca[BYTE_1] = s1DcaFinalDelay[BYTE_0];
    }
    else
    {
        DcaDeley.s1MR30_Dca[BYTE_0] = s1DcaFinalDelay[BYTE_0];
        DcaDeley.s1MR30_Dca[BYTE_1] = s1DcaFinalDelay[BYTE_1];
    }
    SetDelay_DCA(p, &DcaDeley);
...
#if ENABLE_RX_DVS_CAL
typedef struct _DVS_DELAY_T {
    U8 u1DvsStep[DQS_NUMBER];
} DVS_DELAY_T ;

void SetDelay_Dvs(DRAMC_CTX_T *p, DVS_DELAY_T *DvsDelay)
{
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ5), DvsDelay->u1DvsStep[BYTE_0], SHU_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ5), DvsDelay->u1DvsStep[BYTE_1], SHU_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1);
}

void SetDelay_Dvs_Single(DRAMC_CTX_T *p, U16 u2Delay, BYTES_T eByteIdx)
{
    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ5), u2Delay, SHU_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0);
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ5), u2Delay, SHU_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1);
    }
}

void GetDelay_Dvs(DRAMC_CTX_T *p, DVS_DELAY_T *DvsDelay)
{
    DvsDelay->u1DvsStep[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ5), SHU_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0);
    DvsDelay->u1DvsStep[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ5), SHU_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1);
}

static void PrintDelay_Dvs(DRAMC_CTX_T *p, DVS_DELAY_T *DvsDelay)
{
    mcSHOW_DBG_MSG("[PrintDelay_DVS]\n");

    mcSHOW_DBG_MSG("u1DvsStep[BYTE_0] = %d\n", DvsDelay->u1DvsStep[BYTE_0]);
    mcSHOW_DBG_MSG("u1DvsStep[BYTE_1] = %d\n", DvsDelay->u1DvsStep[BYTE_1]);
}

static void ScanRangeDelay_Dvs(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
static U8 GetRxTrackingLeadLagFlag(DRAMC_CTX_T *p, U8 u1byte)
{
    ...
    mcSHOW_DBG_MSG2("Byte%d | LEAD(0x%x 0x%x) | LAG(0x%x 0x%x), ", u1byte, u1rising_lead, u1falling_lead, u1rising_lag, u1falling_lag);
...
static void DramcRxDVSCalPostProcess(DRAMC_CTX_T *p, DVS_DELAY_T *DvsDelay)
{
    if (p->support_rank_num == RANK_SINGLE)
        return;

    // Save global variable for post process
    DVS_final_delay[p->rank][BYTE_0] = DvsDelay->u1DvsStep[BYTE_0];
    DVS_final_delay[p->rank][BYTE_1] = DvsDelay->u1DvsStep[BYTE_1];

    // Get min delay and update to global variable
    if (p->rank == (p->support_rank_num-1))
    {
        DvsDelay->u1DvsStep[BYTE_0] = MIN(DVS_final_delay[RANK_0][BYTE_0], DVS_final_delay[RANK_1][BYTE_0]);
        DvsDelay->u1DvsStep[BYTE_1] = MIN(DVS_final_delay[RANK_0][BYTE_1], DVS_final_delay[RANK_1][BYTE_1]);

        mcSHOW_DBG_MSG("[DramcRxDVSCalPostProcess] B0: %d (RK0: %d, RK1: %d), B1: %d (RK0: %d, RK1: %d)\n"
                                                               , DvsDelay->u1DvsStep[BYTE_0], DVS_final_delay[RANK_0][BYTE_0], DVS_final_delay[RANK_1][BYTE_0]
                                                               , DvsDelay->u1DvsStep[BYTE_1], DVS_final_delay[RANK_0][BYTE_1], DVS_final_delay[RANK_1][BYTE_1]);

        DVS_final_delay[RANK_0][BYTE_0] = DVS_final_delay[RANK_1][BYTE_0] = DvsDelay->u1DvsStep[BYTE_0];
        DVS_final_delay[RANK_0][BYTE_1] = DVS_final_delay[RANK_1][BYTE_1] = DvsDelay->u1DvsStep[BYTE_1];
    }
}

DRAM_STATUS_T DramcRxDVSWindowCal(DRAMC_CTX_T *p)
{
    SCAN_DELAY_T ScanDelay_Dvs;
    DVS_DELAY_T DvsDelay;
    U8 u1ByteIdx, u1DelayIdx;
    U32 u4err_value;
    TA2_CFG_T ta2_cfg;
    U8 u1lead_lag, u1finish_flag[DQS_NUMBER]={0};
    ...
    DramcEngine2Init(p, &ta2_cfg, TA2_SEL_WORST_SI_CFG);//PIC Need to check if need to use UI_SHIFT;//UI_SHIFT + LEN1

    // Choose Delay scan range
    ScanRangeDelay_Dvs(p, &ScanDelay_Dvs);

    mcSHOW_DBG_MSG2("\nRX Delay %d -> %d, step: %d\n", ScanDelay_Dvs.s2DelayStart, ScanDelay_Dvs.s2DelayEnd, ScanDelay_Dvs.u1DelayStep);

    // Calibration scan delay
    for (u1DelayIdx = ScanDelay_Dvs.s2DelayStart; u1DelayIdx < ScanDelay_Dvs.s2DelayEnd; u1DelayIdx += ScanDelay_Dvs.u1DelayStep)
    {
        SetDelay_Dvs_Single(p, u1DelayIdx, ALL_BYTES);

        u4err_value = DramcEngine2Run(p, &ta2_cfg);
        mcSHOW_DBG_MSG2("iDelay= %4d, err_value: 0x%x, ", u1DelayIdx, u4err_value);

        for(u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            u1lead_lag = GetRxTrackingLeadLagFlag(p, u1ByteIdx);

            if (((u1lead_lag)||(u1DelayIdx == (ScanDelay_Dvs.s2DelayEnd-1))) && (u1finish_flag[u1ByteIdx] == 0))
            {
                if (u1DelayIdx >= 4)
                {
                    DvsDelay.u1DvsStep[u1ByteIdx] = u1DelayIdx - 4;
                }
                else
                {
                    DvsDelay.u1DvsStep[u1ByteIdx] = 1;
                    mcSHOW_ERR_MSG("RX DVS window abnormal !!\n");
                    #if __ETT__
                    ASSERT(0);
                    #endif
                }
                u1finish_flag[u1ByteIdx] = 1;
                //mcSHOW_DBG_MSG2("Byte%d find dvs boundary\n");
             }
        }

        if ((u1finish_flag[BYTE_0]==1) && (u1finish_flag[BYTE_1]==1))
        {
    ...
    // Print calibration result
    PrintDelay_Dvs(p, &DvsDelay);

    // Post process get min delay
    DramcRxDVSCalPostProcess(p, &DvsDelay);

    // Set Final delay
    SetDelay_Dvs(p, &DvsDelay);
    ...
    #ifdef FOR_HQA_REPORT_USED
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT3, "DvsStep_", "B0", 0, DvsDelay.u1DvsStep[BYTE_0], NULL);
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT3, "DvsStep_", "B1", 0, DvsDelay.u1DvsStep[BYTE_1], NULL);
    #endif

    #if SUPPORT_SAVE_TIME_FOR_CALIBRATION
    p->pSavetimeData->u1DvsDelay[p->channel][BYTE_0] = DvsDelay.u1DvsStep[BYTE_0];
    p->pSavetimeData->u1DvsDelay[p->channel][BYTE_1] = DvsDelay.u1DvsStep[BYTE_1];
    #endif
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcRxDVSWindowCal(DRAMC_CTX_T *p)
{
    DVS_DELAY_T DvsDelay;
    ...
    DvsDelay.u1DvsStep[BYTE_0] = p->pSavetimeData->u1DvsDelay[p->channel][BYTE_0];
    DvsDelay.u1DvsStep[BYTE_1] = p->pSavetimeData->u1DvsDelay[p->channel][BYTE_1];
...
#if (fcFOR_CHIP_ID == fcA60868) && SUPPORT_TYPE_LPDDR5
    if ((is_lp5_family(p)) && (is_wckon_mode(p) == TRUE)) //workaround for wck sync fail in phyreset
    {
        CKEFixOnOff(p, TO_ALL_RANK, CKE_FIXOFF, TO_ALL_CHANNEL);
        CKEFixOnOff(p, TO_ALL_RANK, CKE_FIXON, TO_ALL_CHANNEL);
...
DRAM_STATUS_T DramcRxdatlatCal(DRAMC_CTX_T *p)
{
    ...
    {
        vAutoRefreshSwitch(p, ENABLE, u1GetRank(p), TO_ONE_CHANNEL);
        DramcEngine2Init(p, &ta2_cfg, TA2_SEL_WORST_SI_CFG);//UI_SHIFT + LEN1
...
#define TX_VREF_RANGE_STEP         2
#endif
//0~7 for weak pull-down, 8~15 for weak pull-up
#define TX_WPULL_RANGE_BEGIN       0
#define TX_WPULL_RANGE_END         15
#define TX_WPULL_RANGE_STEP        1
...
void get_tx_delay(DRAMC_CTX_T *p, struct TxDelay_t *delay)
{
    ...
    delay->u4DqPI[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_SW_ARPI_DQ_B0);
    //Byte0 DQM
    ...
    delay->u4DqmPI[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_SW_ARPI_DQM_B0);
    //Byte1 DQ
    ...
    delay->u4DqPI[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_SW_ARPI_DQ_B1);
    //Byte1 DQM
    ...
    delay->u4DqmPI[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_SW_ARPI_DQM_B1);
}
...
void put_txdq_delay(DRAMC_CTX_T *p, struct TxDelay_t *delay)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), delay->u4DqPI[BYTE_0], SHU_RK_B0_DQ_SW_ARPI_DQ_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), delay->u4DqPI[BYTE_1], SHU_RK_B1_DQ_SW_ARPI_DQ_B1);
}
...
void put_txdqm_delay(DRAMC_CTX_T *p, struct TxDelay_t *delay)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), delay->u4DqmPI[BYTE_0], SHU_RK_B0_DQ_SW_ARPI_DQM_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), delay->u4DqmPI[BYTE_1], SHU_RK_B1_DQ_SW_ARPI_DQM_B1);
}
...
static u32 u4min_winsize_DFE[ALL_BYTES]={0}, u4TempWinSum_DFE[ALL_BYTES]={0,0};

static void DramcTXSetDFE(DRAMC_CTX_T *p, U8 u1DFEValue[])
{
    u1DFEValue[0] |= (u1DFEValue[1] << 4);
    DramcModeRegWriteByRank(p, p->rank, 24, u1DFEValue[0]);
}

static void TxChooseDFE(DRAMC_CTX_T *p, U32 u4DFE_min_winsize[], U32 u4DFE_TempWinSum[], U8 u1Final_DFE[], U8 u1DFELevel)
{
    U8 u1Change_DFE=0, u1ByteIdx;

    for (u1ByteIdx = 0; u1ByteIdx < (p->data_width / DQS_BIT_NUMBER); u1ByteIdx++)
    {
        if ((u4DFE_min_winsize[u1ByteIdx] > u4min_winsize_DFE[u1ByteIdx]) ||
            ((u4DFE_min_winsize[u1ByteIdx] == u4min_winsize_DFE[u1ByteIdx]) && (u4DFE_TempWinSum[u1ByteIdx] > u4TempWinSum_DFE[u1ByteIdx])))
        {
            u4min_winsize_DFE[u1ByteIdx] = u4DFE_min_winsize[u1ByteIdx];
            u4TempWinSum_DFE[u1ByteIdx] = u4DFE_TempWinSum[u1ByteIdx];
            u1Final_DFE[u1ByteIdx] = u1DFELevel;
            mcSHOW_DBG_MSG2(("B%d Find better DFE as 0x%x\n",u1ByteIdx, u1Final_DFE[u1ByteIdx]));
        }
    }
}
#endif
...
void TxWinTransferDelayToUIPI(DRAMC_CTX_T *p, U16 uiDelay, U8 u1AdjustPIToCenter, U8* pu1UILarge_DQ, U8* pu1UISmall_DQ, U8* pu1PI, U8* pu1UILarge_DQOE, U8* pu1UISmall_DQOE)
{
    ...
    U8 u1IsPhaseMode = u1IsPhaseMode_DQ(p);
    U8 u1PiTap = TX_PHASE_DQ_UI_TO_PI_TAP;

    u1Small_ui_to_large = u1MCK2UI_DivShift(p);

#if ENABLE_APHY_GLITCH_WA_FORCE_32PI
    u1AdjustPIToCenter = 1; // force 32PI to 1UI
#endif
    ...
    if (is_lp4_family(p) && u1AdjustPIToCenter && (pu1PI != NULL) && !u1IsPhaseMode)
    {
        if (u1PI < 10)
        {
            u1PI += (u1PiTap) >> 1;
...
#if ENABLE_DRAMOBF
//OE_MCK = DATA_MCK + WDBI_ON + (DRAMOBF_PIPE-1)
//    u2DQOE_shift = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_WODT), SHU_WODT_DRAMOBF_PIPE_NO);
    u2DQOE_shift = getDRAMOBF_PIPE();

//	mcSHOW_DBG_MSG("\nDRAMOBF_PIPE: %d\n", u2DQOE_shift);
    if (u2DQOE_shift>=1)
		u2DQOE_shift --;
    u2TmpValue += u2DQOE_shift<<u1Small_ui_to_large;
#endif
...
static void TXScanRange_PI(DRAMC_CTX_T *p, DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType, U16 *pu2Begin, U16 *pu2End)
{
    ...
    getTxDqInitDelay(&u1MCK, &u1UI, &u1PI);
    mcSHOW_DBG_MSG("TX DQ MCK=%d, UI=%d, PI=%d\n", u1MCK, u1UI, u1PI);
    ...
    if (calType == TX_DQ_DQS_MOVE_DQM_ONLY)
    {
        if(u1MCK<1)
        {
            mcSHOW_ERR_MSG("TX DQ MCK=%d, can't -1MCK\n", u1MCK);
            #if __ETT__
            ASSERT(0);
            #endif
        }        
    ...
    if (vGet_DDR_Loop_Mode(p) == CLOSE_LOOP_MODE) {
        u2DQDelayBegin = (((u1MCK << u1MCK2UI) + u1UI) << u1UI2PI) + u1PI - (1<<(u1UI2PI-1));
    } else {
        if (((u1MCK << u1MCK2UI) + u1UI) >= 1)
            u2DQDelayBegin = (((u1MCK << u1MCK2UI) + u1UI -1 ) << u1UI2PI) + u1PI;
        else
            u2DQDelayBegin = 0;
    }
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        ucdq_ui_large_bak[u1ByteIdx] = (u4RegValue_TXDLY >> (u1ByteIdx << 2)) & 0xF;// MCK
        ucdq_ui_small_bak[u1ByteIdx] = (u4RegValue_dly >> (u1ByteIdx << 2)) & 0xF;// UI
...
static void TXScanRange_Vref(DRAMC_CTX_T *p, U8 u1VrefScanEnable, U16* pu2Range, U16 *pu2Begin, U16 *pu2End, U16 *pu2Setp)
{
    ...
    if (u1VrefScanEnable == TX_K_VREF_SCAN)
    {
    #if (SUPPORT_SAVE_TIME_FOR_CALIBRATION && BYPASS_VREF_CAL)
        if (p->femmc_Ready == 1)
        {
...
#if TX_WPULL_K
static TXSetPerbitWeakPull(DRAMC_CTX_T *p, U8 u1Enable, U8 u1Levels[])
{
    
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ16), u1Enable, SHU_B0_DQ16_RG_TX_ARDQ_VREFCAL_SEL_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ16), u1Enable, SHU_B1_DQ16_RG_TX_ARDQ_VREFCAL_SEL_B1);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DRV_WPULL), P_Fld(u1Levels[0], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ0_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[1], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ1_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[2], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ2_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[3], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ3_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[4], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ4_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[5], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ5_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[6], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ6_DRV_WPULL_B0)
                                                                    | P_Fld(u1Levels[7], SHU_RK_B0_DRV_WPULL_RG_TX_ARDQ7_DRV_WPULL_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DRV_WPULL), P_Fld(u1Levels[8], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ0_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[9], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ1_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[10], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ2_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[11], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ3_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[12], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ4_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[13], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ5_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[14], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ6_DRV_WPULL_B1)
                                                                    | P_Fld(u1Levels[15], SHU_RK_B1_DRV_WPULL_RG_TX_ARDQ7_DRV_WPULL_B1));
}
#endif
    ...
    U8 ucdq_pi[], U8 ucdqm_pi[], U8 ucdq_ui[], U8 ucdqm_ui[], U8 ucdq_mck[], U8 ucdqm_mck[])
{
    ...
    if (calType == TX_DQ_DQS_MOVE_DQ_ONLY || calType == TX_DQ_DQS_MOVE_DQM_ONLY || calType == TX_DQ_DQS_MOVE_FASTK) {
        //make a copy to dramc reg for TX DQ tracking used
        if (calType == TX_DQ_DQS_MOVE_DQ_ONLY || calType == TX_DQ_DQS_MOVE_FASTK) {
...
void TxUpdateTrackingMCK(DRAMC_CTX_T *p)
{
	...
	for (byte_idx = 0; byte_idx < p->data_width / DQS_BIT_NUMBER ; byte_idx++) {
		...
		mcSHOW_DBG_MSG("%s: DQ/DQM from B%d 0x%x 0x%x --> ", __func__, byte_idx, dq_value[byte_idx], dqm_value[byte_idx]);
		...
		if (((dq_value[byte_idx] >> oft) & 0xf) == 0) {
		...
		#if (TX_K_DQM_MODE != 1)
		if (((dqm_value[byte_idx] >> oft) & 0xf) == 0) {
			mcSHOW_ERR_MSG("\n[%s] Error : DQ/DQM MCK = 0 when enable WDBI!!!\n", __func__);
			ASSERT(0);
		} else {
...
void TXSetDelayReg_DQ(DRAMC_CTX_T *p, U8 u1UpdateRegUI, U8 ucdq_ui_large[], U8 ucdq_oen_ui_large[], U8 ucdq_ui_small[], U8 ucdq_oen_ui_small[], U8 ucdql_pi[])
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), ucdql_pi[0], SHU_RK_B0_DQ_SW_ARPI_DQ_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), ucdql_pi[1], SHU_RK_B1_DQ_SW_ARPI_DQ_B1);
}
...
void TXSetDelayReg_DQM(DRAMC_CTX_T *p, U8 u1UpdateRegUI, U8 ucdqm_ui_large[], U8 ucdqm_oen_ui_large[], U8 ucdqm_ui_small[], U8 ucdqm_oen_ui_small[], U8 ucdqm_pi[])
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), ucdqm_pi[0], SHU_RK_B0_DQ_SW_ARPI_DQM_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), ucdqm_pi[1], SHU_RK_B1_DQ_SW_ARPI_DQM_B1);
}
...
void TXSetDelayReg_DelayLine(DRAMC_CTX_T *p, U16 u2DelayCellOfst[])
{
    ...
    if(u1IsPhaseMode_DQ(p))
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY3), u2DelayCellOfst[0], SHU_RK_B0_TXDLY3_TX_ARDQM0_DLY_B0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY3), u2DelayCellOfst[8], SHU_RK_B1_TXDLY3_TX_ARDQM0_DLY_B1);
    }
}
...
void TXSetDelayReg_RDQS(DRAMC_CTX_T *p, U8 u1UpdateRegUI, U8 ucdqs_ui_large[], U8 ucdqs_oen_ui_large[], U8 ucdqs_ui_small[], U8 ucdqs_oen_ui_small[], U8 ucdqs_pi[])
{
    ...
    /* RDQS_t uses DQM PI */
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), ucdqs_pi[0], SHU_RK_B0_DQ_SW_ARPI_DQM_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), ucdqs_pi[1], SHU_RK_B1_DQ_SW_ARPI_DQM_B1);
}
...
static void Tx_Auto_K_Init(DRAMC_CTX_T *p, DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType, U8 ucdq_pi[], U8 u1PI_Len, U16 u2DQDelayStep)
{
    ...
    if (calType == TX_DQ_DQS_MOVE_DQ_DQM)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_ATK_SET0),
                                     P_Fld(ucdq_pi[0], TX_ATK_SET0_TX_ATK_DQ_B0_PI_INIT) |    //Set begin position of DQ B0
                                     P_Fld(ucdq_pi[0], TX_ATK_SET0_TX_ATK_DQ_B1_PI_INIT) |    //Set begin position of DQ B1
                                     P_Fld(ucdq_pi[1], TX_ATK_SET0_TX_ATK_DQM_B0_PI_INIT) |   //Set begin position of DQM B0
                                     P_Fld(ucdq_pi[1], TX_ATK_SET0_TX_ATK_DQM_B1_PI_INIT));   //Set begin position of DQM B1
    }
    else if (calType == TX_DQ_DQS_MOVE_DQM_ONLY)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_ATK_SET0),
                                     P_Fld(ucdq_pi[0], TX_ATK_SET0_TX_ATK_DQM_B0_PI_INIT) |
                                     P_Fld(ucdq_pi[1], TX_ATK_SET0_TX_ATK_DQM_B1_PI_INIT));
    }
	else
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_TX_ATK_SET1), 0x1, TX_ATK_SET1_TX_ATK_DQ_PI_EN);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_ATK_SET0),
                                     P_Fld(ucdq_pi[0], TX_ATK_SET0_TX_ATK_DQ_B0_PI_INIT) |
                                     P_Fld(ucdq_pi[1], TX_ATK_SET0_TX_ATK_DQ_B1_PI_INIT));
    }
...
DRAM_STATUS_T DramcTxWindowPerbitCal(DRAMC_CTX_T *p, DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType, TX_K_VREF_DFE_OPTION Vref_DFE_SCAN, u8 isAutoK)
{
...
#if TX_DFE_K && SUPPORT_TYPE_LPDDR5
    PASS_WIN_DATA_T DFEWinPerBit[DQ_DATA_WIDTH];
    U8 u1DFELevel, u1DFEBegin=0, u1DFEEnd=0, u1DFEStep=1, u1Set_DFE[ALL_BYTES]={0}, u1Final_DFE[ALL_BYTES]={0}, u1DFE_byte=0, u1choose_DFE=0;
    U32 u4DFE_min_winsize[ALL_BYTES]={0xff,0xff}, u4DFE_TempWinSum[ALL_BYTES]={0};
#endif
#if TX_WPULL_K
    U8 u1WPullLevel=0, u1WPullBegin=0, u1WPullEnd=0, u1WPullStep=1;
    U8 u1Final_WPull[DQ_DATA_WIDTH] = {0};
    U8 u1WPullLevelDQ[DQ_DATA_WIDTH] = {0};
    U16 u2WPull_max_winsize[DQ_DATA_WIDTH]={0};
#endif

#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
    U8 u1mck2ui_div; // initial by p, move later
    U8 u1ui2pi = 32;
    U16 u2TempDQMDelay = 0;
#endif
    ...
    #if TX_AUTO_K_SUPPORT
    U8 u1PI_Len, u1dq_shift, u1PI_init[DQS_NUMBER]= {0};
    ...
    U32 u4RegBackupAddress[] =
    {
        (MIX_RG_CHECK(DRAMC_REG_SHURK_SELPH_DQ0)),
        (MIX_RG_CHECK(DRAMC_REG_SHURK_SELPH_DQ2)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_R0_B0_DQ0)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_R0_B1_DQ0)),
        (MIX_RG_CHECK(DRAMC_REG_SHURK_SELPH_DQ1)),
    ...
    TXScanRange_Vref(p, Vref_DFE_SCAN, &u2FinalRange, &u2VrefBegin, &u2VrefEnd, &u2VrefStep);

#if TX_WPULL_K && SUPPORT_TYPE_LPDDR5
    if (Vref_DFE_SCAN == TX_K_WPULL_SCAN)
    {
        u1WPullBegin=TX_WPULL_RANGE_BEGIN, u1WPullEnd=TX_WPULL_RANGE_END, u1WPullStep=TX_WPULL_RANGE_STEP;
    }
#endif

#if TX_DFE_K && SUPPORT_TYPE_LPDDR5
    if (Vref_DFE_SCAN == TX_K_DFE_SCAN)
    {
        u1DFEBegin = 0;
        u1DFEEnd = 3;
        u1DFEStep = 1;
        ...
        for (u1ByteIdx = 0; u1ByteIdx < (p->data_width / DQS_BIT_NUMBER); u1ByteIdx++)
        {
            u4min_winsize_DFE[u1ByteIdx] = 0;
            u4TempWinSum_DFE[u1ByteIdx] = 0;
        }
    }
#endif
...
#endif
{
    ...
    if (calType == TX_DQ_DQS_MOVE_DQM_ONLY)
    {
        DramcEngine2Init(p, &ta2_cfg, TA2_SEL_DQM_TGL_CFG);
    }
    else
#endif
    {
...
#if TX_WPULL_K && SUPPORT_TYPE_LPDDR5
    for (u1WPullLevel = u1WPullBegin; u1WPullLevel <= u1WPullEnd; u1WPullLevel += u1WPullStep)
#endif
    {
        #if TX_WPULL_K && SUPPORT_TYPE_LPDDR5
        if (Vref_DFE_SCAN == TX_K_WPULL_SCAN)
        {
            mcSHOW_DBG_MSG("set u1WPullLevel to 0x%x\n", u1WPullLevel);
            memset(u1WPullLevelDQ, u1WPullLevel, sizeof(u1WPullLevelDQ));
            TXSetPerbitWeakPull(p, ENABLE, u1WPullLevelDQ);
        }
        else
        {
            mcSHOW_DBG_MSG("\n\tTX WPull Scan disable\n");
        }
        #endif

        #if TX_DFE_K && SUPPORT_TYPE_LPDDR5
        for (u1DFELevel = u1DFEBegin; u1DFELevel <= u1DFEEnd; u1DFELevel += u1DFEStep)
        #endif
        {
            #if TX_DFE_K && SUPPORT_TYPE_LPDDR5
            if (Vref_DFE_SCAN == TX_K_DFE_SCAN)
            {
                for (u1ByteIdx = 0; u1ByteIdx < (p->data_width / DQS_BIT_NUMBER); u1ByteIdx++)
                {
                    u1Set_DFE[u1ByteIdx] = u1DFELevel;
                }

                DramcTXSetDFE(p, u1Set_DFE);
                mcSHOW_DBG_MSG2(("Set DFE to 0x%x\n",u1DFELevel));
            }
            #endif
            ...
            for (u2VrefLevel = u2VrefBegin; u2VrefLevel <= u2VrefEnd; u2VrefLevel += u2VrefStep)
            {
                ...
                if (Vref_DFE_SCAN == TX_K_VREF_SCAN)
                {
                    #if (!REDUCE_LOG_FOR_PRELOADER)
                ...
                if (Vref_DFE_SCAN == TX_K_DFE_SCAN)
                {
                    for (u1ByteIdx = 0; u1ByteIdx < (p->data_width / DQS_BIT_NUMBER); u1ByteIdx++)
                    {
                        u4DFE_min_winsize[u1ByteIdx] = (S16)PASS_RANGE_NA;
                        u4DFE_TempWinSum[u1ByteIdx] = 0;
                    }
                }
                #endif
                ...
                {
                #if TX_AUTO_K_SUPPORT
                u1PI_init[0] = u1PI_init[1] = ucdq_pi;
                Tx_Auto_K_Init(p, calType, u1PI_init, u1PI_Len, u2DQDelayStep); //u1PI_Len = 1 means that PI len is 64 PI
                ...
                for (uiDelay = u2DQDelayBegin; uiDelay < u2DQDelayEnd; uiDelay += u2DQDelayStep)
                {
                    ...
                    //audio + xtalk pattern
                    u4err_value = DramcEngine2Run(p, &ta2_cfg);
                    ta2_cfg.pattern = TEST_AUDIO_PATTERN;
                    ...
                    if ((Vref_DFE_SCAN == TX_K_ONLY_TIME_DOMAIN) && (calType != TX_DQ_DQS_MOVE_DQM_ONLY))
                    {
                    ...
                    for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
                    {
                        ...
                        if (WinPerBit[u1BitIdx].first_pass == PASS_RANGE_NA)
                        {
                            if (u4fail_bit == 0) //compare correct: pass
                            {
                                ...
                                #if TX_TDQS2DQ_PRE_CAL
                                if ((u1IsLP4Div4DDR800(p) == FALSE) && (calType == TX_DQ_DQS_MOVE_DQ_ONLY) && (Vref_DFE_SCAN == TX_K_ONLY_TIME_DOMAIN))
                                {
                                    if (u2DQS2DQ_Pre_Cal[p->channel][p->rank][vGet_Div_Mode(p)] == 0)
                                    {
                ...
                for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
                {
                    ...
                    VrefWinPerBit[u1BitIdx].first_pass = u2DQDelayBegin - ucdq_pi + ((u4IO32Read4B(DRAMC_REG_ADDR(PwMaxInitReg[u1BitIdx / 4])) & (0xffUL << u1dq_shift)) >> u1dq_shift);
                    VrefWinPerBit[u1BitIdx].last_pass = ((u4IO32Read4B(DRAMC_REG_ADDR(PwMaxLenReg[u1BitIdx / 4])) & (0xffUL << u1dq_shift)) >> u1dq_shift) + VrefWinPerBit[u1BitIdx].first_pass - u2DQDelayStep;
                    VrefWinPerBit[u1BitIdx].win_size = ((u4IO32Read4B(DRAMC_REG_ADDR(PwMaxLenReg[u1BitIdx / 4])) & (0xffUL << u1dq_shift)) >> u1dq_shift) - u2DQDelayStep + 1;
                    ...
                    if(Vref_DFE_SCAN == TX_K_DFE_SCAN)
                    {
                        if (u1BitIdx < (DQ_DATA_WIDTH/2))
                            u1DFE_byte = BYTE_0;
                        else
                            u1DFE_byte = BYTE_1;

                        if (VrefWinPerBit[u1BitIdx].win_size < u4DFE_min_winsize[u1DFE_byte])
                        {
                            u4DFE_min_winsize[u1DFE_byte] = VrefWinPerBit[u1BitIdx].win_size;
                        }

                        u4DFE_TempWinSum[u1DFE_byte] += VrefWinPerBit[u1BitIdx].win_size;
                    }
                    #endif
                    ...
                    if(Vref_DFE_SCAN == TX_K_WPULL_SCAN)
                    {
                        if(VrefWinPerBit[u1BitIdx].win_size > u2WPull_max_winsize[u1BitIdx])
                        {
                            u1Final_WPull[u1BitIdx] = u1WPullLevel;
                            u2WPull_max_winsize[u1BitIdx] = VrefWinPerBit[u1BitIdx].win_size;
                        }
                    }
                    #endif
                ...
                if (Vref_DFE_SCAN != TX_K_VREF_SCAN)
                {
                    ...
                    if(Vref_DFE_SCAN == TX_K_DFE_SCAN)
                    {
                        mcSHOW_DBG_MSG2(("B0 Window_Sum = %d, Window_min = %d\n",u4DFE_TempWinSum[0],u4DFE_min_winsize[0]));
                        mcSHOW_DBG_MSG2(("B1 Window_Sum = %d, Window_min = %d\n",u4DFE_TempWinSum[1],u4DFE_min_winsize[1]));
                    }
                    #endif
                }
                #endif
                ...
                #if LP4_TX_VREF_PASS_CONDITION
                if ((Vref_DFE_SCAN == TX_K_VREF_SCAN) && (u2TempWinSum < (u2MaxWindowSum * 95 / 100)) && (u1min_winsize < LP4_TX_VREF_PASS_CONDITION))
                #else
                if ((Vref_DFE_SCAN == TX_K_VREF_SCAN) && (u2TempWinSum < (u2MaxWindowSum * 95 / 100)) && (u1min_winsize > TX_PASS_WIN_CRITERIA))
                #endif
                {
            ...
            #if TX_DFE_K && SUPPORT_TYPE_LPDDR5
            if (Vref_DFE_SCAN == TX_K_DFE_SCAN)
                TxChooseDFE(p, u4DFE_min_winsize, u4DFE_TempWinSum, u1Final_DFE, u1DFELevel);
            #endif
        }
    }//end for WPULL scan
        DramcEngine2End(p);
        ...
        if (Vref_DFE_SCAN == TX_K_ONLY_TIME_DOMAIN)// ..if time domain (not vref scan) , calculate window center of all bits.
        {
            ...
            for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
            {
                ...
                for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
                {
                    ...
                    #ifdef FOR_HQA_TEST_USED
                    if ((calType == TX_DQ_DQS_MOVE_DQ_ONLY) && (Vref_DFE_SCAN == TX_K_ONLY_TIME_DOMAIN))
                    {
    ...
    if (Vref_DFE_SCAN == TX_K_VREF_SCAN)
    {
        ...
        if (p->femmc_Ready == 0)////save firtst run Vref value
        {
            ...
            p->pSavetimeData->u1TxWindowPerbitVrefRange_Save[p->channel][p->rank] = u2FinalRange;
        }
        #endif
    ...
    if (Vref_DFE_SCAN == TX_K_WPULL_SCAN)
    {
        ...
        TXSetPerbitWeakPull(p, ENABLE, u1Final_WPull);
        for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
        {
            #if SUPPORT_SAVE_TIME_FOR_CALIBRATION
            p->pSavetimeData->u1TX_PerBit_WPullLevel_Save[p->channel][p->rank][u1BitIdx] = u1Final_WPull[u1BitIdx];
            p->pSavetimeData->u1TX_PerBit_WPullLevel_Save[p->channel][p->rank][u1BitIdx+8] = u1Final_WPull[u1BitIdx+8];
            #endif
            mcSHOW_DBG_MSG("TX Bit%d max_win=%d wpull=%x,   Bit%d max_win=%d wpull=%x,\n", 
                u1BitIdx, u2WPull_max_winsize[u1BitIdx], u1Final_WPull[u1BitIdx], 
                u1BitIdx + 8, u2WPull_max_winsize[u1BitIdx + 8], u1Final_WPull[u1BitIdx + 8]);
        }
        return DRAM_OK;
    }
    #endif
    ...
    if (Vref_DFE_SCAN == TX_K_DFE_SCAN)
    {
        mcSHOW_DBG_MSG2(("B0 TX Set final DFE : CH%d Rank%d, TX DFE = 0x%x\n", p->channel, p->rank, u1Final_DFE[0]));
        mcSHOW_DBG_MSG2(("B1 TX Set final DFE : CH%d Rank%d, TX DFE = 0x%x\n", p->channel, p->rank, u1Final_DFE[1]));
        DramcTXSetDFE(p, u1Final_DFE);
        return DRAM_OK;
    }
    #endif
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        ...
        //tx perbit dly cell will compensate avg-center%8 for semi-open freqs
        uiDelay = (u1EnableDelayCell == 0)? u2DQM_Delay : u2Center_min[u1ByteIdx];

        if ((p->support_rank_num==RANK_DUAL) && (u1IsPhaseMode_DQ(p)))
        {
             if (p->rank == RANK_0)
             {
                 u2Delay_open_mode_bk[calType][u1ByteIdx] = uiDelay;
                 u2DQM_Delay_open_mode_bk[calType][u1ByteIdx] = u2DQM_Delay;
             }
             else if (p->rank == RANK_1)
             {
                 uiDelay = (u2Delay_open_mode_bk[calType][u1ByteIdx] + uiDelay) >> 1;
                 u2DQM_Delay = (u2DQM_Delay_open_mode_bk[calType][u1ByteIdx] + u2DQM_Delay) >> 1;
                 
                 if ((uiDelay % 8)<4)
                 {
                     mcSHOW_DBG_MSG2("PhaseMode adjust DQ PI (-%d)\n", (uiDelay % 8));
                     uiDelay = uiDelay - (uiDelay % 8);
                 }
                 else //((uiDelay % 8)>=4)
                 {
                     mcSHOW_DBG_MSG2("PhaseMode adjust DQ PI (+%d)\n", 8-(uiDelay % 8));
                     uiDelay = uiDelay - (uiDelay % 8)+8;
                 }
                 if ((u2DQM_Delay % 8)<4)
                 {
                     mcSHOW_DBG_MSG2("PhaseMode adjust DQM PI (-%d)\n", (u2DQM_Delay % 8));
                     u2DQM_Delay = u2DQM_Delay - (u2DQM_Delay % 8);
                 }
                 else //((uiDelay % 8)>=4)
                 {
                     mcSHOW_DBG_MSG2("PhaseMode adjust DQM PI (+%d)\n", 8-(u2DQM_Delay % 8));
                     u2DQM_Delay = u2DQM_Delay - (u2DQM_Delay % 8)+8;
                 }
             }
        }
        ...
        if (u1EnableDelayCell)
        {
            ...
            for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
            {
                ...
                if (p->u2DelayCellTimex100 != 0)
                {
                    ...
                    #if ENABLE_APHY_GLITCH_WA_FORCE_32PI
                    //set delayline LSB=0x3 to improve serializer odd data timing
                    if(p->frequency>=3200)
                    {
                        if((u2DelayCellOfst[u1BitTemp]&0x3)==0 && u2DelayCellOfst[u1BitTemp]>0x3)
                        {
                            u2DelayCellOfst[u1BitTemp] -= 0x1;
                        }
                        else
                        {
                            u2DelayCellOfst[u1BitTemp] |= 0x3;
                        }
                        mcSHOW_DBG_MSG("--> WA = %d cells\n", u2DelayCellOfst[u1BitTemp]);
                    }
                    #endif
        ...
        if(p->femmc_Ready == 0)
        {
            u1mck2ui_div = u1MCK2UI_DivShift(p);

            if ((p->rank==RANK_1) && (u1IsPhaseMode_DQ(p)))
                u1rank_start = RANK_0;
            else
                u1rank_start = p->rank;
            ...
            for (u1RankIdx = u1rank_start; u1RankIdx < p->support_rank_num; u1RankIdx++)
            {
                ...
                if (calType == TX_DQ_DQS_MOVE_DQM_ONLY)
                {
                    ...
                    if ((p->DBI_W_onoff[p->dram_fsp]==DBI_ON))
                        u2TempDQMDelay = u2DQM_Delay + (1 << u1mck2ui_div) * u1ui2pi;
                    else
                        u2TempDQMDelay = u2DQM_Delay;
                #endif
                    p->pSavetimeData->u1TX_PerBit_DQM_Only_Save[p->channel][p->rank][u1ByteIdx] = u2TempDQMDelay;
                }
                else if((calType == TX_DQ_DQS_MOVE_DQ_ONLY) && (Vref_DFE_SCAN == TX_K_ONLY_TIME_DOMAIN))
                {
        ...
        if (calType == TX_DQ_DQS_MOVE_DQ_ONLY || calType == TX_DQ_DQS_MOVE_DQ_DQM)
        {
            HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0, "TX_Window_Center_", "Byte_DQ", u1ByteIdx, uiDelay, NULL);
        }
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcTxWindowPerbitCal(DRAMC_CTX_T *p, DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType)
{
    U8 u1BitTemp, u1BitIdx, u1ByteIdx, u1RankIdx, backup_rank;
    ...
    mcSHOW_DBG_MSG("[TxWindowPerbitCal]\n");
    ...
    if (Get_Vref_Calibration_OnOff(p)==VREF_CALI_ON)
    {
        ...
        u2FinalRange = p->pSavetimeData->u1TxWindowPerbitVrefRange_Save[p->channel][p->rank];
        TXSetFinalVref(p, u2FinalRange, u2FinalVref);
    }
    #endif
    ...
    // LP3 only use "TX_DQ_DQS_MOVE_DQ_DQM" scan
    // first freq 800(LP4-1600) doesn't support jitter meter(data < 1T), therefore, don't use delay cell
    if ((p->frequency >= 1333 ) && (p->u2DelayCellTimex100 != 0))
    {
        u1EnableDelayCell = 1;
        mcSHOW_DBG_MSG("[TX_PER_BIT_DELAY_CELL] DelayCellTimex100 =%d/100 ps\n", p->u2DelayCellTimex100);
    }
    ...
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        ...
        if (u1EnableDelayCell)
        {
            for (u1BitIdx = 0; u1BitIdx < DQS_BIT_NUMBER; u1BitIdx++)
            {
                ...
                mcSHOW_DBG_MSG("u2DelayCellOfst[%d]=%d cells\n", u1BitTemp, u2DelayCellOfst[u1BitTemp]);
            }
        }
    ...
    for (u1RankIdx = p->rank; u1RankIdx < RANK_MAX; u1RankIdx++)
    {
        ...
        #if TX_WPULL_K
        TXSetPerbitWeakPull(p, (p->frequency>=TX_WPULL_K_THRESHOLD)? ENABLE : DISABLE, p->pSavetimeData->u1TX_PerBit_WPullLevel_Save[p->channel][p->rank]);
        #endif
...
#if IPM_VERSION >= 22
#define ENABLE_CONTINUIOUS_WFF_RFF   0
#endif
...
#if APPLY_NEW_IO_API
        vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ13),
            PB_Fld(0x0, SHU_B1_DQ13_RG_TX_ARDQS_READ_BASE_DATA_TIE_EN_B1),
            PB_Fld(0x1, SHU_B1_DQ13_RG_TX_ARDQS_READ_BASE_EN_B1));
#else
...
static void TXFifoModeInit(DRAMC_CTX_T *p, struct TxPerbitFifoCal *strTxCal)
{
   ...
   //WRFIFO and RDFIFO's golden data
    ...
    if (is_lp5_family(p)) {
        ...
        DramcMRWriteFldAlign(p, 22, 1, MR22_WRITE_ECC, TO_MR);
        ...
        s1RL_Changed += vSwitchRDBI_RECC_Setting(p, OFF, READ_OPT_CHG_ECC, TO_MR_ONLY, TRUE);
#endif

#if ENABLE_READ_LINK_ECC_FOR_RX_DMI
        s1RL_Changed += vSwitchRDBI_RECC_Setting(p, OFF, READ_OPT_CHG_DBI, TO_MR_ONLY, TRUE);
        s1RL_Changed += vSwitchRDBI_RECC_Setting(p, ON, READ_OPT_CHG_ECC, TO_MR_ONLY, TRUE);
#endif
    }
#endif

#if ENABLE_READ_DBI_FOR_RX_DMI
    s1RL_Changed += vSwitchRDBI_RECC_Setting(p, ON, READ_OPT_CHG_DBI, TO_MR_ONLY, TRUE);
#endif
...
static U8 TxFifoModeXferVirtPIToReal(DRAMC_CTX_T *p, struct TxPerbitFifoDly *strTxDly, U8 u1AdjustPIToCenter)
{
    ...
    #if ENABLE_TX_TRACKING
    if (is_lp4_family(p) && u1AdjustPIToCenter && u1IsPhaseMode)
    {
        if (u1PI < 10)
        {
            u1PI += (u1PiTap) >> 1;
...
static void TxFifoModeFinalSet(DRAMC_CTX_T *p, struct TxPerbitFifoWin *strTxWin, DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType)
{
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY0),
        P_Fld(strTxWin->strDqWin[3].u2Dline, SHU_RK_B0_TXDLY0_TX_ARDQ3_DLY_B0) |
        P_Fld(strTxWin->strDqWin[2].u2Dline, SHU_RK_B0_TXDLY0_TX_ARDQ2_DLY_B0) |
        P_Fld(strTxWin->strDqWin[1].u2Dline, SHU_RK_B0_TXDLY0_TX_ARDQ1_DLY_B0) |
        P_Fld(strTxWin->strDqWin[0].u2Dline, SHU_RK_B0_TXDLY0_TX_ARDQ0_DLY_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_TXDLY1),
        P_Fld(strTxWin->strDqWin[7].u2Dline, SHU_RK_B0_TXDLY1_TX_ARDQ7_DLY_B0) |
        P_Fld(strTxWin->strDqWin[6].u2Dline, SHU_RK_B0_TXDLY1_TX_ARDQ6_DLY_B0) |
        P_Fld(strTxWin->strDqWin[5].u2Dline, SHU_RK_B0_TXDLY1_TX_ARDQ5_DLY_B0) |
        P_Fld(strTxWin->strDqWin[4].u2Dline, SHU_RK_B0_TXDLY1_TX_ARDQ4_DLY_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY0),
        P_Fld(strTxWin->strDqWin[11].u2Dline, SHU_RK_B1_TXDLY0_TX_ARDQ3_DLY_B1) |
        P_Fld(strTxWin->strDqWin[10].u2Dline, SHU_RK_B1_TXDLY0_TX_ARDQ2_DLY_B1) |
        P_Fld(strTxWin->strDqWin[9].u2Dline, SHU_RK_B1_TXDLY0_TX_ARDQ1_DLY_B1) |
        P_Fld(strTxWin->strDqWin[8].u2Dline, SHU_RK_B1_TXDLY0_TX_ARDQ0_DLY_B1));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_TXDLY1),
        P_Fld(strTxWin->strDqWin[15].u2Dline, SHU_RK_B1_TXDLY1_TX_ARDQ7_DLY_B1) |
        P_Fld(strTxWin->strDqWin[14].u2Dline, SHU_RK_B1_TXDLY1_TX_ARDQ6_DLY_B1) |
        P_Fld(strTxWin->strDqWin[13].u2Dline, SHU_RK_B1_TXDLY1_TX_ARDQ5_DLY_B1) |
        P_Fld(strTxWin->strDqWin[12].u2Dline, SHU_RK_B1_TXDLY1_TX_ARDQ4_DLY_B1));
}
...
static void TxFifoModeTrigger_RTSWCmd(DRAMC_CTX_T *p, U32 *u4DQCmpResult, U32 *u4DQMCmpResult)
{
    ...
    *u4DQMCmpResult = u4TmpResult;
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SWCMD_EN),
            0, SWCMD_EN_RTSWCMDEN);
#else /* ENABLE_CONTINUIOUS_WFF_RFF */
    RTSWCMD_PARAM_T param;

    memset((void *)&param, 0, sizeof param);
    param.rank = p->rank;
#if ENABLE_FIFO_DQM_PAT
    param.rddqc_rff_enable_dqmk = 1;
#endif
    param.selector = RUNTIME_SWCMD_WFF;
    DramcTriggerRTSWCMD(p, &param);

    param.selector = RUNTIME_SWCMD_RFF;
    DramcTriggerRTSWCMD(p, &param);

    *u4DQCmpResult = param.rddqc_rff_cmp_result & 0xffff;
#if ENABLE_FIFO_DQM_PAT
    *u4DQMCmpResult = (param.rddqc_rff_cmp_result >> 16) & 0x3;
#endif
#endif
}
#endif
...
#if ENABLE_K_WITH_WORST_SI_UI_SHIFT
    DramcEngine2SelectConfig(p, &ta2_cfg, TA2_SEL_WORST_SI_CFG);
    //DramcEngine2SetPat(p, p->test_pattern, 0, 0, TE_UI_SHIFT);
    u4err_value = DramcEngine2Run(p, &ta2_cfg);
#else
    //audio + xtalk pattern
    DramcEngine2SelectConfig(p, &ta2_cfg, TA2_SEL_DEFAULT_CFG);
    u4err_value = DramcEngine2Run(p, &ta2_cfg);
    ta2_cfg.pattern = TEST_AUDIO_PATTERN;
    DramcEngine2SetPat(p, &ta2_cfg);
    u4err_value |= DramcEngine2Run(p, &ta2_cfg);
...
static void TxFifoMode_Cleanup(DRAMC_CTX_T *p, struct TxPerbitFifoCal *strTxCal)
{
    ...
    if (is_lp5_family(p)) {
#if ENABLE_READ_LINK_ECC_FOR_RX_DMI
        vSwitchRDBI_RECC_Setting(p, OFF, READ_OPT_CHG_ECC, TO_MR_ONLY, TRUE);
#endif

#if ENABLE_RDQS_T_TRAINING
        DramcMRWriteFldAlign(p, 22, OFF, MR22_WRITE_ECC, TO_MR);
    ...
    DRAM_TX_PER_BIT_CALIBRATION_TYTE_T calType, U8 u1VrefScanEnable, u8 isAutoK)
{
    ...
    U32 u4RegBackupAddress[] = {
        (MIX_RG_CHECK(DRAMC_REG_INITK_PAT3)),
        (MIX_RG_CHECK(DRAMC_REG_INITK_PAT2)),
        (MIX_RG_CHECK(DRAMC_REG_INITK_PAT1)),
        (MIX_RG_CHECK(DRAMC_REG_INITK_PAT0)),
        (MIX_RG_CHECK(DRAMC_REG_SWCMD_EN)),
        (MIX_RG_CHECK(DRAMC_REG_SWCMD_CTRL0)),
        (MIX_RG_CHECK(DRAMC_REG_CKECTRL)),
        (MIX_RG_CHECK(DRAMC_REG_RK_REF_CTRL)),
        (MIX_RG_CHECK(DRAMC_REG_HMR4)),
        (MIX_RG_CHECK(DRAMC_REG_SHU_TX_SET0)), /* WDBI */
        (MIX_RG_CHECK(DRAMC_REG_TX_SET0)), /* RDQS_FIFO_TRAINING_MODE */
#if !APPLY_NEW_IO_API
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ7)), /* RDBI */
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ7)), /* RDBI */
#endif
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ13)), /* DQS_DATA_TIE_EN, *RDQS_MCKIO_SEL & DQS OE TIE */
#if !APPLY_NEW_IO_API  //@cc: will report MIX RG error. Shall be processed alone. Backup this is for OE disable usage..
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ13)), /* DQS_DATA_TIE_EN */
#endif
        (MIX_RG_CHECK(DDRPHY_REG_MISC_SHU_STBCAL)), /*Gating Tracking */
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DLL2)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DLL2)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_CTRL1)), /* ARPIDQ_SW */
    };

#if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQS_READ_BASE_DATA_TIE_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQS_READ_BASE_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQS_MCKIO_SEL_B1},
        {DDRPHY_REG_SHU_B0_DQ7, SHU_B0_DQ7_R_DMDQMDBI_SHU_B0},
        {DDRPHY_REG_SHU_B1_DQ7, SHU_B1_DQ7_R_DMDQMDBI_SHU_B1},
    };
#endif
...
void Dramc_K_TX_EyeScan_Log(DRAMC_CTX_T *p)
{
    ...
    U8 ucdq_pi[DQS_NUMBER], ucdq_ui_small, ucdq_ui_large,ucdq_oen_ui_small, ucdq_oen_ui_large;
    U32 uiFinishCount;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DRAMC_REG_SHURK_SELPH_DQ1)),
        (MIX_RG_CHECK(DRAMC_REG_SHURK_SELPH_DQ3)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_DQ)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_DQ)),
    };
    ...
    for(u1ByteIdx=0; u1ByteIdx < p->data_width/DQS_BIT_NUMBER; u1ByteIdx++)
    {
        if (u1ByteIdx == 0)
        {
            tx_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), SHURK_SELPH_DQ0_TXDLY_DQ0)<<(u1MCK2UI+u1UI2PI)) +
                          ...
                          u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_SW_ARPI_DQ_B0);
        }
        else
        {
            tx_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), SHURK_SELPH_DQ0_TXDLY_DQ1)<<(u1MCK2UI+u1UI2PI)) +
                          ...
                          u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_SW_ARPI_DQ_B1);
        }
    }
    ...
    u2VrefRange = 0;
    u2VrefBegin = 0;
    u2VrefEnd = (is_lp5_family(p)?VREF_VOLTAGE_TABLE_NUM_LP5:VREF_VOLTAGE_TABLE_NUM_LP4)-1;
...
//fra        u1MR14Value[p->channel][p->rank][p->dram_fsp] = (u2VrefLevel | (u2VrefRange<<6));
        ...
        #if TX_AUTO_K_SUPPORT
        //Set B0 delay
        TxWinTransferDelayToUIPI(p, tx_pi_delay[0]-32, 0, &ucdq_ui_large, &ucdq_ui_small, &ucdq_pi[0], &ucdq_oen_ui_large, &ucdq_oen_ui_small);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), \
        ...
        TxWinTransferDelayToUIPI(p, tx_pi_delay[1]-32, 0, &ucdq_ui_large, &ucdq_ui_small, &ucdq_pi[1], &ucdq_oen_ui_large, &ucdq_oen_ui_small);
        //Set B1 delay
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), \
        ...
        Tx_Auto_K_Init(p, TX_DQ_DQS_MOVE_DQ_ONLY, ucdq_pi, 0x3,(u1IsPhaseMode_DQ(p)==TRUE ? 8 : 1)); 
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_TX_ATK_SET1), 0x1, TX_ATK_SET1_TX_ATK_TRIG); //TX Auto K start
        Tx_Auto_K_complete_check(p,TX_DQ_DQS_MOVE_DQ_ONLY);

        for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
        {
            u1dq_shift = ((u1BitIdx % 4) * 8);
            u1byte = u1BitIdx/8;
            WinPerBit[u1BitIdx].first_pass = ((u4IO32Read4B(DRAMC_REG_ADDR(PwMaxInitReg[u1BitIdx / 4])) & (0xff << u1dq_shift)) >> u1dq_shift) - ucdq_pi[u1byte] ;
            WinPerBit[u1BitIdx].last_pass = ((u4IO32Read4B(DRAMC_REG_ADDR(PwMaxLenReg[u1BitIdx / 4])) & (0xff << u1dq_shift)) >> u1dq_shift) + WinPerBit[u1BitIdx].first_pass - (u1IsPhaseMode_DQ(p)==TRUE ? 8 : 1);

			if ((WinPerBit[u1BitIdx].last_pass  != PASS_RANGE_NA) && ((WinPerBit[u1BitIdx].last_pass -WinPerBit[u1BitIdx].first_pass) >= (VrefWinPerBit[u1BitIdx].last_pass -VrefWinPerBit[u1BitIdx].first_pass)))
            {
                VrefWinPerBit[u1BitIdx].first_pass = WinPerBit[u1BitIdx].first_pass;
                VrefWinPerBit[u1BitIdx].last_pass = WinPerBit[u1BitIdx].last_pass;
            }

            //update gEyeScan_TotalPassCount
            gEyeScan_TotalPassCount[u1BitIdx]+=VrefWinPerBit[u1BitIdx].last_pass-VrefWinPerBit[u1BitIdx].first_pass;

            //update gEyeScan_ContinueVrefHeight
            if( WinPerBit[u1BitIdx].last_pass > WinPerBit[u1BitIdx].first_pass)
            {
                gEyeScan_ContinueVrefHeight[u1BitIdx]+= EYESCAN_GRAPH_CATX_VREF_STEP;
            }

            if ((EyeScan_index[u1BitIdx] < EYESCAN_BROKEN_NUM) && (WinPerBit[u1BitIdx].first_pass > 0))
            {
                ...
                gEyeScan_MinMax_store_delay[u1byte] =  tx_pi_delay[u1byte]-32; /* save this information for HQA pass/fail judgement used */
            #endif
                EyeScan_index[u1BitIdx]=EyeScan_index[u1BitIdx]+1;
    ...
    #else
        for (uiDelay=0; uiDelay<64; uiDelay+=(u1IsPhaseMode_DQ(p)==TRUE ? 8 : 1))
        {
            TxWinTransferDelayToUIPI(p, tx_pi_delay[0]+uiDelay-32, 0, &ucdq_ui_large, &ucdq_ui_small, &ucdq_pi[0], &ucdq_oen_ui_large, &ucdq_oen_ui_small);
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), \
                                             P_Fld(ucdq_ui_large, SHURK_SELPH_DQ0_TXDLY_DQ0) | \
                                             P_Fld(ucdq_oen_ui_large, SHURK_SELPH_DQ0_TXDLY_OEN_DQ0));
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ2), \
                                             P_Fld(ucdq_ui_small, SHURK_SELPH_DQ2_DLY_DQ0) | \
                                             P_Fld(ucdq_oen_ui_small, SHURK_SELPH_DQ2_DLY_OEN_DQ0));
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), ucdq_pi[0], SHU_RK_B0_DQ_SW_ARPI_DQ_B0);

            TxWinTransferDelayToUIPI(p, tx_pi_delay[1]+uiDelay-32, 0, &ucdq_ui_large, &ucdq_ui_small, &ucdq_pi[1], &ucdq_oen_ui_large, &ucdq_oen_ui_small);
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), \
                                             P_Fld(ucdq_ui_large, SHURK_SELPH_DQ0_TXDLY_DQ1) | \
                                             P_Fld(ucdq_oen_ui_large, SHURK_SELPH_DQ0_TXDLY_OEN_DQ1));
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ2), \
                                             P_Fld(ucdq_ui_small, SHURK_SELPH_DQ2_DLY_DQ1) | \
                                             P_Fld(ucdq_oen_ui_small, SHURK_SELPH_DQ2_DLY_OEN_DQ1));
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), ucdq_pi[1], SHU_RK_B1_DQ_SW_ARPI_DQ_B1);
...
void DramcTxOECalibration(DRAMC_CTX_T *p)
{
    ...
    #if TX_OE_PATTERN_USE_TA2
        DramcEngine2Init(p, &ta2_cfg, TA2_SEL_TXOE_CFG);//UI_SHIFT + LEN1
    #else
...
void Update_tdmy(DRAMC_CTX_T *p)
{
    ...
    for(s1ChIdx=0; s1ChIdx<p->support_channel_num; s1ChIdx++)
    {
        ...
        #if (fcFOR_CHIP_ID == fcLepin)
        tdmy = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_COMMON0), SHU_COMMON0_SHU_COMMON0_RSV); // jouling: fcLepin ECO
        #else
        ...
        if((TXOE_mck_max+7) > tdmy)
        {
            ...
            tdmy = TXOE_mck_max + 7;
            #if (fcFOR_CHIP_ID == fcLepin)
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_COMMON0), tdmy, SHU_COMMON0_SHU_COMMON0_RSV); // jouling: fcLepin EC
            #else
...
static void OECKCKE_Control(DRAMC_CTX_T *p, U32 option)
{
	 ...
	 */
#if APPLY_NEW_IO_API
	static U32 u4OETieSel[7], u4OETieEn[7];
	static U32 u4CKECtl_bak;
	...
	if (option == DISABLE) {
		...
		//@cc: SHU_CA_CMD13 has MIXed attribute, cannot read by 4B...
		...
		u4CKECtl_bak = u4IO32Read4B(DRAMC_REG_ADDR(DRAMC_REG_CKECTRL));
		u4OETieSel[0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCLK_OE_TIE_SEL_CA);
		u4OETieEn[0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCLK_OE_TIE_EN_CA);
		u4OETieSel[1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD13), SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_SEL_CA);
		u4OETieEn[1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD13), SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_EN_CA);
		/* CKE OE controlled by CS OE */
		u4OETieSel[2] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCS_OE_TIE_SEL_CA);
		u4OETieEn[2] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCS_OE_TIE_EN_CA);
		u4OETieSel[3] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA);
		u4OETieEn[3] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA);
		u4OETieSel[4] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14), SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_SEL_CA);
		u4OETieEn[4] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14), SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_EN_CA);
		u4OETieSel[5] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14), SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_SEL_CA);
		u4OETieEn[5] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14), SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_EN_CA);
		u4OETieSel[6] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCA_OE_TIE_SEL_CA);
		u4OETieEn[6] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), CA_CMD2_RG_TX_ARCA_OE_TIE_EN_CA);
		
		u4backup_done = 1;

		/* To control CKE-CLK timing by HW */
		CKEFixOnOff(p, TO_ALL_RANK, CKE_FIXOFF, TO_ONE_CHANNEL);
		mcDELAY_US(1);

		/* CKE0/1 */
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14),
			P_Fld(0, SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_SEL_CA) |
			P_Fld(1, SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_EN_CA) |
			P_Fld(0, SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_SEL_CA) |
			P_Fld(1, SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_EN_CA));

		/* CS/CA */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), PB_Fld( 0, CA_CMD2_RG_TX_ARCS_OE_TIE_SEL_CA),
															PB_Fld( 1, CA_CMD2_RG_TX_ARCS_OE_TIE_EN_CA),
															PB_Fld( 0, CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA),
															PB_Fld( 1, CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA),
															PB_Fld( 0, CA_CMD2_RG_TX_ARCA_OE_TIE_SEL_CA),
															PB_Fld( 0xff, CA_CMD2_RG_TX_ARCA_OE_TIE_EN_CA));

		/* CLK */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), PB_Fld( 0, CA_CMD2_RG_TX_ARCLK_OE_TIE_SEL_CA),
															PB_Fld( 1, CA_CMD2_RG_TX_ARCLK_OE_TIE_EN_CA));
		
		/* CLKB */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD13), PB_Fld( 0, SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_SEL_CA),
															PB_Fld( 1, SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_EN_CA));
	} else {
		...
		/* CLK */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), PB_Fld( u4OETieSel[0], CA_CMD2_RG_TX_ARCLK_OE_TIE_SEL_CA),
															PB_Fld( u4OETieEn[0], CA_CMD2_RG_TX_ARCLK_OE_TIE_EN_CA));
		
		/* CLKB */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD13), PB_Fld( u4OETieSel[1], SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_SEL_CA ),
															PB_Fld( u4OETieEn[1], SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_EN_CA));

		/* CS/CA */
		vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), PB_Fld( u4OETieSel[2], CA_CMD2_RG_TX_ARCS_OE_TIE_SEL_CA),
															PB_Fld( u4OETieEn[2], CA_CMD2_RG_TX_ARCS_OE_TIE_EN_CA),
															PB_Fld( u4OETieSel[3], CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA),
															PB_Fld( u4OETieEn[3], CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA),
															PB_Fld( u4OETieSel[6], CA_CMD2_RG_TX_ARCA_OE_TIE_SEL_CA),
															PB_Fld( u4OETieEn[6], CA_CMD2_RG_TX_ARCA_OE_TIE_EN_CA));

		/* CKE0/1 */
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD14),
			P_Fld(u4OETieSel[4], SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_SEL_CA) |
			P_Fld(u4OETieEn[4], SHU_CA_CMD14_RG_TX_ARCKE0_OE_TIE_EN_CA) |
			P_Fld(u4OETieSel[5], SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_SEL_CA) |
			P_Fld(u4OETieEn[5], SHU_CA_CMD14_RG_TX_ARCKE1_OE_TIE_EN_CA));

		vIO32Write4B(DRAMC_REG_ADDR(DRAMC_REG_CKECTRL), u4CKECtl_bak);
	...
	if (option == DISABLE) {
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), P_Fld( 0, CA_CMD2_RG_TX_ARCS_OE_TIE_SEL_CA) \
															  | P_Fld( 1, CA_CMD2_RG_TX_ARCS_OE_TIE_EN_CA) \
															  | P_Fld( 0, CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA) \
															  | P_Fld( 1, CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA) \
		...
		U32 u4CS1OE_TieSel, u4CS1OE_TieEn;
		...
		u4CS1OE_TieSel = fld_val(u4CA_CMD2_backup, CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA);
		u4CS1OE_TieEn = fld_val(u4CA_CMD2_backup, CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA);
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD2), P_Fld( u4CSOE_TieSel, CA_CMD2_RG_TX_ARCS_OE_TIE_SEL_CA) \
															  ...
															  | P_Fld( u4CS1OE_TieSel, CA_CMD2_RG_TX_ARCS1_OE_TIE_SEL_CA) \
															  | P_Fld( u4CS1OE_TieEn, CA_CMD2_RG_TX_ARCS1_OE_TIE_EN_CA) \
...
#if APPLY_NEW_IO_API
    vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ13), PB_Fld( 0 , SHU_B1_DQ13_RG_TX_ARDQS_OE_TIE_SEL_B1 ),
                                                               PB_Fld( 1       , SHU_B1_DQ13_RG_TX_ARDQS_OE_TIE_EN_B1  ),
                                                               PB_Fld( 0 , SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_SEL_B1 ),
                                                               PB_Fld( 1       , SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_EN_B1  ),
...
#ifdef FOR_HQA_TEST_USED
#if (fcFOR_CHIP_ID == fcLepin)
VCORE_DELAYCELL_T gVcoreDelayCellTable[49]={    {500000, 487},
                                                {506250, 471},
                                                {512500, 456},
                                                {518750, 442},
                                                {525000, 428},
 ...
 */
//-------------------------------------------------------------------------
#if ENABLE_8PHASE_CAL || ENABLE_MIOCK_JMETER_CAL || ENABLE_READ_DCA|| ENABLE_RDQS_DCC
static DRAM_STATUS_T DramcJmeterAutoK(DRAMC_CTX_T *p, JMETER_T *pJmtrInfo, U8 u1ByteIdx)
{
    ...
    U8 u1AutoKSel = u1ByteIdx; //0x0: B0, 0x1: B1 and 0x2: CA
    U8 u1IntVal = 1 /*Set 1 equals to 16 JM delay*/, u1DbgEn = DISABLE;
    ...
    */
    ...
    pJmtrInfo->u1TransCnt[u1AutoKSel] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_JM_8PH_AUTOK_STATUS0), JM_8PH_AUTOK_STATUS0_JM_8PH_AUTOK_TRANS_CNT);
    u1Eb_Fail = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_JM_8PH_AUTOK_STATUS1), JM_8PH_AUTOK_STATUS1_JM_8PH_AUTOK_EB_FAIL);

    mcSHOW_DBG_MSG3("[DramcJmeterAutoK] TransCnt = %d, Early Break Flag = %s\n", pJmtrInfo->u1TransCnt[u1AutoKSel], u1Eb_Fail?"FAIL":"PASS");

    for (u1AutoKTransCnt = 0; u1AutoKTransCnt < pJmtrInfo->u1TransCnt[u1AutoKSel]; u1AutoKTransCnt++)
    {
        ...
        pJmtrInfo->JmtrInfo[u1AutoKSel][u1AutoKTransCnt].u1JmDelay = u1TransPos;
        pJmtrInfo->JmtrInfo[u1AutoKSel][u1AutoKTransCnt].u1TransLevel = u1TransDir;
...
static void SetJmSel(DRAMC_CTX_T *p, JMETER_MODE_T JmeterMode, U8 u1Swap)
{
    U8 u1JmSel;

    //JM_SEL
    if ((JmeterMode == JM_RDCA_MODE) && (u1Swap == 0))
    {
        u1JmSel = 0x2;
    }
    else if ((JmeterMode == JM_RDCA_MODE) && (u1Swap == 1))
    {
        u1JmSel = 0xa;
    }
    else if ((JmeterMode == JM_RDCC_MODE) && (u1Swap == 0))
    {
        u1JmSel = 0x6;
    }
    else if ((JmeterMode == JM_RDCC_MODE) && (u1Swap == 1))
    {
        u1JmSel = 0xe;
    }
    else
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY2), u1JmSel, B0_PHY2_RG_RX_ARDQS_JM_SEL_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY2), u1JmSel, B1_PHY2_RG_RX_ARDQS_JM_SEL_B1);
}

static void DramcJmeterInit(DRAMC_CTX_T *p, JMETER_MODE_T JmeterMode, U8 u1Swap)
{
    if ((JmeterMode == JM_JMETER_MODE)||(JmeterMode == JM_8PHASE_MODE))
    {
        OEDisable(p);
    ...
    //IPMv24, JM_SEL = 1, JM_SEL = 0 for LPBK, JM_SEL = 2(READ_DCA), a(READ_DCA_SWAP), JM_SEL = 6(RDQS_DCC), e(RDQS_DCC_SWAP)
    SetJmSel(p, JmeterMode, u1Swap);
...
static void DramcJmeterCalib(DRAMC_CTX_T *p, JMETER_T *pJmtrInfo, U8 u1IsJmtrK, BYTES_T u1ByteSel)
{
    U16 u2Jm_dly_start = pJmtrInfo->u2JmDlyStart;
    U16 u2Jm_dly_end = pJmtrInfo->u2JmDlyEnd;
    U16 u2Jm_dly_step = pJmtrInfo->u1JmDlyStep;
    U16 ucdqs_dly;
    U32 u4sample_cnt, u4ones_cnt[DQS_NUMBER] = {0};
    U16 fgcurrent_value[DQS_NUMBER] = {0xffff, 0xffff}, fginitial_value[DQS_NUMBER] = {0xffff, 0xffff}, ucsearch_state[DQS_NUMBER]  = {0xffff, 0xffff};
    U8 u1ByteStart = BYTE_0, u1ByteEnd = DQS_NUMBER, u1ByteIdx;
    U8 u1EarlyBreakFlag = 0xFC;
    U8 u1FilterIdx[DQS_NUMBER] = {0}, u1FindTran[DQS_NUMBER] = {0};
    U16 u2Jm_dly_temp[DQS_NUMBER] = {0};

    if (u1ByteSel == BYTE_0)
    {
        u1ByteStart = 0;
        u1ByteEnd = 1;
        u1EarlyBreakFlag = 0xFE;
    }
    else if (u1ByteSel == BYTE_1)
    {
        u1ByteStart = 1;
        u1ByteEnd = 2;
        u1EarlyBreakFlag = 0xFD;
    }
    ...
    for (ucdqs_dly = u2Jm_dly_start; ucdqs_dly < u2Jm_dly_end; ucdqs_dly += u2Jm_dly_step)
    {
        ...
        if ((u1ByteSel == BYTE_0) || (u1ByteSel == ALL_BYTES))
        {
            if(NeedPhyNaoByteSwap(p))
            {
                u4ones_cnt[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_DQS1_ERR_CNT), MISC_DUTY_DQS1_ERR_CNT_DQS1_ERR_CNT);
            }
            else
            {
                u4ones_cnt[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_DQS0_ERR_CNT), MISC_DUTY_DQS0_ERR_CNT_DQS0_ERR_CNT);
            }
            #ifndef DDR_INIT_TIME_PROFILING
            if (u1IsJmtrK == TRUE)
            {
                mcSHOW_DBG_MSG2("%d : %d, %d\n", ucdqs_dly, u4sample_cnt, u4ones_cnt[BYTE_0]);
            }
            #endif
        }
        if ((u1ByteSel == BYTE_1) || (u1ByteSel == ALL_BYTES))
        {
            if(NeedPhyNaoByteSwap(p))
            {
                u4ones_cnt[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_DQS0_ERR_CNT), MISC_DUTY_DQS1_ERR_CNT_DQS1_ERR_CNT);
            }
            else
            {
                u4ones_cnt[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_DQS1_ERR_CNT), MISC_DUTY_DQS0_ERR_CNT_DQS0_ERR_CNT);
            }
            #ifndef DDR_INIT_TIME_PROFILING
            if (u1IsJmtrK == TRUE)
            {
                mcSHOW_DBG_MSG2("%d : %d, %d\n", ucdqs_dly, u4sample_cnt, u4ones_cnt[BYTE_1]);
            }
            #endif
        }

        //mcSHOW_DBG_MSG2("%d : %d, %d %d\n", ucdqs_dly, u4sample_cnt, u4ones_cnt[BYTE_0], u4ones_cnt[BYTE_1]);

        for (u1ByteIdx = u1ByteStart; u1ByteIdx<u1ByteEnd; u1ByteIdx+=1)
        {
            //change to boolean value
            if (u4ones_cnt[u1ByteIdx] < (u4sample_cnt / 2))
            {
                fgcurrent_value[u1ByteIdx] = 0;
            }
            else
            {
                fgcurrent_value[u1ByteIdx] = 1;
            }

            if (ucsearch_state[u1ByteIdx] == 0xffff)
            {
                //record initial value at the beginning
                fginitial_value[u1ByteIdx] = fgcurrent_value[u1ByteIdx];
                ucsearch_state[u1ByteIdx] = 0;
            }
            else
            {
                //Filter glitch
                if (fgcurrent_value[u1ByteIdx] != fginitial_value[u1ByteIdx])
                {
                    u1FindTran[u1ByteIdx] = 1;
                    u2Jm_dly_temp[u1ByteIdx] = ucdqs_dly;
                    fginitial_value[u1ByteIdx] = fgcurrent_value[u1ByteIdx];
                }
                else if ((fgcurrent_value[u1ByteIdx] == fginitial_value[u1ByteIdx]) && (u1FindTran[u1ByteIdx] == 1))
                {
                    u1FilterIdx[u1ByteIdx]++;
                }

                // check if change value
                if (u1FilterIdx[u1ByteIdx]*u2Jm_dly_step > 3)
                {
                    // start of the period
                    u1FindTran[u1ByteIdx] = 0;
                    u1FilterIdx[u1ByteIdx] = 0;
                    pJmtrInfo->JmtrInfo[u1ByteIdx][ucsearch_state[u1ByteIdx]].u1JmDelay = u2Jm_dly_temp[u1ByteIdx];
                    pJmtrInfo->JmtrInfo[u1ByteIdx][ucsearch_state[u1ByteIdx]].u1TransLevel = fgcurrent_value[u1ByteIdx];
                    //mcSHOW_DBG_MSG("B%d: pJmtrInfo->JmtrInfo[u1ByteIdx][ucsearch_state[u1ByteIdx]].u1JmDelay = %d, u2MiddlePeriod = %d, u2EndPeriod = %d\n", u1ByteIdx, pJmtrInfo->JmtrInfo[u1ByteIdx][ucsearch_state[u1ByteIdx]].u1JmDelay, pJmtrInfo->JmtrInfo[u1ByteIdx][ucsearch_state[u1ByteIdx]].u1TransLevel);
                    //mcSHOW_DBG_MSG("B%d: ucsearch_state[u1ByteIdx] = %d\n", u1ByteIdx, ucsearch_state[u1ByteIdx]);

                    ucsearch_state[u1ByteIdx]++;
                    pJmtrInfo->u1TransCnt[u1ByteIdx] = ucsearch_state[u1ByteIdx];
                    if (ucsearch_state[u1ByteIdx] == CYCLE_1T)
                    {
                        u1EarlyBreakFlag |= 0x1<<u1ByteIdx;
                    }
                }
            }
        }

        if (u1EarlyBreakFlag == 0xFF)
...
U16 DramcMiockJmeter(DRAMC_CTX_T *p, U8 isAutoK)
{
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL)),
        #if (fcFOR_CHIP_ID == fcMT8139)
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY)), // need porting to Jmeter
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY + DDRPHY_AO_RANK_OFFSET)), // need porting to Jmeter
        #elif (fcFOR_CHIP_ID == fcLepin || fcFOR_CHIP_ID == fcFigeac)
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_DQSIEN_DLY)), // need porting to Jmeter
        (MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_DQSIEN_DLY + DDRPHY_AO_RANK_OFFSET)), // need porting to Jmeter
        #endif
        ...
        #endif
        //(MIX_RG_CHECK(DDRPHY_REG_MISC_STBCAL2)), // for gating on/off
        //(MIX_RG_CHECK(DDRPHY_REG_MISC_DVFSCTL2)), // for gating on/off
        //(MIX_RG_CHECK(DDRPHY_REG_MISC_SHU_STBCAL)), // for gating on/off
...
#if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_EYE_DLY_DQS_BYPASS_B0},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_EYE_DLY_DQS_BYPASS_B1},
        {DDRPHY_REG_B0_DQ3, B0_DQ3_RG_RX_ARDQ_SMT_EN_B0},
        {DDRPHY_REG_B1_DQ3, B1_DQ3_RG_RX_ARDQ_SMT_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQS_OE_TIE_SEL_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQS_OE_TIE_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_SEL_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCK_OE_TIE_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQSB_OE_TIE_SEL_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQSB_OE_TIE_EN_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_SEL_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_EN_B1},
    };
#endif
    ...
    DramcJmeterInit(p, JM_JMETER_MODE, FALSE);
    ...
    for (u1RxGatingPI = u1RxGatingPI_start; u1RxGatingPI < u1RxGatingPI_end; u1RxGatingPI++)
    {
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            #if (fcFOR_CHIP_ID == fcMT8139)
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY), u1RxGatingPI, SHU_RK_B1_DQSIEN_DLY_DQSIEN_PI_B1); // for rank*_B0
        ...
        if (isAutoK)
        {
            ...
            eStatus |= DramcJmeterAutoK(p, &JmtrInfo, BYTE_0);
        ...
        else
        {
            ...
            JmtrInfo.u1JmDlyStep = 4;
            DramcJmeterCalib(p, &JmtrInfo, TRUE, BYTE_0);
        }

        if ((JmtrInfo.u1TransCnt[0] == CYCLE_1T) || (JmtrInfo.u1TransCnt[0] == CYCLE_05T)) // (1T or 0.5T)
            break;
    }
    ...
    if (JmtrInfo.u1TransCnt[0] == CYCLE_05T)
    {   // for 0.5T
        //Calculate 1 delay cell = ? ps
        // 1T = ? delay cell
        ucstart_period = JmtrInfo.JmtrInfo[0][0].u1JmDelay;
        ucmiddle_period = JmtrInfo.JmtrInfo[0][CYCLE_05T-1].u1JmDelay;
        num_dlycell_perT = (ucmiddle_period - ucstart_period) * 2;
        // 1T = ? ps
    }
    else if (JmtrInfo.u1TransCnt[0] == CYCLE_1T)
    {   // for 1T
        //Calculate 1 delay cell = ? ps
        // 1T = ? delay cell
        ucstart_period = JmtrInfo.JmtrInfo[0][0].u1JmDelay;
        ucend_period = JmtrInfo.JmtrInfo[0][CYCLE_1T-1].u1JmDelay;
    ...
    if (JmtrInfo.u1TransCnt[0] == CYCLE_1T)
    { // 1T
        mcSHOW_DBG_MSG("\n\tMIOCK jitter meter\tch=%d\n\n"
                        "1T = (%d-%d) = %d dly cells\n"
...
void Get_RX_DelayCell(DRAMC_CTX_T *p)
{
    ...
    #endif
            dramc_set_vcore_voltage(dramc_get_vmddr_voltage());  //set vmddr voltage to vcore to K RX delay cell
...
void Get_Vmddr_DelayCell(DRAMC_CTX_T *p)
{
#if defined(FOR_HQA_REPORT_USED) && (FOR_DV_SIMULATION_USED==0) && (SW_CHANGE_FOR_SIMULATION==0)
        ...
        if(p->femmc_Ready == 1)
        {
            ...
            gHQALOG_Vmddr_delay_cell_ps = GetVcoreDelayCellTime(p);
...
DRAM_STATUS_T Dramc8PhaseCal(DRAMC_CTX_T *p, u8 isAutoK)
{
    ...
    DQS_8_PHASE_T e8Phase_SM_nxt = DQS_8PH_DEGREE_0;
    U8 u18ph_dq_final_dly  = 0xff, u18ph_ca_final_dly = 0xff;
    ...
    U8 u1SramIdx = vGet_Current_SRAMIdx(p);
    
    // Jmeter Scan
    JMETER_T JmtrInfo;
    U8 u1JmtrPrintCnt = 0, EdgeLevel = 0;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        //(MIX_RG_CHECK(DDRPHY_REG_CA_CMD2)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ13)),
#if !APPLY_NEW_IO_API //@cc: same as JM, for OE tie and is mixed.
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ13)),
#endif
        //(MIX_RG_CHECK(DDRPHY_REG_SHU_CA_CMD13)),
    };
    ...
    if(p->femmc_Ready == 1)
    {
        u18ph_dq_final_dly = p->pSavetimeData->u18Ph_dq_dly;
        u18ph_ca_final_dly = p->pSavetimeData->u18Ph_ca_dly;
        goto set_result;
    }
#endif
    ...
    DramcJmeterInit(p, JM_8PHASE_MODE, FALSE);
    //u18PhDlyBackup = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ1), SHU_B0_DQ1_RG_ARPI_MIDPI_8PH_DLY_B0);

#if SUPPORT_TYPE_LPDDR5
    if (is_lp5_family(p) && (u1IsPhaseMode_DQ(p))) {
        U16 u1PICK_DQ = g8PhaseResult.u2PICK_DQ[u1SramIdx];
        U8 found = FALSE;
        S8 ii;

        /* Only search those with higher frequency (smaller SRAM idx) */
        for (ii = u1SramIdx - 1; ii >= 0; ii--) {
            if (u1PICK_DQ == g8PhaseResult.u2PICK_DQ[ii]) {

                u18ph_dq_final_dly = 0;
                u18ph_ca_final_dly = g8PhaseResult.u18PhaseDly_DQ[ii];

                mcSHOW_DBG_MSG("%s: SRAM_SHU%d reuse SRAM_SHU%d 8Phase cal result: DQ/CA = (%d/%d)\n",
                    __func__, u1SramIdx, ii, u18ph_dq_final_dly, u18ph_ca_final_dly);
        ...
        if (!found) {
            mcSHOW_ERR_MSG("PICK=%d, dlycell=%d\n", g8PhaseResult.u2PICK_DQ[u1SramIdx], gHQALOG_Vmddr_delay_cell_ps);
            mcSHOW_ERR_MSG("default 8PH DQ=%d, CA=%d\n", g8PhaseResult.u18PhaseDly_DQ[u1SramIdx], g8PhaseResult.u18PhaseDly_CA[u1SramIdx]);
            u18ph_dq_final_dly = g8PhaseResult.u18PhaseDly_DQ[u1SramIdx];
            u18ph_ca_final_dly = 12500000/(g8PhaseResult.u2PICK_DQ[u1SramIdx]*gHQALOG_Vmddr_delay_cell_ps)-6;
            mcSHOW_ERR_MSG("formula 8PH DQ=%d, CA=%d\n", u18ph_dq_final_dly, u18ph_ca_final_dly);
            mcSHOW_ERR_MSG("%s: WARNING: SRAM_SHU%d cannot reuse. Keep default setting\n", __func__, u1SramIdx);
        }
        goto set_result;
    }
#endif
    ...
    do {
        switch (e8Phase_SM)
        {
            case DQS_8PH_DEGREE_0:
                ...
                /* For low speed, may not able to find enough trasitions with Degree 180 target.
                 * Change to use Degree 90 instead
                 */
                if (p->freqGroup > 1066)
                    e8Phase_SM_nxt = DQS_8PH_DEGREE_180;
                else
                    e8Phase_SM_nxt = DQS_8PH_DEGREE_90;
                break;
            case DQS_8PH_DEGREE_180:
                u1DqsienPI = 48;
                u18Ph_start = 0;
                u18Ph_end = 1;
                e8Phase_SM_nxt = DQS_8PH_DEGREE_45;
                break;
            case DQS_8PH_DEGREE_90:
                u1DqsienPI = 32;
                u18Ph_start = 0;
                u18Ph_end = 1;
                e8Phase_SM_nxt = DQS_8PH_DEGREE_45;
            ...
            case DQS_8PH_DEGREE_45:
                ...
                u18Ph_start = 0;
                u18Ph_end = 128;
                e8Phase_SM_nxt = DQS_8PH_DEGREE_MAX;
                break;
            default:
        ...
        mcSHOW_DBG_MSG2("\n[%s] SM_%d, 8PH_DLY (%d~%d), DQSIEN PI = %d, DV_8PH_DLY (DQ/CA) = %d/%d\n",
			__func__, e8Phase_SM, u18Ph_start, u18Ph_end, u1DqsienPI,
			g8PhaseResult.u18PhaseDly_DQ[u1SramIdx], g8PhaseResult.u18PhaseDly_CA[u1SramIdx]);
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            #if (fcFOR_CHIP_ID == fcMT8139)
            // SHU_RK_B0_DQSIEN_PI_DLY_DQSIEN_PI_B0[6] no use (ignore)
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQSIEN_DLY), u1DqsienPI, SHU_RK_B1_DQSIEN_DLY_DQSIEN_PI_B1); // for rank*_B0
        ...
        for (u18Ph_dly = u18Ph_start; u18Ph_dly < u18Ph_end; u18Ph_dly++)
        {
            ...
            if (isAutoK)
            {
                ...
                eDRAMStatus |= DramcJmeterAutoK(p, &JmtrInfo, BYTE_0);
                if (eDRAMStatus == DRAM_FAIL)
                {
                    mcSHOW_ERR_MSG("8Phase Hw AutoK is FAIL (to Default) !!!\n");
                    u18ph_dq_final_dly = g8PhaseResult.u18PhaseDly_DQ[u1SramIdx]; //rollback to init settings
                    u18ph_ca_final_dly = g8PhaseResult.u18PhaseDly_CA[u1SramIdx]; //rollback to init settings
                    e8Phase_SM_nxt = DQS_8PH_DEGREE_MAX;
                    u18Ph_dly = u18Ph_end;
                }
            }
            else
            {
                JmtrInfo.u2JmDlyEnd = 512;
                JmtrInfo.u1JmDlyStep = 1;
                DramcJmeterCalib(p, &JmtrInfo, FALSE, BYTE_0);
            }

            // base on degree 0 to find the High or Low Level
            if ((e8Phase_SM == DQS_8PH_DEGREE_0) && (JmtrInfo.u1TransCnt[BYTE_0] != 0))
                EdgeLevel = JmtrInfo.JmtrInfo[BYTE_0][0].u1TransLevel;

            for (u1JmtrPrintCnt = 0; u1JmtrPrintCnt < JmtrInfo.u1TransCnt[BYTE_0]; u1JmtrPrintCnt++)
            {
                if (JmtrInfo.JmtrInfo[BYTE_0][u1JmtrPrintCnt].u1TransLevel == EdgeLevel)
                {
                    ucdqs_dly = JmtrInfo.JmtrInfo[BYTE_0][u1JmtrPrintCnt].u1JmDelay;
                    ...
                    else if (e8Phase_SM == DQS_8PH_DEGREE_90)
                    {
                        u2R180 = ucdqs_dly;
                        if (u2R180 > u2R0)
                        {
                            u2R = u2R0 + ((u2R180 - u2R0) >> 1); // u2R180 >= u2R0 for (u1R180 - u1R0)/4 for 180 degree. /2 for 90 degree
                            mcSHOW_DBG_MSG2("[8PH=%d] R = %d, R90 (H) = %d\n", u18Ph_dly, u2R, u2R180);
                            break; // break ucdqs_dly for loop
                        }
                    }
                    else if (e8Phase_SM == DQS_8PH_DEGREE_45)
                    {
                        ...
                        if (u2P > u2R0) // u2P ~= DQS_8PH_DEGREE_180
                        {
                            ...
                            mcSHOW_DBG_MSG2("[8PH=%d] ", u18Ph_dly);       

                            if (s2Err_code == 0)
                            {
                                s2Err_code_min = s2Err_code;
                                u18ph_dq_final_dly = u18Ph_dly;
                                if (is_lp5_family(p) && p->freqGroup > 1066) {
                                    u18ph_ca_final_dly = 0;
                                } else {
                                    u18ph_ca_final_dly = u18ph_dq_final_dly;
                                }
                                u18Ph_dly = u18Ph_end; // for loop skip
                            }
            ...
            if (JmtrInfo.u1TransCnt[BYTE_0] == u1JmtrPrintCnt) // Error handing when not find transaction
            {
                u18ph_dq_final_dly = g8PhaseResult.u18PhaseDly_DQ[u1SramIdx];
                u18ph_ca_final_dly = g8PhaseResult.u18PhaseDly_CA[u1SramIdx];
        ...
        e8Phase_SM = e8Phase_SM_nxt;
    }while (e8Phase_SM != DQS_8PH_DEGREE_MAX);
...
set_result:
    ...
    if(p->femmc_Ready == 0)
    {
            p->pSavetimeData->u18Ph_dq_dly = u18ph_dq_final_dly; // only dq 8ph available
            p->pSavetimeData->u18Ph_ca_dly = u18ph_ca_final_dly; // only dq 8ph available
    }
#endif
    g8PhaseResult.u18PhaseDly_DQ[u1SramIdx] = u18ph_dq_final_dly;
    g8PhaseResult.u18PhaseDly_CA[u1SramIdx] = u18ph_ca_final_dly;
    mcSHOW_DBG_MSG("\n[%s] Final 8PH_DLY (DQ/CA) = %d/%d\n\n", __func__, u18ph_dq_final_dly, u18ph_ca_final_dly);
    //mcDUMP_REG_MSG("\n[Dramc8PhaseCal] Final 8PH_DLY = %d\n\n", u18Ph_dly_final);

    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B0_DQ1, u18ph_dq_final_dly, SHU_B0_DQ1_RG_ARPI_MIDPI_8PH_DLY_B0);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B1_DQ1, u18ph_dq_final_dly, SHU_B1_DQ1_RG_ARPI_MIDPI_8PH_DLY_B1);
    vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD1, u18ph_ca_final_dly, SHU_CA_CMD1_RG_ARPI_MIDPI_8PH_DLY_CA);
...
#if SUPPORT_TYPE_LPDDR4
static const U8 ImpLP4VrefSel[IMP_DRV_MAX-1] = {0x1c, 0x13, 0x00, 0x1c}; /* DRVP  DRVN  ODTP  ODTN */
#if fcFOR_CHIP_ID == fcMT8139
static const U8 ImpLP4XVrefSel[IMP_DRV_MAX-1] = {0x4b, 0x3a, 0x00, 0x4b}; /* DRVP  DRVN  ODTP  ODTN */
#else /* To be confirmed */
static const U8 ImpLP4XVrefSel[IMP_DRV_MAX-1] = {0x37, 0x33, 0x00, 0x38}; /* DRVP  DRVN  ODTP  ODTN */
#endif
#endif
...
static const U8 ImpLP5VrefSel[IMP_MAX_OHM][IMP_DRV_MAX-1] = {{0x35, 0x33, 0x00, 0x33}, {0x35, 0x33, 0x00, 0x33}, {0x33, 0x33, 0x00, 0x33}}; /* DRVP  DRVN  ODTP  ODTN */
#endif
#endif
...
static void vImpCalVrefSel(DRAMC_CTX_T *p, IMP_VREF_OHM _imp_k_ohm, U8 u1ImpCalStage)
{
    U8 u1RegTmpValue = 0;
    U32 u4DrvFld = 0;
    const U8 *Imp_VrefSel; /* default: LP4X Vref Sel */

#if SUPPORT_TYPE_LPDDR4
    if (p->dram_type == TYPE_LPDDR4X)
    {
        Imp_VrefSel = ImpLP4XVrefSel;
    }
    ...
    if (is_lp5_family(p))
    {
        Imp_VrefSel = ImpLP5VrefSel[_imp_k_ohm];
    }
    else
#endif
    {
...
void DramcSwImpedanceSaveRegister(DRAMC_CTX_T *p, IMP_VREF_OHM imp_k_ohm)
{
    U32 backup_broadcast;
    U8 u1Hysteresis, u1CKE_CS_Drv, u1CS_DRVP, u1CS_DRVN;
    U8 DRVP_value;
    U8 DRVN_value;
    U8 ODTP_value;
    U8 ODTN_value;
    ...
    DRVP_value = gDramcSwImpedanceResult[imp_k_ohm][DRVP];
    DRVN_value = gDramcSwImpedanceResult[imp_k_ohm][DRVN];
    ODTP_value = gDramcSwImpedanceResult[imp_k_ohm][ODTP];
    ODTN_value = gDramcSwImpedanceResult[imp_k_ohm][ODTN];
    ...
    vImpCalVrefSel(p, imp_k_ohm, IMPCAL_STAGE_TRACKING);

    #if ENABLE_NT_ODT
    if(p->pDramInfo_SA->MR0_Unified_NT_ODT_Behavior == 1)
    {
        ODTN_value = ((p->frequency >= 2133) && (p->pDramInfo_SA->MR0_Unified_NT_ODT_Behavior))? gDramcSwImpedanceResult[imp_k_ohm][NTODT]: gDramcSwImpedanceResult[imp_k_ohm][ODTN];
    }else
    #endif
    {
        ODTN_value = gDramcSwImpedanceResult[imp_k_ohm][ODTN];
    }
    mcSHOW_DBG_MSG("[%s] DRVP_value = %d\n", __func__, DRVP_value);
    mcSHOW_DBG_MSG("[%s] DRVN_value = %d\n", __func__, DRVN_value);
    mcSHOW_DBG_MSG("[%s] ODTN_value = %d\n", __func__, ODTN_value);

    //DQ
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING1), P_Fld(DRVP_value, SHU_MISC_DRVING1_DQDRVP2) | P_Fld(DRVN_value, SHU_MISC_DRVING1_DQDRVN2));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING2), P_Fld(DRVP_value, SHU_MISC_DRVING2_DQDRVP1) | P_Fld(DRVN_value, SHU_MISC_DRVING2_DQDRVN1));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING3), P_Fld(ODTP_value, SHU_MISC_DRVING3_DQODTP2) | P_Fld(ODTN_value, SHU_MISC_DRVING3_DQODTN2));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING4), P_Fld(ODTP_value, SHU_MISC_DRVING4_DQODTP1) | P_Fld(ODTN_value, SHU_MISC_DRVING4_DQODTN1));
    ...
    {  U32 temp_value[IMP_DRV_MAX];
        ...
        {
            temp_value[i] = SwImpedanceAdjust(gDramcSwImpedanceResult[imp_k_ohm][i], 2);
        }
        #if ENABLE_NT_ODT
        ...
        #endif
         vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING1), P_Fld(temp_value[DRVP], SHU_MISC_DRVING1_DQSDRVP2) | P_Fld(temp_value[DRVN], SHU_MISC_DRVING1_DQSDRVN2));
         vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING1), P_Fld(temp_value[DRVP], SHU_MISC_DRVING1_DQSDRVP1) | P_Fld(temp_value[DRVN], SHU_MISC_DRVING1_DQSDRVN1));
         vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING3), P_Fld(temp_value[ODTP], SHU_MISC_DRVING3_DQSODTP2) | P_Fld(ODTN_value, SHU_MISC_DRVING3_DQSODTN2));
         vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING3), P_Fld(temp_value[ODTP], SHU_MISC_DRVING3_DQSODTP) | P_Fld(ODTN_value, SHU_MISC_DRVING3_DQSODTN));
    }
    else
    #endif
    {
        ...
        if (is_lp5_family(p))
        {
            vIO32WriteFldMulti((DDRPHY_REG_MISC_SHU_DRVING7), P_Fld(DRVN_value, MISC_SHU_DRVING7_WCK0_DRVN)
                                                    | P_Fld(DRVP_value, MISC_SHU_DRVING7_WCK0_DRVP)
                                                    | P_Fld(DRVN_value, MISC_SHU_DRVING7_WCK1_DRVN)
                                                    | P_Fld(DRVP_value, MISC_SHU_DRVING7_WCK1_DRVP));
        }
#endif

        vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING1), P_Fld(DRVP_value, SHU_MISC_DRVING1_DQSDRVP2) | P_Fld(DRVN_value, SHU_MISC_DRVING1_DQSDRVN2));
        vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING1), P_Fld(DRVP_value, SHU_MISC_DRVING1_DQSDRVP1) | P_Fld(DRVN_value, SHU_MISC_DRVING1_DQSDRVN1));

        vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING3), P_Fld(ODTP_value, SHU_MISC_DRVING3_DQSODTP2) | P_Fld(ODTN_value, SHU_MISC_DRVING3_DQSODTN2));
        vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING3), P_Fld(ODTP_value, SHU_MISC_DRVING3_DQSODTP) | P_Fld(ODTN_value, SHU_MISC_DRVING3_DQSODTN));
    }

    //CMD & CLK
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING2), P_Fld(DRVP_value, SHU_MISC_DRVING2_CMDDRVP2) | P_Fld(DRVN_value, SHU_MISC_DRVING2_CMDDRVN2));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING2), P_Fld(DRVP_value, SHU_MISC_DRVING2_CMDDRVP1) | P_Fld(DRVN_value, SHU_MISC_DRVING2_CMDDRVN1));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING4), P_Fld(ODTP_value, SHU_MISC_DRVING4_CMDODTP2) | P_Fld(ODTN_value, SHU_MISC_DRVING4_CMDODTN2));
    vIO32WriteFldMulti((DDRPHY_REG_SHU_MISC_DRVING4), P_Fld(ODTP_value, SHU_MISC_DRVING4_CMDODTP1) | P_Fld(ODTN_value, SHU_MISC_DRVING4_CMDODTN1));
    ...
    #if (fcFOR_CHIP_ID == fcMT8139) // for CS and CKE control together
    // @Darren, The CKE/CS Hw Arch IO device Tx drive by 1.1V. Impcal core device Tx drive by VDDQ.
    vIO32WriteFldAlign((DDRPHY_REG_MISC_SHU_DRVING8), 0xD, MISC_SHU_DRVING8_CS_DRVP);
    vIO32WriteFldAlign((DDRPHY_REG_MISC_SHU_DRVING8), 0xF, MISC_SHU_DRVING8_CS_DRVN);
    
    #elif (fcFOR_CHIP_ID == fcLepin)
    // CS drv : 40 ohm CS_DRVP = 0x0A, CS_DRVN = 0x10
    // CS drv : 48 ohm CS_DRVP = 0x07, CS_DRVN = 0x0C
    if((imp_k_ohm == IMP_40_OHM) || (imp_k_ohm == IMP_40_OHM_VQ0dot3V))
    {
        u1CS_DRVP = 0x0A;	u1CS_DRVN = 0x10;
    }
    else if (imp_k_ohm == IMP_48_OHM)
    {
        u1CS_DRVP = 0x07;	u1CS_DRVN = 0x0C;
    }
    else
    {
        mcSHOW_ERR_MSG("[%s] imp cs drv ohm not defined !!!(use 48 ohm) \n", __func__);
        u1CS_DRVP = 0x07;	u1CS_DRVN = 0x0C;
        #if (__ETT__)
        ASSERT(0);
        #endif    
    }
    vIO32WriteFldAlign((DDRPHY_REG_MISC_SHU_DRVING8), u1CS_DRVP, MISC_SHU_DRVING8_CS_DRVP);
    vIO32WriteFldAlign((DDRPHY_REG_MISC_SHU_DRVING8), u1CS_DRVN, MISC_SHU_DRVING8_CS_DRVN);
    #elif (fcFOR_CHIP_ID == fcFigeac)
...
DRAM_STATUS_T DramcSwImpedanceCal(DRAMC_CTX_T *p, IMP_VREF_OHM _imp_kohm)
{
    ...
    U32 u4RegBackupAddress[] =
    {
        (MIX_RG_CHECK(DDRPHY_REG_MISC_LP_CTRL)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_IMPCAL)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_IMP_CTRL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_MISC_IMPCAL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_CA_CMD12)),
    };
    backup_broadcast = GetDramcBroadcast();
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    ...
    vIO32WriteFldMulti(DDRPHY_REG_MISC_LP_CTRL, P_Fld(0x0, MISC_LP_CTRL_RG_ARDMSUS_10) | \
    ...
    vIO32WriteFldAlign(DDRPHY_REG_MISC_IMPCAL, 0, MISC_IMPCAL_IMPCAL_HW);
...
#endif
    {
        //LP4/DDR4 combo:LP4=1, LP4X & DDR4=0
        //LP5/LP4 combo:LP4(X)=1, LP5=0
#if (fcFOR_CHIP_ID == fcMT8139)
        #if FOR_DV_SIMULATION_USED
        u1DDR4 = 0; /* Behave model only support LP4X.*/
        #else
        if (p->dram_type == TYPE_LPDDR4) /* lp4 */
            u1DDR4 = 1;
        else /* LP4X & DDR4 */
            u1DDR4 = 0;
        #endif
    ...
    for (u1DrvType = DRVP; u1DrvType < IMP_DRV_MAX; u1DrvType++) // Calibration sequence for DRVP, DRVN and ODTN
    {
        ...
        vImpCalVrefSel(p, _imp_kohm, u1DrvType);
...
***/
    ...
    gDramcSwImpedanceResult[_imp_kohm][DRVP] = u4DRVP_Result;
    gDramcSwImpedanceResult[_imp_kohm][DRVN] = u4DRVN_Result;
    gDramcSwImpedanceResult[_imp_kohm][ODTP] = 0;
    gDramcSwImpedanceResult[_imp_kohm][ODTN] = u4ODTN_Result;

#if 0//@Darren-SUPPORT_SAVE_TIME_FOR_CALIBRATION
    U8 u1drv;
    for (u1drv = 0; u1drv < 4; u1drv++)
    {
        if (p->femmc_Ready == 0)
            p->pSavetimeData->u1SwImpedanceResult[imp_ktype][u1drv] = gDramcSwImpedanceResult[imp_ktype][u1drv];
        else
        {
            gDramcSwImpedanceResult[imp_ktype][u1drv] = p->pSavetimeData->u1SwImpedanceResult[imp_ktype][u1drv];
            vSetCalibrationResult(p, DRAM_CALIBRATION_SW_IMPEDANCE, DRAM_FAST_K);
        }
    }
#endif

    mcSHOW_DBG_MSG("Reg: DRVP=%d, DRVN=%d, ODTN=%d\n", gDramcSwImpedanceResult[_imp_kohm][DRVP], gDramcSwImpedanceResult[_imp_kohm][DRVN], gDramcSwImpedanceResult[_imp_kohm][ODTN]);
...
#if ENABLE_NT_ODT
    gDramcSwImpedanceResult[_imp_kohm][NTODT] = ((gDramcSwImpedanceResult[_imp_kohm][ODTN]+10)*48/80) -10;
    mcSHOW_DBG_MSG("[NTODT] ODTN = %d\n", gDramcSwImpedanceResult[_imp_kohm][NTODT]);
#endif

#if APPLY_SIGNAL_WAVEFORM_SETTINGS_ADJUST
    gDramcSwImpedanceResult[_imp_kohm][DRVP] = SwImpedanceAdjust(gDramcSwImpedanceResult[_imp_kohm][DRVP], gDramcSwImpedanceAdjust[_imp_kohm][DRVP]);
    gDramcSwImpedanceResult[_imp_kohm][ODTN] = SwImpedanceAdjust(gDramcSwImpedanceResult[_imp_kohm][ODTN], gDramcSwImpedanceAdjust[_imp_kohm][ODTN]);

    mcSHOW_DBG_MSG("freq_region=%d, Reg: DRVP=%d, DRVN=%d, ODTN=%d (After Adjust)\n", gDramcSwImpedanceResult[_imp_kohm][DRVP],
                                        gDramcSwImpedanceResult[_imp_kohm][DRVN], gDramcSwImpedanceResult[_imp_kohm][ODTN]);
#endif
	...
	*/
    ...
    if (!(gDramcSwImpedanceResult[_imp_kohm][DRVP] >= drvp_min && gDramcSwImpedanceResult[_imp_kohm][DRVP] <= drvp_max) ||
        !(gDramcSwImpedanceResult[_imp_kohm][DRVN] >= drvn_min && gDramcSwImpedanceResult[_imp_kohm][DRVN] <= drvn_max) ||
        !(gDramcSwImpedanceResult[_imp_kohm][ODTN] >= odtn_min && gDramcSwImpedanceResult[_imp_kohm][ODTN] <= odtn_max)) 
    {
        #if defined(DRAM_SLT)
            ...
            if(!(gDramcSwImpedanceResult[_imp_kohm][DRVP] >= drvp_min && gDramcSwImpedanceResult[_imp_kohm][DRVP] <= drvp_max))
            {
                mcSHOW_DBG_MSG("[WARNING] DRVN = %d ==> unexpect value\n", gDramcSwImpedanceResult[_imp_kohm][DRVN]);
                PINInfo_flashtool.IMP_ERR_FLAG |= (0x1<<DRVN);
            }
            else if(!(gDramcSwImpedanceResult[_imp_kohm][DRVN] >= drvn_min && gDramcSwImpedanceResult[_imp_kohm][DRVN] <= drvn_max))
            {
                mcSHOW_DBG_MSG("[WARNING] DRVP = %d ==> unexpect value\n", gDramcSwImpedanceResult[_imp_kohm][DRVP]);
                PINInfo_flashtool.IMP_ERR_FLAG |= (0x1<<DRVP);
            }
            else if(!(gDramcSwImpedanceResult[_imp_kohm][ODTN] >= odtn_min && gDramcSwImpedanceResult[_imp_kohm][ODTN] <= odtn_max))
            {
                mcSHOW_DBG_MSG("[WARNING] ODTN = %d ==> unexpect value\n", gDramcSwImpedanceResult[_imp_kohm][ODTN]);
...
#define DUTY_MCK16X_OFFSET_START 2
#define DUTY_MCK16X_OFFSET_END 30
#define DUTY_MCK16X_OFFSET_STEP 10
...
#else
#if defined(DRAM_SLT) || defined(FPC_TEST)
#define DUTY_OFFSET_START -16
#define DUTY_OFFSET_END 16
#define DUTY_OFFSET_STEP 1
#else
#define DUTY_OFFSET_START -DutyPassCriteria
#define DUTY_OFFSET_END DutyPassCriteria
#define DUTY_OFFSET_STEP 1
#endif

#define DUTY_MCK16X_OFFSET_START 4	// can't start from 0 because 0=0x10(16)
#define DUTY_MCK16X_OFFSET_END 26   // code0-5, code26-31 is same, so K 4~26 is ok and save time
#define DUTY_MCK16X_OFFSET_STEP 2

#define CLOCK_PI_START 0
#define CLOCK_PI_END 31 // Aphy will change path after PI > 32, and more worse, so only K 0~31
...
static void DramcClockDutySetClkDelayCell(DRAMC_CTX_T *p, S8 *scDutyDelay)
{
	...
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_TXDUTY),
		tDelay, SHU_CA_TXDUTY_DA_TX_ARCLK_DUTY_DLY);
}
...
//    mcSHOW_DBG_MSG("CH%d, Final DQS1 duty delay cell = %d\n", p->channel, scDutyDelay[1]);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay[0], SHU_B0_TXDUTY_DA_TX_ARDQS_DUTY_DLY_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay[1], SHU_B1_TXDUTY_DA_TX_ARDQS_DUTY_DLY_B1);	
}
...
static void WCKDutyScan_SetWCKDelayCell(DRAMC_CTX_T *p, S8 *scDutyDelay)
{
	...
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay[0], SHU_B0_TXDUTY_DA_TX_ARWCK_DUTY_DLY_B0);
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay[1], SHU_B1_TXDUTY_DA_TX_ARWCK_DUTY_DLY_B1);
}

static void MCKDutyScan_SetMCK16XDelayCell(DRAMC_CTX_T *p, S8 *scDutyDelay)
{
	U8 u1DQSIdx;
	U32 save_offset;

	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD1), scDutyDelay[0], SHU_CA_CMD1_RG_ARPI_MCK8X_DCC_SEL_C0);
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ1), scDutyDelay[1], SHU_B0_DQ1_RG_ARPI_MCK8X_DCC_SEL_B0);
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ1), scDutyDelay[2], SHU_B1_DQ1_RG_ARPI_MCK8X_DCC_SEL_B1);	
}
#endif
...
static void DQDQMDutyScan_SetDQDQMDelayCell(DRAMC_CTX_T *p, U8 u1ChannelIdx, S8 *scDutyDelay, U8 k_type)
{
    ...
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay[0], SHU_B0_TXDUTY_DA_TX_ARDQ_DUTY_DLY_B0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay[1], SHU_B1_TXDUTY_DA_TX_ARDQ_DUTY_DLY_B1);		
    }

    if (k_type == DutyScan_Calibration_K_DQM)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay[0], SHU_B0_TXDUTY_DA_TX_ARDQM_DUTY_DLY_B0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay[1], SHU_B1_TXDUTY_DA_TX_ARDQM_DUTY_DLY_B1);
    }
}
#endif
...
static void DutyScan_Offset_Calibration(DRAMC_CTX_T *p, U8 *u1OfB0, U8 *u1OfB1, U8 *u1OfCA)
{
...
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6),
		PB_Fld(1, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B0),
		PB_Fld(2, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_B0));
#else
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6),
		P_Fld(1, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B0) |
		P_Fld(2, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_B0));
#endif
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DLL_ARPI2),
		P_Fld(0x0, SHU_B0_DLL_ARPI2_RG_ARPI_MPDIV_CG_B0) |
		P_Fld(0x0, SHU_B0_DLL_ARPI2_RG_ARPI_CG_DQS_B0));
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ5),
        0, SHU_B0_DQ5_RG_RX_ARDQ_VREF_BYPASS_B0);
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3),
		PB_Fld(0x38, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B0),
		PB_Fld(0x0, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_OFFSETC_B0),
		PB_Fld(0x1, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_EN_B0));
#else
...
#endif
    mcDELAY_US(1);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ11),
        0x1, SHU_B0_DQ11_RG_RX_ARDQ_OFFSETC_EN_B0);

    //B1
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6),
		PB_Fld(1, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B1),
		PB_Fld(2, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_B1));
#else
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6),
		P_Fld(1, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B1) |
		P_Fld(2, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_B1));
#endif
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DLL_ARPI2),
		P_Fld(0x0, SHU_B1_DLL_ARPI2_RG_ARPI_MPDIV_CG_B1) |
		P_Fld(0x0, SHU_B1_DLL_ARPI2_RG_ARPI_CG_DQS_B1));
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ5),
        0, SHU_B1_DQ5_RG_RX_ARDQ_VREF_BYPASS_B1);
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
		PB_Fld(0x38, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1),
		PB_Fld(0x0, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_OFFSETC_B1),
		PB_Fld(0x1, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_EN_B1));
#else
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
        P_Fld(0x38, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1) |
        P_Fld(0x0, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_OFFSETC_B1) |
        P_Fld(0x1, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_EN_B1));
#endif
    mcDELAY_US(1);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ11),
        0x1, SHU_B1_DQ11_RG_RX_ARDQ_OFFSETC_EN_B1);

	//CA
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD6),
		PB_Fld(1, CA_CMD6_RG_TX_ARCMD_DATA_SWAP_EN),
		PB_Fld(2, CA_CMD6_RG_TX_ARCMD_DATA_SWAP));
#else
...
#endif
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_DLL_ARPI2),
		0x0, SHU_CA_DLL_ARPI2_RG_ARPI_MPDIV_CG_CA);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD5),
        0, SHU_CA_CMD5_RG_RX_ARCMD_VREF_BYPASS);
#if APPLY_NEW_IO_API
	vPhyByteWriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
		PB_Fld(0x38, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_VREF_SEL_CA),
		PB_Fld(0x0, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_OFFSETC_CA),
		PB_Fld(0x1, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_EN_CA));
#else
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
        P_Fld(0x38, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_VREF_SEL_CA) |
        P_Fld(0x0, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_OFFSETC_CA) |
        P_Fld(0x1, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_EN_CA));
#endif
    mcDELAY_US(1);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD11),
        0x1, SHU_CA_CMD11_RG_RX_ARCA_OFFSETC_EN_CA);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ11),
                 0x0, SHU_B0_DQ11_RG_RX_ARDQ_OFFSETC_EN_B0);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ11),
                 0x0, SHU_B1_DQ11_RG_RX_ARDQ_OFFSETC_EN_B1);
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD11),
                 0x0, SHU_CA_CMD11_RG_RX_ARCA_OFFSETC_EN_CA);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6),
                 1, B0_DQ6_RG_RX_ARDQ_LPBK_EN_B0);    
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6),
                 1, B1_DQ6_RG_RX_ARDQ_LPBK_EN_B1);    
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_CMD6),
                 1, CA_CMD6_RG_RX_ARCMD_LPBK_EN);
...
S8 gcFinal_K_MCK16X_delay_cell[CHANNEL_NUM][DQS_NUMBER+CA_NUMBER];
#if APPLY_DQDQM_DUTY_CALIBRATION
...
static DRAM_STATUS_T DutyScan_Calibration_Flow(DRAMC_CTX_T *p, U8 k_type, U8 ft_test_type, DUTYSCAN_SCAN_PI_T scan_type, U8 isAutoK)
{
    DRAM_STATUS_T KResult = DRAM_FAIL;

    S8 scinner_duty_ofst, scFinal_clk_delay_cell[DQS_NUMBER+CA_NUMBER]={0}, scinner_duty_ofst_step;
    ...
    U32 u4DutyDiff, u4DutyDiff_Limit=800;

    U8 vref_sel_value[2], cal_out_value;
    S32 duty_value[2];
    S32 final_duty;
    U8 wck_dly[ALL_BYTES]={0};

    U32 ucperiod_duty_max=0, ucperiod_duty_min=0xffffffff, ucperiod_duty_max_clk_dly=0, ucperiod_duty_min_clk_dly=0;
    U32 ucperiod_duty_averige=0, ucperiod_duty_averige_past=0, ucStart_period_duty_averige[DQS_NUMBER+CA_NUMBER]={0}, ucFinal_period_duty_averige[DQS_NUMBER+CA_NUMBER]={0}, ucmost_approach_50_percent=0xffffffff;

    /* temp local record duty calibration result of every step to find first pass and last pass */
    TEST_DUTY_AVG_LOG_PARAMETER_T record_duty_log[2]={{0},{0}};
    S8 temp_first_pass[2]={0xff,0xff}, temp_last_pass[2]={0xff,0xff};
    U8 min_closetozero=0xff, record_closetozero=0xff, found_duty_pass_criteria=0xff, final_duty_select=0xff;
    U32 record_ucmost_approach_50_percent, temp_value;

    U32 ucFinal_period_duty_max[DQS_NUMBER+CA_NUMBER] = {0}, ucFinal_period_duty_min[DQS_NUMBER+CA_NUMBER] = {0};
    U32 ucFinal_duty_max_clk_dly[DQS_NUMBER+CA_NUMBER]={0},ucFinal_duty_min_clk_dly[DQS_NUMBER+CA_NUMBER]={0};
...
//fra    U8 *str_who_am_I=str_clk_duty;
    ...
    U8 tDelay;
    U32 tmp;
    U8 count_maxmin_duty_5000_percent;
    ...
    if (k_type == DutyScan_Calibration_K_CLK) // K CLK
    {
        ...
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CG_CTRL0), 1, MISC_CG_CTRL0_RG_CG_CMD_OFF_DISABLE);
        ucdqs_i_count = 1;
    }
    else
    {
        ...
        else if (k_type == DutyScan_Calibration_K_DQM) // K DQM
        {
            ...
            // use DQ's clock
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ15), 1, SHU_B0_DQ15_RG_TX_ARDQM_MCKIO_SEL_B0);
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ15), 1, SHU_B1_DQ15_RG_TX_ARDQM_MCKIO_SEL_B1);
        ...
        else if (k_type == DutyScan_Calibration_K_MCK16X) // K MCK16X
        {
            clksel = 4;
            str_who_am_I = "MCK16X";
            ucdqs_i_count = 3;

            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD13),
                0, SHU_CA_CMD13_RG_TX_ARCA_DLY_LAT_EN_CA);
        }
#endif
        ...
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ13),
            0, SHU_B0_DQ13_RG_TX_ARDQ_DLY_LAT_EN_B0);
    ...
    if (k_type == DutyScan_Calibration_K_MCK16X)
    {
        s4PIEnd = s4PIBegin; //MCK16X no need CLK PI delay scan
        scinner_duty_ofst_start = DUTY_MCK16X_OFFSET_START;
        scinner_duty_ofst_end = DUTY_MCK16X_OFFSET_END;
        scinner_duty_ofst_step = DUTY_MCK16X_OFFSET_STEP;
    }

    if (k_type == DutyScan_Calibration_K_WCK)
    {
    	if (scan_type==FIX_PI_WL)
        {
            wck_dly[BYTE_0] = GetWlevDly(p, BYTE_0, PI_BASED);
            wck_dly[BYTE_1] = GetWlevDly(p, BYTE_1, PI_BASED);
        }
        else
        {
            wck_dly[BYTE_0] = 0;
    ...
    if (k_type == DutyScan_Calibration_K_CLK)
    {
        //CLK pi awalys fix at 0, so only K PI=0
        s4PIBegin = 0;
        s4PIEnd = 0;
    }
    ...
    for(ucdqs_i=0; ucdqs_i<ucdqs_i_count; ucdqs_i++)
    {
        ...
        if (k_type == DutyScan_Calibration_K_MCK16X)
        {
            if (ucdqs_i==0)
            {
                mcSHOW_DBG_MSG("[%s CA Duty scan]\n", str_who_am_I);
            }
            else
            {
                mcSHOW_DBG_MSG("[%s B%d Duty scan]\n", str_who_am_I, ucdqs_i-1);
            }
        }		
        else
        {
        ...
        if (k_type == DutyScan_Calibration_K_WCK && (scan_type==FIX_PI_WL||scan_type==FIX_PI_0))
        {
            s4PIEnd = s4PIBegin = wck_dly[ucdqs_i];

            if(isAutoK == AUTOK_ON)
            {
                vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), P_Fld(s4PIBegin, MISC_DUTY_AUTOK1_DUTY_CAL_PI_BEGIN) |
                                                                                P_Fld(s4PIEnd, MISC_DUTY_AUTOK1_DUTY_CAL_PI_MAX) );
            }
        }
        ...
        early_break_count=0;
        found_duty_pass_criteria = 0xff;
        min_closetozero = 0xff;
        count_maxmin_duty_5000_percent = 0;
        ...
        for(scinner_duty_ofst=scinner_duty_ofst_start; scinner_duty_ofst<=scinner_duty_ofst_end; scinner_duty_ofst+=scinner_duty_ofst_step)
        {
            ...
            {
                if (ucdqs_i==0) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay, SHU_B0_TXDUTY_DA_TX_ARDQS_DUTY_DLY_B0);
                else 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay, SHU_B1_TXDUTY_DA_TX_ARDQS_DUTY_DLY_B1);
            }
#if SUPPORT_TYPE_LPDDR5
            if (k_type == DutyScan_Calibration_K_WCK)
            {
                if (ucdqs_i==0) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay, SHU_B0_TXDUTY_DA_TX_ARWCK_DUTY_DLY_B0);
                else 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay, SHU_B1_TXDUTY_DA_TX_ARWCK_DUTY_DLY_B1);
            }
            if (k_type == DutyScan_Calibration_K_MCK16X)
            {
                if (ucdqs_i==0) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD1), scinner_duty_ofst, SHU_CA_CMD1_RG_ARPI_MCK8X_DCC_SEL_C0);
                else if (ucdqs_i==1) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ1), scinner_duty_ofst, SHU_B0_DQ1_RG_ARPI_MCK8X_DCC_SEL_B0);				
                else 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ1), scinner_duty_ofst, SHU_B1_DQ1_RG_ARPI_MCK8X_DCC_SEL_B1);				
            }
#endif
            if (k_type == DutyScan_Calibration_K_CLK)
            {
                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_TXDUTY), tDelay, SHU_CA_TXDUTY_DA_TX_ARCLK_DUTY_DLY);
            }
            ...
            {
                if (ucdqs_i==0) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay, SHU_B0_TXDUTY_DA_TX_ARDQ_DUTY_DLY_B0);
                else 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay, SHU_B1_TXDUTY_DA_TX_ARDQ_DUTY_DLY_B1);
            }

            if (k_type == DutyScan_Calibration_K_DQM)
            {
                if (ucdqs_i==0) 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_TXDUTY), tDelay, SHU_B0_TXDUTY_DA_TX_ARDQM_DUTY_DLY_B0);
                else 
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_TXDUTY), tDelay, SHU_B1_TXDUTY_DA_TX_ARDQM_DUTY_DLY_B1);
            }
#endif
            ...
            if (isAutoK == AUTOK_ON)
            {
                if (k_type == DutyScan_Calibration_K_CLK)
                {
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 1, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_CA);
                    #if defined(FPC_TEST)    
                        while_cnt=0;
                    #endif
                    ...
                    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 0, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_CA);
                ...
                else
                {
                    if (ucdqs_i == 0)
                    {
                        if (NeedPhyNaoByteSwap(p) == TRUE)
                        {
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 1, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_B1);
                            #if defined(FPC_TEST)
                                while_cnt=0;
                            #endif
                            ...
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 0, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_B1);
                        ...
                        {
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 1, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_B0);
                            #if defined(FPC_TEST)
                                while_cnt=0;
                            #endif
                            ...
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTY_AUTOK1), 0, MISC_DUTY_AUTOK1_DUTY_AUTOK_EN_B0);
...
//                        mcSHOW_DBG_MSG("Fra B1 Max Duty = %d (%d), Min Duty = %d (%d)\n", ucperiod_duty_max, ucperiod_duty_max_clk_dly, ucperiod_duty_min, ucperiod_duty_min_clk_dly);
            ...
            else
            {
                for(s4PICnt=s4PIBegin; s4PICnt<=s4PIEnd; s4PICnt+=CLOCK_PI_STEP)
                {
                    ...
                    for(swap_idx=0; swap_idx<2; swap_idx++)
                    {
                        ...
                        else if (k_type == DutyScan_Calibration_K_MCK16X)
                        {
                            if (ucdqs_i==0) 
                                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_DLL2), swap_idx, SHU_CA_DLL2_RG_ARCMD_REV_BIT3_ARPI_MCK_8X_LPBK_INV);
                            else if (ucdqs_i==1) 
                                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DLL2), swap_idx, SHU_B0_DLL2_RG_ARDQ_REV_BIT3_ARPI_MCK_8X_LPBK_INV);
                            else 
                                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DLL2), swap_idx, SHU_B1_DLL2_RG_ARDQ_REV_BIT3_ARPI_MCK_8X_LPBK_INV);
                            ...
                            if (ucdqs_i==0) 
                                ...
                                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3), vref_sel_value[swap_idx], B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1);
                        }
                        else
                        {
                            if (swap_idx==0)
...
#ifdef ENABLE_DEBUG_LOG_TO_DPM_SRAM                                                                           
            *pLog_to_DPM_SRAM++ = ucdqs_i | (p->channel<<16);
            *pLog_to_DPM_SRAM++ = scinner_duty_ofst;
            *pLog_to_DPM_SRAM++ = ucperiod_duty_max;
            *pLog_to_DPM_SRAM++ = ucperiod_duty_min;     
#endif 

#ifdef FOR_HQA_TEST_USED
            if (scinner_duty_ofst==16)
            {
                gFinalMCK16XDuty_16[p->channel][ucdqs_i] = (U16)ucperiod_duty_averige;
            }
#endif
            ...
            record_closetozero = (scinner_duty_ofst<0)?(-scinner_duty_ofst):scinner_duty_ofst;

            if (k_type != DutyScan_Calibration_K_MCK16X)
            if (ucperiod_duty_max==5000 && ucperiod_duty_min==5000)
            {
                count_maxmin_duty_5000_percent++;

                if (count_maxmin_duty_5000_percent>=(DUTY_OFFSET_END-DUTY_OFFSET_START))
                {
            ...
            if (temp_value <= ucmost_approach_50_percent) // To find transaction (Inflection) point
            {
                if (temp_value == ucmost_approach_50_percent) // Same as last transaction (Inflection) point
                    temp_last_pass[1] = scinner_duty_ofst;
                else
                    temp_first_pass[1] = temp_last_pass[1] = scinner_duty_ofst;

                ucmost_approach_50_percent = temp_value;
                record_duty_log[1].ucmost_approach_50_percent = temp_value;
                record_duty_log[1].ucFinal_period_duty_averige = ucperiod_duty_averige;
                record_duty_log[1].ucFinal_period_duty_max = ucperiod_duty_max;
                record_duty_log[1].ucFinal_period_duty_min = ucperiod_duty_min;
                record_duty_log[1].ucFinal_duty_max_clk_dly = ucperiod_duty_max_clk_dly;
                record_duty_log[1].ucFinal_duty_min_clk_dly = ucperiod_duty_min_clk_dly;
            ...
            {
                early_break_count ++;
                if (early_break_count>=2 && scinner_duty_ofst> ((k_type == DutyScan_Calibration_K_MCK16X) ? 16 : 0))
                {
                    mcSHOW_DBG_MSG2("\t ==> Early Break !!!\n");
                    break; //early break;
                }
        ...
        if (k_type == DutyScan_Calibration_K_MCK16X) found_duty_pass_criteria=0; //MCK16X use most approach 50 percent

        /* set this best pass to final result */
        final_duty_select = (found_duty_pass_criteria == 1) ? 0 : 1;
        scFinal_clk_delay_cell[ucdqs_i] = (temp_first_pass[final_duty_select] + temp_last_pass[final_duty_select]) / 2; /* best step is (first pass+last pass)/2 */
        ucFinal_period_duty_averige[ucdqs_i] = record_duty_log[final_duty_select].ucFinal_period_duty_averige;
        ucFinal_period_duty_max[ucdqs_i] = record_duty_log[final_duty_select].ucFinal_period_duty_max;
        ucFinal_period_duty_min[ucdqs_i] = record_duty_log[final_duty_select].ucFinal_period_duty_min;
        ucFinal_duty_max_clk_dly[ucdqs_i] = record_duty_log[final_duty_select].ucFinal_duty_max_clk_dly;
        ucFinal_duty_min_clk_dly[ucdqs_i] = record_duty_log[final_duty_select].ucFinal_duty_min_clk_dly;

        mcSHOW_DBG_MSG4("[%d] record_duty_log_ucmost_approach_50_percent=%d\n",scinner_duty_ofst, record_duty_log[final_duty_select].ucmost_approach_50_percent);
        mcSHOW_DBG_MSG("first_pass[0] = %d, last_pass[0] = %d\n", temp_first_pass[0], temp_last_pass[0]);
        mcSHOW_DBG_MSG("first_pass[1] = %d, last_pass[1] = %d\n", temp_first_pass[1], temp_last_pass[1]);
        mcSHOW_DBG_MSG("Final : first_pass[%d] = %d, last_pass[%d] = %d\n", final_duty_select, temp_first_pass[final_duty_select], final_duty_select, temp_last_pass[final_duty_select]);
    ...
    for(ucdqs_i=0; ucdqs_i<ucdqs_i_count; ucdqs_i++)
    {
        ...
        if (k_type != DutyScan_Calibration_K_MCK16X)
        if ((scFinal_clk_delay_cell[ucdqs_i]>DutyPassCriteria)||(scFinal_clk_delay_cell[ucdqs_i]<-DutyPassCriteria))
        {
        ...
        if ((ucFinal_period_duty_averige[ucdqs_i]-ucStart_period_duty_averige[ucdqs_i])<100)
        {
            ...
            #if __ETT__
            //fra if (gETT_WHILE_1_flag) ASSERT(0);
            #endif
        }
    ...
    if (k_type == DutyScan_Calibration_K_MCK16X)
    {
        ...
        if(p->femmc_Ready==0)
        {
            p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][0] = scFinal_clk_delay_cell[0];
            p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][1] = scFinal_clk_delay_cell[1];
            p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][2] = scFinal_clk_delay_cell[2];			
        }
#endif

        // backup final values
        gcFinal_K_MCK16X_delay_cell[p->channel][0] = scFinal_clk_delay_cell[0];
        gcFinal_K_MCK16X_delay_cell[p->channel][1] = scFinal_clk_delay_cell[1];
        gcFinal_K_MCK16X_delay_cell[p->channel][2] = scFinal_clk_delay_cell[2];		

#ifdef FOR_HQA_TEST_USED
        gFinalMCK16XDuty[p->channel][0] = scFinal_clk_delay_cell[0];
        gFinalMCK16XDuty[p->channel][1] = scFinal_clk_delay_cell[1];
        gFinalMCK16XDuty[p->channel][2] = scFinal_clk_delay_cell[2];
#endif

        MCKDutyScan_SetMCK16XDelayCell(p, scFinal_clk_delay_cell);
    }
#endif
    if (k_type == DutyScan_Calibration_K_CLK)
    {
...
void DramcNewDutyCalibration(DRAMC_CTX_T *p, U16 duty_k_type, DUTYSCAN_SCAN_PI_T scan_type, DUTYSCAN_WHERE_T duty_from_where, U8 isAutoK)
{
    U32 tmp;
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        MIX_RG_CHECK(DDRPHY_REG_B0_DQ6),
        MIX_RG_CHECK(DDRPHY_REG_B0_PHY3),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ5),
#endif
        MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B0_DQ0),

        MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DLL_ARPI2),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DLL_ARPI2),
        MIX_RG_CHECK(DDRPHY_REG_SHU_CA_DLL_ARPI2),
#if !APPLY_NEW_IO_API
        MIX_RG_CHECK(DDRPHY_REG_B1_DQ6),
        MIX_RG_CHECK(DDRPHY_REG_B1_PHY3),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ5),
#endif
        MIX_RG_CHECK(DDRPHY_REG_SHU_RK_B1_DQ0),

        MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ11),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ11),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DQ15),
        MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DQ15),
        MIX_RG_CHECK(DDRPHY_REG_SHU_CA_CMD11),
#if !APPLY_NEW_IO_API
        MIX_RG_CHECK(DDRPHY_REG_CA_CMD6),
        MIX_RG_CHECK(DDRPHY_REG_CA_PHY3),
        MIX_RG_CHECK(DDRPHY_REG_SHU_CA_CMD5),
#endif
        MIX_RG_CHECK(DDRPHY_REG_SHU_RK_CA_CMD),
        ...
        MIX_RG_CHECK(DDRPHY_REG_MISC_DUTY_AUTOK1),
        MIX_RG_CHECK(DDRPHY_REG_MISC_DUTY_AUTOK2),
        MIX_RG_CHECK(DDRPHY_REG_MISC_CG_CTRL2),
    };
#endif

#if APPLY_NEW_IO_API
    U32 u4MixRgBackupList[][2] = {
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_TX_ARDQ_DATA_SWAP_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_RX_ARDQ_LPBK_EN_B0},
        {DDRPHY_REG_B0_DQ6, B0_DQ6_RG_TX_ARDQ_LPBK_SEL_B0},

        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_EN_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_TX_ARDQ_DATA_SWAP_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_RX_ARDQ_LPBK_EN_B1},
        {DDRPHY_REG_B1_DQ6, B1_DQ6_RG_TX_ARDQ_LPBK_SEL_B1},

        {DDRPHY_REG_CA_CMD6, CA_CMD6_RG_TX_ARCMD_DATA_SWAP_EN},
        {DDRPHY_REG_CA_CMD6, CA_CMD6_RG_TX_ARCMD_DATA_SWAP},
        {DDRPHY_REG_CA_CMD6, CA_CMD6_RG_TX_ARCA_LPBK_SEL_CA},
        {DDRPHY_REG_CA_CMD6, CA_CMD6_RG_RX_ARCMD_LPBK_EN},

        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_OFFSETC_B0},
        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B0},
        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_EN_B0},
        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0},

        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_OFFSETC_B1},
        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1},
        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_EN_B1},
        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1},

        {DDRPHY_REG_CA_PHY3, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_OFFSETC_CA},
        {DDRPHY_REG_CA_PHY3, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_VREF_SEL_CA},
        {DDRPHY_REG_CA_PHY3, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_EN_CA},
        {DDRPHY_REG_CA_PHY3, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_CLK_SEL_CA},


        {DDRPHY_REG_SHU_B0_DQ5, SHU_B0_DQ5_RG_RX_ARDQ_VREF_BYPASS_B0},
        {DDRPHY_REG_SHU_B1_DQ5, SHU_B1_DQ5_RG_RX_ARDQ_VREF_BYPASS_B1},
        {DDRPHY_REG_SHU_CA_CMD5, SHU_CA_CMD5_RG_RX_ARCMD_VREF_BYPASS},

        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARDQ_DLY_LAT_EN_B1},
...
mcDUMP_REG_MSG("\n[dumpRG] DramcNewDutyCalibration\n");
    ...
    if(p->femmc_Ready==1)
    {
    	...
    	if (duty_k_type & DutyScan_Calibration_K_CLK) DramcClockDutySetClkDelayCell(p, p->pSavetimeData->s1ClockDuty_clk_delay_cell[p->channel]);
        if (duty_k_type & DutyScan_Calibration_K_DQS) DQSDutyScan_SetDqsDelayCell(p, p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel]);
    #if SUPPORT_TYPE_LPDDR5
        if (is_lp5_family(p))
        {
            if (duty_k_type & DutyScan_Calibration_K_WCK) 
            {
                WCKDutyScan_SetWCKDelayCell(p, p->pSavetimeData->s1WCKDuty_clk_delay_cell[p->channel]);
				//load DQS duty dly cause no K DQS @ LP5 (DQS/WCK use same PI dly)
                DQSDutyScan_SetDqsDelayCell(p, p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel]);
            }
            if (duty_k_type & DutyScan_Calibration_K_MCK16X) MCKDutyScan_SetMCK16XDelayCell(p, p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel]);
        }
    #endif
    #if APPLY_DQDQM_DUTY_CALIBRATION
        #if (CHIP_PROCESS>5)
        if (duty_k_type & DutyScan_Calibration_K_DQM) DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, p->pSavetimeData->s1DQMDuty_clk_delay_cell[p->channel], DutyScan_Calibration_K_DQM);
        #endif
        if (duty_k_type & DutyScan_Calibration_K_DQ) DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, p->pSavetimeData->s1DQDuty_clk_delay_cell[p->channel], DutyScan_Calibration_K_DQ);
    #endif
    ...
    #endif
    {
        ...
        if (Get_Duty_Calibration_Mode(p) == DUTY_DEFAULT)
        {
            ...
            for (u1ChannelIdx=0; u1ChannelIdx<p->support_channel_num; u1ChannelIdx++)
            {
                ...
                gcFinal_K_MCK16X_delay_cell[u1ChannelIdx][0] = 0x10;
                gcFinal_K_MCK16X_delay_cell[u1ChannelIdx][1] = 0x10;
                gcFinal_K_MCK16X_delay_cell[u1ChannelIdx][2] = 0x10;
                #if APPLY_DQDQM_DUTY_CALIBRATION
        ...
        if ((Get_Duty_Calibration_Mode(p) == DUTY_DEFAULT) || (Get_Duty_Calibration_Mode(p) == DUTY_LAST_K))
        {
            ...
            if (duty_k_type & DutyScan_Calibration_K_CLK) DramcClockDutySetClkDelayCell(p, gcFinal_K_CLK_delay_cell[p->channel]);
            if (duty_k_type & DutyScan_Calibration_K_DQS) DQSDutyScan_SetDqsDelayCell(p, gcFinal_K_DQS_delay_cell[p->channel]);
#if SUPPORT_TYPE_LPDDR5
            if (duty_k_type & DutyScan_Calibration_K_WCK) 
            {
                WCKDutyScan_SetWCKDelayCell(p, gcFinal_K_WCK_delay_cell[p->channel]);
                //DQS use WCK's duty dly
                DQSDutyScan_SetDqsDelayCell(p, gcFinal_K_WCK_delay_cell[p->channel]);
            }
#endif
            ...
            if(p->femmc_Ready==0)
            {
                if (duty_k_type & DutyScan_Calibration_K_CLK)
                {
                ...
                if (duty_k_type & DutyScan_Calibration_K_DQS)
                {
                ...
                if (duty_k_type & DutyScan_Calibration_K_WCK)
                {
                    ...
                    //DQS use WCK's duty dly
                    p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel][0] = gcFinal_K_WCK_delay_cell[p->channel][0];
                    p->pSavetimeData->s1DQSDuty_clk_delay_cell[p->channel][1] = gcFinal_K_WCK_delay_cell[p->channel][1];				
                }
                if (duty_k_type & DutyScan_Calibration_K_MCK16X)
                {
                    p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][0] = gcFinal_K_MCK16X_delay_cell[p->channel][0];
                    p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][1] = gcFinal_K_MCK16X_delay_cell[p->channel][1];
                    p->pSavetimeData->s1MCK16XDuty_clk_delay_cell[p->channel][2] = gcFinal_K_MCK16X_delay_cell[p->channel][2];
                }
            }
#endif
...
#if APPLY_DQDQM_DUTY_CALIBRATION
        #if (CHIP_PROCESS>5)
        if (duty_k_type & DutyScan_Calibration_K_DQM) DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, gcFinal_K_DQM_delay_cell[p->channel], DutyScan_Calibration_K_DQM);
        #endif
        if (duty_k_type & DutyScan_Calibration_K_DQ) DQDQMDutyScan_SetDQDQMDelayCell(p, p->channel, gcFinal_K_DQ_delay_cell[p->channel], DutyScan_Calibration_K_DQ);

#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
            if(p->femmc_Ready==0)
            {
                if (duty_k_type & DutyScan_Calibration_K_DQM)
                {
                ...
                if (duty_k_type & DutyScan_Calibration_K_DQ)
                {
            ...
            if (p->frequency < 2133) //MCK16X Need K >= 4266
            {
                return;
            }
        }
...
#if SUPPORT_TYPE_LPDDR5
        if (is_lp5_family(p) && p->freqGroup >= 2133)
        {
            if (duty_k_type & DutyScan_Calibration_K_MCK16X) u2FailStatusByCh |= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_MCK16X, 1, scan_type, AUTOK_OFF);	//MCK16X can't use AUTO K
        }
#endif

        if (!((Get_Duty_Calibration_Mode(p) == DUTY_DEFAULT) || (Get_Duty_Calibration_Mode(p) == DUTY_LAST_K)))
        {
            if (duty_k_type & DutyScan_Calibration_K_CLK) u2FailStatusByCh = DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_CLK, 1, scan_type, isAutoK);
            if (duty_k_type & DutyScan_Calibration_K_DQS) u2FailStatusByCh |= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_DQS, 1, scan_type, isAutoK);
#if SUPPORT_TYPE_LPDDR5
            if (is_lp5_family(p))
            {
                if (duty_k_type & DutyScan_Calibration_K_WCK)
                {
                    ...
                    toggle_wck(p, ENABLE);
                    u2FailStatusByCh |= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_WCK, 1, scan_type, isAutoK);
                    toggle_wck(p, DISABLE);
                }
            }
#endif
            #if APPLY_DQDQM_DUTY_CALIBRATION
            // Wheel 20210910 :
            // Current DQM duty calibration is for CS0/CS1 in CA ( IO type driver).
            // So please bypass DQM duty calibration in FT/normal calibration.
            // LP4 DQM duty need K, follow M_ntr_s_
            #if (CHIP_PROCESS>5)
            if (duty_k_type & DutyScan_Calibration_K_DQM) u2FailStatusByCh |= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_DQM, 1, scan_type, isAutoK);
            #endif
                ...
                #endif
                #endif
            if (duty_k_type & DutyScan_Calibration_K_DQ) u2FailStatusByCh |= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_DQ, 1, scan_type, isAutoK);
            #endif
        }
...
#if ENABLE_DRAMOBF
void DRAMOBF_initializaton(DRAMC_CTX_T *p)
{
    U32 u4loop_count = 0;
    U32 backup_broadcast;
    ...
    //polling all channel Key param valid
    channel_backup_and_set( p, CHANNEL_A);
    do {
        while ((u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_0270_DRAMOBF_PM), 
			REG_0270_DRAMOBF_PM_REG_KEY_VALID_IN_PM) != 0x1))
        {
            mcDELAY_US(1);
            u4loop_count++;
            if (u4loop_count > 100)
            {
                mcSHOW_ERR_MSG("Error! DRAMOBF Key not valid in CH%d!\n",channel_get(p) );
                ASSERT(0);
            }
        }
        channel_advance(p);
    ...
    //enable DCM clock
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_0000_DRAMOBF), 1,
    REG_0000_DRAMOBF_REG_DRAMOBF_CKG_EN);
    //set latch new key param
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_026C_DRAMOBF_PM), 1,
    REG_026C_DRAMOBF_PM_REG_LATCH_NEW_KEY_PARAM);
    //disable DCM clock
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_0000_DRAMOBF), 0,
    REG_0000_DRAMOBF_REG_DRAMOBF_CKG_EN);

    //enable DRAMOBF
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_0000_DRAMOBF), 1,
    REG_0000_DRAMOBF_REG_DRAMOBF_ENABLE);

    //bypass DRAMOBF
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_0000_DRAMOBF), 1,
    REG_0000_DRAMOBF_REG_DRAMOBF_BYPASS_EN);
	...
	0x000[13]=0
	0x000[0]=1
	bypass OBF 0x000[15]=1
	
	
	polling 0x0c0[0]==1
	polling 0x1a0[0]==1
	0x800[0]=0
	disable bypass 0x000[15]=0
#endif	

}
void DRAMOBF_bypass(DRAMC_CTX_T *p, U8 OnOff)
{
    U32 u4loop_count = 0;
    U32 backup_broadcast;
    ...
    {
        //polling all channel DRAMOBF status done
        channel_backup_and_set( p, CHANNEL_A);
        do {
            while ((u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMOBF_CH0_REG_REG_00C0_DRAMOBF),
				REG_00C0_DRAMOBF_REG_DRAMOBF_STATUS_CHN_0) != 0x1))
            {
                mcDELAY_US(1);
                u4loop_count++;
                if (u4loop_count > 100)
                {
                mcSHOW_ERR_MSG("Error! DRAMOBF status not done in CH%d!\n",channel_get(p) );
                   ASSERT(0);
                }
            }
            channel_advance(p);
...
#if ENABLE_RDCA_CAL
#define RDCA_DEBUG_LOG 1
#define RDCA_JMETER_MODE 0
#define RDCA_DUTY_MODE 0
#define RDCA_TA2_MODE 1

typedef struct _RDCA_INFO_T {
    S8 s1BestDelay[DQS_NUMBER];
    U32 u4BestDUTY_P[DQS_NUMBER];
    U32 u4BestDUTY_N[DQS_NUMBER];
    U32 u4BestDUTY_CAL[DQS_NUMBER];
} RDCA_INFO_T ;

typedef struct _RDCA_DELAY_T {
    S8 s1MR69_ReadDca[DQS_NUMBER];
} RDCA_DELAY_T ;

void SetDelay_RDCA(DRAMC_CTX_T *p, RDCA_DELAY_T *ReadDcaDeley)
{
    U8 u1RDCAU = 0, u1RDCAL = 0;

    u1RDCAL = FetchRGSettingVal(ReadDcaDeley->s1MR69_ReadDca[BYTE_0], 3);
    u1RDCAU = FetchRGSettingVal(ReadDcaDeley->s1MR69_ReadDca[BYTE_1], 3);
    mcSHOW_DBG_MSG("(ReadDcaDeley->u1MR69_ReadDca[BYTE_0] = %d, ReadDcaDeley->u1MR69_ReadDca[BYTE_1]= %d)\n", ReadDcaDeley->s1MR69_ReadDca[BYTE_0], ReadDcaDeley->s1MR69_ReadDca[BYTE_1]);
    mcSHOW_DBG_MSG("(u1RDCAL = 0x%x, u1RDCAU= 0x%x)\n", u1RDCAL, u1RDCAU);

    MRWriteFldMulti(p, 69, P_Fld(u1RDCAU, MR69_RDCAU) |
                               P_Fld(u1RDCAL, MR69_RDCAL),
                               TO_MR);
}

void SetDelay_RDCA_Single(DRAMC_CTX_T *p, S16 s2Delay, BYTES_T eByteIdx)
{
    U8 u1RDCAU = 0, u1RDCAL = 0;

    if (eByteIdx == ALL_BYTES)
    {
        u1RDCAU = FetchRGSettingVal(s2Delay, 3);
        u1RDCAL = FetchRGSettingVal(s2Delay, 3);
        MRWriteFldMulti(p, 69, P_Fld(u1RDCAU, MR69_RDCAU) |
                                   P_Fld(u1RDCAL, MR69_RDCAL),
                                   TO_MR);
    }
    ...
    {
        u1RDCAL = FetchRGSettingVal(s2Delay, 3);
        DramcMRWriteFldAlign(p, 69, u1RDCAL, MR69_RDCAL, TO_MR);
    }
    else if (eByteIdx == BYTE_1)
    {
        u1RDCAU = FetchRGSettingVal(s2Delay, 3);
        DramcMRWriteFldAlign(p, 69, u1RDCAU, MR69_RDCAL, TO_MR);
    }
}

void GetDelay_RDCA(DRAMC_CTX_T *p, RDCA_DELAY_T *RdcaDeley)
{
    //U8 u1RDCAU = 0, u1RDCAL = 0;

    //u1RDCAU = (u1MR69Value[p->channel][p->rank][p->dram_fsp]>>4)&0xf;
    //u1RDCAL = u1MR69Value[p->channel][p->rank][p->dram_fsp]&0xf;

    RdcaDeley->s1MR69_ReadDca[BYTE_1] = InvertRGSettingVal(u1MR69Value[p->channel][p->rank][p->dram_fsp]&0xf, 3); //u1RDCAU
    RdcaDeley->s1MR69_ReadDca[BYTE_0] = InvertRGSettingVal((u1MR69Value[p->channel][p->rank][p->dram_fsp]>>4)&0xf, 3); //u1RDCAL
}

static void PrintDelay_RDCA(DRAMC_CTX_T *p, RDCA_DELAY_T *RdcaDeley)
{
    U8 u1RDCAU = 0, u1RDCAL = 0;

    mcSHOW_DBG_MSG("[PrintDelay_READ_DCA]\n");

    mcSHOW_DBG_MSG("MR69_RDCAU = %d\n", RdcaDeley->s1MR69_ReadDca[BYTE_1]);
    mcSHOW_DBG_MSG("MR69_RDCAL = %d\n", RdcaDeley->s1MR69_ReadDca[BYTE_0]);
}

static void ScanRangeDelay_RDCA(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
static void ScanRangeDelay_RdcaTa2(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
static void GetFinalDelay_RDCA(DRAMC_CTX_T *p, RDCA_INFO_T*ReadDcaInfo, RDCA_DELAY_T *FinalDelay_Rdca)
{
    // Get FinalDelay from FinalWinPerBit
    FinalDelay_Rdca->s1MR69_ReadDca[BYTE_0] = ReadDcaInfo->s1BestDelay[BYTE_0];
    FinalDelay_Rdca->s1MR69_ReadDca[BYTE_1] = ReadDcaInfo->s1BestDelay[BYTE_1];
}

static void PrintFinalResult_RDCA(DRAMC_CTX_T *p, RDCA_INFO_T*ReadDcaInfo, RDCA_DELAY_T *FinalDelay_Rdca)
{
    mcSHOW_DBG_MSG("\n[RDCA PrintWidnowInfo]\n");

    //Print Delay information
    PrintDelay_RDCA(p, FinalDelay_Rdca);

#ifdef FOR_HQA_REPORT_USED
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT2, "RDCA", "_L", 0, FinalDelay_Rdca->s1MR69_ReadDca[BYTE_0], NULL);
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT2, "RDCA", "_U", 0, FinalDelay_Rdca->s1MR69_ReadDca[BYTE_1], NULL);
#endif
}

#if RDCA_JMETER_MODE
DRAM_STATUS_T DramcRdcaCal_JmeterMode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdca
    RDCA_DELAY_T FinalDelay_Rdca;
    RDCA_INFO_T RdcaInfo;
    SCAN_DELAY_T ScanDelay_Rdca;
    //Jmeter
    JMETER_T JmtrInfo;
    U16 u2StartPeriod = 0, u2MiddlePeriod = 0, u2EndPeriod = 0;
    U16 u2Num_one_perT = 0, u2Num_zero_perT = 0;
    U32 u4DUTY_P[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF}, u4DUTY_N[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF}, u4DUTY_CAL[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF};
    U32 u4DutyDiffTemp[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF}, u4DutyDiffFinal[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF};
    //Others
    U8 u1BitIdx, u1ByteIdx, u1RankIdx, u1SwapIdx;
    S8 s1DelayIdx;
    U8 u1EarlyBreakCnt = 0, u1Status[DQS_NUMBER] = {0xFF, 0xFF};
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (MIX_RG_CHECK(DDRPHY_REG_MISC_JM_8PH_AUTOK_CFG0)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_JM_8PH_AUTOK_CFG1)),
        (MIX_RG_CHECK(DDRPHY_REG_MISC_SHU_STBCAL)), // for gating on/off backup/restore
        (MIX_RG_CHECK(DDRPHY_REG_SHU_CA_DLL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B0_DLL1)),
        (MIX_RG_CHECK(DDRPHY_REG_SHU_B1_DLL1)),
        (MIX_RG_CHECK(DDRPHY_REG_B0_DQ2)),
    ...
    U32 u4MixRgBackupList[][2] = {
        ...
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_SEL_B1},
        {DDRPHY_REG_SHU_B1_DQ13, SHU_B1_DQ13_RG_TX_ARWCKB_OE_TIE_EN_B1},
        {DDRPHY_REG_B0_PHY3, B0_PHY3_RG_RX_ARDQ_BUFF_EN_SEL_B0},  //in RxPathOnOff(p, ENABLE)
        {DDRPHY_REG_B1_PHY3, B1_PHY3_RG_RX_ARDQ_BUFF_EN_SEL_B1},  //in RxPathOnOff(p, ENABLE)
        {DDRPHY_REG_SHU_B0_DQ10, SHU_B0_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B0},  //in RxPathOnOff(p, ENABLE)
        {DDRPHY_REG_SHU_B0_DQ10, SHU_B0_DQ10_RG_RX_ARDQS_DQSIEN_FORCE_ON_EN_B0},  //in RxPathOnOff(p, ENABLE)
        {DDRPHY_REG_SHU_B1_DQ10, SHU_B1_DQ10_RG_RX_ARDQS_DLY_LAT_EN_B1},  //in RxPathOnOff(p, ENABLE)
        {DDRPHY_REG_SHU_B1_DQ10, SHU_B1_DQ10_RG_RX_ARDQS_DQSIEN_FORCE_ON_EN_B1},  //in RxPathOnOff(p, ENABLE)
    };
#endif
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdcaCal_JmeterMode] Start\n");
    ...
    if (p->femmc_Ready == 1)
    {
        ...
        // Load FinalDelay_ReadDca from eMMC
        FinalDelay_Rdca.s1MR69_ReadDca[BYTE_0] = p->pSavetimeData->s1RDCAL[p->channel][p->rank];
        FinalDelay_Rdca.s1MR69_ReadDca[BYTE_1] = p->pSavetimeData->s1RDCAU[p->channel][p->rank];

        // Set FinalDelay_ReadDca
        SetDelay_RDCA(p, &FinalDelay_Rdca);
    ...
    #endif
    {
        ...
        // Get default Delay
        GetDelay_RDCA(p , &FinalDelay_Rdca);
        PrintDelay_RDCA(p, &FinalDelay_Rdca);

        // Choose Delay scan range
        ScanRangeDelay_RDCA(p, &ScanDelay_Rdca);

        // Variable Initialization
        memset(&RdcaInfo, 0, sizeof(RdcaInfo));

        // Enable RDQS toggle mode
        vSetLP5_WCKON_OnOff(p, ENABLE, WCK_ALWAYS_ON_SW);
        DramcModeRegWriteByRank(p, 0x3, 46, 0x1<<1);

        // Enable RX path
        RxPathOnOff(p, ENABLE);

        // Calibration common setting
        DramcJmeterInit(p, JM_RDCA_MODE, FALSE);

        // Calibration scan delay
        for (s1DelayIdx = ScanDelay_Rdca.s2DelayStart; s1DelayIdx < ScanDelay_Rdca.s2DelayEnd; s1DelayIdx += ScanDelay_Rdca.u1DelayStep)
        {
            SetDelay_RDCA_Single(p, s1DelayIdx, ALL_BYTES);

            //mcSHOW_ERR_MSG("Delay = %d (for measurementwait 3s)!!!\n", s1DelayIdx);
            //mcDELAY_MS(3000);

            for (u1SwapIdx = 0; u1SwapIdx < 2; u1SwapIdx++)
            {
                SetJmSel(p, JM_RDCA_MODE, u1SwapIdx);

                memset(&JmtrInfo, 0, sizeof(JmtrInfo));

                JmtrInfo.u2JmDlyStart = 80;
                JmtrInfo.u2JmDlyEnd = 512;
                JmtrInfo.u1JmDlyStep = 1;
                DramcJmeterCalib(p, &JmtrInfo, FALSE, ALL_BYTES);

                if ((JmtrInfo.u1TransCnt[0] != CYCLE_1T) && (JmtrInfo.u1TransCnt[1] != CYCLE_1T)) // (1T or 0.5T)
                {
                    mcSHOW_ERR_MSG("READ_DCA FAIL (don't get full cycle) !!!\n");
                    mcSHOW_ERR_MSG("JmtrInfo.u1TransCnt[0] = %d, JmtrInfo.u1TransCnt[1] = %d\n", JmtrInfo.u1TransCnt[0], JmtrInfo.u1TransCnt[1]);
                    #if __ETT__
                    ASSERT(0);
                    #endif
                }
                ...
                for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx ++)
                {
                    u2StartPeriod= JmtrInfo.JmtrInfo[u1ByteIdx][0].u1JmDelay;
                    u2MiddlePeriod= JmtrInfo.JmtrInfo[u1ByteIdx][1].u1JmDelay;
                    u2EndPeriod= JmtrInfo.JmtrInfo[u1ByteIdx][2].u1JmDelay;

                    if (JmtrInfo.JmtrInfo[u1ByteIdx][0].u1TransLevel == 1)// find 0->1 in the first time
                    {
                        u2Num_one_perT = u2MiddlePeriod - u2StartPeriod;
                        u2Num_zero_perT = u2EndPeriod - u2MiddlePeriod;
                    }
                    else //if (JmtrInfo.JmtrInfo[u1ByteIdx][0].u1TransLevel == 0)// find 1->0 in the first time
                    {
                        u2Num_zero_perT = u2MiddlePeriod - u2StartPeriod;
                        u2Num_one_perT = u2EndPeriod - u2MiddlePeriod;
                    }

                    #if RDCA_DEBUG_LOG
                    mcSHOW_DBG_MSG("%d:[SWAP%d]B%d: u2Num_one_perT = %d, u2Num_zero_perT = %d (u2StartPeriod = %d, u2MiddlePeriod = %d, u2EndPeriod = %d)\n", s1DelayIdx, u1SwapIdx, u1ByteIdx, u2Num_one_perT, u2Num_zero_perT, u2StartPeriod, u2MiddlePeriod, u2EndPeriod);
                    #endif

                    if (u1SwapIdx == 0)
                    {
                        u4DUTY_P[u1ByteIdx]= (u2Num_one_perT*10000/(u2Num_one_perT+u2Num_zero_perT));
                    }
                    else
                    {
                        u4DUTY_N[u1ByteIdx]= (u2Num_one_perT*10000/(u2Num_one_perT+u2Num_zero_perT));

                        //DUTY_CAL = 0.5*(DUTY_P-u4DUTY_N)+50%
                        u4DUTY_CAL[u1ByteIdx] = (10000 + u4DUTY_P[u1ByteIdx] - u4DUTY_N[u1ByteIdx])>>1 ;

                        if (u4DUTY_CAL[u1ByteIdx] >= 5000)
                        {
                            u4DutyDiffTemp[u1ByteIdx] = u4DUTY_CAL[u1ByteIdx] - 5000;
                            u1Status[u1ByteIdx] = 1;
                        }
                        else
                        {
                            u4DutyDiffTemp[u1ByteIdx] =  5000 - u4DUTY_CAL[u1ByteIdx];
                            u1Status[u1ByteIdx] = 0;
                        }
                        if (u4DutyDiffTemp[u1ByteIdx] < u4DutyDiffFinal[u1ByteIdx])
                        {
                            u4DutyDiffFinal[u1ByteIdx] = u4DutyDiffTemp[u1ByteIdx];
                            RdcaInfo.s1BestDelay[u1ByteIdx] = s1DelayIdx;
                            RdcaInfo.u4BestDUTY_P[u1ByteIdx] = u4DUTY_P[u1ByteIdx];
                            RdcaInfo.u4BestDUTY_N[u1ByteIdx] = u4DUTY_N[u1ByteIdx];
                            RdcaInfo.u4BestDUTY_CAL[u1ByteIdx] = u4DUTY_CAL[u1ByteIdx];
                        }
                    }
                }
            }
            mcSHOW_DBG_MSG("[%2d] B0: %d, B1: %d\n", s1DelayIdx, u1Status[BYTE_0], u1Status[BYTE_1]);
            mcSHOW_DBG_MSG("(s1BestDelay B0= %d, B1= %d, s1DelayIdx = %d)\n", RdcaInfo.s1BestDelay[BYTE_0], RdcaInfo.s1BestDelay[BYTE_1], s1DelayIdx);

            #if RDCA_DEBUG_LOG
            mcSHOW_DBG_MSG("B0: u4DutyDiffTemp=%d   u4DutyDiffFinal=%d ||B1 u4DutyDiffTemp=%d   u4DutyDiffFinal=%d\n", u4DutyDiffTemp[BYTE_0], u4DutyDiffFinal[BYTE_0]	, u4DutyDiffTemp[BYTE_1], u4DutyDiffFinal[BYTE_1]);
            mcSHOW_DBG_MSG("[%d]B0: u4DUTY_P=%d   u4DUTY_N=%d   u4DUTY_CAL=%d B1 u4DUTY_P=%d   u4DUTY_N=%d   u4DUTY_CAL=%d\n", s1DelayIdx, u4DUTY_P[BYTE_0], u4DUTY_N[BYTE_0], u4DUTY_CAL[BYTE_0], u4DUTY_P[BYTE_1], u4DUTY_N[BYTE_1], u4DUTY_CAL[BYTE_1]);
            #endif

            if ((u4DutyDiffTemp[BYTE_0] > u4DutyDiffFinal[BYTE_0]) && (u4DutyDiffTemp[BYTE_1] > u4DutyDiffFinal[BYTE_1]))
            {
                u1EarlyBreakCnt++;
            }
            if (u1EarlyBreakCnt>=5)
            {
                mcSHOW_DBG_MSG("EarlyBreak !!(s1BestDelay B0= %d, B1= %d, s1DelayIdx = %d)\n", RdcaInfo.s1BestDelay[BYTE_0], RdcaInfo.s1BestDelay[BYTE_1], s1DelayIdx);
                break;
            }
        }

        // Get FinalDelay from WinInfo
        GetFinalDelay_RDCA(p, &RdcaInfo, &FinalDelay_Rdca);

        // Set Final delay (DQ, DQM, DQS)
        SetDelay_RDCA(p, &FinalDelay_Rdca);

        // Print calibration result
        PrintFinalResult_RDCA(p, &RdcaInfo, &FinalDelay_Rdca);

        // Disable RDQS toggle mode
        DramcModeRegWriteByRank(p, 0x3, 46, 0x0);
        vSetLP5_WCKON_OnOff(p, DISABLE, WCK_ALWAYS_ON_SW);

        // Set calibration result
        if ((u4DUTY_CAL[BYTE_0]>=4800)&&(u4DUTY_CAL[BYTE_0]<=5200)&&(u4DUTY_CAL[BYTE_1]>=4800)&&(u4DUTY_CAL[BYTE_1]<=5200))
        {
            K_result = DRAM_OK;
        }
        ...
        if (p->femmc_Ready == 0)
        {
            // Save and FinalDelay_XXX to eMMC
            p->pSavetimeData->s1RDCAL[p->channel][p->rank] = FinalDelay_Rdca.s1MR69_ReadDca[BYTE_0];
            p->pSavetimeData->s1RDCAU[p->channel][p->rank] = FinalDelay_Rdca.s1MR69_ReadDca[BYTE_1];
        }
        #endif
    }

    vSetCalibrationResult(p, DRAM_CALIBRATION_RDCA, K_result);
...
#if RDCA_DUTY_MODE
static void RdqsDutyScan_Offset_Calibration(DRAMC_CTX_T *p, U8 *u1OfB0, U8 *u1OfB1)
{
    U8 calibration_sequence[15]={0xf, 0xe, 0xd, 0xc, 0xb, 0xa, 0x9, 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7};
    U8 i, read_val_b0, read_val_b1, read_val_ca;
    U8 cal_i_b0=0xff, cal_i_b1=0xff, cal_i_ca=0xff;
    ...
    if (cal_i_b0==0 || cal_i_b1==0)
    {
        mcSHOW_ERR_MSG("offset calibration i=-7 and AD_RX_*RDQ_O1_B*<2>/AD_RX_*RCA2_O1 ==0 !!\n");
    }
    else if ((read_val_b0==1 && cal_i_b0==0xff) || (read_val_b1==1 && cal_i_b1==0xff))
    {
        mcSHOW_ERR_MSG("offset calibration i=7 and AD_RX_*RDQ_O1_B*<2>/AD_RX_*RCA2_O1 ==1 !!\n");
        if (read_val_b0==1 && cal_i_b0==0xff)
            cal_i_b0 = (i-1); //max
    ...
    mcSHOW_DBG_MSG2("====================================================\n");
    mcSHOW_DBG_MSG("   Final\tB0:%d\tB1:%d\n",DutyScan_Offset_Convert(cal_i_b0),DutyScan_Offset_Convert(cal_i_b1));
...
static DRAM_STATUS_T DramcRdcaCal_DutyMode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdca
    ...
    U8 vref_sel_value[2], cal_out_value;
    S32 duty_value[2];
    S32 final_duty[DQS_NUMBER];
    U32 u4DutyDiffTemp[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF}, u4DutyDiffFinal[DQS_NUMBER] = {0xFFFFFFFF, 0xFFFFFFFF};
    U8 u1OfsB0, u1OfsB1;
    //Others
    U8 u1BitIdx, u1ByteIdx, u1RankIdx, u1SwapIdx;
    S8 s1DelayIdx, s1VrefSelIdx;
    U8 u1EarlyBreakCnt = 0;
    U32 u4backup_broadcast;
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdcaCal_DutyMode] Start\n");
    ...
    #endif
    {
        ...
        RdqsDutyScan_Offset_Calibration(p, &u1OfsB0, &u1OfsB1);

        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ13), 0, SHU_B0_DQ13_RG_TX_ARDQ_DLY_LAT_EN_B0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ13), 0, SHU_B1_DQ13_RG_TX_ARDQ_DLY_LAT_EN_B1);
        //RX_ARDQ_DUTY_VCAL_CLK_SEL = 5: RDQS
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3), 5, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3), 5, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1);
        ...
        for (s1DelayIdx = ScanDelay_Rdca.s2DelayStart; s1DelayIdx < ScanDelay_Rdca.s2DelayEnd; s1DelayIdx += ScanDelay_Rdca.u1DelayStep)
        {
            ...
            for(u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
            {
                for(u1SwapIdx=0; u1SwapIdx<2; u1SwapIdx++)
                {
                    ...
                    vref_sel_value[u1SwapIdx] = 0;
                    if (u1ByteIdx == 0) 
                        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3), vref_sel_value[u1SwapIdx], B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B0);
                    else 
                        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3), vref_sel_value[u1SwapIdx], B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1);

                    for(s1VrefSelIdx=6; s1VrefSelIdx>=0; s1VrefSelIdx--)
                    {
                        vref_sel_value[u1SwapIdx] |= (1<<s1VrefSelIdx);
                        if (u1ByteIdx==0) 
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3), vref_sel_value[u1SwapIdx], B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B0);
                        else 
                            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3), vref_sel_value[u1SwapIdx], B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_VREF_SEL_B1);
                        ...
                        #if RDCA_DEBUG_LOG
                        mcSHOW_DBG_MSG("[%d] B%d SWAP%d: u1VrefSelIdx=%d, vref_sel_value[swap_idx]=%x, cal_out=%d\n", s1DelayIdx, u1ByteIdx, u1SwapIdx, s1VrefSelIdx, vref_sel_value[u1SwapIdx], cal_out_value);
                        #endif
                    ...
                    //duty = vref_sel * 0.624 + 15.366
                    duty_value[u1SwapIdx] = ((vref_sel_value[u1SwapIdx]*624)+15366) / 10;
                }

                final_duty[u1ByteIdx] = 5000+((duty_value[0]-duty_value[1])/2);
                if (final_duty[u1ByteIdx] >= 5000)
                {
                    u4DutyDiffTemp[u1ByteIdx] = final_duty[u1ByteIdx] - 5000;
                }
                else
                {
                    u4DutyDiffTemp[u1ByteIdx] =  5000 - final_duty[u1ByteIdx];
                }
                if (u4DutyDiffTemp[u1ByteIdx] < u4DutyDiffFinal[u1ByteIdx])
                {
                    u4DutyDiffFinal[u1ByteIdx] = u4DutyDiffTemp[u1ByteIdx];
                    RdcaInfo.s1BestDelay[u1ByteIdx] = s1DelayIdx;
                    RdcaInfo.u4BestDUTY_P[u1ByteIdx] = duty_value[0];
                    RdcaInfo.u4BestDUTY_N[u1ByteIdx] = duty_value[1];
                    RdcaInfo.u4BestDUTY_CAL[u1ByteIdx] = final_duty[u1ByteIdx];
                }

                #if RDCA_DEBUG_LOG
                mcSHOW_DBG_MSG("\t[%d]B%d : Vref_Sel=0x%x, Swap Vref_Sel=0x%x\n", s1DelayIdx, u1ByteIdx, vref_sel_value[0], vref_sel_value[1]);
                mcSHOW_DBG_MSG("\t[%d]B%d : duty_value=%d, Swap duty_value=%d\n", s1DelayIdx, u1ByteIdx, duty_value[0], duty_value[1]);
                #endif
                mcSHOW_DBG_MSG("\t[%d]B%d : final_duty=%d\n", s1DelayIdx, u1ByteIdx, final_duty[u1ByteIdx]);
            ...
            mcSHOW_DBG_MSG("(s1BestDelay B0= %d, B1= %d, s1DelayIdx = %d)\n", RdcaInfo.s1BestDelay[BYTE_0], RdcaInfo.s1BestDelay[BYTE_1], s1DelayIdx);
            #if RDCA_DEBUG_LOG
            mcSHOW_DBG_MSG("[%d]B0: u4CurrentDUTY_CAL=%d||B1 u4CurrentDUTY_CAL=%d\n", s1DelayIdx, final_duty[BYTE_0], final_duty[BYTE_1]);
            mcSHOW_DBG_MSG("[%d]B0: u4BestDUTY_CAL=%d||B1 u4BestDUTY_CAL=%d\n", s1DelayIdx, RdcaInfo.u4BestDUTY_CAL[BYTE_0], RdcaInfo.u4BestDUTY_CAL[BYTE_1]);
            mcSHOW_DBG_MSG("[%d]B0: u4DutyDiffTemp=%d   u4DutyDiffFinal=%d ||B1 u4DutyDiffTemp=%d   u4DutyDiffFinal=%d\n", s1DelayIdx, u4DutyDiffTemp[BYTE_0], u4DutyDiffFinal[BYTE_0]	, u4DutyDiffTemp[BYTE_1], u4DutyDiffFinal[BYTE_1]);
            #endif
        ...
        // Set calibration result
        if ((RdcaInfo.u4BestDUTY_CAL[BYTE_0]>=4800)&&(RdcaInfo.u4BestDUTY_CAL[BYTE_0]<=5200)&&(RdcaInfo.u4BestDUTY_CAL[BYTE_1]>=4800)&&(RdcaInfo.u4BestDUTY_CAL[BYTE_1]<=5200))
        {
            K_result = DRAM_OK;
        }
    ...
    mcSHOW_DBG_MSG("[DramcRdcaCal_DutyMode] Done\n");
...
#if RDCA_TA2_MODE
DRAM_STATUS_T DramcRdcaCal_Ta2Mode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdca
    ...
    //RX perbit
    SCAN_DELAY_T ScanDelay_RxTa2;
    PASS_WIN_DATA_T WinPerBit[DQ_DATA_WIDTH+DQM_NUMBER];
    U8 isAutoK = 1, u1EnbleDqmK = 0, u1VrefScanEnable = 0;
    U8 u1RXMode = PATTERN_TEST_ENGINE;
    TA2_CFG_T ta2_cfg;
    U16 PassCriteria_RX = GetPassCriteria_RX(p);
    U32 u4err_value = 0xffffffff;
    U16 u2MinRxWindow[DQS_NUMBER] = {0xFFFF, 0xFFFF}, u2FinalMinRxWindow[DQS_NUMBER] = {0x0, 0x0};
    U8 u1MinRxBit[DQS_NUMBER] = {0xFF, 0xFF}, u1FinalMinRxBit[DQS_NUMBER] = {0xFF, 0xFF};
    U16 u2RxWindowSum[DQS_NUMBER] = {0x0, 0x0}, u2FinalRxWindowSum[DQS_NUMBER] = {0x0, 0x0};
    //Others
    U8 u1BitIdx, u1ByteIdx, u1RankIdx, u1SwapIdx;
    S8 s1DelayIdx;
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdcaCal_Ta2Mode] Start\n");
    ...
    // Choose Delay scan range
    ScanRangeDelay_RDCA(p, &ScanDelay_Rdca);
    ScanRangeDelay_RdcaTa2(p, &ScanDelay_RxTa2);

    // Initialize Final RX min window
    memset(&RdcaInfo, 0, sizeof(RdcaInfo));
    for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
    {
        u2FinalRxWindowSum[u1ByteIdx] = 0x0;
        u2FinalMinRxWindow[u1ByteIdx] = 0x0;
        u1FinalMinRxBit[u1ByteIdx] = 0xff;
    }
    ...
    for (s1DelayIdx = ScanDelay_Rdca.s2DelayStart; s1DelayIdx < ScanDelay_Rdca.s2DelayEnd; s1DelayIdx += ScanDelay_Rdca.u1DelayStep)
    {
        ...
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            for (u1BitIdx = (u1ByteIdx*DQS_BIT_NUMBER); u1BitIdx < (DQS_NUMBER*DQS_BIT_NUMBER); u1BitIdx++)
            {
                if (WinPerBit[u1BitIdx].win_size < u2MinRxWindow[u1ByteIdx])
                {
                    u2MinRxWindow[u1ByteIdx] = WinPerBit[u1BitIdx].win_size;
                    u1MinRxBit[u1ByteIdx] = u1BitIdx;
                }
                u2RxWindowSum[u1ByteIdx] += WinPerBit[u1BitIdx].win_size;
            }
        }
        ...
        for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
                ...
                ||((u2MinRxWindow[u1ByteIdx] == u2FinalMinRxWindow[u1ByteIdx])&&(u2RxWindowSum[u1ByteIdx] > u2FinalRxWindowSum[u1ByteIdx])))
            {
                u2FinalMinRxWindow[u1ByteIdx] = u2MinRxWindow[u1ByteIdx];
                u2FinalRxWindowSum[u1ByteIdx] = u2RxWindowSum[u1ByteIdx];
                u1FinalMinRxBit[u1ByteIdx] = u1MinRxBit[u1ByteIdx];
                RdcaInfo.s1BestDelay[u1ByteIdx] = s1DelayIdx;
                u1EarlyBreakCnt = 1;
            }
        }
        ...
        mcSHOW_DBG_MSG("CH%d RK%d [%d] u2MinRxWindow: B0 %d, B1 %d, u2FinalMinRxWindow: B0 %d, B1 %d ", p->channel, p->rank, s1DelayIdx, u2MinRxWindow[BYTE_0], u2MinRxWindow[BYTE_1], u2FinalMinRxWindow[BYTE_0], u2FinalMinRxWindow[BYTE_1]);
        mcSHOW_DBG_MSG("(s1BestDelay: B0 %d, B1 %d, u1EarlyBreakCnt %d)\n", RdcaInfo.s1BestDelay[BYTE_0], RdcaInfo.s1BestDelay[BYTE_1], u1EarlyBreakCnt);
    ...
    // Set calibration result (RX window >40%UI: PASS)
    if ((u2FinalMinRxWindow[BYTE_0] >= (PassCriteria_RX*8/10)) && (u2FinalMinRxWindow[BYTE_1] >= (PassCriteria_RX*8/10)))
    {
        K_result = DRAM_OK;
    }
    ...
    mcSHOW_DBG_MSG("\n[DramcRdcaCal_Ta2Mode] Done\n");

    return DRAM_OK;
}
#endif

DRAM_STATUS_T DramcRdcaCal(DRAMC_CTX_T *p, U8 u1RdcaMode)
{
    #if RDCA_JMETER_MODE
    if (u1RdcaMode == 0)
        DramcRdcaCal_JmeterMode(p);
    else
    #endif
    #if RDCA_DUTY_MODE
    if (u1RdcaMode == 1)
        DramcRdcaCal_DutyMode(p);
    else
    #endif
    #if RDCA_TA2_MODE
    if (u1RdcaMode == 2)
        DramcRdcaCal_Ta2Mode(p);
    else
    #endif
    {
        mcSHOW_ERR_MSG("[WARNING] DramcRdcaCal mode unexpect!!\n");
        #if __ETT__
        ASSERT(0);
        #endif
    }
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcRdcaCal(DRAMC_CTX_T *p)
{
    RDCA_DELAY_T FinalDelay_Rdca;
    ...
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[FAST_K] Bypass DramcRdcaCal\n");
    ...
    vSetCalibrationResult(p, DRAM_CALIBRATION_RDCA, DRAM_FAST_K);
...
#if ENABLE_RDCC_CAL
#define RDCC_RK_CANT_DIFF_WA 1
#define RDCC_DEBUG_LOG 1
#define RDCC_JMETER_MODE 0
#define RDCC_DUTY_MODE 0
#define RDCC_TA2_MODE 1

typedef struct _RDCC_INFO_T {
    S8 s1BestDelay[DQS_NUMBER];
    U32 u4BestDUTY_P[DQS_NUMBER];
    U32 u4BestDUTY_N[DQS_NUMBER];
    U32 u4BestDUTY_CAL[DQS_NUMBER];
...
typedef struct _RDCC_DELAY_T {
    S8 s1RdccStep[DQS_NUMBER];
} RDCC_DELAY_T ;

void SetDelay_RDCC(DRAMC_CTX_T *p, RDCC_DELAY_T *RdccDeley)
{
    U8 u1DccDelay_RG[DQS_NUMBER], u1DccDelay_RG_L[DQS_NUMBER], u1DccDelay_RG_U[DQS_NUMBER];

    #if RDCC_RK_CANT_DIFF_WA
    rank_backup_and_set(p, 0);
    #endif

    u1DccDelay_RG[BYTE_0] = FetchRGSettingVal(RdccDeley->s1RdccStep[BYTE_0], 5);
    u1DccDelay_RG_U[BYTE_0] = (u1DccDelay_RG[BYTE_0]>>3) & 0x7;
    u1DccDelay_RG_L[BYTE_0] = u1DccDelay_RG[BYTE_0] & 0x7;

    u1DccDelay_RG[BYTE_1] = FetchRGSettingVal(RdccDeley->s1RdccStep[BYTE_1], 5);
    u1DccDelay_RG_U[BYTE_1] = (u1DccDelay_RG[BYTE_1]>>3) & 0x7;
    u1DccDelay_RG_L[BYTE_1] = u1DccDelay_RG[BYTE_1] & 0x7;

     vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ ),
                                    P_Fld(u1DccDelay_RG_U[BYTE_0], SHU_RK_B0_DQ_RG_RX_ARDQS0_F_DLY_DUTY) |
                                    P_Fld(u1DccDelay_RG_L[BYTE_0], SHU_RK_B0_DQ_RG_RX_ARDQS0_R_DLY_DUTY));
     vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ ),
                                    P_Fld(u1DccDelay_RG_U[BYTE_1], SHU_RK_B1_DQ_RG_RX_ARDQS1_F_DLY_DUTY) |
                                    P_Fld(u1DccDelay_RG_L[BYTE_1], SHU_RK_B1_DQ_RG_RX_ARDQS1_R_DLY_DUTY));
    #if RDCC_RK_CANT_DIFF_WA
    rank_restore(p);
    #endif
}

void SetDelay_RDCC_Single(DRAMC_CTX_T *p, S16 s2Delay, BYTES_T eByteIdx)
{
    U8 u1DccDelay_RG, u1DccDelay_RG_L, u1DccDelay_RG_U;

    #if RDCC_RK_CANT_DIFF_WA
    rank_backup_and_set(p, 0);
    #endif

    u1DccDelay_RG = FetchRGSettingVal(s2Delay, 5);
    u1DccDelay_RG_U = (u1DccDelay_RG>>3) & 0x7;
    u1DccDelay_RG_L = u1DccDelay_RG & 0x7;

    if ((eByteIdx== BYTE_0)||(eByteIdx== ALL_BYTES))
    {
         vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ ),
                                        P_Fld(u1DccDelay_RG_U, SHU_RK_B0_DQ_RG_RX_ARDQS0_F_DLY_DUTY) |
                                        P_Fld(u1DccDelay_RG_L, SHU_RK_B0_DQ_RG_RX_ARDQS0_R_DLY_DUTY));
    }
    if ((eByteIdx== BYTE_1)||(eByteIdx== ALL_BYTES))
    {
         vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ ),
                                        P_Fld(u1DccDelay_RG_U, SHU_RK_B1_DQ_RG_RX_ARDQS1_F_DLY_DUTY) |
                                        P_Fld(u1DccDelay_RG_L, SHU_RK_B1_DQ_RG_RX_ARDQS1_R_DLY_DUTY));
    }
    #if RDCC_RK_CANT_DIFF_WA
    rank_restore(p);
    #endif
...
void GetDelay_RDCC(DRAMC_CTX_T *p, RDCC_DELAY_T *RdccDeley)
{
    U8 u1DccDelay_RG[DQS_NUMBER], u1DccDelay_RG_L[DQS_NUMBER], u1DccDelay_RG_U[DQS_NUMBER];

    u1DccDelay_RG_U[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_RG_RX_ARDQS0_F_DLY_DUTY);
    u1DccDelay_RG_L[BYTE_0] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQ), SHU_RK_B0_DQ_RG_RX_ARDQS0_R_DLY_DUTY);
    u1DccDelay_RG[BYTE_0] = (u1DccDelay_RG_U[BYTE_0]<<3) | u1DccDelay_RG_L[BYTE_0];

    u1DccDelay_RG_U[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_RG_RX_ARDQS1_F_DLY_DUTY);
    u1DccDelay_RG_L[BYTE_1] = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQ), SHU_RK_B1_DQ_RG_RX_ARDQS1_R_DLY_DUTY);
    u1DccDelay_RG[BYTE_1] = (u1DccDelay_RG_U[BYTE_1]<<3) | u1DccDelay_RG_L[BYTE_1];

    RdccDeley->s1RdccStep[BYTE_0] = InvertRGSettingVal(u1DccDelay_RG[BYTE_0], 5);
    RdccDeley->s1RdccStep[BYTE_1] = InvertRGSettingVal(u1DccDelay_RG[BYTE_1], 5);
}

static void PrintDelay_RDCC(DRAMC_CTX_T *p, RDCC_DELAY_T *RdccDeley)
{
    mcSHOW_DBG_MSG("[PrintDelay_RDCC]\n");

    mcSHOW_DBG_MSG("s1RdccStep[BYTE_0] = %d\n", RdccDeley->s1RdccStep[BYTE_0]);
    mcSHOW_DBG_MSG("s1RdccStep[BYTE_1] = %d\n", RdccDeley->s1RdccStep[BYTE_1]);
}

static void ScanRangeDelay_RDCC(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;

    s2DelayBegin = -15; //min = -31
    s2DelayEnd = 16; //max = 31
    u1DelayStep = 1;
...
static void ScanRangeDelay_RdccTa2(DRAMC_CTX_T *p, SCAN_DELAY_T *ScanDelay)
{
    S16 s2DelayBegin = 0, s2DelayEnd = 0;
    U8 u1DelayStep = 1;
...
static void GetFinalDelay_RDCC(DRAMC_CTX_T *p, RDCC_INFO_T*ReadDccInfo, RDCC_DELAY_T *FinalDelay_Rdcc)
{
    // Get FinalDelay from FinalWinPerBit
    FinalDelay_Rdcc->s1RdccStep[BYTE_0] = ReadDccInfo->s1BestDelay[BYTE_0];
    FinalDelay_Rdcc->s1RdccStep[BYTE_1] = ReadDccInfo->s1BestDelay[BYTE_1];

    // Restore for post process
    RDCC_final_delay[p->rank][BYTE_0] = ReadDccInfo->s1BestDelay[BYTE_0];
    RDCC_final_delay[p->rank][BYTE_1] = ReadDccInfo->s1BestDelay[BYTE_1];
}

static void PrintFinalResult_RDCC(DRAMC_CTX_T *p, RDCC_INFO_T*ReadDccInfo, RDCC_DELAY_T *FinalDelay_Rdcc)
{
    mcSHOW_DBG_MSG("\n[RDCC PrintWidnowInfo]\n");

    //Print Delay information
    PrintDelay_RDCC(p, FinalDelay_Rdcc);

    #ifdef FOR_HQA_REPORT_USED
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0, "RDCC", "", 0, FinalDelay_Rdcc->s1RdccStep[BYTE_0], NULL);
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT0, "RDCC", "", 1, FinalDelay_Rdcc->s1RdccStep[BYTE_1], NULL);
    #endif
}

static void DramcRdccCalPostProcess(DRAMC_CTX_T *p, RDCC_DELAY_T *RdccDeley)
{
    U8 u1ByteIdx;

    if ((p->support_rank_num == RANK_DUAL) && (u1GetRank(p) == (p->support_rank_num-1)))
    {
        mcSHOW_DBG_MSG("\n[DramcRdccCalPostProcess]\n");

        mcSHOW_DBG_MSG3("(Before Post process)RDCC_final_delay: B0 [RK0 =%d, RK1=%d], B1 [RK0=%d, RK1=%d]\n"
                            , RDCC_final_delay[RANK_0][BYTE_0], RDCC_final_delay[RANK_1][BYTE_0], RDCC_final_delay[RANK_0][BYTE_1], RDCC_final_delay[RANK_1][BYTE_1]);

        for (u1ByteIdx = BYTE_0; u1ByteIdx < DQS_NUMBER; u1ByteIdx++)
        {
            RDCC_final_delay[RANK_0][u1ByteIdx] = (RDCC_final_delay[RANK_0][u1ByteIdx] + RDCC_final_delay[RANK_1][u1ByteIdx])/2;
            RDCC_final_delay[RANK_1][u1ByteIdx] = RDCC_final_delay[RANK_0][u1ByteIdx];
            //mcSHOW_DBG_MSG("RANK0 and RANK1 get avg in dual RK\n");

            RdccDeley->s1RdccStep[u1ByteIdx] = RDCC_final_delay[RANK_0][u1ByteIdx];
        }

        mcSHOW_DBG_MSG3("(After Post process)RDCC_final_delay: B0 [RK0 =%d, RK1=%d], B1 [RK0=%d, RK1=%d]\n"
                            , RDCC_final_delay[RANK_0][BYTE_0], RDCC_final_delay[RANK_1][BYTE_0], RDCC_final_delay[RANK_0][BYTE_1], RDCC_final_delay[RANK_1][BYTE_1]);

        #ifdef FOR_HQA_REPORT_USED
        HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT3_1, "Post_", "RDCC", 0, RdccDeley->s1RdccStep[0], NULL);
        HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT3_1, "Post_", "RDCC", 1, RdccDeley->s1RdccStep[1], NULL);
        #endif
    }

}
#if RDCC_JMETER_MODE
DRAM_STATUS_T DramcRdccCal_JmeterMode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdcc
    RDCC_DELAY_T FinalDelay_Rdcc;
    RDCC_INFO_T RdccInfo;
    SCAN_DELAY_T ScanDelay_Rdcc;
    //Jmeter
    JMETER_T JmtrInfo;
    U16 u2StartPeriod = 0, u2MiddlePeriod = 0, u2EndPeriod = 0;
    U16 u2Num_one_perT = 0, u2Num_zero_perT = 0;
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdccCal_Jmeter] Start\n");
    ...
    if (p->femmc_Ready == 1)
    {
        ...
        // Load FinalDelay_ReadDca from eMMC
        FinalDelay_Rdcc.s1RdccStep[BYTE_0] = p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_0];
        FinalDelay_Rdcc.s1RdccStep[BYTE_1] = p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_1];

        // Set FinalDelay_ReadDca
        SetDelay_RDCC(p, &FinalDelay_Rdcc);
    ...
    #endif
    {
        ...
        // Get default Delay
        GetDelay_RDCC(p , &FinalDelay_Rdcc);
        PrintDelay_RDCC(p, &FinalDelay_Rdcc);

        // Choose Delay scan range
        ScanRangeDelay_RDCC(p, &ScanDelay_Rdcc);

        // Variable Initialization
        memset(&RdccInfo, 0, sizeof(RdccInfo));
        ...
        // Calibration common setting
        DramcJmeterInit(p, JM_RDCC_MODE, FALSE);

        // Calibration scan delay
        for (s1DelayIdx = ScanDelay_Rdcc.s2DelayStart; s1DelayIdx < ScanDelay_Rdcc.s2DelayEnd; s1DelayIdx += ScanDelay_Rdcc.u1DelayStep)
        {
            SetDelay_RDCC_Single(p, s1DelayIdx, ALL_BYTES);
            ...
            for (u1SwapIdx = 0; u1SwapIdx < 2; u1SwapIdx++)
            {
                SetJmSel(p, JM_RDCC_MODE, u1SwapIdx);
                ...
                for (u1ByteIdx = 0; u1ByteIdx < DQS_NUMBER; u1ByteIdx ++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            u4DutyDiffFinal[u1ByteIdx] = u4DutyDiffTemp[u1ByteIdx];
                            RdccInfo.s1BestDelay[u1ByteIdx] = s1DelayIdx;
                            RdccInfo.u4BestDUTY_P[u1ByteIdx] = u4DUTY_P[u1ByteIdx];
                            RdccInfo.u4BestDUTY_N[u1ByteIdx] = u4DUTY_N[u1ByteIdx];
                            RdccInfo.u4BestDUTY_CAL[u1ByteIdx] = u4DUTY_CAL[u1ByteIdx];
                        }
                    }
                }
            }
            mcSHOW_DBG_MSG("[%2d] B0: %d, B1: %d\n", s1DelayIdx, u1Status[BYTE_0], u1Status[BYTE_1]);
            mcSHOW_DBG_MSG("(s1BestDelay B0= %d, B1= %d, s1DelayIdx = %d)\n", RdccInfo.s1BestDelay[BYTE_0], RdccInfo.s1BestDelay[BYTE_1], s1DelayIdx);
        ...
        // Get FinalDelay from WinInfo
        GetFinalDelay_RDCC(p, &RdccInfo, &FinalDelay_Rdcc);

        // Set Final delay (DQ, DQM, DQS)
        SetDelay_RDCC(p, &FinalDelay_Rdcc);

        // Print calibration result
        PrintFinalResult_RDCC(p, &RdccInfo, &FinalDelay_Rdcc);
        ...
        {
            // Save and FinalDelay_XXX to eMMC
            p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_0]= FinalDelay_Rdcc.s1RdccStep[BYTE_0];
            p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_1] = FinalDelay_Rdcc.s1RdccStep[BYTE_1];
        }
        #endif
    }

    vSetCalibrationResult(p, DRAM_CALIBRATION_RDCC, K_result);
...
static DRAM_STATUS_T DramcRdccCal_DutyMode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdcc
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdccCal_DutyMode] Start\n");
    ...
    #endif
    {
        ...
        for(s1DelayIdx = ScanDelay_Rdcc.s2DelayStart; s1DelayIdx < ScanDelay_Rdcc.s2DelayEnd; s1DelayIdx += ScanDelay_Rdcc.u1DelayStep)
        {
            ...
            for(u1ByteIdx=0; u1ByteIdx<DQS_NUMBER; u1ByteIdx++)
            {
                ...
                if (u4DutyDiffTemp[u1ByteIdx] < u4DutyDiffFinal[u1ByteIdx])
                {
                    u4DutyDiffFinal[u1ByteIdx] = u4DutyDiffTemp[u1ByteIdx];
                    RdccInfo.s1BestDelay[u1ByteIdx] = s1DelayIdx;
                    RdccInfo.u4BestDUTY_P[u1ByteIdx] = duty_value[0];
                    RdccInfo.u4BestDUTY_N[u1ByteIdx] = duty_value[1];
                    RdccInfo.u4BestDUTY_CAL[u1ByteIdx] = final_duty[u1ByteIdx];
                }
            ...
            #if RDCC_DEBUG_LOG
            mcSHOW_DBG_MSG("[%d]B0: u4CurrentDUTY_CAL=%d||B1 u4CurrentDUTY_CAL=%d\n", s1DelayIdx, final_duty[BYTE_0], final_duty[BYTE_1]);
            mcSHOW_DBG_MSG("[%d]B0: u4BestDUTY_CAL=%d||B1 u4BestDUTY_CAL=%d\n", s1DelayIdx, RdccInfo.u4BestDUTY_CAL[BYTE_0], RdccInfo.u4BestDUTY_CAL[BYTE_1]);
            mcSHOW_DBG_MSG("[%d]B0: u4DutyDiffTemp=%d   u4DutyDiffFinal=%d ||B1 u4DutyDiffTemp=%d   u4DutyDiffFinal=%d\n", s1DelayIdx, u4DutyDiffTemp[BYTE_0], u4DutyDiffFinal[BYTE_0]	, u4DutyDiffTemp[BYTE_1], u4DutyDiffFinal[BYTE_1]);
            #endif
        ...
        // Set calibration result
        if ((RdccInfo.u4BestDUTY_CAL[BYTE_0]>=4800)&&(RdccInfo.u4BestDUTY_CAL[BYTE_0]<=5200)&&(RdccInfo.u4BestDUTY_CAL[BYTE_1]>=4800)&&(RdccInfo.u4BestDUTY_CAL[BYTE_1]<=5200))
        {
            K_result = DRAM_OK;
        }
    ...
    mcSHOW_DBG_MSG("[DramcRdccCal_DutyMode] Done\n");
...
#if RDCC_TA2_MODE
DRAM_STATUS_T DramcRdccCal_Ta2Mode(DRAMC_CTX_T *p)
{
    DRAM_STATUS_T K_result = DRAM_FAIL;
    DRAM_RANK_T backup_rank;
    //Rdcc
    ...
    // Print current basic information
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[DramcRdccCal_Ta2Mode] Start\n");
    ...
    // Choose Delay scan range
    ScanRangeDelay_RDCC(p, &ScanDelay_Rdcc);
    ScanRangeDelay_RdccTa2(p, &ScanDelay_RxTa2);
    ...
    for (s1DelayIdx = ScanDelay_Rdcc.s2DelayStart; s1DelayIdx < ScanDelay_Rdcc.s2DelayEnd; s1DelayIdx += ScanDelay_Rdcc.u1DelayStep)
    {
        ...
        mcSHOW_DBG_MSG("CH%d RK%d [%d] u2MinRxWindow: B0 %d, B1 %d, u2FinalMinRxWindow: B0 %d, B1 %d ", p->channel, p->rank, s1DelayIdx, u2MinRxWindow[BYTE_0], u2MinRxWindow[BYTE_1], u2FinalMinRxWindow[BYTE_0], u2FinalMinRxWindow[BYTE_1]);
        mcSHOW_DBG_MSG("(s1BestDelay: B0 %d, B1 %d, u1EarlyBreakCnt %d)\n", RdccInfo.s1BestDelay[BYTE_0], RdccInfo.s1BestDelay[BYTE_1], u1EarlyBreakCnt);
    ...
    #if RDCC_RK_CANT_DIFF_WA
    // DramcRdccCalPostProcess
    DramcRdccCalPostProcess(p, &FinalDelay_Rdcc);
    #endif
    ...
    mcSHOW_DBG_MSG("\n[DramcRdccCal_Ta2Mode] Done\n");

    return DRAM_OK;
}
#endif

void DramcRdccCal(DRAMC_CTX_T *p, U8 u1RdccMode)
{
    #if RDCC_JMETER_MODE
    if (u1RdccMode == 0)
        DramcRdccCal_JmeterMode(p);
    else
    #endif
    #if RDCC_DUTY_MODE
    if (u1RdccMode == 1)
        DramcRdccCal_DutyMode(p);
    else
    #endif
    #if RDCC_TA2_MODE
    if (u1RdccMode == 2)
        DramcRdccCal_Ta2Mode(p);
    else
    #endif
    {
        mcSHOW_ERR_MSG("[WARNING] DramcRdccCal mode unexpect!!\n");
        #if __ETT__
        ASSERT(0);
        #endif
    }
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
DRAM_STATUS_T FastK_DramcRdccCal(DRAMC_CTX_T *p)
{
    RDCC_DELAY_T FinalDelay_Rdcc;
    ...
    vPrintCalibrationBasicInfo(p);
    mcSHOW_DBG_MSG("[FAST_K] Bypass DramcRdccCal\n");

    // Load FinalDelay_ReadDcc from eMMC
    FinalDelay_Rdcc.s1RdccStep[BYTE_0] = p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_0];
    FinalDelay_Rdcc.s1RdccStep[BYTE_1] = p->pSavetimeData->s1RDCC[p->channel][p->rank][BYTE_1];

    // Set FinalDelay_ReadDcc
    SetDelay_RDCC(p, &FinalDelay_Rdcc);

    // Print calibration result
    PrintDelay_RDCC(p, &FinalDelay_Rdcc);

    vSetCalibrationResult(p, DRAM_CALIBRATION_RDCC, DRAM_FAST_K);
