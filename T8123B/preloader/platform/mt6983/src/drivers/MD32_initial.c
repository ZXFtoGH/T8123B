...
void MD32_MODE_choose(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG6("IPMV22 low power path setting. low power interface from MD32 FW/RG comb out. MTCMOS/SRAM from SPM \n");
    ...
    if(DUT_p.DRAMC_SHU_SRAM_MTCMOS_SEL== 0) {
	    mcSHOW_DBG_MSG6("IPMV24 DRAMC MTCMOS/SRAM/ setting. DRAMC MTCMOS/SRAM from DPM FW out. MPLL controlled by SPM\n");
        ...
        mcSHOW_DBG_MSG6("IPMV24 DRAMC MTCMOS/SRAM/MPLL setting. DRAMC MTCMOS/SRAM/MPLL from DPM FW out.\n");
...
{
    mcSHOW_DBG_MSG6("[MD32_INIT] in c code >>>>>> \n");
...
//  1.  enale output enable
    mcSHOW_DBG_MSG6("[MD32_INIT] V22 add 1 \n");
...
//!      p_sequencer.md32_regmodel[0].DDRPHY_MD32_blk.LPIF_FSM_CTRL_0.lp_intf_out_enable.write(status,0);
    ...
    mcSHOW_DBG_MSG6("[MD32_INIT] V22 add 1 end \n");
...
//!      p_sequencer.md32_regmodel[0].DDRPHY_MD32_blk.SSPM_INTC_IRQ_GRP2_0.IRQ_GRP2_0.write(status,32'h00020000);
    ...
    mcSHOW_DBG_MSG6("[MD32_INIT] set single top \n");
    ...
    mcSHOW_DBG_MSG6("[MD32_INIT] set dual top \n");
	...
	//set dpm sema priority for SW ZQ
	#if (fcFOR_CHIP_ID == fcLepin)
	//CHA slave
	channel_backup_and_set(p, CHANNEL_A);
	vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_SEMA_2MCU_HOR,
		P_Fld(0xF, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DELAY_CNT_0) |
		P_Fld(0, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DPM_HGIH_PRI));
	channel_restore(p);
	//CHC master
	channel_backup_and_set(p, CHANNEL_B);
	vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_SEMA_2MCU_HOR,
		P_Fld(0xF, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DELAY_CNT_0) |
		P_Fld(1, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DPM_HGIH_PRI));
	channel_restore(p);


	//CHB master
	channel_backup_and_set(p, CHANNEL_C);
	vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_SEMA_2MCU_HOR,
		P_Fld(0xF, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DELAY_CNT_0) |
		P_Fld(1, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DPM_HGIH_PRI));
	channel_restore(p);
	//CHD slave
	channel_backup_and_set(p, CHANNEL_D);
	vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_SEMA_2MCU_HOR,
		P_Fld(0xF, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DELAY_CNT_0) |
		P_Fld(0, LPIF_SEMA_2MCU_HOR_SEMA_HOR_DPM_HGIH_PRI));
	channel_restore(p);
...
//!      p_sequencer.md32_regmodel[0].DDRPHY_MD32_blk.SSPM_DCM_CTRL.R_DCM_EN.write(status,12'h10);
    ...
    //  `uvm_info(get_type_name(),$psprintf("MD32 dvfs RTMRW enable "), UVM_LOW) 
    //end
    mcSHOW_DBG_MSG6("[MD32_INIT] 30 for RTMRW, if have \n");
    ...
    mcSHOW_DBG_MSG6("[MD32_INIT] 31 for DRM, if have \n");
    if(DUT_p.DDR_RESERVE_EN ==1) {
        mcSHOW_DBG_MSG6("[MD32_INIT] 31.1: DRM enable \n");
...
#if FOR_DV_SIMULATION_USED
        mcSHOW_DBG_MSG6("[MD32_INIT] 31.2: Set IRQ 5\n");
//      vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_MD32_REG_SSPM_INTC_IRQ_POL1), 0x2, SSPM_INTC_IRQ_POL1_IRQ_POL1);                                 //DDRPHY_MD32_BASE_ADDRESS + 0x42044
        ...
        if(DUT_p.SINGLE_TOP == 1) { // fcLePin, fcFigeac
            ...
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DDR_RESERVE3), 0x8004, MISC_DDR_RESERVE3_DRM_SM_PASS_CTRL); // for recovery mode wo dpm (wo DPM ack and DMA)
...
void MD32_initializaton(DRAMC_CTX_T *p)
{
    ...
    for (ch_id = CHANNEL_A;ch_id < ch_id_max;ch_id++) {
      mcSHOW_DBG_MSG6("[MD32_INIT] %s Top CH%c >>>>>> \n", (p->u1SingleTop) ? "Single" : "Dual", 'A' + ch_id);
...
{
    mcSHOW_DBG_MSG6("[MD32_CFG] MD32_config_ch >>>>>> \n");
    vSetPHY2ChannelMapping(p, ch_id);
    //step1. release SW RST
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_MD32_REG_SSPM_CFGREG_SW_RSTN), 0X00000001 );
    ...
    mcSHOW_DBG_MSG6("[MD32_CFG] DPM DCM  setting\n");
    //5.1 choose source
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_MD32_REG_SSPM_MCLK_DIV),0,SSPM_MCLK_DIV_MCLK_SRC   );
...
void MD32_config(DRAMC_CTX_T *p)
{
    ...
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    for (ch_id = CHANNEL_A;ch_id < ch_id_max;ch_id++) {
      mcSHOW_DBG_MSG6("[MD32_CFG] %s Top CH%c >>>>>> \n", (p->u1SingleTop) ? "Single" : "Dual", 'A' + ch_id);
