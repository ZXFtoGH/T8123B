...
#define vproc_ll_name           "vcpul"          /*vbuck name for LL*/
#define vsram_ll_name           "vsram_cpul"     /*vbuck name for LL*/
#define PMIC_DEBOUNCE_TIME_US        40	         /*By PMIC HW*/
#define LOW_BATTERY_THRE           3600          /*PMIC Define(uV)*/

/* base on DVFS table */
#define cpu_ll_freq             1100000          /*Unit: kHz*/
#define cci_freq                 910000          /*Unit: kHz*/
#define vproc_set_vol            875000          /*Unit: uV*/
#define vsram_set_vol            875000          /*Unit: uV*/
/**********************************************************/
bool is_Battery_low(void)
{
	int bat_val = 0;

	pmic_auxadc_read_value_ext(AUXADC_CHAN_BATADC, &bat_val,
...
void armpll_ll_boost()
{
...
#if !defined(SLT) //no battery in SLT EVB
	if (is_Battery_low())
		return;
#endif
	...
	set_armpll_ll_rate(cpu_ll_freq);
	set_ccipll_rate(cci_freq);
#if DEBUG_SUPPORT
	/*Print LL CPU Freq.*/
	mt_get_cpu_freq();
#endif
