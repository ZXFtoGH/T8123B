...
#define MT6368_DEBUG 1

#if MT6368_DEBUG
#define PMUTAG                "[MT6368] "
...
#define MT6368_LDO_VOCAL(_name, _volt_table, _enable_reg, en_bit,	\
			 _vsel_reg, _vsel_mask, _vocal_reg,		\
...
#define MT6368_REG_FIXED(_name, _enable_reg, en_bit,		\
...
static struct mt6368_regulator_info mt6368_regulators[] = {
	MT6368_BUCK(VBUCK1, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK1_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VBUCK2, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK2_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VBUCK3, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK3_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VBUCK4, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK4_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VBUCK5, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK5_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VBUCK6, 0, 1193750, 6250, mt_volt_range0,
		    ...
		    MT6368_PMIC_BUCK_VBUCK6_WDTDBG_VOSEL_ADDR),
	MT6368_BUCK(VPA, 500000, 3600000, 50000, mt_volt_range1,
		    ...
		    MT6368_PMIC_BUCK_VPA_WDTDBG_VOSEL_ADDR),
	MT6368_LDO(VSIM1, ldo_volt_table0,
		   MT6368_PMIC_RG_LDO_VSIM1_EN_ADDR, MT6368_PMIC_RG_LDO_VSIM1_EN_SHIFT,
		   MT6368_PMIC_RG_VSIM1_VOSEL_ADDR,
		   MT6368_PMIC_RG_VSIM1_VOSEL_MASK,
		   MT6368_PMIC_RG_VSIM1_VOCAL_ADDR,
		   MT6368_PMIC_RG_VSIM1_VOCAL_MASK,
		   MT6368_PMIC_RG_LDO_VSIM1_LP_ADDR,
		   MT6368_PMIC_RG_LDO_VSIM1_LP_SHIFT),
	MT6368_LDO(VSIM2, ldo_volt_table0,
	...
	MT6368_LDO_VOCAL(VMDDR, ldo_volt_table1,
			 MT6368_PMIC_RG_LDO_VMDDR_EN_ADDR, MT6368_PMIC_RG_LDO_VMDDR_EN_SHIFT,
			 MT6368_PMIC_RG_VMDDR_VOSEL_0_ADDR,
			 MT6368_PMIC_RG_VMDDR_VOSEL_0_MASK,
			 MT6368_PMIC_RG_VMDDR_VOCAL_0_ADDR,
			 MT6368_PMIC_RG_VMDDR_VOCAL_0_MASK, 5000,
			 MT6368_PMIC_RG_LDO_VMDDR_LP_ADDR,
			 MT6368_PMIC_RG_LDO_VMDDR_LP_SHIFT),
	MT6368_LDO_VOCAL(VMDDQ, ldo_volt_table1,
			 MT6368_PMIC_RG_LDO_VMDDQ_EN_ADDR, MT6368_PMIC_RG_LDO_VMDDQ_EN_SHIFT,
			 MT6368_PMIC_RG_VMDDQ_VOSEL_0_ADDR,
			 MT6368_PMIC_RG_VMDDQ_VOSEL_0_MASK,
			 MT6368_PMIC_RG_VMDDQ_VOCAL_0_ADDR,
			 MT6368_PMIC_RG_VMDDQ_VOCAL_0_MASK, 5000,
			 MT6368_PMIC_RG_LDO_VMDDQ_LP_ADDR,
			 MT6368_PMIC_RG_LDO_VMDDQ_LP_SHIFT),
	MT6368_LDO(VUSB, ldo_volt_table0,
	...
	MT6368_LDO(VIBR, ldo_volt_table0,
	...
	MT6368_LDO(VIO28, ldo_volt_table0,
	...
	MT6368_LDO(VFP, ldo_volt_table0,
	...
	MT6368_LDO(VTP, ldo_volt_table0,
	...
	MT6368_LDO(VMC, ldo_volt_table0,
	...
	MT6368_LDO(VEFUSE, ldo_volt_table0,
...
static struct mt6368_regulator_info *mt6368_find_info(unsigned char id)
{
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(mt6368_regulators); i++) {
		if(mt6368_regulators[i].desc.id == id)
			return &mt6368_regulators[i];
	}
	return NULL;
}

static int mt6368_set_voltage(unsigned char id, unsigned int volt, unsigned int max_volt)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	u32 vocal_max;
...
static int mt6368_get_voltage(unsigned char id)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	u32 vocal_max;
...
static int mt6368_enable(unsigned char id, unsigned char en)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
...
static int mt6368_is_enabled(unsigned char id)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	u8 en = 0;
...
static int mt6368_set_mode(unsigned char id, unsigned char mode)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	int ret = 0;
...
static int mt6368_get_mode(unsigned char id)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	u8 mode = 0;
...
static int mt6368_vmdd_set_voltage(unsigned char id, unsigned int volt, unsigned int max_volt)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	...
	if (!info)
		return -ENODEV;
	desc = &info->desc;
	if (volt < desc->min_uV || (desc->max_uV && volt > desc->max_uV + 100000)) {
		mreg_dbg_print("vp\n");
		return -EINVAL;
	}
	...
	if (desc->volt_table) {
		for (i = 0; i < desc->n_voltages; i++) {
			if (volt > desc->volt_table[i] + 100000)
				continue;
			if (volt == desc->volt_table[i]) {
				selector = i;
				break;
			...
			if (volt > desc->volt_table[i])
				vocal = volt - desc->volt_table[i];
			selector = i;
		}
		if (vocal && info->vocal_reg) {
			if (vm_mode == 0x1)
				ret = second_pmic_config_interface(info->vocal_reg + 1, vocal,
								   info->vocal_mask, 0);
			else
...
static int mt6368_vmdd_get_voltage(unsigned char id)
{
	struct mt6368_regulator_info *info = mt6368_find_info(id);
	struct regulator_desc *desc;
	u8 selector = 0, vocal = 0, vm_mode = 0;
	...
	if (desc->volt_table) {
		...
		volt = desc->volt_table[selector] + vocal * 100000;
	}
#else
	return -EINVAL;
#endif /*--LDO_SUPPORT--*/
...
static struct regulator_ctrl mt6368_regulator_ctrl = {
	.set_voltage = mt6368_set_voltage,
	.get_voltage = mt6368_get_voltage,
	.enable = mt6368_enable,
	.is_enabled = mt6368_is_enabled,
	.set_mode = mt6368_set_mode,
	.get_mode = mt6368_get_mode,
};

/* for VMDDR and VMDDQ */
static struct regulator_ctrl mt6368_vmdd_regulator_ctrl = {
	.set_voltage = mt6368_vmdd_set_voltage,
	.get_voltage = mt6368_vmdd_get_voltage,
	.enable = mt6368_enable,
	.is_enabled = mt6368_is_enabled,
	.set_mode = mt6368_set_mode,
	.get_mode = mt6368_get_mode,
...
static struct mtk_regulator mt6368_mreg[] = {
	{
		.name = "vbuck1",
		.id = MT6368_ID_VBUCK1,
		.reg_ops = &mt6368_regulator_ctrl,
	}, {
		.name = "vbuck2",
		.id = MT6368_ID_VBUCK2,
		.reg_ops = &mt6368_regulator_ctrl,
		...
		.name = "vmddr",
		.id = MT6368_ID_VMDDR,
		.reg_ops = &mt6368_vmdd_regulator_ctrl,
	}, {
		.name = "vmddq",
		.id = MT6368_ID_VMDDQ,
		.reg_ops = &mt6368_vmdd_regulator_ctrl,
		...
		.name = "vrf13_aif",
		.id = MT6368_ID_VRF13_AIF,
		.reg_ops = &mt6368_regulator_ctrl,
	}, {
		.name = "vrf18_aif",
		.id = MT6368_ID_VRF18_AIF,
		.reg_ops = &mt6368_regulator_ctrl,
	}, {
		.name = "vant18",
		.id = MT6368_ID_VANT18,
		...
		.name = "vcn33_1",
		.id = MT6368_ID_VCN33_1,
		.reg_ops = &mt6368_regulator_ctrl,
	}, {
		.name = "vcn33_2",
		.id = MT6368_ID_VCN33_2,
		.reg_ops = &mt6368_regulator_ctrl,
	}, {
		.name = "vefuse",
		.id = MT6368_ID_VEFUSE,
 ...
 * ====================
 */
int mt6368_regulator_probe(void)
{
	int ret = 0;
	unsigned int i = 0;

	for (i = 0; i < ARRAY_SIZE(mt6368_mreg); i++) {
		ret = mtk_simple_regulator_register(&mt6368_mreg[i]);
		if (ret < 0) {
...
void mt6368_wdtdbg_vosel(void)
{
	u8 i, val = 0;
	struct regulator_desc *desc;

	for (i = 0; i < ARRAY_SIZE(mt6368_regulators); i++) {
		desc = &mt6368_regulators[i].desc;
