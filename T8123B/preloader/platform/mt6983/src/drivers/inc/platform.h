...
#define BL_PARAM_SIZE    (2400)
#define BL_PARAM_VERSION (6)
typedef struct {
    u32 ver;
	...
	u32 br_logs_addr;
	u32 br_logs_len;
	u16 sctrl_cert_status;
	u16 tool_auth_status;
	u32 sctrl_cert_sw_version;
	u32 reserved0[6];
	u8  socid[SOC_ID_LEN];
...
/* boot tag version */
#define BOOT_TAG_VERSION     0x88610000
struct boot_tag_version {
    u32 version;
};
...
struct boot_tag_boot_time {
    u32 boot_time;
    u32 bl2_ext_boot_time;
    u32 logo_time;
    u32 tfa_boot_time;
    u32 sec_os_boot_time;
    u32 gz_boot_time;
};
...
/* TODO: remove in next migration, is redundant for new chip */
...
#define BOOT_TAG_AEE_EXPCEPTION_INFO 0x8861001D
struct boot_tag_aee_exception_info {
    u8 exp_type;
    u32 wdt_status;
};
...
#define BOOT_TAG_RKP_PARAM (0x88610028)
#endif

/* apusys status */
#define BOOT_TAG_APUSYS_INFO     0x88610027
struct boot_tag_apusys_info {
    u32 apusys_status;
};
...
struct boot_tag_masp_data {
    ...
    unsigned int sbc_pubk_hash[8];
};
#endif

#define BOOT_TAG_ROM_INFO (0x88610035)
struct boot_tag_rom_info {
    u8   m_identifier[16];
    u32  m_rom_info_ver;
    u8   m_platform_id[16];
    u8   m_project_id[16];
    u8   m_secctrl_identifier[16];
    u32  m_sec_cfg_ver;
    u32  m_sec_usb_dl;
    u32  m_sec_boot;
    u8   m_seccfg_ac_en;
    u8   reserve[3];
    u8   m_key_identifier[16];
    u32  m_sec_key_ver;
    u8   crypto_seed[16];
};
...
typedef struct {
    ...
    union {
        struct boot_tag_version version;
        ...
        struct boot_tag_aee_exception_info aee_exception_info;
#if CFG_GZ_SUPPORT
...
#endif
        struct boot_tag_apusys_info apusys_info;
#if defined(CFG_RKP_SUPPORT) && CFG_RKP_SUPPORT
        ...
        struct boot_tag_masp_data masp_info;
#endif
        struct boot_tag_dlpt_info dlpt_info;
        struct boot_tag_meid_info meid_info;
        struct boot_tag_rom_info rom_info;
    } u;
}boot_tag;
...
#define boot_tag_size(type)	ALIGN((sizeof(struct boot_tag_header) + sizeof(struct type)), 8)
...
extern void platform_init_boot_time(void);
extern void platform_set_boot_time(void);
...
extern void jump_to_bl2_ext(u32 jump_addr, mtk_bl_param_t *bl31_info, u32 reserved1, u32 reserved2, u32 reserved3);
...
extern apusys_status_t g_apusys_status;
