...
U16 spare_pwkey, spare_pwkey_lpd, hwid;
...
static bool Writeif_unlock(void)
{
	...
	port_sta = RTC_Read(RTC_SPAR_MACRO) >> RTC_SPAR_PROT_STAT_SHIFT & RTC_SPAR_PROT_STAT_MASK;
	if(port_sta != RTC_PROT_UNLOCK_SUCCESS) {
		RTC_INFO("protect key unmatch\n");
		return false;
	}
...
static bool rtc_android_init(void)
{
	...
	/*When mt6685 E3 2sec reboot, don't need to set default time.*/
	if(!(spare_pwkey_lpd == 1 && spare_pwkey == 1 && hwid >= 0x30)){
...
void rtc_init(void)
{
	...
	/*for mt6685 E3 time counters setting*/
	spare_pwkey_lpd = (RTC_Read(RTC_SPAR_MACRO) >> RTC_SPAR_PWRKEY_MATCH_LPD_SHIFT) & 0x1;
	spare_pwkey = (RTC_Read(RTC_SPAR_MACRO) >> RTC_SPAR_PWRKEY_MATCH_SHIFT) & 0x1;
	/*E2 hwid == 8'h20, E3 hwid == 8'h30*/
	hwid = rtc_config_interface_read(HWCID_L);
	...
	rtc_config_interface(TMA_KEY, 0x7a, TMA_KEY_MASK, TMA_KEY_SHIFT);
	rtc_config_interface(TMA_KEY_H , 0x99, TMA_KEY_H_MASK, TMA_KEY_H_SHIFT);

	/*set_32K1V8_0_driving 8mA*/
	rtc_config_interface(RG_OCT1_RTC32K_1V8_0, 1, RG_OCT1_RTC32K_1V8_0_MASK, RG_OCT1_RTC32K_1V8_0_SHIFT);
	...
	RTC_Write(RTC_IRQ_EN, RTC_Read(RTC_IRQ_EN) & ~RTC_IRQ_EN_ONESHOT);
	Write_trigger();
	RTC_INFO("check RTC_IRQ_EN = 0x%x\n", RTC_Read(RTC_IRQ_EN));
