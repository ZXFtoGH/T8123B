...
extern const u8 usb_string_num;
 ...
 */
static void copy_desc(struct urb *urb, void *data, unsigned int max_length,
					   int max_buf);
...
static void copy_desc(struct urb *p_urb, void *p_data, unsigned int max_length, int max_buf)
{
	int available;
	unsigned int length;
	...
	if ((signed long) max_buf > (signed long) p_urb->buffer_length)
		max_buf = (int) p_urb->buffer_length;

	if ((available = (int)((signed long) max_buf - (signed long) p_urb->actual_length)) <= 0) {
		return;
	}

	if (length > (unsigned int) available) {
		length = (unsigned int) available;
	}
...
static int ep0_standard_setup(struct urb *p_urb)
{
	...
	if ((p_req->bmRequestType & USB_DIR_MASK) == USB_DIR_IN) {
		...
		case STDREQ_SET_ADDRESS:
			...
			p_dev->address = (p_req->wValue) & 0x7f;
...
int mu3d_hal_pio_read_fifo(int ep_num,u8 *p_buf)
{
	...
	if (residue > URB_BUF_SIZE)
		residue = count = URB_BUF_SIZE;
...
static void udc_stall_ep(unsigned int ep_num, u8 dir)
{
	...
	if (ep_num == 0) {
		...
		if (endpoint && GET_EP_DIR(endpoint->endpoint_address) == USB_DIR_OUT) {
...
void mt_setup_ep(struct mt_dev *device, unsigned int ep_num, struct mt_ep *p_ep)
{
	...
	else //(GET_EP_DIR(endpoint->endpoint_address) == USB_DIR_IN)
	{
		...
		if (type == USB_EP_XFER_BULK) {
			...
			USB_LOG("ep %d is int", ep_num);
			csr1 |= TYPE_INT;
			csr2 |= ((u32)p_ep->binterval << TXBINTERVAL_OFST) & TXBINTERVAL;
		}
		/* Write 1 to clear EPIER */
