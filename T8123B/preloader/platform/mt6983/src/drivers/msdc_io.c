...
static void msdc_clr_gpio(u32 bits)
{
	...
	//msdc_pr_info("====%s====%d\n", name, l_val);
	if (0 == l_val) {
		msdc_pr_info("check me! gpio for %s is input, GPIO_EN = 0x%x\n", name,PWR_GPIO_EO);
...
{
	MSG(CFG, "[jyj_SD%d] Turn %s host power \n", host->id, on ? "on" : "off");
...
#ifdef MSDC1_EMMC
		case 1:
			MSG(CFG, "[jyj_SD%d] FPGA_MSDC1 host power level = %d \n", host->id, level);
			if (on) {
				if (level == VOL_1800)
					msdc_set_gpio(PWR_MASK_VOL_18);
				else
...
void msdc_dump_padctl_by_id(u32 id)
{
	...
	if (id == 0) {
		...
		msdc_pr_err("MSDC0 TDSEL0[%x] = 0x%X\tshould: 0x00000???\n",
...
void msdc_apply_timing(unsigned int id)
{
	...
	if ((id == 0) && (mode == 3)) {
		...
		MSDC_SET_FIELD(host->base + 0xb8, (0x7U << 29), 0);      /* MSDC_PB2_CRCSTSENSEL*/
		...
		MSDC_SET_FIELD(host->base + 0xb8, (0x7U << 29), 0);
		MSDC_SET_FIELD(host->base + 0xb8, (0x7 << 16), 0);
		//MSDC_SET_FIELD(host->base + 0xb8, (0x7U << 29), AUTOK_CRC_LATCH_EN_HS200_PORT0_VALUE);
		...
		MSDC_SET_FIELD(host->base + 0xb8, (0x7U << 29), AUTOK_CRC_LATCH_EN_HS_VALUE);
		...
		MSDC_SET_FIELD(host->base + 0xb8, (0x7U << 29), 1);
