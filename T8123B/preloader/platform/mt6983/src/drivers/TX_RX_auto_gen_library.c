...
U8 get_wck_p2s_latency(U8 p2s_ratio, U8 frate)
{
  ...
  if (((p2s_ratio == 2) & (frate == 0)) | (frate > 1)) {
    mcSHOW_ERR_MSG("Error: when p2s_ratio is 2, frate should be 1. But p2s_ratio is %0d, frate is %0d \n", p2s_ratio, frate);
  }
...
U8 get_dq_ca_p2s_latency(U8 p2s_ratio, U8 frate, U8 fake_mode)
{
  ...
  //case 2 : {(DUT_p.DEF_DIGPHY==1) ? (ser_latency = 1.5 ) : (ser_latency = 1.5 ); break;}
  case 2 : {(DUT_p.DEF_DIGPHY==1) ? (ser_latency = 1 ) : (ser_latency = 1 ); break;}
    default : mcSHOW_ERR_MSG("Error: %0d is unexpected p2s ratio in get_dq_ca_p2s_latency \n", p2s_ratio);
  }
  ...
  switch (p2s_ratio)
  {
    ...
    default : mcSHOW_ERR_MSG("Error: %0d is unexpected p2s ratio in get_dqsien_p2s_latency \n", p2s_ratio);
  }
#endif
  return ser_latency;
}
