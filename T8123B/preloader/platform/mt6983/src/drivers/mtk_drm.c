...
int drm_latch_en(int enable)
{
	...
	/*
	 * DRM latch will be triggered when normal/abnormal reset.
	 * RGU will disable DRM latch en before reset for normal reboot.
	*/

	if (enable == 1) {
		/* enable DRM latch */
		tmp = READ_REG(MTK_DRM_LATCH_CTL);
		tmp |= (MTK_DRM_LATCH_EN|MTK_DRM_LATCH_CTL_KEY);
		WRITE_REG(tmp, MTK_DRM_LATCH_CTL);
	} else if (enable == 0) {
		/*disable DRM latch */
		tmp = READ_REG(MTK_DRM_LATCH_CTL);
		tmp &= (~MTK_DRM_LATCH_EN);
		tmp |= MTK_DRM_LATCH_CTL_KEY;
		WRITE_REG(tmp, MTK_DRM_LATCH_CTL);
	} else {
...
void drm_dump_rg_for_debug(void)
{
        pal_log_info("[dbgtop_drm] DRM_MODE : 0x%x\n", READ_REG(MTK_DRM_MODE));
        pal_log_info("[dbgtop_drm] DRM_DEBUG_CTL : 0x%x\n", READ_REG(MTK_DRM_DEBUG_CTL));
        pal_log_info("[dbgtop_drm] DRM_DEBUG_CTL2 : 0x%x\n", READ_REG(MTK_DRM_DEBUG_CTL2));
        pal_log_info("[dbgtop_drm] DRM_DEBUG_CTL3 : 0x%x\n", READ_REG(MTK_DRM_DEBUG_CTL3));
        pal_log_info("[dbgtop_drm] DRM_LATCH_CTL : 0x%x\n", READ_REG(MTK_DRM_LATCH_CTL));
        pal_log_info("[dbgtop_drm] DRM_LATCH_CTL2 : 0x%x\n", READ_REG(MTK_DRM_LATCH_CTL2));
        pal_log_info("[dbgtop_drm] DRM_LATCH_CTL3 : 0x%x\n", READ_REG(MTK_DRM_LATCH_CTL3));
        pal_log_info("[dbgtop_drm] DRM_LATCH_CTL4 : 0x%x\n", READ_REG(MTK_DRM_LATCH_CTL4));
        pal_log_info("[dbgtop_drm] DRM_LATCH_CTL5 : 0x%x\n", READ_REG(MTK_DRM_LATCH_CTL5));
}
...
void mtk_drm_init(void)
{
	...
	latch_ctrl |= MTK_DRM_DRAMC_LATCH_EN;
        latch_ctrl |= MTK_DRM_GPU_LATCH_EN;
...
void drm_print_timeout(void)
{
	pal_log_err("%s DRM_LATCH_CTL2: 0x%x\n", __func__, READ_REG(MTK_DRM_LATCH_CTL2));
	pal_log_err("%s WDT_LATCH_CTL2: 0x%x\n", __func__, READ_REG(MTK_WDT_LATCH_CTL2));
}
