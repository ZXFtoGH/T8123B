...
static const u8 ldo6_ctrl_mask[] = { 0xff, 0x8f, 0x3f, 0xfe, 0xff };
static const u8 ldo7_ctrl_mask[] = { 0xff, 0xff, 0x3f, 0xfe, 0xff };
...
static const u8 ldo6_ctrl_init[] = { 0x00, 0x80, 0x02, 0xec, 0x44 };
static const u8 ldo7_ctrl_init[] = { 0x00, 0xe0, 0x02, 0xec, 0x44 };
#if 0
...
static int mt6360_set_buck1_fsw(void)
{
	...
	/* 2.72 MHz*/
	ret = mt6360_write_byte(0x1e, 0x8e);
	if (ret < 0) {
		dbg_print("%s: fail\n",  __func__);
		goto out;
	}
...
static int mt6360_pmic_init(void)
{
	...
	/* ldo init */
	for (i = 0; i < ARRAY_SIZE(ldo6_ctrl_mask); i++) {
		ret = mt6360_update_bits(0x30 + i, ldo7_ctrl_mask[i],
					 ldo7_ctrl_init[i]);
		if (ret < 0) {
			dbg_print("%s: reg 0x%x set fail\n",
				  __func__, (0x30 + i));
		...
		ret = mt6360_update_bits(0x36 + i, ldo6_ctrl_mask[i],
					 ldo6_ctrl_init[i]);
		if (ret < 0) {
			dbg_print("%s: reg 0x%x set fail\n",
				  __func__, (0x36 + i));
...
{
	int i, ret = 0;
	u8 seq_delay[4] = { 0x04, 0x00, 0x02, 0x02 };
