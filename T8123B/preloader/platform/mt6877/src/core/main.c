...
static int bldr_load_images(u32 *jump_addr)
{
    ...
    *jump_addr = addr;
#endif //MTK_AB_OTA_UPDATER
    BOOTING_TIME_PROFILING_LOG("load lk");
#endif //CFG_LOAD_UBOOT
    ...
    pal_log_info("%s bldr load tee part ret=0x%x, addr=0x%x\n", MOD, ret, addr);
#endif //MTK_AB_OTA_UPDATER
    BOOTING_TIME_PROFILING_LOG("load tee");
#endif
...
#if defined(SLT) || defined(DVT_A64) || defined(DVT)
