...
void fuel_gauge_init(void)
{
	...
	/* read rtc valid status */
	spare3_reg = rtc_get_spare_fg_value();
	...
	if (fg_reset_status == 0 || rtc_invalid == 1)
	...
	pal_log_err(
		...
		shutdown_pmic_time %d hw_id 0x%x sw_id 0x%x, %d %d 0x%x 0x%x %d %d efuse_cal %d,rgspare:[%d %d],rtc[%d %d %d]\n",
			...
			spare3_reg, spare3_reg_valid, rtc_invalid);
	is_fg_init = true;
