...
void DRAMC_HW_FUNC_REF_CTRL_T_config(DRAMC_HW_FUNC_REF_CTRL_T * tr)
{
    tr->TPBR2PBR_OPT = 0; // @Darren from 1 to 0, To aviod DV sim fail
    tr->PBREF_MODE   = 1; // hybrid mode
}

void DIG_HW_NONSHUF_REF_CFG(DRAMC_CTX_T *p,U8 ch_id)
{
    int TPBR2PBR_OPT = 1; // @Darren from 1 to 0, To aviod DV sim fail
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_REFCTRL1)       , P_Fld(   1             , REFCTRL1_PB2AB_OPT                   )
                                                                ...
                                                                | P_Fld(   TPBR2PBR_OPT  , REFCTRL1_REF_OVERHEAD_PBR2PB_ENA     )
                                                                | P_Fld(   0             , REFCTRL1_REFPENDINGINT_OPT1          )
                                                                | P_Fld(   1             , REFCTRL1_REFPB2AB_IGIMP              ));


    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MISCTL0)        , P_Fld(   TPBR2PBR_OPT  , MISCTL0_REFP_ARBMASK_PBR2PBR_ENA     ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SCHEDULER_COM)  , P_Fld(   TPBR2PBR_OPT  , SCHEDULER_COM_PBR2PBR_OPT            ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_REFPEND2)       , P_Fld(   (LPDDR5_EN_S==1)?0x5 :0x8 , REFPEND2_MPENDREFCNT_TH8  ) 
...
{
    U8 irank, backup_rank;;
    mcSHOW_DBG_MSG(("[DIG_HW_SHU_REF_CFG] Group:%1d,Configuration Enter\n", group_id));
    vSetPHY2ChannelMapping(p, ch_id);
    ...
    {
        vSetRank(p, irank);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_REF_CTRL), P_Fld(    1    , SHURK_REF_CTRL_PBREFEN )); //PBREFEN enable need furthter control -TODO
    }
    vSetRank(p, backup_rank);

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHU_REF0      ), P_Fld(    4    , SHU_REF0_MPENDREF_CNT      ));
    ...
    switch(HW_REF_CTRL.PBREF_MODE)
    {
        ...
        default: mcSHOW_DBG_MSG(("[DIG_HW_SHU_REF_CFG] FATAL ERROR: Unexpected HW_REF_CTRL.PBREF_MODE=%1d input, group_id=%2d, \n",HW_REF_CTRL.PBREF_MODE, group_id));
    }
    //----------------------------------------------------------------------------------
    vSetPHY2ChannelMapping(p, 0);
    p->ShuRGAccessIdx = DRAM_DFS_REG_SHU0;
    mcSHOW_DBG_MSG(("[DIG_HW_SHU_REF_CFG] Group:%1d,Configuration Exit\n", group_id));
}

void DIG_HW_NONSHUF_REF_SWITCH(DRAMC_CTX_T *p,U8 ch_id,U8 EN)
{
    vSetPHY2ChannelMapping(p, ch_id);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_REF_CTRL), P_Fld((EN==1)?0:1, RK_REF_CTRL_REFDIS));
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHU_REF_SWITCH] :%s \n", EN==ENABLE?"ENABLE":"DISBALE"));
    #if SA_CONFIG_EN
    if(EN == 0) {
        mcSHOW_DBG_MSG(("[DIG_HW_NONSHU_REF_SWITCH]\n"));
        mcSHOW_DBG_MSG(("######   #######  #######  ######   #######   #####   #     #  \n"));
        mcSHOW_DBG_MSG(("#     #  #        #        #     #  #        #     #  #     #  \n"));
        mcSHOW_DBG_MSG(("#     #  #        #        #     #  #        #        #     #  \n"));
        mcSHOW_DBG_MSG(("######   #####    #####    ######   #####     #####   #######  \n"));
...
{
    mcSHOW_DBG_MSG(("[GROUP:%1d][DIG_HW_SHUF_REF_SWITCH] ,%s \n",group_id, EN==ENABLE?"ENABLE":"DISBALE"));
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;
    //----------------------------------------------------------------------------------
...
{
    tr->REFR_CNT            = 0X80;
    tr->REF_DERATING_EN     = 0x0; //(LPDDR5_EN_S == 1)? 0XA000 :0X00C0; // @Darren, it will enable during actiming flow
    tr->REFRATE_DEBOUNCE_TH = (LPDDR5_EN_S == 1)? 0X0C :0X05; // @Derping, sync MP settings
...
{
    mcSHOW_DBG_MSG(("[DIG_HW_NOSHU_REFR_CFG] CH:%0d,Configuration Enter\n",ch_id));
    vSetPHY2ChannelMapping(p, ch_id);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SWCMD_CTRL0        ), P_Fld(   0                           , SWCMD_CTRL0_MRRRK           ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_HMR4               ), P_Fld(   0                           , HMR4_SPDR_MR4_OPT           )
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_REFCTRL3           ), P_Fld( HW_REFR_CTRL.REF_DERATING_EN       , REFCTRL3_REF_DERATING_EN       ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_REF_BOUNCE1        ), P_Fld( HW_REFR_CTRL.REFRATE_DEBOUNCE_TH   , REF_BOUNCE1_REFRATE_DEBOUNCE_TH));
    vSetPHY2ChannelMapping(p, 0);
    mcSHOW_DBG_MSG(("[DIG_HW_NOSHU_REFR_CFG] CH:%0d,Configuration Exit\n",ch_id));
}

//>>>>
void DIG_HW_SHUF_REFR_CFG(DRAMC_CTX_T *p,U8 ch_id,U8 group_id)
{
    mcSHOW_DBG_MSG(("[DIG_HW_SHUF_REFR_CFG] Group:%1d,Configuration Enter\n", group_id));
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;

    //----------------------------------------------------------------------------------
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHU_HMR4_DVFS_CTRL0), P_Fld(HW_REFR_CTRL.REFR_CNT     , SHU_HMR4_DVFS_CTRL0_REFRCNT));


    //----------------------------------------------------------------------------------
    vSetPHY2ChannelMapping(p, 0);
    p->ShuRGAccessIdx = DRAM_DFS_REG_SHU0;
    mcSHOW_DBG_MSG(("[DIG_HW_SHU_REFR_CFG] Group:%1d,Configuration Exit\n", group_id));
}
...
{
    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHU_REFR_SWITCH] :%s \n", EN==ENABLE?"ENABLE":"DISBALE"));
    //----------------------------------------------------------------------------------
...
{
    mcSHOW_DBG_MSG(("[GROUP:%1d][DIG_HW_SHU_REFR_SWITCH] ,%s \n",group_id, EN==ENABLE?"ENABLE":"DISBALE"));
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;
    //----------------------------------------------------------------------------------
...
{
  mcSHOW_DBG_MSG (("=========================================== \n"));
  mcSHOW_DBG_MSG (("HW_ZQCAL_config\n"               ));
  mcSHOW_DBG_MSG (("=========================================== \n"));
  mcSHOW_DBG_MSG (("ZQCALL              is %d\n", tr->ZQCALL    ));
  mcSHOW_DBG_MSG (("TZQLAT              is %d\n", tr->TZQLAT    ));
  mcSHOW_DBG_MSG (("ZQCSDUAL            is %d\n", tr->ZQCSDUAL  ));
  mcSHOW_DBG_MSG (("ZQCSCNT             is %d\n", tr->ZQCSCNT   ));
  mcSHOW_DBG_MSG (("=========================================== \n"));
}
...
void DIG_HW_NONSHUF_ZQCAL_CFG(DRAMC_CTX_T *p,U8 ch_id)
{
    ...
    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_ZQCAL_CFG],Configuration Enter\n"));
...
void DIG_HW_SHUF_ZQCAL_CFG(DRAMC_CTX_T *p,U8 ch_id,U8 group_id)
{
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;
    mcSHOW_DBG_MSG(("[DIG_HW_SHUF_ZQCAL_CFG] Group:%1d, Configuration Enter\n", group_id));
...
{
    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_ZQCAL_SWITCH],Configuration Enter\n"));
...
{
  mcSHOW_DBG_MSG (("=========================================== \n"));
  mcSHOW_DBG_MSG (("HW_IMPCAL_config\n"               ));
  mcSHOW_DBG_MSG (("=========================================== \n"));
  mcSHOW_DBG_MSG (("TERMINATION         is %d\n", tr->TERMINATION         ));
  mcSHOW_DBG_MSG (("NTODT_EN            is %d\n", tr->NTODT_EN            ));
  mcSHOW_DBG_MSG (("WCK_UPD_DIS         is %d\n", tr->WCK_UPD_DIS         ));
  mcSHOW_DBG_MSG (("DRVP                is %d\n", tr->DRVP                ));
  mcSHOW_DBG_MSG (("DRVN                is %d\n", tr->DRVN                ));
  mcSHOW_DBG_MSG (("ODTN                is %d\n", tr->ODTN                ));
  mcSHOW_DBG_MSG (("ODTP                is %d\n", tr->ODTP                ));
  mcSHOW_DBG_MSG (("RIMP_VREF_SEL_DRVP  is %d\n", tr->RIMP_VREF_SEL_DRVP  ));
  mcSHOW_DBG_MSG (("RIMP_VREF_SEL_DRVN  is %d\n", tr->RIMP_VREF_SEL_DRVN  ));
  mcSHOW_DBG_MSG (("RIMP_VREF_SEL_ODTN  is %d\n", tr->RIMP_VREF_SEL_ODTN  ));
  mcSHOW_DBG_MSG (("RG_RIMP_REV         is %d\n", tr->RG_RIMP_REV         ));
  mcSHOW_DBG_MSG (("IMPCAL_CALICNT      is %d\n", tr->IMPCAL_CALICNT      ));
  mcSHOW_DBG_MSG (("IMPCAL_CALEN_CYCLE  is %d\n", tr->IMPCAL_CALEN_CYCLE  ));
  mcSHOW_DBG_MSG (("IMPCALCNT           is %d\n", tr->IMPCALCNT           ));
  mcSHOW_DBG_MSG (("TXDLY_CMD           is %d\n", tr->TXDLY_CMD           ));
  mcSHOW_DBG_MSG (("CHKCYCLE            is %d\n", tr->CHKCYCLE            ));
  mcSHOW_DBG_MSG (("=========================================== \n"));

}

void DRAMC_HW_FUNC_IMPCAL_config(DRAMC_CTX_T *p, DRAMC_HW_FUNC_IMPCAL_T *tr, U8 group_id)
{
	...
	U8  RL;
	U8  LP5_CAS_MODE = LOWPOWER_CAS_MODE;
	U8  BYTE_MODE = CBT_NORMAL_MODE, RK = RANK_0;
	U16 u4TermDataRate;
	...
	if (DUT_p.ESL_OR_RANDOM == 1) {
	  if (LPDDR5_EN_S) {
	    tr->RIMP_VREF_SEL_DRVP = 0x3a; // max value
	    tr->RIMP_VREF_SEL_DRVN = 0x33; // max value
	    tr->RIMP_VREF_SEL_ODTN = 0x3d; // max value
	  }
	  if (LPDDR4_EN_S) {
	    tr->RIMP_VREF_SEL_DRVP = 0x38; // max value
	    tr->RIMP_VREF_SEL_DRVN = 0x33; // max value
	    tr->RIMP_VREF_SEL_ODTN = 0x38; // max value
	  }
	} else {
    ...
    #if !SA_CONFIG_EN
    if (DUT_p.ESL_OR_RANDOM == 1)
        if      (LPDDR4_EN_S) tr->CHKCYCLE = 7;
        else if (LPDDR5_EN_S) tr->CHKCYCLE = 6;
        else tr->CHKCYCLE = (int)((DFS(group_id)->data_rate) / (5*32*DFS(group_id)->DQ_P2S_RATIO)) + 1; // 5: 200ns:1000ns 32: RTL add 5'h0
    else
	...
	if (LPDDR5_EN_S) {
	    ...
	    tr->WCK_UPD_DIS = (DFS(group_id)->data_rate <= 2400) ? 1 :
	                      (LP5_CAS_MODE == HIGHEFF_CAS_MODE/*HEFF mode*/) ? 1 : 0;
	    ...
	    tr->TXDLY_CMD = A_div_B_RU(2*(DFS(group_id)->CKR)*(RL+8)+5 , DFS(group_id)->DQ_P2S_RATIO);
	}
    else
#endif
	{
	    ...
	    tr->TXDLY_CMD = A_div_B_RU(2000000*RL+4115*DFS(group_id)->data_rate, 1000000*DFS(group_id)->DQ_P2S_RATIO);
...
void DIG_HW_NONSHUF_IMPCAL_CFG(DRAMC_CTX_T *p,U8 ch_id)
{
    ...
    U8 MST_SLV_mode = 1;
#endif
    U8 ECO_OPT   = MST_SLV_mode ? ((ch_id == 0) ? 1 : 0) : 0; // TODO
    U8 IMPSRCEXT = MST_SLV_mode ? ((ch_id != 0) ? 1 : 0) : 0; // TODO
    U8 TRACK_DIS = MST_SLV_mode ? ((ch_id != 0) ? 1 : 0) : 0; // TODO

    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_IMPCAL_CFG],Configuration Enter\n"));

    //if (LPDDR4_EN_S) RIMP_DDR4_SEL = 1;
    //if (LPDDR5_EN_S) RIMP_DDR4_SEL = 0;

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_TX_MCK    ) , P_Fld(0xa, CA_TX_MCK_R_DMRESETB_DRVP_FRPHY      )
                                                                | P_Fld(0xa, CA_TX_MCK_R_DMRESETB_DRVN_FRPHY      )
                                                                | P_Fld(  1                , CA_TX_MCK_R_DMRESET_FRPHY_OPT        ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SREF_DPD_CTRL ) , P_Fld(  0                , SREF_DPD_CTRL_GT_SYNC_MASK           ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DVFS_CTRL0    ) , P_Fld(  0                , DVFS_CTRL0_DVFS_SYNC_MASK            ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_IMPCAL  ) , P_Fld(  ECO_OPT          , MISC_IMPCAL_IMPCAL_ECO_OPT           )
                                                                | P_Fld(  0                , MISC_IMPCAL_DIS_SUS_CH0_DRV          )
...
{
    mcSHOW_DBG_MSG(("[DIG_HW_SHUF_IMPCAL_CFG] Group:%1d, Configuration Enter\n", group_id));
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;

#if !SA_CONFIG_EN
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_IMPEDAMCE_OFFSET1) , P_Fld(  HW_IMPCAL_CTRL.NTODT_EN          , MISC_SHU_IMPEDAMCE_OFFSET1_NTODT_EN ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_MISC_DRVING6          ) , P_Fld(  HW_IMPCAL_CTRL.TXDLY_CMD         , SHU_MISC_DRVING6_IMP_TXDLY_CMD      ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_DRVING8          ) , P_Fld(  0x17              , MISC_SHU_DRVING8_CS_DRVN            )
                                                                             | P_Fld(  0x12              , MISC_SHU_DRVING8_CS_DRVP            ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD12              ) , P_Fld(  HW_IMPCAL_CTRL.RG_RIMP_REV       , SHU_CA_CMD12_RG_RIMP_REV            )
...
{
    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_IMPCAL_SWITCH],Configuration Enter\n"));

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_IMPCAL  ) , P_Fld( EN     , MISC_IMPCAL_IMPCAL_HW                )
                                                                | P_Fld(  1     , MISC_IMPCAL_DRVCGWREF                ));
    vSetPHY2ChannelMapping(p, 0);
}
...
{
    vSetPHY2ChannelMapping(p, ch_id);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_DQSG_SWITCH],Configuration Enter\n"));

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_PRE_TDQSCK1), P_Fld( 1  , MISC_PRE_TDQSCK1_TDQSCK_REG_DVFS      ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_PRE_TDQSCK1), P_Fld( EN , MISC_PRE_TDQSCK1_TDQSCK_HW_SW_UP_SEL  )
                                                                  ...
                                                                  | P_Fld( 1 , MISC_PRE_TDQSCK1_TDQSCK_DBG_EN        )
                                                                  #else
...
{
    U8 irank, backup_rank=p->rank;
    mcSHOW_DBG_MSG(("[DIG_HW_NOSHUF_DMYRD_CFG],Configuration Enter\n"));
    vSetPHY2ChannelMapping(p, ch_id);
    //MP fix
    ...
    {
        vSetRank(p, irank);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_WDATA0), 0X55AAAA55    , RK_DUMMY_RD_WDATA0_DMY_RD_WDATA0  ); //MP error TODO
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_WDATA1), 0X55AAAA55    , RK_DUMMY_RD_WDATA1_DMY_RD_WDATA1  ); //MP error TODO
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_WDATA2), 0X55AAAA55    , RK_DUMMY_RD_WDATA2_DMY_RD_WDATA2  ); //MP error TODO
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_WDATA3), 0X55AAAA55    , RK_DUMMY_RD_WDATA3_DMY_RD_WDATA3  ); //MP error TODO
        
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_ADR    ), P_Fld(     0                              , RK_DUMMY_RD_ADR_DMY_RD_LEN          ));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_ADR    ), P_Fld(    HW_DMYRD_CTRL.BASE_COL          , RK_DUMMY_RD_ADR_DMY_RD_COL_ADR      ));
    ...
    vSetRank(p, backup_rank);
    vSetPHY2ChannelMapping(p, 0);
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_DMYRD_CFG],Configuration Exit\n"));
}
...
{
	int irank, backup_rank;
    mcSHOW_DBG_MSG(("Issue dummy write initilialise dram data \n"));
    backup_rank = p->rank;
    for(irank = RANK_0; irank < RANK_MAX; irank++)
    {
        vSetRank(p, irank);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_ADR    ), P_Fld(1, RK_DUMMY_RD_ADR_DMY_RD_LEN));
    }
    vSetRank(p, backup_rank);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD), P_Fld(1, DUMMY_RD_DMY_WR_DBG ));
    mcDELAY_XUS(1);//waiting dummy write done
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD), P_Fld(0, DUMMY_RD_DMY_WR_DBG ));
    for(irank = RANK_0; irank < RANK_MAX; irank++)
    {
        vSetRank(p, irank);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DUMMY_RD_ADR    ), P_Fld(0, RK_DUMMY_RD_ADR_DMY_RD_LEN));
    ...
    mcSHOW_DBG_MSG(("[DIG_HW_NONSHUF_DMYRD_SWITCH] :%s \n", EN==ENABLE?"ENABLE":"DISBALE"));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD), P_Fld(EN, DUMMY_RD_DUMMY_RD_EN ));
}
...
void print_HW_DQSOSC_CTRL(DRAMC_HW_DQSOSC_CTRL_T *tr)
{
    mcSHOW_DBG_MSG6(("=========================================== \n"));
...
{
    U8 DQS2DQ_FILT_PITHRD_rk0, DQS2DQ_FILT_PITHRD_rk1 ;
    U8 DQS2DQ_FILT_PITHRD_INC_B0, DQS2DQ_FILT_PITHRD_INC_B1;
    U8 DQS2DQ_FILT_PITHRD_DEC_B0, DQS2DQ_FILT_PITHRD_DEC_B1;
    U8 DQS2DQ_FILT_PITHRD_INC_rk0, DQS2DQ_FILT_PITHRD_INC_rk1;
    U8 DQS2DQ_FILT_PITHRD_DEC_rk0, DQS2DQ_FILT_PITHRD_DEC_rk1;
    U8 DQS2DQ_FILT_PITHRD_INC, DQS2DQ_FILT_PITHRD_DEC;
    U8 CKR = p->CKR;
    ...
    tr->DQSOSC_PRDCNT = (U16)(((32 * tr->HW_MR * CKR * u4tCK) + (2 * tOSCO)) / (16 * u4tCK * DQ_P2S_RATIO));
    if (((32 * tr->HW_MR * CKR * u4tCK) + (2 * tOSCO)) % (16 * u4tCK * DQ_P2S_RATIO) != 0)
        tr->DQSOSC_PRDCNT++;
    ...
    if (gu2DQSOSC[WCK2DQI][p->channel][p->rank][0] != 0 && gu2DQSOSC[WCK2DQI][p->channel][p->rank][1] != 0) {
        if (p->rank == RANK_0) {
            DQS2DQ_FILT_PITHRD_INC_B0 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][0][0]- 65))-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][0][0]))/(tr->DQSOSCTHRD_INC_B0_RK[0]));
            DQS2DQ_FILT_PITHRD_INC_B1 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][0][1]- 65))-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][0][1]))/(tr->DQSOSCTHRD_INC_B1_RK[0]));
            DQS2DQ_FILT_PITHRD_INC_rk0 = (DQS2DQ_FILT_PITHRD_INC_B0 >= DQS2DQ_FILT_PITHRD_INC_B1) ? DQS2DQ_FILT_PITHRD_INC_B0 : DQS2DQ_FILT_PITHRD_INC_B1;
            DQS2DQ_FILT_PITHRD_DEC_B0 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][0][0])-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][0][0] + 65)))/(tr->DQSOSCTHRD_DEC_B0_RK[0]));
            DQS2DQ_FILT_PITHRD_DEC_B1 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][0][1])-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][0][1] + 65)))/(tr->DQSOSCTHRD_DEC_B1_RK[0]));
            DQS2DQ_FILT_PITHRD_DEC_rk0 = (DQS2DQ_FILT_PITHRD_DEC_B0 >= DQS2DQ_FILT_PITHRD_DEC_B1) ? DQS2DQ_FILT_PITHRD_DEC_B0 : DQS2DQ_FILT_PITHRD_DEC_B1;
            tr->DQS2DQ_FILT_PITHRD = (DQS2DQ_FILT_PITHRD_INC_rk0 >= DQS2DQ_FILT_PITHRD_DEC_rk0) ? DQS2DQ_FILT_PITHRD_INC_rk0 : DQS2DQ_FILT_PITHRD_DEC_rk0;
            ...
            DQS2DQ_FILT_PITHRD_DEC_B1 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][0][1])-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][0][1] + 65)))/(tr->DQSOSCTHRD_DEC_B1_RK[0]));
            DQS2DQ_FILT_PITHRD_DEC_rk0 = (DQS2DQ_FILT_PITHRD_DEC_B0 >= DQS2DQ_FILT_PITHRD_DEC_B1) ? DQS2DQ_FILT_PITHRD_DEC_B0 : DQS2DQ_FILT_PITHRD_DEC_B1;
            DQS2DQ_FILT_PITHRD_rk0 = (DQS2DQ_FILT_PITHRD_INC_rk0 >= DQS2DQ_FILT_PITHRD_DEC_rk0) ? DQS2DQ_FILT_PITHRD_INC_rk0 : DQS2DQ_FILT_PITHRD_DEC_rk0;
            DQS2DQ_FILT_PITHRD_INC_B0 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][1][0]- 65))-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][1][0]))/(tr->DQSOSCTHRD_INC_B0_RK[1]));
            DQS2DQ_FILT_PITHRD_INC_B1 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][1][1]- 65))-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][1][1]))/(tr->DQSOSCTHRD_INC_B1_RK[1]));
            DQS2DQ_FILT_PITHRD_INC_rk1 = (DQS2DQ_FILT_PITHRD_INC_B0 >= DQS2DQ_FILT_PITHRD_INC_B1) ? DQS2DQ_FILT_PITHRD_INC_B0 : DQS2DQ_FILT_PITHRD_INC_B1;
            DQS2DQ_FILT_PITHRD_DEC_B0 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][1][0])-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][1][0] + 65)))/(tr->DQSOSCTHRD_DEC_B0_RK[1]));
            DQS2DQ_FILT_PITHRD_DEC_B1 = (U8)(((tr->HW_MR * 16 * u4tCK * CKR)/(2 * gu2DQSOSC[WCK2DQI][p->channel][1][1])-(tr->HW_MR * 16 * u4tCK * CKR)/(2 * (gu2DQSOSC[WCK2DQI][p->channel][1][1] + 65)))/(tr->DQSOSCTHRD_DEC_B1_RK[1]));
            DQS2DQ_FILT_PITHRD_DEC_rk1 = (DQS2DQ_FILT_PITHRD_DEC_B0 >= DQS2DQ_FILT_PITHRD_DEC_B1) ? DQS2DQ_FILT_PITHRD_DEC_B0 : DQS2DQ_FILT_PITHRD_DEC_B1;
            DQS2DQ_FILT_PITHRD_rk1 = (DQS2DQ_FILT_PITHRD_INC_rk1 >= DQS2DQ_FILT_PITHRD_DEC_rk1) ? DQS2DQ_FILT_PITHRD_INC_rk1 : DQS2DQ_FILT_PITHRD_DEC_rk1;
            tr->DQS2DQ_FILT_PITHRD = (DQS2DQ_FILT_PITHRD_rk0 >= DQS2DQ_FILT_PITHRD_rk1) ? DQS2DQ_FILT_PITHRD_rk0 : DQS2DQ_FILT_PITHRD_rk1;
            tr->DQS2DQ_WARN_PITHRD = (U8)(tr->DQS2DQ_FILT_PITHRD / 2);
        }
    } else {
    ...
    DQS2DQ_FILT_PITHRD_INC_rk0 = (U8)(((tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*(DUT_p.TDQS2DQ_R0_B0-100))-(tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*DUT_p.TDQS2DQ_R0_B0))/(tr->DQSOSCTHRD_INC_B0_RK[0])+1) + 1;
    DQS2DQ_FILT_PITHRD_INC_rk1 = (U8)(((tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*(DUT_p.TDQS2DQ_R1_B0-100))-(tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*DUT_p.TDQS2DQ_R1_B0))/(tr->DQSOSCTHRD_INC_B0_RK[1])+1) + 1;
    DQS2DQ_FILT_PITHRD_INC = (DQS2DQ_FILT_PITHRD_INC_rk0 >= DQS2DQ_FILT_PITHRD_INC_rk1) ? DQS2DQ_FILT_PITHRD_INC_rk0 : DQS2DQ_FILT_PITHRD_INC_rk1;
    DQS2DQ_FILT_PITHRD_DEC_rk0 = (U8)(((tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*DUT_p.TDQS2DQ_R0_B0)-(tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*(DUT_p.TDQS2DQ_R0_B0+100)))/(tr->DQSOSCTHRD_DEC_B0_RK[0])+1) + 1;
    DQS2DQ_FILT_PITHRD_DEC_rk1 = (U8)(((tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*DUT_p.TDQS2DQ_R1_B0)-(tr->HW_MR*16.0*1000000.0)/(DFS(group_id)->data_rate*(DUT_p.TDQS2DQ_R1_B0+100)))/(tr->DQSOSCTHRD_DEC_B0_RK[1])+1) + 1;
    DQS2DQ_FILT_PITHRD_DEC = (DQS2DQ_FILT_PITHRD_DEC_rk0 >= DQS2DQ_FILT_PITHRD_DEC_rk1) ? DQS2DQ_FILT_PITHRD_DEC_rk0 : DQS2DQ_FILT_PITHRD_DEC_rk1;
    ...
    print_HW_DQSOSC_CTRL(tr);
}

void DIG_HW_NONSHUF_DQSOSC_CFG(DRAMC_CTX_T *p, DRAMC_HW_DQSOSC_CTRL_T *tr, U8 ch_id)
{
    U8 irank = 0;
    ...
    for(irank = RANK_0; irank < RANK_MAX; irank++)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DQSOSC) ,
                P_Fld((irank == 1 ? tr->DQSOSCR_RK1EN : tr->DQSOSCR_RK0EN), RK_DQSOSC_DQSOSCR_RK0EN) |
                P_Fld((irank == 1 ? tr->RK1_BYTE_MODE : tr->RK0_BYTE_MODE), RK_DQSOSC_RK0_BYTE_MODE));
    }
    vSetRank(p, backup_rank);
    ...
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_HWDQSOSC_RTSWCMD_CNT), 0x0, HWDQSOSC_RTSWCMD_CNT_HWDQSOSC_RTSWCMD_CNT);

    vSetPHY2ChannelMapping(p, backup_channel);
}

void DIG_HW_SHUF_DQSOSC_CFG(DRAMC_CTX_T *p, DRAMC_HW_DQSOSC_CTRL_T *tr, U8 ch_id, U8 group_id)
{
    U8 irank = 0;
    U8 backup_channel = p->channel;
    U8 backup_rank = u1GetRank(p);
    U8 backup_shuidx = p->ShuRGAccessIdx;

    mcSHOW_DBG_MSG6(("[DIG_HW_SHUF_DQSOSC_CFG] Group:%1d,Configuration Enter\n", group_id));
    vSetPHY2ChannelMapping(p, ch_id);
    p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHU_DQSOSCR),
            ...
            P_Fld(tr->WCK2DQ_OSC_FM_GROUP, SHU_DQSOSCR_WCK2DQ_OSC_FM_GROUP) |
    ...
    for(irank = RANK_0; irank < RANK_MAX; irank++)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_DQSOSC),
                P_Fld(tr->DQSOSC_BASE_B0_RK[irank], SHURK_DQSOSC_DQSOSC_BASE_RK0) |
                P_Fld(tr->DQSOSC_BASE_B1_RK[irank], SHURK_DQSOSC_DQSOSC_BASE_RK0_B1));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_DQSOSC_THRD_B0),
                P_Fld(tr->DQSOSCTHRD_INC_B0_RK[irank], SHURK_DQSOSC_THRD_B0_DQSOSCTHRD_INC_B0) |
                P_Fld(tr->DQSOSCTHRD_DEC_B0_RK[irank], SHURK_DQSOSC_THRD_B0_DQSOSCTHRD_DEC_B0));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHURK_DQSOSC_THRD_B1),
                P_Fld(tr->DQSOSCTHRD_INC_B1_RK[irank],SHURK_DQSOSC_THRD_B1_DQSOSCTHRD_INC_B1) |
                P_Fld(tr->DQSOSCTHRD_DEC_B1_RK[irank], SHURK_DQSOSC_THRD_B1_DQSOSCTHRD_DEC_B1));
    }
    vSetRank(p, backup_rank);
    ...
    vSetPHY2ChannelMapping(p, backup_channel);
    p->ShuRGAccessIdx = backup_shuidx;
}

void DIG_HW_SHUF_DQSOSC_SWITCH(DRAMC_CTX_T *p,U8 ch_id,U8 group_id,U8 EN)
{
    U8 backup_channel = p->channel;
    U8 backup_shuidx = p->ShuRGAccessIdx;
...
{
    DRAMC_HW_FUNC_REF_CTRL_T_config(&HW_REF_CTRL);
    ...
    DRAMC_HW_FUNC_IMPCAL_config(p, &HW_IMPCAL_CTRL, 0);
}
...
void DIG_HW_NONSHUF_CFG(DRAMC_CTX_T *p,U8 ch_id) // call in DIG_NONSHUF_config.c -> DIG_STATIC_SETTING, for common
{
    ...
    //DIG_HW_NONSHUF_DMYRD_CFG(p,ch_id);
    //DIG_HW_NONSHUF_DQSOSC_CFG(p, ch_id);
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    if (!LPDDR5_EN_S) DIG_HW_NONSHUF_ZQCAL_CFG(p, 0);
    if (!LPDDR5_EN_S) DIG_HW_NONSHUF_ZQCAL_CFG(p, 1);
    DIG_HW_NONSHUF_IMPCAL_CFG(p, 0);
    DIG_HW_NONSHUF_IMPCAL_CFG(p, 1);
...
void DIG_HW_SHUF_CFG(DRAMC_CTX_T *p,U8 ch_id,U8 group_id) // call in DIG_SHU_config.c -> DIG_CONFIG_SHUF, for group 0~9 use
{
    ...
    DRAMC_HW_FUNC_IMPCAL_config(p, &HW_IMPCAL_CTRL, group_id);
...
{
    U8 HW_ZQ_EN, HW_IMPCAL_EN, HW_DQSG_EN;

#if !SA_CONFIG_EN
    HW_ZQ_EN = HW_p.HW_ZQ_EN;
    HW_IMPCAL_EN = HW_p.HW_IMPCAL_EN;
    HW_DQSG_EN = HW_p.HW_DQSG_EN;
#else
    HW_ZQ_EN = HW_IMPCAL_EN = HW_DQSG_EN = 0;
#endif

    //DIG_HW_NONSHUF_REF_SWITCH(p,ch_id,HW_p.HW_REF_EN);
    //DIG_HW_NONSHUF_REFR_SWITCH(p,ch_id,HW_p.HW_REFR_EN);
    //DIG_HW_NONSHUF_DMYRD_SWITCH(p,ch_id,HW_p.HW_DMYRD_EN);
    //DIG_HW_NONSHUF_DQSOSC_SWITCH(p, ch_id, HW_p.HW_DQSOSC_EN);
    if (!LPDDR5_EN_S) DIG_HW_NONSHUF_ZQCAL_SWITCH(p, ch_id, HW_ZQ_EN);
    DIG_HW_NONSHUF_IMPCAL_SWITCH(p, ch_id, HW_IMPCAL_EN);
    DIG_HW_NONSHUF_DQSG_SWITCH(p, ch_id, HW_DQSG_EN); // if change code order to here, have issue TODO
}
...
{
    U8 HW_ZQ_EN, HW_IMPCAL_EN;
	
#if !SA_CONFIG_EN
    HW_ZQ_EN = HW_p.HW_ZQ_EN;
    HW_IMPCAL_EN = HW_p.HW_IMPCAL_EN;
#else
    HW_ZQ_EN = HW_IMPCAL_EN = 0;
#endif
    ...
    if (!LPDDR5_EN_S) DIG_HW_SHUF_ZQCAL_SWITCH(p,ch_id,group_id,HW_ZQ_EN);
    DIG_HW_SHUF_IMPCAL_SWITCH(p,ch_id,group_id,HW_IMPCAL_EN);
}
