...
int mem_clear_region[] = {10, 11, 12, 13, 14, 15, 16};
...
{
#ifdef DDR_RESERVE_MODE
	struct region_to_clear region[MEM_CLEAR_REGION_COUNT];
	int rnum;
#endif
	...
	if((g_ddr_reserve_enable==1) && (g_ddr_reserve_success==1)) {
		...
		for (rnum = 0; rnum < MEM_CLEAR_REGION_COUNT; rnum++) {
			region[rnum].region_id = mem_clear_region[rnum];
			// query SA & EA of region_id
			region[rnum].check = check_emi_mpu_addr(&region[rnum]);
		}
		...
		for (rnum = 0; rnum < MEM_CLEAR_REGION_COUNT; rnum++) {
			...
			if (region[rnum].check == 0)
				do_clear_region(region[rnum].region_sa, region[rnum].region_ea + 1 - region[rnum].region_sa);
		}

	} else  /* normal boot or ddr reserved mode enable but failed */
#endif
	{
		/*In this case, dram will re-init, so we need to clear MPU protection*/
		/*no mrdump, no need to clear region content*/
...
static int do_clear_region(unsigned int start, unsigned int len)
{
	...
	print("[do_clear_region] start: 0x%x len:%d\n", start, len);
	for (i = 0; i < (len >> 2); ++i) {
#if TEST_OVERWRITE
		...
		if (new_val != PATTERN1) {
			print("[do_clear_region] error 0x%x ", new_val);
		return -1;
		}
		dsb();
#else
