...
static void mt6315_init_setting(void)
{
	...
	for (pidx = 0; pidx < MT6315_MAX; pidx++) {
		...
		if (chip->type == MT6315_TYPE_A) {
			...
			/* 1:(LP5), 2:(LP4x), 3:(LP5-2) */
			/* Disable LP5_VDDQ for LP4 project */
			mode = mt6360_pmic_check_hw_id();
			if (mode > 0 && !(mode & 0x1)) {
				mt6315_write_field_byte(chip->spmi_dev,
							MT6315_PMIC_RG_BUCK_VBUCK4_EN_ADDR, 0,
							MT6315_PMIC_RG_BUCK_VBUCK4_EN_MASK,
							MT6315_PMIC_RG_BUCK_VBUCK4_EN_SHIFT);
				MT6315_INFO("Disable 6315_S6_vbuck4.\n");
			}
			...
			/* Disable VUFS12 for LP4 project */
			mode = mt6360_pmic_check_hw_id();
			if (mode > 0 && !(mode & 0x1)) {
				mt6315_write_field_byte(chip->spmi_dev,
							MT6315_PMIC_RG_BUCK_VBUCK3_EN_ADDR, 0,
							MT6315_PMIC_RG_BUCK_VBUCK3_EN_MASK,
							MT6315_PMIC_RG_BUCK_VBUCK3_EN_SHIFT);
				MT6315_INFO("Disable 6315_S3_VBUCK3.\n");
			}
		}
