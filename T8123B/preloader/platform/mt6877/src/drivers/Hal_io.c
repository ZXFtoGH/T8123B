...
U32 u4RegByteAddrTraslate(U32 u4reg_addr)
{
    //Need to check gating
    #if (fcFOR_CHIP_ID == fcIPMv2)
    if (((u4reg_addr >= DDRPHY_REG_SHU_R0_B0_TXDLY0) && (u4reg_addr <= DDRPHY_REG_B0_SHU_MIDPI_CTRL))  
        || ((u4reg_addr >= DDRPHY_REG_RK_B0_RXDVS0) && (u4reg_addr <= DDRPHY_REG_B0_TX_MCK)))
        u4reg_addr += DDRPHY_AO_B0_B1_OFFSET;    
    else if (((u4reg_addr >= DDRPHY_REG_SHU_R0_B1_TXDLY0) && (u4reg_addr <= DDRPHY_REG_B1_SHU_MIDPI_CTRL))
        || ((u4reg_addr >= DDRPHY_REG_RK_B1_RXDVS0) && (u4reg_addr <= DDRPHY_REG_B1_TX_MCK)))
        u4reg_addr -= DDRPHY_AO_B0_B1_OFFSET;    
    #endif
    return u4reg_addr;
}    
...
{
    U32 u4Offset = u4reg_addr & ((1 << POS_BANK_NUM) - 1);
    U32 u4RegType = ((u4reg_addr - Channel_A_DRAMC_NAO_BASE_VIRTUAL) >> POS_BANK_NUM) & 0xf;
    U32 u4BaseAddr = 0;

    //mcSHOW_DBG_MSG(("[u4RegBaseAddrTraslate]  0x%X", u4reg_addr));
    ...
    if (eRank == RANK_1)
    {
        ...
        else if (u4RegType >= 4 && u4RegType <= 5) // PhyA/B NAO Register
        {
            ...
            else if (u4Offset >= DDRPHY_NAO_STB_DBG_INFO_START &&
                u4Offset < DDRPHY_NAO_STB_DBG_INFO_END)
            {
                u4Offset += DDRPHY_NAO_STB_DBG_INFO_RK_OFFSET;
            }
        }
    }
...
U32 u4Dram_Register_Read(DRAMC_CTX_T *p, U32 u4reg_addr)
{
    ...
    if ((p->channel == CHANNEL_A) && (is_lp5_family(p)))        
        u4reg_addr = u4RegByteAddrTraslate(u4reg_addr);
#endif
...
inline void _ucDram_Register_Write(U32 u4reg_addr, U32 u4reg_value)
{
    ...
    {
#if (FOR_DV_SIMULATION_USED == 1) //fra add for test
        U32 u4Offset = u4reg_addr & ((1 << 16) - 1);
        U32 u4RegType = ((u4reg_addr) >> 16) & 0x3f;
        U32 u4BaseAddr = 0;

//mcSHOW_DUMP_INIT_RG_MSG(("==== u4reg_addr=0x%x, u4RegType=%d, u4Offset=0x%x\n",u4reg_addr, u4RegType, u4Offset));

        switch (u4RegType)
        {
             case 1:
                 u4BaseAddr = 0x10230000;
                 break;
             case 2:
                 u4BaseAddr = 0x10234000;
                 break;
             case 3:
                 u4BaseAddr = 0x10238000;
                 break;
             case 4:
                   u4BaseAddr = 0x10240000;
                   break;
             case 5:
                   u4BaseAddr = 0x10244000;
                   break;
             case 6:
                   u4BaseAddr = 0x10248000;
                   break;
             case 7:
                   u4BaseAddr = 0x10236000;
                   break;
             case 8:
                   u4BaseAddr = 0x10246000;
                   break;
             case 16:      
                   u4BaseAddr = 0x10900000;
                   break;                
             case 32:      
                   u4BaseAddr = 0x10A00000;
                   break;                   
        }

        u4reg_addr = u4BaseAddr | u4Offset;
#endif
...
void vIO32Write4BMsk2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 msk32)
{
	...
	//ignore CH-B
    #if (fcFOR_CHIP_ID == fcA60868)
    if (reg32 >= Channel_A_DRAMC_NAO_BASE_VIRTUAL &&
        reg32 <= MAX_BASE_VIRTUAL)
	{
		if(u4RegType%2!=0)
			return;
	}
...
#if (__LP5_COMBO__ == TRUE)    
    if ((p->channel == CHANNEL_A) && (is_lp5_family(p)))        
        reg32 = u4RegByteAddrTraslate(reg32);
#endif
...
U8 u1BCSupport = TRUE;
    ...
    reg32 &= ((1 << POS_BANK_NUM) - 1);     // remove channel information
    ...
    if (u4RegType >= Channel_A_DDRPHY_DPM_BASE_VIRTUAL)//DPM
    {
        ...
        if (u1AllCount > 1 && p->u1SingleTop == FALSE)
            u1AllCount >>= 1;
        u1BCSupport = FALSE;
    }
...
void vIO32Write4BMsk_All2(DRAMC_CTX_T *p, U32 reg32, U32 val32, U32 msk32)
{
    ...
    {
    #if (__LP5_COMBO__ == TRUE)    
        if ((ii == CHANNEL_A) && (is_lp5_family(p)))		
            u4reg32_bk = u4RegByteAddrTraslate(reg32);
        else
    #endif
            u4reg32_bk = reg32;

        u4RegTmp = u4RegBaseAddrTraslate(p->ShuRGAccessIdx, p->rank, u4reg32_bk + ((U32)ii << POS_BANK_NUM));
