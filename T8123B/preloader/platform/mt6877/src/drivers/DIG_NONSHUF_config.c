...
static void DIG_PHY_config(DRAMC_CTX_T *p)
{
...
#if DV_CONFIG_EN
    U8 TDQSCK_THRD = 0x88; //Set RG_tDQSCK_THRD = 'h88 For TBA tDQSCK Hack (Avoid the OVL_PREK false alarm), Confirm with DE(YT) 
    U8 TDQSCK_DBG_EN = 0;
#else
    U8 TDQSCK_THRD = 0x30;
    ...
    //kaihsin: 20200316 move to ANA_CLOCK_SWITCH, before W_CHG_MEM
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_PRE_TDQSCK1), P_Fld(TDQSCK_THRD, MISC_PRE_TDQSCK1_TDQSCK_THRD) \
                                                                         | P_Fld(   TDQSCK_DBG_EN     , MISC_PRE_TDQSCK1_TDQSCK_DBG_EN                        ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DDR_RESERVE)       ,P_Fld (   0xf   , MISC_DDR_RESERVE_WDT_CONF_ISO_CNT                   ) \
                                                                         | P_Fld(   1     , MISC_DDR_RESERVE_WDT_LITE_EN                        ) \
                                                                         | P_Fld(   0     , MISC_DDR_RESERVE_WDT_SM_CLR                         ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTYSCAN1), P_Fld(0x1, MISC_DUTYSCAN1_EYESCAN_DQS_SYNC_EN));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DDR_RESERVE3), P_Fld(0x10, MISC_DDR_RESERVE3_DRM_SM_PASS_CTRL));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DDR_RESERVE2), P_Fld(0x300, MISC_DDR_RESERVE2_DRM_SM_HOLD_CTRL));
...
void GATING_MODE_CFG(Gating_confg_T *tr)
{
    ...
    mcSHOW_DBG_MSG6(("RX_GATING_TRACK_MODE  0: Valid DLY Mode  1: Valid Mode (Ja_de-like) 2: FIFO mode\n"));
...
static void DPHY_GAT_TRACK_Config(DRAMC_CTX_T *p,Gating_confg_T *gat_c)
{
    ...
    switch (gat_c->RX_GATING_TRACK_MODE)
    {
        ...
        case 0:
        {
            //TODO SHU1_DQSG if ja_de-like mode should set STB_UPDMASKCYC = 0 STB_UPDMASK_EN=0 others STB_UPDMASKCYC=9 STB_UPDMASK_EN=1
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY2)           , P_Fld(   1  , CA_PHY2_RG_RX_ARCLK_JM_SEL_CA               ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL)       , P_Fld(   1  , MISC_STBCAL_DQSIENMODE                      ) \
                                                                    ...
                                                                    | P_Fld(   1  , MISC_STBCAL_RKCHGMASKDIS                    ) \
...
void RX_INTPUT_Config(DRAMC_CTX_T *p)
{
    ...
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ5  )    , P_Fld(  1  , B1_DQ5_RG_RX_ARDQS0_DVS_EN_B1          ));// TODO191205
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ5  )    , P_Fld(  1  , B0_DQ5_RG_RX_ARDQS0_DVS_EN_B0          ));// TODO191205
        
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_RXDVS0 )  , P_Fld(  tracking_en  , B0_RXDVS0_R_RX_DLY_TRACK_ENA_B0        )\
                                                              | P_Fld(  tracking_en  , B0_RXDVS0_R_RX_DLY_TRACK_CG_EN_B0      )\
                                                              | P_Fld(  1  , B0_RXDVS0_R_RX_DLY_TRACK_SPM_CTRL_B0   )\
                                                              | P_Fld(  0  , B0_RXDVS0_R_RX_RANKINCTL_B0            )\
                                                              | P_Fld(  1  , B0_RXDVS0_R_RX_RANKINSEL_B0            ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_RXDVS0 )  , P_Fld(  tracking_en  , B1_RXDVS0_R_RX_DLY_TRACK_ENA_B1        )\
                                                              | P_Fld(  tracking_en  , B1_RXDVS0_R_RX_DLY_TRACK_CG_EN_B1      )\
    ...
    {
        vSetRank(p, irank);
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_RK_B0_RXDVS2 )  , P_Fld(  tracking_en  , RK_B0_RXDVS2_R_RK0_RX_DLY_FAL_TRACK_GATE_ENA_B0  ) \
                                                                     | P_Fld(  tracking_en  , RK_B0_RXDVS2_R_RK0_RX_DLY_RIS_TRACK_GATE_ENA_B0  ) \
                                                                     | P_Fld(  2  , RK_B0_RXDVS2_R_RK0_DVS_MODE_B0                   ));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_RK_B1_RXDVS2 )  , P_Fld(  tracking_en  , RK_B1_RXDVS2_R_RK0_RX_DLY_FAL_TRACK_GATE_ENA_B1  ) \
                                                                     | P_Fld(  tracking_en  , RK_B1_RXDVS2_R_RK0_RX_DLY_RIS_TRACK_GATE_ENA_B1  ) \
    ...
    if (RG_MODE_EN == 1)
    {
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RG_DFS_CTRL)    , P_Fld(  tracking_en  , MISC_RG_DFS_CTRL_RG_DPY_RXDLY_TRACK_EN      ));
    } else { 
...
void DIG_CONFIG_NONSHUF_DCM(DRAMC_CTX_T *p,U8 DCM_NONSHUF_EN)
{
    ...
    mcSHOW_DBG_MSG(("[Flow] [DDRPHY] DIG_CONFIG_NONSHUF_DCM    <<<<< \n"));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CG_CTRL2)     , P_Fld(  1               , MISC_CG_CTRL2_RG_MEM_DCM_CG_OFF_DISABLE         ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CTRL3)        , P_Fld(  !DCM_NONSHUF_EN , MISC_CTRL3_R_DDRPHY_RX_PIPE_CG_IG               ) \
                                                                    | P_Fld(  !DCM_NONSHUF_EN , MISC_CTRL3_R_DDRPHY_COMB_CG_IG                  ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RX_AUTOK_CFG0), P_Fld(  DCM_NONSHUF_EN   , MISC_RX_AUTOK_CFG0_RX_AUTOK_CG_CTRL            ) ); 

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_JM_8PH_AUTOK_CFG0), P_Fld(  DCM_NONSHUF_EN   , MISC_JM_8PH_AUTOK_CFG0_JM_8PH_AUTOK_CG_CTRL) ); 

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CG_CTRL10), P_Fld(  !DCM_NONSHUF_EN   , MISC_CG_CTRL10_RG_MEM_DCM_FORCE_ON_MCK8X          ) ); 
...
//dramc part
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DDRCOMMON0   )         , P_Fld(   !DCM_NONSHUF_EN   , DDRCOMMON0_DISSTOP26M                   ) );
    
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD     )         , P_Fld(   1                 , DUMMY_RD_DUMMY_RD_PA_OPT                ) );
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DCM_CTRL0)             , P_Fld(   !DCM_NONSHUF_EN   , DCM_CTRL0_BCLKAR                        ) ); 
    
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DVFS_CTRL0)            , P_Fld(   !DCM_NONSHUF_EN   , DVFS_CTRL0_DVFS_CG_OPT                  ) ); 
    
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CMD_DEC_CTRL0)         , P_Fld(   !DCM_NONSHUF_EN   , CMD_DEC_CTRL0_SELPH_CMD_CG_DIS          ) );
...
#endif
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DCM_SUB_CTRL )         , P_Fld(   0                              ,DCM_SUB_CTRL_SUBCLK_CTRL_SHUFFLE_SM     ));//#45797, V21 only
...
static void DDRPHY_PICG_Config(DRAMC_CTX_T *p, U8 MISC_CG_EN)
{
    U8 PICG_MODE  = 1; // only support new Mode under pe_trus
    //U8 MISC_CG_EN = 1;
    U8 MISC_CG_REVERSE_DEFAULT_ON = 0;  //for default CG enable.

    mcSHOW_DBG_MSG6(("Enter into PICG configuration >>>> \n"));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CTRL4)        ,  P_Fld(   PICG_MODE  , MISC_CTRL4_R_OPT2_CG_DQSIEN                 ));
    ...
    if(MISC_CG_REVERSE_DEFAULT_ON == 1)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DVFS_CTRL0)         , P_Fld(   1      , DVFS_CTRL0_DVFS_CG_OPT                       ));
        //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DUTYSCAN1)    , P_Fld(   1      , MISC_DUTYSCAN1_EYESCAN_DQS_OPT               )); // TODO wuly
...
static void DRAMC_COMMON_Config(DRAMC_CTX_T *p)
{
...
#if !SA_CONFIG_EN
    DMPINMUX = DUT_p.DMPINMUX;
    BGPIPE_EN = DUT_p.BGPIPE_EN;
#else
   #if __LP5_COMBO__
   if (LPDDR5_EN_S)
       DMPINMUX = 0;
   ...
   #endif
   {
    ...
    if(LPDDR5_EN_S == 1) {
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CTRL1)        , P_Fld(   0             , MISC_CTRL1_R_DMPINMUX            ));
    vSetPHY2ChannelMapping(p, CHANNEL_B);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CTRL1)        , P_Fld(   1             , MISC_CTRL1_R_DMPINMUX            ));
    vSetPHY2ChannelMapping(p, CHANNEL_A);
    DramcBroadcastOnOff(DRAMC_BROADCAST_ON);
    } else
#endif
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CTRL1)        , P_Fld(   DMPINMUX, MISC_CTRL1_R_DMPINMUX            ));
    }

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RX_SET0)            , P_Fld(   0             , RX_SET0_DM4TO1MODE               ));

#if 0 // Darren-from DIG_HW_NONSHUF_REF_CFG
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_REFCTRL0)           , P_Fld(   5             , REFCTRL0_REF_PREGATE_CNT             ) \
                                                                    ...
                                                                    | P_Fld(   0             , REFCTRL0_REFRATE_FOR_BK_ARBMASK      ) \
                                                                    | P_Fld(   0             , REFCTRL0_BK_ARBMASK_ALLRATE_EN       ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DLLFRZ_CTRL)        , P_Fld(   0             , DLLFRZ_CTRL_UPDBYWR              ) \
                                                                    | P_Fld(   0             , DLLFRZ_CTRL_DLLFRZ_MON_PBREF_OPT ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DRAMCTRL)           , P_Fld(   0             , DRAMCTRL_ADRDECEN                ) \
                                                                    ...
                                                                    | P_Fld(   0             , DRAMCTRL_ALL_BLOCK_CTO_ALE_DBG_EN));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MISCTL0)            , P_Fld(   1             , MISCTL0_WDLE_DVFS_NO_FLUSH_FIX_ENA));
...
//                                                                    | P_Fld(   0x3           , DRAMC_PD_CTRL_APHYPI_CKCGH_CNT   ) 
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SREF_DPD_CTRL)      , P_Fld(   1             , SREF_DPD_CTRL_SELFREF_AUTOSAVE_EN) \
                                                                    ...
                                                                    | P_Fld(LPDDR5_EN_S ? 1:0, SREF_DPD_CTRL_DSM_HW_EN          ));
...
//    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_HW_MRR_FUN)         , P_Fld(RUNTIME_MRR      , HW_MRR_FUN_TMRR_ENA              ) 
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_HW_MRR_FUN)         , P_Fld(   1             , HW_MRR_FUN_MRR_HW_HIPRI          ));
...
//                                                                    | P_Fld(RD2MRR_EXTEND_EN , HW_MRR_FUN_MANTMRR_EN            ));
    ...
    //Darren-vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_HMR4)               , P_Fld(   1             , HMR4_SPDR_MR4_OPT                ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SCSMCTRL)           , P_Fld(   0             , SCSMCTRL_SC_PG_MAN_DIS           ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_CTRL1)     , P_Fld(   0x10          , CBT_WLEV_CTRL1_CATRAIN_INTV      ) \
                                                                    | P_Fld(   0x0           , CBT_WLEV_CTRL1_CATRAINLAT        ));

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_SET0)            , P_Fld(   1             , TX_SET0_DRSCLR_EN                ) 
                                                                    | P_Fld(   (LPDDR5_EN_S==1)?0x0 :0x1             , TX_SET0_WPRE2T                   ) 
    ...
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DQSOSCR)            , P_Fld(0                , DQSOSCR_TXUPDMODE                )); // TODO: use HW_FUNC_MANAGE.c
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD)           , P_Fld(1                , DUMMY_RD_DMYRD_REORDER_DIS       )); // TODO: use HW_FUNC_MANAGE.c
...
//                                                                    | P_Fld(   0             , DUMMY_RD_INTV_DUMMY_RD_CNT0      ));
    //Byte Mode choose
    vSetRank(p, RANK_1);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DQSOSC)          , P_Fld(((p->dram_cbt_mode[RANK_1] == CBT_BYTE_MODE1)?1:0) , RK_DQSOSC_RK0_BYTE_MODE          ));// noraml:0 byte:1 mix:1
    vSetRank(p, RANK_0);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_RK_DQSOSC)          , P_Fld(((p->dram_cbt_mode[RANK_0] == CBT_BYTE_MODE1)?1:0) , RK_DQSOSC_RK0_BYTE_MODE          ));// normal:0 byte:1 mix:0
    
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_TRACKING_SET0)   , P_Fld(   0             , TX_TRACKING_SET0_TX_TRACKING_OPT ));
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_CG_SET0)         , P_Fld(   1             , TX_CG_SET0_SELPH_4LCG_DIS        )); // TODO wuly
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DRAMC_PD_CTRL)      , P_Fld(   0             , DRAMC_PD_CTRL_DRAMC_ARPI_CG_MCK_CA_FR        ) \
                                                                    | P_Fld(   0             , DRAMC_PD_CTRL_DRAMC_ARPI_CG_CS_FR            ) \
                                                                    | P_Fld(   0             , DRAMC_PD_CTRL_DRAMC_ARPI_CG_CLK_FR           ) \
                                                                    | P_Fld(   0             , DRAMC_PD_CTRL_DRAMC_ARPI_CG_CMD_FR           ) \
                                                                    | P_Fld(   0             , DRAMC_PD_CTRL_DRAMC_ARPI_MPDIV_CG_CA_FR      ));
#if TX_PICG_NEW_MODE
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_CG_SET1)         , P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_DQS_DATA_FR         ) \
                                                                    | P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_DQ_DATA_FR          ) \
                                                                    | P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_DQM_DATA_FR         ) \
                                                                    | P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_MCK_DATA_FR         ) \
                                                                    | P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_MPDIV_DATA_FR       ) \
                                                                    | P_Fld(   0             , TX_CG_SET1_DRAMC_ARPI_CG_DATA_OPT            ));
#else
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TX_CG_SET1)         , P_Fld(   0xf             , TX_CG_SET1_DRAMC_ARPI_CG_DQS_DATA_FR         ) \
                                                                    | P_Fld(   0xf             , TX_CG_SET1_DRAMC_ARPI_CG_DQ_DATA_FR          ) \
                                                                    | P_Fld(   0xf             , TX_CG_SET1_DRAMC_ARPI_CG_DQM_DATA_FR         ) \
                                                                    | P_Fld(   0xf             , TX_CG_SET1_DRAMC_ARPI_CG_MCK_DATA_FR         ) \
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MPC_CTRL)           , P_Fld(   1             , MPC_CTRL_RTMRW_HPRI_EN            ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MPC_CTRL)           , P_Fld(   1             , MPC_CTRL_RTSWCMD_HPRI_EN          ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_STBCAL2 ),      P_Fld(   1             , MISC_STBCAL2_DQSGCNT_BYP_REF      ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_OE_RODT_LPBK_CTRL0 ),P_Fld(   0        , MISC_OE_RODT_LPBK_CTRL0_OE_RODT_LPBK_PAT_LEN));
...
//    }
    ...
    //=============================================
    //mcSHOW_DBG_MSG(("[WORKAROUND]Because of shuffle register SHU_B0_DQ13_RG_TX_ARDQ_IO_ODT_DIS_B0 error connection we should workaround by different frequecny >>>>>\n")); 
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ6  )  , P_Fld(   (DFS(0)->data_rate >= 2667) ? 0 :1             , B0_DQ6_RG_TX_ARDQ_ODTEN_EXT_DIS_B0 ));
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ6  )  , P_Fld(   (DFS(0)->data_rate >= 2667) ? 0 :1             , B1_DQ6_RG_TX_ARDQ_ODTEN_EXT_DIS_B1 ));
    //mcSHOW_DBG_MSG(("[WORKAROUND]Because of shuffle register SHU_B0_DQ13_RG_TX_ARDQ_IO_ODT_DIS_B0 error connection we should workaround by different frequecny <<<<<\n")); 
...
{
    U32 MCP_EN = 1; //for Pe_trus MCP should adjust some setting between CHs (A-B/C-D)
    U32 REF_104M_EN = 0;
    U8 DFS_CLK_CHG_NEW_MODE;
    U8 DFS_RTMRW = 0, DFS_RG_MODE = 0;

#if !SA_CONFIG_EN
    DFS_CLK_CHG_NEW_MODE = DUT_shu_p[0].DVFS_CLK_MODE_SEL;
    DFS_RTMRW = DUT_p.DVFS_RTMRW_EN;
#else
    DFS_CLK_CHG_NEW_MODE = 1;
    DFS_RTMRW = ENABLE_DFS_RUNTIME_MRW; //is_lp5_family(p)?1:0;
    DFS_RG_MODE = !REPLACE_DFS_RG_MODE;
#endif
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DVFS_CTRL0)         , P_Fld(   1             , DVFS_CTRL0_VRCG_EN                               ) \
                                                                    ...
                                                                    | P_Fld(!DFS_RTMRW, DVFS_CTRL0_HWSET_WLRL                         ) \
                                                                    | P_Fld(   0             , DVFS_CTRL0_MRWWOPRA                              )); //Have to fix 0, 1 with bug (some bank without precharge)
    //for DVFS sync
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RG_DFS_CTRL)  , P_Fld(   DFS_RG_MODE             , MISC_RG_DFS_CTRL_SPM_DVFS_CONTROL_SEL            ));//SPM mode TODO should random  0 for SPM mode default
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SRAM_DMA0  )  , P_Fld(   0             , MISC_SRAM_DMA0_DMA_TIMER_EN                      ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SRAM_DMA1  )  , P_Fld(   0x1ffff       , MISC_SRAM_DMA1_SPM_RESTORE_STEP_EN               ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CG_CTRL7   )  , P_Fld(   1             , MISC_CG_CTRL7_ARMCTL_CK_OUT_CG_SEL               ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL    )  , P_Fld(   1             , MISC_DVFSCTL_R_DVFS_PICG_POSTPONE                ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL3  )   , P_Fld(   1             , MISC_DVFSCTL3_RG_PHY_ST_DELAY_AFT_CHG_TO_MCLK    ) \
                                                                    ...
                                                                    | P_Fld(   0             , MISC_DVFSCTL3_RG_PHY_ST_DELAY_BEF_CHG_TO_BCLK    ) \
                                                                    | P_Fld((DFS_CLK_CHG_NEW_MODE?2:3), MISC_DVFSCTL3_RG_DVFS_MEM_CK_SEL_DESTI ) \
                                                                    | P_Fld((DFS_CLK_CHG_NEW_MODE?0:1), MISC_DVFSCTL3_RG_DVFS_MEM_CK_SEL_SOURCE) \
                                                                    | P_Fld(   0             , MISC_DVFSCTL3_RG_CNT_PHY_ST_DELAY_BEF_CHG_TO_BCLK) \
                                                                    | P_Fld(   3             , MISC_DVFSCTL3_RG_CNT_PHY_ST_DELAY_AFT_CHG_TO_MCLK) \
                                                                    | P_Fld(   0             , MISC_DVFSCTL3_RG_CNT_PHY_ST_DELAY_AFT_CHG_TO_BCLK) );
    ...
    //vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL3  )   , P_Fld(   0x10          , MISC_DVFSCTL3_RG_CNT_PHY_ST_DELAY_AFT_CHG_TO_BCLK));
    //flow end??
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL2   )  /*, P_Fld(   1             , MISC_DVFSCTL2_R_DVFS_CDC_OPTION                  )\*/ // TODO wuly
                                                                    ...
                                                                    | P_Fld(DFS_CLK_CHG_NEW_MODE, MISC_DVFSCTL2_RG_DFS_CLK_CHG_NEW_MODE        ) \
...
//    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_CKMUX_SEL  )  , P_Fld( REF_104M_EN     , MISC_CKMUX_SEL_RG_52M_104M_SEL                   ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DVFS_TIMING_CTRL3), P_Fld(0, DVFS_TIMING_CTRL3_RTMRW_MRW1_PASS) // OP CHG & VRCG High
                                                                    | P_Fld(0, DVFS_TIMING_CTRL3_RTMRW_MRW2_PASS) // VRCG Low
                                                                    | P_Fld(DFS_RTMRW, DVFS_TIMING_CTRL3_RTMRW_MRW3_PASS)); // MR2 RL/WL (reduce 50ns)

    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_SHUCTRL1), P_Fld(0x5, SHUCTRL1_VRCGEN_PRDCNT) 
                                                                    | P_Fld(0xc, SHUCTRL1_CKFSPE_PRDCNT)
                                                                    | P_Fld(0xc, SHUCTRL1_CKFSPX_PRDCNT)); //@Berson, Sync MPsetting


#if (fcFOR_CHIP_ID == fcA60892) // @Jimmy, for A60892 only
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL2), P_Fld(1, MISC_DVFSCTL2_RG_IGNORE_PHY_SH_CHG_CLK_RDY_CHB)
                                                                    | P_Fld(0, MISC_DVFSCTL2_RG_IGNORE_PHY_SH_CHG_CLK_RDY_CHA));
#endif
    ...
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    vSetPHY2ChannelMapping(p, CHANNEL_B);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL2   )  , P_Fld( !DUT_p.SINGLE_TOP , MISC_DVFSCTL2_R_DVFS_CLK_CHG_OK_SEL            ));
    vSetPHY2ChannelMapping(p, CHANNEL_A);
    DramcBroadcastOnOff(DRAMC_BROADCAST_ON);

//! IPMV1    if(MCP_EN == 1)
//! IPMV1    {
//! IPMV1        DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
//! IPMV1        vSetPHY2ChannelMapping(p, CHANNEL_B);
//! IPMV1        mcSHOW_DBG_MSG6(("MCP Enable leading 2ch's sync singles should adjust delay margin.")); 
//! IPMV1        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL    )  , P_Fld((REF_104M_EN==1)?6:4, MISC_DVFSCTL_R_DVFS_PICG_MARGIN_NEW        ));
//! IPMV1        vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_DVFSCTL3  )   , P_Fld(   9                , MISC_DVFSCTL3_RG_CNT_PHY_ST_DELAY_BEF_CHG_TO_BCLK    ));
//! IPMV1        vSetPHY2ChannelMapping(p, CHANNEL_A);
//! IPMV1        DramcBroadcastOnOff(DRAMC_BROADCAST_ON);
//! IPMV1    }
...
{
    DIG_CONFIG_NONSHUF_DCM(p,0);//close DCM when DRAMC intial
    DIG_PHY_config(p);
