...
void LP5_MRS(DRAMC_CTX_T *p, U16 reg_addr, U8 reg_op, U8 rank)
{
  ...
  if(DUT_p.DV_FAST_UP == 1) {
    mcSHOW_DBG_MSG(("[LP5_DRAM_INIT_MRS] Backdoor MRW to RK:%1d-MA:%2d-OP:0x%2x @Channle:%1d\n",rank,reg_addr,reg_op,vGetPHY2ChannelMapping(p)));
	  LP5_DRAM_init_sudo(p->channel, rank, reg_addr, reg_op);
  } else
#endif
  {
...
void LP5_FSP_WR_or_OP (DRAMC_CTX_T *p, U8 FSP_WR, U8 FSP_OP, U8 rank)
{
	...
	if (DUT_p.DV_FAST_UP == 0) { // DV use backdoor MRW
...
void lp5_dram_init_single_rank(DRAMC_CTX_T *p,LP5_DRAM_CONFIG_T *tr,U8 rank)
{
    ...
    #if ENABLE_SINGLE_END_LP5 && (SE_BACKUP_SOLUTION == 1)
    if(p->frequency<=800)
    {
        if (rank==0) //use WCK_t and RDQS_t
        {
            WCKmode=1;
            RDQSmode=1;
        }
        else //use WCK_c and RDQS_c
        {
            WCKmode=2;
            RDQSmode=3;
        }
    ...
    for (u1FSP = FSP_0; u1FSP < FSP_MAX; u1FSP++)
    {
        switch(u1FSP)
        {
            ...
            case 2: {LP5_FSP_WR_or_OP(p, 2, 0, rank);break;}
            default:mcSHOW_DBG_MSG6(("[ERROR] Unexpected WORK_FSP under LP5 Initialization."));
        }
        ...
        //LP5_MRS(p, 24, MR17, rank);
        //LP5_MRS(p, 25, MR17, rank);
        ...
        mcSHOW_DBG_MSG(("u1MR14Value[%d][%d][%d] = 0x%x\n", p->channel, rank, u1FSP));
#endif
    }
...
void LP5_single_end_DRAMC_post_config(DRAMC_CTX_T *p)
{
    ...
    if((SE_MODE !=0) && (CAS_MODE ==LOWFREQ_CAS_MODE)) {
...
void LP5_DRAM_INIT(DRAMC_CTX_T *p)
{
    ...
    if (Get_MDL_Used_Flag()==NORMAL_USED)
    {
        ...
        {
            if((gu2MR0_Value[RANK] & 0x2)==0x2 && (gu2MR7_Value[RANK] & 0x10)==0x0 && ((gu2MR8_Value[RANK]>>2) & 0xF)==0x5)
                apply_cond = TRUE;
        }
