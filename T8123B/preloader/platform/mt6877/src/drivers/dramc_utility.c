...
#if ((__LP5_COMBO__ == TRUE) && (fcFOR_CHIP_ID == fcIPMv2))    
...
U8 GetEyeScanEnable(DRAMC_CTX_T * p, U8 get_type)
{
    ...
    DRAM_DFS_SRAM_SHU_T unterm_highest_SRAM = SRAM_SHU4;// LP4
    #if __LP5_COMBO__
    if (is_lp5_family(p))
        unterm_highest_SRAM = SRAM_SHU3;//LP5
    #endif
...
static S8 __ChangeRL_LP5(DRAMC_CTX_T *p, U8 u1Pre, U8 u1Cur, enum ReadOptionChg optChange)
{
    enum ReadLatencySet ePre = RL_SET0, eCur = RL_SET0;
    U8 u1ByteMode = FALSE;
    U8 u1CKR;
    U8 u1RLTblIdxPre = 0, u1RLTblIdxCur = 0;
    U8 u1RLCode;
    U8 rank;
    S8 nCK = 0;

    U8 u1RLTable[3][RL_SET_MAX][12] = {
        [0] = { /* ECC OFF, CKR = 2:1 */
            [RL_SET0] = {6, 8, 10, 12, 16, 18, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            [RL_SET1] = {6, 8, 10, 14, 16, 20, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            [RL_SET2] = {6, 8, 12, 14, 18, 20, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
        },
        [1] = { /* ECC OFF, CKR = 4:1 */
            [RL_SET0] = {3, 4, 5, 6, 8, 9, 10, 12, 13, 15, 16, 17},
            [RL_SET1] = {3, 4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 18},
            [RL_SET2] = {3, 4, 6, 7, 9, 10, 12, 14, 15, 17, 19, 20},
        },
        [2] = { /* ECC ON */
            [RL_SET0] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 12, 13, 15, 17, 18, 19},
            [RL_SET1] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 13, 14, 16, 18, 20, 21},
        }
    };

    u1RLCode = u1MR02Value[p->dram_fsp] & 0xf;
    u1CKR = p->CKR;

    /* For Mix mode, all dies follow x8 latency mode */
    for (rank = RANK_0; rank < p->support_rank_num; rank++) {
        if (p->dram_cbt_mode[rank] == CBT_BYTE_MODE1) {
            u1ByteMode = TRUE;
            break;
        }
    }
    ...
    if (optChange == READ_OPT_CHG_DBI) {
        ...
        if (u1Pre == ON) { /* DBI on, means ECC shall be OFFed TBL 0 or 1 */
            if (u1ByteMode == TRUE) {
                ePre = RL_SET2;
                eCur = RL_SET1;
            } else {
    ...
    if (optChange == READ_OPT_CHG_ECC) {
         ...
         */
        if (u1CKR == 2) /* ECC can only be enabled under 4:1 CKR */
            ASSERT(0);
    ...
    mcSHOW_DBG_MSG3(("%s: ChgOpt %d, PreCfg = %d, CurCfg = %d, RLCode = %d, u1ByteMode = %d, u1CKR = %d, ePre = %d, eCur = %d, u1RLTblIdxPre = %d, u1RLTblIdxCur = %d\n",
        __func__, optChange, u1Pre, u1Cur, u1RLCode, u1ByteMode, u1CKR, ePre, eCur, u1RLTblIdxPre, u1RLTblIdxCur));

    if (u1RLTable[u1RLTblIdxCur][eCur][u1RLCode] == 0xff || u1RLTable[u1RLTblIdxPre][ePre][u1RLCode] == 0xff)
        ASSERT(0);

    nCK = u1RLTable[u1RLTblIdxCur][eCur][u1RLCode] - u1RLTable[u1RLTblIdxPre][ePre][u1RLCode];
    mcSHOW_DBG_MSG3(("%s: RL changed = %d\n", __func__, nCK));

    return nCK;
}

static S8 __ChangeRL_LP4(DRAMC_CTX_T *p, U8 u1Pre, U8 u1Cur, enum ReadOptionChg optChange)
{
    enum ReadLatencySet ePre = RL_SET0, eCur = RL_SET0;
    U8 u1ByteMode = FALSE;
    U8 u1RLTblIdxPre = 0, u1RLTblIdxCur = 0;
    ...
    U8 u1RLTable[2][RL_SET_MAX][8] = {
        [0] = { /* Normal mode */
            [RL_SET0] = {6, 10, 14, 20, 24, 28, 32, 36}, /* RDBI DIS */
            [RL_SET1] = {6, 12, 16, 22, 28, 32, 36, 40}, /* RDBI EN */
        },
        [1] = { /* Byte mode */
            [RL_SET0] = {6, 10, 16, 22, 26, 32, 36, 40},
            [RL_SET1] = {6, 12, 18, 24, 30, 36, 40, 44},
        },
    };

    u1RLCode = u1MR02Value[p->dram_fsp] & 0x7;

    /* For Mix mode, 2 ranks may support different latency mode?? */
    for (rank = RANK_0; rank < p->support_rank_num; rank++) {
        if (p->dram_cbt_mode[rank] == CBT_BYTE_MODE1) {
            u1ByteMode = TRUE;
            break;
    ...
    mcSHOW_DBG_MSG3(("%s: ChgOpt %d, PreCfg = %d, CurCfg = %d, RLCode = %d, u1ByteMode = %d, ePre = %d, eCur = %d, u1RLTblIdxPre = %d, u1RLTblIdxCur = %d\n",
        __func__, optChange, u1Pre, u1Cur, u1RLCode, u1ByteMode, ePre, eCur, u1RLTblIdxPre, u1RLTblIdxCur));
...
S8 DramcCalculateRLChange(DRAMC_CTX_T *p, U8 u1Pre, U8 u1Cur, enum ReadOptionChg optChange)
{
    S8 nCK;
...
#if __LP5_COMBO__
    if (is_lp5_family(p)) {
        nCK = __ChangeRL_LP5(p, u1Pre, u1Cur, optChange);
    } else
#endif
    {
        nCK = __ChangeRL_LP4(p, u1Pre, u1Cur, optChange);
    }

    return nCK;
}

static void __Switch_RDBI_RECC_DramcRG(DRAMC_CTX_T *p, U8 option, enum ReadOptionChg optChange)
{
    if (optChange == READ_OPT_CHG_DBI) {
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ7), option, SHU_B0_DQ7_R_DMDQMDBI_SHU_B0);
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ7), option, SHU_B1_DQ7_R_DMDQMDBI_SHU_B1);
    } else if (optChange == READ_OPT_CHG_ECC) {
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_LP5_LECC), option, SHU_LP5_LECC_RXECC_EN);
    }
    mcSHOW_DBG_MSG(("DRAMC Read %s: %s\n", (optChange == READ_OPT_CHG_DBI) ? "DBI" : "ECC",
        (option == ON) ? "ON" : "OFF"));
}

static void __Switch_RDBI_RECC_DramMR(DRAMC_CTX_T *p, U8 option, enum ReadOptionChg optChange, DRAM_FAST_SWITH_POINT_T which_fsp)
{
    U8 mr_idx;
    U32 mr_fld;
    ...
    DramcMRWriteFldAlign(p, mr_idx, option, mr_fld, TO_MR);
...
S8 vSwitchRDBI_RECC_Setting(DRAMC_CTX_T *p, U8 option, enum ReadOptionChg optChange,
    enum ReadOptionChgTarget target, U8 u1ForCal_Use)
{
    ...
    if (optChange == READ_OPT_CHG_DBI) {
        pre_option = (u1MR03Value[p->dram_fsp] >> 6) & 0x1;
    }
#if __LP5_COMBO__
    else if (is_lp5_family(p) && optChange == READ_OPT_CHG_ECC) {
        pre_option = (u1MR22Value_LP5[p->dram_fsp] >> 6) & 0x1; /* Two bits. BIT[7] is RSV */
    }
#endif
    else {
        ASSERT(0);
    ...
    if (u1ForCal_Use) {
        if (target != READ_OPT_CHG_RG_ONLY) /* RL will be changed only when MR setting change */
            nCK = DramcCalculateRLChange(p, pre_option, option, optChange);
        ...
        if (target != READ_OPT_CHG_MR_ONLY)
            __Switch_RDBI_RECC_DramcRG(p, option, optChange);

        if (target != READ_OPT_CHG_RG_ONLY)
            __Switch_RDBI_RECC_DramMR(p, option, optChange, p->dram_fsp);
    } else {
        ...
        for (channel = CHANNEL_A; channel < p->support_channel_num; channel++) {
            ...
            if (target != READ_OPT_CHG_RG_ONLY) {
                ...
                #else
                    for (fsp = FSP_0; fsp < p->support_fsp_num; fsp++) {
...
void TA2_Test_Run_Time_HW_Presetting(DRAMC_CTX_T * p, U16 u2Row, U8 u1Bank, U16 u2Col, U32 len, TA2_RKSEL_TYPE_T rksel_mode)/* Should call after TA2_Test_Run_Time_Pat_Setting() */
{
    ...
    {
        uiRWOFOEN = 1;
#if !defined(SLT) || !ENABLE_EMI_LPBK_TEST
...
U8 DramcMRWriteBackup(DRAMC_CTX_T *p, U8 u1MRIdx, U8 u1Rank)
{
	...
	else if (u1MRIdx == 21 || u1MRIdx == 22 || u1MRIdx == 51) /* MR only in FSP0/FSP1 */
	{
		if (u1MRIdx == 22)
		{
		...
		else
		{
			...
			if(u1MRIdx == 21)
				...
				TransferReg.u4Fld = MR_BACKUP_04_RK0_FSP1_MRWBK_RK0_FSP1_MR51;
		}
	}
	else /* MR in FSP0/FSP1/FSP2 */
	{
		if (u1MRIdx <= 20)
		{
			if (u1MRIdx <= 10)
				...
				TransferReg.u4Addr = DRAMC_REG_MR_BACKUP_02_RK0_FSP1;
...
void DramcTriggerRTSWCMD(DRAMC_CTX_T *p, RTSWCMD_PARAM_T *param)
{
	...
	if (param->selector == RUNTIME_SWCMD_MRR) {
		param->mrr_value = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RTSWCMD_STATUS),
			RTSWCMD_STATUS_RTSWCMD_MRR_RESULT);
	}
#endif
...
#if __LP5_COMBO__ && ENABLE_RUNTIME_MRW_FOR_LP5
    if (is_lp5_family(p))
    {
...
void DramcBackupRegisters(DRAMC_CTX_T *p, U32 *backup_addr, U32 backup_num)
{
    ...
    {
        mcSHOW_ERR_MSG(("[DramcBackupRegisters] backup number over %d!!!\n", sizeof(u4gpRegBackupVlaue) / sizeof(*u4gpRegBackupVlaue)));
        ASSERT(0);
    }
...
void GetMR4RefreshRate(DRAMC_CTX_T *p, DRAM_CHANNEL_T channel)
{
    ...
    for(u1RandIdx = RANK_0; u1RandIdx < p->support_rank_num; u1RandIdx++)
    {
        if (u1RandIdx == RANK_0)
            ...
            u1RefreshRate = (U8)u4IO32ReadFldAlign((DRAMC_REG_MISC_STATUSA2) + (channel << POS_BANK_NUM), MISC_STATUSA2_REFRESH_RATE1);
...
#if (__ETT__) || defined(SLT) || defined(FOR_HQA_REPORT_USED_VCORE_SHMOO_TEST)
