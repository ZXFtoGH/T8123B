...
void dramc_sw_tx_tracking_upd(DRAMC_CTX_T *p, S8 *delta_pi, U8 shu_idx)
{
	U8 dq_mck = 0, dq_ui = 0;
	...
	U32 fld_value;
	U8 mck2ui = u1MCK2UI_DivShift(p);

	p->ShuRGAccessIdx = shu_idx;
...
void dramc_tx_retry_sw_trigger(DRAMC_CTX_T *p, U8 shu_idx)
{
	U8 response;
	U8 backup_shu = p->ShuRGAccessIdx;
	U8 backup_rk = p->rank;
	U8 rk_idx = 0;
	U16 mr_cnt0 = 0, mr_cnt1 = 0;
...
#if (__LP5_COMBO__ == TRUE)
	if (is_lp5_family(p)) {
		for (rk_idx = 0; rk_idx < p->support_rank_num; rk_idx++) {
			vSetRank(p, rk_idx);
	...
	p->ShuRGAccessIdx = backup_shu;
}

void dramc_sw_tx_tracking_cal(DRAMC_CTX_T *p, S8 *delta_pi, U8 shu_idx)
{
	...
	for (byte_idx = 0; byte_idx < p->data_width / DQS_BIT_NUMBER; byte_idx++) {
		mr_runtime = gu2DQSOscCnt[WCK2DQI][p->channel][p->rank][byte_idx];
		...
		if (mr_runtime >= mr_base) {
			...
			mcSHOW_DBG_MSG6(("SHU%u CH%d RK%d byte%d, mr_base=%x mr_runtime=%x mr_delta=%d dqsosc_inc =%d delta_pi=%d\n",
				p->ShuRGAccessIdx, p->channel, u1GetRank(p), byte_idx, mr_base, mr_runtime, delta_mr, dqsosc_inc, (delta_mr / dqsosc_inc)));
		} else {
			...
			mcSHOW_DBG_MSG6(("SHU%u CH%d RK%d byte%d, mr_base=%x mr_runtime=%x mr_delta=%d dqsosc_dec =%d delta_pi=%d\n",
				p->ShuRGAccessIdx, p->channel, u1GetRank(p), byte_idx, mr_base, mr_runtime, delta_mr, dqsosc_dec, (delta_mr / dqsosc_dec)));
		}
	}
...
void dramc_sw_tx_tracking(DRAMC_CTX_T *p, U8 shu_idx)
{
	U8 backup_shu = p->ShuRGAccessIdx;
	U8 rk_idx = 0;
	U8 backup_rk = u1GetRank(p);
	...
	#if (__LP5_COMBO__ == TRUE)
	if (is_lp5_family(p)) {
		dramc_tx_retry_sw_trigger(p, p->ShuRGAccessIdx);
	}
	else
	#endif
	{
	...
	for (rk_idx = RANK_0; rk_idx < p->support_rank_num; rk_idx++) {
		vSetRank(p, rk_idx);
		dramc_sw_tx_tracking_cal(p, (S8 *)&delta_pi[rk_idx], p->ShuRGAccessIdx);
		dramc_sw_tx_tracking_upd(p, (S8 *)&delta_pi[rk_idx], p->ShuRGAccessIdx);
	}
...
void dramc_runtime_sw_tx_tracking(DRAMC_CTX_T *p)
{
	...
	for(ch_idx = CHANNEL_A; ch_idx < p->support_channel_num; ch_idx++) {
		vSetPHY2ChannelMapping(p, ch_idx);
		shu_idx = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_DVFS_STATUS), DVFS_STATUS_OTHER_SHU_GP);
		p->ShuRGAccessIdx = shu_idx;

		#if (__LP5_COMBO__ == TRUE)
		if (is_lp5_family(p)) {
			if (cur_sram < LP5_TX_TRACKING_SRAM_TH)
				dramc_sw_tx_tracking(p, p->ShuRGAccessIdx);
		}
		else
		#endif
		{
...
void dramc_tx_retry_spm_trigger(DRAMC_CTX_T *p, U8 shu_idx)
{
	U8 backup_shu = p->ShuRGAccessIdx;
	U8 backup_rk = p->rank;
	U8 rk_idx = 0;
	...
	if (is_lp5_family(p)) {
		for (rk_idx = 0; rk_idx < p->support_rank_num; rk_idx++) {
			...
			mcSHOW_DBG_MSG6(("shu%d ch%d rk%d mr runtime value B0:0x%x, B1:0x%x\n",
				p->ShuRGAccessIdx,p->channel, p->rank, gu2DQSOscCnt[WCK2DQI][p->channel][p->rank][0], gu2DQSOscCnt[WCK2DQI][p->channel][p->rank][1]));
		}
		vSetRank(p, backup_rk);
	}
	#endif
...
void dramc_sw_tx_retry_cal(DRAMC_CTX_T *p, S8 *delta_pi, U8 shu_idx, U8 cur_sram, U8 next_sram)
{
	U16 mr_runtime = 0, mr_base = 0;
	...
	cur_ckr = get_runtime_ckr(p, cur_sram);
	next_ckr = get_runtime_ckr(p, next_sram);
...
void dramc_tx_retry_spm_off(DRAMC_CTX_T *p, U8 shu_idx)
{
	U8 backup_shu = p->ShuRGAccessIdx;
...
void dramc_sw_tx_retry(DRAMC_CTX_T *p, U8 shu_idx, U8 cur_sram, U8 next_sram, U8 dst_osc_fm)
{
	U8 backup_shu = p->ShuRGAccessIdx;
	U16 mr19_value = 0;
	U8 rtmr16;
	...
	#if (__LP5_COMBO__ == TRUE)
	if (is_lp5_family(p)) {
		mr19_value = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_1 + (cur_sram << 5),
			LPIF_MR_OP_STORE_SHU_0_1_MR_OP_SET_SHU_0_7);
		rtmr16 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHU_HWSET_VRCG), SHU_HWSET_VRCG_HWSET_VRCG_OP);
		mcSHOW_DBG_MSG6(("SHU%u CH%d RK%d before mr16 value =0x%x, mr_op = 0x%x, mr_wr = 0x%x\n",
			p->ShuRGAccessIdx, p->channel, p->rank, rtmr16, (rtmr16 >> 2) & 0x3, rtmr16 & 0x3));

		for (rk_idx = 0; rk_idx < p->support_rank_num; rk_idx++) {
			vSetRank(p, rk_idx);
			DramcModeRegWriteByRank(p, p->rank, 16, rtmr16);

			if (dst_osc_fm == 1) {
				mr19_value = (mr19_value & 0xff) | 0x10;
				DramcModeRegWriteByRank(p, p->rank, 19, mr19_value);
			} else {
		...
		dramc_tx_retry_spm_trigger(p, p->ShuRGAccessIdx);
		dramc_tx_retry_spm_off(p, p->ShuRGAccessIdx);

		for (rk_idx = 0; rk_idx < p->support_rank_num; rk_idx++) {
			vSetRank(p, rk_idx);
			dramc_sw_tx_retry_cal(p, (S8 *)&delta_pi[rk_idx], !p->ShuRGAccessIdx, cur_sram, next_sram);
			dramc_sw_tx_tracking_upd(p, (S8 *)&delta_pi[rk_idx], !p->ShuRGAccessIdx);
		}
		vSetRank(p, backup_rk);
	}
	...
	for (ch_idx = CHANNEL_A; ch_idx < p->support_channel_num; ch_idx++) {
		...
		if (is_lp5_family(p)) {
			if ((src_osc_fm != dst_osc_fm) && (cur_sram <= LP5_TX_TRACKING_SRAM_TH) && (dst_osc_fm != 3))
				dramc_sw_tx_retry(p, p->ShuRGAccessIdx, cur_sram, next_sram, dst_osc_fm);
		}
		else
		#endif
		{
...
void DqsGatingRetryShuffle(DRAMC_CTX_T *p)
{
    if(p->frequency >= 2133)
    {
        ...
        vIO32WriteFldMulti_All(DDRPHY_REG_MISC_SHU_DQSG_RETRY1,
                P_Fld(1, MISC_SHU_DQSG_RETRY1_RETRY_ALE_BLOCK_MASK) |
...
void DramcImpedanceTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DDRPHY_REG_MISC_IMP_CTRL1), 1, MISC_IMP_CTRL1_IMP_ABN_LAT_CLR);   //MP setting
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DDRPHY_REG_MISC_IMP_CTRL1), P_Fld(3, MISC_IMP_CTRL1_IMP_DIFF_THD)
                                                                //    | P_Fld(0, MISC_IMP_CTRL1_IMP_ABN_LAT_CLR) //MP setting
                                                                    | P_Fld(0, MISC_IMP_CTRL1_IMP_ABN_LAT_EN)); //MP setting
...
void DramcDQSPrecalculation_preset(DRAMC_CTX_T *p)
{
            ...
            for (rank = RANK_0; rank < p->support_rank_num; rank++) {
                ...
                if (byte_idx == 0) {
                    ...
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_INI_UIPI),
                        P_Fld(ui, SHU_R0_B1_INI_UIPI_CURR_INI_UI_B1) |
                        P_Fld(pi, SHU_R0_B1_INI_UIPI_CURR_INI_PI_B1));
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_NEXT_INI_UIPI),
                        P_Fld(ui, SHU_R0_B1_NEXT_INI_UIPI_NEXT_INI_UI_B1) |
                        P_Fld(pi, SHU_R0_B1_NEXT_INI_UIPI_NEXT_INI_PI_B1));
    ...
    if (is_lp5_family(p))
    {
        ...
        vIO32WriteFldAlign_All(DRAMC_REG_SHU_DQSOSC_SET0, prdcnt, SHU_DQSOSC_SET0_DQSOSC_PRDCNT); //MP setting
    }
	else
#endif
    {
...
void DramcDQSPrecalculation_enable(DRAMC_CTX_T *p)
{
    ...
    if(is_lp5_family(p))
    {
        vIO32WriteFldMulti_All(DRAMC_REG_FAST_RDDQS_SET0, P_Fld(1, FAST_RDDQS_SET0_FAST_RDDQS_SPM_MODE) |
                                                          ...
                                                          P_Fld(1, TX_RETRY_SET0_HWDQSOSC_MRR_RTSWCMD_INTV));

        vIO32WriteFldMulti_All(DRAMC_REG_TX_RETRY_SET0, P_Fld(1, TX_RETRY_SET0_HWDQSOSC_MRR_BY_RTSWCMD) |
                                                        P_Fld(1, TX_RETRY_SET0_HWDQSOSC_MPC_BY_RTSWCMD));

        vIO32WriteFldAlign_All(DRAMC_REG_DQSOSCR, 0x0, DQSOSCR_DQSOSCRDIS);

        for(u1Rank=0; u1Rank<p->support_rank_num; u1Rank++)
        {
            vSetRank(p, u1Rank);
            vIO32WriteFldAlign_All(DRAMC_REG_RK_DQSOSC, 0x1, RK_DQSOSC_DQSOSCR_RK0EN);
        }
        vSetRank(p, rank_bk);

        vIO32WriteFldAlign_All(DRAMC_REG_HWDRAMOSCMRR_RTSWCMD_CNT, 0xa, HWDRAMOSCMRR_RTSWCMD_CNT_HWDRAMOSCMRR_RTSWCMD_CNT);
        vIO32WriteFldAlign_All(DRAMC_REG_HWDQSOSC_RTSWCMD_CNT, 0x0, HWDQSOSC_RTSWCMD_CNT_HWDQSOSC_RTSWCMD_CNT);
...
void DramcHWGatingDebugOnOff(DRAMC_CTX_T *p, U8 u1OnOff)
{
    ...
    vIO32WriteFldMulti_All(DDRPHY_REG_MISC_STBCAL2,
        P_Fld(0, MISC_STBCAL2_STB_DBG_EN) |          //MP setting to save power
        P_Fld(0, MISC_STBCAL2_STB_PIDLYCG_IG) |      //MP setting to save power
        P_Fld(0, MISC_STBCAL2_STB_UIDLYCG_IG) |      //MP setting to save power
