...
extern U8 get_dqsien_p2s_latency(U8 p2s_ratio);
extern U8 get_oe_p2s_latency(U8 p2s_ratio);
...
void print_RX_path_rg_val_to_file( RX_path_attribution_T *tr, U8 ch_id, U8 group_id)
{
  FILE *f_group;
  FILE *f_val_CHA;
  FILE *f_val_CHB;
  ...
  f_val_CHA = fopen("f_gating_val_CHA", "a");
  f_val_CHB = fopen("f_gating_val_CHB", "a");
  fprintf(f_group, "%d\n", group_id);
  fclose(f_group);
  if (ch_id == 0) {
    BYTE_swap = 0;
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_UI_P0[1] : tr->RX_UI_P0[0]);
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_PI[1]    : tr->RX_PI[0])   ;
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_UI_P0[0] : tr->RX_UI_P0[1]);
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_PI[0]    : tr->RX_PI[1])   ;
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_UI_P0[3] : tr->RX_UI_P0[2]);
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_PI[3]    : tr->RX_PI[2])   ;
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_UI_P0[2] : tr->RX_UI_P0[3]);
    fprintf(f_val_CHA, "%d\n", BYTE_swap ? tr->RX_PI[2]    : tr->RX_PI[3])   ;
    fclose(f_val_CHA);
  }
  if (ch_id == 1) {
    BYTE_swap = 0;
  ...
  mcSHOW_DBG_MSG (("print f_group_num and f_gating_val done, executed from ch_id=%0d, group_id=%0d \n", ch_id, group_id));
}
#endif
...
{
  U8 BYTE_swap;
  U8 PINMUX_BYTE_SWAP_CHA, PINMUX_BYTE_SWAP_CHB;
  U8 RK, BYTE_MODE=0;
  U8 LP5_CAS_MODE, WCK_PI_RANK_DIFF;
  ...
  if ((LPDDR5_EN_S == 1) && ((LP5_RX_temp.BYTE_MODE[0]==1) || (LP5_RX_temp.BYTE_MODE[1]==1)))   
    BYTE_MODE = 1;  
  #endif  

  if ((LPDDR4_EN_S == 1) && ((LP4_RX_temp.BYTE_MODE[0]==1) || (LP4_RX_temp.BYTE_MODE[1]==1)))      
    BYTE_MODE = 1;
#else
  ...
  if(gu2MR0_Value[p->rank] == 0xffff)  //MR0 is not ready
  {
    ...
    if(p->femmc_Ready == 1)
    {
      BYTE_MODE = p->pSavetimeData->dram_cbt_mode;
    }
    else
    #endif
    {
  ...
  #endif
  {
    ...
    if(p->femmc_Ready == 0)
    {
      p->pSavetimeData->dram_cbt_mode = BYTE_MODE;
    }
    #endif
  }
#endif
...
#else
      RX_C->TDQSCK[0] = 650;//1212;
      RX_C->TDQSCK[1] = 650;//1212;
      RX_C->TDQSCK[2] = 650;//1557;
      RX_C->TDQSCK[3] = 650;//1557;
      RX_C->TDQSCK_min_SPEC = 650  ;	//ps
      RX_C->TDQSCK_max_SPEC = 1600 ;	//ps
#endif
  ...
  RX_C->gating_window_ahead_dqs   = 2;
  RX_C->aphy_slice_delay          = 11;
  RX_C->aphy_dtc_delay            = 100;
  RX_C->aphy_lead_lag_margin      = 16;
  RX_C->read_cmd_out              = 1;
  ...
  RX_C->dqsien_ser_latency = get_dqsien_p2s_latency(RX_C->dq_p2s_ratio);
  RX_C->oe_ser_latency     = get_oe_p2s_latency(RX_C->dq_p2s_ratio);
  RX_C->ca_ser_latency     = get_dq_ca_p2s_latency(RX_C->ca_p2s_ratio, A_D->CA_FULL_RATE);
  ...
  else if ((RX_C->DQSIENMODE == 7) | (RX_C->DQSIENMODE == 3) )
  {
  ...
  {     mcSHOW_DBG_MSG(("Error DQSIENMODE is %0d which is not considered \n", RX_C->DQSIENMODE));  }
  ...
  #if SA_CONFIG_EN
  RX_C->close_loop_mode     = (vGet_DDR_Loop_Mode(p) == CLOSE_LOOP_MODE)?1:0;
  #else
  RX_C->close_loop_mode     = ((RX_C->data_rate > 1866) || ((RX_C->data_rate==1866)&&(DUT_shu_p[group_id].APHY_CLOCK_MODE==0)));
  #endif
  ...
  RX_C->WCK_PI_RANK_DIFF     = WCK_PI_RANK_DIFF;
#if SA_CALIBRATION_EN
  RX_C->NEW_RANK_MODE		 = 1;
  RX_C->RDSEL_TRACK_EN       = 0;
  RX_C->RX_pipeline          = 1;
#else
...
void calculate_RX_path_rg_val (RX_path_config_T *RX_C, RX_path_attribution_T *RX_R)
{
  ...
  U8 LP5_READ_BASE_EN, NEW_RANK_MODE;
  ...
  RX_rdcmdout2rdcmdbus_by_ps    =  3 * RX_C->MCK_unit + RX_C->ca_default_delay * RX_C->ca_ui_unit + RX_C->ca_ser_latency * RX_C->ca_MCKIO_ui_unit /*+ RX_C->ca_default_PI * RX_C->ca_MCKIO_ps / RX_C->ca_ui_pi_ratio*/ ; // 3 is 1.5T pipe to APHY + 0.5T wait posedge then start P2S + 1T read_cmd_out  //MARK to hack
  if (DUT_p.BYPASS_TX_PIPE_MODE == 2/*NEW mode*/) {
    RX_rdcmdout2rdcmdbus_by_ps = RX_rdcmdout2rdcmdbus_by_ps - RX_C->MCK_unit; // Ahead 1T MCK for TXPIPE BYPASS enabled
    mcSHOW_ERR_MSG(("[calculate_RX_path_rg_val] DEBUG_INFO::RX_rdcmdout2rdcmdbus_by_ps decreased for BYPASS_TX_PIPE_MODE == 2 (NEW mode). \n"));
  }
  ///calculate DATLAT
  dq_2_1stDVI4CK = 5 * RX_C->dq_ui_unit * RX_C->dram_ui_ratio;
  RX_R->DATLAT_DSEL     = A_div_B_RU((RX_rdcmdout2rdcmdbus_by_ps + ( RX_C->cs2RL_start + RX_C->RL_max * 2 )* RX_C->CKR * RX_C->dq_ui_unit + tDQSCK_max + dq_2_1stDVI4CK), RX_C->MCK_unit) - RX_C->RDSEL_OFFSET - RX_C->TX_pipeline + RX_C->DATLAT_margin;
  ...
  RX_R->SHU_GW_THRD_NEG = 1; //de-feature  by sw  Lingyun sync from IPM V2
  ...
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" tDQSCK_min                        is %d\n", tDQSCK_min))        ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" tDQSCK_max                        is %d\n", tDQSCK_max))        ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" tDQSCK_shift_left_boundary        is %d\n", tDQSCK_shift_left_boundary))        ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" tDQSCK_shift_right_boundary       is %d\n", tDQSCK_shift_right_boundary))        ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2rdcmdbus_by_ps        is %d\n", RX_rdcmdout2rdcmdbus_by_ps))        ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2GATE_EN_min_by_ps     is %d\n", RX_rdcmdout2GATE_EN_min_by_ps))     ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2GATE_EN_max_by_ps     is %d\n", RX_rdcmdout2GATE_EN_max_by_ps))     ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2dqspre_min_by_ps      is %d\n", RX_rdcmdout2dqspre_min_by_ps))      ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2dq_end_max_by_ps      is %d\n", RX_rdcmdout2dq_end_max_by_ps))      ;
  //use DE algorithm for ASVA_1_3 mcSHOW_DBG_MSG ((" RX_rdcmdout2GATE_EN_end_min_by_ps is %d\n", RX_rdcmdout2GATE_EN_end_min_by_ps)) ;

  //RANKSEL releate
  //DRAMC rank_sel modify timing. 
  if( RX_R->DQSINCTL < 2 ) {
  ...
  RX_R->RANK_RXDLY_UPD_OFFSET = ((NEW_RANK_MODE == 0) && (DUT_p.BYPASS_TX_PIPE_MODE == 2/*New Mode*/)) ? 1 : 2; //redmine #45522
  RX_R->RX_IN_GATE_EN_PRE_OFFSET = RX_C->dramc_dram_ratio ==2 ? 1 : 2; //ref from init_golden_mini_freq_related_vseq.sv
  RX_R->RANKINCTL_RXDLY = (RX_R->DQSINCTL<=2) ? 0 : ((RX_C->RANK_RXDLY_UPDLAT_EN==1) ? (((RX_C->NEW_RANK_MODE==0)&&(RX_C->data_rate>=4266)) ? (RX_R->DQSINCTL-4):(RX_R->DQSINCTL-3)) : (RX_R->DQSINCTL-2)) ;
...
#if (__LP5_COMBO__ == TRUE) && (!SA_CONFIG_EN || !ENABLE_TX_WDQS)
  if (LPDDR5_EN_S && (RX_C->data_rate <= 3200) && (RX_C->WCK_PI_RANK_DIFF == 0) && (LP5_READ_BASE_EN == 0))
...
void RX_PATH_auto_gen_and_set(DRAMC_CTX_T *p,U32 ch_id, U32 group_id)
{
  ...
  RX_PIPE_BYPASS_EN = 0;
#else
  RX_PIPE_BYPASS_EN = DUT_p.RX_PIPE_BYPASS_EN;
#endif

  RX_path_config_T	   RX_C;
  RX_path_attribution_T    RX_R;
  get_RX_path_config       (p, &RX_C, ch_id, group_id) ;
  calculate_RX_path_rg_val (&RX_C, &RX_R)    ;
  print_RX_path_config(&RX_C);
  print_RX_path_rg_val(&RX_R);
#if FOR_DV_SIMULATION_USED
  print_RX_path_rg_val_to_file(&RX_R, ch_id, group_id);
#endif
  ...
  p->ShuRGAccessIdx = (group_id == 0) ? DRAM_DFS_REG_SHU0 : DRAM_DFS_REG_SHU1;
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_RANK_SELPH_UI_DLY),     P_Fld(RX_R.RANK_SEL_UI_P0                   , SHU_B0_RANK_SELPH_UI_DLY_RANKSEL_UI_DLY_B0));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_RANK_SELPH_UI_DLY),     P_Fld(RX_R.RANK_SEL_UI_P0                   , SHU_B1_RANK_SELPH_UI_DLY_RANKSEL_UI_DLY_B1));
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_MISC_RX_PIPE_CTRL),        P_Fld(RX_PIPE_BYPASS_EN                , SHU_MISC_RX_PIPE_CTRL_RX_PIPE_BYPASS_EN        ) );
    //
  ...
  {
    vSetRank(p, rank_sel);
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_MISC_SHU_RK_DQSCTL),           P_Fld(RX_R.DQSINCTL                                   , MISC_SHU_RK_DQSCTL_DQSINCTL ) );
    ...
    //ODT
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_RODTEN_UI_DLY),      P_Fld(RX_R.ODTEN_UI_P0[rank_sel * RX_C.rank_num]      , SHU_RK_B0_RODTEN_UI_DLY_RODTEN_UI_B0 ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_RODTEN_UI_DLY),      P_Fld(RX_R.ODTEN_UI_P0[rank_sel * RX_C.rank_num +1]   , SHU_RK_B1_RODTEN_UI_DLY_RODTEN_UI_B1 ));
    //CUR/NEXT_INIT for precal
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_INI_UIPI),           P_Fld(RX_R.RX_UI_P0[rank_sel * RX_C.rank_num]         , SHU_R0_B0_INI_UIPI_CURR_INI_UI_B0 )\
                                                                              | P_Fld(RX_R.RX_PI[rank_sel * RX_C.rank_num]            , SHU_R0_B0_INI_UIPI_CURR_INI_PI_B0 ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_INI_UIPI),           P_Fld(RX_R.RX_UI_P0[rank_sel * RX_C.rank_num +1]      , SHU_R0_B1_INI_UIPI_CURR_INI_UI_B1 )\
                                                                              | P_Fld(RX_R.RX_PI[rank_sel * RX_C.rank_num +1]         , SHU_R0_B1_INI_UIPI_CURR_INI_PI_B1 ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_NEXT_INI_UIPI),      P_Fld(RX_R.RX_UI_P1[rank_sel * RX_C.rank_num]	      ,	SHU_R0_B0_NEXT_INI_UIPI_NEXT_INI_UI_P1_B0 )\
                                                                              | P_Fld(RX_R.RX_UI_P0[rank_sel * RX_C.rank_num] 	      , SHU_R0_B0_NEXT_INI_UIPI_NEXT_INI_UI_B0 )\
                                                                              | P_Fld(RX_R.RX_PI[rank_sel * RX_C.rank_num]            , SHU_R0_B0_NEXT_INI_UIPI_NEXT_INI_PI_B0  ));
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_NEXT_INI_UIPI),      P_Fld(RX_R.RX_UI_P1[rank_sel * RX_C.rank_num+1]	      , SHU_R0_B1_NEXT_INI_UIPI_NEXT_INI_UI_P1_B1 )\
                                                                              | P_Fld(RX_R.RX_UI_P0[rank_sel * RX_C.rank_num+1]       , SHU_R0_B1_NEXT_INI_UIPI_NEXT_INI_UI_B1 )\
                                                                              | P_Fld(RX_R.RX_PI[rank_sel * RX_C.rank_num+1]          , SHU_R0_B1_NEXT_INI_UIPI_NEXT_INI_PI_B1  ));
  }
