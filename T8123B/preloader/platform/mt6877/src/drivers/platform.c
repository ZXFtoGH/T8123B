...
#include "lastpc.h"
#if ((TARGET_BOARD_PLATFORM == MT6877) && !CFG_FPGA_PLATFORM)
	#include "delsel.h"
#endif
...
#if CFG_GZ_SUPPORT
    ptr = (unsigned *)(unsigned long)gz_config_info_atag((boot_tag *)ptr);
    ptr = (unsigned *)(unsigned long)gz_config_boot_atag((boot_tag *)ptr);
    ptr = (unsigned *)(unsigned long)gz_config_platform_atag((boot_tag *)ptr);
#endif

#if CFG_GZ_PWRAP_ENABLE
    ptr = (unsigned *)(unsigned long)gz_config_pwrap_atag((boot_tag *)ptr);
#endif

#if defined(CFG_RKP_SUPPORT) && CFG_RKP_SUPPORT
    ptr = (unsigned *)(unsigned long)rkp_config_boot_atag((boot_tag *)ptr);
#endif
...
#ifdef MTK_5G_B_MT6360_MT6315
	if (subpmic_reboot == 1) {
		pal_log_err("%s subpmic: power exception(OC/PG/UV/OV)!\n", MOD);
		return BR_POWER_EXC;
...
{
    if(seclib_get_devinfo_with_index(73) & (1 << 23))
        DRV_WriteReg32(0x1000C404, 0x000F0000); /* Disable BJT rotation and Enable Low pass filter*/
    else
        DRV_WriteReg32(0x1000C404, 0x000E0000); /* Disable BJT rotation */
    pal_log_info("effuse register: 0x11F101B4 = 0x%x\n", seclib_get_devinfo_with_index(73));
...
{
    /*
     * PWM no need secure driver enable, but it is default secure enable for mt6877.
     * The code here is to disable secure bit to make pwm work in kernel.
     * This issue only happened in mt6877.
     * PWM is set as default secure disable for future chips.
     */
    DRV_WriteReg32(0x10048078, 0x00000020); /* Disable PWM global SEC, bit[5]=1'b1: non-secure */
    DRV_WriteReg32(0x100480B8, 0x80000020); /* Disable PWM0 SEC, bit[31]=1'b1: unlock, bit[5]=1'b1: non-secure */
    DRV_WriteReg32(0x100480F8, 0x80000020); /* Disable PWM1 SEC, bit[31]=1'b1: unlock, bit[5]=1'b1: non-secure */
    DRV_WriteReg32(0x10048138, 0x80000020); /* Disable PWM2 SEC, bit[31]=1'b1: unlock, bit[5]=1'b1: non-secure */
    DRV_WriteReg32(0x10048178, 0x80000020); /* Disable PWM3 SEC, bit[31]=1'b1: unlock, bit[5]=1'b1: non-secure */
}
...
void platform_pre_init(void)
{
    ...
    bandgap_filter_init();
    /* Disble pwm secure bit register */
    pwm_init();
...
/*In FPGA phase, dram related function should by pass*/
    ...
    #if ((TARGET_BOARD_PLATFORM == MT6877) && !CFG_FPGA_PLATFORM)
    cache_delsel_init( MT_CPU_DVFS_L, H240_730000KHz);
...
void platform_post_init(void)
{
    ...
    //support 2.7G segment
    {
        ...
        extern struct dvfs_vf_tbl (*vf_tbl_det)[NR_EEMSN_DET];
        if ((DRV_Reg32(0x11F10050) & 0xF) == 4) {
            int x, y, pi_vf_num;

            pi_vf_num = (*vf_tbl_det)[1].pi_vf_num;
            for (y = pi_vf_num -1; y >=0; y--) {
                (*vf_tbl_det)[1].pi_freq_tbl[y + 1] = (*vf_tbl_det)[1].pi_freq_tbl[y];
                (*vf_tbl_det)[1].pi_volt_tbl[y + 1] = (*vf_tbl_det)[1].pi_volt_tbl[y];
            }

            (*vf_tbl_det)[1].pi_vf_num = pi_vf_num + 1;
            (*vf_tbl_det)[1].pi_freq_tbl[0] = 2700;
            (*vf_tbl_det)[1].pi_volt_tbl[0] = (DRV_Reg32(0x11F105BC) & 0x0000FF00) >> 8;

            if (is_slt_load()) {
                // 2.7G aging margin 50mV, 8 steps
                (*vf_tbl_det)[1].pi_volt_tbl[0] = (*vf_tbl_det)[1].pi_volt_tbl[0] - 8;
            } else if (is_aging_flavor_load()) {
                // 2.7G aging margin 50mV, 8 steps
                (*vf_tbl_det)[1].pi_volt_tbl[0] = (*vf_tbl_det)[1].pi_volt_tbl[0] - 6;
            }

            pal_log_info("[PICACHU]support 2.7G segment\n");
            for (x = 0; x < NR_EEMSN_DET; x++) {
                pi_vf_num = (*vf_tbl_det)[x].pi_vf_num;
                for (y = 0; y < pi_vf_num; y++) {
                    pal_log_info("[PICACHU][%d][%d] freq=%d volt_step=%d(0x%x)\n",
                        x, y, (*vf_tbl_det)[x].pi_freq_tbl[y],
...
/******************************************************************************
* NOTE: This function is for the internal projects of MT6877.
*        -----------------------------------------
