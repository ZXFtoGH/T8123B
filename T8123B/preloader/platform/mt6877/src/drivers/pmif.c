...
static int pmif_config_mpu(int mstid)
{
	...
	struct pmif_mpu pmif_mpu_table[3] = {
		0x6, 0x0000, 0x16ff, 0xfffcf3c3, 0xffffffff,
		0x3, 0x0000, 0x13ff, 0xfffcf3c3, 0xffffffff,
		0x3, 0x1400, 0x16ff, 0xfffcf300, 0xffffffff,
	};
#else
	struct pmif_mpu pmif_mpu_table[4] = {
		0x9, 0x0000, 0x0226, 0xfff3cfc3, 0xffffffff,
		0x9, 0x022a, 0x02fc, 0xfff3ccc3, 0xffffffff,
		0x9, 0x0300, 0x05ff, 0xfff3cfc3, 0xffffffff,
		0x9, 0x0700, 0x0714, 0xfff7ccd7, 0xffffffff,
	};
#endif
	...
	if (en == TRUE) {
		...
		if ((mstid == SPMI_MASTER_0) || (mstid == SPMI_MASTER_1)) {
			DRV_WriteReg32(arb->base + arb->regs[PMIF_PMIC_ALL_INVLD_SLVID],
					0xffb6);
			/* enable rgn0~N, others protection */
...
static void pmif_spmi_enable(int mstid)
{
	...
	if (hw_bytecnt > 0) {
		bytecnt_per = hw_bytecnt << 24 | hw_bytecnt << 20 |
	...
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_3], 0x8);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_4], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_5], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_6], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_8], 0x4);
...
#endif /* end of #if PMIF_NORMAL_BOOT */
	/* Add latency limitation */
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_CNTER_EN], PMIF_CHAN_P_EN);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_0], 0x0);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_3], 0x3FF);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_4], 0x8);
	DRV_WriteReg32(arb->base + arb->regs[PMIF_LAT_LIMIT_LOADING], PMIF_CHAN_P_EN);
...
void pmif_spmi2_set_lp_mode(void)
{
	...
	PMIF_CRI("%s bringup sleep legacy mode\n", __func__);
#endif
}
