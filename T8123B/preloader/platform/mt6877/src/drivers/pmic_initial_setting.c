...
static struct pmic_setting init_setting[] = {
	...
	{0x3B0, 0x0, 0x300, 0},
	{0x790, 0x3, 0x3, 0},
	{0x796, 0x1D40, 0x3FFF, 0},
	{0x798, 0x1D40, 0x3FFF, 0},
	...
	{0xA3C, 0x1700, 0x1F00, 0},
	{0xA3E, 0x2A1, 0x7FFF, 0},
	{0xA40, 0x1442, 0x7FFF, 0},
	{0xA42, 0x1008, 0x7FFF, 0},
	{0xA44, 0x2506, 0x7FFF, 0},
	{0xA46, 0x29EF, 0x7FFF, 0},
	{0xA48, 0x5D8B, 0x7FFF, 0},
	{0xA4A, 0x4A6D, 0x7FFF, 0},
	{0xA4C, 0x42B1, 0x7FFF, 0},
	{0xA4E, 0x5A96, 0x7FFF, 0},
	{0xA50, 0xE0, 0x7FFF, 0},
	...
	{0x1412, 0x8, 0x8, 0},
	{0x148E, 0x18, 0x7F, 0},
	{0x1492, 0x909, 0x7F7F, 0},
	{0x150E, 0x7, 0x7F, 0},
	{0x1512, 0xC00, 0x7F00, 0},
	{0x158E, 0x18, 0x7F, 0},
	{0x1592, 0xF0F, 0x7F7F, 0},
	{0x160E, 0x18, 0x7F, 0},
	{0x1694, 0x2, 0x2, 0},
	{0x16A0, 0x0, 0x2, 0},
	{0x16AE, 0x50, 0x7F, 0},
	...
	{0x1E8E, 0x10, 0x7F, 0},
	{0x1E92, 0xF14, 0x7F7F, 0},
	{0x1EAA, 0x10, 0x8010, 0},
	{0x1EAE, 0x10, 0x7F, 0},
	{0x1EB2, 0xF14, 0x7F7F, 0},
...
static void wk_rf_power_setting(void)
{
	...
	/* VFE28 use VFE28 in MT6877 */
	pmic_config_interface(PMIC_RG_LDO_VFE28_OP_EN_SET_ADDR, 0x2,
			      PMIC_RG_LDO_VFE28_OP_EN_SET_MASK,
			      PMIC_RG_LDO_VFE28_OP_EN_SET_SHIFT);
	pmic_config_interface(PMIC_RG_LDO_VFE28_OP_MODE_ADDR, 0x5,
			      PMIC_RG_LDO_VFE28_OP_MODE_MASK,
...
void pmic_default_voltage(void)
{
	...
	struct mtk_regulator reg_vrf_dig = empty_regulator;
	int vproc1_vol = 650000;
	int vproc2_vol = 693750;
	int vsram_proc1_vol;
	int vsram_proc2_vol;
	...
	if (mtk_regulator_get("vrf_dig", &reg_vrf_dig))
		ret |= (1 << 11);
 ...
 */
	...
	mtk_regulator_set_voltage(&reg_vmodem, 800000, 800000);
#endif
	wk_rf_power_setting();
	...
	pal_log_info("vrf_dig = %d uV\n", mtk_regulator_get_voltage(&reg_vrf_dig));
...
static void mt6359_code_check(void)
{
	...
	code = (code & 0xE000) >> 13;
	if (code != 0x7) {
		pal_log_info("MT6359 PF_CODE=%d is wrong for MT6877\n", code);
		ASSERT(0);
	}
}
...
void pmic_init_setting(void)
{
	...
	pal_log_err("pmic init setting - wifi only\n");

	unsigned int val = 0;

	pmic_config_interface(PMIC_RG_BUCK_VCORE_HW1_OP_EN_ADDR, 0x0,
			      PMIC_RG_BUCK_VCORE_HW1_OP_EN_MASK,
			      PMIC_RG_BUCK_VCORE_HW1_OP_EN_SHIFT);
	pmic_read_interface(PMIC_RG_BUCK_VCORE_HW1_OP_EN_ADDR, &val,
			    PMIC_RG_BUCK_VCORE_HW1_OP_EN_MASK,
			    PMIC_RG_BUCK_VCORE_HW1_OP_EN_SHIFT);
	pal_log_err("wifi only. vcore control by sw: %x\n", val);
	pmic_config_interface(PMIC_RG_BUCK_VCORE_EN_ADDR, 0x0,
			      PMIC_RG_BUCK_VCORE_EN_MASK,
			      PMIC_RG_BUCK_VCORE_EN_SHIFT);
	pmic_config_interface(PMIC_RG_VGPUVCORE_PH2_OFF_ADDR, 0x1,
			      PMIC_RG_VGPUVCORE_PH2_OFF_MASK,
			      PMIC_RG_VGPUVCORE_PH2_OFF_SHIFT);
	pmic_read_interface(PMIC_RG_VGPUVCORE_PH2_OFF_ADDR, &val,
			    PMIC_RG_VGPUVCORE_PH2_OFF_MASK,
			    PMIC_RG_VGPUVCORE_PH2_OFF_SHIFT);
	pal_log_err("wifi only. merged phase vgpu ph2: %x\n", val);
	pmic_config_interface(PMIC_RG_BUCK_VMODEM_HW1_OP_EN_ADDR, 0x0,
			      PMIC_RG_BUCK_VMODEM_HW1_OP_EN_MASK,
			      PMIC_RG_BUCK_VMODEM_HW1_OP_EN_SHIFT);
	pmic_read_interface(PMIC_RG_BUCK_VMODEM_HW1_OP_EN_ADDR, &val,
			    PMIC_RG_BUCK_VMODEM_HW1_OP_EN_MASK,
			    PMIC_RG_BUCK_VMODEM_HW1_OP_EN_SHIFT);
	pal_log_err("wifi only. vmodem power control by sw: %x\n", val);
	...
	pmic_config_interface(PMIC_RG_BUCK_VPU_EN_ADDR, 0x0,
			      PMIC_RG_BUCK_VPU_EN_MASK,
			      PMIC_RG_BUCK_VPU_EN_SHIFT);
	pmic_read_interface(PMIC_RG_BUCK_VPU_EN_ADDR, &val,
			    PMIC_RG_BUCK_VPU_EN_MASK,
			    PMIC_RG_BUCK_VPU_EN_SHIFT);
	pal_log_err("wifi only. vpu power off: %x\n", val);
	pmic_config_interface(PMIC_RG_BUCK_VPU_HW0_OP_MODE_ADDR, 0x0,
			      0x7fff,
			      PMIC_RG_BUCK_VPU_HW0_OP_MODE_SHIFT);
	pmic_read_interface(PMIC_RG_BUCK_VPU_HW0_OP_MODE_ADDR, &val,
			    0x7fff,
			    PMIC_RG_BUCK_VPU_HW0_OP_MODE_SHIFT);
	pal_log_err("wifi only. vpu hw op mode: %x\n", val);
	pmic_config_interface(PMIC_RG_BUCK_VPA_CON0_CLR_ADDR, 0xFFFF,
			      PMIC_RG_BUCK_VPA_CON0_CLR_MASK,
			      PMIC_RG_BUCK_VPA_CON0_CLR_SHIFT);
#endif
	...
	pal_log_info("[PMIC] pmic_init_setting end. v210121\n");
