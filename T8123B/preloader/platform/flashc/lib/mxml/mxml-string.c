	       ...
	       ...)			/* I - Additional arguments as needed */
{
  ...
  va_start(ap, format);
  bytes = xml_vsnprintf(buffer, bufsize, format, ap);
  va_end(ap);
 		...
 		va_list    ap)		/* I - Pointer to additional arguments */
{
  ...
  size = 0; //init it, or warning appeared.
 /*
  ...
  */
  ...
  while (*format)
  {
	...
	*/
	...
	xml_snprintf(tptr, sizeof(tformat) - (tptr - tformat), "%d", width);
	tptr += strlen(tptr);
      }
      else
      {
        ...
        if (*format == '*')
	{
	  ...
	  */
	  ...
	  xml_snprintf(tptr, sizeof(tformat) - (tptr - tformat), "%d", prec);
	  tptr += strlen(tptr);
	}
	else
	{
...
#  else
  ...
  va_copy(apcopy, ap);
  if ((bytes = xml_vsnprintf(temp, sizeof(temp), format, apcopy)) <
      sizeof(temp))
  {
   /*
...
#  endif /* _WIN32 */
  ...
  */
  ...
  if ((buffer = calloc(1, bytes + 1)) != NULL)
    xml_vsnprintf(buffer, bytes + 1, format, ap);
