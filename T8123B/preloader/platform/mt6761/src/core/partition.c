...
struct mem_info item[] = {
    ...
    [PL_DRAMBUF] = {
        .start = PL_BOOTARG_BASE,
        .size  = PL_BOOTARG_MAX_SIZE, /* include 1MB boottags */
        .name  = "boottags"
    },
    [LK] = {
...
static void if_overlap_with_valid_items(u32 img_start, u32 img_end)
{
    ...
    reserved = mblock_query_reserved(item[SSPM].name, NULL);
    if (reserved) {
        item[SSPM].start = reserved->start;
        item[SSPM].size  = reserved->size;
    }
...
int part_load(blkdev_t *bdev, part_t *part, u32 *addr, u32 offset, u32 *size)
{
    ...
    if(!cur_part_name){
        pal_log_err("[%s]part_load:cur_part_name  error\n",MOD);
        return -1;
    }
