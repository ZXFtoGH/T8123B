...
void mt_pll_init(void)
{
	...
	*/
	...
	/* config other clocks */
	/* scp_sel=top_ap_clk_ctrl_f26m_ck(0), mm_sel=mmpll_ck(1), ddrphycfg_ck=don't touch,  mem_sel=DMPLL_CK(1),axi=syspll1_d7(1) */
	DRV_WriteReg32(CLK_CFG_0, 0x00010101);
...
unsigned int opp_freq[] = {
	[CPU_OPP0] = 0x80133D89, //2001MHZ
	[CPU_OPP1] = 0x80126EC4, //1917MHZ
	[CPU_OPP2] = 0x8011A276, //1834MHZ
};

void set_armpll_rate(enum cpu_opp opp)
{
	unsigned int temp;
	...
	temp = DRV_Reg32(MP0_PLL_DIV_CFG );
	DRV_WriteReg32(MP0_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x0 << 9)); // [10:9] muxsel: switch to 26M

	temp = DRV_Reg32(MP1_PLL_DIV_CFG);
	DRV_WriteReg32(MP1_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x0 << 9)); // [10:9] muxsel: switch to 26M

	temp = DRV_Reg32(BUS_PLL_DIV_CFG);
	DRV_WriteReg32(BUS_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x0 << 9)); // [10:9] muxsel: switch to 26M
	// mt_get_abist_freq(40);

	/* disable armpll */
	temp = DRV_Reg32(ARMPLL_CON0);
	DRV_WriteReg32(ARMPLL_CON0, temp & 0xFFFFFFFE);

	temp = DRV_Reg32(ARMPLL_CON3);
	DRV_WriteReg32(ARMPLL_CON3, temp | 0x2);

	temp = DRV_Reg32(ARMPLL_CON3);
	DRV_WriteReg32(ARMPLL_CON3, temp & 0xFFFFFFFE);

	/* set CPU to new freq */
	pal_log_info("ARMPLL switch to OPP %d\n", opp);
	DRV_WriteReg32(ARMPLL_CON1, opp_freq[opp]);

	/* enable armpll  */
	temp = DRV_Reg32(ARMPLL_CON3);
	DRV_WriteReg32(ARMPLL_CON3, temp | 0x1);

	temp = DRV_Reg32(ARMPLL_CON3);
	DRV_WriteReg32(ARMPLL_CON3, temp & 0xFFFFFFFD);
	...
	temp = DRV_Reg32(MP0_PLL_DIV_CFG);
	DRV_WriteReg32(MP0_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x01 << 9)); // [10:9] muxsel: switch to ARMPLL

	temp = DRV_Reg32(MP1_PLL_DIV_CFG);
	DRV_WriteReg32(MP1_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x01 << 9)); // [10:9] muxsel: switch to ARMPLL

	temp = DRV_Reg32(BUS_PLL_DIV_CFG);
	DRV_WriteReg32(BUS_PLL_DIV_CFG, (temp & 0xFFFFF9FF) | (0x01 << 9)); // [10:9] muxsel: switch to ARMPLL

	pal_log_info("ARMPLL_CON1: 0x%x\n", DRV_Reg32(ARMPLL_CON1));
}
