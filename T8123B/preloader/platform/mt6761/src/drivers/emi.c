...
void get_dram_freq_step(u32 dram_freq_step[])
{
	unsigned int i;
	unsigned int ddr_type = mt_get_dram_type_from_hw_trap();
	unsigned int defined_step_ddr3[DRAMC_FREQ_CNT] = {1866, 1534, 1466};
	unsigned int defined_step_ddr4[DRAMC_FREQ_CNT] = {3200, 2400, 1534};

#ifdef MTK_LP3_HIGHEST_DDR1600
	defined_step_ddr3[0] = 1600;
#endif
#ifdef MTK_LP4_HIGHEST_DDR2400
	defined_step_ddr4[0] = 2400;
#endif
#ifdef MTK_LP4X_HIGHEST_DDR2400
	defined_step_ddr4[0] = 2400;
#endif

	for (i = 0; i < DRAMC_FREQ_CNT; i++) {
		if (ddr_type == TYPE_LPDDR3) {
			dram_freq_step[i] = defined_step_ddr3[i];
		} else if (ddr_type == TYPE_LPDDR4 || ddr_type == TYPE_LPDDR4X) {
			dram_freq_step[i] = defined_step_ddr4[i];
		}
	}
}
...
#ifdef MTK_LP3_HIGHEST_DDR1600
    dramc_crit("[DRAMC] detect MTK_LP3_HIGHEST_DDR1600 for LP4X\n");
#endif
