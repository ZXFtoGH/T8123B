...
#if CFG_TEE_SECURE_MEM_TAG_FORMAT
#if (CFG_TEE_SUPPORT || (CFG_GZ_SUPPORT && CFG_GZ_ENABLE_BOOT))
#include <tz_tags.h>
#endif
#endif
...
static void atf_dram_reserve_mem(void){
	...
	/* page size alignment */
	u64 align_addr = SMALL_PAGE_SIZE;
	u64 atf_ramdump_addr;

	atf_dram_res = (u32)mblock_alloc(
		(u64) PL_ATF_MAX_SIZE, (u64)align_addr, limit_addr, PL_ATF_BASE, 0, "atf-reserved");
	if (atf_dram_res != PL_ATF_BASE) {
		DBG_ERR("atf_dram_res:0x%x, should:0x%x, assert!\n", atf_dram_res, PL_ATF_BASE);
		ASSERT(0);
	}

	limit_addr = (u64)ATF_RAMDUMP_BUFFER_LIMIT_ADDR;
	align_addr = (u64)ATF_RAMDUMP_BUFFER_ADDR_ALIGNMENT;

	/* TF-A Ramdump reserved */
	atf_ramdump_addr = (u32)mblock_alloc(
		(u64) ATF_RAMDUMP_BUFFER_SIZE, (u64)align_addr, limit_addr, 0, 0, "atf-ramdump-reserved");
}
...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    tee_log_buf_start = (u32)mblock_reserve_ext(&bootarg.mblock_info,
            (u64)(TEE_LOG_BUF_SIZE), (u64)0x200000, 0xC0000000, 0, "tee-log-reserved");
    if (!tee_log_buf_start)
        DBG_ERR("Fail to allocate tee log buffer: 0x%x, 0x%x\n",
                TEE_LOG_BUF_SIZE, 0x200000);

	tee_ree_share_buf_start = mblock_reserve_ext(&bootarg.mblock_info,
    (u64)TEEI_RESERVED_REE_SHAREMEM_SIZE, (u64)0x200000,
    limit_addr, 0, "tee_ree_reserved");

    if(!tee_ree_share_buf_start){
        DBG_ERR("Fail to allocate tee log buffer: 0x%x, 0x%x\n",
...
void trustzone_pre_init(void)
{
...
#if CFG_ATF_LOG_SUPPORT
    {
        ...
        atf_log_buf_start = mblock_alloc(
            (u64)ATF_LOG_BUFFER_SIZE, (u64)TEE_MEM_ALIGNMENT,
            limit_addr, 0, 0, "atf-log-reserved");
...
#if CFG_TEE_SECURE_MEM_TAG_FORMAT
#if (CFG_TEE_SUPPORT || (CFG_GZ_SUPPORT && CFG_GZ_ENABLE_BOOT))
    tz_secmem_static_reserve(SEC_MEM_RESERVED_MAX_SIZE);
#endif
#endif
}
...
#if CFG_TRUSTONIC_TEE_SUPPORT
    tbase_boot_param_prepare(TEE_BOOT_ARG_ADDR, tee_entry_addr, tee_secmem_size,
...
#if CFG_TEE_SECURE_MEM_TAG_FORMAT
#if (CFG_TEE_SUPPORT || (CFG_GZ_SUPPORT && CFG_GZ_ENABLE_BOOT))
    tz_secmem_tags_prepare(TEE_PARAMETER_ADDR, 0x0, 0x0);
#endif
#endif
