...
void UpdateHighestFreqInDFSTbl(DRAMC_CTX_T *p, DRAM_PLL_FREQ_SEL_T new_freq_sel)
{
	...
	cur_freq_sel = gFreqTbl[DRAM_DFS_SHUFFLE_1].freq_sel;
	if (cur_freq_sel == new_freq_sel)
		return;

	gFreqTbl[DRAM_DFS_SHUFFLE_1].freq_sel = new_freq_sel;

	mcSHOW_DBG_MSG(("[%s] update Highest Freq Done %d\n", __func__));
#endif
}
...
DRAM_STATUS_T DramcModeRegInit_LP4(DRAMC_CTX_T *p)
{
	 ...
	 */
	{
		...
		mcSHOW_DBG_MSG3(("nWR fixed to %d\n", (gFreqTbl[DRAM_DFS_SHUFFLE_1].frequency == 1600) ? 30:34));
	#if __ETT__
		if (u2DFSGetHighestFreq(p) != 1866 && u2DFSGetHighestFreq(p) != 1600)
		{
			mcSHOW_ERR_MSG(("Workaround only suitable for projects with %s as highest freq!\n", (gFreqTbl[DRAM_DFS_SHUFFLE_1].frequency == 1600) ? "LP4_DDR3200":"LP4_DDR3733"));
			while (1);
		}
	#endif
	}
...
{
    if (u1IsLP4Family(p->dram_type)) {
        return gFreqTbl[DRAM_DFS_SHUFFLE_1].frequency;
    }
#if ENABLE_LP3_SW
    else {
        return gFreqTbl_LP3[DRAM_DFS_SHUFFLE_1].frequency;
    }
#endif
}
...
void DramcDFS(DRAMC_CTX_T *p, int iDoDMA)
{
    ...
    ///TODO: DFS scenario
    if(targetFreqSel == gFreqTbl[DRAM_DFS_SHUFFLE_1].freq_sel)
    {
        DramcDFSDirectJump(p, 0);
    }
    else if(targetFreqSel == gFreqTbl[DRAM_DFS_SHUFFLE_2].freq_sel)
    {
        DramcDFSDirectJump(p, 1);
    }
    else if(targetFreqSel == gFreqTbl[DRAM_DFS_SHUFFLE_3].freq_sel)
    {
        DramcDFSDirectJump(p, 2);
    }

    if(targetFreqSel == gFreqTbl_LP3[DRAM_DFS_SHUFFLE_1].freq_sel)
    {
        DramcDFSDirectJump(p, 0);
    }
    else if(targetFreqSel == gFreqTbl_LP3[DRAM_DFS_SHUFFLE_2].freq_sel)
    {
        DramcDFSDirectJump(p, 1);
    }
    else if(targetFreqSel == gFreqTbl_LP3[DRAM_DFS_SHUFFLE_3].freq_sel)
    {
        DramcDFSDirectJump(p, 2);
    }
#if ENABLE_DRS
