...
void platform_set_boot_args_by_atag(unsigned *ptr)
{
  ...
  //----------------
    ...
    tags->u.emi_info.dram_type = mt_get_dram_type_from_hw_trap();
...
void platform_dump_boot_atag(void)
{
 ...
 //               pal_log_info("EMI common MTK_EMI_COMMON: %d\n", MTK_EMI_COMMON);
#ifdef MTK_EMI_COMMON
				pal_log_info("EMI common dram_type: %d\n", tags->u.emi_info.dram_type);
...
void platform_bootarg_reserve(void)
{
	...
	/* Reserve dram buffer */
	pl_bootarg_res = (u32)mblock_alloc((u64) PL_BOOTARG_MAX_SIZE,
                (u64)align_addr, limit_addr, PL_BOOTARG_BASE, 1, "pl-bootarg");
