...
unsigned int is_discrete_lpddr4x(void)
{
	unsigned int value = 0;
	#if( !__ETT__ && (FOR_DV_SIMULATION_USED==0))
	if (mt_get_gpio_in(GPIO_110) == 1){
		value= 1;
		dramc_crit("[DRAMC]gpio110 detect 1, highest freq 3200 \n");
	}else{
		value =0;
		dramc_crit("[DRAMC]gpio110 detect 0, highest freq 3600 \n");
	}
	#endif
	return value;
}
...
void get_dram_freq_step(u32 dram_freq_step[])
{
	unsigned int i;
	unsigned int ddr_type = mt_get_dram_type_from_hw_trap();
	unsigned int defined_step_ddr3[DRAMC_FREQ_CNT] = {1866, 1534, 1200};
	unsigned int defined_step_ddr4[DRAMC_FREQ_CNT] = {3600, 2400, 1534};
