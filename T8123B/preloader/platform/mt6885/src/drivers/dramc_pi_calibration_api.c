...
#elif  (CFG_DRAM_LOG_TO_STORAGE && !CFG_K_LOG_TO_STORAGE)
U8 gEye_Scan_color_flag = 0;
U8 gCBT_EYE_Scan_flag = 0;
U8 gCBT_EYE_Scan_only_higheset_freq_flag = 1;
	    ...
	    u8 pin_num)
{
	...
	*/
	...
	if (cnt == 0){
		mcSHOW_DBG_MSG(("[cbt_autok] Resp fail (time out) for "))
		switch (autok_mode) {
		case AUTOK_CS:
			mcSHOW_DBG_MSG(("CSTrain\n"));
			break;
		case AUTOK_CA:
			mcSHOW_DBG_MSG(("CATrain\n"));
			break;
		case AUTOK_DQS:
			mcSHOW_DBG_MSG(("DQSTrain\n"));
			break;
		}
	}
	...
	for (i = 0; i < pin_num; i++) {
		mcSHOW_DBG_MSG3(("cmp0[%d]=0x%x, cmp1[%d]=0x%x\n",
			    i, cmp0_array[i],
			    i, cmp1_array[i]));
	}
	...
	case AUTOK_CS:
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL1),
				P_Fld(1, CBT_WLEV_ATK_CTRL1_UICS_SW));
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL0),
	...
	case AUTOK_CA:
		...
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL1),
				P_Fld(1, CBT_WLEV_ATK_CTRL1_UICA_SW));
		vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_ATK_CTRL0),
	    ...
	    new_cbt_pat_cfg_t *pncm)
{
	...
	vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_CBT_WLEV_CTRL5),
			...
			P_Fld((is_lp5_family(p)? 6: 5), CBT_WLEV_CTRL5_NEW_CBT_CA_NUM) |
			P_Fld(7, CBT_WLEV_CTRL5_NEW_CBT_PAT_NUM) |
...
void CATrainingPosCal(DRAMC_CTX_T *p, U8* pu1DieNum, S8* ps1CAMacroDelay, S16 *ps1CACenterDiff, U8 u1CABitNum)
{
	U8 u1RankIdx, u1DieIdx, u1CAIdx, u1MacroIdx, u1MacroNum=0;
	...
	else //CA perbyte
	{
		for(u1MacroIdx =0; u1MacroIdx < u1MacroNum; u1MacroIdx++)
		{
			ps1CAMacroDelay[u1MacroIdx] = s1Intersect_min_byMacro[u1MacroIdx] + (s1Intersect_max_byMacro[u1MacroIdx] - s1Intersect_min_byMacro[u1MacroIdx])/2;
...
static U32 CBTDelayCSCompare(DRAMC_CTX_T *p, U32 uiDelay)
{
    ...
    //Set CS output delay
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_CA_CMD0), uiDelay, SHU_R0_CA_CMD0_RG_ARPI_CS);
	    ...
	    PASS_WIN_DATA_T *pwin)
{
	...
	if (!res) {
		mcSHOW_DBG_MSG3(("can't find zero window.\n"));
		return 0;
	} else {
		mcSHOW_DBG_MSG3(("find max zero window [%d, %d]\n",
			    win_left, win_right));
	    ...
	    u8 operation_frequency)
{
	...
	*/
	...
	#else
	CKEFixOnOff(p, CKE_WRITE_TO_ALL_RANK, CKE_DYNAMIC, CKE_WRITE_TO_ONE_CHANNEL);
	#endif
...
DRAM_STATUS_T CmdBusTrainingLP45(DRAMC_CTX_T *p, int autok)
{
...
#if FOR_DV_SIMULATION_USED == 1
	pi_step = 8; //for simulation speed up
#else
	pi_step = get_capi_step(p);
#endif
	...
	#endif
		{
			if ((Get_Vref_Calibration_OnOff(p) != VREF_CALI_OFF)||GetEyeScanEnable(p, 0))
			{
...
void LP5_ShiftWCKUI(DRAMC_CTX_T *p, S8 iShiftUI, BYTES_T eByteIdx)
{
...
void vSetLP5Dram_WCK2CK_WlevOnOff(DRAMC_CTX_T *p, U8 u1OnOff)
{
...
#if (fcFOR_CHIP_ID == fcA60868) || (fcFOR_CHIP_ID == fcPetrus) // And Margaux
...
// Note that below procedure is based on "ODT off"
    ...
    u32 bitmap31_0[DQS_NUMBER_LP4] = {0};
    u32 bitmap63_32[DQS_NUMBER_LP4] = {0};
    u8 idx = 0;
...
#endif
    {
        ...
        for (iDelay = s4DlyBegin; iDelay <= s4DlyEnd; iDelay += u1PIStep)
        {
    		...
    		if (isAutoK == TRUE)
            {
                ...
                {
                    u32 cmp0[DQS_NUMBER_LP4], cmp1[DQS_NUMBER_LP4];
                ...
                for (byte_i = 0; byte_i < DQS_NUMBER_LP4; byte_i++)
                {
...
void vResetDelayChainBeforeCalibration(DRAMC_CTX_T *p)
{
    ...
    for(u1RankIdx=RANK_0; u1RankIdx<RANK_MAX; u1RankIdx++)
    {
        ...
        vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_CA_TXDLY0,
        ...
        vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_CA_TXDLY1,
	    ...
	    vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_B0_TXDLY0, P_Fld(0, SHU_R0_B0_TXDLY0_TX_ARDQ0_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY0_TX_ARDQ1_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY0_TX_ARDQ2_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY0_TX_ARDQ3_DLY_B0));
        vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_B0_TXDLY1, P_Fld(0, SHU_R0_B0_TXDLY1_TX_ARDQ4_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY1_TX_ARDQ5_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY1_TX_ARDQ6_DLY_B0)
            | P_Fld(0, SHU_R0_B0_TXDLY1_TX_ARDQ7_DLY_B0));        
            
        vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_B1_TXDLY0, P_Fld(0, SHU_R0_B1_TXDLY0_TX_ARDQ0_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY0_TX_ARDQ1_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY0_TX_ARDQ2_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY0_TX_ARDQ3_DLY_B1));
        vIO32WriteFldMulti(DDRPHY_REG_SHU_R0_B1_TXDLY1, P_Fld(0, SHU_R0_B1_TXDLY1_TX_ARDQ4_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY1_TX_ARDQ5_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY1_TX_ARDQ6_DLY_B1)
            | P_Fld(0, SHU_R0_B1_TXDLY1_TX_ARDQ7_DLY_B1));
        vIO32WriteFldAlign(DDRPHY_REG_SHU_R0_B0_TXDLY3, 0x0, SHU_R0_B0_TXDLY3_TX_ARDQM0_DLY_B0);
        vIO32WriteFldAlign(DDRPHY_REG_SHU_R0_B1_TXDLY3, 0x0, SHU_R0_B1_TXDLY3_TX_ARDQM0_DLY_B1);
    }
...
//-------------------------------------------------------------------------
#if ZQ_SWCMD_MODE
DRAM_STATUS_T ZQ_SWCMD_MODE_Cal(DRAMC_CTX_T *p, U8 rank)
{
    U32 u4Response;
    U32 u4TimeCnt = TIME_OUT_CNT;
...
U8 u1GetGatingStartPos(DRAMC_CTX_T *p, U8 u1AutoK)
{
	...
	U8 use_enhance_rdqs)
{
	...
	for (dqs_i=0; dqs_i<(p->data_width/DQS_BIT_NUMBER); dqs_i++) {
		...
		mcSHOW_DBG_MSG(("best DQS%d dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
			...
			rxdqs_best_win.best_dqsien_dly_ui[dqs_i],
			rxdqs_best_win.best_dqsien_dly_pi[dqs_i]));
		mcDUMP_REG_MSG(("best DQS%d dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
			rxdqs_best_win.best_dqsien_dly_mck[dqs_i],
			rxdqs_best_win.best_dqsien_dly_ui[dqs_i],
			rxdqs_best_win.best_dqsien_dly_pi[dqs_i]));
		/* cc mark mcFPRINTF((fp_A60501,"best DQS%d dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
			rxdqs_best_win.best_dqsien_dly_mck[dqs_i],
			rxdqs_best_win.best_dqsien_dly_ui[dqs_i],
			rxdqs_best_win.best_dqsien_dly_pi[dqs_i]));
		*/
		...
		if (vGet_Div_Mode(p) == DIV8_MODE) {
			u1TX_dly_DQSgated = rxdqs_best_win.best_dqsien_dly_mck[dqs_i];
		} else { // LPDDR4 for 1:4 mode and LPDDR3
			u1TX_dly_DQSgated  = ((rxdqs_best_win.best_dqsien_dly_mck[dqs_i] << 1) |
				((rxdqs_best_win.best_dqsien_dly_ui[dqs_i] >> 2) & 0x1));
		}
	...
	for (dqs_i=0; dqs_i<(p->data_width/DQS_BIT_NUMBER); dqs_i++) {
		...
		mcSHOW_DBG_MSG(("best DQS%d P1 dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
			...
			rxdqs_best_win.best_dqsien_dly_ui_p1[dqs_i],
			rxdqs_best_win.best_dqsien_dly_pi_p1[dqs_i]));
		mcDUMP_REG_MSG(("best DQS%d P1 dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
			rxdqs_best_win.best_dqsien_dly_mck_p1[dqs_i],
			rxdqs_best_win.best_dqsien_dly_ui_p1[dqs_i],
			rxdqs_best_win.best_dqsien_dly_pi_p1[dqs_i]));
		/* cc mark mcFPRINTF((fp_A60501,"best DQS%d P1 dly(2T, 0.5T, PI) = (%d, %d, %d)\n", dqs_i,
		...
		*/
		...
		if (vGet_Div_Mode(p) == DIV8_MODE) {
			u1TX_dly_DQSgated  = rxdqs_best_win.best_dqsien_dly_mck_p1[dqs_i];
		} else { // LPDDR4 for 1:4 mode and LPDDR3
			u1TX_dly_DQSgated  = ((rxdqs_best_win.best_dqsien_dly_mck_p1[dqs_i] <<1)|((rxdqs_best_win.best_dqsien_dly_ui_p1[dqs_i] >>2)&0x1));
		}
	...
	/* Set PI */
#if 0//ENABLE_DDR800_OPEN_LOOP_MODE_OPTION -> No 0.5 UI after A60868
	if (u1IsDDR800PhaseMode(p) == TRUE) {
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_DQ0),
			rxdqs_best_win.best_dqsien_dly_pi[0] >> 4,
			SHU_R0_B0_DQ0_DA_ARPI_DDR400_0D5UI_RK0_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_DQ0),
			rxdqs_best_win.best_dqsien_dly_pi[1] >> 4,
			SHU_R0_B1_DQ0_DA_ARPI_DDR400_0D5UI_RK0_B1);
	} else
#endif
	{
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B0_DQSIEN_PI_DLY),
			rxdqs_best_win.best_dqsien_dly_pi[0],
			SHU_RK_B0_DQSIEN_PI_DLY_DQSIEN_PI_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_RK_B1_DQSIEN_PI_DLY),
			rxdqs_best_win.best_dqsien_dly_pi[1],
			SHU_RK_B1_DQSIEN_PI_DLY_DQSIEN_PI_B1);
	}

#if 0//RDSEL_TRACKING_EN
	//Byte 0
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_INI_UIPI),
		(ucbest_coarse_mck_backup[p->rank][0] << 3) | (ucbest_coarse_ui_backup[p->rank][0]),
		SHU_R0_B0_INI_UIPI_CURR_INI_UI_B0);//UI
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_INI_UIPI), rxdqs_best_win.best_dqsien_dly_pi[0],
		SHU_R0_B0_INI_UIPI_CURR_INI_PI_B0); //PI
	//Byte 1
	vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_INI_UIPI),
		(ucbest_coarse_mck_backup[p->rank][1] << 3) | (ucbest_coarse_ui_backup[p->rank][1]),
...
void RDDQCPinmuxWorkaround(DRAMC_CTX_T *p)
{
...
#endif
    const U8 uiLPDDR4_RDDQC_Mapping_POP[CHANNEL_NUM][16] =
    {
        //CH-A
        {
            8, 9, 11, 10, 13, 12, 15, 14,
    ...
    #endif
    {
        uiLPDDR_RDDQC_Mapping = (U8 *)uiLPDDR4_RDDQC_Mapping_POP[p->channel];
    }
                                              ...
                                              u8 isAutoK)
{
    ...
    U16 u1min_bit_by_vref[DQS_NUMBER], u1min_winsize_by_vref[DQS_NUMBER];
    U16 u1min_bit[DQS_NUMBER], u1min_winsize[DQS_NUMBER]={0};
    U8 u1CalDQMNum = 0;
        ...
        for (u2VrefLevel = u2VrefBegin; u2VrefLevel <= u2VrefEnd; u2VrefLevel += u2VrefStep)
        {
            if (u1VrefScanEnable == TRUE)
            {
                ...
                if (u1KnownVref[0] != 0xff && u1KnownVref[1] != 0xff)
                {
                    ...
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_PHY_VREF_SEL),
                            P_Fld(u1KnownVref[0], SHU_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B0) |
                            P_Fld(u1KnownVref[0], SHU_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B0));
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_PHY_VREF_SEL),
                            P_Fld(u1KnownVref[1], SHU_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B1) |
                            P_Fld(u1KnownVref[1], SHU_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B1));
                }
                else  //normal vref calibration
                {
                    ...
                    mcSHOW_DBG_MSG(("                         [Byte1]: %d\n", u2VrefLevel));
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_PHY_VREF_SEL),
                            P_Fld(u2VrefLevel, SHU_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B0) |
                            P_Fld(u2VrefLevel, SHU_B0_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B0));
                    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_PHY_VREF_SEL),
                            P_Fld(u2VrefLevel, SHU_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_LB_B1) |
                            P_Fld(u2VrefLevel, SHU_B1_PHY_VREF_SEL_RG_RX_ARDQ_VREF_SEL_UB_B1));
                }
            }
...
//#endif
                ...
                for (u1BitIdx = 0; u1BitIdx < p->data_width; u1BitIdx++)
                {
                    ...
                    FinalWinPerBit[u1BitIdx].win_size   = _dq_win_len;
                    FinalWinPerBit[u1BitIdx].last_pass  = _dq_win_begin + _dq_win_len - ((_dq_win_len == 0)? 0: 1);
                }

                // Debug info -- the largest 5 broken windows
                int jj = 0;
                ...
                if (dbg_en)
                {
                    ...
                    for (u1BitIdx = idx_start; u1BitIdx < idx_end; u1BitIdx++)
                    {
                        vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_RX_AUTOK_CFG1), u1BitIdx, MISC_RX_AUTOK_CFG1_RX_CAL_OUT_DBG_SEL);
                        for (jj = 0; jj < 5; jj++)
                        {
                            U32 _dbg_dq_win_begin = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_RX_AUTOK_STATUS0 + _RG_offset * jj),
                                    RX_AUTOK_STATUS0_RO_RX_CAL_OUT_WIN1_BEGIN_ARDQX);
                            U32 _dbg_dq_win_len = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_RX_AUTOK_STATUS0 + _RG_offset * jj),
                                    RX_AUTOK_STATUS0_RO_RX_CAL_OUT_WIN1_LEN_ARDQX);
                            mcSHOW_DBG_MSG2(("Dbg DQ %d: %d(%d)\n", u1BitIdx, _dbg_dq_win_begin, _dbg_dq_win_len));
                        }
                    }
                }
    ...
    for(u1ByteIdx=0; u1ByteIdx < p->data_width/DQS_BIT_NUMBER; u1ByteIdx++)
    {
        if (u1ByteIdx == 0)
        {
            tx_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), SHURK_SELPH_DQ0_TXDLY_DQ0)<<(u1MCK2UI+u1UI2PI)) +
                          ...
                          u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_DQ0), SHU_R0_B0_DQ0_SW_ARPI_DQ_B0)*(u1IsDDR800PhaseMode(p)==TRUE ? 8 : 1);
            ...
            tx_dqm_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ1), SHURK_SELPH_DQ1_TXDLY_DQM0)<<(u1MCK2UI+u1UI2PI)) +
                              ...
                              u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_DQ0), SHU_R0_B0_DQ0_SW_ARPI_DQM_B0)*(u1IsDDR800PhaseMode(p)==TRUE ? 8 : 1);
        }
        else
        {
            tx_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ0), SHURK_SELPH_DQ0_TXDLY_DQ1)<<(u1MCK2UI+u1UI2PI)) +
                          ...
                          u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_DQ0), SHU_R0_B1_DQ0_SW_ARPI_DQ_B1)*(u1IsDDR800PhaseMode(p)==TRUE ? 8 : 1);
            ...
            tx_dqm_pi_delay[u1ByteIdx] = (u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_SHURK_SELPH_DQ1), SHURK_SELPH_DQ1_TXDLY_DQM1)<<(u1MCK2UI+u1UI2PI)) +
                              ...
                              u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_DQ0), SHU_R0_B1_DQ0_SW_ARPI_DQM_B1)*(u1IsDDR800PhaseMode(p)==TRUE ? 8 : 1);
        }
    }
...
DRAM_STATUS_T Dramc8PhaseCal(DRAMC_CTX_T *p)
{
    ...
    U8 u1DqsLevel = 0xff, u18Ph_dly_loop_break = 0, u1Fail_loop_early_break = 0;
    U8 u1DqsienPI = 0;
...
void DramcSwImpedanceSaveRegister(DRAMC_CTX_T *p, U8 ca_freq_option, U8 dq_freq_option, U8 save_to_where)
{
    ...
    #elif (fcFOR_CHIP_ID == fcPetrus) // @Darren, confirm with ACD Kaffee
...
***/
    ...
    if (freq_region==0)
    {
        if (!(gDramcSwImpedanceResult[freq_region][DRVP] >= 1 && gDramcSwImpedanceResult[freq_region][DRVP] <= 0x16) ||
            !(gDramcSwImpedanceResult[freq_region][DRVN] >= 10 && gDramcSwImpedanceResult[freq_region][DRVN] <= 23) ||
            !(gDramcSwImpedanceResult[freq_region][ODTN] >= 3 && gDramcSwImpedanceResult[freq_region][ODTN] <= 0x13)) {
    ...
    {   // freq_region == 1
        ...
        //DRVN 10~30
        //ODTN 3~12
        if (!(gDramcSwImpedanceResult[freq_region][DRVP] >= 1 && gDramcSwImpedanceResult[freq_region][DRVP] <= 19) ||
            !(gDramcSwImpedanceResult[freq_region][DRVN] >= 10 && gDramcSwImpedanceResult[freq_region][DRVN] <= 30) ||
            !(gDramcSwImpedanceResult[freq_region][ODTN] >= 3 && gDramcSwImpedanceResult[freq_region][ODTN] <= 12)) {
            mcSHOW_ERR_MSG(("[DramcInit] SLT_BIN2\n"));
            while (1);
        }    
    }
...
*/
U8 DramcDutyDelayRGSettingConvert(DRAMC_CTX_T *p, S8 scDutyDelay,
	U8 *tDly)
{
	U8 tDelay;
...
void WCKDutyScan_SetWCKDelayCell(DRAMC_CTX_T *p, S8 *scDutyDelay)
{
    U8 u1ShuffleIdx = 0, u1DQSIdx, u1RankIdx = 0;
    U32 save_offset;
    U8 tDelay[2];
...
void DutyScan_SetDutyOffset(DRAMC_CTX_T *p, U8 u1B0, U8 u1B1, U8 u1CA)
{
...
void DutyScan_Offset_Calibration(DRAMC_CTX_T *p, U8 *u1OfB0, U8 *u1OfB1, U8 *u1OfCA)
{
    ...
    for(i=0; i<15; i++)
    {
		...
		//TODO
		// A60868 should use RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B0/B1/C0
		// But A60868 design has bug,
		// RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B0/B1/C0 were unconnected.
		// In other words, A60868 can't use dutycal.
		//
		// Pe_trus has ECO to fix above design bugs.
		//RG_TX_ARDQS0_DRVP_PRE_B0<2> = 1 to select AD_RX_ARDQ_O1_B0_BIT2 =RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B0
		//RG_TX_ARDQS0_DRVP_PRE_B1<2> = 1 to select AD_RX_ARDQ_O1_B1_BIT2 =RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B1
		//RG_TX_ARCLK_DRVP_PRE<2> = 1 to select AD_RX_ARCA2_O1 =RGS_RX_ARCA_DUTY_VCAL_CMP_OUT_C0

		//for Pe_trus begin
        //read_val_b0 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B0_BIT2);
        read_val_b0 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B0);
        read_val_b0 = (read_val_b0 >> 2) & 0x1; //AD_RX_ARDQ_O1_B0_BIT2, [2]

        //read_val_b1 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B1_BIT2);
        read_val_b1 = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B1);
        read_val_b1 = (read_val_b1 >> 2) & 0x1; //AD_RX_ARDQ_O1_B1_BIT2, [18]
...
//fra    U8 *str_who_am_I=str_clk_duty;
    ...
    if (k_type == DutyScan_Calibration_K_DQ) // K DQ
    {
		...
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3),
			1, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
			1, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
			1, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_CLK_SEL_CA);
    ...
    else if (k_type == DutyScan_Calibration_K_DQM) // K DQM
    {
		...
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3),
			3, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
			3, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
			3, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_CLK_SEL_CA);
    ...
    {
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3),
			0, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
			0, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
			0, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_CLK_SEL_CA);
    ...
    {
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B0_PHY3),
			2, B0_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B0);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_B1_PHY3),
			2, B1_PHY3_RG_RX_ARDQ_DUTY_VCAL_CLK_SEL_B1);
		vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_CA_PHY3),
			2, CA_PHY3_RG_RX_ARCA_DUTY_VCAL_CLK_SEL_CA);
                ...
                for(swap_idx=0; swap_idx<2; swap_idx++)
                {
					...
					for(i=6; i>=0; i--)
                    {
                        if (k_type == DutyScan_Calibration_K_CLK)
                        {
							...
							//TODO
							//RGS_RX_ARCA_DUTY_VCAL_CMP_OUT_C0
                        ...
                        else
                        {
                            ...
                            if (ucdqs_i==0)
                            {
                                ...
                                cal_out_value = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B0);
                                cal_out_value = (cal_out_value >> 2) & 0x1; //AD_RX_ARDQ_O1_B0_BIT2, [2]
                                //TODO
                                //RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B0
                            }
                            else
                            {
                                //cal_out_value = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B1_BIT2);
                                cal_out_value = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_MISC_AD_RX_DQ_O1), MISC_AD_RX_DQ_O1_AD_RX_ARDQ_O1_B1);
                                cal_out_value = (cal_out_value >> 2) & 0x1; //AD_RX_ARDQ_O1_B1_BIT2, [18]
                                //TODO
                                //RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B1
                            }
    ...
    *GPIO_GPIO_DOUT0  |= 0x04000000; //PAD_PWM_0,      // Lafi_te / round done
    ...
    for(ucdqs_i=0; ucdqs_i<ucdqs_i_count; ucdqs_i++)
    {
        ...
        else
        {
            ...
            #if defined(SLT)
//remove for YY mcSHOW_ERR_MSG(("[dramc] DRAM_FATAL_ERR_FLAG = 0x80000000\n"));            
...
void DramcNewDutyCalibration(DRAMC_CTX_T *p)
{
    ...
    DRAM_STATUS_T u2FailStatusByCh[CHANNEL_NUM]={0};
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (DDRPHY_REG_MISC_CG_CTRL0 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_MISC_CTRL4 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_MISC_CTRL3 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B0_DQ6 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B0_PHY3 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B0_DQ5 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B0_DQ0 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_R0_B0_DQ0 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B0_DLL_ARPI2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B1_DLL_ARPI2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_CA_DLL_ARPI2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B0_DQ13 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B1_DQ6 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B1_PHY3 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B1_DQ5 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B1_DQ0 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B0_DQ11 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B1_DQ11 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_CA_CMD11 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_B1_DQ13 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_CA_CMD6 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_CA_PHY3 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_CA_CMD5 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_CA_CMD0 + SHIFT_TO_CHB_ADDR),

#if (CHANNEL_NUM > 2)
        (DDRPHY_REG_MISC_CG_CTRL0 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_MISC_CTRL4 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_MISC_CTRL3 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B0_DQ6 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B0_PHY3 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B0_DQ5 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B0_DQ0 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_R0_B0_DQ0 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B0_DLL_ARPI2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B1_DLL_ARPI2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_CA_DLL_ARPI2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B0_DQ13 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B1_DQ6 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B1_PHY3 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B1_DQ5 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B1_DQ0 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B0_DQ11 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B1_DQ11 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_CA_CMD11 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_B1_DQ13 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_CA_CMD6 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_CA_PHY3 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_CA_CMD5 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_CA_CMD0 + SHIFT_TO_CHC_ADDR),

        (DDRPHY_REG_MISC_CG_CTRL0 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_MISC_CTRL4 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_MISC_CTRL3 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B0_DQ6 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B0_PHY3 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B0_DQ5 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B0_DQ0 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_R0_B0_DQ0 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B0_DLL_ARPI2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B1_DLL_ARPI2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_CA_DLL_ARPI2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B0_DQ13 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B1_DQ6 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B1_PHY3 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B1_DQ5 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B1_DQ0 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B0_DQ11 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B1_DQ11 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_CA_CMD11 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_B1_DQ13 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_CA_CMD6 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_CA_PHY3 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_CA_CMD5 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_CA_CMD0 + SHIFT_TO_CHD_ADDR),
#endif
    };
#endif
...
#endif
    {
        ...
        #endif
        {
            ...
            for(u1ChannelIdx=CHANNEL_A; u1ChannelIdx<p->support_channel_num; u1ChannelIdx++)
            {
                ...
                //for Pe_trus begin
                // Pe_trus has ECO to fix unconnected design bugs.
                //RG_TX_ARDQS0_DRVP_PRE_B0<2> = 1 to select AD_RX_ARDQ_O1_B0_BIT2 =RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B0
                //RG_TX_ARDQS0_DRVP_PRE_B1<2> = 1 to select AD_RX_ARDQ_O1_B1_BIT2 =RGS_RX_ARDQ_DUTY_VCAL_CMP_OUT_B1
                //RG_TX_ARCLK_DRVP_PRE<2> = 1 to select AD_RX_ARCA2_O1 =RGS_RX_ARCA_DUTY_VCAL_CMP_OUT_C0
                tmp = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ0), SHU_B0_DQ0_RG_TX_ARDQS0_DRVP_PRE_B0);
                tmp |= (1 << 2);
                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_DQ0), tmp, SHU_B0_DQ0_RG_TX_ARDQS0_DRVP_PRE_B0);
                
                tmp = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ0), SHU_B1_DQ0_RG_TX_ARDQS0_DRVP_PRE_B1);
                tmp |= (1 << 2);
                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B1_DQ0), tmp, SHU_B1_DQ0_RG_TX_ARDQS0_DRVP_PRE_B1);
                
                tmp = u4IO32ReadFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD0), SHU_CA_CMD0_RG_TX_ARCLK_DRVP_PRE);
                tmp |= (1 << 2);
                vIO32WriteFldAlign(DRAMC_REG_ADDR(DDRPHY_REG_SHU_CA_CMD0), tmp, SHU_CA_CMD0_RG_TX_ARCLK_DRVP_PRE);
                //for Pe_trus end
                ...
                if (is_lp5_family(p))
                    u2FailStatusByCh[u1ChannelIdx]= DutyScan_Calibration_Flow(p, DutyScan_Calibration_K_WCK, 1);
