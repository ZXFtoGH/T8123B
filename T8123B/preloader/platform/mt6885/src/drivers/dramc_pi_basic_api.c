...
void DramcHMR4_Presetting(DRAMC_CTX_T *p)
{
    ...
    //    vSetRank(p, RANK_0);
        ...
        if (p->support_rank_num == RANK_DUAL)
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_HMR4), 1, HMR4_HMR4_TOG_OPT);   // Read both rank0 and rank1
        else
            vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_HMR4), 0, HMR4_HMR4_TOG_OPT);   // Read rank0 only
...
// ignor SW impedance tracking when doing time profling
	...
	/* enable debug IRQ at last */
#if defined(ENABLE_DBG_2_0_IRQ) && defined(__ETT__)
	DramcDBGIRQOnOff(p, ENABLE);
	mcSHOW_DBG_MSG(("ENABLE_DBG_2_0_IRQ: ON\n"));
#else
	mcSHOW_DBG_MSG(("ENABLE_DBG_2_0_IRQ: OFF\n"));
#endif
...
void SPMTx_Track_Retry_OnOff(DRAMC_CTX_T *p, U8 shu_level, U8 onoff)
{
    ...
    if (shu_level == DRAM_DFS_SRAM_SHU9)
    {
        gIsddr800TxRetry = 1;
    }

    if ((gIsddr800TxRetry == 1) && (shu_level != DRAM_DFS_SRAM_SHU9)) //Need to do tx retry when DDR800 -> DDr1200
    {
        if (onoff == ENABLE)
        {
...
unsigned int DDRPhyFreqMeter()
{
    ...
    after_value = FMeter(ABIST_FMETER, 46);
