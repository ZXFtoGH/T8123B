...
int ufs_aio_init_mphy(struct ufs_hba *hba)
{
	...
	/* MPHY init */
	s = (readl(0x11C107A8) >> 7) & 0x1F;
	m = readl(0x11C10278) & 0xFFFFFFF1;
	if (m == 0x1 || (s >= 0x1 && s <= 0x5))
	{
		writel((readl(UFS_MPHY_BASE + 0x0058) & ~0xF0) | (0x3 << 4),
			UFS_MPHY_BASE + 0x0058);
		writel((readl(UFS_MPHY_BASE + 0x81A4) & ~0x7) | 0x4,
			UFS_MPHY_BASE + 0x81A4);
		writel((readl(UFS_MPHY_BASE + 0xA108) & ~0xC),
			UFS_MPHY_BASE + 0xA108);
		writel((readl(UFS_MPHY_BASE + 0xA108) & ~0x4000),
			UFS_MPHY_BASE + 0xA108);
		writel((readl(UFS_MPHY_BASE + 0xA11C) & ~0x300000) | (0x2 << 20),
			UFS_MPHY_BASE + 0xA11C);
		writel((readl(UFS_MPHY_BASE + 0xA120) & ~0xC0000000) | (0x2 << 30),
			UFS_MPHY_BASE + 0xA120);
		writel((readl(UFS_MPHY_BASE + 0xA180) & ~0x1FF0000) | (0x102 << 16),
			UFS_MPHY_BASE + 0xA180);
		writel((readl(UFS_MPHY_BASE + 0x8104) & ~0x1000),
			UFS_MPHY_BASE + 0x8104);
		writel((readl(UFS_MPHY_BASE + 0xA148) & ~0xE00) | (0x4 << 9),
			UFS_MPHY_BASE + 0xA148);
		writel((readl(UFS_MPHY_BASE + 0x8104) & ~0x10),
			UFS_MPHY_BASE + 0x8104);
	}
