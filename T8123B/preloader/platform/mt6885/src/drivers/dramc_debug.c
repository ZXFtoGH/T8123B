...
void print_HQA_measure_message(DRAMC_CTX_T *p)
{
    ...
    U8 shuffleIdx, local_channel_num = CHANNEL_NUM, shuffle_index;
...
void print_EyeScanVcent_for_HQA_report_used(DRAMC_CTX_T *p, U8 print_type, U8 u1ChannelIdx, U8 u1RankIdx, U8 u1ByteIdx, U8 *EyeScanVcent, U8 EyeScanVcentUpperBound, U8 EyeScanVcentUpperBound_bit, U8 EyeScanVcentLowerBound, U8 EyeScanVcentLowerBound_bit)
{
    ...
    if (print_type == 1 || print_type == 2)
    {
        ...
        for (u1BitIdx = u1BitIndex_Begin; u1BitIdx < u1BitIndex_End; u1BitIdx++)
        {
            ...
            if (print_type == 1)
            {
                Perbit_Window_percent = ((gEyeScan_WinSize[EyeScanVcent[1] / EYESCAN_GRAPH_RX_VREF_STEP][u1BitIdx] * gHQALOG_RX_delay_cell_ps_075V * p->frequency * 2) + (1000000 - 1)) / 1000000;
                Perbit_BestWindow_percent = ((gEyeScan_WinSize[EyeScanVcent[10 + u1BitIdx] / EYESCAN_GRAPH_RX_VREF_STEP][u1BitIdx] * gHQALOG_RX_delay_cell_ps_075V * p->frequency * 2) + (1000000 - 1)) / 1000000;
                Perbit_Window_Upperbond_percent = ((gEyeScan_WinSize[EyeScanVcent[3] / EYESCAN_GRAPH_RX_VREF_STEP][u1BitIdx] * gHQALOG_RX_delay_cell_ps_075V * p->frequency * 2) + (1000000 - 1)) / 1000000;
                Perbit_Window_Lowerbond_percent = ((gEyeScan_WinSize[EyeScanVcent[5] / EYESCAN_GRAPH_RX_VREF_STEP][u1BitIdx] * gHQALOG_RX_delay_cell_ps_075V * p->frequency * 2) + (1000000 - 1)) / 1000000;
...
for (u1ByteIdx = u1ByteIdx_Begin; u1ByteIdx <= u1ByteIdx_End; u1ByteIdx++)
{
    ...
    if ((print_type == 0 && u1CBTEyeScanEnable) || (print_type == 1 && u1RXEyeScanEnable) || (print_type == 2 && u1TXEyeScanEnable))
    {
        ...
        {
            if (print_type == 1||(print_type != 1&&p->dram_type==TYPE_LPDDR5))
            {
...
//            EYESCAN_LOG_Print_Prefix_String(); mcSHOW_DBG_MSG(("delay cell %d/100ps\n", u2gdelay_cell_ps_all[shuffleIdx][u1ChannelIdx]));
    ...
    {   U8 space_num;
        ...
        {
            u2DQDelayBegin = -80; //jimmy gEyeScan_Min[gFinalRXVrefDQ[u1ChannelIdx][u1RankIdx]][u1BitIdx][EyeScan_Index]-5; 
            u2DQDelayEnd = 200;
...
DRAM_CHANNEL_T channelIdx;
...
#if ENABLE_REFRESH_RATE_DEBOUNCE
    {
        ...
        for (channelIdx = CHANNEL_A; channelIdx < p->support_channel_num; channelIdx++)
        {
            ...
            mcSHOW_ERR_MSG(("\t[CH%d] RK0 B0 Int Status=0x%x\n", channelIdx, pre_refresh_rate[0]));
            mcSHOW_ERR_MSG(("\t[CH%d] RK1 B0 Int Status=0x%x\n", channelIdx, pre_refresh_rate[1]));
            mcSHOW_ERR_MSG(("\t[CH%d] RK0 B1 Int Status=0x%x\n", channelIdx, pre_refresh_rate[2]));
            mcSHOW_ERR_MSG(("\t[CH%d] RK1 B1 Int Status=0x%x\n", channelIdx, pre_refresh_rate[3]));  

            mcSHOW_ERR_MSG(("\t[CH%d] MISC_STATUSA_REFRESH_RATE=%d\n", channelIdx, u4IO32ReadFldAlign(DRAMC_REG_ADDR(DRAMC_REG_MISC_STATUSA), MISC_STATUSA_REFRESH_RATE)));
        }
 ...
 */
//-------------------------------------------------------------------------
void DramcRegDumpRange(DRAMC_CTX_T *p, U32 u4StartAddr, U32 u4EndAddr)
{
    U32 ii;
...
#endif
void DumpAoNonShuReg(DRAMC_CTX_T *p)
{
    mcSHOW_DBG_MSG(("\n\tCHA_DRAMC_AO_NONSHU_BASE\n"));
    mcDUMP_REG_MSG(("\n\tCHA_DRAMC_AO_NONSHU_BASE\n"));
    DramcRegDumpRange(p, DRAMC_REG_DDRCOMMON0, DRAMC_REG_SA_RESERVE);
    DramcRegDumpRange(p, DRAMC_REG_NONSHU_RSV, DRAMC_REG_DRAM_CLK_CTRL);
    DramcRegDumpRange(p, DRAMC_REG_RK_TEST2_A1, DRAMC_REG_RK_DQSOSC);
    DramcRegDumpRange(p, DRAMC_REG_WO_MR_BACKUP_00_RK0_FSP0, DRAMC_REG_WO_MR_BACKUP_07_RK0_FSP2);
    DramcRegDumpRange(p, DRAMC_REG_WO_MR_BACKUP_00_RK1_FSP0, DRAMC_REG_WO_MR_BACKUP_07_RK1_FSP2);
    ...
    DramcRegDumpRange(p, DDRPHY_REG_MISC_STBCAL, DDRPHY_REG_MISC_DQ_SE_PINMUX_CTRL1);
...
void DumpAoShuReg(DRAMC_CTX_T *p, U32 u4DramcShuOffset, U32 u4DDRPhyShuOffset)
{
    mcSHOW_DBG_MSG(("\n\tCHA_DRAMC_AO_SHU_BASE\n"));
    mcDUMP_REG_MSG(("\n\tCHA_DRAMC_AO_SHU_BASE\n"));
    DramcRegDumpRange(p, DRAMC_REG_SHURK_SELPH_DQ0 + u4DramcShuOffset, DRAMC_REG_SHURK_CKE_CTRL + u4DramcShuOffset);
    DramcRegDumpRange(p, DRAMC_REG_SHU_MATYPE + u4DramcShuOffset, DRAMC_REG_SHU_ACTIM7 + u4DramcShuOffset);
    

    mcSHOW_DBG_MSG(("\n\tCHA_PHY_AO_SHU_BASE\n"));
    mcDUMP_REG_MSG(("\n\tCHA_PHY_AO_SHU_BASE\n"));
    DramcRegDumpRange(p, DDRPHY_REG_SHU_PHYPLL0 + u4DDRPhyShuOffset, DDRPHY_REG_SHU_PLL2 + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_B0_TXDLY0 + u4DDRPhyShuOffset, DDRPHY_REG_SHU_B0_PHY_VREF_SEL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_B0_DQ0 + u4DDRPhyShuOffset, DDRPHY_REG_B0_SHU_MIDPI_CTRL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_B1_TXDLY0 + u4DDRPhyShuOffset, DDRPHY_REG_SHU_B1_PHY_VREF_SEL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_B1_DQ0 + u4DDRPhyShuOffset, DDRPHY_REG_B1_SHU_MIDPI_CTRL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_CA_TXDLY0 + u4DDRPhyShuOffset, DDRPHY_REG_SHU_CA_PHY_VREF_SEL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_CA_CMD0 + u4DDRPhyShuOffset, DDRPHY_REG_CA_SHU_MIDPI_CTRL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_MISC_SHU_RK_DQSCTL + u4DDRPhyShuOffset, DDRPHY_REG_MISC_SHU_RK_DQSCAL + u4DDRPhyShuOffset);
    DramcRegDumpRange(p, DDRPHY_REG_MISC_SHU_DRVING7 + u4DDRPhyShuOffset, DDRPHY_REG_MISC_SHU_CG_CTRL0 + u4DDRPhyShuOffset);
    //RO
    if(u4DDRPhyShuOffset==0)
    {
        DramcRegDumpRange(p, DDRPHY_REG_MISC_STBERR_ALL + u4DDRPhyShuOffset, DDRPHY_REG_MISC_DDR_RESERVE_STATE + u4DDRPhyShuOffset);
    }
    return;
}
...
void DumpNaoReg(DRAMC_CTX_T *p)
{
    ...
    #if (fcFOR_CHIP_ID == fcA60868)
    DramcRegDumpRange(p, DDRPHY_REG_DQSIEN_AUTOK_CA_RK1_STATUS0, DDRPHY_REG_DLINE_MON_TRACK_DBG);
...
void DumpAllChShu0RG(DRAMC_CTX_T *p)
{
    U8 chIdx, u1ch_backup=vGetPHY2ChannelMapping(p);
...
{    
    ...
    for(u1ChannelIdx=CHANNEL_A; u1ChannelIdx<p->support_channel_num; u1ChannelIdx++)
    {
        ...
        mcDUMP_REG_MSG(("\n[dumpRG]AllChannelAllShu-start Channel=%d\n", p->channel));
        DumpShuRG(p);
    }
            ...
            for (u1FSPIdx = 0; u1FSPIdx < p->support_fsp_num; u1FSPIdx++)
            {
                ...
                for (u1MRIdx = 0; u1MRIdx < MR_NUM; u1MRIdx++)
                {
                    ...
                    if (u2MRValue != 0xffff) //value changed
                    {
                        ...
                        DramcMRWriteBackup(p, u1MRIdx, p->rank);
...
void vApplyProgramSequence(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG(("Trigger Samsung Test mode after 5 sec\n"));
    mcDELAY_MS(5000); 
    while(1)
    {
        ProgramModeEnter(p);
...
void CKEFixOnOff_dbg(DRAMC_CTX_T *p, U8 u1RankIdx, CKE_FIX_OPTION option, CKE_FIX_CHANNEL WriteChannelNUM)
{
...
{
    U8 u1DelayCellOfst[16] = {0};
    U32 u4value = (i<8) ? 0 : 1;

    if(i<16)
    {
        u1DelayCellOfst[i] = 0xff;
    }
    else
    {
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_TXDLY0),
               P_Fld(u1DelayCellOfst[3], SHU_R0_B0_TXDLY0_TX_ARDQ3_DLY_B0)
             | P_Fld(u1DelayCellOfst[2], SHU_R0_B0_TXDLY0_TX_ARDQ2_DLY_B0)
             | P_Fld(u1DelayCellOfst[1], SHU_R0_B0_TXDLY0_TX_ARDQ1_DLY_B0)
             | P_Fld(u1DelayCellOfst[0], SHU_R0_B0_TXDLY0_TX_ARDQ0_DLY_B0));
     vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B0_TXDLY1),
               P_Fld(u1DelayCellOfst[7], SHU_R0_B0_TXDLY1_TX_ARDQ7_DLY_B0)
             | P_Fld(u1DelayCellOfst[6], SHU_R0_B0_TXDLY1_TX_ARDQ6_DLY_B0)
             | P_Fld(u1DelayCellOfst[5], SHU_R0_B0_TXDLY1_TX_ARDQ5_DLY_B0)
             | P_Fld(u1DelayCellOfst[4], SHU_R0_B0_TXDLY1_TX_ARDQ4_DLY_B0));
     vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_TXDLY0),
               P_Fld(u1DelayCellOfst[11], SHU_R0_B1_TXDLY0_TX_ARDQ3_DLY_B1)
             | P_Fld(u1DelayCellOfst[10], SHU_R0_B1_TXDLY0_TX_ARDQ2_DLY_B1)
             | P_Fld(u1DelayCellOfst[9], SHU_R0_B1_TXDLY0_TX_ARDQ1_DLY_B1)
             | P_Fld(u1DelayCellOfst[8], SHU_R0_B1_TXDLY0_TX_ARDQ0_DLY_B1));
     vIO32WriteFldMulti(DRAMC_REG_ADDR(DDRPHY_REG_SHU_R0_B1_TXDLY1),
               P_Fld(u1DelayCellOfst[15], SHU_R0_B1_TXDLY1_TX_ARDQ7_DLY_B1)
             | P_Fld(u1DelayCellOfst[14], SHU_R0_B1_TXDLY1_TX_ARDQ6_DLY_B1)
             | P_Fld(u1DelayCellOfst[13], SHU_R0_B1_TXDLY1_TX_ARDQ5_DLY_B1)
             | P_Fld(u1DelayCellOfst[12], SHU_R0_B1_TXDLY1_TX_ARDQ4_DLY_B1));
...
#else
        GetPhyPllFrequency(p); // for MD32 PST mode
#endif
...
#ifdef TA2_STRESS //Not call this API since it will clean W operation
        //TA2_Test_Run_Time_HW_Status(p);//Check TA2 status
#endif
