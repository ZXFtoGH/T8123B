...
//))= Row = 15-bit
  ...
  mt_emi_sync_write(EMI_APB_BASE+0x00000004,0x182e2d33); //3733 (1:8) r4  - r1  overhead
  mt_emi_sync_write(EMI_APB_BASE+0x00000008,0x0f251025); //3733 (1:8) r8  - r5  overhead
  mt_emi_sync_write(EMI_APB_BASE+0x0000000c,0x122a1027); //3733 (1:8) r12 - r9  overhead
  mt_emi_sync_write(EMI_APB_BASE+0x00000010,0x1a31162d); //3733 (1:8) r16 - r13 overhead
  ...
  mt_emi_sync_write(EMI_APB_BASE+0x00000030,0x37373a57); //3733 (1:8) r8  - r2  non-align overhead
  mt_emi_sync_write(EMI_APB_BASE+0x00000014,0x3f3f3c39); //3733 (1:8) r16 - r10 non-align overhead
...
void EMI_ESL_Setting2(void)
{
  ...
  // EMI_MPU_REG interrupt workaround for Pe_trus
...
void get_dram_freq_step(u32 dram_freq_step[])
{
	...
	unsigned int defined_step[DRAMC_FREQ_CNT] = {
		3733, 3068, 2366, 1866, 1534, 1200, 800};
...
#ifdef COMBO_MCP
static void dram_type_remapping(void)
{
	int i, index = 0;

	for(index = 0; index < num_of_emi_records; index++)
	{
		if(emi_settings[index].type == 0x0006)
		{
			emi_settings[index].type = 0x0202;
		}
		else
		{
			dramc_crit("[EMI] DRAM type setting ERROR \r\n");
			ASSERT(0);
		}
	}
}
