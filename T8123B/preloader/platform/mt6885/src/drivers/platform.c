...
#if CFG_APUSYS_CHK
apusys_status_t g_apusys_status = APUSYS_NORMAL;
#endif
...
#if CFG_APUSYS_CHK
    tags = (boot_tag *)ptr;
    tags->hdr.size = boot_tag_size(boot_tag_apusys_info);
    tags->hdr.tag = BOOT_TAG_APUSYS_INFO;
    tags->u.apusys_info.apusys_status = g_apusys_status;
    ptr += tags->hdr.size;
#endif
	...
	for (tags = (boot_tag *)(boottag); tags->hdr.size; tags = (boot_tag *)boot_tag_next(tags))
	{
        switch(tags->hdr.tag)
		{
			...
			case BOOT_TAG_META_COM:
				...
				break;
#if CFG_APUSYS_CHK
			case BOOT_TAG_APUSYS_INFO:
				pal_log_info("APUSYS_INFO: %d\n", tags->u.apusys_info.apusys_status);
				break;
#endif
...
{
    pal_log_info("bandgap filter: 0x11C101B4 = 0x%x\n", DRV_Reg32(0x11C101B4));
    if (DRV_Reg32(0x11C101B4) & 0x00400000) {
        if (DRV_Reg32(0x11C101B4) & 0x00200000)
            DRV_WriteReg32(0x1000C404, 0x000E0060); /* Disable BJT rotation and chopper, chopper phase=0 */
        else
            DRV_WriteReg32(0x1000C404, 0x000E0020); /* Disable BJT rotation and chopper, chopper phase=1 */
    } else
...
void platform_init(void)
{
    ...
    if (!ram_console_is_abnormal_boot() &&
        ...
        apu_check_2();
        BOOTING_TIME_PROFILING_LOG("apu chk 2");
    }
#endif
	...
	lvts_thermal_init();
        ...
        lvts_print_mcu_temp_1_io(700000);
        mdelay(10);

        lvts_print_mcu_temp_1_io(650000);
        mdelay(10);

        lvts_print_mcu_temp_1_io(575000);
	mdelay(10);

        lvts_compare_all_temp();
        lvts_restore_Vcore();
#endif
...
void platform_post_init(void)
{
    ...
    pmif_spmi_post_init(SPMI_MASTER_0);
...
/******************************************************************************
* NOTE: This function is for the internal projects of MT6885.
*        -----------------------------------------
