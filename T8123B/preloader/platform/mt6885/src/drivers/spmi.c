...
#if SPMI_RCS_SUPPORT
static int spmi_enable_rcs(struct spmi_device *dev);
int spmi_read_eint_sta(int mstid, u8 *slv_eint_sta);
#endif
#if SPMI_DEBUG
static int spmi_rw_test(struct spmi_device *dev);
static int spmi_buck_rw_test(int mstid);
...
static int spmi_config_io(int mstid, int driving)
{
	...
	if (driving == 2) {
		DRV_WriteReg32(IOCFG_BM_DRV_CFG2_CLR, 0x3f << 15);
	} else if (driving == 4) {
		DRV_WriteReg32(IOCFG_BM_DRV_CFG2_SET, 0x9 << 15);
		DRV_WriteReg32(IOCFG_BM_DRV_CFG2_CLR, 0x36 << 15);
	} else if (driving == 6) {
		DRV_WriteReg32(IOCFG_BM_DRV_CFG2_SET, 0x12 << 15);
		DRV_WriteReg32(IOCFG_BM_DRV_CFG2_CLR, 0x2d << 15);
	}
	DRV_WriteReg32(IOCFG_BM_EH_CFG1_CLR, 0x3ff << 6);
	DRV_WriteReg32(IOCFG_BM_IES_CFG1_CLR, 0x1 << 19);
	DRV_WriteReg32(IOCFG_BM_IES_CFG1_SET, 0x1 << 20);
	DRV_WriteReg32(IOCFG_BM_PD_CFG1_CLR, 0x3 << 19);
	DRV_WriteReg32(IOCFG_BM_PU_CFG1_CLR, 0x3 << 19);
	DRV_WriteReg32(IOCFG_BM_RDSEL_CFG0_CLR, 0xf << 28);
	DRV_WriteReg32(IOCFG_BM_SMT_CFG0_SET, 0x3 << 26);
	DRV_WriteReg32(IOCFG_BM_TDSEL_CFG1_CLR, 0xff << 24);
...
#else
	/* Should be enable when all PMICs use spmi
	 * DRV_WriteReg32(CLK_CFG_16_SET, 0x3);
	 */
#endif
...
static int spmi_buck_rw_test(int mstid)
{
	struct spmi_device *cpu_dev = get_spmi_device(mstid, SPMI_SLAVE_6);
	struct spmi_device *gpu_dev = get_spmi_device(mstid, SPMI_SLAVE_7);
	unsigned char wdata = 0, rdata = 0, i = 0;

	/* BUCK_CPU test */
	wdata = DEFAULT_VALUE_READ_TEST;
	spmi_ext_register_writel(cpu_dev, MT6315_PMIC_RG_BUCK_EFUSE_RSV_ADDR,
			&wdata, 1);

	spmi_ext_register_readl(cpu_dev, MT6315_PMIC_RG_BUCK_EFUSE_RSV_ADDR,
			&rdata, 1);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		SPMI_ERR("%s cpu_fail_r, slvid:%d rdata = 0x%x.\n",
			__func__, cpu_dev->slvid, rdata);
		return -EIO;
	} else
		SPMI_CRI("%s cpu_pass_r, slvid:%d\n", __func__,
				cpu_dev->slvid);

	/* BUCK_GPU test */
	wdata = DEFAULT_VALUE_READ_TEST;
	spmi_ext_register_writel(gpu_dev, MT6315_PMIC_RG_BUCK_EFUSE_RSV_ADDR,
			&wdata, 1);
	spmi_ext_register_readl(gpu_dev, MT6315_PMIC_RG_BUCK_EFUSE_RSV_ADDR,
			&rdata, 1);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		SPMI_ERR("%s gpu_fail_r, slvid:%d rdata = 0x%x.\n",
			__func__, gpu_dev->slvid, rdata);
		return -EIO;
	} else
		SPMI_CRI("%s gpu_pass_r, slvid:%d\n", __func__,
				gpu_dev->slvid);
...
int spmi_init(struct pmif *pmif_arb)
{
	...
	for (i = 0; i < spmi_device_cnt; i++) {
		...
		spmi_writel(pmif_arb->mstid, SPMI_MST_RCS_CTRL, 0x4);
		spmi_enable_rcs(&spmi_dev[i]);
#endif
...
#if SPMI_DEBUG
	ret = spmi_buck_rw_test(pmif_arb->mstid);
	if(ret) {
		SPMI_ERR("EIO err\n");
		return ret;
	}
