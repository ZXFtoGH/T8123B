	...
	unsigned int wdata, unsigned int *rdata)
{
	...
	return_value = wait_for_state_idle(wait_for_fsm_idle, TIMEOUT_WAIT_IDLE,
			(UINT32)(PMIF_SPI_PMIF_SWINF_0_STA + 0x40 * swinf_no),
			(UINT32)(PMIF_SPI_PMIF_SWINF_0_VLD_CLR + 0x40 * swinf_no), 0, pmifid);
	if (return_value != 0) {
	...
	if (write == 0) {
		...
		return_value =
			wait_for_state_ready(wait_for_fsm_vldclr, TIMEOUT_READ,
					(UINT32)(PMIF_SPI_PMIF_SWINF_0_STA + 0x40 * swinf_no), &reg_rdata, pmifid);
		if (return_value != 0) {
...
static signed int _pwrap_init_reg_clock(unsigned int regck_sel)
{
	...
	WRAP_WR32(IOCFG_TM_DRV_CFG0_CLR, 0x7 << 21);
	WRAP_WR32(IOCFG_TM_DRV_CFG0_SET, 0x1 << 21);
...
signed int pwrap_init(void)
{
	...
	WRAP_WR32(PMIF_SPI_PMIF_SPI_MODE_CTRL, WRAP_RD32(PMIF_SPI_PMIF_SPI_MODE_CTRL) |  0x3 << 12);
	WRAP_WR32(PMIF_SPI_PMIF_SPI_MODE_CTRL, WRAP_RD32(PMIF_SPI_PMIF_SPI_MODE_CTRL) & ~(0x1 << 11));
