...
static U32 u4ReserveRegBackupAddress[] =
{
    ...
    (DDRPHY_REG_MISC_SHU_CG_CTRL0),//pipe line DCM
    (DDRPHY_REG_MISC_SHU_CG_CTRL0 + SHIFT_TO_CHB_ADDR),//0x0DA4
    (DDRPHY_REG_MISC_SHU_CG_CTRL0 + SHIFT_TO_CHC_ADDR),//0x0DA4
    (DDRPHY_REG_MISC_SHU_CG_CTRL0 + SHIFT_TO_CHD_ADDR),//0x0DA4
...
void LP_Mode_DRM_Setting(void)
{
    DRAMC_CTX_T * p;
    p = &DramCtx_LPDDR4;
    U32 Temp = u4IO32ReadFldAlign(DDRPHY_REG_SHU_MISC_CLK_CTRL0, SHU_MISC_CLK_CTRL0_M_CK_OPENLOOP_MODE_SEL);//DDRPHY_AO_BASE_ADDRESS + 0x0D40[4]

    mcSHOW_DRM_MSG(("LP_Mode_DRM_Setting()\n"));
    ...
    if(Temp == 0x1)
    {
        ...
        Temp = u4IO32ReadFldAlign(DDRPHY_REG_SHU_MISC_CLK_CTRL0, SHU_MISC_CLK_CTRL0_M_CK_OPENLOOP_MODE_SEL);//DDRPHY_AO_BASE_ADDRESS + 0x0D40[4]
    ...
    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CG_CTRL0, 1, MISC_CG_CTRL0_CLK_MEM_SEL);//DDRPHY_AO_BASE_ADDRESS + 0x04EC[5:4]
    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CLK_CTRL, 0, MISC_CLK_CTRL_DVFS_MEM_CK_MUX_SEL_MODE);//DDRPHY_AO_BASE_ADDRESS + 0x0520[13:12]
...
//          (u4IO32ReadFldAlign(DDRPHY_REG_MISC_DMA_DEBUG0 + SHIFT_TO_CHD_ADDR, MISC_DMA_DEBUG0_SRAM_DONE) != 1));
...
#endif
    LoadShuffleSRAMtoDramc(p, 0, DRAM_DFS_SHUFFLE_1); //NEED_DE_NOTICE
    ...
    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CTRL1, 0, MISC_CTRL1_R_DMARPIDQ_SW);//SW force reload from AO to NAO
...
//    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_CG_CTRL0, 0x0, MISC_CG_CTRL0_RG_CG_PHY_OFF_DIABLE);   //0x04EC[10] 
    ...
    //*** WARNING***SA please change below write cmd to make sure write 2 MD32(Pe_trus)
    //*** WARNING***SA please change below write cmd to make sure write 1 MD32(Mar_gaux)
    vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_0, 1, LPIF_LOW_POWER_CFG_0_PHYPLL_MODE_SW);//==TODO  
...
//    ASVA5_8_CSCA_Pull_Down_EN();
    ...
    //DDRPhyFreqMeter();    mcSHOW_DRM_MSG(("End of Dramc_DDR_Reserved_Mode_setting \n"));
...
//    if(p->support_channel_num !=2) mcSHOW_DRM_MSG(("Set DRAMC Dual Channel"));//FOR_DV_SIMULATION_USE, never delet
//    p->support_channel_num = 2;//FOR_DV_SIMULATION_USE, never delet
    ...
    for(u1channel = CHANNEL_A; u1channel < p->support_channel_num; u1channel++)
    {
        ...
        DramcModeRegWriteByRank(p, u1set_mrsrk, 13, u4LP4_MR13FSP);
...
{
    vIO32Write4B_All(DRAMC_REG_TEST2_A0, 0x80100);
    vIO32Write4B_All(DRAMC_REG_TEST2_A3, 0x12000980);
    ...
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_02_RK0_FSP0, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_03_RK0_FSP0, 0xFFFFFF38);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_04_RK0_FSP0, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_00_RK0_FSP1, 0xFF321A66);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_01_RK0_FSP1, 0xFF1843FF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_02_RK0_FSP1, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_03_RK0_FSP1, 0xFFFFFF04);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_04_RK0_FSP1, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_02_RK1_FSP0, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_03_RK1_FSP0, 0xFFFFFF38);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_04_RK1_FSP0, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_00_RK1_FSP1, 0xFF321A66);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_01_RK1_FSP1, 0xFF1843FF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_02_RK1_FSP1, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_03_RK1_FSP1, 0xFFFFFF2C);
    vIO32Write4B_All(DRAMC_REG_WO_MR_BACKUP_04_RK1_FSP1, 0xFFFFFFFF);
    vIO32Write4B_All(DRAMC_REG_SELFREF_HWSAVE_FLAG, 0x0);
    vIO32Write4B_All(DDRPHY_REG_B0_RXDVS0, 0x20400203);
    vIO32Write4B_All(DDRPHY_REG_B1_RXDVS0, 0x20400203);
    vIO32Write4B_All(DDRPHY_REG_B1_PHY3, 0x0);
    vIO32Write4B_All(DDRPHY_REG_CA_LP_CTRL0, 0x1FFEA31E);
    vIO32Write4B_All(DDRPHY_REG_CA_CMD6, 0x200008);
    vIO32Write4B_All(DDRPHY_REG_CA_PHY3, 0x0);
    ...
    vIO32Write4B_All(DDRPHY_REG_MISC_CTRL1, 0x8100308C);
    vIO32Write4B_All(DRAMC_REG_SHURK_DQSOSC, 0x0);
    vIO32Write4B_All(DRAMC_REG_SHURK_DQSOSC_THRD, 0x10001);
    vIO32Write4B_All(DRAMC_REG_SHU_TX_SET0, 0x1C130535);
    ...
    vIO32Write4B_All(DDRPHY_REG_SHU_B0_DQ9, 0x0);
    vIO32Write4B_All(DDRPHY_REG_SHU_B1_DQ11, 0x60083);
    vIO32Write4B_All(DDRPHY_REG_SHU_B1_DQ9, 0x0);
    vIO32Write4B_All(DDRPHY_REG_SHU_CA_CMD10, 0xC);
    vIO32Write4B_All(DDRPHY_REG_SHU_CA_CMD11, 0x60002);
    vIO32Write4B_All(DDRPHY_REG_SHU_MISC_DRVING1, 0xCF03C0F0);
    vIO32Write4B_All(DDRPHY_REG_SHU_MISC_DRVING2, 0x8F03C0F0);
    vIO32Write4B_All(DDRPHY_REG_MISC_SHU_RX_CG_CTRL, 0x20004);
}
//----------------------------------------
// Auto Gen Code -- END
//----------------------------------------
...
void ETT_DRM(DRAMC_CTX_T *p) 
{
    ...
    //Patch_ReK(p);
    //MustStop = 1;
    //Init_DRAM(TYPE_LPDDR4X, CBT_R0_R1_NORMAL, NULL, NORMAL_USED);//CBT_R0_R1_NORMAL for DRAM H9HKNNNFBMMVAR
}
#endif
#endif
