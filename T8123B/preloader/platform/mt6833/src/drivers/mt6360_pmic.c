...
static const u8 ldo_ctrl_mask[] = { 0xff, 0xe0, 0x3f, 0xff, 0xff };

/* buck/ldo init value */
static const u8 sysuv_ctrl_init[] = { 0x25, 0xc0, 0x50 };
...
static const u8 buck2_ctrl_init[] = {
	0xa9, 0x80, 0x81, 0x6a, 0x20, 0x00, 0x02, 0x40,
};
#else
static const u8 buck2_ctrl_init[] = {
	0xa9, 0x80, 0x81, 0x6a, 0x00, 0x00, 0x02, 0x40,
};
#endif

static const u8 ldo6_ctrl_init[] = { 0x06, 0xc0, 0x02, 0xed, 0x44 };
static const u8 ldo7_ctrl_init[] = { 0x06, 0xc0, 0x02, 0xed, 0x44 };
#if 0
...
static int mt6360_pmic_init(void)
{
	...
	/* set buck2 FaultB un-mask */
	tmp[0] = 0xfb;
	tmp[1] = 0x0f;
	ret = i2c_write(&mt6360_pmu_i2c, tmp, 2);
	if (ret < 0) {
		dbg_print("%s: set buck2 FaultB un-mask fail\n", __func__);
		return ret;
	}
	...
	/* Update VRF13 vosel to 1.3V */
	ret = mt6360_update_bits(0x20, 0xFF, 0xC8);
	if (ret < 0) {
		dbg_print("%s: buck2 set fail\n",  __func__);
		return ret;
	}
#if 1
...
static int mt6360_pmic_is_hw_exist(void)
{
	...
	tmp = 0x0F;
	ret = i2c_write_read(&mt6360_pmu_i2c, &tmp, 1, 1);
	if (ret < 0 || (tmp & 0x60) != 0x60) {
		dbg_print("%s: ID:%x is wrong for MT6833\n", __func__, tmp);
		ASSERT(0);
	}
...
{
	int i, ret = 0;
	u8 seq_delay[4] = { 0x04, 0x00, 0x00, 0x02 };
...
int mt6360_pmic_i2c_probe(void)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (data != 0)
			g_subpmic_boot_status = 1; /* OC/PG/OV/UV */
	}
