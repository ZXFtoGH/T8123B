...
static void clear_vio_status(uint32_t pd_base, uint32_t vio_sta_num)
{
	uint32_t i, reg;

	/* SHIFT_STA */
	reg_write32(pd_base + PD_SHIFT_STA_OFFSET, STA_INIT);
	reg = reg_read32(pd_base + PD_SHIFT_STA_OFFSET);
	if (reg)
		MSG(DOM, "SHIFT_STA: 0x%x\n", reg);

	/* VIO_STA */
	for (i = 0; i < vio_sta_num; i++) {
		reg_write32(pd_base + PD_VIO_STA_OFFSET + 0x4 * i, STA_INIT);

		reg = reg_read32(pd_base + PD_VIO_STA_OFFSET + 0x4 * i);
		if (reg)
			MSG(DOM, "VIO_STA_%u: 0x%x\n", i, reg);
	}
}

static void devapc_infra_clear(void)
{
	clear_vio_status(DEVAPC_INFRA_PD_BASE, VIO_MASK_STA_NUM_INFRA);
}

static void devapc_peri_clear(void)
{
	clear_vio_status(DEVAPC_PERI_PD_BASE, VIO_MASK_STA_NUM_PERI);
}

static void devapc_peri2_clear(void)
{
	clear_vio_status(DEVAPC_PERI_PD2_BASE, VIO_MASK_STA_NUM_PERI2);
}

static void devapc_peri_par_clear(void)
{
	clear_vio_status(DEVAPC_PERI_PAR_PD_BASE, VIO_MASK_STA_NUM_PERI_PAR);
}
...
void device_APC_dom_setup(void)
{
	...
	devapc_peri_clear();
	devapc_peri2_clear();
	devapc_peri_par_clear();

	/* Set DEVAPC AO reg to SWd access only */
	reg_write32(DEVAPC_INFRA_AO_APC_CON,
			0x1 << APC_CON_SEN_BIT || 0x1 << APC_CON_APC_VIO_BIT);
	reg_write32(DEVAPC_PERI_AO_APC_CON,
			0x1 << APC_CON_SEN_BIT || 0x1 << APC_CON_APC_VIO_BIT);
	reg_write32(DEVAPC_PERI_AO2_APC_CON,
			0x1 << APC_CON_SEN_BIT || 0x1 << APC_CON_APC_VIO_BIT);
	reg_write32(DEVAPC_PERI_PAR_AO_APC_CON,
			0x1 << APC_CON_SEN_BIT || 0x1 << APC_CON_APC_VIO_BIT);
	reg_write32(DEVAPC_FMEM_AO_APC_CON,
			0x1 << APC_CON_SEN_BIT || 0x1 << APC_CON_APC_VIO_BIT);
...
/* INFRA MASTER DOMAIN settings */
	...
	MSG(DOM, "Domain Setup: PERI_PAR_AO (0x%x), (0x%x), (0x%x)\n",
