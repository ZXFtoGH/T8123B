...
inline void spmi_dump_mst_record_reg(int mstid)
{
	...
	SPMI_CRI("[0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x\r\n",
...
static unsigned int is_hw_monitor_mode = 0, chan = 0x2f7;
...
int spmi_pmif_dbg_init(struct pmif *arb)
{
	...
	if (is_hw_monitor_mode == 1) {
		...
		int_en = DRV_Reg32(arb->base + arb->dbgregs[PMIF_IRQ_EVENT_EN_4]);
		DRV_WriteReg32(arb->base + arb->dbgregs[PMIF_IRQ_EVENT_EN_4],
				int_en | (0x1 << 10));
