...
#define TEST_LOW_POWER_WITH_1_SEC_DELAY 0 //Add 1 second delay between suspend and resume to avoid APHY control PATH is not switched to SPM; EX: DMSUS not switched, pwr iso switched
...
void Emi_Sleep_Protect(DRAMC_CTX_T *p, unsigned int enable) {
    unsigned int tmp, timeout;

    if (enable == ENABLE) {
        mcSHOW_DBG_MSG3(("Emi_Sleep_Protect Enable\n"));

        vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_2, 0x1, LPIF_LOW_POWER_CFG_2_EMI_SLEEP_PROT_EN);

        timeout = 0xFF;
        do {
            tmp = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MISC_CTL_0, LPIF_MISC_CTL_0_EMI_SLEEP_IDLE);
            timeout--;
        } while (tmp == 0 && timeout > 0);

        if (timeout == 0) {
            mcSHOW_DBG_MSG3(("Emi_Sleep_Protect timeout\n"));
        }

    } else {
        mcSHOW_DBG_MSG3(("Emi_Sleep_Protect Disable\n"));
        vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_2, 0x0, LPIF_LOW_POWER_CFG_2_EMI_SLEEP_PROT_EN);
    }
}
