...
unsigned int is_discrete_lpddr4(void)
{
	...
	/* GPIO156, mcp lp4x: L, dis lp4x: H*/
	//value = ((*((volatile unsigned int *)GPIO_TRAPPING_REG) >> 23) & 0x1);
	value = mt_get_gpio_in(0x80000000 | 156);
	dramc_debug("%s : Value_gpio156=%d \n", __func__, value);
	return value;
}
...
void get_dram_freq_step(u32 dram_freq_step[])
{
	...
	unsigned int defined_step[DRAMC_FREQ_CNT] = {
		4266, 3200, 2400, 1866, 1600, 1200, 800,400};
...
void log_to_storage(const char c)
{
	...
	if (log_start && (!logen)) {
		...
		for (clr_count = 0; clr_count < 3072 ; clr_count++) //3M you can set it to 3072*7
