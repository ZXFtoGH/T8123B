...
#define DIS_PWR_CON		(SPM_BASE + 0x0354)
...
int spm_mtcmos_ctrl_dis(int state)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write(INFRA_TOPAXI_PROTECTEN_MM_2_SET, DIS_PROT_STEP1_1_MASK);
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTEN_MM_2_STA1) & DIS_PROT_STEP1_1_ACK_MASK) != DIS_PROT_STEP1_1_ACK_MASK) {
		}
#endif
...
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTEN_MM_2_STA1) & DIS_PROT_STEP2_2_ACK_MASK) != DIS_PROT_STEP2_2_ACK_MASK) {
		}
#endif
		...
		spm_write(INFRA_TOPAXI_PROTECTEN_MM_2_CLR, DIS_PROT_STEP1_1_MASK);
#ifndef IGNORE_MTCMOS_CHECK
		/* Note that this protect ack check after releasing protect has been ignored */
#endif
