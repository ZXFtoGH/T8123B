...
#if defined(MTK_MSDC_BRINGUP_DEBUG)
	if (id == 0) {
		msdc_pr_err("MSDC0 MODE8[%x] = 0x%X\tshould: 0x1111111?\n",
			MSDC0_GPIO_MODE8, MSDC_READ32(MSDC0_GPIO_MODE8));
		msdc_pr_err("MSDC0 MODE9[%x] = 0x%X\tshould: 0x???11111\n",
		...
		msdc_pr_err("MSDC0 SMT   [%x] = 0x%X\tshould: 0x????3FFC\n",
...
void msdc_set_driving_by_id(u32 id, struct msdc_cust *msdc_cap, bool sd_18)
{
	if (id == 0) {
		...
		MSDC_SET_FIELD(MSDC0_GPIO_DRV0_ADDR_0, MSDC0_DRV0_DAT5_MASK,
			msdc_cap->dat_drv);
		MSDC_SET_FIELD(MSDC0_GPIO_DRV0_ADDR_1, MSDC0_DRV0_DAT6_MASK,
			msdc_cap->dat_drv);
		MSDC_SET_FIELD(MSDC0_GPIO_DRV0_ADDR_1, MSDC0_DRV0_DAT7_MASK,
			msdc_cap->dat_drv);
