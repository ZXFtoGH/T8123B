...
void print_HQA_measure_message(DRAMC_CTX_T *p)
{
    ...
    if (gHQALog_flag == 1)
    {
        ...
        HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("fsp = %u\n", p->dram_fsp));
        HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("odt_onoff = %u\n", p->odt_onoff));
        HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("byte_mode = %u\n", p->dram_cbt_mode[p->rank]));
        HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("DivMode = %u\n", vGet_Div_Mode(p)));
    }
