...
#define SSPM_CFGREG_AHB_M1_ADDR     (SSPM_SW_RSTN + 0x74)
    ...
    DRV_SetReg32(0x100053f0, 0x66600000);
    DRV_SetReg32(0x10005400, 0x00000066);
    //Setup GPIO pinmux for SSPM UART2
    DRV_SetReg32(0x10005370, 0x40000000);
    DRV_SetReg32(0x10005380, 0x00000004);
#endif
...
static int bldr_load_mcupm_part(blkdev_t *bdev, u32 *addr, u32 *size)
{
    ...
    part_t *part;
    ptimg_hdr_t *hdr;
    region_info_t *region_info = NULL;
	...
	switch (hdr->id) {
		...
		case PT_ID_MCUPM_XFILE:
			...
			break;
		default:
			pal_log_err("%s Warning: Ignore unknow SSPM image_%d\n", MOD, hdr->id);
			break;
	}
	img += ROUNDUP(hdr->img_size, hdr->align);
	hdr = (ptimg_hdr_t *) img;
    ...
    mcupm_ldimg = (unsigned char *)tmp_addr;
    mcupm_ldsize = (unsigned int)MCUPM_LOADER_SIZE;
    mcupm_img = (unsigned char *)((unsigned int ) tmp_addr + (unsigned int)MCUPM_IMG_OFS);
    mcupm_imgsize = (unsigned int)mcupm_pmsize - (unsigned int)MCUPM_IMG_OFS;

    DRV_WriteReg32(MCUPM_SW_RSTN, 0x0);
    mcupm_memcpy((unsigned char *) CFG_MCUPM_MEMADDR, mcupm_ldimg, mcupm_ldsize);

    //set region info
    region_info = (region_info_t *) (CFG_MCUPM_MEMADDR + 0x4);
    region_info->loader_start = (unsigned int)mcupm_ldimg;
    region_info->loader_size = mcupm_ldsize;
    region_info->fw_start = (unsigned int )mcupm_img;
    region_info->fw_size = mcupm_imgsize - RESERVED_SRAM_SIZE;
    region_info->rdmp_start = (unsigned int ) tmp_addr + *size;
    region_info->rdmp_size = MCUPM_TCM_SZ;

    mcupm_dbg("region info loader start = %x, size = %x, fw start = %x, size = %x, rdmp_start = %x\n", region_info->loader_start, 
        region_info->loader_size, region_info->fw_start, region_info->fw_size, region_info->rdmp_start);
    ...
    pal_log_info("%s MCUPM start with %s boot\n", MOD, DRV_Reg32(MCUPM_SRAM_GPR1) ? "abnormal" : "normal");
    DRV_WriteReg32(MCUPM_SRAM_GPR14, (unsigned int) region_info->rdmp_start);
    DRV_WriteReg32(MCUPM_SRAM_GPR15, (unsigned int) region_info->rdmp_size);

    mcupm_dbg("mcupm: ram_console_is_abnormal_boot = 0x%x\n", ram_console_is_abnormal_boot());
...
#define MCUPM_CFGREG_SW_RSTN_DMA_BUSY_MASK  (0x1 << 1)
    ...
    mcupm_dbg("mcupm: MCUPM_SRAM_GPR0: 0x%x, 0x%x\n", MCUPM_SRAM_GPR0, DRV_Reg32(MCUPM_SRAM_GPR0));
    mcupm_dbg("mcupm: MCUPM_SRAM_GPR1: 0x%x, 0x%x\n", MCUPM_SRAM_GPR1, DRV_Reg32(MCUPM_SRAM_GPR1));
...
#if CFG_LOAD_SLT_DPM
static void dpm_memcpy(void *dest, const void *src, int count)
{
    u32* tmp = (u32*)dest;
...
static int bldr_load_dpm_part_slt(blkdev_t *bdev, u32 *addr, u32 *size)
{
#define DPM_PM1_SRAM_BASE        0x10900000
#define DPM_DM1_SRAM_BASE        0x10920000
#define DPM_PM_ROM_MEMADDR       0x72000000
#define DPM_DM_ROM_MEMADDR       0x72200000

    int ret, i;
    u32 *psram;

    /* load and verify PM image */
    *addr = DPM_PM_ROM_MEMADDR;
    ret = bldr_load_part("DPM_PM", bdev, addr, size);
    if (ret) {
        pal_log_err("%s DPM_PM slt load fail, addr=0x%x\n", MOD, *addr);
        return ret;
    }
    psram = (u32*)DPM_PM1_SRAM_BASE;
    dpm_memcpy (psram, *addr, *size);

    *addr = DPM_DM_ROM_MEMADDR;
    ret = bldr_load_part("DPM_DM", bdev, addr, size);
    if (ret) {
        pal_log_err("%s DPM_DM slt load fail, addr=%x\n", MOD, *addr);
        return ret;
    }
    psram = (u32*)DPM_DM1_SRAM_BASE;
    dpm_memcpy (psram, *addr, *size);

    pal_log_info("%s DPM part. slt load finished\n",MOD);
    DRV_SetReg32(0x10940000, 0x1);
...
#define SSPM_MEM_LIMIT  0x80000000
    ...
    *jump_addr = addr;
    ...
    if (ram_console_is_abnormal_boot()) {
        ...
        if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2))
            ret = bldr_load_aee_lk("lk_a", bootdev, &addr, &size);
        else
            ret = bldr_load_aee_lk("lk_b", bootdev, &addr, &size);

        *jump_addr = addr;
    } else {
        addr = CFG_BL2_EXT_MEMADDR;
        if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2)) {
            ret = bldr_load_bl2_ext("lk_a", bootdev, &addr, &size);
        } else {
            ret = bldr_load_bl2_ext("lk_b", bootdev, &addr, &size);
        }
        /*
...
#if CFG_LOAD_SLT_DPM
    ret = bldr_load_dpm_part_slt(bootdev, &addr, &size);
    if (ret)
        return ret;
#endif
