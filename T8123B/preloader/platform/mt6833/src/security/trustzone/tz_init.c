...
static void atf_dram_reserve_mem(void){
	...
	atf_dram_res = (u32)mblock_alloc(
		(u64) BL31_DRAM_SIZE, (u64)align_addr, limit_addr, BL31_DRAM_BASE, 0, "atf-reserved");
...
#if CFG_TEE_SUPPORT
    DBG_MSG("%s Jump to next BL, then 0x%x and 0x%llx\n", MOD, tee_entry_addr, addr);
#else
    DBG_MSG("%s Jump to next BL, then 0x%llx\n", MOD, addr);
#endif
...
#if CFG_BL2_EXT_MEMADDR
    int abnormal_boot = ram_console_is_abnormal_boot();

    DBG_ERR("%s Jump to %s: 0x%x\n\n", MOD, abnormal_boot ? "AEE_LK" : "BL2_EXT",
        abnormal_boot ? CFG_AEE_LK_MEMADDR : CFG_BL2_EXT_MEMADDR);
    if (abnormal_boot)
        platform_set_addr_to_reset_vector(CFG_AEE_LK_MEMADDR);
    else
        platform_set_addr_to_reset_vector(CFG_BL2_EXT_MEMADDR);
#else
    platform_set_addr_to_reset_vector(BL31_ENTRY);
#endif
    jumparch64(&(g_dram_buf->bl31_info), reserved, reserved, reserved);
}
