...
#endif
static unsigned int mr6;
static unsigned int mr7;
...
unsigned int dramc_get_vdd2_voltage(unsigned int ddr_type)
{
	...
	if (u1IsLP4Family(ddr_type)) {
		if (g_rt5738_hw_exist[RT5738_VDD2])
			...
			sum = hl7593_get_voltage(hl7593_VDD2);
		else if (g_fan53526_hw_exist[FAN53526_VDD2])
			sum = fan53526_get_voltage(FAN53526_VDD2);
	} else {
		/* LPDDR3 */
#ifdef MTK_PMIC_CHIP_MT6357
...
unsigned int dramc_get_vddq_voltage(unsigned int ddr_type)
{
	...
	if (ddr_type == TYPE_LPDDR4)
		...
		if (g_rt5738_hw_exist[RT5738_VDDQ])
			...
			sum = hl7593_get_voltage(hl7593_VDDQ);
		else if (g_fan53526_hw_exist[FAN53526_VDDQ])
			sum = fan53526_get_voltage(FAN53526_VDDQ);
	}
	return sum;
}
...
void setup_dramc_voltage_by_pmic(void)
{
	...
	if (u1IsLP4Family(ddr_type)) {
		...
		if (g_rt5738_hw_exist[RT5738_VDD2])
			...
			ret = hl7593_enable(hl7593_VDD2, 1);
		else if (g_fan53526_hw_exist[FAN53526_VDD2])
			ret = fan53526_enable(FAN53526_VDD2, 1);

		if (ret == 0) {
			dramc_crit("enable VDD2 fail (ret = %d)\n", ret);
			return;
		}
		...
		if (ddr_type == TYPE_LPDDR4X) {
			if (g_rt5738_hw_exist[RT5738_VDDQ])
				...
				ret = hl7593_enable(hl7593_VDDQ, 1);
			else if (g_fan53526_hw_exist[FAN53526_VDDQ])
				ret = fan53526_enable(FAN53526_VDDQ, 1);

			if (ret == 0) {
				dramc_crit("enable VDDQ fail (ret = %d)\n", ret);
				return;
			}
			...
			if (g_rt5738_hw_exist[RT5738_VDDQ])
				...
				hl7593_set_mode(hl7593_VDDQ, 0x3);
			else if (g_fan53526_hw_exist[FAN53526_VDDQ])
				fan53526_set_mode(FAN53526_VDDQ, 0x3);
		...
		if (g_rt5738_hw_exist[RT5738_VDD2])
			...
			hl7593_set_mode(hl7593_VDD2, 0x3);
		else if (g_fan53526_hw_exist[FAN53526_VDD2])
			fan53526_set_mode(FAN53526_VDD2, 0x3);
...
void switch_dramc_voltage_to_auto_mode(void)
{
	...
	if (u1IsLP4Family(mt_get_dram_type_from_hw_trap())) {
		if (g_rt5738_hw_exist[RT5738_VDD2])
			...
			hl7593_set_mode(hl7593_VDD2, 0x0);
		else if (g_fan53526_hw_exist[FAN53526_VDD2])
			fan53526_set_mode(FAN53526_VDD2, 0x0);

		if (g_rt5738_hw_exist[RT5738_VDDQ])
			rt5738_set_mode(RT5738_VDDQ, 0x0);
		else if (g_hl7593_hw_exist[hl7593_VDDQ])
			hl7593_set_mode(hl7593_VDDQ, 0x0);
		else if (g_fan53526_hw_exist[FAN53526_VDDQ])
			fan53526_set_mode(FAN53526_VDDQ, 0x0);
	}
}
...
unsigned int get_dram_mr(unsigned int index)
{
	...
	switch (index) {
	case 5:
		value = last_dramc_info_ptr->mr567 >> 16;
		break;
	case 6:
		value = last_dramc_info_ptr->mr567 >> 8;
		break;
	case 7:
		value = last_dramc_info_ptr->mr567;
	default:
		break;
	}
...
{
	unsigned int a = (*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS + 0x00e4)) & 0x6>> 1);
	return a;
	/* HPM = 0, LPM = 1, ULPM = 2; */
}

//Adding ddr rate here
unsigned int get_dram_data_rate(void)
{
	unsigned int u4PllIdx, u4ShuLevel, u4SDM_PCW, u4PREDIV, u4POSDIV, u4CKDIV4, u4VCOFreq, u4DataRate = 0, dram_type;
	int channels;
	unsigned int pcw_ofs[2] = { 0xd9c, 0xd94 };
	unsigned int prepost_ofs[2] = { 0xda8, 0xda0 };

	channels =  get_dram_channel_nr();
	u4ShuLevel = get_shuffle_status();

	/* only CHA have this bit */
	u4PllIdx =(*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS+0x510)) >> 31) & 0x00000001;

	u4SDM_PCW = (*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS + pcw_ofs[u4PllIdx] + 0x500 * u4ShuLevel)) >> 16);
	u4PREDIV = (*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS + prepost_ofs[u4PllIdx] + 0x500 * u4ShuLevel)) & 0x000c0000) >> 18;
	u4POSDIV = (*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS + prepost_ofs[u4PllIdx] + 0x500 * u4ShuLevel)) & 0x00000007);
	u4CKDIV4 = (*((volatile unsigned *)(Channel_A_PHY_AO_BASE_ADDRESS + 0xd18 + 0x500 * u4ShuLevel)) & 0x08000000) >> 27;

	u4VCOFreq = ((52 >> u4PREDIV) * (u4SDM_PCW >> 8)) >> u4POSDIV;
	u4DataRate = u4VCOFreq >> u4CKDIV4;
	dram_type = mt_get_dram_type_from_hw_trap();

	/*
 	 *pal_log_info("PCW=0x%X, u4PREDIV=%d, u4POSDIV=%d,"
  	 *	"  CKDIV4=%d, DataRate=%d\n",
...
void get_dram_freq_step(u32 dram_freq_step[])
{
	unsigned int i;
	unsigned int ddr_type = mt_get_dram_type_from_hw_trap();
	unsigned int defined_step_ddr3[DRAMC_FREQ_CNT -1] = {1534, 1200};
	unsigned int defined_step_ddr4[DRAMC_FREQ_CNT -1] = {2400, 1534};

	for (i = 0; i < DRAMC_FREQ_CNT; i++) {
        if (i == 0) {
			dram_freq_step[i] = get_dram_data_rate();
			continue;
        }
		if (ddr_type == TYPE_LPDDR3) {
			dram_freq_step[i] = defined_step_ddr3[i-1];
		} else if (ddr_type == TYPE_LPDDR4 || ddr_type == TYPE_LPDDR4X) {
			dram_freq_step[i] = defined_step_ddr4[i-1];
		}
	}
}
...
{
#ifdef LAST_EMI
	isu_info_ptr->buf_size = 0x800000;
	...
	isu_info_ptr->con_addr = (unsigned long long)(&(last_emi_info_ptr->decs_ctrl));
#endif
}
#endif
...
void reserve_dramc_dummy_read(void)
{
	...
	for (i = 0; i < (unsigned int)rank_nr; i++) {
		...
		get_dramc_addr(&dram_addr, 0x0);
#ifdef CUSTOM_CONFIG_MAX_DRAM_SIZE
		if (dram_addr.full_sys_addr > (get_config_max_dram_size() + 0x40000000LL))
			break;
#endif
...
#endif
		last_dramc_info_ptr->mr567 = (mr5 << 16) | (mr6 << 8) | mr7;
	} else {
...
int check_emi_mpu_addr(struct region_to_clear *info)
{
	...
	dsb();
	info->region_sa = (unsigned int)((READ_REG(EMI_MPU_SA(info->region_id))) << 16);
	info->region_ea = (unsigned int)((READ_REG(EMI_MPU_EA(info->region_id))) << 16) + 0xFFFF;
	info->region_sa += 0x40000000;
	info->region_ea += 0x40000000;

	pal_log_info("%s : 0x%x 0x%x %d\n", __func__, info->region_sa, info->region_ea, info->region_id);
	return 0;
}
void clr_emi_mpu_prot(void)
{
	...
	for (region = 0; region < EMI_MPU_REGION_NUM; region++) {
		mt_reg_sync_write(EMI_MPU_SA(region), 0x0);
		mt_reg_sync_write(EMI_MPU_EA(region), 0x0);
		for (dgroup = EMI_MPU_DGROUP_NUM - 1; dgroup >= 0; dgroup--) {
			mt_reg_sync_write(EMI_MPU_APC(region, dgroup), 0x0);
		}
	}
#endif
}
