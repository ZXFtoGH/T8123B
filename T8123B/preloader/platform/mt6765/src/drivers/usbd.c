...
static void copy_desc (struct urb *urb, void *data, unsigned int max_length, int max_buf)
{
    int available;
    unsigned int length;
    ...
    if ((signed long) max_buf > (signed long) urb->buffer_length)
        max_buf = (int) urb->buffer_length;

    if ((available = (int)((signed long) max_buf - (signed long) urb->actual_length)) <= 0)
    {
        return;
    }
    ...
    else
    {
        ...
        switch (request->bRequest)
        {
            ...
            case STDREQ_SET_ADDRESS:
                device->address = (request->wValue) & 0x7f;
                set_address = 1;
                return 0;
