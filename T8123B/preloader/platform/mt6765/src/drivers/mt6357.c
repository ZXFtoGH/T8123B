...
static int mt6357_set_voltage(unsigned char id, unsigned int volt, unsigned int max_volt) {
	...
	if (mt6357_regu_info[id].rtype == REGULAR_VOLTAGE)
		selector = DIV_ROUND_UP((volt - mt6357_regu_info[id].min_uV),
					(mt6357_regu_info[id].step_uV));
#ifdef LDO_SUPPORT
	...
	ret = pmic_config_interface(mt6357_regu_info[id].vol_reg, selector,
				    mt6357_regu_info[id].vol_mask,
				    mt6357_regu_info[id].vol_shift);
	return ret;
}
...
static int mt6357_get_voltage(unsigned char id)
{
	...
	ret = pmic_read_interface(mt6357_regu_info[id].vol_reg, &selector,
				 mt6357_regu_info[id].vol_mask,
				 mt6357_regu_info[id].vol_shift);
	if (ret)
		return -1;
	...
	if (mt6357_regu_info[id].rtype == REGULAR_VOLTAGE)
		volt = ((selector * mt6357_regu_info[id].step_uV) + mt6357_regu_info[id].min_uV);
#ifdef LDO_SUPPORT
	else if (mt6357_regu_info[id].rtype == NON_REGULAR_VOLTAGE)
...
static int mt6357_enable(unsigned char id, unsigned char en)
{
	...
	if (mt6357_regu_info[id].enable_reg == 0)
		return -1;
	mreg_dbg_print("2 %d,%d\n", id, en);
	ret = pmic_config_interface(mt6357_regu_info[id].enable_reg, en,
				    0x1, mt6357_regu_info[id].enable_shift);
...
static int mt6357_is_enabled(unsigned char id)
{
	...
	if (mt6357_regu_info[id].enable_reg == 0)
		return -1;
	ret = pmic_read_interface(mt6357_regu_info[id].enable_reg, &en,
				  0x1, mt6357_regu_info[id].enable_shift);
...
static int mt6357_set_mode(unsigned char id, unsigned char mode)
{
	...
	if (mt6357_regu_info[id].mode_reg == 0)
		return -1;
	mreg_dbg_print("4 %d,%d\n", id, mode);
	ret = pmic_config_interface(mt6357_regu_info[id].mode_reg, mode,
				    0x1, mt6357_regu_info[id].mode_shift);
...
static int mt6357_get_mode(unsigned char id)
{
	...
	if (mt6357_regu_info[id].mode_reg == 0)
		return -1;
	ret = pmic_read_interface(mt6357_regu_info[id].mode_reg, &mode,
				  0x1, mt6357_regu_info[id].mode_shift);
...
void mt6357_wdtdbg_vosel(void)
{
	...
	print("latch VPROC %d uV\n", vproc_min_uV + vproc_step_uV * gray_to_binary(val));
	...
	print("latch VSRAM_PROC %d uV\n", vsram_proc_min_uV + vsram_proc_step_uV * gray_to_binary(val));
	...
	print("latch VSRAM_OTHERS %d uV\n", vsram_others_min_uV + vsram_others_step_uV * gray_to_binary(val));
	...
	print("latch VCORE %d uV\n", vcore_min_uV + vcore_step_uV * gray_to_binary(val));
	...
	print("latch VMODEM %d uV\n", vmodem_min_uV + vmodem_step_uV * gray_to_binary(val));
}
