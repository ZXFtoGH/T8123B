...
static void mt6315_init_setting(void)
{
	...
	for (pidx = 0; pidx < MT6315_MAX; pidx++) {
		...
		if (chip->type == MT6315_TYPE_A)
		...
		else if (chip->type == MT6315_TYPE_B) {
			...
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_SHIFT);
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_EN_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_EN_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_EN_SHIFT);
		} else if (chip->type == MT6315_TYPE_C) {
			...
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_CFG_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_CFG_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_CFG_SHIFT);
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_EN_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_EN_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK1_HW0_OP_EN_SHIFT);
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_CFG_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_CFG_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_CFG_SHIFT);
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_EN_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_EN_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK3_HW0_OP_EN_SHIFT);
			mt6315_write_field_byte(chip->spmi_dev,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_ADDR, 1,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_MASK,
						MT6315_PMIC_RG_BUCK_VBUCK4_HW0_OP_CFG_SHIFT);
		...
		/* VDIG18 low power setting */
		mt6315_write_field_byte(chip->spmi_dev,
					MT6315_PMIC_RG_LDO_VDIG18_HW_OP_EN_ADDR, 1,
					MT6315_PMIC_RG_LDO_VDIG18_HW_OP_EN_MASK,
					MT6315_PMIC_RG_LDO_VDIG18_HW_OP_EN_SHIFT);
...
static void wk_set_nlim_trim(struct mt6315_chip *chip)
{
	unsigned int efuse_data = 0;
	unsigned char rval = 0;

	/* bit 640 ~ 655, bit641: check wk for NLIM_TRIM */
	efuse_data = mt6315_read_efuse_nolock(chip, 40);
	if ((efuse_data & 0x2) == 0) {
#if 1
		//TODO
		spmi_ext_register_readl(chip->spmi_dev, 0x99A, &rval, 1);
		pal_log_info("0x99A=0x%x: ", rval);
		spmi_ext_register_readl(chip->spmi_dev, 0x16B4, &rval, 1);
		pal_log_info("0x16B4=0x%x: ", rval);
		spmi_ext_register_readl(chip->spmi_dev, 0x16BB, &rval, 1);
		pal_log_info("0x16BB=0x%x: ", rval);
		spmi_ext_register_readl(chip->spmi_dev, 0x99B, &rval, 1);
		pal_log_info("0x99B=0x%x: ", rval);
		spmi_ext_register_readl(chip->spmi_dev, 0x16C2, &rval, 1);
		pal_log_info("0x16C2=0x%x: ", rval);
		spmi_ext_register_readl(chip->spmi_dev, 0x16C9, &rval, 1);
		pal_log_info("0x16C9=0x%x: \n", rval);
#endif
		mt6315_write_field_byte(chip->spmi_dev,
					MT6315_PMIC_TMA_KEY_H_ADDR, 0x9C,
					MT6315_PMIC_TMA_KEY_H_MASK,
		...
		spmi_ext_register_readl(chip->spmi_dev, 0x99A, &rval, 1);
		if (rval & 0x2) {
			mt6315_write_field_byte(chip->spmi_dev, 0x99A,
						0x1, 0x3, 0);
			mt6315_write_field_byte(chip->spmi_dev, 0x16B4,
						0xF, 0xF, 0);
		}
		if (rval & 0x20) {
			mt6315_write_field_byte(chip->spmi_dev, 0x99A,
						0x10, 0x30, 0);
			mt6315_write_field_byte(chip->spmi_dev, 0x16BB,
						0xF, 0xF, 0);
		}
		spmi_ext_register_readl(chip->spmi_dev, 0x99B, &rval, 1);
		if (rval & 0x2) {
			mt6315_write_field_byte(chip->spmi_dev, 0x99B,
						0x1, 0x3, 0);
			mt6315_write_field_byte(chip->spmi_dev, 0x16C2,
						0xF, 0xF, 0);
		}
		if (rval & 0x20) {
			mt6315_write_field_byte(chip->spmi_dev, 0x99B,
						0x10, 0x30, 0);
			mt6315_write_field_byte(chip->spmi_dev, 0x16C9,
						0xF, 0xF, 0);
		}
		mt6315_write_field_byte(chip->spmi_dev,
					MT6315_PMIC_TMA_KEY_ADDR, 0,
...
static void mt6315_efuse_sw_load(void)
{
	...
	/* for MT6315_S6 */
	chip = &mt6315_chip[MT6315_S6];
	if (!chip->spmi_dev) {
		MT6315_ERR("%s fail.\n", __func__);
		return;
	}
	mt6315_efuse_get_id(chip);
	wk_set_nlim_trim(chip);
	/* bit 640 ~ 655 */
	efuse_data = mt6315_read_efuse_nolock(chip, 40);
	/* Check 4phase buck code */
	if ((efuse_data & 0x7C) != 0x2C)
		pal_log_info("%s Code=0x%x wrong\n", __func__, efuse_data);

	/* for MT6315_S7 */
	chip = &mt6315_chip[MT6315_S7];
	if (!chip->spmi_dev) {
		MT6315_ERR("%s fail.\n", __func__);
		return;
	}
