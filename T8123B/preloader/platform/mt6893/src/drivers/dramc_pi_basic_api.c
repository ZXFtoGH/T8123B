...
void DramcPowerOnSequence(DRAMC_CTX_T *p)
{
    ...
    DramCLKAlwaysOnOff(p, ON, CK_WRITE_TO_ALL_CHANNEL);
    ...
    DramCLKAlwaysOnOff(p, OFF, CK_WRITE_TO_ALL_CHANNEL);
...
void vReplaceDVInit(DRAMC_CTX_T *p)
{
...

    {
        ...
        //YY update CAL SEL's setting @ 2021/1/25
        //PI cap sel\Freq(Mhz)   2400    1866    1534    1200    800
        //Opt setting            0x0b    0x14    0x15    0x18    0x18
        //Old setting            0x14    0x18    0x18    0x18    0x18
        if(p->frequency<=600)
        {
            u1DQ_CAP_SEL= 0x18;
            u1CA_CAP_SEL= 0x18;
        ...
        {
            u1DQ_CAP_SEL= 0x14; //0x18;
            u1CA_CAP_SEL= 0x14; //0x18;
        }
        else if(p->frequency<=1200)
        {
            u1DQ_CAP_SEL= 0x14;  //no change after test
            u1CA_CAP_SEL= 0x14;  //no change after test
        }
        ...
        vIO32WriteFldAlign_All(DRAMC_REG_REFCTRL1, 0, REFCTRL1_REFPENDINGINT_OPT1);
		...
		//Setting of CTLE is used to improve RX EYE if RX input buff offc k is enable
        if (p->frequency >= 2133)
        {
            vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B0_DQ11, 3, SHU_B0_DQ11_RG_RX_ARDQ_BW_SEL_B0);
            vIO32WriteFldAlign_All(DDRPHY_REG_SHU_B1_DQ11, 3, SHU_B1_DQ11_RG_RX_ARDQ_BW_SEL_B1);
            vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD11, 3, SHU_CA_CMD11_RG_RX_ARCA_BW_SEL_CA);
            //vIO32WriteFldAlign_All(DDRPHY_REG_SHU_CA_CMD10, 3, SHU_CA_CMD10_RG_RX_ARCLK_BW_SEL_CA);
        }
        else
        {
...
#if ENABLE_LP4Y_DFS
void ENABLE_ClkSingleEndRG (DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset, U8 u1OnOff)
{
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_CA_CMD13 + u4DDRPhyShuOffset, P_Fld( u1OnOff, SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_SEL_CA) \
                                                      | P_Fld( u1OnOff, SHU_CA_CMD13_RG_TX_ARCLKB_OE_TIE_EN_CA));
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_CA_CMD7+ u4DDRPhyShuOffset , P_Fld( u1OnOff, SHU_CA_CMD7_R_LP4Y_SDN_MODE_CLK));
}

void ENABLE_WDQSSingleEndRG (DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset, U8 u1OnOff)
{
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B0_DQ13 + u4DDRPhyShuOffset, P_Fld( !u1OnOff, SHU_B0_DQ13_RG_TX_ARDQSB_READ_BASE_EN_B0) \
    ...
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B1_DQ13 + u4DDRPhyShuOffset, P_Fld(!u1OnOff, SHU_B1_DQ13_RG_TX_ARDQSB_READ_BASE_EN_B1) \
    ...
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B0_DQ7 + u4DDRPhyShuOffset  , P_Fld( u1OnOff , SHU_B0_DQ7_R_LP4Y_SDN_MODE_DQS0));
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B1_DQ7 + u4DDRPhyShuOffset  , P_Fld( u1OnOff , SHU_B1_DQ7_R_LP4Y_SDN_MODE_DQS1));
}

void ENABLE_RDQSSingleEndRG (DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset, U8 u1OnOff)
{
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B0_DQ10 + u4DDRPhyShuOffset , P_Fld( u1OnOff, SHU_B0_DQ10_RG_RX_ARDQS_SE_EN_B0));
    vIO32WriteFldMulti_All(DDRPHY_REG_SHU_B1_DQ10 + u4DDRPhyShuOffset , P_Fld( u1OnOff, SHU_B1_DQ10_RG_RX_ARDQS_SE_EN_B1));
}

void ENABLE_SingleEndRGEnable (DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset, U16 u2Freq)
{
    if (u2Freq <= 800)
    {
        ENABLE_ClkSingleEndRG (p, u4DDRPhyShuOffset, ENABLE);
        ENABLE_WDQSSingleEndRG (p, u4DDRPhyShuOffset, ENABLE);
        ENABLE_RDQSSingleEndRG (p, u4DDRPhyShuOffset, ENABLE);
    }
}
#endif
...
void DramcShuTrackingDcmSeEnBySRAM(DRAMC_CTX_T *p)
{
#if (fcFOR_CHIP_ID == fcA60868)
    ...
    for (u1ShuffleIdx = 0; u1ShuffleIdx <= DRAM_DFS_SRAM_MAX; u1ShuffleIdx++) //fill SHU1 of conf while (u1ShuffleIdx==DRAM_DFS_SRAM_MAX)
    {
...
#if ENABLE_TX_TRACKING
        int enable_tx_tracking = (doe_get_config("dram_tx_tracking"))? 0: 1;
        if (enable_tx_tracking)
            Enable_TX_Tracking(p, u4DramcShuOffset);
#endif
#if RDSEL_TRACKING_EN
        ...
        EnableCmdPicgEffImprove(p, u4DramcShuOffset);
        ...
        ENABLE_SingleEndRGEnable (p, u4DDRPhyShuOffset, u2Freq);
#endif
...
void DramcHMR4_Presetting(DRAMC_CTX_T *p)
{
    ...
    //    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DQSOSC), 0, RK_DQSOSC_RK0_BYTE_MODE);
    //    vSetRank(p, RANK_1);
    //    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RK_DQSOSC), 1, RK_DQSOSC_RK0_BYTE_MODE);
    //    vSetRank(p, RANK_0);
...
#if defined(DPM_CONTROL_AFTERK) && (DRAMC_DFS_MODE!=2) // for MD32 RG/PST mode
	DPMInit(p);
...
#if ENABLE_RX_TRACKING
    int enable_rx_tracking = (doe_get_config("dram_rx_tracking"))? 0: 1;
    if (enable_rx_tracking) {
...
/* HW gating - Disabled by default(in preloader) to save power (DE: HJ Huang) */
    ...
    int enable_hw_gating = (doe_get_config("dram_hw_gating"))? 0: 1;
    char *hw_g_str = (enable_hw_gating == 1)? ("ON") : ("OFF");
    mcSHOW_DBG_MSG(("HW_GATING DBG: %s\n", hw_g_str));
...
#else
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_ZQ_SET1), 0, ZQ_SET1_ZQCALDISB);// LP3 and LP4 are different, be careful.
...
#ifdef DUMMY_READ_FOR_TRACKING
    int enable_dummyread_tracking = (doe_get_config("dram_dummyread_tracking"))? 0: 1;
    if (enable_dummyread_tracking) {
...
// ignor SW impedance tracking when doing time profling
    ...
    {
        int enable_impedance_tracking = (doe_get_config("dram_impedance_tracking"))? 0: 1;
        int enable_impedance_hw_saving = (doe_get_config("dram_impedance_hw_saving"))? 0: 1;
        if (enable_impedance_tracking) {
            DramcImpedanceTrackingEnable(p);
#ifdef IMPEDANCE_HW_SAVING
            if (enable_impedance_hw_saving) {
                DramcImpedanceHWSaving(p);
                mcSHOW_DBG_MSG(("IMPEDANCE_HW_SAVING: ON\n"));
            } else {
                mcSHOW_DBG_MSG(("IMPEDANCE_HW_SAVING: OFF\n"));
            }
#endif
...
#ifdef TEMP_SENSOR_ENABLE
    int enable_temp_sensor = (doe_get_config("dram_temp_sensor"))? 0: 1;
...
void SPMTx_Track_Retry_OnOff(DRAMC_CTX_T *p, U8 shu_level, U8 onoff)
{
    ...
    if ((gIsddr800TxRetry == 1) && (shu_level != SRAM_SHU6)) //Need to do tx retry when DDR800 -> DDr1200
    {
        if (onoff == ENABLE)
        {
            ...
            vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, CHAN_MODE, LPIF_LOW_POWER_CFG_1_TX_TRACKING_RETRY_EN);
...
void RODTSettings(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DDRPHY_REG_B0_DQ6, !(p->odt_onoff), B0_DQ6_RG_TX_ARDQ_ODTEN_EXT_DIS_B0);
    vIO32WriteFldAlign(DDRPHY_REG_B1_DQ6, !(p->odt_onoff), B1_DQ6_RG_TX_ARDQ_ODTEN_EXT_DIS_B1);
    vIO32WriteFldAlign(DDRPHY_REG_CA_CMD6, !(p->odt_onoff), CA_CMD6_RG_TX_ARCMD_ODTEN_EXT_DIS);
...
void LP4_UpdateInitialSettings(DRAMC_CTX_T *p)
{
    ...
    #if (fcFOR_CHIP_ID == fcA60868)
    vIO32WriteFldAlign(DDRPHY_REG_SHU_CA_CMD14, 0x50, SHU_CA_CMD14_RG_TX_ARCA_MCKIO_SEL_CA);
    #elif (fcFOR_CHIP_ID == fcPetrus)
    vIO32WriteFldAlign(DDRPHY_REG_SHU_CA_CMD14, 0xC0, SHU_CA_CMD14_RG_TX_ARCA_MCKIO_SEL_CA);
    #endif
    ...
    vIO32WriteFldMulti(DRAMC_REG_DRAMCTRL, P_Fld(0x1, DRAMCTRL_ALL_BLOCK_CTO_ALE_DBG_EN) | P_Fld(0x1, DRAMCTRL_SELFREF_BLOCK_CTO_ALE_DBG_EN));
    ...
    vIO32WriteFldAlign(DDRPHY_REG_MISC_CTRL1, 1, MISC_CTRL1_R_DMARPIDQ_SW); // @Darren, Fix DQ/DQM duty PI not work.
...
unsigned int DDRPhyFreqMeter(void)
{
    ...
    unsigned int before_value, after_value;
    unsigned int frq_result ;
