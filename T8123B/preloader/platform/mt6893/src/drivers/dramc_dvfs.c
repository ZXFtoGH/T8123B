...
DRAM_DFS_FREQUENCY_TABLE_T* get_FreqTbl_by_shuffleIndex(DRAMC_CTX_T *p, U8 index)
{
    ...
    if (u1ShuffleIdx == DRAM_DFS_SHUFFLE_MAX)
    {
...
void SramDebugModeRead(DRAMC_CTX_T *p, U8 sram_shu_level, U32 u4Reg)
{
    U32 u4Value=0;
...
void DFSRuntimeMRW_preset(DRAMC_CTX_T *p, U8 sram_shu_level)
{
    ...
    U8 u1MR03_Value = 0, u1MR51_Value = 0;
    U32 u4MD32Offset = 0;
    ...
    for (u1ChIdx = CHANNEL_A; u1ChIdx < p->support_channel_num; u1ChIdx += DPM_CH_NUM)
    {
#if CHANNEL_NUM > 2
        if (u1ChIdx >= CHANNEL_C)
            u4MD32Offset = ((u1ChIdx >> 1) << POS_BANK_NUM);
#endif
        //! save shux mr12/mr14
        vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_2 + u4MD32Offset + (sram_shu_level << 4),
            ...
            P_Fld(u1MR12Value[u1ChIdx+1][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_2_MR_OP_SET_SHU_0_10) |
            P_Fld(u1MR12Value[u1ChIdx+1][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_2_MR_OP_SET_SHU_0_11));

        vIO32WriteFldMulti(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_3 + u4MD32Offset + (sram_shu_level << 4),
            P_Fld(u1MR14Value[u1ChIdx][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_3_MR_OP_SET_SHU_0_12) |
            P_Fld(u1MR14Value[u1ChIdx][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_3_MR_OP_SET_SHU_0_13) |
            P_Fld(u1MR14Value[u1ChIdx+1][RANK_0][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_3_MR_OP_SET_SHU_0_14) |
            P_Fld(u1MR14Value[u1ChIdx+1][RANK_1][p->dram_fsp], LPIF_MR_OP_STORE_SHU_0_3_MR_OP_SET_SHU_0_15));
    }
...
static void DFSRuntimeMRWEn(DRAMC_CTX_T *p, U8 cur_shu_mux_index, U8 nxt_shu_level, U8 pingpong_fsp)
{
    ...
    for (u1ChIdx = ch_start; u1ChIdx < ch_end; u1ChIdx++)
    {
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < p->support_rank_num; u1RankIdx++)
        {
            ...
            //! get shux mr12/mr14/
            rtmr12 = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_2 + MD32Offset + (nxt_shu_level << 4),  Fld(8, u1FldIdx*8));
            rtmr14 = u4IO32ReadFldAlign(DDRPHY_MD32_REG_LPIF_MR_OP_STORE_SHU_0_3 + MD32Offset + (nxt_shu_level << 4),  Fld(8, u1FldIdx*8));
...
static void DFSHwSetWA(DRAMC_CTX_T *p, U8 cur_shu_mux_index, U8 nxt_shu_level)
{
    ...
    for (u1ChIdx = ch_start; u1ChIdx < ch_end; u1ChIdx++)
    {
        ...
        if ((nxt_shu_level == SRAM_SHU7) || (nxt_shu_level == SRAM_SHU0) || (nxt_shu_level == SRAM_SHU1)) // for term shuffle level
            u1MR13_OP |= 0xC0; //! MR13 OP7 = 1, OP6 = 1, from CLRPLL to PHYPLL
        else
            u1MR13_OP &= 0x3F; //! MR13 OP7 = 0, OP6 = 0, from PHYPLL to CLRPLL
...
void TxReadBaseODTWA(DRAMC_CTX_T *p, U8 next_shu_level)
{
    ...
    if ((next_shu_level == SRAM_SHU7) || (next_shu_level == SRAM_SHU0) || (next_shu_level == SRAM_SHU1)) // for DDR4266/DDR3200
...
/*7*/    {DDRPHY_REG_SHU_B0_DQ9, DDRPHY_REG_SHU_B1_DQ9},
};
...
void SRAMShuRestoreToDPHYWA(DRAMC_CTX_T *p, U8 sram_shu_level, U8 pingpong_shu_level)
{
    ...
    for (u1ChannelIdx=CHANNEL_A; u1ChannelIdx < (p->support_channel_num); u1ChannelIdx++)
    {
        ...
        for (u1RankIdx = RANK_0; u1RankIdx < (p->support_rank_num); u1RankIdx++)
        {
            ...
            {
                u2Offset = u1ByteIdx*DDRPHY_AO_B0_B1_OFFSET;
                ...
                p->ShuRGAccessIdx = pingpong_shu_level;
                vIO32Write4B(DRAMC_REG_ADDR(DDRPHY_REG_SHU_B0_PHY_VREF_SEL+u2Offset), u4Byte_PHY_VREF_SEL);
                p->ShuRGAccessIdx = DRAM_DFS_REG_SHU0;
            }
        }
    }
...
void DVFSSettings(DRAMC_CTX_T *p)
{
     ...
     */
#if (FOR_DV_SIMULATION_USED == 0 && SW_CHANGE_FOR_SIMULATION == 0 && __Petrus_TO_BE_PORTING__)
    ...
    vIO32WriteFldMulti_All(DDRPHY_REG_MISC_DVFSCTL2, P_Fld(1, MISC_DVFSCTL2_R_DVFS_CDC_OPTION) //Lewis@20170331: Not set SHUCTRL2_R_DVFS_CDC_OPTION to 1 since it will lead DDR reserve mode fail in DDR2400 and DDR1600
		| P_Fld(1, MISC_DVFSCTL2_R_DVFS_SYNC_MODULE_RST_SEL) 
    ...
    //Cann_on CDC options
    vIO32WriteFldMulti_All(DDRPHY_REG_MISC_CTRL0, P_Fld(1, MISC_CTRL0_IMPCAL_CDC_ECO_OPT) | P_Fld(1, MISC_CTRL0_IDLE_DCM_CHB_CDC_ECO_OPT));
    //DLL_SHUFFLE should be set enable before switch frequency
    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_DVFS_EMI_CLK, 1, MISC_DVFS_EMI_CLK_RG_DLL_SHUFFLE_DDRPHY);
    vIO32WriteFldAlign_All(DDRPHY_REG_MISC_DVFSCTL2, 1, MISC_DVFSCTL2_RG_DLL_SHUFFLE);
...
void DVS_DMY_RD_ENTR(DRAMC_CTX_T *p)
{
    ...
    //! diff with WE
    vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, CHAN_MODE, LPIF_LOW_POWER_CFG_1_DMY_EN_MOD_SEL);

    mcDELAY_US(1);

    /*TINFO="DRAM : set sc_dmyrd_intv_sel = 1"*/
    //! diff with WE
    vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, CHAN_MODE, LPIF_LOW_POWER_CFG_1_DMYRD_INTV_SEL);

    mcDELAY_US(1);

    /*TINFO="DRAM : set sc_dmyrd_en = 1"*/
    //! diff with WE
    vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, CHAN_MODE, LPIF_LOW_POWER_CFG_1_DMYRD_EN);
...
void EnableDramcTrackingByShuffle(DRAMC_CTX_T *p, U8 u1OnOff)
{
    if (u1OnOff == ENABLE)
    {
...
{
    if (p->frequency >= 1600)
        vIO32WriteFldAlign(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_1, CHAN_MODE, LPIF_LOW_POWER_CFG_1_DPHY_RXDLY_TRACK_EN);
}
...
//for A60868 only
void DramcDFSDirectJump_SystemMode(DRAMC_CTX_T *p, U8 shu_level)
{
    U8 u1ShuAck = 0;
    U8 i = 0;
    bool pingpong_shu_level; // for shu0/1
    U8 cur_shu_level = vGet_Current_SRAMShuLevel(p);
    U8 u1load_ack = 0, u1shu_en_ack = 0, u1restore_ack = 0;
    ...
    if (p->u1PLLMode == PHYPLL_MODE)
    {
        mcSHOW_DBG_MSG3(("DramcDFSDirectJump_SystemMode to CLRPLL, SHU_LEVEL=%d, ACK=%x\n", shu_level, u1ShuAck));
    }
    else
    {
        mcSHOW_DBG_MSG3(("DramcDFSDirectJump_SystemMode to PHYPLL, SHU_LEVEL=%d, ACK=%x\n", shu_level, u1ShuAck));
    }

    vIO32WriteFldAlign(DDRPHY_REG_MISC_STBCAL2, 0x1, MISC_STBCAL2_STB_DBG_STATUS); // HJ Huang
    /*TINFO="DRAM : set ddrphy_fb_ck_en=1"*/
    vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1, RG_156_SC_DDRPHY_FB_CK_CH0_EN);
    vIO32WriteFldAlign(SYSTEM_REG_RG_164, 1, RG_164_SC_DR_SHU_LEVEL_SRAM_LATCH_CH0);
    mcDELAY_US(1);
    vIO32WriteFldAlign(SYSTEM_REG_RG_164, 0, RG_164_SC_DR_SHU_LEVEL_SRAM_LATCH_CH0);

    pingpong_shu_level = u4IO32ReadFldAlign(SYSTEM_REG_RG_160, RG_160_SC_DR_SHU_LEVEL_CH0);

    if (p->u1PLLMode == PHYPLL_MODE)
    {
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0, RG_156_SC_PHYPLL_SHU_EN_CH0);
        vIO32WriteFldAlign(SYSTEM_REG_RG_160, !pingpong_shu_level, RG_160_SC_DR_SHU_LEVEL_CH0);
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1, RG_156_SC_PHYPLL2_SHU_EN_CH0);
        mcSHOW_DBG_MSG3(("Enable CLRPLL\n"));
    }
    else
    {
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0, RG_156_SC_PHYPLL2_SHU_EN_CH0);
        vIO32WriteFldAlign(SYSTEM_REG_RG_160, !pingpong_shu_level, RG_160_SC_DR_SHU_LEVEL_CH0);
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1, RG_156_SC_PHYPLL_SHU_EN_CH0);
        mcSHOW_DBG_MSG3(("Enable PHYPLL\n"));
    }
    mcDELAY_US(1);

    vIO32WriteFldMulti((SYSTEM_REG_RG_160), P_Fld(shu_level, RG_160_SC_DR_SHU_LEVEL_SRAM_CH0)
                        | P_Fld(shu_level, RG_160_SC_DR_SHU_LEVEL_SRAM_CH1));

    vIO32WriteFldAlign(SYSTEM_REG_RG_160, 1, RG_160_SC_DR_SRAM_LOAD_CH0);
    u1load_ack = (u4IO32ReadFldAlign(SYSTEM_REG_RG_28, RG_28_SPM_CTRL_ACK) >> 12) & 0x3;
    while ((u1load_ack & u1ShuAck ) != u1ShuAck)
    {
        mcSHOW_DBG_MSG3(("\twait sram load ack.\n"));
    }
    vIO32WriteFldAlign(SYSTEM_REG_RG_160, 0, RG_160_SC_DR_SRAM_LOAD_CH0);
...
#endif
    if (p->u1PLLMode == PHYPLL_MODE)
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1, RG_156_SC_PHYPLL2_MODE_SW_CH0);
    else
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1, RG_156_SC_PHYPLL_MODE_SW_CH0);
#if ENABLE_DFS_SSC_WA    
    ...
    /*TINFO="DRAM : set ddrphy_fb_ck_en=0"*/
    vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0, RG_156_SC_DDRPHY_FB_CK_CH0_EN);
    ...
    vIO32WriteFldAlign(SYSTEM_REG_RG_156, 1,  RG_156_SC_DR_SHU_EN_CH0);
    u1shu_en_ack = (u4IO32ReadFldAlign(SYSTEM_REG_RG_28, RG_28_SPM_CTRL_ACK) >> 2) & 0x3;
    while ((u1shu_en_ack & u1ShuAck ) != u1ShuAck)
    {
        mcSHOW_DBG_MSG3(("\twait shu_en ack.\n"));
    }
    vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0,  RG_156_SC_DR_SHU_EN_CH0);
    vIO32WriteFldAlign(DDRPHY_REG_MISC_STBCAL2, 0x4, MISC_STBCAL2_STB_DBG_STATUS);
    mcSHOW_DBG_MSG3(("SHUFFLE End\n"));

    if (p->u1PLLMode == PHYPLL_MODE)
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0, RG_156_SC_PHYPLL_MODE_SW_CH0); // PHYPLL off
    else
        vIO32WriteFldAlign(SYSTEM_REG_RG_156, 0, RG_156_SC_PHYPLL2_MODE_SW_CH0); // CLRPLL off
    ...
    vIO32WriteFldAlign(SYSTEM_REG_RG_160, 1, RG_160_SC_SHU_RESTORE_CH0);
    u1restore_ack = (u4IO32ReadFldAlign(SYSTEM_REG_RG_28, RG_28_SPM_CTRL_ACK) >> 10) & 0x3;
    while ((u1restore_ack & u1ShuAck ) != u1ShuAck)
    {
        mcSHOW_DBG_MSG3(("\twait sram restore ack.\n"));
    }
    vIO32WriteFldAlign(SYSTEM_REG_RG_160, 0, RG_160_SC_SHU_RESTORE_CH0);
...
void DramcDFSDirectJump_SPMMode(DRAMC_CTX_T *p, U8 shu_level)
{
    ...
    EnableDramcTrackingByShuffle(p, DISABLE);
    ...
    /*TINFO="DRAM : set ddrphy_fb_ck_en=0"*/
    vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_0, 0, LPIF_LOW_POWER_CFG_0_DDRPHY_FB_CK_EN);
    EnableDramcTrackingByShuffle(p, ENABLE);
...
void DramcDFSDirectJump(DRAMC_CTX_T *p, U8 shu_level)
{
    ...
    if (gDVFSCtrlSel == 0)
    {
        if (shu_level == SRAM_SHU0) // DDR3733
