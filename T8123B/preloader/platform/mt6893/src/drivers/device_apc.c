...
void device_APC_dom_setup(void)
{
	...
	MSG(DOM, "APC_CON: INFRA_AO(0x%x), PERI_AO(0x%x), PERI_AO2(0x%x)\n",
		reg_read32(DEVAPC_INFRA_AO_APC_CON),
		reg_read32(DEVAPC_PERI_AO_APC_CON),
		reg_read32(DEVAPC_PERI_AO2_APC_CON));
...
/* INFRA MASTER DOMAIN settings */
	...
	reg_set_field(DEVAPC_INFRA_AO_MAS_DOM_2, MD1, DOMAIN_1);

	/* Set CONN to DOMAIN2(CONN) */
	reg_set_field(DEVAPC_INFRA_AO_MAS_DOM_1, CONN, DOMAIN_2);

	/* Set SSHUB(SSHUB=SCP=Sensor Hub) to DOMAIN3(SCP) */
	/* SCP domain is not set by DEVAPC */

	/* Set MFG(GPU) to DOMAIN6(MFG) */
	reg_set_field(DEVAPC_INFRA_AO_MAS_DOM_2, MFG, DOMAIN_6);

	/* Set SSPM to DOMAIN8(SSPM) */
	reg_set_field(DEVAPC_INFRA_AO_MAS_DOM_2, SSPM, DOMAIN_8);

	/* Set MCUPM to DOMAIN14 */
	reg_set_field(DEVAPC_INFRA_AO_MAS_DOM_0, MCUPM, DOMAIN_14);

	/* Set SPM to DOMAIN9(SPM) */
	reg_set_field(DEVAPC_PERI_AO_MAS_DOM_2, SPM, DOMAIN_9);
	...
	MSG(DOM, "Domain Setup: infra_ao (0x%x), (0x%x), (0x%x)\n",
		reg_read32(DEVAPC_INFRA_AO_MAS_DOM_0),
		reg_read32(DEVAPC_INFRA_AO_MAS_DOM_1),
		reg_read32(DEVAPC_INFRA_AO_MAS_DOM_2));

	MSG(DOM, "Domain Setup: peri_ao (0x%x), (0x%x), (0x%x)\n",
		reg_read32(DEVAPC_PERI_AO_MAS_DOM_0),
		reg_read32(DEVAPC_PERI_AO_MAS_DOM_1),
		reg_read32(DEVAPC_PERI_AO_MAS_DOM_2),
		reg_read32(DEVAPC_PERI_AO_MAS_DOM_3));
	...
	/* Set GPU protected mode - South EMI */
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, OSID0, MFG_NS_D6);
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, OSID1, MFG_NS_D6);
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, OSID2, MFG_NS_D6);
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, OSID3, MFG_NS_D6);
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, FM_EN, MFG_NS_D6);
	reg_set_field(INFRA_AO_SEC_MFG_HYP2, SEC_EN, MFG_S_D6);

	reg_set_field(INFRA_AO_SEC_MFG_HYP2, REMAP_EN, 1);

	MSG(DOM, "MFG SEC HYP Setup: %s: 0x%x\n", "INFRA_AO_SEC_MFG_HYP2",
			reg_read32(INFRA_AO_SEC_MFG_HYP2));
...
/* DOMAIN REMAP settings */
	...
	reg_set_field(DEVAPC_INFRA_AO_DOM_RMP_0_0, INFRA_TINY_SCP_DOM,
			DOMAIN_3);
	reg_set_field(DEVAPC_INFRA_AO_DOM_RMP_0_0, INFRA_TINY_AP_DOM,
			DOMAIN_0);

	MSG(DOM, "TINYSYS master DOM_REMAP 0:0x%x\n",
