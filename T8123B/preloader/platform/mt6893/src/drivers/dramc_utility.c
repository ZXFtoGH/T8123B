...
void DramCLKAlwaysOnOff(DRAMC_CTX_T *p, U8 option, CK_FIX_CHANNEL WriteChannelNUM)
{
    if (WriteChannelNUM == CK_WRITE_TO_ALL_CHANNEL) {
...
#else
        // >>AXI_CHAN_BIT_WIDTH: drop bit8 bit9; >>OFFSET_OF_RG_BASE_AND_AXI: align with RG row;  >>3: RG start with bit 3
        u4BaseR0 = (((u4Addr & ~0x3ff) >> AXI_CHAN_BIT_WIDTH) | (u4Addr & 0xff)) >> (OFFSET_OF_RG_BASE_AND_AXI + 3);
#endif
...
#if ETT_MINI_STRESS_USE_TA2_LOOP_MODE
void TA2_Test_Run_Time_Stop_Loop_Mode(DRAMC_CTX_T * p)
{
    U8 u1ChannelIdx = 0;
...
U32 TA2_Test_Run_Time_Err_Status(DRAMC_CTX_T *p)
{
    ...
    for(u1ChannelIdx=CHANNEL_A; u1ChannelIdx<(p->support_channel_num); u1ChannelIdx++)
    {
        ...
        mcSHOW_DBG_MSG(("CH[%d] LOOP_CNT:0x%x \n", u1ChannelIdx, u4Value));
    }
...
#endif
#define REG_BACKUP_LENS 128
U32 u4gpRegBackupVlaue[REG_BACKUP_LENS];
void DramcBackupRegisters(DRAMC_CTX_T *p, U32 *backup_addr, U32 backup_num)
{
    U32 u4RegIdx;

#ifdef __ETT__
    if ((backup_num > REG_BACKUP_LENS) || u1gpRegBackup)
    {
