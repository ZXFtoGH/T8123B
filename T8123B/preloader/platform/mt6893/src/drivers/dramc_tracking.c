...
static DRAM_STATUS_T DramcStartDQSOSC_RTSWCMD(DRAMC_CTX_T *p)
{
	...
	if (u4TimeCnt == 0)//time out
	{
		mcSHOW_DBG_MSG(("[LP5 RT SW Cmd MRW ] Resp fail (time out)\n"));
		return DRAM_FAIL;
	}
...
DRAM_STATUS_T DramcDQSOSCShuSettings(DRAMC_CTX_T *p)
{
    ...
    if (gu1MR23[p->channel][RANK_1] > gu1MR23[p->channel][RANK_0])
        u1MR23 = gu1MR23[p->channel][RANK_1];
    else
        u1MR23 = gu1MR23[p->channel][RANK_0];

    u2PRDCNT = (u1MR23 / u1PRDCNT_DIV) + 3;
...
{
    //Only enable at DDR3733/DDR4266
    if (p->pDFSTable->shuffleIdx == SRAM_SHU7 || p->pDFSTable->shuffleIdx == SRAM_SHU0) //add if(u1ShuffleIdx==DRAM_DFS_SRAM_MAX) to avoid enable tx-tracking when running DDR800 as RG-SHU0
    {
...
void Enable_TxWDQS(DRAMC_CTX_T *p, U32 u4DDRPhyShuOffset)
{
...
void Enable_and_Trigger_TX_Retry(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_TX_RETRY_SET0, P_Fld(1, TX_RETRY_SET0_TX_RETRY_SHU_RESP_OPT)
                                            | P_Fld(1, TX_RETRY_SET0_TX_RETRY_UPDPI_CG_OPT)
...
void DramcImpedanceTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti_All(DDRPHY_REG_MISC_IMPCAL, P_Fld(1, MISC_IMPCAL_IMPCAL_HW) | P_Fld(0, MISC_IMPCAL_IMPCAL_EN) | 
                                             ...
                                             P_Fld(0, MISC_IMPCAL_IMPCAL_ECO_OPT) | P_Fld(1, MISC_IMPCAL_DRV_ECO_OPT));
    ...
    vIO32WriteFldMulti(DDRPHY_REG_MISC_IMPCAL, P_Fld(0, MISC_IMPCAL_DIS_SUS_CH0_DRV) | P_Fld(1, MISC_IMPCAL_DIS_SUS_CH1_DRV));
    vIO32WriteFldMulti(DDRPHY_REG_MISC_IMPCAL + SHIFT_TO_CHB_ADDR, P_Fld(1, MISC_IMPCAL_DIS_SUS_CH0_DRV) | P_Fld(0, MISC_IMPCAL_DIS_SUS_CH1_DRV));
...
void FreqJumpRatioCalculation(DRAMC_CTX_T *p)
{
    ...
    if (p->frequency != 400)
    {
        ...
        for (shuffle_dst_index = DRAM_DFS_SHUFFLE_1; shuffle_dst_index < DRAM_DFS_SHUFFLE_MAX; shuffle_dst_index++)
        {
            ...
            mcDUMP_REG_MSG(("[DUMPLOG] %d Jump_RATIO[%d]=%d, Jump_DDR%d=%d, Jump_DDR%d=%d\n", p->frequency*2, jump_ratio_index, u2JumpRatio[jump_ratio_index], get_shuffleIndex_by_Freq(p), shuffle_src_freq << 1, shuffle_dst_index, u2Freq << 1));
            jump_ratio_index++;
        }
    }
