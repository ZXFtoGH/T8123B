...
static U8 slt_su4DfsCnt =0;
static bool slt_high2low = TRUE;
extern U8 gDfs_max_cnt;
DRAM_DVFS_TABLE_T slt_gDVFSTbl[] = {
    {LP4_DDR800, SRAM_SHU6, 575000},
    {LP4_DDR1866, SRAM_SHU3, 575000},
#ifdef SLT_2400_EXIT_PRELOADER
    {LP4_DDR2400, SRAM_SHU0, 600000},
    {LP4_DDR2400, SRAM_SHU0, 725000},
#else
#ifdef SLT_4266_EXIT_PRELOADER
    {LP4_DDR2400, SRAM_SHU2, 600000},
    {LP4_DDR3200, SRAM_SHU1, 600000},
    {LP4_DDR4266, SRAM_SHU0, 712500},
    {LP4_DDR4266, SRAM_SHU0, 750000},
#else
    {LP4_DDR2400, SRAM_SHU2, 600000},
    {LP4_DDR2400, SRAM_SHU2, 725000},
    {LP4_DDR4266, SRAM_SHU7, 712500},
    {LP4_DDR4266, SRAM_SHU7, 750000},
...
void SLT_DramcDFS(DRAMC_CTX_T *p, int iDoDMA)
{
    ...
    ///TODO: DFS scenario
    DramcDFSDirectJump(p,  p->pDFSTable->shuffleIdx);
...
//        return;
    ...
    vSetDFSTable(p, get_FreqTbl_by_shuffleIndex(p, slt_gDVFSTbl[slt_su4DfsCnt].shuffleIdx));
...
{   U8 ii;
    ...
    for (ii = 0; ii < jump_table_number; ii++)
    {
        ...
        vSetDFSFreqSelByTable(p, get_FreqTbl_by_shuffleIndex(p, slt_gDVFSTbl[ii].shuffleIdx));

//        DramcDFSDirectJump_SRAMShuRGMode(p, SRAM_SHU1);
        DramcDFSDirectJump_SRAMShuRGMode(p, slt_gDVFSTbl[ii].shuffleIdx);
        GetPhyPllFrequency(p);
        mcSHOW_DBG_MSG(("Fra set vcore %d here\n", slt_gDVFSTbl[slt_su4DfsCnt ].u4Vcore));
        dramc_set_vcore_voltage(slt_gDVFSTbl[slt_su4DfsCnt ].u4Vcore);
        slt_su4DfsCnt ++;
        if (slt_su4DfsCnt  == slt_gDfs_max_cnt) slt_su4DfsCnt =0;
#endif
...
U32 EMI_LPBK_memory_test_start(DRAMC_CTX_T * p, U8 K_win)
{
    ...
    U32 test_addr[5] =
        ...
        // emi hash rule after efus
        emi_hash_rule = u4IO32Read4B(0x10002050);
        mcSHOW_DBG_MSG(("=== hash_rule = 0x%x\n",emi_hash_rule));

        if (emi_hash_rule & 0x01ff0000) {
            emi_hash_rule = (emi_hash_rule >> 16);
        }
        emi_hash_rule = emi_hash_rule & 0x1ff;
                    ...
                    for (addr_idx = 0; addr_idx < 1; addr_idx++) //use different address to K Tx DQ/DQS dly
                    {
                        ...
                        for (pi_dly = 0; pi_dly <= max_idx; pi_dly += pi_dly_step) //robin // PI dly
                        {
                            ...
                            for (addr_offset = 0; addr_offset < len; addr_offset += step)
                            {
                                ...
                                // update emu hash rule after efus
                                if (emi_hash_rule == 0x21) {
                                    // CH_A, CH_B, CH_C, CH_D
                                    //addr_w = (addr_w & (~0x300)) | 0x100*channel_idx;

                                } else if (emi_hash_rule == 0xE7) {
                                    // CH_A, CH_B, CH_D, CH_C
                                    if (channel_idx==CHANNEL_C)
                                        addr_w = (addr_w & (~0x300)) | 0x100*CHANNEL_D;
                                    else if (channel_idx==CHANNEL_D)
                                        addr_w = (addr_w & (~0x300)) | 0x100*CHANNEL_C;
                                }
#if !EMI_USE_TA2
...
DRAM_DFS_FREQUENCY_TABLE_T gFreqTbl[DRAM_DFS_SHUFFLE_MAX] = {
    ...
    {   //EMI LPBK Test + DFS
        ...
        //slt prealoder used
        //U8 shu_num = 3, shu_idx[7] = {6, 0, 1, 5, 4, 3, 2}; // 0:3200, 1:3733, 2:800, 3:1866, 4:1200, 5: 2400, 6:1600
        //ETT load used
#if EMI_USE_TA2 || EMI_LPBK_1W1R
        U8 shu_num = 1, shu_idx[7] = {6, 5, 4, 3, 0, 1}; // 0:3200, 1:3733, 2:800, 3:1866, 4:1200, 5: 2400, 6:1600
#else //EMI_LPBK_8W1R
        U8 shu_num = 1, shu_idx[7] = {6, 5, 4, 3, 0, 1, 2}; // 0:3200, 1:3733, 2:800, 3:1866, 4:1200, 5: 2400, 6:1600
#endif
        S32 ii,jj,test_cnt;
        U8 u4RankIdx;
...
if (shu_num>1)
{
        ...
        DramcDFSDirectJump_SPMMode(p, gFreqTbl[next_shu].shuffleIdx);
}
...
int SLT_Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_EXTERN_T dram_cbt_mode_extern, DRAM_INFO_BY_MRR_T *DramInfo, U8 get_mdl_used)
{
    ...
    for (u1ShuIdx = DRAM_DFS_SRAM_MAX - 2; u1ShuIdx >= 0; u1ShuIdx--)
    {
        ...
        if(u1ShuIdx==6)
        {
            gFreqTbl[1].shuffleIdx = gFreqTbl[u1ShuIdx].shuffleIdx;
            gFreqTbl[u1ShuIdx].shuffleIdx = SRAM_SHU0;
        }
#else
#ifdef SLT_4266_EXIT_PRELOADER
        if(u1ShuIdx==2)
...
//need K 1866        if (u1ShuIdx == 4) continue; //skip scan 1866
        if (u1ShuIdx == 1) continue; //skip scan 3733
...
if (u2TimeProfileCnt == (DDR_INIT_TIME_PROFILING_TEST_CNT - 1)) //last time of loop
    ...
    vIO32Write4B(0x10006000, 0x0B160001);
    vIO32Write4B(0x10006488, u4IO32Read4B(0x10006488) | (0x1 << 4));
    vIO32Write4B(0x10006004, u4IO32Read4B(0x10006004) | (0x1 << 28));
