 ...
 * ====================
 */
int mt6691_probe(struct mt6691_chip *mt6691_chip, unsigned char num_chip)
{
	int ret = 0;
	unsigned int i = 0, j = 0;

	for (i = 0; i < num_chip; i++, mt6691_chip++) {
		if (mt6691_chip->id != mt6691_regulator[j].id)
			continue;
		if (mt6691_chip->is_hw_exist) {
			ret = mtk_simple_regulator_register(&mt6691_regulator[j]);
			if (ret < 0) {
				/* register mtk regulator error */
				MT6691LOG("mt6691 regulator, %s\n",
					  mt6691_regulator[j].name);
				return ret;
			}
		}
		j++;
