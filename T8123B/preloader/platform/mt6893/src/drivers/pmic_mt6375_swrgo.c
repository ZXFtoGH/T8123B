...
/* MT6893 needs APU check after PMIC WDT enable */
#include <apu_check.h>
...
/* PMIC warm reset/cold reset for MT6893 APU */
...
static int auxadc_channel_mappings[AUXADC_LIST_MAX] = {
	AUXADC_CHAN_BATADC,
...
static int auxadc_channel_reverse_mappings[AUXADC_CHAN_MAX] = {
	[AUXADC_CHAN_BATADC] = AUXADC_LIST_BATADC,
	[AUXADC_CHAN_BAT_TEMP] = AUXADC_LIST_BATTEMP,
	[AUXADC_CHAN_CHIP_TEMP] = AUXADC_LIST_CHIP_TEMP,
	[AUXADC_CHAN_VCORE_TEMP] = AUXADC_LIST_VCORE_TEMP,
	[AUXADC_CHAN_VPROC_TEMP] = AUXADC_LIST_VPROC_TEMP,
	[AUXADC_CHAN_VGPU_TEMP] = AUXADC_LIST_VGPU_TEMP,
	[AUXADC_CHAN_ACCDET] = AUXADC_LIST_ACCDET,
	[AUXADC_CHAN_VDCXO] = AUXADC_LIST_VDCXO,
	[AUXADC_CHAN_TSX_TEMP] = AUXADC_LIST_TSX_TEMP,
	[AUXADC_CHAN_HPOFS_CAL] = AUXADC_LIST_HPOFS_CAL,
	[AUXADC_CHAN_DCXO_TEMP] = AUXADC_LIST_DCXO_TEMP,
	[AUXADC_CHAN_VBIF] = AUXADC_LIST_VBIF,
};

#define PMIC_AUXADC_CHAN_SPEC(_chan) { .channel = _chan, }

static const struct auxadc_chan_spec pmic_auxadc_chan_specs[] = {
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_CHIP_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_VCORE_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_VPROC_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_VGPU_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_ACCDET),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_VDCXO),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_TSX_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_HPOFS_CAL),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_DCXO_TEMP),
	PMIC_AUXADC_CHAN_SPEC(AUXADC_CHAN_VBIF),
};
			    ...
			    enum auxadc_val_type type)
{
	...
	int channel = auxadc_channel_reverse_mappings[chan->channel];
...
static struct auxadc_device pmic_auxadc_device = {
	...
	.num_chans = ARRAY_SIZE(pmic_auxadc_chan_specs),
...
int pmic_get_auxadc_value(unsigned short channel)
{
	...
	if (channel >= AUXADC_LIST_MAX) {
		pal_log_info("[%s] Invalid channel(%d)\n", __func__, channel);
		return -1;
	}
	chan = auxadc_channel_mappings[channel];
	if (chan == AUXADC_CHAN_BATADC || chan == AUXADC_CHAN_BAT_TEMP)
		auxadc = pmic_auxadc_get_by_name("subpmic");
	else
		auxadc = pmic_auxadc_get_by_name("pmic");
	/* Audio request HPOFS to return raw data */
	if (chan == AUXADC_CHAN_HPOFS_CAL)
		pmic_auxadc_read_value(auxadc, chan, &val, AUXADC_VAL_RAW);
	else
		pmic_auxadc_read_value(auxadc, chan, &val, AUXADC_VAL_PROCESSED);
	return val;
}
#else
/* extend r_val to 10 times from PMIC MT6359 */
...
U32 pmic_init (void)
{
	...
	if (!ram_console_is_abnormal_boot() &&
		...
		/* MT6893 needs APU check after PMIC WDT enable in pmic_init */
		apu_check();
...
#ifdef MT6375_AUXADC
	mtk_subpmic_init();
	if (pmic_auxadc_device_register(&pmic_auxadc_device) < 0)
...
{
    pmic_config_interface(PMIC_RG_VIBR_VOSEL_ADDR, 9,
			      PMIC_RG_VIBR_VOSEL_MASK,
			      PMIC_RG_VIBR_VOSEL_SHIFT);
    pmic_config_interface(PMIC_RG_LDO_VIBR_EN_ADDR, 1,
...
{
    pmic_config_interface(PMIC_RG_LDO_VIBR_EN_ADDR, 0,
			      PMIC_RG_LDO_VIBR_EN_MASK,
			      PMIC_RG_LDO_VIBR_EN_SHIFT);
}
