...
/* VMDDR */
#define hl7593_I2C_CHANNEL_VMDDR	I2C6
#define hl7593_SLAVE_ADDR_VMDDR		0x61

/* VUFS12 */
#define hl7593_I2C_CHANNEL_VUFS12	I2C6
#define hl7593_SLAVE_ADDR_VUFS12	0x60
...
#define DECL_HL7593_CHIP(_id, _vsel)			\
{					\
	.id = MTK_REGULATOR_HL7593_##_id,		\
	.i2c_channel = hl7593_I2C_CHANNEL_##_id,	\
	.slave_addr = hl7593_SLAVE_ADDR_##_id,		\
	.buck_ctrl = MT6691_VSEL##_vsel,		\
	.mode_shift = _vsel,	\
	.en_ctrl = MT6691_VSEL##_vsel,			\
	.en_shift = 7,		\
}
...
static struct mt6691_chip mt6691_chip[] = {
	DECL_CHIP(VMDDR, 0),
	DECL_CHIP(VUFS12, 0),
#ifndef MTK_PLAT_MT6885_EMULATION
	DECL_HL7593_CHIP(VMDDR, 0),
	DECL_HL7593_CHIP(VUFS12, 0),
#endif
};
...
static int mt6691_check_id(void)
{
	...
	for (i = 0; i < num_chip; i++) {
		chip = &mt6691_chip[i];
		ret2 = mt6691_read_interface(chip, MT6691_MONITOR,
					     &PGOOD, 0x1, MT6691_PGOOD_SHIFT);
		if (ret2 || !PGOOD)
			continue;
		ret = 0;
		chip->is_hw_exist = true;
		if (i >= HL7593_CHIP_BASE)
			continue;
...
#endif
		/* slew rate 12mV */
		ret2 = mt6691_config_interface(chip, MT6691_CTRL2, 0x1,
...
#ifndef MTK_PLAT_MT6885_EMULATION
	if (mt6691_chip[HL7593_CHIP_BASE].is_hw_exist &&
	    mt6691_chip[HL7593_CHIP_BASE + 1].is_hw_exist) {
		pal_log_info("i2c 0x51/0x56 ack error is normal, probe i2c 0x61/0x60\n");
		ret = hl7593_probe(&mt6691_chip, num_chip);
		if (ret < 0) {
			pal_log_info("HL7593 regulator init fail\n");
			return ret;
		}
	} else if (mt6691_chip[0].is_hw_exist &&
		   mt6691_chip[1].is_hw_exist) {
		pal_log_info("i2c 0x61/0x60 ack error is normal, probe i2c 0x51/0x56\n");
		ret = mt6691_probe(&mt6691_chip, num_chip);
		if (ret < 0) {
			pal_log_info("MT6691 regulator init fail\n");
			return ret;
		}
	} else {
		pal_log_info("probe i2c 0x51+0x60 or 0x61+0x56\n");
		ret = hl7593_probe(&mt6691_chip, num_chip);
		ret |= mt6691_probe(&mt6691_chip, num_chip);
		if (ret < 0) {
			pal_log_info("HL7593 or MT6691 regulator init fail\n");
			return ret;
		}
	}
#else
