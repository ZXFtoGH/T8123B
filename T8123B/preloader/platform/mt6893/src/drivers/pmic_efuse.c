...
void pmic_efuse_sw_load(void)
{
	...
	/* bit 1975 for VSRAM_DIGRF(VA09) */
	efuse_data = pmic_read_efuse_nolock(123);
	if ((efuse_data & 0x80) != 0x80) {
		pmic_config_interface(PMIC_TMA_KEY_ADDR, 0x9CA6,
				      PMIC_TMA_KEY_MASK,
				      PMIC_TMA_KEY_SHIFT);
		pmic_config_interface(PMIC_RG_VA09_VOCAL_ADDR, 0,
				      PMIC_RG_VA09_VOCAL_MASK,
				      PMIC_RG_VA09_VOCAL_SHIFT);
