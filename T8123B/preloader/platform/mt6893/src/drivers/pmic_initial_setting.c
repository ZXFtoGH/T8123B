...
static void doe_load(void)
{
	...
	if (vproc1_rrate || vproc1_frate) {
		...
		mt6315_set_buck_slew_rate(SPMI_SLAVE_6, 2,
					  vproc1_rrate, vproc1_frate);
		mt6315_set_buck_slew_rate(SPMI_SLAVE_6, 3,
					  vproc1_rrate, vproc1_frate);
		mt6315_set_buck_slew_rate(SPMI_SLAVE_6, 4,
					  vproc1_rrate, vproc1_frate);
	}
	if (vproc1_rrate) {
	...
	if (vproc2_rrate || vproc2_frate) {
		mt6315_set_buck_slew_rate(SPMI_SLAVE_7, 3,
...
static void mt6359_code_check(void)
{
	...
	code = (code & 0xE000) >> 13;
	if (code != 0x1) {
		pal_log_info("MT6359 PF_CODE=%d is wrong for MT6893\n", code);
		ASSERT(0);
	}
}

/* PMIC warm/cold reset for MT6893 APU */
...
void pmic_init_setting(void)
{
	...
	pal_log_info("[PMIC] pmic_init_setting end. v200723\n");
