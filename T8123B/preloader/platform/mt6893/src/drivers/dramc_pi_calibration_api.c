...
#if ENABLE_RX_TRACKING_HW_SAVE_WA
    //Lynx: for Pexxxs/xxx868 only, hw_save rg: DQM0/DQS0/DQ3 WA
    vIO32WriteFldAlign_All(DDRPHY_REG_B0_RXDVS0,  0, B0_RXDVS0_R_HWSAVE_MODE_ENA_B0);
    vIO32WriteFldAlign_All(DDRPHY_REG_B1_RXDVS0,  0, B1_RXDVS0_R_HWSAVE_MODE_ENA_B1);
#endif
...
DRAM_STATUS_T CmdBusTrainingLP45(DRAMC_CTX_T *p, int autok)
{
...
#endif
	{
		...
		mcSHOW_DBG_MSG(("u1VRangeStart=%d, u1VRangeEnd=%d, u1VrefScanStart=%d, u1VrefScanEnd=%d\n",
...
DRAM_STATUS_T ZQ_SWCMD_MODE_Cal(DRAMC_CTX_T *p, U8 rank)
{
    ...
    DramCLKAlwaysOnOff(p, ON, CK_WRITE_TO_ONE_CHANNEL);
    mcDELAY_US(1);
...
DRAM_STATUS_T DramcRXInputBufferOffsetCal(DRAMC_CTX_T *p)
{
	...
	U32 u4RestltDQ, u4RestltDQM[DQM_BYTE_NUM];
	U32 u4RegBackupAddress[] =
	{
		(DRAMC_REG_ADDR(DDRPHY_REG_B0_DQ5)),
		(DRAMC_REG_ADDR(DDRPHY_REG_B1_DQ5)),
...
U8 u1GetGatingStartPos(DRAMC_CTX_T *p, U8 u1AutoK)
{
    ...
    mcDUMP_REG_MSG(("[DUMPLOG] %d GatingStartPos_MR0_LatencyMode %d, u1RealRL %d , u4TDQSCK_UI_min %d, 1:4ExtraMCK %d\n", p->frequency * 2, u1MR0_LatencyMode, u1RealRL, u4TDQSCK_UI_min, u1ExtraMCKfor1_4mode));
...
#endif
    {
        ...
        for (u2VrefLevel = u2VrefBegin; u2VrefLevel <= u2VrefEnd; u2VrefLevel += u2VrefStep)
        {
            ...
            if (isAutoK == TRUE)
            {
                ...
                DRAM_STATUS_T u4Response = DRAM_FAIL;
                U32 _begin = 0;
                u8 dbg_en = GetEyeScanEnable(p, 1)|ENABLE_RX_AUTOK_DEBUG_MODE;
                u8 dbg_sel = 0xFF;
...
//#endif
                ...
                mcSHOW_DBG_MSG3(("PASS(1)/FAIL(0) = %d\n", _autok_result));
            ...
            if(u2VrefLevel>60 && (u1min_winsize[0]>RX_PASS_WIN_CRITERIA && u1min_winsize[1]>RX_PASS_WIN_CRITERIA) && (u2TempWinSum[0] < (u2rx_window_sum[0]*95/100)) && (u2TempWinSum[1] < (u2rx_window_sum[1]*95/100))&& u1RXEyeScanEnable == 0)
            {
...
#if TX_K_DQM_WITH_WDBI
        //p->DBI_W_onoff[p->dram_fsp]?evaluates to an address that could be at negative offset of an array.
        //modify for coverity check fail
        if ((p->DBI_W_onoff[(U32)p->dram_fsp]!=DBI_ON))
        {
...
void DutyScan_Offset_Calibration(DRAMC_CTX_T *p, U8 *u1OfB0, U8 *u1OfB1, U8 *u1OfCA)
{
    ...
    mcSHOW_DBG_MSG(("   Offset\tB0\tB1\tCA\n"));
    mcSHOW_DBG_MSG(("====================================================\n"));
...
//fra    U8 *str_who_am_I=str_clk_duty;
    ...
    for(ucdqs_i=0; ucdqs_i<ucdqs_i_count; ucdqs_i++)
    {
        ...
        mcSHOW_DBG_MSG(("   DutyOffset\tMaxDuty\tMinDuty\tAvgDuty\tAvg-50%%\tMax-Min\n"));
        ...
        for(scinner_duty_ofst=scinner_duty_ofst_start; scinner_duty_ofst<=scinner_duty_ofst_end; scinner_duty_ofst+=scinner_duty_ofst_step)
        {
            ...
            mcSHOW_DBG_MSG(("   [%d]     ",scinner_duty_ofst));
            mcSHOW_DBG_MSG(("\t%d%%",ucperiod_duty_max));
            /*TINFO="\tMAX Duty = %d%%(X100), CLK PI=%d\n",ucperiod_duty_max, ucperiod_duty_max_clk_dly */
            mcSHOW_DBG_MSG(("\t%d%%",ucperiod_duty_min));
            /*TINFO="\tMIN Duty = %d%%(X100), CLK PI=%d\n",ucperiod_duty_min, ucperiod_duty_min_clk_dly */
            mcSHOW_DBG_MSG(("\t%d%%", ucperiod_duty_averige));
            /*TINFO="\tAVG Duty = %d%%(X100)\n", ucperiod_duty_averige */
            mcSHOW_DBG_MSG(("\t%d%%", (ucperiod_duty_averige-ClockDutyMiddleBound)*(ucperiod_duty_averige>=ClockDutyMiddleBound?1:-1)));
            /*TINFO="\tAVG Duty = %d%%(X100)\n", ucperiod_duty_averige */
            mcSHOW_DBG_MSG(("\t%d%%", (ucperiod_duty_max-ucperiod_duty_min)));
            /*TINFO="\tAVG Duty = %d%%(X100)\n", ucperiod_duty_averige */
            ...
            if (ucperiod_duty_averige >= ClockDutyMiddleBound)
            {
                     ...
                     (scinner_duty_ofst>0 && ((ucperiod_duty_averige-ClockDutyMiddleBound+(ucperiod_duty_max-ucperiod_duty_min)/2) < ucmost_approach_50_percent)))
                {
                    ...
                    mcSHOW_DBG_MSG(("\t ==> ucmost_approach_50_percent = %d%%(X100) !!!",ucmost_approach_50_percent));
...
void DramcNewDutyCalibration(DRAMC_CTX_T *p)
{
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (DDRPHY_REG_SHU_R0_CA_CMD0 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B0_DQ2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_B1_DQ2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_CA_CMD2 + SHIFT_TO_CHB_ADDR),
        (DDRPHY_REG_SHU_CA_CMD13 + SHIFT_TO_CHB_ADDR),
        ...
        (DDRPHY_REG_SHU_R0_CA_CMD0 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B0_DQ2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_B1_DQ2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_CA_CMD2 + SHIFT_TO_CHC_ADDR),
        (DDRPHY_REG_SHU_CA_CMD13 + SHIFT_TO_CHC_ADDR),
        ...
        (DDRPHY_REG_SHU_R0_CA_CMD0 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B0_DQ2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_B1_DQ2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_CA_CMD2 + SHIFT_TO_CHD_ADDR),
        (DDRPHY_REG_SHU_CA_CMD13 + SHIFT_TO_CHD_ADDR),
...
#endif
    {
        ...
        #endif
        {
            ...
            for(u1ChannelIdx=CHANNEL_A; u1ChannelIdx<p->support_channel_num; u1ChannelIdx++)
            {
                ...
                //restore to orignal value
                DramcRestoreRegisters(p, u4RegBackupAddress, sizeof(u4RegBackupAddress)/sizeof(U32));
                for (tmp = 0; tmp < ARRAY_SIZE(u4RegBackupAddress); tmp++)
                    u4RegBackupAddress[tmp] += (1 << POS_BANK_NUM); /* SHIFT for Next CHANNEL */
                #endif
