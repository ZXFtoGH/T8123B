...
void Emi_Sleep_Protect(DRAMC_CTX_T *p, unsigned int enable) {
    ...
    if (enable == ENABLE) {
        mcSHOW_DBG_MSG(("Emi_Sleep_Protect Enable\n"));

        vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_2, 0x1, LPIF_LOW_POWER_CFG_2_EMI_SLEEP_PROT_EN);
        ...
        if (timeout == 0) {
            mcSHOW_DBG_MSG(("Emi_Sleep_Protect timeout\n"));
        }    
    
    } else {
        mcSHOW_DBG_MSG(("Emi_Sleep_Protect Disable\n"));
        vIO32WriteFldAlign_All(DDRPHY_MD32_REG_LPIF_LOW_POWER_CFG_2, 0x0, LPIF_LOW_POWER_CFG_2_EMI_SLEEP_PROT_EN);
    }
}
