...
DRAM_DFS_FREQUENCY_TABLE_T gFreqTbl[DRAM_DFS_SHUFFLE_MAX] = {
    {LP4_DDR3200 /*0*/, DIV8_MODE, SRAM_SHU1, DUTY_LAST_K,  VREF_CALI_OFF,  CLOSE_LOOP_MODE},  // highest freq of term group (4266) must k first.
    {LP4_DDR3733 /*1*/, DIV8_MODE, SRAM_SHU0, DUTY_LAST_K,  VREF_CALI_OFF,  CLOSE_LOOP_MODE},  // highest freq of term group (4266) must k first.
    {LP4_DDR4266 /*2*/, DIV8_MODE, SRAM_SHU7, DUTY_NEED_K,  VREF_CALI_ON,   CLOSE_LOOP_MODE},  // highest freq of term group (4266) must k first.
    {LP4_DDR800  /*3*/, DIV4_MODE, SRAM_SHU6, DUTY_DEFAULT, VREF_CALI_OFF,  SEMI_OPEN_LOOP_MODE},  //Darren: DDR1600 for MRW (DramcModeRegInit_LP4 and CBT)
    {LP4_DDR1866 /*4*/, DIV8_MODE, SRAM_SHU3, DUTY_LAST_K,  VREF_CALI_OFF,  CLOSE_LOOP_MODE},  // highest freq of unterm group (2400) must k first.
    {LP4_DDR1200 /*5*/, DIV8_MODE, SRAM_SHU5, DUTY_LAST_K,  VREF_CALI_OFF,  CLOSE_LOOP_MODE},  // highest freq of unterm group (2400) must k first.
    {LP4_DDR2400 /*6*/, DIV8_MODE, SRAM_SHU2, DUTY_NEED_K,  VREF_CALI_ON,   CLOSE_LOOP_MODE},  // highest freq of unterm group (2400) must k first.
    {LP4_DDR1600 /*7*/, DIV8_MODE, SRAM_SHU4, DUTY_DEFAULT, VREF_CALI_ON,  CLOSE_LOOP_MODE},  //Darren: DDR1600 for MRW (DramcModeRegInit_LP4 and CBT)
};
...
void vSetVcoreByFreq(DRAMC_CTX_T *p)
{
	...
	case SRAM_SHU7:  //4266
	#ifdef VOLTAGE_SEL
		vcore = vcore_voltage_select(KSHU7, efuse);
		if (!vcore)
	#endif
	...
	case SRAM_SHU1:   //3200
	#ifdef VOLTAGE_SEL
		vcore = vcore_voltage_select(KSHU1, efuse);
		if (!vcore)
	#endif
		vcore = (get_vcore_uv_table(1) + get_vcore_uv_table(2)) >> 1;
		break;
	case SRAM_SHU2:  //2400
	#ifdef VOLTAGE_SEL
		vcore = vcore_voltage_select(KSHU2, efuse);
		if (!vcore)
	#endif
	...
	case SRAM_SHU6:  //800
	#ifdef VOLTAGE_SEL
		vcore = vcore_voltage_select(KSHU3, efuse);
		if (!vcore)
	#endif
		vcore = (get_vcore_uv_table(1) + get_vcore_uv_table(4)) >> 1;
		break;
	}
#else
	switch (vGet_Current_ShuLevel(p)) {
	...
	#else
		if (efuse == 1)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU7, 25);
		else if (efuse == 2)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU7, 50);
		else
			vcore = SEL_PREFIX_VCORE(LP4, KSHU7, 0);
	#endif
		break;
	case SRAM_SHU0: // 3733
	#ifdef VOLTAGE_SEL
	...
	#else
		if (efuse == 1)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU0, 25);
		else if (efuse == 2)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU0, 50);
		else
			vcore = SEL_PREFIX_VCORE(LP4, KSHU0, 0);
	#endif
		break;
	case SRAM_SHU1: // 3200
	#ifdef VOLTAGE_SEL
	...
	#else
		if (efuse == 1)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU1, 25);
		else if (efuse == 2)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU1, 50);
		else
			vcore = SEL_PREFIX_VCORE(LP4, KSHU1, 0);
	#endif
		break;
	case SRAM_SHU2: // 2400
	#ifdef VOLTAGE_SEL
	...
	#else
		if (efuse == 1)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU2, 25);
		else if (efuse == 2)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU2, 50);
		else
			vcore = SEL_PREFIX_VCORE(LP4, KSHU2, 0);
	#endif
		break;
	case SRAM_SHU3: //1866
	case SRAM_SHU4: //1600
	...
	#else
		if (efuse == 1)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU3, 25);
		else if (efuse == 2)
			vcore = SEL_PREFIX_VCORE(LP4, KSHU3, 50);
		else
			vcore = SEL_PREFIX_VCORE(LP4, KSHU3, 0);
	#endif
		break;
	default:
		return;
...
void PRE_MIOCK_JMETER_HQA_USED(DRAMC_CTX_T *p)
{
    ...
    if(p->femmc_Ready==0)
    {
        ...
        for(channel_idx=CHANNEL_A; channel_idx<p->support_channel_num; channel_idx++)
        {
            for (shuffleIdx = DRAM_DFS_SHUFFLE_1; shuffleIdx < DRAM_DFS_SHUFFLE_MAX; shuffleIdx++)
            {
                p->pSavetimeData->u2g_num_dlycell_perT_all[channel_idx] = u2g_num_dlycell_perT_all[p->shu_type][channel_idx];
                p->pSavetimeData->u2gdelay_cell_ps_all[channel_idx] = u2gdelay_cell_ps_all[p->shu_type][channel_idx];
            }
        }
        #endif
        p->pSavetimeData->u2num_dlycell_perT = p->u2num_dlycell_perT;
...
{
    U8 u1RankBak;
    DRAM_STATUS_T ret_value=DRAM_OK;
    U8 channel_i;
...
#if __FLASH_TOOL_DA__ && PIN_CHECK_TOOL
    for(channel_i=CHANNEL_A; channel_i<p->support_channel_num; channel_i++)
    {
        vSetPHY2ChannelMapping(p, channel_i);

        ret_value = DramcWriteLeveling(p, AUTOK_ON, PI_BASED);

        if (ret_value == DRAM_OK) break; //early break;
    }
    vSetPHY2ChannelMapping(p, CHANNEL_A);
#else
    ret_value = DramcWriteLeveling(p, AUTOK_ON, PI_BASED);
...
#if ENABLE_EYESCAN_GRAPH
        if (GetEyeScanEnable(p, 1) == ENABLE)
        {
...
int Init_DRAM(DRAM_DRAM_TYPE_T dram_type, DRAM_CBT_MODE_EXTERN_T dram_cbt_mode_extern, DRAM_INFO_BY_MRR_T *DramInfo, U8 get_mdl_used)
{
    ...
    vSetDFSFreqSelByTable(p, &gFreqTbl[1]); //0:3200 1:3733, 2:800, 3:1866, 4:1200, 5:2400, 6:1600
#else
...
void DPI_vDramCalibrationSingleChannel(DRAMC_CTX_T *DramConfig, cal_sv_rand_args_t *psra)
{
...
#endif
    {
    	...
    	if (!psra || psra->rx_perbit) {
            ...
            #if ENABLE_EYESCAN_GRAPH
                if (GetEyeScanEnable(DramConfig, 1) == ENABLE)
                {
...
void DPI_SW_main_LP4(DRAMC_CTX_T *ExtConfig, cal_sv_rand_args_t *psra)
{
    ...
    if (u1IsLP4Family(p->dram_type))
    {
        if(ExtConfig->freq_sel==LP4_DDR3733)
        {
            p->pDFSTable = &gFreqTbl[DRAM_DFS_SHUFFLE_1];
        ...
        {
            p->pDFSTable = &gFreqTbl[DRAM_DFS_SHUFFLE_3];
...
void sa_calibration(cal_sv_rand_args_t *psra)
{
	...
	enter_function();
	if (psra == NULL) {
		mcSHOW_DBG_MSG(("psv_args is NULL.\n"));
		goto out;
	}

	print_sv_args(psra);
	if (!valid_magic(psra)) {
		mcSHOW_DBG_MSG(("sv arguments is invalid.\n"));
		goto out;
	}
