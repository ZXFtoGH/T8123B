...
void print_HQA_measure_message(DRAMC_CTX_T *p)
{
    ...
    if (gHQALog_flag == 1)
    {
        ...
        //p->dram_cbt_mode[p->rank]?evaluates to an address that could be at negative offset of an array.
        //modify for coverity check fail
        HQA_LOG_Print_Prefix_String(p); mcSHOW_DBG_MSG(("byte_mode = %d\n", p->dram_cbt_mode[(U32)p->rank]));
...
if (gHQALog_flag == 1)
{
    ...
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT6, "Impedance freq_region=1 ", "DRVP", 0, gDramcSwImpedanceResult[1][DRVP], NULL);
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT6, "Impedance freq_region=1 ", "DRVN", 0, gDramcSwImpedanceResult[1][DRVN], NULL);
    HQA_Log_Message_for_Report(p, p->channel, p->rank, HQA_REPORT_FORMAT6, "Impedance freq_region=1 ", "ODTN", 0, gDramcSwImpedanceResult[1][ODTN], NULL);
...
#endif
{
    ...
    */
    ...
    if (gHQALog_flag == 1)
    {
        ...
        {
            gHQALog_SLT_BIN[vGet_Current_ShuLevel(p)] = 2; //SLT_BIN2
        }
    }
#endif
    /*
...
if (gHQALog_flag == 1)
{
        for (u1ChannelIdx = CHANNEL_A; u1ChannelIdx < local_channel_num; u1ChannelIdx++)
        {
            for (u1RankIdx = RANK_0; u1RankIdx < 2; u1RankIdx++)
            {
                ...
                {
                    gHQALog_SLT_BIN[vGet_Current_ShuLevel(p)] = 4; //SLT_BIN4
                }
            }
        }
}
        ...
        */
        ...
        for (u1ChannelIdx = CHANNEL_A; u1ChannelIdx < local_channel_num; u1ChannelIdx++)
        {
            ...
            if (gHQALog_flag == 1)
            {
                ...
                {
                    gHQALog_SLT_BIN[vGet_Current_ShuLevel(p)] = 3; //SLT_BIN3
                }
            }
            else
#endif
        ...
        else
        {
            ...
            if (i == (int)(Max_EyeScan_Min_val) || i == (int)(Max_EyeScan_Min_val + PI_of_1_UI))
            {
                if (gEye_Scan_color_flag)
                {
                    //p->dram_cbt_mode[p->rank]?evaluates to an address that could be at negative offset of an array.
                    //modify for coverity check fail
                    if (p->dram_cbt_mode[(U32) p->rank] == CBT_BYTE_MODE1 && p->dram_fsp == FSP_0 && draw_type==0 && u1VrefIdx < EYESCAN_SKIP_UNTERM_CBT_EYESCAN_VREF)
                    {
...
void vGetErrorTypeResult(DRAMC_CTX_T *p)
{
    ...
    else
    {
        for (u1CHIdx = 0; u1CHIdx < p->support_channel_num; u1CHIdx++)
        {
            ...
            if ((PINInfo_flashtool.TOTAL_ERR>>(u1CHIdx*4) & 0xf) == (p->support_rank_num==RANK_DUAL?0xf:0x3))
            {
                ...
                {
                    mcSHOW_DBG_MSG(("Suspect EMI%d_CKE_ contact issue\n",u1CHIdx_EMI));
                    mcSHOW_DBG_MSG(("Suspect EMI%d_CS_ contact issue\n",u1CHIdx_EMI));
                }
            }
            else
            {
...
void DumpAoShuNoRkReg(DRAMC_CTX_T *p)
{
    ...
    //RO
    DramcRegDumpRange(p, DDRPHY_REG_MISC_STBERR_ALL, DDRPHY_REG_MISC_DDR_RESERVE_STATE);
...
void DumpAoShuRkReg(DRAMC_CTX_T *p)
{
    ...
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_B0_TXDLY0, DDRPHY_REG_SHU_B0_PHY_VREF_SEL);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_B1_TXDLY0, DDRPHY_REG_SHU_B1_PHY_VREF_SEL);
    DramcRegDumpRange(p, DDRPHY_REG_SHU_R0_CA_TXDLY0, DDRPHY_REG_SHU_CA_PHY_VREF_SEL);
...
void DumpAllRkRG(DRAMC_CTX_T *p, U8 u1ShuType)
{
    ...
    {
        mcDUMP_REG_MSG(("\n[DUMPLOG] Channel=%d, Rank=NON, SHU=SRAM%d, FREQ=%d, AORG, start \n", vGetPHY2ChannelMapping(p), vGet_Current_ShuLevel(p), GetFreqBySel(p, vGet_PLL_FreqSel(p))));
        DumpAoShuNoRkReg(p);
    ...
    for (u1RankIdx=RANK_0; u1RankIdx<p->support_rank_num; u1RankIdx++)
    {
        ...
        if (u1ShuType == SHUFFLE_RG||u1ShuType == BOTH_SHU_NONSHU_RG)
        {
            mcDUMP_REG_MSG(("\n[DUMPLOG] Channel=%d, Rank=%d, SHU=SRAM%d, FREQ=%d, AORG, start \n", vGetPHY2ChannelMapping(p), u1GetRank(p), vGet_Current_ShuLevel(p), GetFreqBySel(p, vGet_PLL_FreqSel(p))));
            DumpAoShuRkReg(p);
...
void NoDramDramcRegDumpRange(U32 u4StartAddr, U32 u4EndAddr)
{
    ...
    for (ii = u4StartAddr; ii < u4EndAddr; ii += 4)
    {
        mcDUMP_REG_MSG(("*(volatile unsigned int *)0x%X = 0x%X;\n", ii, (*(volatile unsigned int *)(ii))));
...
void vDramCPUReadWriteTestAfterCalibration(DRAMC_CTX_T *p)
{
	...
	if (u1RankIdx >= 1)
        ...
        mcSHOW_DBG_MSG(("(uiFixedAddr 0x%X, Pass count =%d, Fail count =%d)", uiFixedAddr, pass_count, err_count));
#endif
        mcSHOW_ERR_MSG(("\n"));
    }
