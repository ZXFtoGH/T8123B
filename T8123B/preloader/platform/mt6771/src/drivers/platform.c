...
static void bc_init(void)
{
    ...
    val1 = (readl(DEVINFO_BASE + 0x44) >> 20) & 0x3;
    val2 = readl(DEVINFO_BASE + 0x4C) & 0x1;
    val3 = readl(DEVINFO_BASE + 0x50) & 0xFF;
    val4 = readl(DEVINFO_BASE + 0x700) & 0x1;

    if((val1 == 0x1) && (val2 == 0x1) && (val3 == 0x4)) {
        if (val4 == 0x1)
            print("l\n");
        else {
            print("[bc]bc function on.\n");
            val1 = (readl(DDRPHY_BASE + 0xe1c) >> 8) & 0x3F;
            if (val1 > 32)
                val1 -= 16;
            else
                val1 += 16;
            writel((readl(DDRPHY_BASE + 0xe1c) & ~0x3F00) | (val1 << 8), DDRPHY_BASE + 0xe1c);
        }
    }
    else {
        print("l\n");
