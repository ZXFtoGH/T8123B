...
int cw2015_enable = 0;
int g_cw2015_capacity = 10;
int g_cw2015_vol = 3555;
int g_cw2015_current;
EXPORT_SYMBOL(cw2015_enable);
EXPORT_SYMBOL(g_cw2015_capacity);
EXPORT_SYMBOL(g_cw2015_vol);
EXPORT_SYMBOL(g_cw2015_current);
...
//		rc = smblib_get_prop_batt_capacity(chg, pval);
		if(cw2015_enable == 0){
			...
			pval->intval = g_cw2015_capacity;
			pr_err("yky add use cw2015 CAPACITY = %d\n",pval->intval);
		}
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if(cw2015_enable == 0){
			...
			pval->intval = g_cw2015_vol;
			pr_err("xyybat usb cw2015 VOLTAGE_NOW = %d\n",pval->intval);
		}
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	chip = iio_priv(indio_dev);	// 获取私有数据空间
	chip->nchannels = ARRAY_SIZE(smb5_chans_pmic);

	//用于存储ADC采样通道信息（电压、电流、温度等）
	...
	chg->dev = &pdev->dev;		// 设备指针
	chg->debug_mask = &__debug_mask;
	chg->pd_disabled = 0;	// 默认不关闭PD
	chg->weak_chg_icl_ua = 500000;	// 默认弱充电流500mA
	chg->mode = PARALLEL_MASTER;	// 并联充电主模式
	chg->irq_info = smb5_irqs;
	chg->die_health = -EINVAL;	
	chg->connector_health = -EINVAL;
	chg->otg_present = false;
	chg->main_fcc_max = -EINVAL;
	mutex_init(&chg->adc_lock);		//ADC访问互斥锁

	//获取寄存器映射（Regmap） 用于读写PMIC硬件寄存器
	...
	rc = smb5_iio_init(chip, pdev, indio_dev);	//IIO子系统初始化  注册IIO设备，提供ADC读取接口
	if (rc < 0)
		return rc;

	rc = smb5_chg_config_init(chip);	// 配置充电参数初始化
	if (rc < 0) {
		if (rc != -EPROBE_DEFER)
	...
	rc = smb5_parse_dt(chip);	// 解析设备树
	if (rc < 0) {
		pr_err("Couldn't parse device tree rc=%d\n", rc);
		return rc;
	}

	if (alarmtimer_get_rtcdev())		//用于低功耗检测（LPD）重检定时器
