...
//读取硬件检测结果，判断插入的是 SDP (普通USB)、DCP (普通充电器)、CDP 还是高通的 QC 充电器
...
/*
给插入的充电器“验明正身”：判断当前插入的到底是 PD 快充、QC3.5 快充，还是普通的 USB 充电器，
并将最终确定的类型更新到系统变量 chg->real_charger_type 中
*/
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	逻辑：如果 USB PD（Power Delivery）协议已经激活，直接认定为 PD 充电器。
	原因：注释中解释了，一旦 PD 协议激活，APSD 硬件检测机制就会被禁用（因为 PD 是通过 CC 引脚通信的，不走 D+/D- 检测），所以此时硬件返回的 APSD 结果是无效的，必须以软件层面的 PD 状态为准。*/
	if (chg->pd_active) {	//优先级 1：USB PD 协议 (最高优先级)
		chg->real_charger_type = POWER_SUPPLY_TYPE_USB_PD;
	} else if (chg->qc3p5_detected) {	//优先级 2：QC 3.5 快充
		chg->real_charger_type = QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5;
	} else {	//优先级 3：常规 APSD 结果 (带防误判机制)
		/*
		 * Update real charger type only if its not FLOAT
		 * detected as SDP
		 * 逻辑：如果既不是 PD 也不是 QC3.5，就采用硬件 APSD 的检测结果（比如 SDP、DCP、CDP 等）。
			防误判机制（重点）：这里做了一个特殊的拦截判断。如果硬件报告的是 FLOAT（悬浮态），且当前系统已经识别为普通 USB，那么拒绝更新。
			原因：在实际使用中，如果用户插入充电器时动作太慢（慢插入），或者数据线 D+/D- 焊接有轻微接触不良，硬件很容易瞬间误判为 FLOAT。为了防止充电状态在“USB”和“FLOAT”之间反复横跳，驱动在这里加了一个“防抖/过滤”逻辑。
		 */
	...
	printk("zxf %s line = %d APSD=%s PD=%d QC3P5=%d\n", __func__, __LINE__, apsd_result->name, chg->pd_active, chg->qc3p5_detected);
	return apsd_result;
}
